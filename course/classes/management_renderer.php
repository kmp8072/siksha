...
class core_course_management_renderer extends plugin_renderer_base {
    ...
    public function search_listitem_actions(course_in_list $course) {
        ...
        if ($course->can_access()) {
            ...
            if ($course->can_change_visibility()) {
                ...
                if (!empty($CFG->audiencevisibility)) {
                    $hidetooltip = $showtooltip = get_string('manageaudincevisibility', 'shezar_cohort');
                }
                $actions[] = $this->output->action_icon(
    ...
    public function management_buttons($categoryid) {
        ...
        if (shezar_feature_visible('programs')) {
            ...
            $url = new moodle_url('/shezar/program/manage.php', array('categoryid' => $categoryid));
            $programcaps = array('shezar/program:createprogram', 'shezar/program:deleteprogram', 'shezar/program:configuredetails');
        ...
        if (shezar_feature_visible('certifications')) {
            ...
            $url = new moodle_url('/shezar/program/manage.php', array('categoryid' => $categoryid, 'viewtype' => 'certification'));
