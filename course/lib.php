...
//Course types
define('shezar_COURSE_TYPE_ELEARNING', 0);
define('shezar_COURSE_TYPE_BLENDED', 1);
define('shezar_COURSE_TYPE_FACETOFACE', 2);

global $shezar_COURSE_TYPES;
$shezar_COURSE_TYPES = array(
    'elearning' => shezar_COURSE_TYPE_ELEARNING,
    'blended' => shezar_COURSE_TYPE_BLENDED,
    'facetoface' => shezar_COURSE_TYPE_FACETOFACE,
);
...
function make_log_url($module, $url) {
    switch ($module) {
        ...
        case 'program':
        case 'plan':
            $url = '/shezar/' . $module . '/' . $url;
            break;
        case 'position':
        case 'organisation':
        case 'competency':
        case 'goal':
            $url = '/shezar/hierarchy/' . $url;
            break;
        case 'priorityscales':
        case 'objectivescales':
            $url = '/shezar/plan/' . $module . '/' . $url;
            break;
        case 'my':
            // shezar: we need to keep this even after My learning removal, this is for data in database.
            $url = '/my/' . $url;
            break;
        // End shezar specific modules
        case 'grade':
            $url = "/grade/$url";
            break;
        case 'rbembedded':
...
function can_edit_in_category($categoryid = 0) {
    ...
    return has_any_capability(array('moodle/category:manage', 'moodle/course:create',
                                    'moodle/course:update', 'shezar/program:createprogram', 'shezar/program:configuredetails',
                                    'shezar/certification:createcertification', 'shezar/certification:configurecertification'),
                              $context);
}
...
function update_course($data, $editoroptions = NULL) {
    ...
    // shezar: Update the custom fields.
    customfield_save_data($data, 'course', 'course');

    // shezar performance improvement - invalidate static caching of course information.
    global $CFG;
