...
if ($form->is_cancelled()) {
    ...
    if (completion_can_unlock_data($course->id)) {
        ...
        if ($unlockdelete) {
            ...
            \shezar_core\event\course_completion_reset::create_from_course($course)->trigger();
            $completion->delete_course_completion_data();
        } else if ($unlockonly) {
            // The "Unlock without deleting" button was clicked, so just log it and continue.
            \shezar_core\event\course_completion_unlocked::create_from_course($course)->trigger();
        }
...
/// process data if submitted
    ...
    shezar_set_notification(get_string('completioncriteriachanged', 'core_completion'), $url, array('class' => 'notifysuccess'));
}
