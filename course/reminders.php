...
if ($courseid) {
    if($courseid == SITEID){
        ...
        print_error('noeditsite', 'shezar_coursecatalog');
    } else if (!$course = $DB->get_record('course', array('id' => $courseid))) {
        print_error('error:courseidincorrect', 'shezar_core');
    }
    ...
    require_login();
    print_error('error:courseidorcategory', 'shezar_coursecatalog');
}
...
if ($delete) {
    ...
    \shezar_core\event\reminder_deleted::create_from_reminder($reminder)->trigger();

    $PAGE->set_title(get_string('editcoursereminders', 'shezar_coursecatalog'));
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('deletedreminder', 'shezar_coursecatalog', format_string($reminder->title)));
...
if ($reminderform->is_cancelled()){
    ...
    if (empty($reminder->id)) {
        if (!$reminder->insert()) {
            print_error('error:createreminder', 'shezar_coursecatalog');
        }
        \shezar_core\event\reminder_created::create_from_reminder($reminder)->trigger();
    } else {
        if (!$reminder->update()) {
            print_error('error:updatereminder', 'shezar_coursecatalog');
        }
        \shezar_core\event\reminder_updated::create_from_reminder($reminder)->trigger();
    }
    ...
    foreach (array('invitation', 'reminder', 'escalation') as $mtype) {
        ...
        if ($mtype === 'escalation') {
            if (!empty($data->$nosend)) {
                ...
                if ($message->id) {
                    ...
                    if (!$message->update()) {
                        print_error('error:deletereminder', 'shezar_coursecatalog');
                    }
                }
        ...
        if (empty($message->id)) {
            if (!$message->insert()) {
                print_error('errro:createreminder', 'shezar_coursecatalog');
            }
        } else {
            if (!$message->update()) {
                print_error('error:updatereminder', 'shezar_coursecatalog');
...
if ($reminder->id > 0) {
    ...
    $buttonhtml = $OUTPUT->single_button(
        ...
        get_string('deletereminder', 'shezar_coursecatalog', format_string($reminder->title)),
        'get'
    );
}

$streditcoursereminders = get_string('editcoursereminders', 'shezar_coursecatalog');
...
$tabs[] = new tabobject('new', $CFG->wwwroot.'/course/reminders.php?courseid='.$course->id.'&id=-1', get_string('new', 'shezar_coursecatalog'));
...
if (!$completion->is_enabled()) {
    echo $OUTPUT->box(get_string('noactivitieswithcompletionenabled', 'shezar_coursecatalog'), 'generalbox adminerror boxwidthwide boxaligncenter');
    echo $OUTPUT->continue_button(new moodle_url('/course/view.php', array('id' => $course->id)));
} else if (!get_coursemodules_in_course('feedback', $course->id)) {
    echo $OUTPUT->notification(get_string('nofeedbackactivities', 'shezar_coursecatalog'), 'notifynotice');
    echo $OUTPUT->continue_button(new moodle_url('/course/view.php', array('id' => $course->id)));
} else {
