...
require_capability('shezar/hierarchy:viewcompetency', $context);
...
$PAGE->requires->string_for_js('assigncoursecompletiontocompetencies', 'shezar_hierarchy');
$jargs = '{"id":'.$course->id;
...
$jsmodule = array(
        'name' => 'shezar_coursecompetency',
        'fullpath' => '/shezar/core/js/course.competency.js',
        'requires' => array('json'));
$PAGE->requires->js_init_call('M.shezar_coursecompetency.init', $args, false, $jsmodule);

$strcompetenciesusedincourse = get_string("competenciesusedincourse", 'shezar_hierarchy');
...
<div class="singlebutton centerbutton">
    <form action="<?php echo $CFG->wwwroot ?>/shezar/hierarchy/prefix/competency/course/add.php?id=<?php echo $id ?>" method="get">
        <div>
            <?php if (!empty($CFG->competencyuseresourcelevelevidence)) { ?>
                <input type="submit" id="show-coursecompetency-dialog" value="<?php echo get_string('addcourseevidencetocompetencies', 'shezar_hierarchy'); ?>" />
            <?php } else { ?>
                <input type="submit" id="show-coursecompetency-dialog" value="<?php echo get_string('assigncoursecompletiontocompetencies', 'shezar_hierarchy'); ?>" />
            <?php } ?>
...
echo $OUTPUT->single_button(
    ...
    $options),
    get_string('returntocourse', 'shezar_core'),
    'get'
);
