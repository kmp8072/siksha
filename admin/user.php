<?php
    ...
    $strundelete = get_string('undelete', 'shezar_core');
    ...
    if (has_capability('shezar/core:seedeletedusers', $sitecontext)) {
    ...
    if ($confirmuser and confirm_sesskey()) {
        ...
        if ($confirm != md5($delete)) {
            ...
            echo $OUTPUT->confirm(get_string('deleteusercheckfull', 'shezar_core', "'$fullname'"), $deletebutton, $returnurl);
            echo $OUTPUT->footer();
            die;
        ...
        // shezar - allow full delete of partially deleted users.
        else if (data_submitted() and $user->deleted) {
        ...
        if (!has_capability('shezar/core:undeleteuser', $sitecontext)) {
            print_error('undeleteusernoperm', 'shezar_core');
        }
        if (!$user = $DB->get_record('user', array('id' => $undelete))) {
            print_error('userdoesnotexist', 'shezar_core');
        }
        if (preg_match($preg_emailhash, $user->email)) {
            // ensure we're not trying to undelete a legacy-deleted (hash in email) user
            print_error('cannotundeleteuser', 'shezar_core');
        }

        if ($confirm != md5($undelete)) {
            echo $OUTPUT->header();
            $fullname = fullname($user, true);
            echo $OUTPUT->heading(get_string('undeleteuser', 'shezar_core'));
            $optionsyes = array('undelete' => $undelete, 'confirm' => md5($undelete), 'sesskey' => sesskey());
            echo $OUTPUT->confirm(get_string('undeletecheckfull', 'shezar_core', "'$fullname'"), new moodle_url($returnurl, $optionsyes), $returnurl);
            echo $OUTPUT->footer();
            die;
        } else if (data_submitted() && $user->deleted) {
            if (undelete_user($user)) {
                shezar_set_notification(get_string('undeletedx', 'shezar_core', fullname($user, true)), $returnurl, array('class' => 'notifysuccess'));
            } else {
                shezar_set_notification(get_string('undeletednotx', 'shezar_core', fullname($user, true)), $returnurl);
            }
        }
