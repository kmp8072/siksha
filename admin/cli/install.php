...
$help =
"Command line shezar installer, creates config.php and initializes database.
...
--wwwroot=URL         Web address for the shezar site,
...
--dbname=NAME         Database name. Default is shezardb
...
--adminpass=PASSWORD  Password for the admin account,
                      required in non-interactive mode.
--adminemail=STRING   Email address for the shezar admin account.
...
if (version_compare(phpversion(), "5.5.9") < 0) {
    ...
    fwrite(STDERR, "shezar 9.0 or later requires at least PHP 5.5.9 (currently using version $phpversion).\n");
    fwrite(STDERR, "Please upgrade your server software or install older shezar version.\n");
    exit(1);
}
...
$databases = array('mysqli' => moodle_database::get_driver_instance('mysqli', 'native'),
                   ...
                   // 'oci'    => moodle_database::get_driver_instance('oci',    'native'), // shezar: no Oracle!
...
if (empty($databases)) {
    ...
    // shezar: make pgsql default if available, it is considered to be the best option.
...
cli_logo();
echo PHP_EOL;
echo get_string('cliinstallheader', 'install', $shezar->release)."\n";
...
if ($interactive) {
    if (!$options['agree-license']) {
        ...
        echo "shezar LMS - shezar Learning Management System\n\n";
