...
if (!empty($shezarinfo->shezarupgradeerror)){
    cli_error(get_string($shezarinfo->shezarupgradeerror, 'shezar_core', $shezarinfo), 1);
}

// shezar: moodle_needs_upgrading() now checks for shezar upgrade too.
if (!moodle_needs_upgrading()) {
    cli_error(get_string('cliupgradenoneed', 'core_admin', $shezarinfo->newversion), 0);
}
...
if ($interactive) {
    echo cli_heading(get_string('databasechecking', '', $shezarinfo)) . PHP_EOL;
}
...
if ($interactive) {
    echo html_to_text(get_string('upgradesure', 'admin', $shezarinfo))."\n";
...
if ($shezarinfo->upgradecore) {
    // shezar: this is executed when Moodle version or shezar releases changed.
