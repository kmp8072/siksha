 ...
 *
 * @author Moises Burgos <moises.burgos@shezarlms.com>
 * @package shezar
 * @subpackage admin_user
 */

/**
 * Script for bulk enabling/disabling shezar sync checkbox for users
 */
...
if ($confirm) {
    ...
    while ($users = shezar_pop_n($SESSION->bulk_users, 1000)) {
        ...
        if (!empty($users)) {
            ...
            foreach ($rs as $user) {
                if (!$DB->set_field('user', 'shezarsync', $enable, array('id' => $user->id))) {
                    $errornotifications .= $OUTPUT->notification(get_string('toggleshezarsyncerror', 'shezar_core', fullname($user, true)), 'notifynotice');
                    $errorcounter++;
                } else {
                    $usercount++;
                }
    ...
    if(!empty($badusernotification)) {
        echo $OUTPUT->notification(get_string('toggleshezarsyncerror:noidnumber', 'shezar_core', implode(',', $badusernotification)), 'notifynotice');
    }
    ...
    while ($users = shezar_pop_n($bulkusers, 1000)) {
    ...
    $formcontinue = $OUTPUT->single_button(new moodle_url('user_bulk_toggleshezarsync.php', array('confirm' => 1)), get_string('continue'));
    $formcancel = $OUTPUT->single_button(new moodle_url('user_bulk.php'), get_string('cancel'), 'get');

    echo $OUTPUT->box_start('generalbox', 'notice');
    echo html_writer::tag('p', get_string('enabledisableshezarsync', 'shezar_core', $usernames));
