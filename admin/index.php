...
if (version_compare(phpversion(), '5.5.9') < 0) {
    ...
    echo "shezar 9.0 or later requires at least PHP 5.5.9 (currently using version $phpversion).<br />";
    echo "Please upgrade your server software or install older shezar version.";
    die();
}
...
if (!function_exists('iconv')) {
    ...
    echo 'shezar requires the iconv PHP extension. Please install or enable the iconv extension.';
    die();
}
...
if (!function_exists('json_encode') || !function_exists('json_decode')) {
    ...
    echo 'shezar requires the json PHP extension. Please install or enable the json extension.';
    die();
}
...
$release = null;
$branch = null;
$shezar = new stdClass(); // Prevent IDE complaints.
...
// Setup shezar version variables and verify upgrade is possible,
// note that lib/setup.php does upgrade checks for 1.x/2.2.x upgrade path.
$shezarinfo = shezar_version_info();
if (!empty($shezarinfo->shezarupgradeerror)){
    $PAGE->set_pagelayout('maintenance');
    $PAGE->set_popup_notification_allowed(false);
    print_error($shezarinfo->shezarupgradeerror, 'shezar_core', new moodle_url('/admin/'), $shezarinfo);
}
...
if (!core_tables_exist()) {
    ...
    if (empty($agreelicense)) {
        ...
        $PAGE->set_title($strinstallation.' - shezar '.$shezar->release);
    ...
    if (empty($confirmrelease)) {
        ...
        $PAGE->set_heading($strinstallation . ' - shezar ' . $shezar->release);
        $PAGE->set_cacheable(false);

        /** @var core_admin_renderer $output */
        $output = $PAGE->get_renderer('core', 'admin');
        echo $output->install_environment_page($maturity, $envstatus, $environment_results, $shezar->release);
        die();
    }
...
if (!$cache and $shezarinfo->upgradecore) {
    // shezar: this is executed when Moodle version or shezar release changed.
    ...
    if (empty($confirmupgrade)) {
        $strdatabasechecking = get_string('databasechecking', '', $shezarinfo);

        $PAGE->set_title($stradministration);
        $PAGE->set_heading($strdatabasechecking);
        $PAGE->set_cacheable(false);

        echo $output->upgrade_confirm_page($shezarinfo, $maturity, $testsite);
        die();
        ...
        echo $output->upgrade_environment_page($shezar->release, $envstatus, $environment_results);
        die();
    ...
    // shezar: this is not reached because we do the test above on $a->shezarupgradeerror
...
if (empty($site->shortname)) {
    ...
    //unset_config('registerauth'); // Not useful for shezar customers.
...
// shezar: Check to see if we are downloading latest errors
$geterrors = optional_param('geterrors', 0, PARAM_BOOL);
if ($geterrors) {
    shezar_errors_download();
    die();
}
...
echo $output->admin_notifications_page($maturity, $insecuredataroot, $errorsdisplayed, $cronoverdue, $dbproblems,
                                       ...
                                       $registered, $cachewarnings, $latesterror, $activeusers, $shezar->release);
