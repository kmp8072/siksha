...
$string['configmessageinboundhost'] = 'The address of the server that shezar should check mail against. To specify a non-default port, you can use the [server]:[port], for example mail.example.com:587 format. If you leave this field blank, shezar will use the default port for the type of mail server you specify.';
...
$string['incomingmailserversettings_desc'] = 'shezar is capable of connecting to appropriately configured IMAP servers. You can specify the settings used to connect to your IMAP server here.';
...
$string['messageinboundgeneralconfiguration_desc'] = 'Inbound message processing allows you to receive and process email within shezar. This has applications such as sending email replies to forum posts or adding files to a user\'s private files.';
...
$string['messageinboundhostssl_desc'] = 'Some mail servers support an additional level of security by encrypting communication between shezar and your server. We recommend using this SSL encryption if your server supports it.';
...
$string['validateaddress_help'] = 'When a message is received from a user, shezar attempts to validate the message by comparing the email address of the sender with the email address in their user profile.
