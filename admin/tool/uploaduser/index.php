                ...
                if ($datatype = $DB->get_field('user_info_field', 'datatype', array('shortname' => $field_shortname))) {
                    switch ($datatype) {
                        case 'datetime':
                            ...
                            $parsed_date = shezar_date_parse_from_format($csvdateformat, trim($value), true);
                            if ($parsed_date) {
                                $value = $parsed_date;
                            }
                            break;
                        case 'date':
                            ...
                            $csvdateformat = (isset($CFG->csvdateformat)) ? $CFG->csvdateformat : get_string('csvdateformatdefault', 'shezar_core');
                            // if date can't be parsed, assume it is a unix timestamp and leave unchanged
                            $parsed_date = shezar_date_parse_from_format($csvdateformat, trim($value), true, 'UTC');
                            if ($parsed_date) {
                                $value = $parsed_date;
                            }
                            break;
        ...
        if ($existinguser) {
            ...
            if ($isinternalauth) {
                if (empty($user->password)) {
                    ...
                    // will be automatically updated to a higher cost factor the first
                    // time the user logs in.
                    $clearpassword = $user->password; # <====== Add this line 
            ...
            $a->signoff = fullname($supportuser, true).' ('. $supportuser->email .')'; 
            $message = get_string('newusernewpasswordtext', '', $a); 
            $subject = format_string($site->fullname) .': '. get_string('newusernewpasswordsubj'); 
            email_to_user($user, $supportuser, $subject, $message); // Add lines to here 
