 ...
 */
class shezar_sync_element_settings_form extends moodleform {
    function definition() {
        global $CFG;
        $mform =& $this->_form;
        ...
        if ($sources = $element->get_sources()) {
            ...
            foreach ($sources as $s) {
                $sourceoptions[$s->get_name()] = get_string('displayname:'.$s->get_name(), 'tool_shezar_sync');
            }
            $mform->addElement('select', 'source_'.$elementname,
                get_string('source', 'tool_shezar_sync'), $sourceoptions);
            $mform->setDefault('source_'.$elementname, get_config('shezar_sync', 'source_'.$elementname));
        } else {
            $mform->addElement('static', 'nosources', '('.get_string('nosources', 'tool_shezar_sync').')');
        ...
        try {
            ...
            if ($source->has_config()) {
                $mform->addElement('static', 'configuresource', '', html_writer::link(new moodle_url('/admin/tool/shezar_sync/admin/sourcesettings.php', array('element' => $element->get_name(), 'source' => $source->get_name())), get_string('configuresource', 'tool_shezar_sync')));
            }
 ...
 */
class shezar_sync_source_settings_form extends moodleform {
    ...
    function set_data($data) {
        ...
        if ($this->elementname == 'user') {
            ...
            if (get_config('shezar_sync_element_user', 'allow_create')) {
                ...
                if (!get_config('shezar_sync_element_user', 'allowduplicatedemails')) {
 ...
 */
class shezar_sync_config_form extends moodleform {
    function definition() {
        global $CFG;
        ...
        // File access.
        if (has_capability('tool/shezar_sync:setfileaccess', context_system::instance())) {
            $mform->addElement('header', 'fileheading', get_string('files', 'tool_shezar_sync'));
            $dir = get_string('fileaccess_directory', 'tool_shezar_sync');
            $upl = get_string('fileaccess_upload', 'tool_shezar_sync');
            $mform->addElement('select', 'fileaccess', get_string('fileaccess', 'tool_shezar_sync'),
                array(FILE_ACCESS_DIRECTORY => $dir, FILE_ACCESS_UPLOAD => $upl));
            $mform->setType('fileaccess', PARAM_INT);
            $mform->setDefault('fileaccess', $dir);
            $mform->addHelpButton('fileaccess', 'fileaccess', 'tool_shezar_sync');
            $mform->addElement('text', 'filesdir', get_string('filesdir', 'tool_shezar_sync'), array('size' => 50));
            $mform->setType('filesdir', PARAM_TEXT);
            $mform->disabledIf('filesdir', 'fileaccess', 'eq', FILE_ACCESS_UPLOAD);
        }

        // Notifications.
        $mform->addElement('header', 'notificationheading', get_string('notifications', 'tool_shezar_sync'));
        $notifytypes = array();
        $notifytypes[] = $mform->createElement('checkbox', 'notifytypes[error]', '', get_string('errorplural', 'tool_shezar_sync'));
        $notifytypes[] = $mform->createElement('checkbox', 'notifytypes[warn]', '', get_string('warnplural', 'tool_shezar_sync'));
        $mform->addGroup($notifytypes, 'notifytypes', get_string('notifytypes', 'tool_shezar_sync'), '<br/>', false);

        $mform->addElement('text', 'notifymailto', get_string('notifymailto', 'tool_shezar_sync'));
        $mform->setType('notifymailto', PARAM_TEXT);
        $mform->setDefault('notifymailto', $CFG->supportemail);
        $mform->addHelpButton('notifymailto', 'notifymailto', 'tool_shezar_sync');
        $mform->setExpanded('notificationheading');

        $mform->addElement('header', 'scheduleheading', get_string('schedule', 'tool_shezar_sync'));
        $mform->addElement('advcheckbox', 'cronenable', get_string('enablescheduledsync', 'tool_shezar_sync'));
        $mform->setDefault('cronenable', 1);

        $complexscheduling = $this->_customdata['complexscheduling'];
        if (!$complexscheduling) {
            $mform->addElement('scheduler', 'schedulegroup', get_string('scheduleserver', 'tool_shezar_sync'));
            ...
            $url = new moodle_url('/admin/tool/task/scheduledtasks.php', array('action' => 'edit', 'task' => 'shezar_core\task\tool_shezar_sync_task'));
            $editlink = html_writer::link($url, get_string('scheduleadvancedlink', 'shezar_core'));
            $advancedschedulestr = get_string('scheduleadvanced', 'shezar_core', $editlink);
            $mform->addElement('static', 'advancedschedule', get_string('scheduleserver', 'tool_shezar_sync'), $advancedschedulestr);
        } else {
            $advancedschedulestr = get_string('scheduleadvancednopermission', 'shezar_core');
            $mform->addElement('static', 'advancedschedule', get_string('scheduleserver', 'tool_shezar_sync'), $advancedschedulestr);
        }
    ...
    function validation($data, $files) {
        ...
        if ($data['fileaccess'] == FILE_ACCESS_DIRECTORY && isset($data['filesdir'])) {
            ...
            if (!preg_match($pattern, $filesdir)) {
                $errors['filesdir'] = get_string('pathformerror', 'tool_shezar_sync');
            } else if (!is_dir($filesdir)) {
                $errors['filesdir'] = get_string('notadirerror', 'tool_shezar_sync', $filesdir);
            } else if (!is_writable($filesdir)) {
                $errors['filesdir'] = get_string('readonlyerror', 'tool_shezar_sync', $filesdir);
            }
        }
        ...
        if (!empty($data['notifymailto'])) {
            ...
            foreach ($emailaddresses as $mailaddress) {
                if (!validate_email($mailaddress)) {
                    $errors['notifymailto'] = get_string('invalidemailaddress', 'tool_shezar_sync', format_string($mailaddress));
                    break;
                }
            }
        }
 ...
 */
class shezar_sync_source_files_form extends moodleform {
    function definition() {
        ...
        if (!count($elements)) {
            $mform->addElement('html', html_writer::tag('p',
                get_string('noenabledelements', 'tool_shezar_sync')));
            return;
        }

        foreach ($elements as $e) {
            $name = $e->get_name();
            if (!has_capability("tool/shezar_sync:upload{$name}", context_system::instance())) {
                continue;
            }
            $mform->addElement('header', "header_{$name}",
                get_string("displayname:{$name}", 'tool_shezar_sync'));
            $mform->setExpanded("header_{$name}");

            try {
                $source = $e->get_source();
            } catch (shezar_sync_exception $e) {
                $link = "{$CFG->wwwroot}/admin/tool/shezar_sync/admin/elementsettings.php?element={$name}";
                $mform->addElement('html', html_writer::tag('p',
                    get_string('nosourceconfigured', 'tool_shezar_sync', $link)));
                continue;
            }

            if (!$source->uses_files()) {
                $mform->addElement('html', html_writer::tag('p',
                    get_string('sourcedoesnotusefiles', 'tool_shezar_sync')));
                continue;
            }


            $mform->addElement('filepicker', $name,
            get_string('displayname:'.$source->get_name(), 'tool_shezar_sync'), 'size="40"');

            if (get_config('shezar_sync', 'fileaccess') == FILE_ACCESS_UPLOAD) {
                $usercontext = context_user::instance($USER->id);
                $systemcontext = context_system::instance();
                $fs = get_file_storage();

                //check for existing draft area to prevent massive duplication
                $existing_files = $fs->get_area_files($systemcontext->id, 'shezar_sync', $name);
                if (sizeof($existing_files) > 0) {
                    ...
                    if (sizeof($existing_draft) < 1) {
                        ...
                        file_prepare_draft_area($draftid, $systemcontext->id, 'shezar_sync', $name, null, $FILEPICKER_OPTIONS);
                        ...
                        foreach ($existing_files as $file) {
                            ...
                            $mform->addElement('static', '', '',
                                get_string('note:syncfilepending', 'tool_shezar_sync'));
                        }
                    }
