...
require_once($CFG->dirroot.'/admin/tool/shezar_sync/sources/classes/source.user.class.php');
require_once($CFG->dirroot.'/admin/tool/shezar_sync/lib.php');

class shezar_sync_source_user_csv extends shezar_sync_source_user {
    ...
    function config_form(&$mform) {
        ...
        if (empty($this->config->import_deleted)) {
            ...
            $deletedwarning = get_string('deletednotforjobassign', 'tool_shezar_sync', $a);
        }

        $delimiter = $this->config->delimiter;
        $info = get_string('csvimportfilestructinfo', 'tool_shezar_sync', implode($delimiter, $filestruct)) . $deletedwarning;
        ...
        if (get_config('shezar_sync', 'fileaccess') == FILE_ACCESS_DIRECTORY) {
            $mform->addElement('static', 'nameandloc', get_string('nameandloc', 'tool_shezar_sync'),
        ...
        $mform->addElement('select', 'csvuserencoding', get_string('csvencoding', 'tool_shezar_sync'), $encodings);
    ...
    function import_data($temptable) {
        ...
        if ($fileaccess == FILE_ACCESS_DIRECTORY) {
            ...
            $fieldid = get_config('shezar_sync', 'sync_user_itemid');

            // Check the file exist
            if (!$fs->file_exists($systemcontext->id, 'shezar_sync', 'user', $fieldid, '/', '')) {
            ...
            // Get the file
            $fsfiles = $fs->get_area_files($systemcontext->id, 'shezar_sync', 'user', $fieldid, 'id DESC', false);
            ...
            $fs->delete_area_files($systemcontext->id, 'shezar_sync', 'user', $itemid);
        ...
        foreach ($this->fields as $f) {
            ...
            if ($f == 'posidnumber' && shezar_feature_disabled('positions')) {
                unset($this->config->import_posidnumber);
                continue;
            }
            if (!in_array($f, $fields)) {
                throw new shezar_sync_exception($this->get_element_name(), 'importdata', 'csvnotvalidmissingfieldx', $f);
            }
        }
        ...
        while ($csvrow = fgetcsv($file, 0, $this->config->delimiter)) {
            ...
            $csvrow = shezar_sync_clean_fields($csvrow);
            ...
            if (empty($csvrow['timemodified'])) {
                ...
                $parsed_date = shezar_date_parse_from_format($csvdateformat, trim($csvrow['timemodified']), true);
                if ($parsed_date) {
            ...
            foreach ($datefields as $datefield) {
                if (!empty($dbrow[$datefield])) {
                    // Try to parse the contents - if parse fails assume a unix timestamp and leave unchanged.
                    $parsed_date = shezar_date_parse_from_format($csvdateformat, trim($csvrow[$datefield]), true);
                    if ($parsed_date) {
        ...
        fclose($file);
        if ($badtimezones) {
            $OUTPUT->notification(get_string('badusertimezonemessage', 'tool_shezar_timezonefix'), 'notifynotice');
        }
