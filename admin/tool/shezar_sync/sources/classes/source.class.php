...
abstract class shezar_sync_source {
    protected $config;
     ...
     *
     * @return sync table name (without prefix), e.g shezar_sync_org
    ...
    function __construct() {
        ...
        $this->filesdir = rtrim(get_config('shezar_sync', 'filesdir'), '/');

        // Ensure child class specified temptablename
        if (!isset($this->temptablename)) {
            throw shezar_sync_exception($this->get_element_name, 'setup', 'error',
    ...
    function addlog($info, $type='info', $action='') {
        shezar_sync_log($this->get_element_name(), $info, $type, $action);
    }
        ...
        foreach ($datarows as $i => $datarow) {
            foreach ($datarow as $name => $value) {
                if ((($columnsinfo[$name]->type == 'varchar' || $columnsinfo[$name]->type == 'nvarchar') &&
                    ...
                    // Prepare value to display in error message and for shezar_sync_log table.
                    if (core_text::strlen($value) > 75) {
                        $value = core_text::substr($value, 0, 75) . ' ...';
                    }
                    $this->addlog(get_string('lengthlimitexceeded', 'tool_shezar_sync', (object)array('idnumber' => $datarow['idnumber'], 'field' => $field,
