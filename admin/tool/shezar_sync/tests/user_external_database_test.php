...
/**
 * Class tool_shezar_sync_user_external_database_testcase
 *
 * These tests require an external database to be configured and defined in the sites config.
 * See the README.md file in /admin/tool/shezar_sync/
 *
 * These tests will be skipped if an external database is not defined.
 */
class tool_shezar_sync_user_external_database_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        if (defined('TEST_SYNC_DB_TYPE') ||
            ...
            $this->dbtable = $CFG->phpunit_prefix . 'shezar_sync_user_source';
        }
        ...
        set_config('source_user', 'shezar_sync_source_user_database', 'shezar_sync');
        ...
        set_config('database_dateformat', 'Y-m-d', 'shezar_sync_source_user_database');
        foreach ($this->configdb as $k => $v) {
            set_config($k, $v, 'shezar_sync_source_user_database');
        }
        foreach ($this->config as $k => $v) {
            set_config($k, $v, 'shezar_sync_element_user');
        }
    }
    ...
    protected function populate_external_user_table() {
