...
class core_admin_renderer extends plugin_renderer_base {
    ...
    public function install_licence_page() {
        ...
        $output .= $this->header();
        $output .= $this->heading('<a href="http://www.shezarlms.com/">shezar LMS</a> - shezar Learning Management System', 2, 'centered');
    ...
    public function install_environment_page($maturity, $envstatus, $environment_results, $release) {
        ...
        $output .= $this->heading("shezar {$shezar->release}");
        $output .= $this->active_flavour_notice(); // shezar flavours info.
    ...
    public function admin_notifications_page($maturity, $insecuredataroot, $errorsdisplayed,
            ...
            $buggyiconvnomb, $registered, array $cachewarnings = array(), $latesterror, $activeusers, $shezar_release) {
        global $CFG, $PAGE;
        $output = '';
        /** @var shezar_core_renderer $shezar_renderer */
        ...
        $output .= $shezar_renderer->is_registered();

        if ($latesterror) {
            $output .= $shezar_renderer->errorlog_link($latesterror);
        }
        // list count of active users
        $output .= $shezar_renderer->active_users($activeusers);
        /// Display shezar version information
        $output .= $shezar_renderer->shezar_print_copyright($shezar_release);

        // skodak: Martin likes to threaten people, so just style his copyright notice the standard way.
        // shezar: Remove generalbox markup.
        $output .= html_writer::start_div('copyright-acknowledgements');
        $output .= get_string('shezarcopyrightacknowledge', 'shezar_core');
        ...
        // shezar: Additional acknowledgement.
        $output .= $shezar_renderer->acknowledgements();
    ...
    public function environment_check_page($versions, $version, $envstatus, $environment_results) {
        ...
        // shezar: hack the version list to display our version numbers 9.0 and above.
        if (isset($versions['3.0'])) {
            $versions['3.0'] = str_replace('3.0', '9.0', $versions['3.0']);
        }

        // Print the component download link
        /* No env updates in shezar yet
        ...
        $select->label = get_string('shezarversion', 'shezar_core');
    ...
    protected function release_notes_link() {
        $releasenoteslink = get_string('releasenoteslink', 'admin', 'http://community.shezarlms.com/mod/forum/view.php?id=1834');
    ...
    public function plugins_check_table(core_plugin_manager $pluginman, $version, array $options = array()) {
        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                // shezar: add Bootstrap 3 classes
    ...
    public function active_flavour_notice() {
        $flavournotice = \shezar_flavour\helper::get_active_flavour_notice($this);
        if (!$flavournotice) {
            return null;
        }
        ...
        foreach ($environment_results as $environment_result) {
            ...
            if ($continue) {
                ...
                if ($environment_result->getPart() == 'custom_check'){
                    ...
                    if ($type === 'moodle' and !empty($CFG->shezar_release)) {
                        $type = 'shezar';
                        $report = str_replace($CFG->release, $CFG->shezar_release, $report);
                    }
