...
$hasmodconfig = has_capability('shezar/core:modconfig', $systemcontext);
if ($hassiteconfig || $hasmodconfig) {
    ...
    // shezar user delete hack.
    $options = array(
        'full' => get_string('authdeleteusersfull', 'shezar_core'),
        'partial' => get_string('authdeleteuserspartial', 'shezar_core'),
    );
    $temp->add(new admin_setting_configselect('authdeleteusers', new lang_string('authdeleteusers', 'shezar_core'),
        new lang_string('authdeleteusers_desc', 'shezar_core'), 'full', $options));
    // End of shezar hack.
    ...
    $temp->add(new admin_setting_configselect('guestloginbutton', new lang_string('guestloginbutton', 'auth'),
                                              new lang_string('showguestlogin', 'auth'), '0', array('0'=>new lang_string('hide'), '1'=>new lang_string('show')))); // shezar: disable guests by default.
...
// Add any settings from shezar modules.
foreach (core_component::get_plugin_list('shezar') as $plugin => $plugindir) {
    $settings_path = "$plugindir/settings.php";
    if (file_exists($settings_path)) {
        include($settings_path);
        continue;
