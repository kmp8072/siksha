...
$capabilities = array(
    ...
    'moodle/tag:manage',
    'shezar/core:editmainmenu',
    'shezar/core:appearance',
    'shezar/dashboard:manage',
);
...
if ($hassiteconfig or has_any_capability($capabilities, $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    $temp = new admin_settingpage('themesettings', new lang_string('themesettings', 'admin'),  array('shezar/core:appearance'));
...
preferences,moodle|/user/preferences.php|preferences',
    ...
    $ADMIN->add('themes', new admin_externalpage('themeselector', new lang_string('themeselector','admin'), $CFG->wwwroot . '/theme/index.php', array('shezar/core:appearance')));
    $ADMIN->add('themes', new admin_externalpage('elementlibrary', new lang_string('elementlibrary','shezar_core'), $CFG->wwwroot . '/elementlibrary/index.php', array('shezar/core:appearance')));
    ...
    foreach (core_component::get_plugin_list('theme') as $theme => $themedir) {
        ...
        if (file_exists($settings_path)) {
            $settings = new admin_settingpage('themesetting'.$theme, new lang_string('pluginname', 'theme_'.$theme), array('shezar/core:appearance'));
    ...
    // Calendar settings.
    $temp = new admin_settingpage('calendar', new lang_string('calendarsettings','admin'), array('shezar/core:appearance'));
    ...
    // blog
    $temp = new admin_settingpage('blog', new lang_string('blog','blog'), array('shezar/core:appearance'), empty($CFG->enableblogs));
    ...
    // Navigation settings
    $temp = new admin_settingpage('navigation', new lang_string('navigation'), array('shezar/core:appearance'));
    if (!shezar_feature_disabled('shezardashboard')) {
        $choices = array(
            HOMEPAGE_SITE => new lang_string('site'),
            HOMEPAGE_shezar_DASHBOARD => new lang_string('shezardashboard', 'admin')
        );
        $temp->add(new admin_setting_configselect('defaulthomepage', new lang_string('defaulthomepage', 'admin'),
            new lang_string('configdefaulthomepage', 'admin'), HOMEPAGE_shezar_DASHBOARD, $choices));
        $temp->add(new admin_setting_configcheckbox('allowdefaultpageselection', new lang_string('allowdefaultpageselection', 'shezar_dashboard'),
            new lang_string('allowdefaultpageselection_desc', 'shezar_dashboard'), '1'));
    }
    // shezar: allowguestmymoodle setting was remove from shezar 9
    ...
    $ADMIN->add('appearance', $temp);
     ...
     // shezar navigation.
    $ADMIN->add('appearance', new admin_externalpage('shezarnavigation', new lang_string('shezarnavigation', 'shezar_core'),
            new moodle_url('/shezar/core/menu/index.php'), array('shezar/core:editmainmenu')));

    // "htmlsettings" settingpage
    $temp = new admin_settingpage('htmlsettings', new lang_string('htmlsettings', 'admin'), array('shezar/core:appearance'));
    ...
    // The "media" subpage.
    $temp = new admin_settingpage('mediasettings', get_string('mediasettings', 'core_media'), array('shezar/core:appearance'));
    ...
    $temp = new admin_settingpage('documentation', new lang_string('moodledocs'), array('shezar/core:appearance'));
    ...
    $temp = new admin_externalpage('profilepage', new lang_string('myprofile', 'admin'), $CFG->wwwroot . '/user/profilesys.php',
            array('shezar/core:appearance'));
    $ADMIN->add('appearance', $temp);

    // Dashboard settings.
    $temp = new admin_externalpage('shezardashboard', new lang_string('dashboards', 'shezar_dashboard'),
        $CFG->wwwroot . '/shezar/dashboard/manage.php', array('shezar/dashboard:manage'), shezar_feature_disabled('shezardashboard'));
    $ADMIN->add('appearance', $temp);

    // coursecontact is the person responsible for course - usually manages enrolments, receives notification, etc.
    $temp = new admin_settingpage('coursecontact', new lang_string('courses'), array('shezar/core:appearance'));
    ...
    $temp = new admin_settingpage('ajax', new lang_string('ajaxuse'), array('shezar/core:appearance'));
    ...
    $temp = new admin_settingpage('additionalhtml', new lang_string('additionalhtml', 'admin'), array('shezar/core:appearance'));
