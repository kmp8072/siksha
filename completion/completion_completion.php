...
class completion_completion extends data_object {
    ...
    public function mark_enrolled($timeenrolled = null) {
        ...
        if (!$DB->record_exists('block_shezar_stats', $data)) {
            shezar_stats_add_event(time(), $this->userid, STATS_EVENT_COURSE_STARTED, '', $this->course);
        }
    }
    ...
    public function mark_inprogress($timestarted = null) {
        ...
        if (!$wasenrolled) {
            ...
            $data['data2'] = $this->course;
            if (!$DB->record_exists('block_shezar_stats', $data)) {
                shezar_stats_add_event($timenow, $this->userid, STATS_EVENT_COURSE_STARTED, '', $this->course);
            }
        }
    ...
    public function mark_complete($timecomplete = null) {
        ...
        if ($this->positionid === null) {
            $jobassignment = \shezar_job\job_assignment::get_first($this->userid, false);
            if ($jobassignment) {
        ...
        if ($result = $this->_save()) {
            ...
            $data['data2'] = $this->course;
            if (!$DB->record_exists('block_shezar_stats', $data)) {
                shezar_stats_add_event(time(), $this->userid, STATS_EVENT_COURSE_COMPLETE, '', $this->course);
            }
