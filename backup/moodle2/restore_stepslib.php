...
class restore_course_structure_step extends restore_structure_step {
    ...
    public function process_visibleaudience($data){
        ...
        if ($CFG->audiencevisibility && !empty($data->cohortid)) {
            ...
            if ($DB->record_exists('cohort', array('id' => $data->cohortid))) {
                shezar_cohort_add_association($data->cohortid, $courseid, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
            } else {
        ...
        if ($data->field_data) {
            if (!$field = $DB->get_record('course_info_field', array('shortname' => $data->field_name))) {
                ...
                if (isset($dataid) && isset($data->field_data_id) && (($data->field_type === 'file') || ($data->field_type === 'textarea'))) {
                    ...
                    $this->set_mapping('shezar_course_custom_field', $data->field_data_id, $dataid, true, context_system::instance()->id);
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('shezar_customfield', 'course_filemgr', 'shezar_course_custom_field', context_system::instance()->id);
        // Restore files for text area custom field types.
        $this->add_related_files('shezar_customfield', 'course', 'shezar_course_custom_field', context_system::instance()->id);
