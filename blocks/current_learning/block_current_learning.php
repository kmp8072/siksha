...
class block_current_learning extends block_base {
    ...
    public function get_content() {
        ...
        foreach ($items as $item) {
            ...
            switch ($itemclass) {
                ...
                case 'shezar_plan\user_learning\course':
                    $template = 'block_current_learning/course_row';
                    break;
                case 'shezar_program\user_learning\item':
                case 'shezar_certification\user_learning\item':
            ...
            if ($template !== false) {
                ...
                if ($item instanceof \shezar_core\user_learning\item_has_dueinfo && !empty($itemdata->dueinfo)) {
                ...
                if ($item instanceof \shezar_program\user_learning\item) {
                    $itemdata->title = get_string('thisisaprogram', 'block_current_learning');
                    $itemdata->icon = 'program';
                }

                if ($item instanceof \shezar_certification\user_learning\item) {
        ...
        if (!empty($contextdata['learningitems'])) {
            ...
            $rollink = new moodle_url('/shezar/plan/record/index.php', array('userid' => $USER->id));
     ...
     *
     * @param \shezar_core\user_learning\item_base[] $items
     * @return \shezar_core\user_learning\item_base[]
     */
    ...
    private function filter_collective_content(array $items) {
        ...
        $progcertcourses = [];
        foreach ($items as $item) {
            if ($item instanceof \shezar_program\user_learning\item || $item instanceof \shezar_certification\user_learning\item) {
        ...
        foreach ($items as $key => $item) {
            ...
            if ($item instanceof \core_course\user_learning\item) {
                ...
                if (empty($counts[$item->id]) && (!$item->has_owner() || !($item->get_owner() instanceof \shezar_plan\user_learning\item))) {
    ...
    private function get_user_learning_items() {
        ...
        /** @var \shezar_core\user_learning\item_base[] $classes */
        $classes = core_component::get_namespace_classes('user_learning', 'shezar_core\user_learning\item_base');
        /** @var \shezar_core\user_learning\item_base[] $items */
        $items = [];
        foreach ($classes as $class) {
            ...
            /** @var \shezar_core\user_learning\item_base[] $classitems */
    ...
    private function expand_item_specialisations(array $items) {
        foreach ($items as $item) {
            if ($item instanceof \shezar_plan\user_learning\item) {
