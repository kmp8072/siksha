...
define(['jquery', 'core/config', 'core/templates'], function ($, mdlconfig, templates) {
   var report_table = {
        ...
        change_links: function (uniqueid, blockid) {
            ...
            $('.block_shezar_report_table.' + uniqueid + ' .content .rb-display-table-container').
                    ...
                    if ($(e.target).siblings('.instantreportwait').length === 0) {
                        templates.renderIcon('loading', '', 'ft-size-600').done(function (html) {
                            var $splash = $('<div class="instantreportwait">' + html +'</div>'),
                                $content = $('.block_shezar_report_table.' + uniqueid + ' .content .rb-display-table-container'),
        ...
        refresh_results: function (uniqueid, querystring, callback) {
            // Make the ajax call.
            return $.get(
                mdlconfig.wwwroot + '/blocks/shezar_report_table/ajax_instantreport.php?' + querystring
            ).done( function (data) {
                var $content = $('.block_shezar_report_table.' + uniqueid + ' .content');
        ...
        populatelist: function() {
            ...
            $('#id_config_reportid').change(function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: mdlconfig.wwwroot + '/blocks/shezar_report_table/ajax_list_saved.php',
                    data: {
                        'reportid': reportId,
                    ...
                    success: function(data) {
                        ...
                        savedListControl.append('<option value="">' +
                            M.util.get_string('allavailabledata', 'block_shezar_report_table') + '</option>');
