...
function shezar_block_addtoplan_get_content($courseid, $userid) {
    global $CFG, $DB, $OUTPUT;

    $plans = dp_get_plans($userid, array(DP_PLAN_STATUS_UNAPPROVED, DP_PLAN_STATUS_PENDING, DP_PLAN_STATUS_APPROVED));

    $course_include = $CFG->dirroot . '/shezar/plan/components/course/course.class.php';
    ...
    $html = $OUTPUT->container_start(null, 'block_shezar_addtoplan_text');

    if (!empty($plans)) {
        $html .= html_writer::tag('p', get_string('addtoplanhint', 'block_shezar_addtoplan'));
        ...
        $html .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'plan-add-item-button', 'id' => 'show-course-dialog', 'value' => get_string('add', 'block_shezar_addtoplan')));
    ...
    if (!empty($exclude_plans) && is_array($exclude_plans)) {
        ...
        $html .= html_writer::tag('p', html_writer::tag('strong', get_string($planstring, 'block_shezar_addtoplan')));
        $inplans = array();
    ...
    $html .= $OUTPUT->container_end();  // block_shezar_addtoplan_text
