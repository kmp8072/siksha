...
class block_shezar_addtoplan extends block_base {

    function init() {
        $this->title = get_string('pluginname', 'block_shezar_addtoplan');
    }

    function get_content() {
        global $CFG, $USER, $COURSE;

        require_once($CFG->dirroot . '/blocks/shezar_addtoplan/lib.php');

        $args = array('args' => '{"courseid":' . $COURSE->id . '}');
        $jsmodule = array(
            'name' => 'block_shezar_addtoplan',
            'fullpath' => '/blocks/shezar_addtoplan/block.js',
            'requires' => array('json'));
        $this->page->requires->js_init_call('M.block_shezar_addtoplan.init', $args, false, $jsmodule);
        ...
        $this->content->text = shezar_block_addtoplan_get_content($COURSE->id, $USER->id);
