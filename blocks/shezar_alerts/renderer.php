 ...
 * @package shezar
 * @subpackage blocks_shezar_alerts
 */
class block_shezar_alerts_renderer extends plugin_renderer_base {
    /**
     ...
     * @param object $config shezar alert block config
     *
    ...
    public function display_alerts($alerts, $total, $config) {
        ...
        $count = count($alerts);
        if ($count) {
            $output .= html_writer::tag('p', get_string('showingxofx', 'block_shezar_alerts', array('count' => $count, 'total' => $total)));
        } else {
            if (!empty($CFG->block_shezar_alerts_showempty)) {
                if (!empty($config->showempty)) {
                    $output .= html_writer::tag('p', get_string('noalerts', 'block_shezar_alerts'));
                } else {
                    return '';
                }
            } else {
        ...
        if (!empty($alerts)) {
            $url = new moodle_url('/shezar/message/alerts.php', array('sesskey' => sesskey()));
            $link = html_writer::link($url, get_string('viewallnot', 'block_shezar_alerts'));
    ...
    public function display_alert($alert) {
        $output = '';
        $cssclass = shezar_message_cssclass($alert->msgtype);
        ...
        $moreinfotext = get_string('clickformoreinfo', 'block_shezar_alerts');
        $icon = $this->pix_icon('i/info', $moreinfotext, 'moodle', array('class'=>'informationicon', 'title' => $moreinfotext, 'alt' => $moreinfotext));
        $detailjs = shezar_message_alert_popup($alert->id, null, 'detailalert');
        $url = new moodle_url($msglink);
