 ...
 */
class block_shezar_program_completion extends block_base {

    public function init() {
        $this->title = get_string('programcompletion', 'block_shezar_program_completion');
    ...
    public function get_content() {
        ...
        if (!$programs = block_shezar_program_completion_programs::get_programs($this->instance->id)) {
            return $this->content;
        }

        $args = array('instanceid' => $this->instance->id);
        $this->page->requires->strings_for_js(array('more', 'less'), 'block_shezar_program_completion');
        $this->page->requires->js_call_amd('block_shezar_program_completion/block', 'init', $args);
        ...
        foreach ($programs as $p) {
            ...
            $programlink = new moodle_url('/shezar/program/view.php', array('id' => $p->id));
        ...
        if (!empty($this->config->maxshow) && $count > $this->config->maxshow) {
            $this->content->footer = html_writer::link('#', get_string('more', 'block_shezar_program_completion'),
                array('class' => 'block-shezar-prog-completion-morelink' . $this->instance->id)
            );
        }
