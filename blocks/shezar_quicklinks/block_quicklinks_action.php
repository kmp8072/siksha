...
if (!confirm_sesskey() || isguestuser()) {
    print_error('accessdenied', 'block_shezar_quicklinks', $referer);
}
...
if (!$blockinstance = $DB->get_record('block_instance', array('id' => $blockinstanceid))) {
    print_error('accessdenied', 'block_shezar_quicklinks');
}

$blockcontext = context_block::instance($blockinstanceid);

require_capability('block/shezar_quicklinks:manageownlinks', $blockcontext);

switch ($action) {
    case 'deletelink' :
        if (!$DB->delete_records('block_quicklinks', array('id' => $id))) {
            print_error('error:deletequicklink', 'block_shezar_quicklinks');
        }
