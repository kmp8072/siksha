...
require_capability('block/shezar_quicklinks:manageownlinks', $context);
...
$baseurl = new moodle_url('/blocks/shezar_quicklinks/managelinks.php', $urlparams);
$PAGE->set_url($baseurl);

$mform = new shezar_quicklinks_add_quicklink_form(null, array('blockinstanceid' => $blockinstanceid));
...
if ($data = $mform->get_data()) {
    ...
    shezar_set_notification(get_string('newlinkadded', 'block_shezar_quicklinks'), $baseurl, array('class' => 'notifysuccess'));
}
...
$strmanage = get_string('managelinks', 'block_shezar_quicklinks');

$PAGE->set_pagelayout('standard');
$PAGE->set_title($strmanage);
$PAGE->set_heading(format_string($SITE->fullname));

$managefeeds = new moodle_url('/blocks/shezar_quicklinks/managelinks.php', $urlparams);
$PAGE->navbar->add(get_string('blocks'));
$PAGE->navbar->add(get_string('managelinks', 'block_shezar_quicklinks'), $managefeeds);
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('managelinks', 'block_shezar_quicklinks'), 2, null, 'managelinks');

$table = new flexible_table('shezar-quicklinks-display-links');

$table->define_columns(array('title', 'url', 'actions'));
$table->define_headers(array(get_string('linktitle', 'block_shezar_quicklinks'), get_string('url', 'block_shezar_quicklinks'), get_string('actions', 'moodle')));
...
foreach ($links as $link) {
    ...
    $linkicons = $OUTPUT->action_icon($deleteurl, $deleteicon, new confirm_action(get_string('deletelinkconfirm', 'block_shezar_quicklinks'))) . ' ';
