...
$block = $DB->get_record('block_instances', array('id' => $blockid, 'blockname' => 'shezar_report_graph'), '*', MUST_EXIST);
...
$svgdata = \block_shezar_report_graph\util::get_svg_data($blockid, $config);

if ($type === 'svg') {
    \block_shezar_report_graph\util::send_svg($svgdata);

} else {
    \block_shezar_report_graph\util::send_pdf($svgdata);
}
