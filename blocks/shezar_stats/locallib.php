...
/**
 * adds an event to the shezar stats table.
 *
 ...
 */
function shezar_stats_add_event($time, $userid, $eventtype, $data=null, $data2=null) {
    global $DB;
    ...
    return $DB->insert_record('block_shezar_stats', $newevent);
}

/**
 * removes an event from the shezar stats table
 *
 * @param int $userid - userid this is related to
 * @param int $eventtype - see defines above for possible values
 * @param int $data2 - stores int related data for this event - eg time, id of record.
 * @return boolean (result of insert_record)
 */
function shezar_stats_remove_event($userid, $eventtype, $data2) {
    global $DB;
    return $DB->delete_records('block_shezar_stats', array('userid' => $userid, 'eventtype' => $eventtype, 'data2' => $data2));
}
 ...
 * @return array
 */
function shezar_stats_timespent($from, $to) {
    global $CFG;
    $minutesbetweensession = 30; //used to define new session
    if (!empty($CFG->block_shezar_stats_minutesbetweensession)) {
        $minutesbetweensession = $CFG->block_shezar_stats_minutesbetweensession;
    }
    //calculate timespent by each user
    $logs = shezar_stats_get_logs($from, $to);
    $totalTime = array();
    $lasttime = array();
        if (!empty($logs)){
 ...
 * @return array
 */
function shezar_stats_manager_stats($user, $config=null) {
    global $DB;
    ...
    if (empty($config) || !empty($config->statlearnerhours)) {
        ...
        $statssql[1]->sql = "SELECT count(DISTINCT userid) FROM {block_shezar_stats} ".
                            "WHERE eventtype = ". STATS_EVENT_TIME_SPENT.$commonsql.
                            "AND userid IN (SELECT bts2.userid FROM {block_shezar_stats} bts2 GROUP BY bts2.userid HAVING sum(bts2.data2) > " . $numhours*60*60 . ")";
    ...
    if (empty($config) || !empty($config->statcoursesstarted)) {
        ...
        $statssql[2]->sql = "SELECT count(*) FROM {block_shezar_stats} ".
    ...
    if (empty($config) || !empty($config->statcoursescompleted)) {
        ...
        $statssql[3]->sql = "SELECT count(*) FROM {block_shezar_stats} ".
    ...
    if (empty($config) || !empty($config->statcompachieved)) {
        ...
        $statssql[4]->sql = "SELECT count(*) FROM {block_shezar_stats} ".
    ...
    if (empty($config) || !empty($config->statobjachieved)) {
        ...
        $statssql[5]->sql = "SELECT count(*) FROM {block_shezar_stats} ".
 ...
 *
 * @param object $statsql - object from shezar_stats_***_stats functions.
 * @return array
 */
function shezar_stats_sql_helper($statsql) {
    global $DB, $OUTPUT;
    $results = array();
    $i = 0;
    foreach ($statsql as $stat) {
        ...
        $data = new stdClass();
        $data->displaystring = get_string($stat->string, 'block_shezar_stats', $stringparam);
        $data->icon = $OUTPUT->pix_icon($stat->string, '', 'block_shezar_stats');
        $results[$i] = $data;
        $i++;
    }
    return $results;
...
/**
 * obtains log data from logs tables - used by shezar_stats_timespent
 *
 * @param int $from - timestamp for start of stats generation
 * @param int $to - timestamp for end of stats generation
 ...
 * @return recordset
 */
function shezar_stats_get_logs($from, $to, $courseid=null) {
    global $DB;
