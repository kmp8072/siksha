...
class block_course_overview_renderer extends plugin_renderer_base {
    ...
    public function course_overview($courses, $overviews) {
        ...
        foreach ($courses as $key => $course) {
            ...
            if ($course->id > 0) {
                // shezar: We need may need to check audience visibility before adding the dimmed class.
                global $CFG;
                require_once($CFG->dirroot . '/shezar/coursecatalog/lib.php');
                $attributes['class'] = shezar_get_style_visibility($course);
    ...
    public function editing_bar_head($max = 0) {
        ...
        $url = new moodle_url('/shezar/dashboard/index.php');
    ...
    public function hidden_courses($total, $showing) {
        ...
        $vars->total = $total;
        $url = new moodle_url('/shezar/plan/record/courses.php', array('status' => 'all'));
        $vars->url = $url->out();
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            ...
            $a->showalllink = html_writer::link(new moodle_url('/shezar/dashboard/index.php', array('mynumber' => block_course_overview::SHOW_ALL_COURSES)),
