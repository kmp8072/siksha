 ...
 * @package shezar
 * @subpackage block_shezar_certifications
 */
 ...
 */
class block_shezar_certifications extends block_base {

    public function init() {
        $this->title   = get_string('pluginname', 'block_shezar_certifications');
    }
    ...
    public function get_content() {
        ...
        foreach ($renewals as $renewal) {
            ...
            $url = new moodle_url('/shezar/program/required.php', array('id' => $renewal->pid));
            ...
            if ($prog_completion) {
                $duedatestr = (empty($prog_completion->timedue) || $prog_completion->timedue == COMPLETION_TIME_NOT_SET)
                    ? get_string('duedatenotset', 'shezar_program')
                    : userdate($prog_completion->timedue, get_string('strftimedate', 'langconfig'));
            } else {
                $duedatestr =  get_string('duedatenotset', 'shezar_program');
            }
            $certification->date = $duedatestr;
            $certifications[] = $certification;
        }
        // Display 'required' list, certifications only.
        $url = new moodle_url('/shezar/program/required.php', array('userid' => $USER->id, 'filter' => 'certification'));
        if (count($certifications) > 0) {
            $this->content->footer = html_writer::link($url, get_string('allmycertifications', 'block_shezar_certifications'));
        }

        $renderer = $this->page->get_renderer('block_shezar_certifications');
