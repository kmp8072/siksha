...
// Do not close the session for now because we need it for shezar notification.
...
if ($result) {
    shezar_set_notification(get_string('serversynced', 'auth_connect'), new moodle_url('/auth/connect/index.php'), array('class'=>'notifysuccess'));
} else {
    shezar_set_notification(get_string('serversyncerror', 'auth_connect'), new moodle_url('/auth/connect/index.php'));
}
