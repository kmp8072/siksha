...
class message_output_email extends message_output {
    ...
    function send_message($eventdata) {
        ...
        // shezar specific extension to allow system to send alert/task but prevent email
        // even when users have it enabled
        require_once($CFG->dirroot.'/shezar/message/messagelib.php');
        if (isset($eventdata->sendemail) && $eventdata->sendemail !== shezar_MSG_EMAIL_YES) {
            return true;
        }
        ...
        if (core_user::is_real_user($eventdata->userto->id)) {
            $footerplain = $strmgr->get_string('alertfooter2', 'shezar_message', $CFG->wwwroot . '/message/edit.php', $eventdata->userto->lang);
            $footerhtml = str_repeat(html_writer::empty_tag('br'), 2) . html_writer::empty_tag('hr') . $strmgr->get_string('alertfooter2html', 'shezar_message', $CFG->wwwroot . '/message/edit.php', $eventdata->userto->lang);
        }
