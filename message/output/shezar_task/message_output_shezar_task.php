...
/**
 * Task message processor - stores the message to be shown using the shezar task notification system
 */

require_once($CFG->dirroot.'/message/output/lib.php');
require_once($CFG->dirroot.'/shezar/message/messagelib.php');

class message_output_shezar_task extends message_output{
    ...
    public function send_message($eventdata) {
        ...
        //hold onto the task processor id because /admin/cron.php sends a lot of messages at once
        ...
        if (empty($processorid)) {
            $processor = $DB->get_record('message_processors', array('name' => 'shezar_task'), '*', MUST_EXIST);
        ...
        if (!empty($messageid)) {
            \shezar_message\event\task_sent::create_from_message_data($eventdata, $messageid)->trigger();
        }
