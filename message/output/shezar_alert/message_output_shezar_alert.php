...
/**
 * Alert message processor - stores the message to be shown using the shezar alert notification system
 */

require_once($CFG->dirroot.'/message/output/lib.php');
require_once($CFG->dirroot.'/shezar/message/messagelib.php');

class message_output_shezar_alert extends message_output{
    ...
    public function send_message($eventdata) {
        ...
        if (empty($processorid)) {
            $processor = $DB->get_record('message_processors', array('name' => 'shezar_alert'), '*', MUST_EXIST);
        ...
        if (!empty($messageid)) {
            \shezar_message\event\alert_sent::create_from_message_data($eventdata, $messageid)->trigger();
        }
