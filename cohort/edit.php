...
if ($editform->is_cancelled()) {
    ...
    if ($data->id) {
        ...
        // shezar: handle tags and go to view page after update.
        if ($usetags) {
            if (isset($data->tags)) {
        ...
        shezar_set_notification(get_string('successfullyupdated','shezar_cohort'), $url, array('class' => 'notifysuccess'));
    } else {
        ...
        // shezar: handle tags and go to relevant page after insert.
        if ($usetags) {
            if (isset($data->tags)) {
        ...
        } else if (has_capability('shezar/cohort:managerules', $context)) {
            $url = new moodle_url('/shezar/cohort/rules.php', array('id' => $data->id));
        } else {
            $url = new moodle_url('/cohort/view.php', array('id' => $data->id));
        }
        redirect($url);
