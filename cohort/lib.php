...
function cohort_add_cohort($cohort, $addcollections=true) {
    ...
    shezar_cohort_increment_automatic_id($cohort->idnumber);
...
function cohort_delete_cohort($cohort) {
    ...
    $DB->delete_records('shezar_dashboard_cohort', array('cohortid' => $cohort->id));
    // Remove audience from scheduled reports.
    $DB->delete_records('report_builder_schedule_email_audience', array('cohortid' => $cohort->id));

    //delete associations
    $associations = shezar_cohort_get_associations($cohort->id);
    foreach ($associations as $ass) {
        shezar_cohort_delete_association($cohort->id, $ass->id, $ass->type);
    }
...
function cohort_edit_controls(context $context, moodle_url $currenturl) {
    ...
    if ($syscontext->id == $context->id) {
        ...
        if ($context->contextlevel == CONTEXT_COURSECAT) {
            $strcohorts = get_string('categorycohorts', 'shezar_cohort');
        } else {
...
function cohort_print_tabs($currenttab, $cohortid, $cohorttype, $cohort) {
    ...
    $canmanagerules = has_capability('shezar/cohort:managerules', $cohortcontext);
    $cancreateplancohort = has_capability('shezar/plan:cancreateplancohort', $systemcontext);
    $canmanagevisibility = has_capability('shezar/coursecatalog:manageaudiencevisibility', $systemcontext);
    ...
    if ($canassign && $cohorttype == cohort::TYPE_STATIC) {
        $toprow[] = new tabobject('editmembers', new moodle_url('/cohort/assign.php', array('id' => $cohortid)),
            get_string('editmembers','shezar_cohort'));
    }

    if ($canview) {
        $toprow[] = new tabobject('enrolledlearning', new moodle_url('/shezar/cohort/enrolledlearning.php', array('id' => $cohortid)),
            get_string('enrolledlearning', 'shezar_cohort'));
    }

    if (!empty($CFG->audiencevisibility) && $canview) {
        $toprow[] = new tabobject('visiblelearning', new moodle_url('/shezar/cohort/visiblelearning.php', array('id' => $cohortid)),
            get_string('visiblelearning', 'shezar_cohort'));
    }

    if (shezar_feature_visible('learningplans') && $canmanage && $cancreateplancohort) {
        $toprow[] = new tabobject('plans', new moodle_url('/shezar/cohort/learningplan.php', array('id' => $cohortid)),
            get_string('learningplan', 'shezar_cohort'));
    }

    if (shezar_feature_visible('goals') && $canview) {
        $toprow[] = new tabobject('goals', new moodle_url('/shezar/cohort/goals.php', array('id' => $cohortid)),
            get_string('goals', 'shezar_hierarchy'));
    }

    if ($canassignroles) {
        $toprow[] = new tabobject('roles', new moodle_url('/shezar/cohort/assignroles.php', array('id' => $cohortid)),
            get_string('assignroles', 'shezar_cohort'));
    }
