...
if ($delete && $cohort->id && $canedit) {
    if ($confirm and confirm_sesskey()) {
        ...
        $roles = shezar_get_cohort_roles($cohort->id);
        // Get members of the cohort.
        $members = shezar_get_members_cohort($cohort->id);
        $memberids = array_keys($members);
        // Unassign members from roles.
        shezar_unset_role_assignments_cohort($roles, $cohort->id, $memberids);

        $result = cohort_delete_cohort($cohort);
        shezar_set_notification(get_string('successfullydeleted', 'shezar_cohort'), $returnurl->out(), array('class' => 'notifysuccess'));
    }

    $yesurl = new moodle_url('/cohort/view.php', array('id'=>$cohort->id, 'delete'=>1, 'confirm'=>1,'sesskey'=>sesskey()));

    $strheading = get_string('delcohort', 'shezar_cohort');
    ...
    echo $OUTPUT->confirm(get_string('delconfirm', 'shezar_cohort', format_string($cohort->name)), $buttoncontinue, $buttoncancel);
...
if ($clone && $cohort->id && $canedit) {
    if ($confirm && confirm_sesskey()) {
        $result = shezar_cohort_clone_cohort($cohort->id);
        if ($result) {
            $successurl = new moodle_url($CFG->wwwroot.'/cohort/view.php', array('id'=>$result));
            shezar_set_notification(
                get_string('successfullycloned', 'shezar_cohort'),
                $successurl->out(),
                array('class' => 'notifysuccess')
            );
        } else {
            shezar_set_notification(get_string('failedtoclone', 'shezar_cohort'), $returnurl->out());
        }
    }
    $yesurl = new moodle_url($CFG->wwwroot.'/cohort/view.php', array('id'=>$cohort->id, 'clone'=>1, 'confirm'=>1, 'sesskey'=>sesskey()));

    $strheading = get_string('clonecohort', 'shezar_cohort');
    shezar_cohort_navlinks($cohort->id, format_string($cohort->name), $strheading);
    echo $OUTPUT->header();

    $buttoncontinue = new single_button($yesurl, get_string('yes'), 'post');
    $buttoncancel   = new single_button($nourl, get_string('no'), 'post');
    echo $OUTPUT->confirm(get_string('cloneconfirm', 'shezar_cohort', format_string($cohort->name)), $buttoncontinue, $buttoncancel);
    echo $OUTPUT->footer();
    die();
}

$strheading = get_string('overview', 'shezar_cohort');
...
$item = html_writer::tag('div', get_string('type', 'shezar_cohort'), array('class' => 'fitemtitle'));
$item .= html_writer::tag('div', ($cohort->cohorttype == cohort::TYPE_DYNAMIC) ? get_string('dynamic', 'shezar_cohort') : get_string('set', 'shezar_cohort'), array('class' => 'felement ftext'));
$out .= $OUTPUT->container($item, 'fitem required alternate');

$item = html_writer::tag('div', get_string('idnumber', 'shezar_cohort'), array('class' => 'fitemtitle'));
...
$item = html_writer::tag('div', get_string('startdate', 'shezar_cohort'), array('class' => 'fitemtitle'));
...
$item = html_writer::tag('div', get_string('enddate', 'shezar_cohort'), array('class' => 'fitemtitle'));
$ud = ($cohort->enddate) ? userdate($cohort->enddate, get_string('strftimedate')) : '';
$item .= html_writer::tag('div', $ud, array('class' => 'felement ftext'));
$out .= $OUTPUT->container($item, 'fitem required alternate');

$item = html_writer::tag('div', get_string('alertmembers', 'shezar_cohort'), array('class' => 'fitemtitle'));
$item .= html_writer::tag('div', $COHORT_ALERT[$cohort->alertmembers], array('class' => 'felement ftext'));
$out .= $OUTPUT->container($item, 'fitem required');


$item = html_writer::tag('div', get_string('members', 'shezar_cohort'), array('class' => 'fitemtitle'));
...
if ($cohort->cohorttype == cohort::TYPE_DYNAMIC) {
    ...
    $out .= $OUTPUT->heading(get_string('dynamiccohortcriterialower', 'shezar_cohort'));

    $out .= html_writer::start_tag('div', array('class' => 'mform'));
    $out .= html_writer::start_tag('fieldset');

    $item = html_writer::tag('div', get_string('rulestitle', 'shezar_cohort'), array('class' => 'fitemtitle'));
    if (empty($rulesets)) {
        $item .= html_writer::tag('div', get_string('norules', 'shezar_cohort'), array('class' => 'felement ftext'));
    } else {
        $item .= html_writer::start_tag('div', array('class' => 'felement ftext'));
        $item .= html_writer::start_tag('ul');
        $cohortoperator = get_string($COHORT_RULES_OP[$cohort->rulesetoperator], 'shezar_cohort');
        $i = 0;
        foreach ($rulesets as $ruleset) {
            ...
            $item .= $ruleset->name;
            $rulesetoperator = get_string($COHORT_RULES_OP[$ruleset->operator], 'shezar_cohort');
            ...
            if (!empty($rules)) { // Print its rules
                ...
                foreach ($rules as $rulerec) {
                    ...
                    if ($rule) {
                        ...
                        $a->name = $rulerec->name;
                        $content = get_string('cohortbrokenrule', 'shezar_cohort', $a);
...
if ($canedit) {
    ...
    echo $OUTPUT->single_button($cloneurl, get_string('clonethiscohort', 'shezar_cohort'));
    echo $OUTPUT->single_button($delurl, get_string('deletethiscohort', 'shezar_cohort'));
}
echo $OUTPUT->footer();
