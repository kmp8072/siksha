...
class cohort_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $placeholder = get_string('datepickerlongyearplaceholder', 'shezar_core');
        $hint = get_string('dateformatlongyearhint', 'shezar_cohort', $placeholder);
        ...
        $mform->setDefault('idnumber', shezar_cohort_next_automatic_id());
        ...
        */
        ...
        if (!$cohort->id) {
            $mform->addElement('select', 'cohorttype', get_string('type', 'shezar_cohort'), cohort::getCohortTypes());
            $mform->addHelpButton('cohorttype', 'type', 'shezar_cohort');
        }

        $mform->addElement('editor', 'description_editor', get_string('description', 'cohort'), null, $editoroptions);
        $mform->setType('description_editor', PARAM_RAW);

        $mform->addElement('date_selector', 'startdate', get_string('startdate', 'shezar_cohort'), array('optional' => true));
        $mform->addHelpButton('startdate', 'startdatelimited', 'shezar_cohort');

        $mform->addElement('date_selector', 'enddate', get_string('enddate', 'shezar_cohort'), array('optional' => true));
        $mform->addHelpButton('enddate', 'enddatelimited', 'shezar_cohort');
        ...
        if (!empty($alertoptions)) {
            $mform->addElement(
                'select',
                'alertmembers',
                get_string('alertmembers', 'shezar_cohort'),
                $alertoptions
            );
            $mform->addHelpButton('alertmembers', 'alertmembers', 'shezar_cohort');
        } else {
    ...
    public function validation($data, $files) {
        ...
        if ($data['startdate'] > $data['enddate'] && $data['enddate']) {
            $errstr = get_string('error:startafterfinish','shezar_cohort');
