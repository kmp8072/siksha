...
shezar_cohort_navlinks($cohort->id, $cohort->name, get_string('assign', 'cohort'));
...
$currentcohortroles = shezar_get_cohort_roles($cohort->id);
...
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        ...
        shezar_set_role_assignments_cohort($currentcohortroles, $cohort->id, array_keys($newids));
        // Notify users.
        shezar_cohort_notify_add_users($cohort->id, $newids);

        // Create learning plans.
        $config = \shezar_cohort\learning_plan_config::get_config($cohort->id);
...
if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoremove)) {
        ...
        shezar_unset_role_assignments_cohort($currentcohortroles, $cohort->id, array_keys($delids));
        // Notify users.
        shezar_cohort_notify_del_users($cohort->id, $delids);
