...
class appraisal {
    ...
    public function save($override = false) {
        ...
        if ($this->id > 0) {
            ...
            \shezar_appraisal\event\appraisal_updated::create_from_instance($this)->trigger();
        } else {
            $this->id = $DB->insert_record('appraisal', $todb);

            \shezar_appraisal\event\appraisal_created::create_from_instance($this)->trigger();
        }
        // Refresh data.
        $this->load($this->id);
        return $this;
    ...
    public function set_status($newstatus) {
        ...
        if (!in_array($newstatus, $allowedstatus[$this->status])) {
            ...
            throw new appraisal_exception(get_string('error:cannotchangestatus', 'shezar_appraisal', $a));
        } else {
    ...
    public function activate($time = 0) {
        ...
        $assign = new shezar_assign_appraisal('appraisal', $this);
        ...
        $event = \shezar_appraisal\event\appraisal_activation::create(
            array(
                'objectid' => $this->id,
    ...
    public function validate($time = null) {
        ...
        if ($this->status != self::STATUS_DRAFT) {
            $err['status'] = get_string('appraisalinvalid:status', 'shezar_appraisal');
        }
        ...
        if (empty($rolescananswer)) {
            $err['roles'] = get_string('appraisalinvalid:roles', 'shezar_appraisal');
        }
        ...
        // for appraisees.
        $assignment = new shezar_assign_appraisal('appraisal', $this);
        if ($assignment->get_current_users_count() < 1) {
            $key = 'learners';
            $errdesc = get_string('appraisalinvalid:learners', 'shezar_appraisal');
        ...
        foreach ($stages as $stage) {
            ...
            if ($checkstage->timedue && in_array($checkstage->timedue, $timesdue)) {
                $err['timedue'] = get_string('appraisalinvalid:stagesamedue', 'shezar_appraisal');
            }
    ...
    public function validate_roles($live = null) {
        ...
        if ($missing->appraiseecount === 0) {
            $err = get_string('appraisalinvalid:learners', 'shezar_appraisal');
            return ['learners' => $err];
        }
        ...
        foreach ($shown as $appraiseeid => $roles) {
            $csv = array_reduce(
                ...
                function ($accumulated, $role) use ($roledesc) {
                    $desc = get_string($roledesc[$role], 'shezar_appraisal');
            ...
            $param->role = $csv;
            $error = get_string(
                'appraisalinvalid:missingrole', 'shezar_appraisal', $param
            );
        ...
        if ($excess > 0) {
            $err = get_string('xmoremissingroles', 'shezar_appraisal', $excess);
    ...
    public function get_missingrole_users() {
        throw new coding_exception('get_missingrole_users has been deprecated since 9.0. Use functions from shezar_assign_appraisal instead.');
    }
    ...
    public function get_changedrole_users() {
        throw new coding_exception('get_changedrole_users has been deprecated since 9.0. Use functions from shezar_assign_appraisal instead.');
    }
    ...
    public function close($formdata = null) {
        ...
        if (isset($formdata->sendalert) && $formdata->sendalert) {
            ...
            $alert->subject = get_string('closealerttitledefault', 'shezar_appraisal', $this);
            ...
            foreach ($roleusers as $roleuser) {
                ...
                $alert->fullmessage = get_string('closealertadminbody', 'shezar_appraisal', $formdata);
    ...
    public function save_snapshot($filepath, $roleassignmentid, $time = 0) {
        ...
        $meta = array('contextid' => $context->id, 'component' => 'shezar_appraisal', 'filearea' => 'snapshot_'.$this->id,
            'itemid' => $roleassignmentid, 'filepath' => '/', 'filename' => $filename, 'timecreated' => $time,
            'userid' => $USER->id);
        $file = $fs->create_file_from_pathname($meta, $filepath);

        return moodle_url::make_pluginfile_url($context->id, 'shezar_appraisal', 'snapshot_'.$this->id, $roleassignmentid, '/',
    ...
    public static function list_snapshots($appraisalid, $roleassignmentid) {
        ...
        $files = $fs->get_area_files($context->id, 'shezar_appraisal', 'snapshot_'.$appraisalid, $roleassignmentid, 'timecreated',
                false);

        $snapshots = array();
        foreach ($files as $file) {
            $snapshot = new stdClass();
            $snapshot->url = moodle_url::make_pluginfile_url($context->id, 'shezar_appraisal', 'snapshot_'.$appraisalid,
    ...
    public function delete_all_snapshots() {
        $context = context_system::instance();
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'shezar_appraisal', 'snapshot_', $this->id);
    }
    ...
    public function delete() {
        ...
        // Remove files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'shezar_appraisal', 'appraisal', $this->id);

        // Remove appraisal.
        $DB->delete_records('appraisal', array('id' => $this->id));
        $this->id = null;

        \shezar_appraisal\event\appraisal_deleted::create_from_instance($snapshot)->trigger();
    }
    ...
    public static function display_status($status) {
        ...
        switch ($status) {
            case self::STATUS_ACTIVE:
                $result = get_string('active', 'shezar_appraisal');
                break;
            case self::STATUS_DRAFT:
                $result = get_string('draft', 'shezar_appraisal');
                break;
            case self::STATUS_CLOSED:
                $result = get_string('closed', 'shezar_appraisal');
                break;
            case self::STATUS_COMPLETED:
                $result = get_string('completed', 'shezar_appraisal');
                break;
        }
        return $result;
    }
        ...
        foreach ($stages as $stage) {
            ...
            foreach ($pages as $page) {
                ...
                foreach ($questionrecords as $questionrecord) {
                    ...
                    $a->type = $info['type'];
                    $item->name = get_string('questionandtype', 'shezar_question', $a);
            ...
            foreach ($pages as $page) {
                ...
                foreach ($questionrecords as $questionrecord) {
                    ...
                    if (in_array($question->get_element()->datatype, $aggregatetypes)) {
                        ...
                        $list[$question->id] = get_string('aggregateselectordisplay', 'shezar_appraisal', $strvar);
                    }
                }
            }
        }
    ...
    public static function get_aggregate_question_answers($userid, $stagepageid, $questionid, $paramids) {
        ...
        foreach ($types as $qst) {
            ...
            if ($qst->datatype == 'ratingnumeric') {
                ...
                print_error('error:invalidquestiontype', 'shezar_appraisal');
            }
        }
    ...
    public static function get_latest($subjectid) {
        ...
        if (!$results) {
            throw new appraisal_exception(get_string('error:subjecthasnoappraisals', 'shezar_appraisal'));
        }
    ...
    public static function delete_learner_assignments($userid) {
        ...
        foreach ($userassignments as $userassignment) {
            ...
            foreach ($roleassignments as $roleassignment) {
                ...
                $fs->delete_area_files($context->id, 'shezar_appraisal', 'snapshot_', $appraisalid, $roleassignmentid);
            }
    ...
    public static function get_manage_list() {
        ...
        foreach ($appraisals as $appraisal) {
            if ($appraisal->status == self::STATUS_DRAFT) {
                $assign = new shezar_assign_appraisal('appraisal', new appraisal($appraisal->id));
    ...
    public static function duplicate_appraisal($appraisalid, $daysoffset = 0) {
        ...
        require_capability('shezar/appraisal:cloneappraisal', $systemcontext);
        ...
        $assign = new shezar_assign_appraisal('appraisal', new appraisal($appraisalid));
    ...
    public static function check_feature_enabled() {
        if (shezar_feature_disabled('appraisals')) {
            print_error('appraisalsdisabled', 'shezar_appraisal');
        }
    }
...
class appraisal_stage {
    ...
    public function save() {
        ...
        if ($this->id > 0) {
            ...
            \shezar_appraisal\event\stage_updated::create_from_instance($this)->trigger();
        } else {
            $this->id = $DB->insert_record('appraisal_stage', $todb);

            \shezar_appraisal\event\stage_created::create_from_instance($this)->trigger();
        }
    ...
    public function duplicate($appraisalid, $daysoffset = 0) {
        ...
        $data->descriptionformat = FORMAT_HTML;
        $data = file_prepare_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'shezar_appraisal', 'appraisal_stage', $srcstageid);

        $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'shezar_appraisal', 'appraisal_stage', $newstage->id);
    ...
    public function validate($time = null) {
        ...
        if (empty($this->timedue) || ($this->timedue > 0 && $this->timedue < $time)) {
                $err['stagedue'.$this->id] = get_string('appraisalinvalid:stagedue', 'shezar_appraisal',
        ...
        if (empty($pages)) {
            $err['stageempty' . $this->id] = get_string('appraisalinvalid:stageempty', 'shezar_appraisal',
                        format_string($this->name));
        }
        ...
        if (empty($rolesinvolved)) {
            $err['stagenocananswer' . $this->id] =
                    get_string('appraisalinvalid:stagenoonecananswer', 'shezar_appraisal',
                        format_string($this->name));
        }
    ...
    public function get_may_answer() {
        ...
        foreach ($roles as $role) {
            $out[$role->appraisalrole] = get_string($strroles[$role->appraisalrole], 'shezar_appraisal');
        }
    ...
    public function complete_for_user($subjectid) {
        ...
        $event = \shezar_appraisal\event\appraisal_stage_completion::create(
            array(
    ...
    public function delete() {
        ...
        // Remove files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'shezar_appraisal', 'appraisal_stage', $this->id);

        $DB->delete_records('appraisal_stage', array('id' => $this->id));
        $this->id = null;

        \shezar_appraisal\event\stage_deleted::create_from_instance($snapshot)->trigger();
...
class appraisal_page {
    ...
    public function save() {
        ...
        // Put default name.
        if (!$todb->name) {
            $todb->name = get_string('pagedefaultname', 'shezar_appraisal');
        }
        ...
        if ($this->id > 0) {
            ...
            \shezar_appraisal\event\page_updated::create_from_instance($this)->trigger();
        } else {
            $this->id = $DB->insert_record('appraisal_stage_page', $todb);

            \shezar_appraisal\event\page_created::create_from_instance($this)->trigger();
        }
        // Refresh data.
        $this->load($this->id);
        return $this;
    ...
    public function validate() {
        ...
        if (empty($questionrecords)) {
            $err['page' . $this->id] = get_string('appraisalinvalid:pageempty', 'shezar_appraisal',
                        format_string($this->name));
        }
        ...
        foreach ($questionrecords as $questionrecord) {
            ...
            if ($question->is_invalid_redisplay()) {
                $err['page' . $this->id . '_' . $question->id] = get_string('appraisalinvalid:redisplayfuture',
                        'shezar_appraisal', format_string($this->name));
            }
        }
    ...
    public static function delete($pageid) {
        ...
        \shezar_appraisal\event\page_deleted::create_from_instance($snapshot)->trigger();
...
class appraisal_question extends question_storage {
    ...
    public function save() {
        ...
        if ($this->id > 0) {
            ...
            \shezar_appraisal\event\question_updated::create_from_instance($this, $stage->id)->trigger();
        } else {
            $todb->datatype = $this->datatype;
            $this->id = $DB->insert_record('appraisal_quest_field', $todb);

            \shezar_appraisal\event\question_created::create_from_instance($this, $stage->id)->trigger();
        }
    ...
    public function populate_roles_element(appraisal_role_assignment $roleassignment,
        ...
        foreach ($questroles as $eachrole => $rights) {
            if (isset($otherassignments[$eachrole]) && ($rights & appraisal::ACCESS_CANANSWER) == appraisal::ACCESS_CANANSWER) {
                ...
                $questioninfo->label = get_string('role_answer_' . $rolecodestrings[$eachrole], 'shezar_appraisal');
    ...
    public static function delete($questid) {
        ...
        \shezar_appraisal\event\question_deleted::create_from_instance($snapshot, $stage->id)->trigger();
 ...
 */
function shezar_appraisal_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $USER;
    $systemcontext = context_system::instance();
    ...
    $fs = get_file_storage();
    if (!$file = $fs->get_file($context->id, 'shezar_appraisal', $filearea, $assignmentid, '/', $filename)) {
        send_file_not_found();
    }
 ...
 */
function shezar_appraisal_install_example_appraisal() {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/shezar/question/field/multichoice.class.php');
    require_once($CFG->dirroot . '/shezar/question/field/goals.class.php');
    ...
    $appraisal->name = get_string('example:appraisalname', 'shezar_appraisal');
    $appraisal->description = get_string('example:appraisaldescription', 'shezar_appraisal');
    $appraisal->status = 0;
    ...
    foreach (range(1, 3) as $num) {
        $stage = new stdClass();
        $stage->appraisalid = $appraisalid;
        $stage->name = get_string("example:stage{$num}name", 'shezar_appraisal');
        $stage->description = get_string("example:stage{$num}description", 'shezar_appraisal');
    ...
    foreach ($stageids as $stagenum => $stageid) {
        ...
        foreach (range(1, $pagesperstage[$stagenum]) as $pagenum) {
            $page = new stdClass();
            $page->appraisalstageid = $stageid;
            $page->name = get_string("example:stage{$stagenum}page{$pagenum}name", 'shezar_appraisal');
            $page->sortorder = $sort;
            $sort++;
    ...
    $scale = new stdClass();
    $scale->name = get_string('example:scaleyesnoname', 'shezar_appraisal');
    $admin = get_admin();
...
class appraisal_user_assignment {
    ...
    public function with_job_assignment($jobassignmentid) {
        ...
        // shezar_job\job_assignment::shezar_assign_appraisal
