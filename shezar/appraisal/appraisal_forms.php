...
class appraisal_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($readonly) {
            ...
            $mform->addElement('static', 'name', get_string('name', 'shezar_appraisal'));
            $mform->addElement('static', null, get_string('description'), $appraisal->desc);
        } else {
            $mform->addElement('text', 'name', get_string('name', 'shezar_appraisal'), 'maxlength="255" size="50"');
            $mform->addRule('name', null, 'required');
            $mform->addHelpButton('name', 'name', 'shezar_appraisal');

            $mform->addElement('editor', 'description_editor', get_string('description'), null, $TEXTAREA_OPTIONS);
            $mform->addHelpButton('description_editor', 'description', 'shezar_appraisal');

            $submittitle = get_string('createappraisal', 'shezar_appraisal');
            if ($appraisal->id > 0) {
...
class appraisal_close_form extends moodleform {
    ...
    public function definition() {
        ...
        $appraisal->alerttitle = get_string('closealerttitledefault', 'shezar_appraisal', $appraisal);
        ...
        $mform->addElement('checkbox', 'sendalert', get_string('closesendalert', 'shezar_appraisal'));
        $mform->setType('sendalert', PARAM_BOOL);

        $mform->addElement('text', 'alerttitle', get_string('closealerttitle', 'shezar_appraisal'),
                'maxlength="255" size="50"');
        $mform->setType('alerttitle', PARAM_TEXT);

        $mform->addElement('editor', 'alertbody_editor', get_string('closealertbody', 'shezar_appraisal'), null,
                $TEXTAREA_OPTIONS);
        $mform->setType('alertbody_editor', PARAM_CLEANHTML);

        $this->add_action_buttons(true, get_string('closeappraisal', 'shezar_appraisal'));
    ...
    public function definition_after_data() {
        ...
        foreach ($questions as $question) {
            ...
            if (($rights & appraisal::ACCESS_CANANSWER) == appraisal::ACCESS_CANANSWER) {
                ...
                $elem->label = get_string('role_answer_you', 'shezar_appraisal');

                if (!empty($viewerroles)) {
                    $elem->viewers = array();
                    foreach ($viewerroles as $viewerrole) {
                        if ($viewerrole != $roleassignment->appraisalrole) {
                            $elem->viewers[] = get_string($rolecodestrings[$viewerrole], 'shezar_appraisal');
                        }
                    }
                }
            }
        ...
        if ($button) {
            $this->add_action_buttons(false, get_string($button, 'shezar_appraisal'));
        }
    }
...
class appraisal_stage_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($readonly) {
            $mform->addElement('header', 'stageheader', get_string('viewstageheading', 'shezar_appraisal'));
        } else if ($stage->id > 0) {
            $mform->addElement('header', 'stageheader', get_string('editstageheading', 'shezar_appraisal'));
        } else {
            $mform->addElement('header', 'stageheader', get_string('addstage', 'shezar_appraisal'));
        }
        $mform->addElement('hidden', 'id');
        $mform->setType('id', PARAM_INT);
        ...
        if ($readonly) {
            ...
            $mform->addElement('static', 'name', get_string('name', 'shezar_appraisal'), 'maxlength="255" size="50"');
        } else {
            $mform->addElement('text', 'name', get_string('name', 'shezar_appraisal'), 'maxlength="255" size="50"');
        }
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required');
        $mform->addHelpButton('name', 'namestage', 'shezar_appraisal');

        $mform->addElement('editor', 'description_editor', get_string('description'), null, $TEXTAREA_OPTIONS);
        $mform->setType('description_editor', PARAM_CLEANHTML);
        $mform->addHelpButton('description_editor', 'descriptionstage', 'shezar_appraisal');
        $submittitle = ($stage->id > 0) ? get_string('savechanges', 'moodle') : get_string('addstage', 'shezar_appraisal');

        if ($readonly) {
            $stage->timedue = userdate($stage->timedue, get_string('strftimedate'));
            $mform->addElement('static', 'timedue', get_string('completeby', 'shezar_appraisal'));
        } else {
            $mform->addElement('date_selector', 'timedue', get_string('completeby', 'shezar_appraisal'), array('optional' => 'true'));
            $mform->addElement('static', 'timedue_hint', '', get_string('completebystage_help', 'shezar_appraisal'));
        }
        $mform->setType('timedue', PARAM_TEXT);

        // Rights matrix for roles.
        $mform->addElement('header', 'lockstageheader', get_string('locks', 'shezar_appraisal'));
        $roles = appraisal::get_roles();
        $rolegroup = array();
        foreach ($roles as $role => $rolename) {
            $rolegroup[] = & $mform->createElement('advcheckbox', $role, null, get_string($rolename, 'shezar_appraisal'));
        }
        $mform->addElement('group', 'locks', get_string('locks', 'shezar_appraisal'), $rolegroup, array('<br/> '));
        $mform->addElement('static', 'locks_hint', '', get_string('locks_help', 'shezar_appraisal'));

        // Initial pages, only when it is a new stage.
        if (!$stage->id > 0) {
            $mform->addElement('header', 'initialpagesheader', get_string('stageinitialpagesheader', 'shezar_appraisal'));
            $mform->addElement('textarea', 'stageinitialpagetitles', get_string('stageinitialpagetitles', 'shezar_appraisal'),
                    array('cols' => '90', 'rows' => '5'));
            $mform->addHelpButton('stageinitialpagetitles', 'stageinitialpagetitles', 'shezar_appraisal');
        }

        if ($readonly) {
            $backurl = new moodle_url('/shezar/appraisal/stage.php', array('appraisalid'=>$stage->appraisalid));
...
class appraisal_stage_page_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($page->id > 0) {
            $mform->addElement('header', 'stageheader', get_string('editpageheading', 'shezar_appraisal'));
        } else {
            $mform->addElement('header', 'stageheader', get_string('createpageheading', 'shezar_appraisal'));
        }
        $mform->addElement('hidden', 'id');
        $mform->setType('id', PARAM_INT);
        ...
        $submittitle = get_string('addpage', 'shezar_appraisal');
        if ($page->id > 0) {
            $submittitle = get_string('savechanges', 'moodle');
        }
...
class appraisal_quest_edit_form extends question_base_form {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'questionheader', get_string('questionmanage', 'shezar_question'));
        $mform->addElement('static', 'stagetitle', get_string('stagename', 'shezar_appraisal', $stagename));
        $mform->addElement('static', 'pagetitle', get_string('pagename', 'shezar_appraisal', $pagename));
        ...
        if ($element->requires_permissions()) {
            ...
            $mform->addElement('header', 'perms', get_string('permissions', 'shezar_appraisal') . $requiredstr);
    ...
    protected function add_role_matrix() {
        $mform = & $this->_form;
        $headcolumns = html_writer::tag('th', get_string('role', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('answer', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('required', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('viewother', 'shezar_appraisal'), array('class' => 'header'));
        ...
        foreach ($roles as $roleid => $name) {
            ...
            $strrolename = html_writer::start_tag('tr', array('class' => $rowclass)) .
                html_writer::tag('td', get_string($name, 'shezar_appraisal')) .
        ...
        $mform->addElement('html', get_string('roleaccessnotice', 'shezar_appraisal'));
    }
    ...
    protected function add_role_override_matrix() {
        $mform = & $this->_form;
        $headcolumns = html_writer::tag('th', get_string('role', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('view', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('viewother', 'shezar_appraisal'), array('class' => 'header'));
        $header = html_writer::tag('thead', html_writer::tag('tr', $headcolumns));
        $mform->addElement('html', html_writer::start_tag('table', array('class' => 'role_matrix')) . $header .
            html_writer::start_tag('tbody'));
    ...
    protected function add_role_inherits_matrix() {
        ...
        $mform = & $this->_form;
        $headcolumns = html_writer::tag('th', get_string('role', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('redisplay', 'shezar_appraisal') .
                $OUTPUT->help_icon('redisplay', 'shezar_appraisal', null), array('class' => 'header')) .
            html_writer::tag('th', get_string('overrideviewother', 'shezar_appraisal') .
                $OUTPUT->help_icon('overrideviewother', 'shezar_appraisal', null), array('class' => 'header'));
        $header = html_writer::tag('thead', html_writer::tag('tr', $headcolumns));
        $mform->addElement('html', html_writer::start_tag('table', array('class' => 'role_matrix')) . $header .
            html_writer::start_tag('tbody'));
    ...
    protected function add_role_viewers() {
        $mform = & $this->_form;
        $headcolumns = html_writer::tag('th', get_string('role', 'shezar_appraisal'), array('class' => 'header')) .
            html_writer::tag('th', get_string('visibility', 'shezar_appraisal'), array('class' => 'header'));
        $header = html_writer::tag('thead', html_writer::tag('tr', $headcolumns));
        $mform->addElement('html', html_writer::start_tag('table', array('class' => 'role_matrix')) . $header .
            html_writer::start_tag('tbody'));
    ...
    public function validation($data, $files) {
        ...
        if ($element->requires_permissions()) {
            ...
            $strerr = get_string('error:viewrequired', 'shezar_appraisal');
            if ($element->is_answerable()) {
                $accesskey = appraisal::ACCESS_CANANSWER;
                $strerr = get_string('error:writerequired', 'shezar_appraisal');
            }
            if ($element->inherits_permissions()) {
                $accesskey = appraisal::ACCESS_CANANSWER;
                $strerr = get_string('error:redisplayrequired', 'shezar_appraisal');
            }
    ...
    public function add_clone_roles() {
        $mform = & $this->_form;
        $mform->addElement('advcheckbox', "cloneprevroles", get_string('sameaspreceding', 'shezar_appraisal'));
    }
...
class appraisal_message_form extends moodleform {
    ...
    public function definition() {
        ...
        $eventslist = array('0' => get_string('eventactivation', 'shezar_appraisal'));
        $stages = appraisal_stage::get_list($appraisalid);
        foreach ($stages as $stage) {
            $eventslist[$stage->id] = get_string('eventstage', 'shezar_appraisal', format_string($stage->name));
        }
        $typeslist = array(appraisal_message::EVENT_STAGE_COMPLETE => get_string('eventstagecomplete', 'shezar_appraisal'),
            appraisal_message::EVENT_STAGE_DUE => get_string('eventstagedue', 'shezar_appraisal'));
        ...
        $mform->addGroup($eventgrp, 'event', get_string('event', 'shezar_appraisal'), null, false);

        // Timing.
        $timegrp = array();
        $timegrp[] = $mform->createElement('radio', 'timing', '', get_string('eventtimenow', 'shezar_appraisal'), '0');
        $timegrp[] = $mform->createElement('radio', 'timing', '', get_string('eventtimebefore', 'shezar_appraisal'), '-1');
        $timegrp[] = $mform->createElement('radio', 'timing', '', get_string('eventtimeafter', 'shezar_appraisal'), '1');

        $mform->addGroup($timegrp, 'timinggrp', get_string('eventtiming', 'shezar_appraisal'), html_writer::empty_tag('br'));

        // How much.
        $deltatypes = array(appraisal_message::PERIOD_DAY => get_string('perioddays', 'shezar_appraisal'),
            appraisal_message::PERIOD_WEEK => get_string('periodweeks', 'shezar_appraisal'),
            appraisal_message::PERIOD_MONTH => get_string('periodmonths', 'shezar_appraisal'));
        $deltagrp = array();
        ...
        $mform->addGroup($deltagrp, 'deltagrp', get_string('periodchoose', 'shezar_appraisal'), null, false);

        // Recipients.
        $roles = appraisal::get_roles();
        $rolesgrp = array();
        foreach ($roles as $role => $rolename) {
            $name = get_string($rolename, 'shezar_appraisal');
            $rolesgrp[] = $mform->createElement('advcheckbox', $role, '', $name);
        }
        $mform->addGroup($rolesgrp, 'rolegrp', get_string('eventrecipients', 'shezar_appraisal'), html_writer::empty_tag('br'));
        $mform->addRule('rolegrp', null, 'required');

        // Send for completed.
        $compgrp = array();
        $compgrp[] = $mform->createElement('advcheckbox', 'stageis', '', get_string('eventsendstagecompleted', 'shezar_appraisal'));
        $compgrp[] = $mform->createElement('radio', 'complete', '', get_string('eventstageisincomplete', 'shezar_appraisal'), '-1');
        $compgrp[] = $mform->createElement('radio', 'complete', '', get_string('eventstageiscomplete', 'shezar_appraisal'), '1');
        $mform->addGroup($compgrp, 'completegrp', get_string('stagesendingoption', 'shezar_appraisal'), html_writer::empty_tag('br'));
        ...
        // Message recipients.
        $messageall = array('all' => get_string('eventsendroleall', 'shezar_appraisal'),
            'each' => get_string('eventsendroleeach', 'shezar_appraisal'));
        ...
        $mform->addElement('text', 'messagetitle[0]',
                get_string('eventmessagetitle', 'shezar_appraisal') . $requiredstr,
        ...
        $mform->addElement('textarea', 'messagebody[0]',
                get_string('eventmessagebody', 'shezar_appraisal') . $requiredstr,
        ...
        foreach ($roles as $role => $rolename) {
            ...
            $mform->addElement('text', "messagetitle[$role]",
                    get_string('eventmessageroletitle', 'shezar_appraisal', $name) . $requiredstr,
                    array('class' => 'appraisal-event-title hide-disabled'));
            $mform->setType("messagetitle[$role]", PARAM_CLEANHTML);
            $mform->addElement('textarea', "messagebody[$role]",
                    get_string('eventmessagerolebody', 'shezar_appraisal', $name) . $requiredstr,
                    array('class' => 'appraisal-event-body hide-disabled'));
        ...
        if ($readonly) {
            $mform->addElement('static', 'backlink', '', html_writer::link(new moodle_url('/shezar/appraisal/message.php',
    ...
    public function validation($data, $files) {
        ...
        if ($data['eventid'] == 0) {
            ...
            if ($data['timinggrp']['timing'] < 0) {
                $err["timinggrp[timing]"] = get_string('error:beforedisabled', 'shezar_appraisal');
            }
        } else {
            if ($data['eventtype'] == 'stage_completion') {
                ...
                if ($data['timinggrp']['timing'] != 0) {
                    ...
                    if (!$isdeltaperiod || (int) $data['delta'] < 1) {
                        $err['deltagrp'] = get_string('error:numberrequired', 'shezar_appraisal');
                    }
                }
            }
        }
        ...
        if (empty($selectedroles)) {
            $err['rolegrp'] = get_string('error:rolemessage', 'shezar_appraisal');
        }

        if ($data['messagetoall'] == 'all') {
            // Common message must be not empty.
            if (trim($data['messagetitle'][0]) == '') {
                $err['messagetitle[0]'] = get_string('error:messagetitleyrequired', 'shezar_appraisal');
            }
            if (trim($data['messagebody'][0]) == '') {
                $err['messagebody[0]'] = get_string('error:messagebodyrequired', 'shezar_appraisal');
            }
        } else {
            // Each role message must be not empty.
            foreach ($selectedroles as $key => $role) {
                if (trim($data['messagetitle'][$key]) == '') {
                    $err["messagetitle[$key]"] = get_string('error:messagetitleyrequired', 'shezar_appraisal');
                }
                if (trim($data['messagebody'][$key]) == '') {
                    $err["messagebody[$key]"] = get_string('error:messagebodyrequired', 'shezar_appraisal');
                }
            }
        }
        return $err;
...
class appraisal_print_stages_form extends moodleform {
    public function definition() {
        ...
        $group = $mform->addGroup($stageselect, 'stages',  get_string('sectioninclude', 'shezar_appraisal'),
                html_writer::empty_tag('br'));
        $group->setAttributes(array('id' => 'stages-list'));
        $mform->addElement('advcheckbox', 'spaces', '', get_string('leavespace', 'shezar_appraisal'));
    }
