...
require_once($CFG->dirroot.'/shezar/core/dialogs/dialog_content_plan.class.php');
require_once($CFG->dirroot.'/shezar/core/dialogs/dialog_content_goals.class.php');
require_once($CFG->dirroot.'/shezar/core/dialogs/dialog_content_required_learning.class.php');
require_once($CFG->dirroot.'/shezar/core/dialogs/dialog_content_plan_evidence.class.php');
...
if (!$roleassignment) {
    echo get_string('noassignments', 'shezar_appraisal');
} else {
    // Display page.
    if (!$nojs) {
        ...
        if ($datatype == 'goals') {
            ...
            $dialog = new shezar_dialog_content_goals($frameworkid, false, $subjectid);
            ...
            if (!$dialog->showcompany && !$dialog->showpersonal) {
                print_error('error:goalselectionmustallowsomething', 'shezar_question');
            ...
            $dialog = new shezar_dialog_content_required_learning($subjectid);
            $dialog->load_items();
        } else {
            if ($datatype == 'evidencefromplan') {
                $dialog = new shezar_dialog_content_plan_evidence('evidence', $planid, $showhidden, $subjectid);
            } else {
                $dialog = new shezar_dialog_content_plan($question->get_element()->component, $planid, $showhidden, $subjectid);
            }
        ...
        $dialog->lang_file = 'shezar_appraisal';
