...
foreach ($rolesplayed as $roleplayed) {
    ...
    $row[] = new tabobject($roleplayed,
            new moodle_url('/shezar/appraisal/index.php', $urlparams),
            get_string('as' . $allroles[$roleplayed], 'shezar_appraisal'));
}
$tabs[] = $row;
...
if ($role == appraisal::ROLE_LEARNER) {
    $pageurl = new moodle_url('/shezar/appraisal/index.php');
    $PAGE->navbar->add(get_string('myappraisals', 'shezar_appraisal'), $pageurl);
} else {
    $params = array('role' => $role);
    if (!empty($subjectid)) {
        $params['subjectid'] = $subjectid;
    }
    $pageurl = new moodle_url('/shezar/appraisal/index.php', $params);
    $PAGE->navbar->add(get_string('teamappraisals', 'shezar_appraisal'), $pageurl);
}
...
if ($subjectid == $USER->id) {
    echo $OUTPUT->heading(get_string('allappraisals', 'shezar_appraisal'));
} else {
    $user = $DB->get_record('user', array('id' => $subjectid), '*', MUST_EXIST);
    $a = fullname($user);
    echo $OUTPUT->heading(get_string('allappraisalsfor', 'shezar_appraisal', $a));
}
