...
/**
 * Javascript file containing JQuery bindings for shezar appraisals
 */

M.shezar_appraisal_stage = M.shezar_appraisal_stage || {
  ...
  init : function(Y, args) {
    ...
    if ( typeof $ === 'undefined') {
      throw new Error('M.shezar_appraisal_stage.init()-> jQuery dependency required for this module to function.');
    }
    ...
    function loadPages(sID, stageLink, clearQS, newPage) {
      ...
      $.get(M.cfg.wwwroot + '/shezar/appraisal/ajax/stage.php', {
        id : sID
      }).done(function(data) {
    ...
    function modalForm(content, func, args) {
      ...
      var label = M.util.get_string('addpage', 'shezar_appraisal');
    ...
    function modalDelete(url, el, func, args) {
      ...
      if (hasRedisplay) {
        bodyContent = M.util.get_string('confirmdeleteitemwithredisplay', 'shezar_appraisal');
      } else {
        bodyContent = M.util.get_string('confirmdeleteitem', 'shezar_appraisal');
      }
      var dialogue = new M.core.dialogue({
      ...
      dialogue.addButton({
        ...
        action : function (e) {
          e.preventDefault();
            $.get(url, {sesskey: M.shezar_appraisal_stage.config.sesskey, confirm: 1}).done(function(data){
              if (data == 'success') {
                if ($el) {
                ...
                modalError(M.util.get_string('error:cannotdelete','shezar_appraisal'));
              }
            }).fail(function(){
              modalError(M.util.get_string('error:cannotdelete','shezar_appraisal'));
            });
      ...
      $('#appraisal-questions').load(url, function(){
        ...
        $('a.action-icon.edit', '#appraisal-quest-list').on('click', function() {
          $.get($(this).attr('href')).done(function(data){
            modalAddEditQuestion(data, M.cfg.wwwroot + '/shezar/appraisal/ajax/question.php?appraisalstagepageid=' + pageID);
          });
          return false;
        });
        ...
        $('a.action-icon.copy', '#appraisal-quest-list').on('click', function() {
          $.get($(this).attr('href') + '&sesskey=' + M.shezar_appraisal_stage.config.sesskey, function(data) {
            if (data == 'success') {
              pageContent(url);
            }
          });
        ...
        if ($('#appraisal-page-list').find('.move').length) {
          makeDraggable(M.cfg.wwwroot + '/shezar/appraisal/ajax/question.php', '#appraisal-quest-list', 'li', [{id:'#appraisal-page-list', type:'li'},{id:'.appraisal-stages', type:'tr'}]);
        }
      });
    }
    ...
    function makeDraggable(url, elID, elType, s) {
      ...
      YUI().use('dd', function(Y) {
        ...
        Y.DD.DDM.on('drag:end', function(e) {
          ...
          $.get(url, {
            sesskey: M.shezar_appraisal_stage.config.sesskey,
            action: 'pos',
    ...
    function attachEvents() {
      ...
      if ($('#appraisal-page-list').find('.move').length) {
        makeDraggable(M.cfg.wwwroot + '/shezar/appraisal/ajax/page.php?sesskey=' + M.shezar_appraisal_stage.config.sesskey,
