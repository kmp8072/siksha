...
if ($action == 'pages') {
        ...
        if (!$preview) {
            ...
            if (!$formissubmitted) {
                ...
                if (($answers->submitaction == 'saveprogress') && ($roleassignment->activepageid == $pageid)) {
                    ...
                    $returnurl = new moodle_url('/shezar/appraisal/myappraisal.php', array('role' => $role,
                        'subjectid' => $subjectid, 'appraisalid' => $appraisalid, 'action' => $action, 'pageid' => $pageid));
                    shezar_set_notification(get_string('progresssaved', 'shezar_appraisal'), $returnurl,
                    ...
                    if (!$formisvalid) {
                        shezar_set_notification(get_string('error:submitteddatainvalid', 'shezar_appraisal'));
                        ...
                        if ($appraisal->save_answers($answers, $roleassignment)) {
                            ...
                            if ($answers->submitaction == 'next') {
                                ...
                                redirect(new moodle_url('/shezar/appraisal/myappraisal.php', array('role' => $role,
                                ...
                                shezar_set_notification(get_string('stagecompleted', 'shezar_appraisal'), $returnurl,
                                    array('class' => 'notifysuccess'));
                                ...
                                $returnurl = new moodle_url('/shezar/appraisal/myappraisal.php', array('role' => $role,
                                    ...
                                    'pageid' => $pageid));
                                shezar_set_notification(get_string('changessaved', 'shezar_appraisal'), $returnurl,
                                    array('class' => 'notifysuccess'));
                            }
                        }
                    }
...
else if ($action == shezar_appraisal_constants::ACTION_ASSIGN_JOB) {
    $jobassignmentid = optional_param(
        shezar_appraisal_constants::PARAM_JOB_ID, null, PARAM_INT
    );

    if (empty($jobassignmentid)) {
        $msg = get_string('error:cannotassignjob', 'shezar_appraisal');
    ...
    shezar_set_notification(
        get_string('jobassignmentselected', 'shezar_appraisal'),
        new moodle_url(shezar_appraisal_constants::URL_MYAPPRAISAL, $urlparams),
        array('class' => 'notifysuccess')
    );
}
...
$PAGE->requires->strings_for_js(array('printnow', 'printyourappraisal', 'snapshotgeneration', 'downloadnow',
    'snapshotdialogtitle'), 'shezar_appraisal');
$PAGE->requires->js_call_amd('shezar_appraisal/myappraisal', 'init', array('args' => json_encode(array(
...
$pageurl = new moodle_url('/shezar/appraisal/myappraisal.php', $urlparams);
$PAGE->set_shezar_menu_selected('appraisals');
if ($role == appraisal::ROLE_LEARNER) {
    $PAGE->navbar->add(get_string('myappraisals', 'shezar_appraisal'), new moodle_url('/shezar/appraisal/index.php'));
} else {
    $PAGE->navbar->add(get_string('teamappraisals', 'shezar_appraisal'),
            new moodle_url('/shezar/appraisal/index.php', array('role' => $role)));
}
...
if ($action == 'stages') {
    ...
    if ($subjectid == $USER->id) {
        $showprint = has_capability('shezar/appraisal:printownappraisals', $systemcontext);
    } else {
        $showprint = has_capability('shezar/appraisal:printstaffappraisals', $usercontext);
    }
