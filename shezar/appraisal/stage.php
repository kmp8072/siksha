...
$returnurl = new moodle_url('/shezar/appraisal/stage.php', array('appraisalid' => $appraisalid));
...
switch ($action) {
    case 'edit':
        ...
        $defaults = file_prepare_standard_editor($defaults, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'shezar_appraisal', 'appraisal_stage', $id);
        ...
        if ($isdraft && $fromform = $mform->get_data()) {
            ...
            if (empty($fromform->submitbutton)) {
                shezar_set_notification(get_string('error:unknownbuttonclicked', 'shezar_appraisal'), $returnurl);
            }
            if (!empty($fromform->timedue)) {
                ...
                if ($fromform->timedue < time()) {
                    shezar_set_notification(get_string('error:completebyinvalid', 'shezar_appraisal'));
                    break;
                }
            }
            $stage->set($fromform);
            ...
            shezar_set_notification(get_string('stageupdated', 'shezar_appraisal'), $returnurl,
                    array('class' => 'notifysuccess'));
        }
        break;
    case 'delete':
        if ($stage->id < 1) {
            shezar_set_notification(get_string('error:stagenotfound', 'shezar_appraisal'), $returnurl,
        ...
        if ($isdraft) {
            ...
            if ($confirm == 1) {
                ...
                $stage->delete();
                shezar_set_notification(get_string('deletedstage', 'shezar_appraisal'), $returnurl,
                        array('class' => 'notifysuccess'));
            }
        } else {
...
$PAGE->requires->string_for_js('confirmdeleteitem', 'shezar_appraisal');
$PAGE->requires->string_for_js('confirmdeleteitemwithredisplay', 'shezar_appraisal');
$PAGE->requires->string_for_js('error:cannotdelete', 'shezar_appraisal');
$PAGE->requires->string_for_js('addpage', 'shezar_appraisal');
$PAGE->requires->string_for_js('close', 'form');
$PAGE->requires->string_for_js('error:cannotdelete', 'shezar_appraisal');

$jsmodule = array(
    'name' => 'shezar_appraisal_stage',
    'fullpath' => '/shezar/appraisal/js/stage.js',
    'requires' => array('json'));

$args = array('args' => '{"sesskey":"'.sesskey().'"}');
$PAGE->requires->js_init_call('M.shezar_appraisal_stage.init', $args, false, $jsmodule);
...
switch ($action) {
    case 'delete':
        echo $output->heading(get_string('deletestage', 'shezar_appraisal', $stage->name),3);
    ...
    case 'edit':
        ...
        break;
    default:
        echo $output->heading(get_string('stages', 'shezar_appraisal'), 3);
