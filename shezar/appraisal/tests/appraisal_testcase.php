...
abstract class appraisal_testcase extends advanced_testcase {
    ...
    protected function prepare_appraisal_with_users(array $def = array(), array $users = array()) {
        ...
        if (empty($users)) {
            $admin = get_admin();
            $adminja = \shezar_job\job_assignment::create_default($admin->id);
            for ($a = 0; $a < 2; $a++) {
                $user = $this->getDataGenerator()->create_user();
                // Set admin as a manager for users
                // By default definiton manager role is not involved.
                \shezar_job\job_assignment::create_default($user->id, array('managerjaid' => $adminja->id));
                $users[] = $user;
            }
        }
        ...
        $assign = new shezar_assign_appraisal('appraisal', $appraisal1);
    ...
    protected function update_job_assignments(\appraisal $appraisal) {
        $assignment = new shezar_assign_appraisal('appraisal', $appraisal);
