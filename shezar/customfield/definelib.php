...
class customfield_define_base {
    ...
    function define_form(&$form, $typeid=0, $tableprefix) {
        $form->addElement('header', '_commonsettings', get_string('commonsettings', 'shezar_customfield'));
        $this->define_form_common($form, $typeid, $tableprefix);

        $form->addElement('header', '_specificsettings', get_string('specificsettings', 'shezar_customfield'));
    ...
    function define_form_common(&$form, $typeid=0, $tableprefix) {
        ...
        $strrequired = get_string('customfieldrequired', 'shezar_customfield');
        ...
        $form->addHelpButton('fullname', 'customfieldfullname', 'shezar_customfield');

        $form->addElement('text', 'shortname', get_string('shortname', 'shezar_customfield'), 'maxlength="100" size="25"');
        $form->addRule('shortname', $strrequired, 'required', null, 'client');
        $form->setType('shortname', PARAM_ALPHANUM);
        $form->addHelpButton('shortname', 'customfieldshortname', 'shezar_customfield');

        $form->addElement('editor', 'description_editor', get_string('description', 'shezar_customfield'), null, $TEXTAREA_OPTIONS);
        $form->setType('description_editor', PARAM_CLEANHTML);
        $form->addHelpButton('description_editor', 'description', 'shezar_customfield');

        $form->addElement('selectyesno', 'required', get_string('customfieldrequired', 'shezar_customfield'));
        $form->addHelpButton('required', 'customfieldrequired', 'shezar_customfield');
        $form->setDefault('required', 0);

        $form->addElement('selectyesno', 'locked', get_string('locked', 'shezar_customfield'));
        $form->addHelpButton('locked', 'customfieldlocked', 'shezar_customfield');
        ...
        if ($form->getElementValue('datatype') != 'file' && $form->getElementValue('datatype') != 'url') {
            $form->addElement('selectyesno', 'forceunique', get_string('forceunique', 'shezar_customfield'));
            $form->addHelpButton('forceunique', 'customfieldforceunique', 'shezar_customfield');
        } else {
            $form->addElement('hidden', 'forceunique', '0');
            $form->setType('forceunique', PARAM_INT);
        }

        $form->addElement('selectyesno', 'hidden', get_string('visible', 'shezar_customfield'));
        $form->addHelpButton('hidden', 'customfieldhidden', 'shezar_customfield');
    ...
    function define_validate_common($data, $files, $typeid, $tableprefix) {
        ...
        if (empty($data->shortname)) {
            $err['shortname'] = get_string('customfieldrequired', 'shezar_customfield');
        ...
        /// Check the shortname is unique
            if ($field and $field->id <> $data->id) {
                $err['shortname'] = get_string('shortnamenotunique', 'shezar_customfield');
            }
        }

        // Prevent custom fields being both required and locked.
        if (!empty($data->required) && !empty($data->locked) ) {
            $err['required'] = get_string('requiredandlockednotallowed', 'shezar_customfield');
        }
    ...
    function define_save($data, $tableprefix) {
        ...
        $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_customfield', 'textarea', $data->id);
        $DB->set_field($tableprefix.'_info_field', 'description', $data->description, array('id' => $data->id));
        if ($data->datatype == 'textarea') {
            $data = file_postupdate_standard_editor($data, 'defaultdata', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_customfield', 'textarea', $data->id);
