...
class customfield_datetime extends customfield_base {
    ...
    static function display_item_data($data, $extradata=array()) {
        ...
        if (empty($data)) {
            return get_string('notset', 'shezar_customfield');
        } else {
    ...
    public function edit_validate_field ($itemnew, $prefix, $tableprefix) {
        ...
        if ($data) {
            ...
            if (!$timestamp) {
                $errors["{$this->inputname}"] = get_string ('error:invaliddateformat', 'shezar_customfield',
                    array ('data' => $data, 'field' => $this->field->shortname));
            } else if ($timestamp < $lower_limit) {
                $errors["{$this->inputname}"] = get_string ('error:invaliddatetooearly', 'shezar_customfield',
                    array ('data' => $data, 'field' => $this->field->shortname, 'year' => $this->field->param1));
            } else if ($timestamp > $upper_limit) {
                $errors["{$this->inputname}"] = get_string ('error:invaliddatetoolate', 'shezar_customfield',
                ...
                if ($result) {
                    $errors["{$this->inputname}"] = get_string ('error:invaliddatenotunqiue', 'shezar_customfield',
                        array ('data' => $data, 'field' => $this->field->shortname));
                }
            }
        }
