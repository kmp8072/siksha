...
class customfield_define_url extends customfield_define_base {
    ...
    public function define_form_specific(&$form) {
        // The link URL.
        $form->addElement('text', 'defaultdata', get_string('customfielddefaultdataurl', 'shezar_customfield'), 'maxlength="2048" size="50"');
        $form->setType('defaultdata', PARAM_URL);

        // Param 1 for the link text.
        $form->addElement('text', 'param1', get_string('customfielddefaultdataurltext', 'shezar_customfield'), 'maxlength="2048" size="50"');
        $form->setType('param1', PARAM_TEXT);
        $form->addHelpButton('param1', 'customfielddefaultdataurltext', 'shezar_customfield');

        // Param 2 for option to open link in a new window.
        $form->addElement('checkbox', 'param2', get_string('customfielddefaultdataurltarget', 'shezar_customfield'));
    ...
    public function define_validate_specific($data, $files, $tableprefix) {
        ...
        if (!empty($data->defaultdata)) {
            if (substr($data->defaultdata, 0, 7) !== 'http://' && substr($data->defaultdata, 0, 8) !== 'https://' && substr($data->defaultdata, 0, 1) !== '/') {
                return array('defaultdata' => get_string('customfieldurlformaterror', 'shezar_customfield'));
            }
        }
