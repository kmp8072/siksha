...
class customfield_url extends customfield_base {
    ...
    public function edit_field_add(&$mform) {
        ...
        $urllabel = html_writer::tag('label', get_string('customfieldtypeurl', 'shezar_customfield'), array('for' => 'id_' . $this->inputname . '_url'));
        $urlgrp[] = $mform->createElement('static', null, null, $urllabel);
        $urlgrp[] = $mform->createElement('text', $this->inputname . '[url]');
        $textlabel = html_writer::tag('label', get_string('customfieldtypeurltext', 'shezar_customfield'), array('for' => 'id_' . $this->inputname . '_text'));
        ...
        $checkboxlabel = html_writer::tag('label', get_string('customfieldtypeurltarget', 'shezar_customfield'), array('for' => 'id_' . $this->inputname . '_target'));
        ...
        $mform->addHelpButton($urlgrpname, 'customfieldurl', 'shezar_customfield');
    ...
    public function edit_validate_field($itemnew, $prefix, $tableprefix) {
        ...
        if (!empty($urldata['url'])) {
            if (substr($urldata['url'], 0, 7) !== 'http://' && substr($urldata['url'], 0, 8) !== 'https://' && substr($urldata['url'], 0, 1) !== '/') {
                return array($urlgrpname => get_string('customfieldurlformaterror', 'shezar_customfield'));
            }
        }
