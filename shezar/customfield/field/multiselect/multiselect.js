...
M.shezar_customfield_multiselect = M.shezar_customfield_multiselect || {
  Y : null,
  ...
  init : function(Y, args) {
    ...
    if (typeof $ === 'undefined') {
      throw new Error('M.shezar_customfield_multiselect.init()-> jQuery dependency required for this module to function.');
    }

    var savedchoices = M.shezar_customfield_multiselect.config.savedchoices;
    var numVisible = 0;
    var $container = $('#id_' + M.shezar_customfield_multiselect.config.jsid);
    var max = M.shezar_customfield_multiselect.config.max;
    ...
    $allOptions.each(function(){
        ...
        if ($($this.find('span')).length > 0) {
            ...
            var $makeDefaultLink = $('<a href="#">' + M.util.get_string('defaultmake', 'shezar_customfield') + '</a>');
            $makeDefault.append($makeDefaultLink);
            $makeDefaultLink.on('click', function(){
                if (M.shezar_customfield_multiselect.config.oneAnswer == 1) {
            ...
            var $unselectLink = $('<a href="#" class="js-hide">' + M.util.get_string('defaultselected', 'shezar_customfield') + '</a>');
