...
class customfield_define_multiselect extends customfield_define_base {
    ...
    public function define_form_specific(&$form) {
        ...
        for ($menuind = 0; $menuind < self::MAX_CHOICES; $menuind++) {
            ...
            $group[] = $form->createElement('advcheckbox', 'default', '',
                    get_string('defaultselected', 'shezar_customfield'), array('class' => 'makedefault'));
        ...
        $form->addHelpButton('multiselectitem[0]', 'customfieldmultiselectoptions', 'shezar_customfield');
    }
        ...
        foreach ($data->multiselectitem as $ind => $item) {
            ...
            if ($item['option'] != '') {
                if (in_array($item['option'], $addoptions)) {
                    $err["multiselectitem[$ind]"] = get_string('menunotuniqueoptions', 'shezar_customfield');
                }
                $addoptions[] = $item['option'];
            }
        }
        if ($itemerr) {
            $err['multiselectitem[0]'] = get_string('menunooptions', 'shezar_customfield');
        }
        return $err;
    }
    ...
    public function define_load_preprocess($data) {
        if (isset($data->param1) && $data->param1 != '') {
            ...
            foreach ($data->multiselectitem as $key => $item) {
                list($src, $alt) = shezar_icon_url_and_alt('course', $item['icon']);
    ...
    public function define_add_js($form, $jsid = '', $limitone = true) {
        ...
        $jsmodule = array(
            'name' => 'shezar_customfield_multiselect',
            'fullpath' => '/shezar/customfield/field/multiselect/multiselect.js',
            'requires' => array('json')
        );

        $PAGE->requires->js_init_call('M.shezar_customfield_multiselect.init', $args, false, $jsmodule);
