...
class customfield_multiselect extends customfield_base {
        ...
        while ($ind < count($this->options)) {
            ...
            while ($grp < $this->groupsize) {
                if (isset($this->options[$ind])) {
                    $iconhtml = shezar_icon_picker_preview('course', $this->options[$ind]['icon']);
    ...
    public static function get_item_string($title, $icon, $display) {
        if ($icon == '') {
            $iconhtml = shezar_icon_picker_preview('course', 'default', '', $title);
        } else {
            $iconhtml = shezar_icon_picker_preview('course', $icon, '', $title);
        }
    ...
    public function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule('grp_' . $this->fieldid . '_0',
                    get_string('customfieldrequired', 'shezar_customfield'), 'required', null,
                    'client');
        }
    }
