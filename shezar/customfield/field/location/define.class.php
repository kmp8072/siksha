...
require_once($CFG->dirroot . '/shezar/customfield/field/location/field.class.php');
...
class customfield_define_location extends customfield_define_base {
    ...
    public static function add_radio_group($groupfieldname, $grouparray, $options, $form) {
        ...
        foreach ($options as $size => $name) {
            $grouparray[] =& $form->createElement(
                ...
                '',
                get_string('customfieldtypelocation_' . $groupfieldname . $name, 'shezar_customfield'),
                $name, // Value.
                array(
    ...
    public static function add_location_field_form_elements($form, $fieldname = 'Location', $fielddefinition = true) {
        ...
        $form->addElement(
            'textarea',
            $formprefix . 'address',
            get_string('customfieldtypelocation_address'.$stringsuffix, 'shezar_customfield'),
            array(
                "cols" => "50",
                "rows" => "10"
            )
        ...
        $form->addGroup(
            $mapsizeoptions,
            $formprefix . 'size',
            get_string('customfieldtypelocation_mapsize'.$stringsuffix, 'shezar_customfield'),
            '<br />',
            false
        );
        ...
        $form->addGroup(
            $mapviewoptions,
            $formprefix . 'view',
            get_string('customfieldtypelocation_mapview'.$stringsuffix, 'shezar_customfield'),
            '<br />',
            false
        );
        ...
        $form->addGroup(
            ...
            $formprefix . 'display',
            get_string('customfieldtypelocation_display'.$stringsuffix, 'shezar_customfield'),
            '<br />',
            false
        );
        if (self::has_google_maps_client_id()) {
            ...
            $form->setDefault($formprefix . 'display', GMAP_DISPLAY_MAP_AND_ADDRESS);
            $form->addElement('static', 'googlemapapitoscheck', null, $OUTPUT->notification(get_string('gmaptosnotice_apikey', 'shezar_customfield'), 'notifymessage'));
        } else {
            $form->setDefault($formprefix . 'display', GMAP_DISPLAY_ADDRESS_ONLY);
            $form->addElement('static', 'googlemapapitoscheck', null, $OUTPUT->notification(get_string('gmaptosnotice_nokey', 'shezar_customfield'), 'notifymessage'));
        }
        ...
        $mapelements[] = $form->createElement(
            ...
            get_string('customfieldtypelocation_useaddress', 'shezar_customfield'),
            array(
        ...
        $mapelements[] = $form->createElement('static', 'selectpersonalheader', null,
            html_writer::tag('b', get_string('customfieldtypelocation_or', 'shezar_customfield')));

        $mapelements[] = $form->createElement(
            'text',
            $formprefix . 'addresslookup',
            get_string('customfieldtypelocation_addresslookup'.$stringsuffix, 'shezar_customfield')
        );
        ...
        $mapelements[] = $form->createElement(
            ...
            get_string('customfieldtypelocation_searchbutton', 'shezar_customfield'),
            array(
                'class' => 'btn_search'
            )
        );
        ...
        $form->addGroup($mapelements, $formprefix . 'mapelements', get_string('customfieldtypelocation_setmap', 'shezar_customfield'), ' ', false);
        $form->addHelpButton($formprefix . 'mapelements', 'customfieldtypelocation_setmap', 'shezar_customfield');
    ...
    public static function define_add_js($args = null) {
        ...
        $PAGE->requires->js_call_amd('shezar_customfield/field_location', 'init', array($args));
    }
