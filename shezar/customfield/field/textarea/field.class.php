...
class customfield_textarea extends customfield_base {
    ...
    function edit_field_add(&$mform) {
        ...
        if ($this->itemid == 0) { // If its new record.
            ...
            if (!empty($this->field->defaultdata)) {
                $data = file_rewrite_pluginfile_urls($this->field->defaultdata, 'pluginfile.php', $context->id, 'shezar_customfield', 'textarea', $this->fieldid);
                $mform->setDefault($this->inputname, array('text' => $data));
            }
            $mform->setType($this->inputname, PARAM_CLEANHTML);
        } else { // If its existing record.
            if ($this->is_locked()) {
                $data = file_rewrite_pluginfile_urls($this->data, 'pluginfile.php', $context->id, 'shezar_customfield', $this->prefix, $this->dataid);
                $mform->addElement('static', 'freezedisplay', format_string($this->field->fullname), format_text($data, FORMAT_MOODLE));
            } else {
                $mform->addElement('editor', $this->inputname, format_string($this->field->fullname), array('cols' => $cols, 'rows' => $rows), $TEXTAREA_OPTIONS);
                $data = file_rewrite_pluginfile_urls($this->data, 'pluginfile.php', $context->id, 'shezar_customfield', 'textarea', $this->fieldid);
                $mform->setDefault($this->inputname, array('text' => $data));
                $mform->setType($this->inputname, PARAM_CLEANHTML);
            }
        }
    ...
    */
    ...
    function sync_data_preprocess($itemnew) {
        ...
        $itemnew = file_prepare_standard_editor($itemnew, $shortinputname, $textarea_options, $systemcontext, 'shezar_customfield', $this->prefix);
    ...
    */
    function edit_save_data($itemnew, $prefix, $tableprefix) {
        ...
        $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_customfield', $prefix, $data->id);
    ...
    * Display the data for this field
    ...
    static function display_item_data($data, $extradata=array()) {
        ...
        $context = context_system::instance();
        $data = file_rewrite_pluginfile_urls($data, 'pluginfile.php', $context->id, 'shezar_customfield', $extradata['prefix'], $extradata['itemid']);
