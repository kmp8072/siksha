...
/***** The following methods must be overwritten by child classes *****/
    ...
    function edit_field_add(&$mform) {
        print_error('error:abstractmethod', 'shezar_customfield');
    }
...
/***** The following methods may be overwritten by child classes *****/
        ...
        if ($this->is_unique()) {
            ...
            switch ($this->field->datatype) {
                ...
                case 'textarea':
                    ...
                    $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_customfield', $prefix, $itemnew->id);
    ...
    function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule($this->inputname, get_string('customfieldrequired', 'shezar_customfield'), 'required', null, 'client');
        }
    }
...
/***** The following methods generally should not be overwritten by child classes *****/
    ...
    public function delete() {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'shezar_customfield', $this->prefix . '_filemgr', $this->dataid);
        $fs->delete_area_files($this->context->id, 'shezar_customfield', $this->prefix, $this->dataid);
        // Delete data.
        ...
        if ($field->datatype == 'textarea' && !$formfield->is_locked()) {
            ...
            if ($formfield->data == $formfield->field->defaultdata) {
                ...
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'shezar_customfield', 'textarea', $formfield->fieldid);
            } else {
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'shezar_customfield', $prefix, $formfield->dataid);
            }
        }
    }
}
...
function customfield_validation_filedata($itemnew, $prefix, $tableprefix) {
    ...
    foreach ($fields as $field) {
        if (isset($itemnew->{$field->shortname})) {
            ...
            $err += (array)get_string('error:novalue', 'shezar_customfield', $field->shortname);
        }
    }
    ...
    foreach ($fields as $field) {
        ...
        switch ($field->datatype) {
            ...
            case 'file':
                ...
                break;
            case 'datetime':
                require_once($CFG->dirroot.'/shezar/customfield/field/datetime/field.class.php');
