...
abstract class multichoice extends question_base {
    ...
    public function define_validate($data, $files) {
        ...
        if ($data->saveoptions && trim($data->saveoptionsname) == '') {
            $err['savegroup'] = get_string('choicesmustbenamed', 'shezar_question');
        }
        ...
        if (!isset($data->selectchoices) || !$data->selectchoices) {
            ...
            if (!$atleastone) {
                $err['choiceheader'] = get_string('atleastonerequired', 'shezar_question');
            }
        }
    ...
    protected function add_choices_menu(MoodleQuickForm $form, $readonly = false, $jsid = 'availablechoices',
        ...
        $form->addElement('header', $jsid, get_string($headerstringkey, 'shezar_question') . $requiredstr);
        ...
        if (!empty($saved)) {
            ...
            $savedchoices = array('0' => get_string('createnewchoices', 'shezar_question')) + $opsets;
        ...
        if (!$readonly) {
            $form->addElement('static', 'addoptionelem', '',
                html_writer::link('#', get_string('addanotheroption', 'shezar_question'),
                        array('id' => "addoptionlink_$jsid", 'class'=>'addoptionlink')));

            $save = array();
            $save[] = $form->createElement('advcheckbox', 'saveoptions', 0, get_string('savechoicesas', 'shezar_question'));
    ...
    protected function add_choices_menu_item($form, $i, $readonly) {
        ...
        if ($readonly) {
            ...
            $choice[] = $form->createElement('advcheckbox', 'default', '',
                    get_string('defaultmake', 'shezar_question'), array('class' => 'makedefault'));
        }
    ...
    protected function add_choices_js($form, $jsid, $savedchoices=array(), $limitone = true) {
        ...
        $PAGE->requires->strings_for_js(array('defaultmake', 'defaultselected', 'defaultunselect'), 'shezar_question');
        $args = array('args' => '{"savedchoices": ' . $jsonsavedchoices . ', "oneAnswer": "' . $limitone . '", "jsid": "' .
                        $jsid . '", "max": ' . $max . '}');

        $jsmodule = array(
            'name' => 'shezar_question_multichoice',
            'fullpath' => '/shezar/question/field/multichoice.js',
            'requires' => array('json')
        );

        $PAGE->requires->js_init_call('M.shezar_question_multichoice.init', $args, false, $jsmodule);
    ...
    public function to_html($value) {
        ...
        if ($scalevalue) {
            ...
            return get_string('userselectednothing', 'shezar_question');
        }
    }
