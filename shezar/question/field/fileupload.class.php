...
class question_fileupload extends question_base{
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_QUESTION,
                     'type' => get_string('questiontypefile', 'shezar_question'));
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        $form->addElement('header', 'uploadheader', get_string('uploadoptions', 'shezar_question'));
        $form->setExpanded('uploadheader');

        if ($readonly) {
            $form->addElement('static', 'maxnum', get_string('uploadmaxnum', 'shezar_question'));
        } else {
            $form->addElement('text', 'maxnum', get_string('uploadmaxnum', 'shezar_question'));
    ...
    protected function define_validate($data, $files) {
        $err = array();
        if ($data->maxnum < 1) {
            $err['maxnum'] = get_string('uploadmaxinvalid', 'shezar_question');
        }
        return $err;
    }
    ...
    public function add_field_specific_edit_elements(MoodleQuickForm $form) {
        ...
        if (isguestuser() || !isloggedin()) {
            $form->addElement('static', '', '', get_string('unavailableforguest', 'shezar_question'));
            return;
        }
    ...
    public function edit_get($dest) {
        ...
        // Using this prefix contruction means we can only use questions in shezar modules.
        $component = 'shezar_' . $this->prefix;
    ...
    public function to_html($value) {
        ...
        if (isguestuser() || !isloggedin()) {
            return get_string('unavailableforguest', 'shezar_question');
        }
        $fs = get_file_storage();
        $files = $fs->get_area_files($FILEPICKER_OPTIONS['context']->id, 'shezar_' . $this->prefix, 'quest_'.$this->id,
