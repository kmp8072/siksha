...
abstract class review extends multichoice {
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        $jsmodule = array(
            'name' => 'shezar_review',
            'fullpath' => '/shezar/question/field/reviewsettings.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.shezar_review_settings.init', null, false, $jsmodule);

        // Form for choosing range.
        $form->addElement('advcheckbox', 'hasmultifield', get_string('multiplefields', 'shezar_question'));
        $form->setType('hasmultifield', PARAM_BOOL);
        $form->addHelpButton('hasmultifield', 'multiplefields', 'shezar_question');
    ...
    protected function add_common_review_edit_elements(MoodleQuickForm $form) {
        ...
        $PAGE->requires->string_for_js('selectall', 'shezar_question');

        // Load selected items.
        $items = $this->get_grouped_items();

        $renderer = $PAGE->get_renderer('shezar_question');

        // Choose items.
        if ($this->has_review_items() || $this->preview) {
            if ($this->can_select_items()) {
                // Add the chooser button.
                $PAGE->requires->strings_for_js(array('choose' . $this->datatype . 'review', 'removeconfirm'), 'shezar_question');
                if ($this->preview) {
            ...
            if ($this->subjectid == $USER->id) {
                $form->addElement('static', 'noitems', '', get_string('noself' . $this->datatype, 'shezar_question'));
            } else {
                $user = $DB->get_record('user', array('id' => $this->subjectid));
                $form->addElement('static', 'noitems', '', get_string('nolearner' . $this->datatype, 'shezar_question',
        ...
        $jsmodule = array(
            'name' => 'shezar_review',
            'fullpath' => '/shezar/question/field/review.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.shezar_review.init', $args, false, $jsmodule);
        ...
        if (!empty($items)) {
            ...
            if (!$this->can_select_items()) {
                $form->addElement('static', '', '', html_writer::tag('em', get_string('nothingselected', 'shezar_question')));
        ...
        if ($this->required) {
            ...
            if ($this->has_review_items() && empty($items)) {
                $errors[$this->get_prefix_form() . '_choosereviewitem'] =
                        get_string('error:reviewmustselectitem', 'shezar_question');
            }
                    ...
                    if ($multifield) {
                        foreach ($scalevalues as $scalevalue) {
                            ...
                            if (!isset($fromform[$field]) || $fromform[$field] == '') {
                                $errors[$field] = get_string('required', 'shezar_question');
                            }
                        }
                    } else {
                        $field = $this->get_prefix_form() . '_reviewitem_' . $currentuseritems[0]->id;
