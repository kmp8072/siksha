...
class question_datepicker extends question_base{
    public static function get_info() {
        return array('group' => question_manager::GROUP_QUESTION,
                     'type' => get_string('questiontypedate', 'shezar_question'));
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        $form->addElement('header', 'dateheader', get_string('dateselection', 'shezar_question'));
        $form->setExpanded('dateheader');

        if ($readonly) {
            $form->addElement('static', 'startyear', get_string('datefirstyear', 'shezar_question'));
            $form->addElement('static', 'stopyear', get_string('datelastyear', 'shezar_question'));
        } else {
            $form->addElement('text', 'startyear', get_string('datefirstyear', 'shezar_question'));
            $form->setDefault('startyear', '1970');
            $form->addElement('text', 'stopyear', get_string('datelastyear', 'shezar_question'));
        ...
        $form->addElement('advcheckbox', 'withtime', '', get_string('dateincludetime', 'shezar_question'));
    }

    protected function define_validate($data, $files) {
        $err = array();
        if ($data->startyear >= $data->stopyear) {
            $err['startyear'] = get_string('dateinvalid', 'shezar_question');
        }
        return $err;
    }
