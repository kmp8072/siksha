...
class question_evidencefromplan extends review {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_REVIEW,
                     'type' => get_string('questiontypeevidencefromplan', 'shezar_question'));
    }

    public function __construct($storage, $subjectid = 0, $answerid = 0) {
        $this->buttonlabel = get_string('chooseevidencefromplanreview', 'shezar_question');
            ...
            foreach ($items as $item) {
                if (!isset($item->fullname)) {
                    $item->fullname = html_writer::tag('em',
                            get_string('reviewevidencefromplanassignmissing', 'shezar_question'));
                    $item->ismissing = true;
                }
            }
            return $items;
