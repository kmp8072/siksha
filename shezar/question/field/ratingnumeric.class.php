...
class question_ratingnumeric extends question_base{
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_QUESTION,
                     'type' => get_string('questiontyperatingnum', 'shezar_question'));
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        $form->addElement('header', 'rangeheader', get_string('answerrange', 'shezar_question'));
        $form->setExpanded('rangeheader');

        if ($readonly) {
            $form->addElement('static', 'rangefrom', get_string('rangefrom', 'shezar_question'));
            $form->addElement('static', 'rangeto', get_string('rangeto', 'shezar_question'));
        } else {
            $form->addElement('text', 'rangefrom', get_string('rangefrom', 'shezar_question'));
            $form->addElement('text', 'rangeto', get_string('rangeto', 'shezar_question'));
            $strrequired = get_string('fieldrequired', 'shezar_question');
        ...
        $setdefault = array();
        $setdefault[] = $form->createElement('advcheckbox', 'setdefault', 0, get_string('setdefault', 'shezar_question'));
        if ($readonly) {
        ...
        $list = array();
        $list[] = $form->createElement('radio', 'list', '', get_string('rangeslider', 'shezar_question'), self::DISPLAY_SLIDER);
        $list[] = $form->createElement('radio', 'list', '', get_string('rangeinput', 'shezar_question'), self::DISPLAY_INPUT);
        $form->addGroup($list, 'listtype', get_string('displaysettings', 'shezar_question'), array('<br/>'), false);
    ...
    protected function define_validate($data, $files) {
        ...
        if ($data->rangefrom >= $data->rangeto) {
            $err['rangefrom'] = get_string('rangeinvalid', 'shezar_question');
        } else if (($data->rangeto - $data->rangefrom) > 1000) {
            $err['rangefrom'] = get_string('rangelimit', 'shezar_question');
        }

        if (isset($data->setdefault) && $data->setdefault) {
            if ($data->defaultvalue < $data->rangefrom || $data->defaultvalue > $data->rangeto) {
                $err['setdefaultgroup'] = get_string('defaultvalueoutrange', 'shezar_question');
            }
        }
        return $err;
    }
    ...
    public function edit_validate($fromform) {
        ...
        if (isset($fromform[$this->get_prefix_form()])) {
            if ($fromform[$this->get_prefix_form()] < $this->get_min() || $fromform[$this->get_prefix_form()] > $this->get_max()) {
                $errors[$this->get_prefix_form()] = get_string('valueoutsiderange', 'shezar_question');
            }
    ...
    public function add_field_specific_edit_elements(MoodleQuickForm $form) {
        ...
        switch ($this->param2) {
            case self::DISPLAY_SLIDER:
                ...
                $jsmodule = array(
                    'name' => 'shezar_question_ratingnumeric',
                    'fullpath' => '/shezar/question/field/ratingnumeric.js',
                    'requires' => array('json'));

                $PAGE->requires->js_init_call('M.shezar_question_ratingnumeric.init', $args, false, $jsmodule);
            ...
            case self::DISPLAY_INPUT:
                ...
                $form->addElement('static', $prefixform . '_range', null,
                        get_string('ratingrequiredrange', 'shezar_question', array('from' => $from, 'to' => $to)));
                break;
        }
