...
class question_ratingcustom extends multichoice {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_QUESTION,
                     'type' => get_string('questiontyperatingcustom', 'shezar_question'));
    }
    ...
    public function define_validate($data, $files) {
        ...
        for ($i = 0; $i < count($data->choice); $i++) {
            ...
            if (isset($choice['option']) && trim($choice['option']) != "" && $choice['score'] == "") {
                $err["choice[{$i}]"] = get_string('ratingchoicemusthavescore', 'shezar_question');
            } else if (isset($choice['score']) && $choice['score'] != "" && trim($choice['option']) == "") {
                $err["choice[{$i}]"] = get_string('ratingscoremusthavechoice', 'shezar_question');
            }
        }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        // Add select type.
        $list = array();
        $list[] = $form->createElement('radio', 'list', '', get_string('multichoiceradio', 'shezar_question'), 1);
        $list[] = $form->createElement('radio', 'list', '', get_string('multichoicemenu', 'shezar_question'), 2);
        $form->addGroup($list, 'listtype', get_string('displaysettings', 'shezar_question'), array('<br/>'), true);
    ...
    protected function add_choices_menu_header($form, $readonly) {
        ...
        $header[] = $form->createElement('static', 'choice', '', html_writer::tag('span',
                html_writer::tag('b', get_string('choice', 'shezar_question')),
                array('class' => 'question-rating-scale-header')));
        $header[] = $form->createElement('static', 'rating', '', html_writer::tag('span',
                html_writer::tag('b', get_string('score', 'shezar_question')),
                array('class' => 'question-rating-scale-header')));
    ...
    protected function add_choices_menu_item($form, $i, $readonly) {
        ...
        $form->addGroupRule("choice[$i]",
                array("score" => array(array(get_string('error:scorenumeric', 'shezar_question'), 'numeric', '', 'client'))));
    }
