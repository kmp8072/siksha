...
class question_redisplay extends question_base{
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_OTHER,
                     'type' => get_string('questiontyperedisplay', 'shezar_question'));
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        if ($readonly) {
            $form->addElement('static', '', get_string('redisplay', 'shezar_question'), format_string($this->get_name()));
        } else {
            $list = $module::get_redisplay_question_list($moduleinfo);

            $select = $form->createElement('selectgroups', 'linkedquestion', get_string('redisplay', 'shezar_question'),
                    null, null, true);
            $select->addOptGroup(get_string('chooseexisting', 'shezar_question'), array());
            $group = 0;
            ...
            $form->addHelpButton('linkedquestion', 'redisplay', 'shezar_question');
    ...
    public function add_field_form_elements(MoodleQuickForm $form) {
        print_error('error:invalidfunctioncalledinredisplay', 'shezar_question');
    }
