...
class question_fixedimage extends question_base{
    public static function get_info() {
        return array('group' => question_manager::GROUP_OTHER,
                     'type' => get_string('questiontypefixedimage', 'shezar_question'));
    }
    ...
    public function define_get(stdClass $toform) {
        ...
        $toform = file_prepare_standard_filemanager($toform, 'image', $options, $options['context'], 'shezar_'.$this->prefix,
            'quest_'.$this->id, 0);
    ...
    public function define_set(stdClass $fromform) {
        ...
        $fromform = file_postupdate_standard_filemanager($fromform, 'image', $options, $options['context'], 'shezar_'.$this->prefix,
                'quest_'.$this->getid(), 0);
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        if ($readonly) {
            ...
            $form->addElement('filemanager', 'image_filemanager', get_string('image', 'shezar_question'), null, $options);
    ...
    public function add_field_specific_view_elements(MoodleQuickForm $form) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'shezar_'.$this->prefix, 'quest_'.$this->id);
        foreach ($files as $draftfile) {
            if (!$draftfile->is_directory()) {
                $path = '/'.$contextid.'/'.'shezar_'.$this->prefix.'/'.'quest_'.$this->id.$draftfile->get_filepath().
    ...
    public function delete() {
        global $FILEPICKER_OPTIONS;
        $fs = get_file_storage();
        $fs->delete_area_files($FILEPICKER_OPTIONS['context']->id, 'shezar_'.$this->prefix, 'quest_'.$this->id, 0);
        parent::delete();
    }
