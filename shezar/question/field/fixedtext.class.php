...
class question_fixedtext extends question_base{
    public static function get_info() {
        return array('group' => question_manager::GROUP_OTHER,
                     'type' => get_string('questiontypefixedtext', 'shezar_question'));
    }
    ...
    public function define_set(stdClass $fromform) {
        ...
        $fromform = file_postupdate_standard_editor($fromform, 'fixedtext', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                'shezar_'.$this->prefix, 'quest_'.$this->getid(), 0);
        $this->param1 = $fromform->fixedtext;
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'shezar_'.$this->prefix, 'draft');
        return $fromform;
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        if ($readonly) {
            ...
            $form->addElement('editor', 'fixedtext_editor', get_string('questiontypefixedtext', 'shezar_question'), null, $options);
    ...
    public function add_field_specific_view_elements(MoodleQuickForm $form) {
        ...
        $fixedtext_editor = file_rewrite_pluginfile_urls($this->param1, 'pluginfile.php',
                $TEXTAREA_OPTIONS['context']->id, 'shezar_'.$this->prefix, 'quest_'.$this->getid(), 0, $TEXTAREA_OPTIONS);
    ...
    public function delete() {
        global $TEXTAREA_OPTIONS;
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'shezar_'.$this->prefix, 'quest_'.$this->id, 0);
        parent::delete();
    }
