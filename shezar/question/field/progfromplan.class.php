...
class question_progfromplan extends review {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_REVIEW,
                     'type' => get_string('questiontypeprogfromplan', 'shezar_question'));
    }

    public function __construct($storage, $subjectid = 0, $answerid = 0) {
        $this->buttonlabel = get_string('chooseprogfromplanreview', 'shezar_question');
    ...
    public static function check_enabled() {
        ...
        return !shezar_feature_disabled('learningplans') && !shezar_feature_disabled('programs');
    }
            ...
            foreach ($items as $item) {
                if (!isset($item->fullname) || !isset($item->planname)) {
                    $item->fullname = html_writer::tag('em',
                            get_string('reviewprogfromplanassignmissing', 'shezar_question'));
                    $item->ismissing = true;
                }
            }
            return $items;
