...
class question_compfromplan extends reviewrating {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_REVIEW,
                     'type' => get_string('questiontypecompfromplan', 'shezar_question'));
    }

    public function __construct($storage, $subjectid = 0, $answerid = 0) {

        $this->buttonlabel = get_string('choosecompfromplanreview', 'shezar_question');
    ...
    public static function check_enabled() {
        ...
        return !shezar_feature_disabled('learningplans') && !shezar_feature_disabled('competencies');
    }
    ...
    protected function add_rating_selector(MoodleQuickForm $form, $item) {
        ...
        if (isset($comprecord) && isset($comprecord->proficiency) && $comprecord->proficiency > 0) {
            ...
            $scalevaluename = get_string('notset', 'shezar_hierarchy');
        }
        ...
        if (!$this->viewonly && $this->can_update_competency($item->itemid) === true) {
            ...
            $options = array();
            $options[0] = get_string('notset', 'shezar_hierarchy');
            ...
            $form->addElement('select', $name, get_string('competencystatus', 'shezar_question'), $options,
                    array('class' => 'rating_selector rating_item_compfromplan_' . $item->itemid));
            $form->setDefault($name, $scalevalueid);
        } else {
            $form->addElement('static', '', get_string('competencystatus', 'shezar_question'), $scalevaluename);
        }
    }
            ...
            foreach ($items as $item) {
                if (!isset($item->fullname) || !isset($item->planname)) {
                    $item->fullname = html_writer::tag('em',
                            get_string('reviewcompfromplanassignmissing', 'shezar_question'));
                }
            }
            return $items;
        } else {
    ...
    public function update_competency($compassignid, $scalevalueid) {
        ...
        $jobassignment = \shezar_job\job_assignment::get_first($this->subjectid, false);
        if ($jobassignment) {
