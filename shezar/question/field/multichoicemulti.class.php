...
class question_multichoicemulti extends multichoice {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_QUESTION,
                     'type' => get_string('questiontypemultichoicemulti', 'shezar_question'));
    }
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        // Add select type.
        $list = array();
        $list[] = $form->createElement('radio', 'list', '', get_string('multichoicecheck', 'shezar_question'),
                self::MENU_CHECKBOX);
        $list[] = $form->createElement('radio', 'list', '', get_string('multichoicemultimenu', 'shezar_question'),
                self::MENU_SELECT);
    ...
    public function add_field_specific_edit_elements(MoodleQuickForm $form) {
        ...
        if ($this->required) {
            $form->addRule($this->get_prefix_form(), get_string('error:selectatleastone', 'shezar_question'), 'required');
        }
    }
    ...
    public function edit_validate($fromform) {
        ...
        if ($this->required) {
            ...
            if (!$found) {
                $errors[$this->get_prefix_form()] = get_string('error:selectatleastone', 'shezar_question');
            }
        }
