...
M.shezar_question_multichoice = M.shezar_question_multichoice || {
  Y : null,
  ...
  init : function(Y, args) {
    ...
    if ( typeof $ === 'undefined') {
      throw new Error('M.shezar_question_multichoice.init()-> jQuery dependency required for this module to function.');
    }

    var savedchoices = M.shezar_question_multichoice.config.savedchoices;
    var numVisible = 0;
    var $container = $('#id_'+M.shezar_question_multichoice.config.jsid);
    var max = M.shezar_question_multichoice.config.max;
    ...
    $allOptions.each(function(){
        ...
        if ($($this.find('span')).length > 0) {
            ...
            var $makeDefaultLink = $('<a href="#">' + M.util.get_string('defaultmake', 'shezar_question') + '</a>');
            var $unselect = $('<span class="unselectlink">' + M.util.get_string('defaultselected', 'shezar_question') + ' </span>').hide();
            var $unselectLink = $('<a href="#">' + M.util.get_string('defaultunselect', 'shezar_question') + '</a>');

            $makeDefault.append($makeDefaultLink);
            $makeDefaultLink.on('click', function(){
                if (M.shezar_question_multichoice.config.oneAnswer == 1) {
