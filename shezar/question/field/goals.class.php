...
class question_goals extends reviewrating {
    ...
    public static function get_info() {
        return array('group' => question_manager::GROUP_REVIEW,
                     'type' => get_string('questiontypegoals', 'shezar_question'));
    }

    public function __construct($storage, $subjectid = 0, $answerid = 0) {
        $this->buttonlabel = get_string('choosegoalsreview', 'shezar_question');
    ...
    protected function add_field_specific_settings_elements(MoodleQuickForm $form, $readonly, $moduleinfo) {
        ...
        $selection[] = $form->createElement('static', 'selectcompanyheader', null,
                html_writer::tag('b', get_string('goalscompany', 'shezar_question')));
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanyusercanchoose', 'shezar_question'), self::SELECT_COMPANY_USERCANCHOOSE);
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanyaddall', 'shezar_question'), self::SELECT_COMPANY_ADDALL);
        $selection[] = $form->createElement('radio', 'selectcompany', null,
                get_string('selectcompanydonotreview', 'shezar_question'), 0);

        // Personal.
        $selection[] = $form->createElement('static', 'selectpersonalheader', null,
                html_writer::tag('b', get_string('goalspersonal', 'shezar_question')));
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonalusercanchoose', 'shezar_question'), self::SELECT_PERSONAL_USERCANCHOOSE);
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonaladdall', 'shezar_question'), self::SELECT_PERSONAL_ADDALL);
        $selection[] = $form->createElement('radio', 'selectpersonal', null,
                get_string('selectpersonaldonotreview', 'shezar_question'), 0);

        $form->addGroup($selection, 'selection', get_string('goalselection', 'shezar_question'),
                array('<br/>'), true);
        $form->addRule('selection', null, 'required');

        $form->addElement('advcheckbox', 'includepersonal', get_string('reviewincludepersonal', 'shezar_question'));
    ...
    public function define_validate($data, $files) {
        ...
        if (!$selection) {
            $err['selection'] = get_string('error:allowselectgoals', 'shezar_question');
        }
    ...
    protected function add_rating_selector(MoodleQuickForm $form, $item) {
        ...
        if (($scalevalueid == 0) || ($item->scope == goal::SCOPE_PERSONAL && $goalitem->scaleid == 0)) {
            // There is no scale.
            $form->addElement('static', '', get_string('goalstatus', 'shezar_question'),
                    html_writer::tag('em', get_string('goalhasnoscale', 'shezar_question')));
        } else {
            ...
            if (!$this->viewonly && $caneditstatus) {
                ...
                $form->addElement('select', $name, get_string('goalstatus', 'shezar_question'), $options,
                        array('class' => 'rating_selector rating_item_goal_' . $item->itemid . '_' . $item->scope));
                $form->setDefault($name, $scalevalueid);
            } else {
                $form->addElement('static', '', get_string('goalstatus', 'shezar_question'), format_string($scalevalue->name));
            }
        }
    }
            ...
            foreach ($items as $item) {
                if (!isset($item->fullname)) {
                    $item->fullname = html_writer::tag('em',
                            get_string('reviewgoalsassignmissing', 'shezar_question'));
                    $item->ismissing = true;
                }
            }
            return $items;
