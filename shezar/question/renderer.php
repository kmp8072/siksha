...
/**
 * Output renderer for shezar_question module
 */
class shezar_question_renderer extends plugin_renderer_base {
    ...
    public function add_review_item($form, $itemgroup, review $review) {
        ...
        if ($review->can_select_items() && $review->can_delete_item($itemgroup)) {
            $deleteurl = new moodle_url("/shezar/$prefix/ajax/removeitem.php",
        ...
        if (isset($anyitem->planname)) {
            ...
            $title = get_string('reviewnamewithplan', 'shezar_question', $a);
        } else {
        ...
        if ($review->cananswer) {
            if (!empty($review->viewers)) {
                $viewersstring = get_string('visibleto', 'shezar_question', implode(', ', $review->viewers));
                ...
                if ($multifield) {
                    ...
                    foreach ($scalevalues as $scalevalue) {
                        ...
                        if ($currentuseritems[$scalevalue->id]->content != '') {
                            ...
                            $content .= html_writer::tag('em', get_string('notanswered', 'shezar_question'));
                        }
                    }
                } else {
                    $content = format_string($currentuseritems[0]->content);
                }
                $form->addElement(new MoodleQuickForm_static('', get_string('youranswer', 'shezar_question'), $content));
            } else {
                if ($multifield) {
                    $youranswerlabel = get_string('youranswer', 'shezar_question');
                    foreach ($scalevalues as $scalevalue) {
                    ...
                    $formelement = $form->addElement(
                            ...
                            get_string('youranswer', 'shezar_question'), $text_area_options));
                }
            }
        }
