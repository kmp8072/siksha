...
abstract class question_base {
    ...
    public function add_settings_form_elements(MoodleQuickForm $form, $readonly = false, $info = null) {
        if ($this->requires_name()) {
            if ($readonly) {
                $form->addElement('static', 'name', get_string('question', 'shezar_question'), 'size="50"');
            } else {
                $form->addElement('text', 'name', get_string('question', 'shezar_question'), 'size="50"');
                $form->addRule('name', get_string('maximumchars', '', 255), 'maxlength', 255, 'client');
                $form->addRule('name', null, 'required');
                $form->setType('name', PARAM_TEXT);
            }
            $form->addHelpButton('name', 'question', 'shezar_question');
        }
    ...
    public function define_validate_all($data, $files) {
        ...
        if ($this->requires_name()) {
            if (empty($data->name)) {
                $err['name'] = get_string('customfieldrequired', 'shezar_customfield');
            }
        }
    ...
    public function add_field_form_elements(MoodleQuickForm $form) {
        ...
        if ($this->cananswer) {
            if (!empty($this->viewers)) {
                $viewersstring = get_string('visibleto', 'shezar_question', implode(', ', $this->viewers));
...
class MoodleQuickForm_staticcallback extends MoodleQuickForm_static {
    ...
    public function __construct($elementname = null, $elementlabel = null, $callback = null) {
        ...
        $this->_text = html_writer::tag('em', get_string('notanswered', 'shezar_question'));
    }
...
MoodleQuickForm::registerElementType('staticcallback', "$CFG->dirroot/shezar/question/lib.php", 'MoodleQuickForm_staticcallback');
