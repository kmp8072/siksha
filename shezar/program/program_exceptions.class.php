...
class prog_exceptions_manager {
    ...
    function __construct($programid) {
        ...
        $this->exceptiontype_descriptors = array(
            EXCEPTIONTYPE_TIME_ALLOWANCE    => get_string('timeallowance', 'shezar_program'),
            EXCEPTIONTYPE_ALREADY_ASSIGNED  => get_string('currentlyassigned', 'shezar_program'),
            EXCEPTIONTYPE_COMPLETION_TIME_UNKNOWN  => get_string('completiontimeunknown', 'shezar_program'),
            EXCEPTIONTYPE_UNKNOWN => get_string('unknownexception', 'shezar_program'),
            EXCEPTIONTYPE_DUPLICATE_COURSE  => get_string('duplicatecourse', 'shezar_program')
        );
    ...
    public function override_dismissed_exception($userid) {
        ...
        if (!empty($assignments)) {
            ...
            throw new ProgramExceptionException(get_string('error:assignmentnotfound', 'shezar_program'));
        }
    ...
    public function handle_exception($exceptionid, $action) {
        ...
        if (!$exception = $DB->get_record('prog_exception', array('id' => $exceptionid))) {
            throw new ProgramExceptionException(get_string('exceptionnotfound', 'shezar_program'));
        }

        if (!array_key_exists($exceptiontype, $this->exceptiontype_classnames)) {
            throw new ProgramExceptionException(get_string('exceptiontypenotfound', 'shezar_program'));
        }
