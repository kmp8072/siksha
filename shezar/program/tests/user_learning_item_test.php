...
class shezar_program_user_learning_item_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        // See note in shezar/program/tests/program_content_test.php for more info.
    ...
    public function test_all_programs() {
        ...
        $program_items = \shezar_program\user_learning\item::all($this->user1->id);
        ...
        require_once($CFG->dirroot . '/shezar/program/classes/user_learning/item.php');

        // Reflection property so we can access program.
        $rp = new ReflectionProperty('shezar_program\user_learning\item', 'program');
    ...
    public function test_one_program() {
        ...
        $program_item = \shezar_program\user_learning\item::one($this->user1->id, $this->program2->id);
    ...
    public function test_get_courseset_courses_multicourseset() {
        ...
        $program_item = \shezar_program\user_learning\item::one($this->user1->id, $this->program1->id);

        // We need to manually include the file for reflection.
        require_once($CFG->dirroot . '/shezar/program/classes/user_learning/item.php');

        $rm = new ReflectionMethod('shezar_program\user_learning\item', 'get_courseset_courses');
    ...
    public function test_ensure_program_loaded() {
        ...
        $rm = new ReflectionMethod('shezar_program\user_learning\item', 'ensure_program_loaded');
        $rm->setAccessible(true);
    ...
    public function test_ensure_course_sets_loaded() {
        ...
        $rp = new ReflectionProperty('shezar_program\user_learning\item', 'coursesets');
        $rp->setAccessible(true);
        // Force to be null, so we can test the function.
        $rp->setValue($program_item, null);

        $rm = new ReflectionMethod('shezar_program\user_learning\item', 'ensure_course_sets_loaded');
        $rm->setAccessible(true);
    ...
    public function test_ensure_completion_loaded() {
        ...
        $progress_canbecompleted = new ReflectionProperty('shezar_program\user_learning\item', 'progress_canbecompleted');
        $progress_canbecompleted->setAccessible(true);

        $progress_percentage = new ReflectionProperty('shezar_program\user_learning\item', 'progress_percentage');
        $progress_percentage->setAccessible(true);

        $progress_summary = new ReflectionProperty('shezar_program\user_learning\item', 'progress_summary');
        ...
        $rm = new ReflectionMethod('shezar_program\user_learning\item', 'ensure_completion_loaded');
        $rm->setAccessible(true);
    ...
    public function test_ensure_duedate_loaded() {
        ...
        $rp = new ReflectionProperty('shezar_program\user_learning\item', 'duedate');
        $rp->setAccessible(true);

        $this->assertEmpty($rp->getValue($program_item));

        $rm = new ReflectionMethod('shezar_program\user_learning\item', 'ensure_duedate_loaded');
        $rm->setAccessible(true);
    ...
    public function test_export_dueinfo_for_template() {
        ...
        $duedateformatted = userdate($futuredate, get_string('strftimedateshortshezar', 'langconfig'));
        $tooltipdate = userdate($futuredate, get_string('strftimedatetimeon', 'langconfig'));
        $duetooltipformat = get_string('programduex', 'shezar_program', $tooltipdate);

        $expected = new stdClass();
        $expected->duetext = get_string('userlearningdueonx', 'shezar_core', $duedateformatted);
        ...
        $expected = new stdClass();
        $expected->duetext = get_string('userlearningoverduesincex', 'shezar_core', $duedateformatted);
    ...
    function test_process_coursesets_1() {
        ...
        require_once($CFG->dirroot . '/shezar/program/program_courseset.class.php'); // Needed for the constants.
