...
$capabilities = array('shezar/program:createprogram', 'moodle/category:manage');
...
$PAGE->set_url('/shezar/program/search.php', $searchcriteria + $urlparams);
...
if ($viewtype == 'program') {
    $strprograms = new lang_string('programs', 'shezar_program');
    $strprogram = new lang_string('program', 'shezar_program');
} else {
    $strprograms = new lang_string('certifications', 'shezar_certification');
    $strprogram = new lang_string('certification', 'shezar_certification');
}
$strsearch = new lang_string('search');
$strsearchresults = new lang_string('searchresults');

$PAGE->navbar->add($strprograms, new moodle_url('/shezar/program/index.php'));
$PAGE->navbar->add($strsearch, new moodle_url('/shezar/program/search.php'));
if (!empty($search)) {
...
if (empty($searchcriteria)) {
    ...
    if ((can_edit_in_category() || !empty($usercatlist))) {
        $aurl = new moodle_url('/shezar/program/manage.php', $searchcriteria);
        $managestring = ($viewtype == 'program') ? get_string('manageprograms', 'admin') : get_string('managecertifications', 'shezar_core');
