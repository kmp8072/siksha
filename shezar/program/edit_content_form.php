...
class program_content_edit_form extends moodleform {
    ...
    function add_errors() {
        ...
        if (!empty($mform->_errors)) {
            ...
            $mform->setConstant('errors', get_string('errorsinform', 'shezar_program'));
    ...
    function validation($data, $files) {
        ...
        foreach ($data as $elementname=>$elementvalue) {
            ...
            if (preg_match('/[0-9]timeallowednum/', $elementname)) {
                ...
                if ($timeallowednum < 0) {
                    $errors[$elementname] = get_string('error:timeallowednum_nonzero', 'shezar_program');
                }
            }
            ...
            if (preg_match('/[0-9]recurrencetimenum/', $elementname)) {
                $timeallowednum = $elementvalue;
                if ($timeallowednum <= 0) {
                    $errors[$elementname] = get_string('error:recurrence_nonzero', 'shezar_program');
                }
            }
            ...
            if (preg_match('/[0-9]recurcreatetimenum/', $elementname)) {
                $timeallowednum = $elementvalue;
                if ($timeallowednum <= 0) {
                    $errors[$elementname] = get_string('error:coursecreationrepeat_nonzero', 'shezar_program');
                }
            }
            ...
            if (preg_match('/[0-9]courses$/', $elementname)) {
                ...
                if (empty($courses)) {
                    $errors[$elementname] = get_string('error:courses_nocourses', 'shezar_program');
                }
            }
        }
        return $errors;
