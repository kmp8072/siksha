...
class program_edit_form extends moodleform {
    ...
    function definition() {
        ...
        if ($action == 'delete') {
            ...
            $mform->addElement('html', get_string('checkprogramdelete', 'shezar_program', $program->fullname));
            $buttonarray = array();
...
//--------------------------------------------------------------------------------
        $mform->addElement('header','programdetails', get_string('programdetails', 'shezar_program'));

        if ($action == 'edit') {
            $mform->addElement('html', html_writer::start_tag('p', array('class' => 'instructions')) . get_string('instructions:programdetails', 'shezar_program') . html_writer::end_tag('p'));
        }

        // Must have create program capability in both categories in order to move program
        if (has_capability('shezar/program:createprogram', $context)) {
            $displaylist = array();
            $attributes = array();
            $attributes['class'] = 'shezar-limited-width';
            $displaylist = coursecat::make_categories_list('shezar/program:createprogram');
            $mform->addElement('select', 'category', get_string('category', 'shezar_program'), $displaylist, $attributes);
        ...
        // (as programs are treated like courses in this respect)
            ...
            $mform->addHelpButton('category', 'programcategory', 'shezar_program');
            $mform->setDefault('category', $category->id);
        }

        $mform->addElement('text','fullname', get_string('fullname', 'shezar_program'),'maxlength="254" size="50"');
        $mform->setType('fullname', PARAM_TEXT);
        if ($action == 'view') {
            $mform->hardFreeze('fullname');
        } else {
            $mform->addHelpButton('fullname', 'programfullname', 'shezar_program');
            if ($iscertif) {
                $mform->setDefault('fullname', get_string('defaultcertprogramfullname', 'shezar_certification'));
            } else {
                $mform->setDefault('fullname', get_string('defaultprogramfullname', 'shezar_program'));
            }
            $mform->addRule('fullname', get_string('missingfullname'), 'required', null, 'client');

        }

        $mform->addElement('text','shortname', get_string('shortname', 'shezar_program'),'maxlength="100" size="20"');
        $mform->setType('shortname', PARAM_TEXT);
        if ($action=='view') {
            $mform->hardFreeze('shortname');
        } else {
            $mform->addHelpButton('shortname', 'programshortname', 'shezar_program');
            if ($iscertif) {
                $mform->setDefault('shortname', get_string('defaultcertprogramshortname', 'shezar_certification'));
            } else {
                $mform->setDefault('shortname', get_string('defaultprogramshortname', 'shezar_program'));
            }
            $mform->addRule('shortname', get_string('missingshortname', 'shezar_program'), 'required', null, 'client');
        }

        $mform->addElement('text','idnumber', get_string('idnumberprogram', 'shezar_program'),'maxlength="100"  size="10"');
        $mform->setType('idnumber', PARAM_TEXT);
        if ($action == 'view') {
            $mform->hardFreeze('idnumber');
        } else {
            $mform->addHelpButton('idnumber', 'programidnumber', 'shezar_program');
        }

        $mform->addElement('date_selector', 'availablefrom', get_string('availablefrom', 'shezar_program'), array('optional' => true));
        $mform->setType('availablefrom', PARAM_INT);
        if ($action == 'view') {
            $mform->hardFreeze('availablefrom');
        } else {
            $mform->addHelpButton('availablefrom', 'programavailability', 'shezar_program');
        }

        $mform->addElement('date_selector', 'availableuntil', get_string('availableuntil', 'shezar_program'), array('optional' => true));
        $mform->setType('availableuntil', PARAM_INT);
        if ($action == 'view') {
            $mform->hardFreeze('availableuntil');
        } else {
            $mform->addHelpButton('availableuntil', 'programavailability', 'shezar_program');
        }

        if ($action == 'view') {
            if ($program) {
                $summary = file_rewrite_pluginfile_urls($program->summary, 'pluginfile.php',
                    $context->id, 'shezar_program', 'summary', 0);
                if (!empty($summary)) {
                    $mform->addElement('static', null, get_string('description', 'shezar_program'), $summary);
                }
            }
        } else {
            $mform->addElement('editor', 'summary_editor', get_string('description', 'shezar_program'), null, $editoroptions);
            $mform->addHelpButton('summary_editor', 'summary', 'shezar_program');
            ...
            if ($action == 'view') {
                if (!empty($overviewfiles)) {
                    ...
                    if (!empty($contentfiles)) {
                        $mform->addElement('static', null, get_string('programoverviewfiles', 'shezar_program'), $contentfiles);
                    }
                }
            } else {
                $mform->addElement('filemanager', 'overviewfiles_filemanager',
                    get_string('programoverviewfiles', 'shezar_program'), null, $overviewfilesoptions);
                $mform->addHelpButton('overviewfiles_filemanager', 'programoverviewfiles', 'shezar_program');
            }
        }
        ...
        if ($action == 'view') {
            if ($program) {
                $endnote = file_rewrite_pluginfile_urls($program->endnote, 'pluginfile.php',
                    $context->id, 'shezar_program', 'endnote', 0);
                if (!empty($endnote)) {
                    $mform->addElement('static', null, get_string('endnote', 'shezar_program'), $endnote);
                }
            }
        } else {
            $mform->addElement('editor', 'endnote_editor', get_string('endnote', 'shezar_program'), null, $editoroptions);
            $mform->addHelpButton('endnote_editor', 'endnote', 'shezar_program');
        ...
        if (empty($CFG->audiencevisibility)) {
            if ($action == 'view') {
                $mform->addElement('static', 'visibledisplay', get_string('visible', 'shezar_program'), $program->visible ? get_string('yes') : get_string('no'));
            } else {
                $mform->addElement('advcheckbox','visible', get_string('visible', 'shezar_program'), null, null, array(0, 1));
                $mform->addHelpButton('visible', 'programvisibility', 'shezar_program');
            ...
            if ($action == 'view') {
                $mform->addElement('header', 'visiblecohortshdr', get_string('audiencevisibility', 'shezar_cohort'));
                $mform->addElement('static', 'visibledisplay', get_string('audiencevisibility', 'shezar_cohort'), $COHORT_VISIBILITY[$program->audiencevisible]);
                $cohorts = shezar_cohort_get_visible_learning($program->id, $instancetype);
                if (!empty($cohorts)) {
                    $cohortsclass = new shezar_cohort_visible_learning_cohorts();
                ...
                if (has_capability('shezar/coursecatalog:manageaudiencevisibility', $context)) {
                    $mform->addElement('header', 'visiblecohortshdr', get_string('audiencevisibility', 'shezar_cohort'));
                    $mform->addElement('select', 'audiencevisible', get_string('visibility', 'shezar_cohort'), $COHORT_VISIBILITY);
                    $mform->addHelpButton('audiencevisible', 'visiblelearning', 'shezar_cohort');
                    ...
                    $mform->addElement('button', 'cohortsaddvisible', get_string('cohortsaddvisible', 'shezar_cohort'));
                    $mform->setExpanded('visiblecohortshdr');
                }
            }
        }

        //replacement for old shezar/core/icon classes
        $programicon = ($program && !empty($program->icon)) ? $program->icon : 'default';
        shezar_add_icon_picker($mform, $action, 'program', $programicon, $nojs, false);
        ...
        if (!empty($CFG->enableprogramextensionrequests)) {
            $label = get_string('allowextensionrequests', 'shezar_program');
            $mform->addElement('header','othersettings', get_string('othersettings', 'shezar_program'));
            ...
            if ($action == 'view') {
                ...
                $mform->addHelpButton('allowextensionrequests', 'allowextensionrequests', 'shezar_program');
            }
        ...
        if (in_array($action, array('add', 'edit'))) {
            ...
            if (!empty($customfields)) {
                $mform->addElement('header', 'customfields', get_string('customfields', 'shezar_customfield'));
        ...
        if ($action == 'add') {
            ...
            $buttonarray[] = $mform->createElement('cancel', 'cancel', get_string('cancel', 'shezar_program'), 'class="program-cancel"');
            $mform->addGroup($buttonarray, 'buttonar', '', array(' '), false);
            $mform->closeHeaderBefore('buttonar');
        } else if ($action == 'edit') {
            $buttonarray = array();
    ...
    function validation($data, $files) {
        ...
        if ($data['availablefrom'] != 0 && $data['availableuntil'] != 0) {
            if ($data['availablefrom'] > $data['availableuntil']) {
                $errors['availableuntil'] = get_string('error:availibileuntilearlierthanfrom', 'shezar_program');
            }
        }

        $id = isset($data['id']) ? $data['id'] : 0;
        if (!empty($data['idnumber']) && shezar_idnumber_exists('prog', $data['idnumber'], $id)) {
...
class program_edit_details_button_form extends moodleform {
    ...
    function definition() {
        ...
        $button = $OUTPUT->single_button(new moodle_url($this->_form->getAttribute('action')),
            get_string('editprogramdetails', 'shezar_program'), 'get');
...
class program_content_nonedit_form extends moodleform {
    ...
    function definition() {
       ...
       $mform->addElement('header','programcontent', get_string('programcontent', 'shezar_program'));

       // Get the total time allowed for this certification/program
       if ($program->certifid) {
           $this->display_course_sets($mform, $content, CERTIFPATH_CERT, get_string('oricertpath', 'shezar_certification'));
           $this->display_course_sets($mform, $content, CERTIFPATH_RECERT, get_string('recertpath', 'shezar_certification'));
       } else {
           $this->display_course_sets($mform, $content, CERTIFPATH_STD, '');
       }

        // Check capabilities.
        if (has_capability('shezar/program:configurecontent', $program->get_context())) {
            $button = $OUTPUT->single_button(new moodle_url($this->_form->getAttribute('action'), array('id' => $program->id)),
                get_string('editprogramcontent', 'shezar_program'), 'get');
    ...
    function display_course_sets(&$mform, $content, $certifpath, $formlabel) {
        ...
        if (count($coursesets)) {
            ...
            $mform->addElement('static', 'progcontent', '', get_string('nocontent', 'shezar_program'));
       }
    }
    ...
    function display_time_allowed(&$mform, $content, $certifpath) {
        ...
        if ($total_time_allowed > 0) {
            ...
            $timeallowedstr = html_writer::tag('p', get_string('allowtimeforprogram' . $numperiod->periodkey, 'shezar_program', $numperiod->num), array('class' => 'timeallowed'));
...
// form definition
//--------------------------------------------------------------------------------
        $mform->addElement('header','programassignments', get_string('programassignments', 'shezar_program'));
        ...
        // Check capabilities
        if (has_capability('shezar/program:configureassignments', $program->get_context())) {
            $button = $OUTPUT->single_button(new moodle_url($this->_form->getAttribute('action'), array('id' => $program->id)),
                get_string('editprogramassignments', 'shezar_program'), 'get');
...
// form definition
//--------------------------------------------------------------------------------
        $mform->addElement('header','programmessages', get_string('programmessages', 'shezar_program'));
        ...
        // Check capabilities
        if (has_capability('shezar/program:configuremessages', $program->get_context())) {
            $button = $OUTPUT->single_button(new moodle_url($this->_form->getAttribute('action'), array('id' => $program->id)),
                get_string('editprogrammessages', 'shezar_program'), 'get');
...
class program_certifications_nonedit_form extends moodleform {
    ...
    function definition() {
        ...
        // form definition
        //--------------------------------------------------------------------------------
        $mform->addElement('header','hdrcertification', get_string('certification', 'shezar_certification'));

        $certification = $DB->get_record('certif', array('id' => $program->certifid));
        if (!$certification || $certification->activeperiod == 0) {
            $mform->addElement('static', 'el1', '' , get_string('nocertifdetailsfound', 'shezar_certification'));
        } else {
            $parts = explode(' ', $certification->activeperiod);
            $mform->addElement('static', 'el2', get_string('editdetailsactive', 'shezar_certification'),
                            $parts[0] . ' ' . mb_strtolower(get_string($parts[1].'s', 'shezar_certification'), 'UTF-8'));
            $parts = explode(' ', $certification->windowperiod);
            $mform->addElement('static', 'el3', get_string('editdetailswindow', 'shezar_certification'),
                            $parts[0] . ' ' . mb_strtolower(get_string($parts[1].'s', 'shezar_certification'), 'UTF-8'));
            $mform->addElement('static', 'el4', get_string('editdetailsrcopt', 'shezar_certification'),
                            $CERTIFRECERT[$certification->recertifydatetype]);
        }

        // Check capabilities
        if (has_capability('shezar/certification:configurecertification', $program->get_context())) {
            $button = $OUTPUT->single_button(new moodle_url($this->_form->getAttribute('action'),
                 array('id' => $program->id)), get_string('editcertification', 'shezar_certification'), 'get');
...
//--------------------------------------------------------------------------------
        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'delete', get_string('deleteprogrambutton', 'shezar_program'));
        $mform->addGroup($buttonarray, 'buttonar', '', array(' '), false);
