 ...
 * @subpackage program
 */
M.shezar_programcontent = M.shezar_programcontent || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.shezar_programcontent.init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_programcontent.init()-> jQuery dependency required for this module to function.');
        }

        /**
         * Dialog and dialog handler definitions, shezarDialog_program_cat is
        ...
        */
        ...
        // Create handler
        var shezarDialog_handler_addmulticourse = function(contenturl) {
            this.contenturl = contenturl;
        };

        // Set the dialog handler as a multi select - shezar/core/js/lib/shezar_dialog.js
        shezarDialog_handler_addmulticourse.prototype = new shezarDialog_handler_treeview_multiselect();


        // Adapt the handler's save function
        shezarDialog_handler_addmulticourse.prototype._save = function() {
            ...
            if (courseids.length === 0) {
                alert(M.util.get_string('error:courses_nocourses', 'shezar_program'));
                return false;
            }
        ...
        // Define the dialog
        var shezarDialog_addmulticourse = function() {

            this.url = M.cfg.wwwroot+'/shezar/program/content/';
            this.find_url = 'find_courses.php?id='+module.config.id;
            this.ajax_url = 'get_html.php?id='+module.config.id;

            // Setup the handler
            var handler = new shezarDialog_handler_addmulticourse(this.url + this.ajax_url);
            ...
            // Call the parent dialog object and link us (shezar_dialog.js)
            shezarDialog.call(
                this,
                ...
                {                 // config
                    buttons: buttonsObj,
                    title: '<h2>'+M.util.get_string('addcourseset', 'shezar_program')+'</h2>',
                },
                default_url,
                handler
            );
        ...
        // Create handler
        var shezarDialog_handler_addcompetency = function(contenturl) {
            this.contenturl = contenturl;
        };

        // Set the dialog handler as a single select
        shezarDialog_handler_addcompetency.prototype = new shezarDialog_handler_treeview_singleselect();

        // Adapt the handler's save function
        shezarDialog_handler_addcompetency.prototype._save = function() {

            // retrieve the competency id
            var id = $('#treeview_selected_val_addcompetency').val();
            if (id === "0") {
                alert(M.util.get_string('error:nocompetency', 'shezar_program'));
                return false;
            }
        ...
        // Define the dialog
        var shezarDialog_addcompetency = function() {
            ...
            // Setup the handler
            var handler = new shezarDialog_handler_addcompetency(this.url + this.ajax_url);
            ...
            shezarDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcompetency', 'shezar_program') + module.config.display_selected_addcompetency +'</h2>'
                },
                default_url,
                handler
            );
        ...
        // Create handler
        shezarDialog_handler_addrecurringcourse = function(contenturl) {
            this.contenturl = contenturl;
        }

        // Set the dialog handler as a single select
        shezarDialog_handler_addrecurringcourse.prototype = new shezarDialog_handler_treeview_singleselect();

        // Adapt the handler's save function
        shezarDialog_handler_addrecurringcourse.prototype._save = function() {

            // retrieve the course id
            var courseid = $('#treeview_selected_val_addrecurringcourse').val();
            if (courseid === "0") {
                alert(M.util.get_string('error:recur_nocourse', 'shezar_program'));
                return false;
            }
        ...
        // Define the dialog
        shezarDialog_addrecurringcourse = function() {
            ...
            // Setup the handler
            var handler = new shezarDialog_handler_addrecurringcourse(this.url + this.ajax_url);
            ...
            shezarDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcourse', 'shezar_program') + module.config.display_selected_addrecurringcourse +'</h2>'
                },
                default_url,
                handler
            );
        ...
        shezarDialog_handler_amendmulticourse = function(contenturl) {
            this.contenturl = contenturl;
        ...
        // Set the dialog handler as a multiselect (./shezar/core/js/lib/shezar_dialog.js)
        shezarDialog_handler_amendmulticourse.prototype = new shezarDialog_handler_treeview_multiselect();


        // Adapt the handler's save function
        shezarDialog_handler_amendmulticourse.prototype._save = function() {
        ...
        // Define the dialog
        shezarDialog_amendmulticourse = function() {

            var url = M.cfg.wwwroot+'/shezar/program/content/';
            ...
            // Setup the handler
            this.handler = new shezarDialog_handler_amendmulticourse(url + this.ajax_url);

            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('ok','shezar_program')] = function() { self.handler._save(); };
            buttonsObj[M.util.get_string('cancel', 'shezar_program')] = function() { self.handler._cancel(); };
            ...
            shezarDialog.call(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+ M.util.get_string('addcourses', 'shezar_program') +'</h2>'
                },
                this.default_url,
                this.handler
            );
        ...
        shezarDialog_savechanges = function() {
            ...
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('saveallchanges','shezar_program')] = function() { self.save(); };
            buttonsObj[M.util.get_string('editcontent', 'shezar_program')] = function() { handler._cancel(); };
            ...
            shezarDialog.call(
              ...
              {
                  buttons: buttonsObj,
                  title: '<h2>'+ M.util.get_string('confirmcontentchanges', 'shezar_program') +'</h2>'
              },
        ...
        window.onbeforeunload = function(e) {
            ...
            var str = M.util.get_string('youhaveunsavedchanges', 'shezar_program');
        ...
        shezarDialogs['addmulticourse'] = new shezarDialog_addmulticourse();
        shezarDialogs['addcompetency'] = new shezarDialog_addcompetency();
        shezarDialogs['addrecurringcourse'] = new shezarDialog_addrecurringcourse();
        shezarDialogs['amendcourses'] = new shezarDialog_amendmulticourse();
        shezarDialogs['savechanges'] = new shezarDialog_savechanges();
    ...
    initCoursesets: function(){
        ...
        $('div.courseadder input:submit').val(M.util.get_string('addcourses', 'shezar_program'));
    ...
    changeCompletionTypeString: function(el, prefix){
        if (el.value == this.config.COMPLETIONTYPE_ANY || el.value == this.config.COMPLETIONTYPE_SOME || el.value == this.config.COMPLETIONTYPE_OPTIONAL) {
            var completiontypestr = M.util.get_string('or', 'shezar_program');
        } else {
            var completiontypestr = M.util.get_string('and', 'shezar_program');
        }
    ...
    addContent: function(suf){
        ...
        if (contenttype == this.config.CONTENTTYPE_MULTICOURSE) {
            shezarDialogs['addmulticourse'].suf = suf; // store on dialog object
            shezarDialogs['addmulticourse'].open();
        } else if (contenttype == this.config.CONTENTTYPE_COMPETENCY) {
            shezarDialogs['addcompetency'].open();
        } else if (contenttype == this.config.CONTENTTYPE_RECURRING) {
            shezarDialogs['addrecurringcourse'].open();
        }
    ...
    amendCourses: function(coursesetprefix){
        ...
        var dialog_url = M.cfg.wwwroot+'/shezar/program/content/find_courses.php?id='+this.config.id+'&selectedcourseids=' + selectedcourses_encoded;

        var mydialog = shezarDialogs['amendcourses'];
    ...
    deleteCourse: function(coursesetid, coursesetprefix, coursetodelete_id){
        ...
        var contenturl = M.cfg.wwwroot+'/shezar/program/content/get_html.php?id='+M.shezar_programcontent.config.id;
        ...
        $.getJSON(contenturl +
            ...
            M.shezar_programcontent.initCoursesets();
            ...
            $('.edit-program .coursedeletelink').click(function(){
                M.shezar_programcontent.deleteCourse(this.getAttribute('data-coursesetid'),
                                                     this.getAttribute('data-coursesetprefix'),
                                                     this.getAttribute('data-coursetodelete_id'));
                return true;
            });
    ...
    handleSaveChanges: function(){
        ...
        var dialog = shezarDialogs['savechanges'];
        ...
        if (window.currentassignmentcount !== undefined) {
            html = M.util.get_string('affectedusercount', 'shezar_program') + window.currentassignmentcount;
        }
        html = html + '<'+'p>' + M.util.get_string('tosavecontent', 'shezar_program') +'<'+'/p>';
        dialog.open(html);
