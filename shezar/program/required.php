...
} else if (shezar_feature_disabled('programs') &&
    shezar_feature_disabled('certifications')) {
    print_error('programsandcertificationsdisabled', 'shezar_program');
}

$PAGE->set_url('/shezar/program/required.php');
...
if (isset($program) && $program->user_is_assigned($userid)) {
    if (prog_is_accessible($program)) {
        ...
        echo html_writer::start_tag('p') . get_string('programnotcurrentlyavailable', 'shezar_program') . html_writer::end_tag('p');
    ...
    $heading = get_string('requiredlearning', 'shezar_program');
    $pagetitle = format_string(get_string('requiredlearning', 'shezar_program'));
    ...
    if ($userid == $USER->id) {
        $requiredlearninginstructions = html_writer::start_tag('div', array('class' => 'instructional_text')) . get_string('requiredlearninginstructions', 'shezar_program') . html_writer::end_tag('div');
    } else {
        $user = $DB->get_record('user', array('id' => $userid));
        $userfullname = fullname($user);
        $requiredlearninginstructions = html_writer::start_tag('div', array('class' => 'instructional_text')) . get_string('requiredlearninginstructionsuser', 'shezar_program', $userfullname) . html_writer::end_tag('div');
    }
    ...
    if (($filter == 'all' || $filter == 'program') && shezar_feature_visible('programs')) {
        echo $OUTPUT->container_start('', 'required-learning-list');
        echo $OUTPUT->heading(get_string('programs', 'shezar_program'), 3);

        $requiredlearninghtml = prog_display_required_programs($userid);

        if (empty($requiredlearninghtml)) {
            echo get_string('norequiredlearning', 'shezar_program');
        } else {
    ...
    if (($filter == 'all' || $filter == 'certification') && shezar_feature_visible('certifications')) {
        echo $OUTPUT->container_start('', 'certification-learning-list');
        echo $OUTPUT->heading(get_string('certifications', 'shezar_program'), 3);

        $certificationhtml = prog_display_certification_programs($userid);

        if (empty($certificationhtml)) {
            echo get_string('nocertificationlearning', 'shezar_program');
        } else {
