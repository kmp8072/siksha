...
$baseurl = new moodle_url('/shezar/program/exceptions.php', $pageparams);

if (!has_capability('shezar/program:handleexceptions', $programcontext)) {
...
echo $OUTPUT->heading(get_string('programexceptions', 'shezar_program'));
echo html_writer::start_tag('p') . get_string('instructions:programexceptions', 'shezar_program') . html_writer::end_tag('p');
...
$PAGE->requires->string_for_js('confirmresolution', 'shezar_program');
...
$jsmodule = array(
     'name' => 'shezar_programexceptions',
     'fullpath' => '/shezar/program/exceptions.js',
     'requires' => array('json')
     );

$PAGE->requires->js_init_call('M.shezar_programexceptions.init',$args, false, $jsmodule);
