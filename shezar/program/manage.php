...
if ($id) {
    ...
    $PAGE->set_url(new moodle_url('/shezar/program/manage.php', array('categoryid' => $id, 'viewtype' => $viewtype)));
    ...
    $PAGE->navbar->add($navname, new moodle_url('/shezar/program/index.php', array('viewtype' => $viewtype)));
    ...
    if (!can_edit_in_category($coursecat->id)) {
        redirect(new moodle_url('/shezar/program/index.php', array('viewtype' => $viewtype)));
    }
} else {
    $context = context_system::instance();
    $PAGE->set_context($context);
    $PAGE->set_url(new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype)));
...
if ($action == 'deletecategory' and confirm_sesskey()) {
    ...
    if ($mform->is_cancelled()) {
        redirect(new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype)));
    }
        ...
        if ($data->fulldelete == 1 && $cattodelete->can_delete_full()) {
            ...
            if ($viewtype == 'program') {
                foreach ($deletedprograms as $program) {
                    echo $OUTPUT->notification(get_string('programdeletesuccess', 'shezar_program', $program->shortname), 'notifysuccess');
                }
            } else {
                foreach ($deletedcertifs as $certif) {
                    echo $OUTPUT->notification(get_string('certificationdeletesuccess', 'shezar_certification', $certif->shortname),
                         'notifysuccess');
                }
            }
            echo $OUTPUT->notification(get_string('coursecategorydeleted', 'moodle', $cattodeletename), 'notifysuccess');
            echo $OUTPUT->continue_button(new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype)));
        ...
        if (preg_match('/^c\d+$/', $key)) {
            ...
            if ($movingprogram = $DB->get_record('prog', array('id' => $programid))) {
                if ($id && $movingprogram->category != $id ) {
                    print_error('error:programdoesnotbelongtocategory', 'shezar_program');
                }
            } else {
                print_error('error:prognotmoved', 'shezar_program');
            }
        }
    }
...
if ((!empty($hide) or !empty($show)) && confirm_sesskey()) {
    if (empty($CFG->audiencevisibility)) {
        ...
        require_capability('shezar/program:visibility', $programcontext);
...
if (can_edit_in_category()) {
    ...
    if ($id) {
        navigation_node::override_active_url(new moodle_url('/shezar/program/index.php', array('categoryid' => $id, 'viewtype' => $viewtype)));
    }
    $pagetype = ($viewtype == 'program') ? 'programmgmt' : 'managecertifications';
    admin_externalpage_setup($pagetype, '', $urlparams, $CFG->wwwroot . "/shezar/program/manage.php");
...
if (!empty($searchcriteria)) {
    ...
    if (!empty($CFG->enableprogramcompletioneditor) && has_capability('shezar/program:editcompletion', context_system::instance())) {
        $checkallurl = new moodle_url('/shezar/program/check_completion.php', array('progorcert' => $viewtype));
    ...
    if ($viewtype == 'program') {
        $strcategory = get_string('programcategories', 'shezar_program');
        $strtype = get_string('programs', 'shezar_program');
    } else {
        $strcategory = get_string('certifcategories', 'shezar_certification');
        $strtype = get_string('certifications', 'shezar_certification');
    }
    ...
    $select = new single_select(new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype)),
                  'categoryid', $displaylist, $coursecat->id, null, 'switchcategory');
    $strcategory = ($viewtype == 'program') ? get_string('programcategories', 'shezar_program') :
...
if ($canmanage) {
    ...
    if (has_any_capability($coursecaps, $context)) {
        $title = get_string('managecoursesinthiscat', 'shezar_program');
    }
    echo $OUTPUT->single_button($url, $title, 'get');
    if ($viewtype == 'program') {
        // Print button for switching to certification management.
        if (shezar_feature_visible('certifications')) {
            $url = new moodle_url('/shezar/program/manage.php', array('categoryid' => $id, 'viewtype' => 'certification'));
            $programcaps = array('shezar/certification:createcertification',
                                 'shezar/certification:deletecertification',
                                 'shezar/certification:configurecertification');
            if (has_any_capability($programcaps, $context)) {
                $title = get_string('managecertifsinthiscat', 'shezar_certification');
            }
            echo $OUTPUT->single_button($url, $title, 'get');
        }
    } else {
        ...
        if (shezar_feature_visible('programs')) {
            $url = new moodle_url('/shezar/program/manage.php', array('categoryid' => $id));
            $programcaps = array('shezar/program:createprogram',
                                 'shezar/program:deleteprogram',
                                 'shezar/program:configuredetails');
            if (has_any_capability($programcaps, $context)) {
                $title = get_string('manageprogramsinthiscat', 'shezar_program');
            }
            echo $OUTPUT->single_button($url, $title, 'get');
        }
    }
...
if (!empty($searchcriteria)) {
    ...
    $baseurl = new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype));
...
if (!$programs) {
    ...
    if (empty($subcategorieswereshown)) {
        echo $OUTPUT->heading(get_string('noprogramsyet', 'shezar_program'));
    }
} else {
    ...
    $strtype = ($viewtype == 'program') ? get_string('programs', 'shezar_program') : get_string('certifications', 'shezar_certification');
    $table->head = array(
                    $strtype,
                    get_string('edit'),
                    get_string('select')
    ...
    $baseurl = new moodle_url('/shezar/program/manage.php', $urlparams + array('sesskey' => sesskey(), 'viewtype' => $viewtype));
    foreach ($programs as $aprogram) {
        ...
        $programurl = new moodle_url('/shezar/program/view.php', array('id' => $aprogram->id, 'viewtype' => $viewtype));
        $attributes = array();
        $attributes['class'] = shezar_get_style_visibility($aprogram);
        $programname = format_string($aprogram->fullname);
        $programname = html_writer::link($programurl, $programname, $attributes);

        $icons = array();
        // "Update program" icon.
        $capability = ($viewtype == 'program') ? 'shezar/program:configuredetails' : 'shezar/certification:configuredetails';
        if (has_capability($capability, $programcontext)) {
            $url = new moodle_url('/shezar/program/edit.php', array('id' => $aprogram->id, 'category' => $id));
            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/edit', get_string('settings')));
        }

        // "Role assignment" icon.
        if (has_capability('shezar/program:configureassignments', $programcontext)) {
            $url = new moodle_url('/shezar/program/edit_assignments.php', array('id' => $aprogram->id));
            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/enrolusers', get_string('enrolledusers', 'enrol')));
        }

        // "Delete program" icon.
        $capability = ($viewtype == 'program') ? 'shezar/program:deleteprogram' : 'shezar/certification:deletecertification';
        if (has_capability($capability, $programcontext)) {
            $url = new moodle_url('/shezar/program/delete.php', array('id' => $aprogram->id, 'category' => $id));
        ...
        if (!empty($CFG->audiencevisibility) && has_capability('shezar/coursecatalog:manageaudiencevisibility', $context)) {
            $url = new moodle_url('/shezar/program/edit.php', array('id' => $aprogram->id, 'action' => 'edit'));
            $url->set_anchor('id_visiblecohortshdr');
            $icon = 'hide';
            if ($aprogram->audiencevisible == COHORT_VISIBLE_NOUSERS) {
                $icon = 'show';
            }
            $icons[] = $OUTPUT->action_icon($url, new pix_icon("t/{$icon}", get_string('manageaudincevisibility', 'shezar_cohort')));
        } else {
            $capabilities = ($viewtype == 'program') ? array('shezar/program:visibility', 'shezar/program:viewhiddenprograms') :
                                                       array('shezar/certification:viewhiddencertifications', 'shezar/program:visibility');
    ...
    if ($abletomoveprograms) {
        ...
        $label = ($viewtype == 'program') ? get_string('moveselectedprogramsto', 'shezar_program') :
                                            get_string('moveselectedcertificationsto', 'shezar_certification');
    ...
    $actionurl = new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype));
    $pagingurl = new moodle_url('/shezar/program/manage.php',
...
if ($canmanage && $numprograms > 1 && empty($searchcriteria)) {
    ...
    $url = new moodle_url('/shezar/program/manage.php',
                    array('categoryid' => $id, 'resort' => 'name', 'sesskey' => sesskey(), 'viewtype' => $viewtype));
    echo $OUTPUT->single_button($url, get_string('resortprogramsbyname', 'shezar_program'), 'get');
}

$cancreateprog = has_capability('shezar/program:createprogram', $context) && !shezar_feature_disabled('programs');
$cancreatecert = has_capability('shezar/certification:createcertification', $context) && !shezar_feature_disabled('certifications');
if (empty($searchcriteria)) {
    if ($viewtype == 'program' && $cancreateprog) {
        // Print button to create a new program.
        $url = new moodle_url('/shezar/program/add.php');
        if ($coursecat->id) {
            $url->params(array('category' => $coursecat->id));
        } else {
            $url->params(array('category' => $CFG->defaultrequestcategory));
        }
        echo $OUTPUT->single_button($url, get_string('addnewprogram', 'shezar_program'), 'get');
    } else if ($viewtype == 'certification' && $cancreatecert) {
        // Print button to create a new certification.
        $url = new moodle_url('/shezar/certification/add.php');
        if ($coursecat->id) {
            $url->params(array('category' => $coursecat->id));
        } else {
            $url->params(array('category' => $CFG->defaultrequestcategory));
        }
        echo $OUTPUT->single_button($url, get_string('addnewcertification', 'shezar_certification'), 'get');
    }
}
...
function print_category_edit(html_table $table, coursecat $category, $catlist, $viewtype = 'program', $depth = -1, $up = false, $down = false) {
    ...
    if ($category->id) {
        ...
        $categoryurl = new moodle_url('/shezar/program/manage.php',
        ...
        if ($canmanage) {
            ...
            // Delete category.
            $icons[] = $OUTPUT->action_icon(
                            new moodle_url('/shezar/program/manage.php', array('categoryid' => $category->id,
            ...
            if (!empty($category->visible)) {
                $icons[] = $OUTPUT->action_icon(
                                new moodle_url('/shezar/program/manage.php',
        ...
        if ($canmanage && is_array($catlist)) {
            $popupurl = new moodle_url('/shezar/program/manage.php',
