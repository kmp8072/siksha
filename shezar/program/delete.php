...
$PAGE->set_url('/shezar/program/delete.php', array('id' => $id));
$PAGE->set_context($categorycontext);
$PAGE->set_pagelayout('admin');
navigation_node::override_active_url(new moodle_url('/course/management.php', array('categoryid'=>$program->category)));

$returnurl = "{$CFG->wwwroot}/shezar/program/edit.php?id={$program->id}";
$deleteurl = "{$CFG->wwwroot}/shezar/program/delete.php?id={$program->id}&amp;sesskey={$USER->sesskey}&amp;category={$category}&amp;delete=".md5($program->timemodified);
...
if (!$delete) {
    ...
    echo $OUTPUT->header();
    $strdelete = get_string('checkprogramdelete', 'shezar_program');
    ...
    if ($incomplete_program_learners && $incomplete_program_learners > 0) {
        $strdelete .= html_writer::empty_tag('br') . html_writer::empty_tag('br') . get_string('xlearnerscurrentlyenrolled', 'shezar_program', $incomplete_program_learners);
    }
...
if ($delete != md5($program->timemodified)) {
    print_error('error:badcheckvariable', 'shezar_program');
}
...
if ($program->delete()) {
    if (prog_fix_program_sortorder($program->category)) {
        ...
        throw new Exception(get_string('error:failfixprogsortorder', 'shezar_program'));
    }
    $viewtype = ($program->certifid ? 'certification' : 'program');
    $notification_url = "{$CFG->wwwroot}/shezar/program/index.php?categoryid={$category}&amp;viewtype={$viewtype}";
    shezar_set_notification(get_string('programdeletesuccess', 'shezar_program', $program->fullname), $notification_url, array('class' => 'notifysuccess'));
}
