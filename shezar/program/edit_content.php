...
require_once($CFG->dirroot . '/shezar/program/edit_content_form.php');
...
require_capability('shezar/program:configurecontent', $programcontext);
$program->check_enabled();

$PAGE->set_url(new moodle_url('/shezar/program/edit_content.php', array('id' => $id)));
...
$overviewurl = $CFG->wwwroot."/shezar/program/edit.php?id={$id}&action=view";
...
if ($rawdata = data_submitted()) {
    ...
    if (!$programcontent->setup_content($rawdata)) {
        print_error('error:unabletosetupprogcontent', 'shezar_program');
    }
    ...
    if (isset($rawdata->{'addcontent'.$addsuffix})) {
        if (!$programcontent->add_set($rawdata->{'contenttype'.$addsuffix})) {
            echo $OUTPUT->notification(get_string('error:unabletoaddset', 'shezar_program'));
        }
    } else if (isset($rawdata->{'update'.$updatesuffix})) {
        $programcontent->update_content();
        echo $OUTPUT->notification(get_string('contentupdatednotsaved', 'shezar_program'));
    } else if ($setnumber = $programcontent->check_set_action('delete', $rawdata)) {
        if (!$programcontent->delete_set($setnumber)) {
            echo $OUTPUT->notification(get_string('error:deleteset', 'shezar_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->add_course($setnumber, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunabletoaddcourse', 'shezar_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->add_competency($setnumber, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunableaddcompetency', 'shezar_program'));
        } else {
            $submitdata = true;
        }
        ...
        if (!$programcontent->delete_course($action->setnumber, $action->courseid, $rawdata)) {
            echo $OUTPUT->notification(get_string('error:setunabletodeletecourse', 'shezar_program', $action->setnumber));
        } else {
            $submitdata = true;
        }
    }
...
if ($contenteditform->is_cancelled()) {
    shezar_set_notification(get_string('programupdatecancelled', 'shezar_program'), $overviewurl, array('class' => 'notifysuccess'));
}
...
if ($data = $contenteditform->get_data()) {
    if (isset($data->savechanges) || $submitdata) {
        ...
        if (!$programcontent->setup_content($data)) {
            print_error('error:setupprogcontent', 'shezar_program');
        }
        ...
        if (!$programcontent->save_content()) {
            shezar_set_notification(get_string('programupdatefail', 'shezar_program'), $currenturl);
        } else {
            $coursesetids = array();
            ...
            if (isset($data->savechanges) || $submitdata) {
                shezar_set_notification(get_string('programcontentsaved', 'shezar_program'),
...
$PAGE->requires->strings_for_js(array('addcourseset', 'addcourses', 'cancel', 'ok', 'addcompetency',
            ...
            'shezar_program');
$selected_addrecurringcourse = json_encode(dialog_display_currently_selected(get_string('selected', 'shezar_hierarchy'), 'addrecurringcourse'));
$selected_addcompetency = json_encode(dialog_display_currently_selected(get_string('selected', 'shezar_hierarchy'), 'addcompetency'));
...
$jsmodule = array(
     'name' => 'shezar_programcontent',
     'fullpath' => '/shezar/program/content/program_content.js',
     'requires' => array('json','event-delegate')
     );
$PAGE->requires->js_init_call('M.shezar_programcontent.init', $args, false, $jsmodule);
