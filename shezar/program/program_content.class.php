...
class prog_content {
    ...
    function __construct($programid) {
        ...
        foreach ($sets as $set) {
            if (!array_key_exists($set->contenttype, $this->courseset_classnames)) {
                throw new ProgramContentException(get_string('contenttypenotfound', 'shezar_program'));
            }
    ...
    public function save_content() {
        ...
        $event = \shezar_program\event\program_contentupdated::create_from_data($dataevent)->trigger();
    ...
    public function add_set($contenttype) {
        ...
        $courseset->label = get_string('legend:courseset', 'shezar_program', $courseset->sortorder);
    ...
    public function get_content_form_template(&$mform, &$template_values, $coursesets=null, $updateform=true, $iscertif=false, $certifpath=CERTIFPATH_CERT) {
        ...
        if ($iscertif) {
            ...
            $templatehtml .= html_writer::start_tag('legend', array('class' => 'ftoggler', 'id' => 'certifpath'.$suffix))
                . get_string(($certifpath == CERTIFPATH_CERT ? 'oricertpath' : 'recertpath'), 'shezar_certification')
                . html_writer::end_tag('legend');
            $templatehtml .= html_writer::start_tag('p')
                . get_string(($certifpath == CERTIFPATH_CERT ? 'oricertpathdesc' : 'recertpathdesc'), 'shezar_certification')
                . html_writer::end_tag('p');

            if ($certifpath == CERTIFPATH_RECERT && $numcoursesets == 0) {
                // ask for cert content to be copied to recert
                $label = get_string('sameascert', 'shezar_certification');
        ...
        $templatehtml .= $OUTPUT->heading(get_string('programcontent', 'shezar_program'));
        ...
        if ($iscertif) {
            $templatehtml .= html_writer::start_tag('p') . get_string('certificationcontent', 'shezar_certification') . html_writer::end_tag('p');
        } else {
            $templatehtml .= html_writer::start_tag('p') . get_string('instructions:programcontent', 'shezar_program') . html_writer::end_tag('p');
        }
        ...
        if ($numcoursesets == 0) { // if there's no content yet
            $templatehtml .= html_writer::start_tag('p') . get_string('noprogramcontent', 'shezar_program') . html_writer::end_tag('p');
        } else {
            foreach ($coursesets as $courseset) {
        ...
        if (!$recurring) {
            ...
            if ($updateform) {
                ...
                $contentoptions = array(
                    CONTENTTYPE_MULTICOURSE => get_string('setofcourses', 'shezar_program'),
                );
                if (!$iscertif) {
                    if (shezar_feature_visible('competencies')) {
                        $contentoptions[CONTENTTYPE_COMPETENCY] = get_string('competency', 'shezar_program');
                    }
                    if ($numcoursesets == 0) { // don't allow recurring course to be added if the program already has other content
                        $contentoptions[CONTENTTYPE_RECURRING] = get_string('recurringcourse', 'shezar_program');
                    }
                }

                $mform->addElement('select', 'contenttype'.$suffix, get_string('addnew', 'shezar_program'), $contentoptions,
            ...
            $templatehtml .= html_writer::start_tag('label', array('for' => 'contenttype'.$suffix)) . get_string('addnew', 'shezar_program')
            ...
            if ($iscertif) {
                $helpbutton = $OUTPUT->help_icon('addcertifprogramcontent', 'shezar_certification');
            } else {
                $helpbutton = $OUTPUT->help_icon('addprogramcontent', 'shezar_program');
            }
     ...
     *
     * @param int $operator Constant to search for a value in shezar_search_for_value function
    ...
    public function get_visibility_coursesets($operator, $visibility) {
        ...
        foreach ($this->coursesets as $set) {
            if (get_class($set) === $this->courseset_classnames[CONTENTTYPE_MULTICOURSE]) {
                $courseaudiencevisibility += shezar_search_for_value($set->courses, 'audiencevisible', $operator, $visibility);
            } else if (get_class($set) === $this->courseset_classnames[CONTENTTYPE_COMPETENCY]){
                $courses = $set->get_competency_courses();
                $courseaudiencevisibility += shezar_search_for_value($courses, 'audiencevisible', $operator, $visibility);
            }
        }
