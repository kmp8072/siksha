...
class prog_messages_manager {
        ...
        if (count($messages) > 0) {
            foreach ($messages as $message) {
                ...
                if (!array_key_exists($message->messagetype, $this->message_classnames)) {
                    throw new ProgramMessageException(get_string('meesagetypenotfound', 'shezar_program'));
                }
            ...
            $enrolment_message->messagesubject = get_string('defaultenrolmentmessage_subject', 'shezar_program');
            $enrolment_message->mainmessage = get_string('defaultenrolmentmessage_message', 'shezar_program');
            ...
            $exception_report_message->messagesubject = get_string('defaultexceptionreportmessage_subject', 'shezar_program');
            $exception_report_message->mainmessage = get_string('defaultexceptionreportmessage_message', 'shezar_program');
    ...
    public function display_form_label() {
        $out = '';
        $out .= get_string('instructions:messages1', 'shezar_program');
        return $out;
    }
    ...
    public function display_form_element() {
        ...
        if (count($this->messages)) {
            ...
            foreach ($this->messages as $message) {
                ...
                $component = (substr($messageclassname, 0, 11) == 'prog_recert' ? 'shezar_certification' : 'shezar_program');
                $out .= html_writer::tag('p', get_string($messageclassname, $component), array('class' => $styleclass));
                $messagecount++;
            }
        } else {
            $out .= get_string('noprogrammessages', 'shezar_program');
        }
    ...
    public function get_message_form_template(&$mform, &$template_values, $messages=null, $updateform=true) {
        ...
        if ($updateform) {
            $mform->addElement('submit', 'update', get_string('update', 'shezar_program'));
        ...
        $templatehtml .= $OUTPUT->heading(get_string('programmessages', 'shezar_program'));
        $templatehtml .= html_writer::tag('p', get_string('instructions:programmessages', 'shezar_program'));

        $templatehtml .= html_writer::start_tag('div', array('id' => 'messages'));

        if ($nummessages == 0) { // if there are no messages yet
            $templatehtml .= html_writer::tag('p', get_string('noprogrammessages', 'shezar_program'));
        } else {
        ...
        if ($canaddmessage) {
            ...
            if ($updateform) {
                $messageoptions = array(
                    MESSAGETYPE_ENROLMENT => get_string('enrolment', 'shezar_program'),
                    MESSAGETYPE_EXCEPTION_REPORT => get_string('exceptionsreport', 'shezar_program'),
                    MESSAGETYPE_UNENROLMENT => get_string('unenrolment', 'shezar_program'),
                    MESSAGETYPE_PROGRAM_DUE => get_string('programdue', 'shezar_program'),
                    MESSAGETYPE_PROGRAM_OVERDUE => get_string('programoverdue', 'shezar_program'),
                    MESSAGETYPE_PROGRAM_COMPLETED => get_string('programcompleted', 'shezar_program'),
                    MESSAGETYPE_COURSESET_DUE => get_string('coursesetdue', 'shezar_program'),
                    MESSAGETYPE_COURSESET_OVERDUE => get_string('coursesetoverdue', 'shezar_program'),
                    MESSAGETYPE_COURSESET_COMPLETED => get_string('coursesetcompleted', 'shezar_program'),
                    MESSAGETYPE_LEARNER_FOLLOWUP => get_string('learnerfollowup', 'shezar_program')
                );

                // add extra messages if a certification-program
                $prog = $DB->get_record('prog', array('id' => $this->programid));
                if ($prog->certifid > 0) {
                    $messageoptions[MESSAGETYPE_RECERT_WINDOWOPEN]     = get_string('recertwindowopen', 'shezar_certification');
                    $messageoptions[MESSAGETYPE_RECERT_WINDOWDUECLOSE] = get_string('recertwindowdueclose', 'shezar_certification');
                    $messageoptions[MESSAGETYPE_RECERT_FAILRECERT]     = get_string('recertfailrecert', 'shezar_certification');
                }

                $mform->addElement('select', 'messagetype', get_string('addnew', 'shezar_program'), $messageoptions, array('id'=>'messagetype'));
                $mform->setType('messagetype', PARAM_INT);
                $template_values['%messagetype%'] = array('name'=>'messagetype', 'value'=>null);
            }
            $templatehtml .= html_writer::tag('label', get_string('addnew', 'shezar_program'), array('for' => 'messagetype'));
            $templatehtml .= '%messagetype%';
            $templatehtml .= html_writer::tag('span', get_string('toprogram', 'shezar_program'));
        ...
        if ($updateform) {
            $mform->addElement('cancel', 'cancel', get_string('cancel', 'shezar_program'));
