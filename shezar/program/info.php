...
if (!$id and !$name) {
    print_error(get_string('error:noprogramid', 'shezar_program'));
}

if ($name) {
    if (!$program = $DB->get_record("prog", array("shortname" => $name))) {
        print_error('error:invalidshortname','shezar_program');
    }
} else {
    if (!$program = $DB->get_record("prog", array("id" => $id))) {
        print_error('error:invalidid','shezar_program');
    }
}
...
if ((!$program->visible) && !has_capability('shezar/program:viewhiddenprograms', $context)) {
    print_error('programhidden', '', $CFG->wwwroot .'/');
}

$PAGE->set_url(new moodle_url('/shezar/program/info.php', array('id' => $program->id)));
...
$summary = file_rewrite_pluginfile_urls($program->summary, 'pluginfile.php',
    $programcontext->id, 'shezar_program', 'summary', 0);
