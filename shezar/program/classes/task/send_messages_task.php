...
class send_messages_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('sendmessagestask', 'shezar_program');
    }
    ...
    protected function program_cron_enrolment_messages(&$programs) {
        ...
        $lastrun = get_config('shezar_program', 'enrolment_messages_last_run');
        if (empty($lastrun)) {
            ...
            if (empty($lastrun)) {
                ...
                $lastrun = $DB->get_field('config_plugins', 'value', array('plugin' => 'shezar_program', 'name' => 'lastcron'));
                if (empty($lastrun)) {
        ...
        set_config('enrolment_messages_last_run', $currentrun, 'shezar_program');
