 ...
 * @package shezar_reportbuilder
 */
class timewindowopens extends \shezar_reportbuilder\rb\display\base {
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        if ($value < time()) {
            ...
            if ($extrafields->status != CERTIFSTATUS_EXPIRED) {
                $extra = $OUTPUT->notification(get_string('windowopen', 'shezar_certification'), 'notifysuccess');
            } else {
                $extra = $OUTPUT->notification(get_string('status_expired', 'shezar_certification'), 'notifyproblem');
            }
        } else {
            ...
            if ($days_remaining == 1) {
                $extra = $OUTPUT->notification(get_string('windowopenin1day', 'shezar_certification'), 'notifynotice');
            } else if ($days_remaining < 10 && $days_remaining > 0) {
                $extra = $OUTPUT->notification(get_string('windowopeninxdays', 'shezar_certification', $days_remaining), 'notifynotice');
            }
        }
