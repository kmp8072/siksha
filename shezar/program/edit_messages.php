...
require_capability('shezar/program:configuremessages', $programcontext);
$program->check_enabled();

$PAGE->set_url(new moodle_url('/shezar/program/edit_messages.php', array('id' => $id)));
...
if ($rawdata = data_submitted()) {
    ...
    if (!$programmessagemanager->setup_messages($rawdata)) {
        print_error('error:setupprogrammessages', 'shezar_program');
    }

    if (isset($rawdata->addmessage)) {
        if (!$programmessagemanager->add_message($rawdata->messagetype)) {
            echo $OUTPUT->notification(get_string('error:unableaddmessagetypeunrecog', 'shezar_program'));
        }
    } else if (isset($rawdata->update)) {
        $programmessagemanager->update_messages();
        echo $OUTPUT->notification(get_string('progmessageupdated', 'shezar_program'));
        ...
        if (!$programmessagemanager->delete_message($messagenumber)) {
            echo $OUTPUT->notification(get_string('error:unabledeletemessagenotfound', 'shezar_program'));
        }
...
if ($data = $messageseditform->get_data()) {
    ...
    if (isset($data->savechanges)) {
        ...
        // Trigger event
        \shezar_program\event\update_messages::create_from_instance($program)->trigger();
        ...
        if (!$programmessagemanager->save_messages($data)) {
            ...
            shezar_set_notification(get_string('programmessagessaved', 'shezar_program'), 'edit_messages.php?id='.$id, array('class' => 'notifysuccess'));
        }
    }
...
$jsmodule = array(
     'name' => 'shezar_programmessages',
     'fullpath' => '/shezar/program/messages/program_messages.js',
     'requires' => array('json'));
$PAGE->requires->js_init_call('M.shezar_programmessages.init',$args, false, $jsmodule);
