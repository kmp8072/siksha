 ...
 * @subpackage program
 */
M.shezar_programexceptions = M.shezar_programexceptions || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.shezar_programexceptions.init()-> Required config \'id\' not available.');
        }
        ...
        $('#selectiontype').change(function() {
            ...
            var url = M.cfg.wwwroot+'/shezar/program/exception/updateselections.php?id='+module.config.id,
        ...
        var buttonsObj = {};
        buttonsObj[M.util.get_string('ok', 'moodle')] = function() { dialog.save(M.cfg.wwwroot+'/shezar/program/exception/resolve.php?id='+module.config.id); };
        ...
        var dialog = new shezarDialog(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+M.util.get_string('confirmresolution', 'shezar_program')+'</h2>',
                            height: '250'
                },
                M.cfg.wwwroot+'/shezar/program/exception/confirm_resolution.php',
                handler
            );
        ...
        dialog._update = function(response) {
            this.hide();
            window.location.href = M.cfg.wwwroot+'/shezar/program/exceptions.php?id='+module.config.id+'&search='+module.config.search_term;
        };

        shezarDialogs['applyaction'] = dialog;
    },

    /**
     * wrapper for shezar 1.9 globally declared Items() function, when called sets an
    ...
    /**
     * wrapper for shezar 1.9 globally declared Item() function
