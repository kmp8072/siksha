...
abstract class course_set {
    ...
    protected function get_completion_type_string() {
        ...
        case COMPLETIONTYPE_OPTIONAL:
            $completiontypestr = get_string('or', 'shezar_program');
            break;
        case COMPLETIONTYPE_ALL:
            $completiontypestr = get_string('and', 'shezar_program');
            break;
        default:
            return false;
            break;
    ...
    protected function get_completion_explanation_html() {
        ...
        $typestr = get_string('completeallcourses', 'shezar_program');
        if ($this->completiontype != COMPLETIONTYPE_ALL) {
            if ($this->completiontype == COMPLETIONTYPE_ANY) {
                // Only one course must be completed.
                $typestr = get_string('completeanycourse', 'shezar_program');
            } else {
                $a = new stdClass();
                $a->mincourses = $this->mincourses;
                $typestr = get_string('completemincourses', 'shezar_program', $a);
            }
        }

        if ($this->timeallowed > 0) {
            $numperiod = program_utilities::get_duration_num_and_period($this->timeallowed);
            $typestr .= html_writer::tag('p', get_string('allowtimeforset' . $numperiod->periodkey, 'shezar_program', $numperiod->num));
        }
    ...
    public function get_default_label() {
        return get_string('untitledset', 'shezar_program');
    }
    ...
    public function save_set() {
        ...
        if ($this->label == $this->get_default_label()) {
            $this->label = get_string('legend:courseset', 'shezar_program', $this->sortorder);
        }
   ...
   protected function get_course_warnings($course) {
       ...
       if (!empty($modinfo->instances['facetoface'])) {
           ...
           if ($DB->record_exists_select('facetoface', 'multiplesessions = 0 AND id ' . $sql, $params)) {
               $content .= $OUTPUT->notification(get_string('multiplefacetofacewarning', 'shezar_program'), 'notifyproblem');
           }
       }
       return $content;
   }
        ...
        if ($completion = $DB->get_record('prog_completion',
            ...
            if ($update_success = $DB->update_record('prog_completion', $completion)) {
                if ($eventtrigger) {
                    ...
                    $event = \shezar_program\event\program_courseset_completed::create(
                        array(
        ...
        if (!isset($this->islastset) || $this->islastset === false) {
            ...
            switch ($this->nextsetoperator) {
                case NEXTSETOPERATOR_THEN:
                    $out .= html_writer::tag('div', get_string('then', 'shezar_program'), array('class' => 'operator-then'));
                    break;
                case NEXTSETOPERATOR_OR:
                    $out .= html_writer::tag('div', get_string('or', 'shezar_program'), array('class' => 'operator-or'));
                    break;
                case NEXTSETOPERATOR_AND:
                    $out .= html_writer::tag('div', get_string('and', 'shezar_program'), array('class' => 'operator-and'));
                    break;
            }
    ...
    public function get_nextsetoperator_select_form_template(&$mform, &$template_values, $formdataobject, $prefix, $updateform=true) {
        ...
        if ($updateform) {
            if (isset($this->islastset) && $this->islastset) {
                ...
                $options = array(
                    NEXTSETOPERATOR_THEN => get_string('then', 'shezar_program'),
                    NEXTSETOPERATOR_OR => get_string('or', 'shezar_program'),
                    NEXTSETOPERATOR_AND => get_string('and', 'shezar_program')
                );
                $mform->addElement('select', $prefix.'nextsetoperator', get_string('label:nextsetoperator', 'shezar_program'), $options);
...
class multi_course_set extends course_set {
    ...
    public function display($userid=null, $previous_sets=array(), $next_sets=array(), $accessible=true, $viewinganothersprogram=false) {
        ...
        switch ($this->completiontype) {
            case COMPLETIONTYPE_ALL:
                $out .= html_writer::tag('p', html_writer::tag('strong', get_string('completeallcourses', 'shezar_program')));
                break;
            case COMPLETIONTYPE_ANY:
                $out .= html_writer::tag('p', html_writer::tag('strong', get_string('completeanycourse', 'shezar_program')));
                break;
            case COMPLETIONTYPE_SOME:
                $str = new stdClass();
                ...
                if (!empty($str->mincourses) && !empty($str->sumfield) && !empty($str->sumfieldtotal)) {
                    $completestr = get_string('completemincoursesminsum', 'shezar_program', $str);
                } else if (!empty($this->mincourses)) {
                    $completestr = get_string('completemincourses', 'shezar_program', $str);
                } else {
                    $completestr = get_string('completeminsumfield', 'shezar_program', $str);
                }
                ...
                if (!empty($str->sumfield)) {
                    ...
                    $criteriacompletionset = get_string('criteriacompletioncourseset', 'shezar_program', $str->sumfield);
                    $out .= html_writer::tag('p', html_writer::tag('strong', $criteriacompletionset));
                }

                break;
            case COMPLETIONTYPE_OPTIONAL:
                $out .= html_writer::tag('p', html_writer::tag('strong', get_string('completeoptionalcourses', 'shezar_program')));
                break;
        }
        ...
        if ($this->timeallowed > 0) {
            $out .= html_writer::tag('p', get_string('allowtimeforset' . $numperiod->periodkey, 'shezar_program', $numperiod->num));
        }
        ...
        if (count($this->courses) > 0) {
            ...
            $table->head = array(get_string('coursename', 'shezar_program'));
            ...
            if (!empty($this->certifpath)) {
                ...
                if ($certifpath_user == CERTIFPATH_RECERT && !certif_iswindowopen($certificationid, $userid)) {
                    $launchviewlabel = get_string('viewcourse', 'shezar_program');
                }
            }
            ...
            foreach ($this->courses as $course) {
                ...
                $coursedetails = html_writer::empty_tag('img', array('src' => shezar_get_icon($course->id, shezar_ICON_TYPE_COURSE),
                    'class' => 'course_icon', 'alt' => ''));

                $showcourseset = false;
                if ($userid) {
                    $showcourseset = (is_enrolled($coursecontext, $userid) || shezar_course_is_viewable($course->id, $userid))
                                     && $accessible;
                }
                ...
                if (is_siteadmin($USER->id)) {
                    ...
                    $dimmed = shezar_get_style_visibility($course);
                    ...
                    $launch = html_writer::tag('div', $OUTPUT->single_button(new moodle_url('/course/view.php', array('id' => $course->id)),
                                    get_string('launchcourse', 'shezar_program'), null), array('class' => 'prog-course-launch'));
                } else {
                    ...
                    if ($showcourseset) {
                        ...
                        $requrl = new moodle_url('/shezar/program/required.php', $params);
                        $button = $OUTPUT->single_button($requrl, get_string('launchcourse', 'shezar_program'), null);
                        $launch = html_writer::tag('div', $button, array('class' => 'prog-course-launch'));
                    } else {
                        $coursedetails .= $coursename;
                        $launch = html_writer::tag('div', $OUTPUT->single_button(null, get_string('notavailable', 'shezar_program'), null,
                ...
                if ($userid) {
                    ...
                    $cells[] = new html_table_cell(shezar_display_course_progress_icon($userid, $course->id, $status));
                    $markstaff = (\shezar_job\job_assignment::is_managing($USER->id, $userid) && has_capability('shezar/program:markstaffcoursecomplete', $usercontext));
                    $markuser = has_capability('shezar/core:markusercoursecomplete', $usercontext);
                    ...
                    if ($showcourseset && ($markstaff || $markuser || $markcourse)) {
                        ...
                        $url = new moodle_url('/shezar/program/content/completecourse.php',
                        ...
                        if (!$completeheading) {
                            $table->head[] = get_string('markcompletheading', 'shezar_program');
            ...
            $out .= html_writer::tag('p', get_string('nocourses', 'shezar_program'));
        }
    ...
    public function display_form_element() {
        ...
        if (count($courses)) {
            ...
            $out .= get_string('nocourses', 'shezar_program');
        }
        ...
        if (!isset($this->islastset) || $this->islastset === false) {
            if ($this->nextsetoperator != 0) {
                $out .= html_writer::start_tag('div', array('class' => 'nextsetoperator'));
                if ($this->nextsetoperator == NEXTSETOPERATOR_THEN) {
                    $out .= get_string('then', 'shezar_program');
                } else if ($this->nextsetoperator == NEXTSETOPERATOR_OR) {
                    $out .= get_string('or', 'shezar_program');
                } else {
                    $out .= get_string('and', 'shezar_program');
                }
                $out .= html_writer::end_tag('div');
            }
        }
    ...
    public function print_courses() {
        ...
        $out .= html_writer::tag('div', get_string('courses', 'shezar_program'). ':', array('class' => 'fitemtitle'));
        if (isset($this->courses) && is_array($this->courses) && count($this->courses)>0) {

            if (!$completiontypestr = $this->get_completion_type_string()) {
                print_error('unknowncompletiontype', 'shezar_program', '', $this->sortorder);
            }
            ...
            foreach ($this->courses as $course) {
                ...
                $content .= html_writer::start_tag('div', array('class' => 'shezar-item-group delete_item'));
                $content .= html_writer::start_tag('a',
                                array('class' => 'shezar-item-group-icon coursedeletelink', 'href' => 'javascript:;',
            ...
            $out .= html_writer::tag('div', get_string('nocourses', 'shezar_program'), array('class' => 'felement'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => $prefix.'courses', 'value' => ''));
        }
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('multicourseset', 'shezar_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('untitledset', 'shezar_program',
        ...
        $helpbutton = $OUTPUT->help_icon('setlabel', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:setname', 'shezar_program') . ' ' . $helpbutton,
        ...
        if ($updateform) {
            $completiontypeoptions = array(
                COMPLETIONTYPE_ANY => get_string('onecourse', 'shezar_program'),
                COMPLETIONTYPE_ALL => get_string('allcourses', 'shezar_program'),
                COMPLETIONTYPE_SOME => get_string('somecourses', 'shezar_program'),
                COMPLETIONTYPE_OPTIONAL => get_string('completionoptional', 'shezar_program'),
            );
            $onchange = 'return M.shezar_programcontent.changeCompletionTypeString(this, '.$prefix.');';
            $mform->addElement('select', $prefix.'completiontype', get_string('label:learnermustcomplete', 'shezar_program'),
        ...
        $helpbutton = $OUTPUT->help_icon('completiontype', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:learnermustcomplete', 'shezar_program') . ' ' . $helpbutton,
        ...
        $helpbutton = $OUTPUT->help_icon('mincourses', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('mincourses', 'shezar_program') . ' ' . $helpbutton,
        ...
        if ($updateform) {
            ...
            foreach ($customfields as $i => $field) {
                if ($field->hidden && !$canviewhidden) {
                    $customfields[$i] = get_string('hiddenfield', 'shezar_program');
                } else {
                    $customfields[$i] = format_string($field->fullname);
                }
            }
            $customfields = array(0 => get_string('none')) + $customfields;
            $mform->addElement('select', $prefix.'coursesumfield', get_string('label:coursescorefield', 'shezar_program'),
        ...
        $helpbutton = $OUTPUT->help_icon('coursescorefield', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:coursescorefield', 'shezar_program') . ' ' . $helpbutton,
        ...
        $helpbutton = $OUTPUT->help_icon('minimumscore', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:minimumscore', 'shezar_program') . ' ' . $helpbutton,
        ...
        if ($updateform) {
            ...
            $templatehtml .= html_writer::tag('label', get_string('timeperiod', 'shezar_program'), array('for' => $prefix.'timeperiod', 'class' => 'accesshide'));
        ...
        $helpbutton = $OUTPUT->help_icon('minimumtimerequired', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:minimumtimerequired', 'shezar_program') . ' ' . $helpbutton,
        ...
        if (count($courseoptions) > 0) {
            if ($updateform) {
                ...
                $mform->addElement('submit', $prefix.'addcourse', get_string('addcourse', 'shezar_program'),
                                 array('onclick' => "return M.shezar_programcontent.amendCourses('$prefix')"));
            ...
            $templatehtml .= html_writer::tag('p', get_string('nocoursestoadd', 'shezar_program'));
        }
    ...
    public function get_courses_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $templatehtml = '';
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::tag('div', get_string('courses', 'shezar_program'). ':', array('class' => 'fitemtitle'));
        if (isset($this->courses) && is_array($this->courses) && count($this->courses)>0) {
            ...
            $templatehtml .= html_writer::tag('div', get_string('nocourses', 'shezar_program'), array('class' => 'felement'));
        }
    ...
    public function get_course_text($courseset) {
        if ($courseset->completiontype == COMPLETIONTYPE_ALL) {
            return get_string('allcoursesfrom', 'shezar_program') . ' "' . format_string($courseset->label) . '"';
        }
        else if ($courseset->completiontype == COMPLETIONTYPE_SOME) {
            return get_string('somecoursesfrom', 'shezar_program') . ' "' . format_string($courseset->label) . '"';
        }
        else if ($courseset->completiontype == COMPLETIONTYPE_OPTIONAL) {
            return get_string('nocoursesfrom', 'shezar_program') . ' "' . format_string($courseset->label) . '"';
        }
        else {
            return get_string('onecoursesfrom', 'shezar_program') . ' "' . format_string($courseset->label) . '"';
        }
    }
...
class competency_course_set extends course_set {
    ...
    public function display($userid=null,$previous_sets=array(),$next_sets=array(),$accessible=true, $viewinganothersprogram=false) {
        ...
        if ($courses && count($courses) > 0) {
            ...
            foreach ($courses as $course) {
                ...
                $cells[] = new html_table_cell($coursedetails);
                if (is_siteadmin($USER->id) || $userid && $accessible && shezar_course_is_viewable($course->id, $userid)) {
                    $launch = html_writer::tag('div', $OUTPUT->single_button(new moodle_url('/course/view.php', array('id' => $course->id)),
                                     get_string('launchcourse', 'shezar_program'), null), array('class' => 'prog-course-launch'));
                } else if ($userid && $accessible && !empty($CFG->audiencevisibility) && $course->audiencevisible != COHORT_VISIBLE_NOUSERS) {
                    // If the program has been assigned but the user is not yet enrolled in the course,
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('competencycourseset', 'shezar_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('legend:courseset', 'shezar_program',
                        $this->sortorder)) . ' ' . $helpbutton;
        $templatehtml .= html_writer::tag('legend', $legend);
        $templatehtml .= html_writer::start_tag('div', array('class' => 'setbuttons'));
        ...
        if ($this->competencyid > 0) {
            if ($competency = $DB->get_record('comp', array('id' => $this->competencyid))) {
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
                $templatehtml .= html_writer::tag('label', get_string('label:competencyname', 'shezar_program'));
                $templatehtml .= html_writer::end_tag('div');
        ...
        if ($this->competencyid > 0) {
            ...
            if (count($course_competencies) > 0) {
                if ($updateform) {
                    ...
                    $mform->addElement('submit', $prefix.'addcompetency', get_string('addcompetency', 'shezar_program'));
                    $template_values['%'.$prefix.'competencyid%'] = array('name'=>$prefix.'competencyid', 'value'=>null);
                ...
                $templatehtml .= '%'.$prefix.'competencyid%'."\n";
                $templatehtml .= html_writer::tag('p', get_string('nocompetenciestoadd', 'shezar_program'));
...
class recurring_course_set extends course_set {
    ...
    public function get_courseset_form_template(&$mform, &$template_values, &$formdataobject, $updateform=true) {
        ...
        $helpbutton = $OUTPUT->help_icon('recurringcourseset', 'shezar_program');
        $legend = ((isset($this->label) && ! empty($this->label)) ? $this->label : get_string('legend:recurringcourseset', 'shezar_program',
                        $this->sortorder)) . ' ' . $helpbutton;
        $templatehtml .= html_writer::tag('legend', $legend);
        ...
        if (is_object($this->course)) {
            if (isset($this->course->fullname)) {
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
                $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
                $templatehtml .= html_writer::tag('label', get_string('coursename', 'shezar_program'));
                $templatehtml .= html_writer::end_tag('div');
                ...
                if (count($courseoptions) > 0) {
                    ...
                    $templatehtml .= '%'.$prefix.'courseid%'."\n";
                    $templatehtml .= $OUTPUT->help_icon('recurringcourse', 'shezar_program');
                    $formdataobject->{$prefix.'courseid'} = $this->course->id;
                } else {
                    $templatehtml .= html_writer::tag('p', get_string('nocoursestoselect', 'shezar_program'));
                }
                $templatehtml .= html_writer::end_tag('div');
        ...
        $helpbutton = $OUTPUT->help_icon('recurrence', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:recurrence', 'shezar_program') . ' ' . $helpbutton,
            array('for' => $prefix.'recurrencetimenum'));
        $templatehtml .= html_writer::end_tag('div');
        $templatehtml .= html_writer::tag('div', get_string('repeatevery', 'shezar_program') . ' %' . $prefix .
        ...
        $helpbutton = $OUTPUT->help_icon('coursecreation', 'shezar_program');
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitem'));
        $templatehtml .= html_writer::start_tag('div', array('class' => 'fitemtitle'));
        $templatehtml .= html_writer::tag('label', get_string('label:recurcreation', 'shezar_program') . ' ' . $helpbutton,
            array('for' => $prefix.'recurcreatetimenum'));
        $templatehtml .= html_writer::end_tag('div');
        $templatehtml .= html_writer::tag('div', get_string('createcourse', 'shezar_program') . ' %' .
            $prefix . 'recurcreatetimenum% %' . $prefix . 'recurcreatetimeperiod% '.
            get_string('beforecourserepeats', 'shezar_program'), array('class' => 'felement'));
        $templatehtml .= html_writer::end_tag('div');
        ...
        if ($updateform) {
            $mform->addElement('submit', $prefix.'update', get_string('update', 'shezar_program'),
