 ...
 * @subpackage program
 */
M.shezar_programassignment = M.shezar_programassignment || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.shezar_programassignment.init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_programassignment.init()-> jQuery dependency required for this module to function.');
        }
         ...
         */
        this.shezarDialog_program_cat = function(category, catid, name, find_url, title) {

            // Store some bits
            this.category = category;
            this.catid = catid;
            this.name = name;
            this.url = M.cfg.wwwroot+'/shezar/program/assignment/';
            ...
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('ok','shezar_program')] = function() { self.category.save(); };
            buttonsObj[M.util.get_string('cancel','shezar_program')] = function() { self.hide(); };
        ...
        var shezarDialog_completion_handler = function() {};

        shezarDialog_completion_handler.prototype = new shezarDialog_handler();

        shezarDialog_completion_handler.prototype.first_load = function() {
            M.shezar_core.build_datepicker(module.Y, 'input[name="completiontime"]', M.util.get_string('datepickerlongyeardisplayformat', 'shezar_core'));
            $('#ui-datepicker-div').css('z-index',1600);
        };

        shezarDialog_completion_handler.prototype.every_load = function() {
        ...
        var shezarDialog_completion = function() {

            //this.item = item;
            //this.url = url + 'choose_completion.php';

            // Setup the handler
            var handler = new shezarDialog_completion_handler();
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+ M.util.get_string('completioncriteria', 'shezar_program') +'</h2>'
            },
            this.url,
            handler
            );
            ...
            $(document).on('click', '.relativeeventtime', function(event) {
                ...
                if (unitformat.test(timeunit) === false) {
                    alert(M.util.get_string('pleaseentervalidunit', 'shezar_program'));
                    ...
                    alert(M.util.get_string('pleasepickaninstance', 'shezar_program'));
                } else {
        ...
        var shezarDialog_savechanges = function() {
            ...
            buttonsObj[M.util.get_string('editassignments','shezar_program')] = function() { handler._cancel() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+M.util.get_string('confirmassignmentchanges', 'shezar_program')+'</h2>'
            },
        ...
        shezarDialog_completion_event = function() {

            // Setup the handler
            var handler = new shezarDialog_handler_treeview_singleselect('instance', 'instancetitle');

            // Store reference to this
            var self = this;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('ok','shezar_program')] = function() { self.save() };
            buttonsObj[M.util.get_string('cancel','shezar_program')] = function() { handler._cancel() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+M.util.get_string('chooseitem', 'shezar_program') + module.config.display_selected_completion_event +'</h2>'
            },
        ...
        $('#category_select button').click(function() {
            var url = M.cfg.wwwroot+'/shezar/program/assignment/get_items.php?progid='+module.config.id;
        ...
        $('.assignment-duedates').on('click', function(){
            $.get($(this).attr('href'), function(result){
                M.shezar_programassignment.datesDialogue = new M.core.dialogue({
                    headerContent: null,
                    bodyContent  : result,
                    width        : 900,
                    centered     : true,
                ...
                M.shezar_programassignment.datesDialogue.show();
            });
            return false;
        });

        // On click events for column sorting and paging inside "View dates" popup.
        $('#page-shezar-program-edit_assignments').on('click',
            ...
            if (!event.target.closest('td') || !$(event.target.closest('td')).hasClass('cell')) {
                $.get($(event.target).attr('href'), function(result) {
                    M.shezar_programassignment.datesDialogue.bodyNode.setHTML(result);
                });
            } else {
        ...
        shezarDialogs['completion'] = new shezarDialog_completion();
        shezarDialogs['savechanges'] = new shezarDialog_savechanges();
        shezarDialogs['completionevent'] = new shezarDialog_completion_event();
    ...
    handleSaveChanges: function(event){
        ...
        shezarDialogs['savechanges'].open(html);
        shezarDialogs['savechanges'].save = function() {
            shezarDialogs['savechanges'].savechanges = true;
            this.hide();
...
function category(id, name, find_url, title, programid) {
    ...
    this.dialog_additem = new M.shezar_programassignment.shezarDialog_program_cat(this, id, name, find_url, title);
...
function item(category, element, isexistingitem) {
    ...
    // Update the hidden inputs
        ...
        $.get(url, function(data) {
            if (data == 'error') {
                ...
                if (data == M.util.get_string('setcompletion', 'shezar_program')) {
                    ...
                    if (deletecompletiondatelink.length == 0) {
                        var newlink = $('<a href="#" class="deletecompletiondatelink" title="' + M.util.get_string('removecompletiondate', 'shezar_program') + '"></a>');
                        self.completionlink.parent().append(newlink);

                        require(['core/templates'], function (templates) {
                            templates.renderIcon('delete', M.util.get_string('removecompletiondate', 'shezar_program')).done(function (html) {
                                newlink.html(html);
                            });
                        });
                    }
                ...
                if (typeof completiontime === 'undefined' || completiontime == M.shezar_programassignment.config.COMPLETION_TIME_NOT_SET) {
                ...
                if (typeof completionevent === 'undefined' || completionevent == M.shezar_programassignment.config.COMPLETION_EVENT_NONE) {
    ...
    this.element.on('click', '.deletecompletiondatelink', function(event){
        event.preventDefault();
        self.update_completiontime(M.shezar_programassignment.config.COMPLETION_TIME_NOT_SET, 0, 0, 0, 0);
    });
    ...
    this.element.on('click', '.completionlink', function(event){
        ...
        event.preventDefault();
        shezarDialogs['completion'].item = self;
        shezarDialogs['completion'].default_url = self.category.url + 'set_completion.php?programid=' + self.category.programid;
        shezarDialogs['completion'].open();
    });
