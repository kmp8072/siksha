...
*/
class shezar_program_renderer extends plugin_renderer_base {
    ...
    */
    public function get_cancel_button($params=null, $url='') {
        if (empty($url)) {
            $url = "/shezar/program/edit.php";
        }
        $link = new moodle_url($url, $params);
        $output = $this->output->action_link($link, get_string('cancelprogrammanagement', 'shezar_program'), null, array('id' => 'cancelprogramedits'));
    ...
    */
    public function render_current_status($data) {
        ...
        $programstatusstring = get_string($data->statusstr, 'shezar_program');

        $learnerinfo = html_writer::empty_tag('br') . html_writer::start_tag('span', array('class' => 'assignmentcount'));
        if ($data->exceptions > 0) {
            $learnerinfo .= get_string('learnersassignedexceptions', 'shezar_program', $data);
        } else {
            $learnerinfo .= get_string('learnersassigned', 'shezar_program', $data);
        }
        ...
        if ($data->audiencevisibilitywarning) {
            $coursevisibilityinfo .= get_string('audiencevisibilityconflictmessage', 'shezar_program');
        }
        if ($data->assignmentsdeferred) {
            $coursevisibilityinfo .= get_string('assignmentsdeferred', 'shezar_program');
        }
    ...
    public function assignment_category_display($assignment_class, $headers, $buttonname, $data, $canadd) {
        ...
        $html .= html_writer::start_tag('div', array('class' => 'total_user_count')) . get_string('total', 'shezar_program') . ': ';
    ...
    public function display_edit_assignment_form($id, $categories, $certificationpath) {
        ...
        $out .= $this->output->heading(get_string('programassignments', 'shezar_program'), 3);
        ...
        $out .= html_writer::tag('p', get_string('instructions:programassignments', 'shezar_program'));
        $out .= html_writer::start_tag('div', array('id' => 'assignment_categories'));

        // Display the categories!
        $js = '';
        foreach ($categories as $category) {
            if (get_class($category) === 'positions_category' && shezar_feature_disabled('positions')) {
                $canadd = false;
            } else {
                $canadd = true;
            }
        ...
        if (!empty($dropdown_options)) {
            ...
            $out .= html_writer::tag('label', get_string('addnew', 'shezar_program'), array('for' => 'menucategory_select_dropdown'));
            $out .= html_writer::select($dropdown_options, 'category_select_dropdown', array('initialvalue' => 1));
            $out .= html_writer::tag('span', get_string('toprogram', 'shezar_program'));
            $out .= html_writer::tag('button', get_string('add'));
            $out .= html_writer::end_tag('div');
        }
        $out .= html_writer::start_tag('div', array('class' => 'overall_total'));
        $out .= $this->output->help_icon('totalassignments', 'shezar_program');
        $out .= ' ' . get_string('totalassignments', 'shezar_program') . ': ';
    ...
    */
    public function display_user_message_box($user, $a) {
        ...
        $out .= html_writer::tag('strong', get_string('youareviewingxsrequiredlearning', 'shezar_program', $a));
        $out .= html_writer::end_tag('div');
        return $out;
    }
    ...
    */
    public function print_exceptions_link($url, $excount) {
        ...
        $out .= get_string('unresolvedexceptions', 'shezar_program', $excount);
        $out .= html_writer::end_tag('span');
        $out .= html_writer::start_tag('span', array('class' => 'exceptionslink'));
        $out .= html_writer::link($url, get_string('viewexceptions', 'shezar_program'));
        $out .= html_writer::end_tag('span');
    ...
    */
    public function print_search($programid, $previoussearch='', $resultcount = 0) {
        $url = new moodle_url('/shezar/program/exceptions.php', array('id' => $programid));
        $out = html_writer::start_tag('form', array('action' => $url->out(), 'method' => 'get', 'id' => 'exceptionssearchform'));
        $out .= html_writer::tag('label', get_string('searchforindividual', 'shezar_program'), array('for' => 'exception_search'));
        ...
        if ($previoussearch != '' && $resultcount > 0) {
            ...
            $out .= html_writer::tag('p', get_string('xresultsfory', 'shezar_core', $a));
        }
        return $out;
    }
    ...
    */
    public function print_exceptions_form($numexceptions, $numselectedexceptions, $programid, $selectiontype, $tabledata) {
        ...
        if ($numexceptions == 0) {
            $out .= html_writer::start_tag('p') . get_string('noprogramexceptions', 'shezar_program') . html_writer::end_tag('p');
        } else {
            ...
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => 'applyactionbutton', 'name' => "submit", 'value' => get_string('proceed', 'shezar_program')));
            $out .= html_writer::end_tag('div');

            $out .= html_writer::start_tag('div') . html_writer::start_tag('p');
            $out .= html_writer::start_tag('span', array('id' => 'numselectedexceptions')) . $numselectedexceptions . html_writer::end_tag('span') . ' ' .get_string('learnersselected', 'shezar_program');
            ...
            $table->id = 'exceptions';
            $table->head = array(
                get_string('header:hash', 'shezar_program'),
                get_string('header:learners', 'shezar_program'),
                get_string('header:id','shezar_program'),
                get_string('header:issue','shezar_program'),
            );
    ...
    public function get_exceptiontype_selector($selectiontype) {
        ...
        $out = '';
        $options = array();
        $options[SELECTIONTYPE_NONE] = get_string('select', 'shezar_program');
        $options[SELECTIONTYPE_ALL] = get_string('alllearners', 'shezar_program');
        $options[SELECTIONTYPE_TIME_ALLOWANCE] = get_string('alltimeallowanceissues', 'shezar_program');
        $options[SELECTIONTYPE_ALREADY_ASSIGNED] = get_string('allcurrentlyassignedissues', 'shezar_program');
        $options[SELECTIONTYPE_COMPLETION_TIME_UNKNOWN] = get_string('allcompletiontimeunknownissues', 'shezar_program');
        $options[SELECTIONTYPE_DUPLICATE_COURSE] = get_string('allduplicatecourseissues', 'shezar_program');
        $out .= html_writer::start_tag('div');
    ...
    public function get_exceptionaction_selector() {
        ...
        $out = '';
        $options = array();
        $options[SELECTIONACTION_NONE] = get_string('action', 'shezar_program');
        $options[SELECTIONACTION_AUTO_TIME_ALLOWANCE] = get_string('setrealistictimeallowance', 'shezar_program');
        $options[SELECTIONACTION_OVERRIDE_EXCEPTION] = get_string('overrideandaddprogram', 'shezar_program');
        $options[SELECTIONACTION_DISMISS_EXCEPTION] = get_string('dismissandtakenoaction', 'shezar_program');
        $out .= html_writer::start_tag('div');
    ...
    */
    public function display_set_completion($programid = null) {
        ...
        $out .= html_writer::start_tag('span', array('class' => 'legend')) . get_string('completeby', 'shezar_program') . html_writer::end_tag('span');
        ...
        $out .= html_writer::empty_tag('input', array('class' => 'completiontime', 'type' => 'text', 'name' => "completiontime", 'placeholder' => get_string('datepickerlongyearplaceholder', 'shezar_core')));
        $out .= html_writer::end_tag('span');

        // Matching display of time in RTL to what is done in lib/form/datetimeselector.php.
        if (right_to_left()) {
            $out .= ' '.get_string('datepickerattime', 'shezar_core'). ' ';
        ...
        $out .= ' ' . html_writer::start_tag('button', array('class' => 'fixeddate')) .
            get_string('setfixedcompletiondate', 'shezar_program') . html_writer::end_tag('button');
        $out .= html_writer::end_tag('div');
        $out .= html_writer::end_tag('fieldset');

        $out .= html_writer::start_tag('div', array('id' => 'prog-completion-or-string'));
        $out .= get_string('or', 'shezar_program');
        $out .= html_writer::end_tag('div');

        $out .= html_writer::start_tag('div', array('id' => 'prog-completion-relative-date'));
        $out .= html_writer::tag('span', get_string('completewithin', 'shezar_program'));
        ...
        $out .= ' ' . get_string('of', 'shezar_program') . ' ';
        ...
        $out .= html_writer::start_tag('button', array('class' => 'relativeeventtime')) . get_string('settimerelativetoevent', 'shezar_program') . html_writer::end_tag('button');
        $out .= html_writer::end_tag('div');
    ...
    */
    public function display_set_extension() {
        $out = '';
        $out .= html_writer::start_tag('div');
        $out .= html_writer::start_tag('label', array('for' => 'extensiontime')) . get_string('extenduntil', 'shezar_program') . html_writer::end_tag('label');
        $out .= html_writer::empty_tag('input', array('class' => 'extensiontime', 'type' => 'text', 'name' => 'extensiontime', 'id' => 'extensiontime', 'size' => '20', 'maxlength' => '10', 'placeholder' => get_string('datepickerlongyearplaceholder', 'shezar_core')));
        ...
        $out .= html_writer::start_tag('div');
        $out .= html_writer::start_tag('label', array('for' => 'extensionreason')) . get_string('reasonforextension', 'shezar_program') . html_writer::end_tag('label');
    ...
    public function display_duedate_highlight_info($duedate) {
        ...
        if (!empty($duedate)) {
            ...
            if ($duedate < $now) {
                    $out .= $this->notification(get_string('overdue', 'shezar_plan'), 'notifyproblem');
            } else {
                $days = floor(($duedate - $now) / DAYSECS);
                if ($days == 0) {
                    $out .= $this->notification(get_string('duetoday', 'shezar_plan'), 'notifyproblem');
                } else if ($days > 0 && $days < 10) {
                    $out .= $this->notification(get_string('dueinxdays', 'shezar_plan', $days), 'notifynotice');
                }
            }
            $out .= html_writer::end_tag('span');
        }
    ...
    protected function coursecat_category(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        ...
        $categorycount = shezar_get_category_item_count($coursecat->id, $type);
        ...
        if ($type == 'program') {
            $createcapability = 'shezar/program:createprogram';
        } else {
            $createcapability = 'shezar/certification:createcertification';
        }
        ...
        $categoryname = html_writer::link(new moodle_url('/shezar/program/index.php',
                        array('categoryid' => $coursecat->id, 'viewtype' => $type)),
                        $categoryname);
        $categoryname .= html_writer::tag('span', ' (' . $categorycount . ')',
                        array('title' => get_string('numberofprograms', 'shezar_program')));
    ...
    protected function coursecat_subcategories(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        ...
        if ($totalcount > count($subcategories)) {
            if ($paginationurl) {
                ...
                if ($viewmoreurl->compare(new moodle_url('/shezar/program/index.php'), URL_MATCH_BASE)) {
    ...
    protected function coursecat_category_content(programcat_helper $chelper, $coursecat, $depth, $type = 'program') {
        ...
        if ($chelper->get_show_programs() > shezar_program_renderer::COURSECAT_SHOW_PROGRAMS_COUNT) {
            $programs = array();
     ...
     *
     * Invoked from /shezar/program/index.php
     *
    ...
    public function program_category($category, $viewtype = 'program', $subcategoryforajax = false) {
        ...
        if (!$subcategoryforajax) {
            ...
            $label = ($viewtype == 'program') ? get_string('programcategories', 'shezar_program') :
                                                get_string('certifcategories', 'shezar_certification');
            if (!$coursecat->id) {
                if (can_edit_in_category()) {
                    // Add 'Manage' button instead of program search form.
                    $titlebutton = ($viewtype == 'program' ? get_string('manageprograms', 'admin') :
                                                             get_string('managecertifications', 'shezar_certification'));
                    $managebutton = $this->single_button(new moodle_url('/shezar/program/manage.php', array('viewtype' => $viewtype)),
                                    $titlebutton, 'get');
                ...
                if (coursecat::count_all() == 1) {
                    ...
                    $strfulllistofprograms = get_string('fulllistofprograms', 'shezar_program');
                    $this->page->set_title("$site->shortname: $strfulllistofprograms");
                } else {
                    $this->page->set_title($label);
                    $this->page->navbar->add($label, new moodle_url('/shezar/program/index.php', array('viewtype' => $viewtype)));
                }
            } else {
                ...
                $select = new single_select(new moodle_url('/shezar/program/index.php', array('viewtype' => $viewtype)), 'categoryid',
        ...
        $baseurl = new moodle_url('/shezar/program/index.php');
        if ($coursecat->id) {
        ...
        if ($browse === 'categories' || !$hasitems && $coursecat->id != 0) {
            ...
            $programdisplayoptions['viewmoretext'] = new lang_string("viewall{$viewtype}s", "shezar_{$viewtype}");
        } else {
        ...
        if (!$subcategoryforajax) {
            ...
            if ($viewtype == 'program' && has_capability('shezar/program:createprogram', $context)) {
                // Print link to create a new program, for the 1st available category.
                if ($coursecat->id) {
                    $url = new moodle_url('/shezar/program/add.php', array('category' => $coursecat->id, 'returnto' => 'category'));
                } else {
                    $url = new moodle_url('/shezar/program/add.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
                }
                $output .= $this->single_button($url, get_string('addnewprogram', 'shezar_program'), 'get');
            } else if (has_capability('shezar/certification:createcertification', $context)) {
                if ($coursecat->id) {
                    $url = new moodle_url('/shezar/certification/add.php', array('category' => $coursecat->id, 'returnto' => 'category'));
                } else {
                    $url = new moodle_url('/shezar/certification/add.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
                }
                $output .= $this->single_button($url, get_string('addnewcertification', 'shezar_certification'), 'get');
            }
    ...
    public function progcat_ajax($category, $categorytype) {
        ...
        $chelper = new programcat_helper();
        $baseurl = new moodle_url('/shezar/program/index.php', array('categoryid' => $category->id));
    ...
    function program_search_form($type = 'program', $value = '', $format = 'plain') {
        ...
        if ($type == 'program') {
            $strsearchprograms = get_string('searchprograms', 'shezar_program');
        } else {
            $strsearchprograms = get_string('searchcertifications', 'shezar_certification');
        }
        $searchurl = new moodle_url('/shezar/program/search.php');
        ...
        if (!empty($searchcriteria)) {
            ...
            if (!$totalcount) {
                if (!empty($searchcriteria['search'])) {
                    $content .= html_writer::tag('p', get_string('noprogramsfound', 'shezar_program', $searchcriteria['search']));
                } else {
                    $content .= html_writer::tag('p', get_string('novalidprograms', 'shezar_program'));
                }
            } else {
     ...
     * please use {@link shezar_program_renderer::program_info_box()}
     *
     * @param programcat_helper $chelper various display options
    ...
    protected function coursecat_programbox(programcat_helper $chelper, $program, $additionalclasses = '') {
        ...
        $programicon = shezar_get_icon($program->id, shezar_ICON_TYPE_PROGRAM);
        $dimmed = shezar_get_style_visibility($program);
        $programnamelink = html_writer::link(new moodle_url('/shezar/program/view.php', array('id' => $program->id)),
        ...
        if ($chelper->get_show_programs() < self::COURSECAT_SHOW_PROGRAMS_EXPANDED) {
            if ($program->has_summary() || $program->has_program_overviewfiles()) {
                $url = new moodle_url('/shezar/program/info.php', array('id' => $program->id));
    ...
    public function coursecat_programbox_content(programcat_helper $chelper, $program) {
        ...
        foreach ($program->get_program_overviewfiles() as $file) {
            ...
            $url = moodle_url::make_pluginfile_url($file->get_contextid(), 'shezar_program',
        ...
        if ($chelper->get_show_programs() == self::COURSECAT_SHOW_PROGRAMS_EXPANDED_WITH_CAT) {
            ...
            if ($cat = coursecat::get($program->category, IGNORE_MISSING)) {
                ...
                $content .= get_string('category').': '.
                                html_writer::link(new moodle_url('/shezar/program/index.php', array('categoryid' => $cat->id)),
    ...
    /**
     * Serves requests to /shezar/program/category.ajax.php
     *
    ...
    public function program_description_ajax(program $program) {
        ...
        $summary = file_rewrite_pluginfile_urls($program->summary, 'pluginfile.php', $context->id, 'shezar_program', 'summary', 0);
    ...
    protected function include_js() {
        ...
        if (!$jsloaded) {
            ...
            $this->page->requires->yui_module('moodle-shezar_program-categoryexpander', 'M.program.categoryexpander.init');
            $jsloaded = true;
        }
    }
    ...
    public function get_completion_checker_results($data) {
            ...
            if (!empty($errors)) {
                ...
                if (!isset($aggregatedata[$problemkey])) {
                    ...
                    foreach ($errors as $errorkey => $errorfield) {
                        $errorstrings[] = get_string($errorkey, 'shezar_program');
                    }
                ...
                $userurl = new moodle_url('/shezar/program/edit_completion.php',
        ...
        if (!empty($data->progname)) {
            $out .= html_writer::tag('p', get_string('completionfilterbyprogram', 'shezar_program', $data->progname));
        }
        if (!empty($data->username)) {
            $out .= html_writer::tag('p', get_string('completionfilterbyuser', 'shezar_program', $data->username));
        }
        $out .= html_writer::tag('p', get_string('completionrecordcounttotal', 'shezar_program', $count));
        $out .= html_writer::tag('p', get_string('completionrecordcountproblem', 'shezar_program', $problemcount));
...
class programcat_helper extends coursecat_helper {
    ...
    protected $showprograms = 20; /* shezar_program_renderer::COURSECAT_SHOW_PROGRAMS_EXPANDED */
    ...
    public function set_show_programs($showprograms) {
        ...
        $this->programsdisplayoptions['summary'] = $showprograms >= shezar_program_renderer::COURSECAT_SHOW_PROGRAMS_AUTO;
        return $this;
    }
