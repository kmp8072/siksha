...
function xmldb_shezar_message_upgrade($oldversion) {
    ...
    if ($oldversion < 2012012701) {
        ...
        $types = array('alert' => 'shezar_msg_send_alrt_emails', 'task' => 'shezar_msg_send_task_emails');
        foreach ($types as $type => $oldsetting) {

            // find old settings
            $prefs = $DB->get_records('user_preferences', array('name' => $oldsetting));
            foreach ($prefs as $pref) {
                $newpref = "shezar_{$type}";
                if ($pref->value == 1) {
                    $newpref .= ',email';
                }

                // set new ones
                set_user_preference("message_provider_shezar_message_{$type}_loggedin", $newpref, $pref->userid);
                set_user_preference("message_provider_shezar_message_{$type}_loggedoff", $newpref, $pref->userid);
    ...
    if ($oldversion < 2012012702) {
        ...
        if ($dbman->table_exists($table)) {
            ...
            foreach ($msgs as $msg) {
                unset($msg->id);
                //fix contexturl to change /local/ to /shezar/ for shezar modules only
                $msg->contexturl = str_replace('/local/plan','/shezar/plan', $msg->contexturl);
                $msg->contexturl = str_replace('/local/program','/shezar/program', $msg->contexturl);
            ...
            $count = count($msgs);
            if ($count > 0) {
                $pbar = new progress_bar('migrateshezarmessages', 500, true);
                $i = 0;
                ...
                foreach ($msgs as $msg) {
                    ...
                    $pbar->update($i, $count, "Migrating shezar messages - message $i/$count.");
                }
                $pbar->update($count, $count, "Migrated shezar messages - done!");
            }
            ...
            echo $OUTPUT->notification('shezar/message: Recreated existing alerts and tasks ('.count($msgs).')', 'notifysuccess');
        }
        ...
        echo $OUTPUT->notification('Dropping obsolete shezar_msg tables', 'notifysuccess');
    ...
    if ($oldversion < 2012120400) {
        ...
        shezar_upgrade_mod_savepoint(true, 2012120400, 'shezar_message');
    }

    if ($oldversion < 2013020700) {
        $reportlist = $DB->get_fieldset_select('report_builder', 'id', 'source = ?', array('shezarmessages'));
        if ($reportlist) {
        ...
        shezar_upgrade_mod_savepoint(true, 2013020700, 'shezar_message');
    }
