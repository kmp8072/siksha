...
define('shezar_MSG_STATUS_UNDECIDED', 0);
define('shezar_MSG_STATUS_OK', 1);
define('shezar_MSG_STATUS_NOTOK', 2);

// message type constants
define('shezar_MSG_TYPE_UNKNOWN', 0);
define('shezar_MSG_TYPE_COURSE', 1);
define('shezar_MSG_TYPE_FORUM', 2);
define('shezar_MSG_TYPE_GRADING', 3);
define('shezar_MSG_TYPE_CHAT', 4);
define('shezar_MSG_TYPE_LESSON', 5);
define('shezar_MSG_TYPE_QUIZ', 6);
define('shezar_MSG_TYPE_FACE2FACE', 7);
define('shezar_MSG_TYPE_SURVEY', 8);
define('shezar_MSG_TYPE_SCORM', 9);
define('shezar_MSG_TYPE_LINK', 10);
define('shezar_MSG_TYPE_PROGRAM', 11);

// Message email constants (used to override default message processor behaviour):
// Send email via processor as normal (default)
define('shezar_MSG_EMAIL_YES', 0);
// Override to prevent the message sending an email (even if the user has asked for it)
define('shezar_MSG_EMAIL_NO', 1);
...
define('shezar_MSG_EMAIL_MANUAL', 2);

// message type shortnames
global $shezar_MESSAGE_TYPES;
$shezar_MESSAGE_TYPES = array(
    shezar_MSG_TYPE_UNKNOWN => 'unknown',
    shezar_MSG_TYPE_COURSE => 'course',
    shezar_MSG_TYPE_PROGRAM => 'program',
    shezar_MSG_TYPE_FORUM => 'forum',
    shezar_MSG_TYPE_GRADING => 'grading',
    shezar_MSG_TYPE_CHAT => 'chat',
    shezar_MSG_TYPE_LESSON => 'lesson',
    shezar_MSG_TYPE_QUIZ => 'quiz',
    shezar_MSG_TYPE_FACE2FACE => 'face2face',
    shezar_MSG_TYPE_SURVEY => 'survey',
    shezar_MSG_TYPE_SCORM => 'scorm',
    shezar_MSG_TYPE_LINK => 'link',
);

// list of supported categories
global $shezar_MESSAGE_CATEGORIES;
$shezar_MESSAGE_CATEGORIES = array_merge(array_keys($shezar_MESSAGE_TYPES),
        array('facetoface', 'learningplan', 'objective', 'resource'));

// message urgency constants
define('shezar_MSG_URGENCY_LOW', -4);
define('shezar_MSG_URGENCY_NORMAL', 0);
define('shezar_MSG_URGENCY_URGENT', 4);
 ...
 *  msgstatus - int Message Status see shezar_MSG_STATUS* constants
 *  msgtype - int Message Type see shezar_MSG_TYPE* constants
 *
...
function tm_message_send($eventdata) {
    ...
    $eventdata->userfrom = shezar_get_user_from($eventdata->userfrom);
    ...
    // call core message processing - this will trigger either output_shezar_task output_shezar_alert
...
function tm_insert_metadata($eventdata, $processorid) {
    ...
    $eventdata->msgtype = isset($eventdata->msgtype) ? $eventdata->msgtype : shezar_MSG_TYPE_UNKNOWN;
    $eventdata->msgstatus = isset($eventdata->msgstatus) ? $eventdata->msgstatus : shezar_MSG_STATUS_UNDECIDED;
    $eventdata->urgency = isset($eventdata->urgency) ? $eventdata->urgency : shezar_MSG_URGENCY_NORMAL;
...
function tm_alert_send($eventdata) {
    ...
    (!isset($eventdata->msgtype)) && $eventdata->msgtype = shezar_MSG_TYPE_UNKNOWN;
    (!isset($eventdata->msgstatus)) && $eventdata->msgstatus = shezar_MSG_STATUS_UNDECIDED;
    (!isset($eventdata->urgency)) && $eventdata->urgency = shezar_MSG_URGENCY_NORMAL;
    (!isset($eventdata->sendemail)) && $eventdata->sendemail = shezar_MSG_EMAIL_YES;

    $eventdata->component         = 'shezar_message';
    ...
    $alertemailpref = get_user_preferences('message_provider_shezar_message_alert_loggedoff', null, $eventdata->userto->id);
    if ($result && strpos($alertemailpref, 'email') !== false && $eventdata->sendemail == shezar_MSG_EMAIL_MANUAL) {
        ...
        if ($eventdata->contexturl) {
            $eventdata->fullmessagehtml .= html_writer::empty_tag('br') . html_writer::empty_tag('br') . $string_manager->get_string('viewdetailshere', 'shezar_message', $eventdata->contexturl, $eventdata->userto->lang);
        }
        ...
        if (core_user::is_real_user($eventdata->userto->id)) {
            $footer = $string_manager->get_string('alertfooter2', 'shezar_message', $CFG->wwwroot."/message/edit.php", $eventdata->userto->lang);
...
function tm_task_send($eventdata) {
    ...
    if (!empty($eventdata->contexturl)) {
        $eventdata->fullmessagehtml .= html_writer::empty_tag('br').html_writer::empty_tag('br').get_string('viewdetailshere', 'shezar_message', $eventdata->contexturl);
    }
 ...
 *          $a->plan = s($this->name);
 *          $event->subject = get_string('plan-request-manager-short', 'shezar_plan', $a);
 *          $event->fullmessage = get_string('plan-request-manager-long', 'shezar_plan', $a);
 *          $event->acceptbutton = get_string('approve', 'shezar_plan').' '.get_string('plan', 'shezar_plan');
 *          $event->accepttext = get_string('approveplantext', 'shezar_plan');
 *
 *
 * @param object $task information about the message (userfrom, userto, ...)
 * @return boolean success
...
function tm_workflow_send($eventdata) {
    ...
    $eventdata->msgtype = shezar_MSG_TYPE_LINK; // tells us how to treat the display
    (!isset($eventdata->msgstatus)) && $eventdata->msgstatus = shezar_MSG_STATUS_UNDECIDED;
    (!isset($eventdata->urgency)) && $eventdata->urgency = shezar_MSG_URGENCY_NORMAL;
...
function tm_message_task_accept($id, $reasonfordecision) {
    ...
    if ($message) {
        // get the event data
        $eventdata = shezar_message_eventdata($id, 'onaccept');
...
function tm_message_task_link($id) {
    ...
    if ($message) {
        // get the event data
        $eventdata = shezar_message_eventdata($id, 'oninfo');
...
function tm_message_task_reject($id, $reasonfordecision) {
    ...
    if ($message) {
        // Get the event data.
        $eventdata = shezar_message_eventdata($id, 'onreject');
        if (!empty($reasonfordecision)) {
            $eventdata->data['reasonfordecision'] = $reasonfordecision;
        }
        // grab the onaccept handler
...
function tm_message_workflow_object($action) {
    ...
    require_once($CFG->dirroot.'/shezar/message/workflow/lib.php');
    $file = $CFG->dirroot.'/shezar/message/workflow/plugins/'.$action.'/workflow_'.$action.'.php';
    if (!file_exists($file)) {
    ...
    // create the object
    $ctlclass = 'shezar_message_workflow_'.$action;
...
/**
 * Set the default config values for shezar ouput types on install
 *
 * @param string $output
 */
function tm_set_preference_defaults() {
    set_config('shezar_task_provider_shezar_message_task_permitted', 'permitted', 'message');
    set_config('shezar_alert_provider_shezar_message_alert_permitted', 'permitted', 'message');
    set_config('shezar_alert_provider_shezar_message_task_permitted', 'disallowed', 'message');
    set_config('message_provider_shezar_message_alert_loggedin', 'shezar_alert,email', 'message');
    set_config('message_provider_shezar_message_alert_loggedoff', 'shezar_alert,email', 'message');
    set_config('message_provider_shezar_message_task_loggedin', 'shezar_task,email', 'message');
    set_config('message_provider_shezar_message_task_loggedoff', 'shezar_task,email', 'message');
    set_config('popup_provider_shezar_message_task_permitted', 'disallowed', 'message');
    set_config('email_provider_shezar_message_task_permitted', 'permitted', 'message');
    set_config('jabber_provider_shezar_message_task_permitted', 'disallowed', 'message');
    set_config('shezar_task_provider_shezar_message_alert_permitted', 'disallowed', 'message');
    set_config('popup_provider_shezar_message_alert_permitted', 'permitted', 'message');
    set_config('email_provider_shezar_message_alert_permitted', 'permitted', 'message');
    set_config('jabber_provider_shezar_message_alert_permitted', 'permitted', 'message');
}
...
function tm_message_set_default_message_preferences($user) {
    ...
    foreach ($providers as $providerid => $provider) {
        ...
        if ($provider->name == 'instantmessage') {
            ...
            //shezar alert
            $offlineprocessortouse = $onlineprocessortouse = 'shezar_alert,email';
        }
        else if ($provider->name == 'task') {
            //shezar task
            $offlineprocessortouse = $onlineprocessortouse = 'shezar_task,email';
        }
        else {
