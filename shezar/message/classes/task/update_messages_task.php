...
class update_messages_task extends \core\task\scheduled_task {
    ...
    const shezar_MSG_CRON_DISMISS_ALERTS = 30;

    // Age for expiring undismissed tasks - days.
    const shezar_MSG_CRON_DISMISS_TASKS = 30;
    ...
    public function get_name() {
        return get_string('updatemessagestask', 'shezar_message');
    }
    ...
    public function execute() {
        ...
        // Dismiss old alerts.
        $time = time() - (self::shezar_MSG_CRON_DISMISS_ALERTS * (24 * 60 * 60));
        $msgs = $this->tm_messages_get_by_time('shezar_alert', $time);
        $deleted = array();
        foreach ($msgs as $msg) {
        ...
        // Dismiss old tasks.
        $time = time() - (self::shezar_MSG_CRON_DISMISS_TASKS * (24 * 60 * 60));
        $msgs = $this->tm_messages_get_by_time('shezar_task', $time);
        foreach ($msgs as $msg) {
            tm_message_dismiss($msg->id);
            // Store message ids for bulk delete.
            if (!in_array($msg->id, $deleted)) {
