...
class shezar_message_update_messages_testcase extends advanced_testcase {
    public function test_name_present() {
        $task = new \shezar_message\task\update_messages_task();
        $task->get_name();
    }
    ...
    public function test_execute() {
        ...
        $messages[0]->timecreated = $messages[0]->timecreated - (24*60*60*\shezar_message\task\update_messages_task::shezar_MSG_CRON_DISMISS_ALERTS) - 3600;
        $DB->update_record('message', $messages[0]);

        $messages[3]->timecreated = $messages[3]->timecreated - (24*60*60*\shezar_message\task\update_messages_task::shezar_MSG_CRON_DISMISS_TASKS) - 3600;
