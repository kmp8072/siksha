 ...
 */
M.shezar_message = M.shezar_message || {
    ...
    create_dialog: function( Y, id, selector, clean_fullme, sesskey, extrabuttonjson ){
        // dismiss dialog
        (function() {
            var url = M.cfg.wwwroot+'/shezar/message/';
            var handler = new shezarDialog_handler_confirm();
            var name = selector+id;
            var buttonsObj = {};
            buttonsObj[M.util.get_string('dismiss', 'shezar_message')] = function() { handler._confirm(M.cfg.wwwroot+'/shezar/message/dismiss.php?id='+id+'&sesskey='+sesskey, clean_fullme) };
            ...
            shezarDialogs[name] = new shezarDialog(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>'+M.util.get_string('reviewitems', 'block_shezar_alerts')+'</h2>',
                    width: dialogueWidth,
                    height: 400
                },
    ...
    create_action_dialog: function( Y, action, action_str, clean_fullme, sesskey ){
        ...
        (function() {
            ...
            var name = action+'msg';
            var buttonsObj = {};
            buttonsObj[action_str] = function() { handler._confirm(M.cfg.wwwroot+'/shezar/message/action.php?'+action+'='+action+'&sesskey='+sesskey, clean_fullme) };
            buttonsObj[M.util.get_string('cancel', 'moodle')] = function() { handler._cancel() };
            ...
            shezarDialogs[name].load = function(url, method, onsuccess) {
                    // Add loading animation
                    this.dialog.html('');
                    this.showLoading();
                    msgids = [];
                    $('form#shezar_messages input[type="checkbox"]:checked').each(
                                function () {
    ...
    create_accept_reject_dialog: function( Y, id, type, type_str, dialog_title, returnto, sesskey ){
        ...
        (function() {
            ...
            var buttonsObj = {};
            buttonsObj[type_str] = function() { handler._confirm(M.cfg.wwwroot+'/shezar/message/'+type+'.php?id='+id+'&sesskey='+sesskey, returnto) };
            buttonsObj[M.util.get_string('cancel', 'moodle')] = function() { handler._cancel() };
    ...
    dismiss_input_toggle: function(){
        $('#shezar_messages input[type=checkbox]').bind('click', function() {
            if ($('form#shezar_messages input[type=checkbox]:checked').length) {
                $('#shezar-dismiss').attr('disabled', false);
            } else {
                $('#shezar-dismiss').attr('disabled', true);
            }
        });
    },
    ...
    select_all_none_checkbox: function(){
        $('th.message_values_dismiss_link').html('<div id="shezar_message_selects"><a id="all" href="#">'+M.util.get_string('all', 'moodle')+
                                        '</a>/<a id="none" href="#">'+M.util.get_string('none', 'moodle')+'</a></div>');
        function jqCheckAll(flag) {
           if (flag === false) {
              $("form#shezar_messages [type='checkbox']").prop('checked', false);
              if ($('form#shezar_messages input[type=checkbox]:checked').length) {
                  $('#shezar-dismiss').attr('disabled', false);
              } else {
                  $('#shezar-dismiss').attr('disabled', true);
              ...
              $("form#shezar_messages [type='checkbox']").prop('checked', true);
              if ($('form#shezar_messages input[type=checkbox]:checked').length) {
                  $('#shezar-dismiss').attr('disabled', false);
              } else {
                  $('#shezar-dismiss').attr('disabled', true);
        ...
        $('#shezar_message_selects #all').click(function() {jqCheckAll(true); return false;});
        $('#shezar_message_selects #none').click(function() {jqCheckAll(false); return false;});
    }
};

/*****************************************************************************/
/** shezarDialog_handler_confirm **/
...
shezarDialog_handler_confirm = function() {};
shezarDialog_handler_confirm.prototype = new shezarDialog_handler();
 ...
 * @return void
 */
shezarDialog_handler_confirm.prototype._confirm = function(url, returnto, extrabutton) {
 ...
 * @return void
 */
shezarDialog_handler_confirm.prototype.setReturnTo = function(url) {
    this._returnTo = url;
    return;
};
 ...
 * @return void
 */
shezarDialog_handler_confirm.prototype._redirect = function() {
    this._dialog.hide();
