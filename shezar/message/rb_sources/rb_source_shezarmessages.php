...
class rb_source_shezarmessages extends rb_base_source {
    ...
    public function __construct($groupid, rb_global_restriction_set $globalrestrictionset = null) {
        ...
        $this->sourcetitle = get_string('sourcetitle', 'rb_source_shezarmessages');
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions = array(
            new rb_column_option(
                'message_values',
                'subject',
                get_string('subject', 'rb_source_shezarmessages'),
                'msg.subject',
                array('joins' => 'msg',
                      'dbdatatype' => 'text',
                      'outputformat' => 'text')
            ...
            new rb_column_option(
                'message_values',
                'statement',
                get_string('statement', 'rb_source_shezarmessages'),
                'msg.fullmessagehtml',
                array('joins' => 'msg',
                      'dbdatatype' => 'text',
                      'outputformat' => 'text')
            ...
            new rb_column_option(
                'message_values',
                'urgency',
                get_string('msgurgencyicon', 'rb_source_shezarmessages'),
                'base.urgency',
                array(
                    'defaultheading' => get_string('msgurgency', 'rb_source_shezarmessages'),
            ...
            new rb_column_option(
                'message_values',
                'urgency_text',
                get_string('msgurgency', 'rb_source_shezarmessages'),
                'base.urgency',
            ...
            new rb_column_option(
                'message_values',
                'msgtype',
                get_string('msgtype', 'rb_source_shezarmessages'),
                'base.msgtype',
                array(
                    'joins' => array('msg'),
            ...
            new rb_column_option(
                'message_values',
                'msgtype_text',
                get_string('msgtypetext', 'rb_source_shezarmessages'),
                'base.msgtype',
                array(
                    'defaultheading' => get_string('msgtype', 'rb_source_shezarmessages'),
            ...
            new rb_column_option(
                'message_values',
                'category',
                get_string('msgcategory', 'rb_source_shezarmessages'),
            ...
            new rb_column_option(
                'message_values',
                'sent',
                get_string('sent', 'rb_source_shezarmessages'),
                'msg.timecreated',
                array('joins' => 'msg',
            ...
            new rb_column_option(
                'message_values',
                'checkbox',
                get_string('select', 'rb_source_shezarmessages'),
                'base.messageid',
            ...
            new rb_column_option(
                'message_values',
                'msgid',
                get_string('msgid', 'rb_source_shezarmessages'),
                'base.messageid',
                array('nosort' => true,
                      'noexport' => true,
                      'hidden' => 1,
    ...
    protected function define_filteroptions() {
        $filteroptions = array(
            new rb_filter_option(
                'message_values',       // type
                'sent',                 // value
                get_string('datesent', 'rb_source_shezarmessages'),            // label
                'date',                 // filtertype
                array()                 // options
            ),
            new rb_filter_option(
    ...
    protected function define_requiredcolumns() {
        $requiredcolumns = array(
            new rb_column(
                'message_values',
                'dismiss_link',
                get_string('dismissmsg', 'rb_source_shezarmessages'),
                'base.messageid',
    ...
    function rb_display_urgency_link($comp, $row, $export = 0) {
        global $OUTPUT;
        $display = shezar_message_urgency_text($row->message_values_urgency);
        if ($export) {
            return $display['text'];
        }
    ...
    function rb_display_urgency_text($urgency, $row) {
        $display = shezar_message_urgency_text($urgency);
        return $display['text'];
    }
    ...
    function rb_display_msgtype_link($comp, $row, $export = 0) {
        ...
        return $OUTPUT->pix_icon("/msgicons/" . $icon, $subject, 'shezar_core', array('title' => $subject));
    }

    // generate status type text
    function rb_display_msgtype_text($msgtype, $row) {
        $display = shezar_message_msgtype_text($msgtype);
        return $display['text'];
    }
    ...
    function rb_display_msgcategory_text($comp, $row) {
        ...
        if ($comp != '' && in_array($comp, $shezar_MESSAGE_CATEGORIES)) {
            return get_string($comp, 'shezar_message');
        }
        return $comp;
    }
    ...
    function rb_display_dismiss_link($id, $row) {
        $out = shezar_message_dismiss_action($id);
        $out .= html_writer::checkbox('shezar_message_' . $id, $id, false, '', array('id' => 'shezar_msgcbox_'.$id, 'class' => "selectbox"));
    ...
    function rb_display_task_links($id, $row) {
        ...
        $out = shezar_message_accept_reject_action($id);
        $out .= shezar_message_dismiss_action($id);
    ...
    function rb_display_message_checkbox($id, $row) {
        return html_writer::checkbox('shezar_message_' . $id, $id, false, '', array('id' => 'shezar_message', 'class' => "selectbox"));
    }
    ...
    function rb_filter_message_urgency_list() {
        ...
        $urgencyselect = array();
        $urgencyselect[shezar_MSG_URGENCY_NORMAL] = get_string('urgencynormal', 'shezar_message');
        $urgencyselect[shezar_MSG_URGENCY_URGENT] = get_string('urgencyurgent', 'shezar_message');
    ...
    function rb_filter_message_type_list() {
        ...
        foreach (array('competency', 'course', 'evidence',
            ...
            $typename = get_string($type, 'shezar_message');
            $out[$type] = $OUTPUT->pix_icon('/msgicons/' . $type . '-regular', $typename, 'shezar_core') . '&nbsp;' . $typename;
        }
