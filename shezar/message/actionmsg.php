...
foreach ($_POST as $parm => $value) {
    if (preg_match('/^shezar\_message\_(\d+)$/', $parm)) {
...
foreach ($msgids as $msgid) {
    ...
    if ($msgid) {
        ...
        if (!$message || $message->useridto != $USER->id || !confirm_sesskey()) {
            print_error('notyours', 'shezar_message', $msgid);
        }
        $ids[$msgid] = $message;
    }
}
...
$tab = new html_table();
$tab->head  = array(get_string('type', 'block_shezar_alerts'),
                     get_string('from', 'block_shezar_alerts'),
                     get_string('statement', 'block_shezar_alerts'));
...
foreach ($ids as $msgid => $msg) {
    ...
    if ($accept && isset($metadata->onaccept) && $metadata->msgtype == shezar_MSG_TYPE_LINK) {
        continue;
    }

    $display = isset($metadata->msgtype) ? shezar_message_msgtype_text($metadata->msgtype) : array('icon' => '', 'text' => '');
    $type = $display['icon'];
    ...
    $icon = $OUTPUT->pix_icon('/msgicons/'.$metadata->icon, format_string($msg->subject), 'shezar_core', array('class'=>'msgicon', 'title' => format_string($msg->subject)));
    $cells = array();
