...
define('shezar_MSG_ALERT_LIMIT', 5);
define('shezar_MSG_TASK_LIMIT', 5);
 ...
 */
function shezar_message_msgstatus_text($msgstatus) {
    global $CFG;

    if ($msgstatus == shezar_MSG_STATUS_OK) {
        $status = 'go';
        $text = get_string('statusok', 'block_shezar_alerts');
    }
    else if ($msgstatus == shezar_MSG_STATUS_NOTOK) {
        $status = 'stop';
        $text = get_string('statusnotok', 'block_shezar_alerts');
    }
    else {
        $status = 'grey_undecided';
        $text = get_string('statusundecided', 'block_shezar_alerts');
    }
 ...
 * @return array('text' => , 'icon' => '')
 */
function shezar_message_urgency_text($urgency) {
    global $CFG;

    if ($urgency == shezar_MSG_URGENCY_URGENT) {
        $level = 'stop';
        $text = get_string('urgent', 'block_shezar_alerts');
    }
    else {
        $level = 'go';
        $text = get_string('normal', 'block_shezar_alerts');
    }
 ...
 * @return array('text' => , 'icon' => '')
 */
function shezar_message_cssclass($msgtype) {
    global $shezar_MESSAGE_TYPES;

    return $shezar_MESSAGE_TYPES[$msgtype];
}
 ...
 * @return array('text' => '', 'icon' => '')
 */
function shezar_message_msgtype_text($msgtype) {
    global $CFG, $shezar_MESSAGE_TYPES;

    if (array_key_exists($msgtype, $shezar_MESSAGE_TYPES)) {
        $text = get_string($shezar_MESSAGE_TYPES[$msgtype], 'shezar_message');
    } else {
        $text = get_string('unknown', 'shezar_message');
    }
 ...
 */
function shezar_message_eventdata($id, $event, $metadata=null) {
    global $DB;
...
function shezar_message_dismiss_action($id) {
    ...
    $out .= html_writer::tag('noscript',
        html_writer::tag('form',
            ...
            html_writer::empty_tag('input', array('type' => 'image', 'class' => 'iconsmall', 'src' => $OUTPUT->pix_url('t/delete_grey', 'shezar_core'), 'title' => $str, 'alt' => $str)),
        array('action' => $CFG->wwwroot . '/shezar/message/dismiss.php?id=' . $id, 'method' => 'post'))
    );
    return $out;
}
 ...
 */
function shezar_message_alert_popup($id, $extrabuttons=array(), $messagetype) {
    ...
    $PAGE->requires->string_for_js('cancel', 'moodle');
    $PAGE->requires->string_for_js('dismiss', 'shezar_message');
    $PAGE->requires->string_for_js('reviewitems', 'block_shezar_alerts');
    ...
    if ($eventdata && isset($eventdata->action)) {
        switch ($eventdata->action) {
            ...
            case 'prog_extension':
                ...
                if (empty($CFG->enableprogramextensionrequests) || !shezar_prog_extension_allowed($eventdata->data['programid'])) {
 ...
 */
function shezar_message_checkbox_all_none() {
    global $PAGE;
    $PAGE->requires->strings_for_js(array('all','none'), 'moodle');
    $PAGE->requires->js_init_call('M.shezar_message.select_all_none_checkbox');
}
 ...
 */
function shezar_message_action_button($action) {
    global $CFG, $FULLME, $PAGE;

    $clean_fullme = clean_param($FULLME, PARAM_LOCALURL);
    // Button Lang Strings
    $str = get_string($action, 'shezar_message');
    $PAGE->requires->string_for_js('cancel', 'moodle');

    $args = array('action'=>$action, 'action_str' => $str, 'clean_fullme'=>$clean_fullme, 'sesskey'=>sesskey());
    $PAGE->requires->js_init_call('M.shezar_message.create_action_dialog', $args);
}
 ...
 */
function shezar_message_accept_reject_action($id) {
    global $CFG, $FULLME, $PAGE, $DB, $OUTPUT;

    debugging("shezar_message_accept_reject_action() has been deprecated because it is no longer used in the codebase.", DEBUG_DEVELOPER);
    ...
    $msgacceptdata = shezar_message_eventdata($id, 'onaccept');

    $returnto = ($msgmeta->msgtype == shezar_MSG_TYPE_LINK && isset($msgacceptdata->data['redirect'])) ? $msgacceptdata->data['redirect'] : $clean_fullme;

    // Validate redirect
    $return_host = parse_url($returnto);
    $site_host = parse_url($CFG->wwwroot);
    if ($return_host['host'] != $site_host['host']) {
        print_error('error:redirecttoexternal', 'shezar_message');
    }

    $subject = format_string($msg->subject);
    $onaccept_str = format_string(isset($msgacceptdata->acceptbutton) ? $msgacceptdata->acceptbutton : get_string('onaccept', 'block_shezar_tasks'));
    $onreject_str = get_string('onreject', 'block_shezar_tasks');
    ...
    if (!empty($msgmeta->onaccept)) {
        ...
        $PAGE->requires->js_init_call('M.shezar_message.create_accept_reject_dialog', $args);
        ...
        $out .= html_writer::tag('noscript',
            ...
            array('action' => $CFG->wwwroot . '/shezar/message/accept.php?id=' . $id, 'method' => 'post'))
        );
    }
    if (!empty($msgmeta->onreject)) {
        ...
        $out .= html_writer::tag('noscript',
            ...
            array('action' => $CFG->wwwroot . '/shezar/message/reject.php?id=' . $id, 'method' => 'post'))
        );
    }
    return $out;
}
