...
require_once($CFG->dirroot . '/shezar/completionimport/reset_form.php');
...
$heading = get_string('resetimport', 'shezar_completionimport');
$thisurl = '/shezar/completionimport/reset.php';

$PAGE->set_context($context);
$PAGE->set_heading(format_string($heading));
$PAGE->set_title(format_string($heading));
$PAGE->set_url($thisurl, $pageparams);
admin_externalpage_setup('shezar_completionimport_reset');
...
if ($pageparams['confirm']) {
    if (confirm_sesskey()) {
        ...
        if ($pageparams['certification'] && shezar_feature_visible('certifications')) {
    ...
    echo $OUTPUT->confirm(get_string('resetconfirm', 'shezar_completionimport', $toreset), $confirmurl, $thisurl);
} else {
    $mform->display();
}
echo $OUTPUT->footer();