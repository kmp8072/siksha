 ...
 *
 * Please note that all functions must have a matching entry in shezar/job/db/services.php
 * Internally all functions must perform full access and control checks.
 */
class shezar_job_external extends external_api {
    ...
    public static function resort_job_assignments($userid, array $newsortorders) {
        ...
        if (!shezar_job_can_view_job_assignments($targetuser)) {
        ...
        if (!shezar_job_can_edit_job_assignments($targetuser->id)) {
            // Generic error - we don't want to give too much away. Unless debugging is on.
        ...
        $jobassignments = \shezar_job\job_assignment::resort_all($targetuser->id, array_values($map));

        // Generate a result in the format described by \shezar_job_external::resort_job_assignments_returns.
        $result = [];
    ...
    public static function delete_job_assignment($userid, $jobassignmentid) {
        ...
        \shezar_job\job_assignment::delete($jobassignment);

        // Generate a result in the format described by \shezar_job_external::resort_job_assignments_returns.
        $result = [];
        foreach (\shezar_job\job_assignment::get_all($targetuser->id) as $jobassignment) {
            $result[] = ['jobassignid' => $jobassignment->id, 'sortorder' => $jobassignment->sortorder];
        }
        return $result;
    }
