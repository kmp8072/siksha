...
namespace shezar_job;

use Horde\Socket\Client\Exception;
use shezar_job\event\job_assignment_viewed;
use shezar_job\event\job_assignment_updated;
...
class job_assignment {
        ...
        try {
            ...
            if (!empty($descriptioneditor)) {
                ...
                file_postupdate_standard_editor($extrafields, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                    'shezar_job', 'job_assignment', $extrafields->id);
    ...
    public function __get($name) {
        ...
        if ($name === 'description') {
            $description = file_rewrite_pluginfile_urls($this->description, 'pluginfile.php', $TEXTAREA_OPTIONS['context']->id,
                'shezar_job', 'job_assignment', $this->id);
            return $description;
            ...
            if (!isset($this->fullname) || $this->fullname === "") {
                return get_string('jobassignmentdefaultfullname', 'shezar_job', $this->idnumber);
            } else {
    ...
    private function updated_temporary_manager($oldtempmanagerjaid, $oldtempmanagerexpirydate) {
        ...
        $msg->contexturlname = get_string('xpositions', 'shezar_job', fullname($user));
        $msgparams = (object)array(
            'staffmember' => fullname($user),
            'tempmanager' => fullname($newtempmanager),
            'expirytime' => userdate($this->tempmanagerexpirydate, get_string('datepickerlongyearphpuserdate', 'shezar_core')),
            'url' => $msg->contexturl
        );
        ...
        if (!empty($oldtempmanagerjaid) && self::get_with_id($oldtempmanagerjaid)->userid == $newtempmanager->id) {
            ...
            if ($oldtempmanagerexpirydate != $this->tempmanagerexpirydate) {
                ...
                // Notify staff member.
                $msg->userto = $user;
                $msg->subject = get_string('tempmanagerexpiryupdatemsgstaffsubject', 'shezar_job', $msgparams);
                $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgstaff', 'shezar_job', $msgparams);
                $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgstaff', 'shezar_job', $msgparams);
                tm_alert_send($msg);

                // Notify real manager.
                if (!empty($realmanager)) {
                    $msg->userto = $realmanager;
                    $msg->subject = get_string('tempmanagerexpiryupdatemsgmgrsubject', 'shezar_job', $msgparams);
                    $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgmgr', 'shezar_job', $msgparams);
                    $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgmgr', 'shezar_job', $msgparams);
                ...
                $msg->subject = get_string('tempmanagerexpiryupdatemsgtmpmgrsubject', 'shezar_job', $msgparams);
                $msg->fullmessage = get_string('tempmanagerexpiryupdatemsgtmpmgr', 'shezar_job', $msgparams);
                $msg->fullmessagehtml = get_string('tempmanagerexpiryupdatemsgtmpmgr', 'shezar_job', $msgparams);
                $msg->roleid = $CFG->managerroleid;
                tm_alert_send($msg);
            } else {
            ...
            // Notify staff member.
            $msg->userto = $user;
            $msg->subject = get_string('tempmanagerassignmsgstaffsubject', 'shezar_job', $msgparams);
            $msg->fullmessage = get_string('tempmanagerassignmsgstaff', 'shezar_job', $msgparams);
            $msg->fullmessagehtml = get_string('tempmanagerassignmsgstaff', 'shezar_job', $msgparams);
            tm_alert_send($msg);

            // Notify real manager.
            if (!empty($realmanager)) {
                $msg->userto = $realmanager;
                $msg->subject = get_string('tempmanagerassignmsgmgrsubject', 'shezar_job', $msgparams);
                $msg->fullmessage = get_string('tempmanagerassignmsgmgr', 'shezar_job', $msgparams);
                $msg->fullmessagehtml = get_string('tempmanagerassignmsgmgr', 'shezar_job', $msgparams);
                $msg->roleid = $CFG->managerroleid;
                tm_alert_send($msg);
            }
            ...
            // Notify temp manager.
            $msg->userto = $newtempmanager;
            $msg->subject = get_string('tempmanagerassignmsgtmpmgrsubject', 'shezar_job', $msgparams);
            $msg->fullmessage = get_string('tempmanagerassignmsgtmpmgr', 'shezar_job', $msgparams);
            $msg->fullmessagehtml = get_string('tempmanagerassignmsgtmpmgr', 'shezar_job', $msgparams);
            $msg->roleid = $CFG->managerroleid;
            tm_alert_send($msg);
        }
    }
    ...
    public static function delete(job_assignment &$jobassignment) {
        ...
        //\shezar_job\event\job_assignment_deleted::create_from_instance($this)->trigger();
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $data->appraiserid            = $this->appraiserid;
        $data->sortorder              = $this->sortorder;
        $data->editurl = new \moodle_url('/shezar/job/jobassignment.php', array('jobassignmentid' => $this->id));
