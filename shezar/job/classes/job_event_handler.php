...
class job_event_handler {
    ...
    public static function user_deleted(\core\event\user_deleted $event) {
        ...
        $jas = \shezar_job\job_assignment::get_all($userid);
        $jas = array_reverse($jas); // Reverse it, so that there's not lots of sort order messing around.
        foreach ($jas as $ja) {
            \shezar_job\job_assignment::delete($ja); // Will tidy up staff associations (manager, temp manager).
        }

        // Remove the user as appraiser in all job assignments.
        \shezar_job\job_assignment::update_to_empty_by_criteria('appraiserid', $userid);
    }
}
