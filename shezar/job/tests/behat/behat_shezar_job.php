 ...
 */
class behat_shezar_job extends behat_base {

    /**
     * Checks the number of job assignments the user has.
     *
     * @Given /^there should be "(\d+)" shezar job assignments$/
     *
     * @throws ExpectationException
     * @param int $count
     */
    public function there_should_be_x_shezar_job_assignments($count) {
        ...
        $elementxpath = '//*[contains(@class, \'shezar-job-management-listing\')]/*/li';
    ...
    public function job_assignment_at_position_X_should_be($position, $fullname) {
        // Note we don't use $this->find here - we should never ever need spin here.
        // If content becomes dynamic THEN we may need to convert this to spin, but lets try avoid it first.
        $elementxpath = '(//*[contains(@class, \'shezar-job-management-listing\')]/*/li)['.(string)(int)$position.']';
        $nodes = $this->getSession()->getDriver()->find($elementxpath);
        ...
        $li = reset($nodes);
        $link = $li->find('xpath', '/a[text()='.$this->getSession()->getSelectorsHandler()->xpathLiteral($fullname).']');
        if (empty($link)) {
    ...
    public function i_move_job_assignment($fullname, $direction) {
        ...
        // Note we don't use $this->find here - we should never ever need spin here.
        // If content becomes dynamic THEN we may need to convert this to spin, but lets try avoid it first.
        $fnliteral = $this->getSession()->getSelectorsHandler()->xpathLiteral($fullname);
        $elementxpath = '//*[contains(@class, \'shezar-job-management-listing\')]/*/li//a[text()='.$fnliteral.']/parent::li//a[@title="Move '.$direction.'"]';
        $nodes = $this->getSession()->getDriver()->find($elementxpath);
    ...
    public function i_click_the_delete_icon_for_the_job_assignment($fullname) {
        ...
        // If content becomes dynamic THEN we may need to convert this to spin, but lets try avoid it first.
        $fnliteral = $this->getSession()->getSelectorsHandler()->xpathLiteral($fullname);
        $elementxpath = '//*[contains(@class, \'shezar-job-management-listing\')]/*/li//a[text()='.$fnliteral.']/parent::li//a[@title="Delete"]';
        $nodes = $this->getSession()->getDriver()->find($elementxpath);
     ...
     *
     * @Given /^I (should|should not) be able to delete the "([^"]+)" shezar job assignment$/
     *
     * @throws ExpectationException
     * @throws DriverException
     * @param string $not
     */
    public function i_should_be_able_to_delete_the_shezar_job_assignment($not, $fullname) {
     ...
     * Check a user can/cannot sort a job assignment.
     *
     * @Given /^I (should|should not) be able to sort the "([^"]+)" shezar job assignment$/
     *
     * @throws ExpectationException
     * @throws DriverException
     * @param string $not
     */
    public function i_should_be_able_to_sort_the_shezar_job_assignment($not, $fullname) {
        ...
        // If content becomes dynamic THEN we may need to convert this to spin, but lets try avoid it first.
        $fnliteral = $this->getSession()->getSelectorsHandler()->xpathLiteral($fullname);
        $elementxpath = '//*[contains(@class, \'shezar-job-management-listing\')]/*/li//a[text()='.$fnliteral.']/parent::li//a[@data-function="move"]';
        $nodes = $this->getSession()->getDriver()->find($elementxpath);
