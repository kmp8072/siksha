...
require_capability('shezar/hierarchy:delete'.$prefix.'type', $sitecontext);
...
$back_url = "{$CFG->wwwroot}/shezar/hierarchy/type/index.php?prefix=$prefix&class=$class";
...
if (!$delete) {
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('deletetype', 'shezar_hierarchy', format_string($type->fullname)), 1);
    ...
    if ($itemsassigned = $DB->count_records($hierarchy->shortprefix, array('typeid' => $type->id))) {
        $strdelete = get_string('deletechecktypeassociated', 'shezar_hierarchy', $itemsassigned);
    } else {
        $strdelete = get_string('deletechecktype', 'shezar_hierarchy');
    }
    echo $OUTPUT->confirm($strdelete . html_writer::empty_tag('br') . html_writer::empty_tag('br'),
        "{$CFG->wwwroot}/shezar/hierarchy/type/delete.php?prefix={$prefix}&amp;id={$type->id}&amp;delete=".
...
if ($deleteresult === true) {
    ...
    shezar_set_notification(get_string($prefix.'deletedtype', 'shezar_hierarchy', $type->fullname),
        "{$CFG->wwwroot}/shezar/hierarchy/type/index.php?prefix={$prefix}&class={$class}", array('class' => 'notifysuccess'));
} else {
    shezar_set_notification(get_string($prefix.'error:deletedtype', 'shezar_hierarchy', $type->fullname),
        "{$CFG->wwwroot}/shezar/hierarchy/type/index.php?prefix={$prefix}&class={$class}");
}
