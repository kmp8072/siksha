...
if ($prefix == 'goal') {
    $returnurl = $CFG->wwwroot . '/shezar/hierarchy/type/index.php?prefix='. $prefix . '&class=' . $class;
} else {
    $returnurl = $CFG->wwwroot . '/shezar/hierarchy/type/index.php?prefix='. $prefix;
}
...
if (file_exists($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/type/edit.php')) {
    require_once($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/type/edit_form.php');
    require_once($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/type/edit.php');
    die;
} else {
    require_once($CFG->dirroot.'/shezar/hierarchy/type/edit_form.php');
}
...
if ($id == 0) {
    // creating new type
    require_capability('shezar/hierarchy:create'.$prefix.'type', $context);
    ...
    require_capability('shezar/hierarchy:update'.$prefix.'type', $context);
    if (!$type = $hierarchy->get_type_by_id($id, $typetable)) {
        print_error('incorrecttypeid', 'shezar_hierarchy');
    }
}

$PAGE->requires->strings_for_js(array('choosecohort'), 'shezar_hierarchy');
...
if ($shortprefix == 'goal_user') {
    if (empty($type->id)) {
        ...
        $enrolledselected = shezar_cohort_get_goal_type_cohorts($type->id, 'c.id');
        $enrolledselected = !empty($enrolledselected) ? implode(',', array_keys($enrolledselected)) : '';
    }

    $jsmodule = array(
        'name' => 'shezar_cohortdialog',
        'fullpath' => "/shezar/cohort/dialog/{$prefix}cohort.js",
        'requires' => array('json'));
    $args = array("enrolledselected" => $enrolledselected);
    $PAGE->requires->js_init_call("M.shezar_{$prefix}cohort.init", $args, true, $jsmodule);
...
$type = file_prepare_standard_editor($type, 'description', $editoroptions, $context,
                                          'shezar_hierarchy', $shortprefix.'_type', $type->id);
...
// update data
    ...
    if ($typenew->id == 0) {
        ...
        $typenew = file_postupdate_standard_editor($typenew, 'description', $editoroptions, $context, 'shezar_hierarchy', $shortprefix.'_type', $typenew->id);
        $DB->set_field($shortprefix.'_type', 'description', $typenew->description, array('id' => $typenew->id));

        shezar_hierarchy_save_cohorts_for_type($shortprefix, $prefix . 'id', $typenew);
    ...
    // Existing type.
        ...
        $typenew = file_postupdate_standard_editor($typenew, 'description', $editoroptions,
            $context, 'shezar_hierarchy', $shortprefix.'_type', $typenew->id);
    ...
    shezar_set_notification(get_string($notification->text, 'shezar_hierarchy', $typenew->fullname), $notification->url, $notification->params);
}


/// Display page header
$PAGE->navbar->add(get_string("{$prefix}types", 'shezar_hierarchy'), $returnurl);

if ($id == 0) {
    $PAGE->navbar->add(get_string('addtype', 'shezar_hierarchy'));
} else {
    $PAGE->navbar->add(get_string('editgeneric', 'shezar_hierarchy', format_string($type->fullname)));
}

echo $OUTPUT->header();

if ($type->id == 0) {
    echo $OUTPUT->heading(get_string('addtype', 'shezar_hierarchy'));
} else {
    echo $OUTPUT->heading(get_string('editgeneric', 'shezar_hierarchy', format_string($type->fullname)));
}
