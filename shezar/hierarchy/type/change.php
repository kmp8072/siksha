...
require_once($CFG->dirroot.'/shezar/hierarchy/type/changelib.php');
...
if ($itemid) {
    ...
    $returnurl = $CFG->wwwroot . '/shezar/hierarchy/item/edit.php?prefix=' .
    ...
    $item_sql = '';
    $item_param = array();
    $returnurl = $CFG->wwwroot . '/shezar/hierarchy/type/index.php?prefix=' . $prefix . '&amp;page=' . $page;
...
require_capability('shezar/hierarchy:update' . $prefix, $sitecontext);
...
if ($affected_item_count == 0) {
    shezar_set_notification(get_string('error:nonefound' . $optype, 'shezar_hierarchy'), $returnurl);
}
...
$cfs_by_type = shezar_group_records($rs, 'typeid');
...
if ($itemid) {
    ...
    $PAGE->navbar->add(get_string('edit'.$prefix, 'shezar_hierarchy'), new moodle_url('/shezar/hierarchy/item/edit.php', array('prefix' => $prefix, 'id' => $itemid)));
    $PAGE->navbar->add(get_string('changetype', 'shezar_hierarchy'));

} else {
    $PAGE->navbar->add(get_string('types', 'shezar_hierarchy'), new moodle_url('/shezar/hierarchy/type/index.php', array('prefix' => $prefix)));
    $PAGE->navbar->add(get_string('bulktypechanges', 'shezar_hierarchy'));
}
...
if ($itemid) {
    ...
    $itemstr = ($affected_item_count == 1) ? strtolower(get_string($prefix, 'shezar_hierarchy')) :
        strtolower(get_string($prefix.'plural', 'shezar_hierarchy'));
    $a->num = $affected_item_count;
    $a->items = $itemstr;
}
echo $OUTPUT->heading(get_string('reclassify1of2' . $optype, 'shezar_hierarchy', $a), 1);

echo $OUTPUT->container_start();
echo html_writer::start_tag('p');
echo get_string('reclassify1of2desc', 'shezar_hierarchy');
// if there's data to transfer, let people know they'll get the chance to move it
// in step 2
if ($current_type_cfs) {
    echo ' ' . get_string('reclassifytransferdata', 'shezar_hierarchy');
}
echo html_writer::end_tag('p');
echo $OUTPUT->container_end();

echo $OUTPUT->heading(get_string('currenttype', 'shezar_hierarchy'), 3, 'hierarchy-bulk-type');
...
foreach ($types as $type) {
    ...
    $row[] = $OUTPUT->single_button(new moodle_url('/shezar/hierarchy/type/changeconfirm.php', array('prefix' => $prefix, 'typeid' => $typeid, 'newtypeid' => $type->id, 'itemid' => $itemid, 'page' => $page, 'class' => $class)), get_string('choose'), 'get');
...
if ($typeid != 0) {
    $row = array();
    $row[] = $OUTPUT->single_button(new moodle_url('/shezar/hierarchy/type/changeconfirm.php', array('prefix' => $prefix, 'typeid' => $typeid, 'newtypeid' => 0, 'itemid' => $itemid, 'class' => $class)), get_string('choose'), 'get');
    $row[] = get_string('unclassified', 'shezar_hierarchy');
    $row[] = get_string('nocustomfields', 'shezar_hierarchy');
    $table->data[] = $row;
}


echo $OUTPUT->heading(get_string('newtype', 'shezar_hierarchy'), 3, 'hierarchy-bulk-type');
