...
require_once($CFG->dirroot.'/shezar/hierarchy/type/change_form.php');
...
if ($itemid) {
    ...
    $returnurl = new moodle_url("/shezar/hierarchy/item/edit.php", array('prefix' => $prefix, 'id' => $itemid, 'page' => $page));
    ...
    $returnurl = new moodle_url("/shezar/hierarchy/type/index.php", array('prefix' => $prefix, 'page' => $page, 'class' => $class));
    $optype = 'bulk';
...
if ($changeform->is_cancelled()) {
    ...
    if (isset($data->field)) {
        ...
        if (!empty($errors)) {
            foreach ($errors as $field => $val) {
                shezar_set_notification("$field : " . $val);
            }
            $a = new stdClass();
            $a->from = $typename;
            $a->to = $newtypename;
            $DB->force_transaction_rollback();
            shezar_set_notification(get_string('error:couldnotreclassify' . $optype, 'shezar_hierarchy', $a), "$CFG->wwwroot/shezar/hierarchy/index.php?prefix=$prefix");
        }
    }
    ...
    if (!$status) {
        ...
        $a->from = $typename;
        $a->to = $newtypename;
        shezar_set_notification(get_string('error:couldnotreclassify' . $optype,
            'shezar_hierarchy', $a),
            "$CFG->wwwroot/shezar/hierarchy/type/index.php?prefix=$prefix");
    }
    ...
    shezar_set_notification(get_string('reclassifysuccess' . $optype, 'shezar_hierarchy', $a),
...
echo $OUTPUT->heading(get_string('reclassifyingfromxtoy' . $optype, 'shezar_hierarchy', $a), 1);
