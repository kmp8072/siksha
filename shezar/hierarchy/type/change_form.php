...
class type_change_form extends moodleform {
    ...
    function definition() {
        ...
        if (!$current_type_cfs || $affected_data_count == false) {
            ...
            $confirmtext = get_string('confirmproceed', 'shezar_hierarchy');
            $buttontext = get_string('reclassifyitems', 'shezar_hierarchy');
            ...
            $confirmtext = get_string('deletedataconfirmproceed', 'shezar_hierarchy', html_writer::alist($message));
            $buttontext = get_string('reclassifyitemsanddelete', 'shezar_hierarchy');
            ...
            $mform->addElement('html', markdown_to_html(get_string('choosewhattodowithdata', 'shezar_hierarchy')));
            ...
            if ($old_cfs && $new_cfs) {
                ...
                foreach ($current_type_cfs as $old_cf) {
                    ...
                    $options = array(0 => get_string('deletethisdata', 'shezar_hierarchy'));
                    foreach ($new_type_cfs as $new_cf) {
                        if (hierarchy_allowed_datatype_conversion($old_cf->datatype, $new_cf->datatype)) {
                            $options[$new_cf->id] = get_string('transfertox', 'shezar_hierarchy', $new_cfs[$new_cf->id]);
                        }
                    }

                    if (array_key_exists($old_cf->id, $affected_data_count) && count($options) > 1) {
                        $mform->addElement('select', 'field[' . $old_cf->id . ']', get_string('datainx', 'shezar_hierarchy', $old_cfs[$old_cf->id]), $options);
                        $mform->setType('field[' . $old_cf->id . ']', PARAM_INT);
                    } else if (array_key_exists($old_cf->id, $affected_data_count)) {
                        $mform->addElement('static', 'message[' . $old_cf->id . ']', get_string('datainx', 'shezar_hierarchy', $old_cfs[$old_cf->id]), get_string('deletethisdata', 'shezar_hierarchy'));
                        $mform->addElement('hidden', 'field[' . $old_cf->id . ']', 0);
                        $mform->setType('field[' . $old_cf->id . ']', PARAM_INT);
                    } else {
                        $mform->addElement('static', 'field[' . $old_cf->id . ']', get_string('datainx', 'shezar_hierarchy', $old_cfs[$old_cf->id]), get_string('nodata', 'shezar_hierarchy'));
                    }
                }
            }

            $confirmtext = '';
            $buttontext = get_string('reclassifyitemsandtransfer', 'shezar_hierarchy');
        }
        ...
        if ($current_type_cfs && $new_type_cfs) {
            ...
            foreach ($current_type_cfs as $oldcf) {
                ...
                if (in_array($newfield, $used)) {
                    $errors['field[' . $oldcf->id . ']'] = get_string('error:alreadyassigned', 'shezar_hierarchy');
                }
                ...
                if (!hierarchy_allowed_datatype_conversion($oldcf->datatype, $newcf->datatype)) {
                    ...
                    $errors['field[' . $oldcf->id . ']'] = get_string('error:cannotconvertfieldfromxtoy', 'shezar_hierarchy', $a);
