...
function hierarchy_get_type_name($typeid, $shortprefix) {
    ...
    if ($typeid != 0) {
        ...
        return get_string('unclassified', 'shezar_hierarchy');
    }
}
...
function hierarchy_get_formatted_custom_fields($data) {
    ...
    foreach ($data as $item) {
        ...
        $hidden = ($item->hidden) ? get_string('hidden', 'shezar_hierarchy') . ' ' : '';
        $fields[$item->id] = format_string($item->fullname) . ' (' . $hidden . get_string('customfieldtype' . $item->datatype, 'shezar_customfield') . ')';
    }
