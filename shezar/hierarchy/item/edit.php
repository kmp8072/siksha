...
require_once($CFG->dirroot.'/shezar/hierarchy/item/edit_form.php');
...
if ($id == 0) {
    ...
    if (!$item = $DB->get_record($shortprefix, array('id' => $id))) {
        print_error('incorrectid', 'shezar_hierarchy');
    }
...
$item = file_prepare_standard_editor($item, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'shezar_hierarchy', $shortprefix, $item->id);
...
if ($itemform->is_cancelled()) {
    ...
    redirect("{$CFG->wwwroot}/shezar/hierarchy/index.php?prefix=$prefix&amp;frameworkid={$item->frameworkid}&amp;page=$page");

// Update data
} else if ($itemnew = $itemform->get_data()) {

    if (isset($itemnew->changetype)) {
        redirect($CFG->wwwroot . "/shezar/hierarchy/type/change.php?prefix=$prefix&amp;frameworkid={$item->frameworkid}&amp;page={$page}&typeid={$itemnew->typeid}&amp;itemid={$itemnew->id}");
    }
    ...
    if ($itemnew->id == 0) {
        ...
        if ($updateditem = $hierarchy->add_hierarchy_item($itemnew, $itemnew->parentid, $itemnew->frameworkid, false, false)) {
            ...
            $itemnew = file_postupdate_standard_editor($itemnew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_hierarchy', $shortprefix, $itemnew->id);
            $DB->set_field($shortprefix, 'description', $itemnew->description, array('id' => $itemnew->id));

            $notification->text = 'added';
            $notification->url = "{$CFG->wwwroot}/shezar/hierarchy/item/view.php?prefix=$prefix&id={$updateditem->id}";
            $notification->params = array('class' => 'notifysuccess');
        } else {
            $notification->text = 'error:add';
            $notification->url = "{$CFG->wwwroot}/shezar/hierarchy/index.php?prefix=$prefix";
        ...
        $notification->url = "{$CFG->wwwroot}/shezar/hierarchy/item/view.php?prefix=$prefix&id={$itemnew->id}";
        $notification->params = array('class' => 'notifysuccess');
    }
    ...
    shezar_set_notification(get_string($notification->text . $prefix, 'shezar_hierarchy', format_string($itemnew->fullname)), $notification->url, $notification->params);
}
...
if ($item->id) {
    ...
    $PAGE->navbar->add(get_string('edit'.$prefix, 'shezar_hierarchy'));
} else {
    $PAGE->navbar->add(get_string('addnew'.$prefix, 'shezar_hierarchy'));
}
...
if ($item->id == 0) {
    echo $OUTPUT->heading(get_string('addnew'.$prefix, 'shezar_hierarchy'));
} else {
    echo $OUTPUT->heading(get_string('edit'.$prefix, 'shezar_hierarchy'));
}
