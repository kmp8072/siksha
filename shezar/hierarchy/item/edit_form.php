...
class item_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $typename = ($type) ? $type->fullname : get_string('unclassified', 'shezar_hierarchy');
        ...
        $mform->addElement('text', 'framework', get_string($prefix.'framework', 'shezar_hierarchy'));
        ...
        $mform->addRule('fullname', get_string($prefix.'missingname', 'shezar_hierarchy'), 'required', null);
        $mform->setType('fullname', PARAM_MULTILANG);

        if (!empty($CFG->showhierarchyshortnames)) {
            $mform->addElement('text', 'shortname', get_string($prefix.'shortname', 'shezar_hierarchy'), 'maxlength="100" size="20"');
            $mform->addHelpButton('shortname', $prefix.'shortname', 'shezar_hierarchy');
            $mform->setType('shortname', PARAM_MULTILANG);
        }

        $mform->addElement('text', 'idnumber', get_string($prefix.'idnumber', 'shezar_hierarchy'), 'maxlength="100"  size="10"');
        $mform->addHelpButton('idnumber', $prefix.'idnumber', 'shezar_hierarchy');
        $mform->setType('idnumber', PARAM_TEXT);

        // If we are in a dialog, hide the htmleditor. It messes with the jquery code
        if (!$dialog) {
            $mform->addElement('editor', 'description_editor', get_string('description', 'shezar_hierarchy'), null, $TEXTAREA_OPTIONS);
            $mform->addHelpButton('description_editor', $prefix.'description', 'shezar_hierarchy');
        ...
        if ($item->id) {
            ...
            $group[] = $mform->createElement('submit', 'changetype', get_string('changetype', 'shezar_hierarchy'));
            $mform->addGroup($group, 'typegroup', get_string('type', 'shezar_hierarchy'), array(' &nbsp; '), false);

            $mform->setDefault('type', $typename);
            $mform->addHelpButton('typegroup', $prefix.'type', 'shezar_hierarchy');
            ...
            if ($types) {
                ...
                $mform->addElement('select', 'typeid', get_string('type', 'shezar_hierarchy'), $select, shezar_select_width_limiter());
        ...
        if (get_config('shezar_sync', "element_{$shortprefix}_enabled")) {
            $mform->addElement('advcheckbox', 'shezarsync', get_string('shezarsync', 'tool_shezar_sync').'?');
            $mform->addHelpButton('shezarsync', 'shezarsync', 'tool_shezar_sync');
        }
    ...
    function validation($itemnew, $files) {
        ...
        if (!empty($itemnew->idnumber) && shezar_idnumber_exists($shortprefix, $itemnew->idnumber, $itemnew->id)) {
