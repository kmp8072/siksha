...
class item_bulkaction_form extends moodleform {
    ...
    function definition() {
        ...
        if (count($all_selected_item_ids) != count($hierarchy->get_items_excluding_children($all_selected_item_ids))) {
            $message = get_string('parentchildselectedwarning' . $action, 'shezar_hierarchy');
        ...
        $available_str = get_string('availablex', 'shezar_hierarchy', get_string($prefix . 'plural', 'shezar_hierarchy'));
        $selected_str = get_string('selectedx', 'shezar_hierarchy', get_string($prefix . 'plural', 'shezar_hierarchy'));
        ...
        $mform->addElement('text', 'search', '',
            array('placeholder' => get_string('searchavailable', 'shezar_hierarchy')));
        $mform->setDefault('search', $searchterm);
        $mform->setType('search', PARAM_TEXT);
        $mform->addElement('submit', 'submitsearch', get_string('search'));
        if (strlen(trim($searchterm)) != 0) {
            $mform->addElement('submit', 'clearsearch', get_string('showall', 'moodle', get_string($prefix.'plural', 'shezar_hierarchy')));
        }
        ...
        if ($displayed_available_items) {
            ...
            $select->addOption(get_string('noxfound', 'shezar_hierarchy',
        ...
        $mform->addElement('submit', 'remove_all_items',
            get_string('clearselection', 'shezar_hierarchy'), $remove_attr);
        ...
        case 'delete':
            $mform->addElement('submit', 'deletebutton',
                get_string('deleteselectedx', 'shezar_hierarchy',
                strtolower(get_string($prefix.'plural', 'shezar_hierarchy'))));
            break;
        case 'move':
            $mform->addElement('html', get_string('moveselectedxto', 'shezar_hierarchy', strtolower(get_string($prefix.'plural', 'shezar_hierarchy'))));
            $options = $hierarchy->get_parent_list($items, $all_selected_item_ids);
            $select =& $mform->createElement('select', 'newparent', '', $options, shezar_select_width_limiter());
            $mform->addElement($select);
            $mform->addElement('submit', 'movebutton', get_string('move'));
            break;
        default:
            // this shouldn't happen
            print_error('error:unknownaction', 'shezar_hierarchy');
        }
