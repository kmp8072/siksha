...
*/
function shezar_hierarchy_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/shezar_hierarchy/$filearea/$args[0]/$args[1]";
 ...
 * @returns true
 */
function shezar_hierarchy_install_default_comp_scale() {
    global $USER, $DB;
    $now = time();

    $todb = new stdClass;
    $todb->name = get_string('competencyscale', 'shezar_hierarchy');
    $todb->description = '';
    ...
    $comp_scale_vals = array(
        array('name'=>get_string('competent', 'shezar_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 1, 'usermodified' => $USER->id, 'timemodified' => $now, 'proficient' => 1),
        array('name'=>get_string('competentwithsupervision', 'shezar_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 2, 'usermodified' => $USER->id, 'timemodified' => $now),
        array('name'=>get_string('notcompetent', 'shezar_hierarchy'), 'scaleid' => $scaleid, 'sortorder' => 3, 'usermodified' => $USER->id, 'timemodified' => $now)
        );
...
class hierarchy {
        ...
        if ($id == 0) {
            ...
            if (!$framework = $DB->get_record_sql($sql, null, true)) {
                if ($noframeworkok) {
                    ...
                    print_error('noframeworks', 'shezar_hierarchy');
                }
            }
        } else {
            if (!$framework = $DB->get_record($this->shortprefix.'_framework', array('id' => $id))) {
                print_error('frameworkdoesntexist', 'shezar_hierarchy', '', $this->prefix);
            }
        }
    ...
    function get_item_descendants($id) {
        ...
        if ($path) {
            ...
            print_error('nopathfoundforid', 'shezar_hierarchy', '', (object)array('prefix' => $this->prefix, 'id' => $id));
        }
    }
    ...
    */
    ...
    function display_framework_selector($page = 'index.php', $simple = false, $return = false, $showhidden = false) {
        ...
        if (!$simple) {
            ...
            $select->label = get_string('switchframework', 'shezar_hierarchy');
    ...
    function display_add_item_button($page=0) {
        ...
        $url = new moodle_url('/shezar/hierarchy/item/edit.php', $options);
        return $OUTPUT->single_button($url, get_string('addnew'.$this->prefix, 'shezar_hierarchy'), 'get');
    }
    ...
    function display_add_multiple_items_button($page=0) {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/shezar/hierarchy/item/bulkadd.php', $options);
    ...
    function display_bulk_actions_picker($can_add_item, $can_edit_item, $can_delete_item, $can_manage_type, $page=0) {
        ...
        $options = array();
        if ($can_add_item) {
            $options['/shezar/hierarchy/item/bulkadd.php?prefix='.$this->prefix.'&frameworkid='.$this->frameworkid.'&page='.$page] = get_string('add');
        }
        if ($can_delete_item) {
            $options['/shezar/hierarchy/item/bulkactions.php?action=delete&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('delete');
        }
        if ($can_edit_item) {
            $options['/shezar/hierarchy/item/bulkactions.php?action=move&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('move');
        }
        if ($can_manage_type) {
            $class = '';
            if ($this->prefix == 'goal') {
            ...
            $options['/shezar/hierarchy/type/index.php?prefix='.$this->prefix.$class.'#bulkreclassify'] = get_string('reclassifyitems', 'shezar_hierarchy');
        }

        if (count($options) > 0) {
            $out .= $OUTPUT->container_start('hierarchy-bulk-actions-picker');
            $select = new url_select($options, '', array('' => get_string('bulkactions', 'shezar_hierarchy')));
    ...
    public function display_add_type_button($page=0, $class='') {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/shezar/hierarchy/type/edit.php', $options);
        echo $OUTPUT->single_button($url, get_string('addtype', 'shezar_hierarchy'), 'get');
    }
    ...
    function reorder_hierarchy_item($id, $swapwith) {
        ...
        if (!$destid) {
            ...
            echo $OUTPUT->notification(get_string('error:couldnotmoveitemnopeer','shezar_hierarchy',$this->prefix));
            return false;
        }
    ...
    public function delete_hierarchy_item($id, $triggerevent = true) {
        ...
        $frameworkid = isset($this->frameworkid) ? $this->frameworkid :
            ...
            while ($delete_items = shezar_pop_n($delete_list, 1000)) {
    ...
    function get_item_data($item, $cols = NULL) {
        ...
        foreach ($cols as $datatype) {
            if ($datatype == 'description') {
                $value = file_rewrite_pluginfile_urls($item->$datatype, 'pluginfile.php', context_system::instance()->id, 'shezar_hierarchy', $this->shortprefix, $item->id);
            } else {
                $value = $item->$datatype;
            }
            $data[] = array(
                'title' => get_string($datatype.'view', 'shezar_hierarchy'),
                'value' => $value
            );
        }
        ...
        $typename = ($itemtype) ? $itemtype->fullname : get_string('unclassified', 'shezar_hierarchy');
        $data[] = array(
            'title' => get_string('type', 'shezar_hierarchy'),
            'value' => $typename,
        );
    ...
    static function get_short_prefix($prefixname) {
        ...
        $libpath = $CFG->dirroot.'/shezar/hierarchy/prefix/'.$cleanprefixname.'/lib.php';
        if (!file_exists($libpath)) {
            print_error('error:hierarchyprefixnotfound', 'shezar_hierarchy', '', $cleanprefixname);
        }
    ...
    static function load_hierarchy($prefix) {
        ...
        // Check file exists
        $libpath = $CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/lib.php';
        if (!file_exists($libpath)) {
            print_error('error:hierarchyprefixnotfound', 'shezar_hierarchy', '', $prefix);
        }
    ...
    function display_hierarchy_item($record, $include_custom_fields=false, $indicate_depth=true, $cfields=array(), $types=array()) {
        ...
        $out .= $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => $this->prefix, 'id' => $record->id)), format_string($record->fullname), null, array('class' => $cssclass));
        if ($include_custom_fields) {
            $out .= html_writer::empty_tag('br');
            // Print description if available.
            if ($record->description) {
                $record->description = file_rewrite_pluginfile_urls($record->description, 'pluginfile.php', context_system::instance()->id, 'shezar_hierarchy', $this->shortprefix, $record->id);
            ...
            if ($record->typeid !=0 && is_array($types) && array_key_exists($record->typeid, $types)) {
                $out .= html_writer::tag('div', html_writer::tag('strong', get_string('type', 'shezar_hierarchy') . ': ') . format_string($types[$record->typeid]->fullname), array('class' => 'itemtype' . $cssclass));
                if (!empty($CFG->showhierarchyshortnames)) {
                    $out .= html_writer::tag('div', html_writer::tag('strong', get_string('shortnametype', 'shezar_hierarchy') . ': ') . format_string($types[$record->typeid]->shortname), array('class' => 'itemtype' . $cssclass));
                }
            }
    ...
    function display_search_box($query, $placeholdertext=null) {
        ...
        $out = $renderer->shezar_search('', $hiddenfields, $placeholdertext, $query, 'hierarchy-search-form', 'hierarchy-search-text-field');
        return $out;
    }
    ...
    function display_showhide_detail_button($displaymode, $query='', $page=0) {
        ...
        $buttontext = ($displaymode) ? get_string('showdetails', 'shezar_hierarchy') : get_string('hidedetails', 'shezar_hierarchy');
        $urlparams = array(
            'prefix' => $this->prefix,
            'frameworkid' => $this->frameworkid,
            'query' => $query,
    ...
    function export_select($baseurl=null) {
        global $CFG;
        require_once($CFG->dirroot.'/shezar/hierarchy/export_form.php');
        if (empty($baseurl)) {
    ...
    function export_data($format) {
        ...
        if ($searchactive) {
            $headings = array('hierarchyidnumber' => get_string($this->prefix.'frameworkidnumber','shezar_hierarchy'),
                'hierarchyname' => get_string('name'),
                'typename' => get_string('type','shezar_hierarchy'));
        } else {
            $headings = array('hierarchyidnumber' => get_string($this->prefix.'frameworkidnumber','shezar_hierarchy'),
                'typename' => get_string('type', 'shezar_hierarchy'));
        }
        ...
        if ($searchactive) {
            ...
            $keywords = shezar_search_parse_keywords($query);
            ...
            list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, $dbfields);
            $where .= ' AND (' . $searchsql. ')';
    ...
    function parse_customfield($fieldid, $fieldvalue, $isexport=false) {
        ...
        $classfilepath = $CFG->dirroot . '/shezar/customfield/field/' . $datatype . '/field.class.php';
    ...
    function download_ods($fields, $query, $params, $maxdepth, $file=null, $searchactive=false) {
        ...
        if (!$searchactive) {
            for ($depth = 1 ; $depth <= $maxdepth ; $depth++) {
                $worksheet[0]->write($row, $col, get_string('depth', 'shezar_hierarchy', $depth));
                $col++;
            }
        }
    ...
    function download_csv($fields, $query, $params, $maxdepth, $file=null, $searchactive=false) {
        ...
        if (!$searchactive) {
            $headers = array();
            for ($depth = 1 ; $depth <= $maxdepth ; $depth++) {
                $headers[] = get_string('depth', 'shezar_hierarchy' ,$depth);
            }
        }
    ...
    public function output_delete_message($stats) {
        ...
        $a->children_string = $stats['children'] == 1 ? get_string('child', 'shezar_hierarchy') : get_string('children', 'shezar_hierarchy');
        ...
        $message .= get_string($langstr, 'shezar_hierarchy', $a) . html_writer::empty_tag('br');

        if ($stats['cf_data'] > 0) {
            $message .= get_string('deleteincludexcustomfields', 'shezar_hierarchy', $stats['cf_data']) . html_writer::empty_tag('br');
        }
    ...
    public function get_move_message($ids, $parentid) {
        ...
        if (is_array($ids) && count($ids) != 1) {
            $itemstr = get_string($this->prefix.'plural', 'shezar_hierarchy');
            $num = count($ids);
        } else {
            $itemstr = get_string($this->prefix, 'shezar_hierarchy');
            $num = 1;
        }

        $parentname = ($parentid == 0) ? get_string('top', 'shezar_hierarchy') :
        ...
        return get_string('confirmmoveitems', 'shezar_hierarchy', $a);
    }
    ...
    public function get_items_excluding_children($ids) {
        ...
        if ($items) {
            ...
            $items_by_depth = shezar_group_records($items, 'depthlevel');
            // sort ascending
    ...
    public function get_parent_list($items, $selected = array(), $inctop = true) {
        ...
        if ($inctop) {
            ...
            $out[0] = get_string('top', 'shezar_hierarchy');
        }
    ...
    protected function get_next_child_sortthread($parentid, $frameworkid = null) {
        ...
        if (!$maxthread || strlen($maxthread->sortthread) == 0) {
            if ($parentid == 0) {
                ...
                return shezar_int2vancode(1);
            } else {
                // parent has no children yet
                return $DB->get_field($this->shortprefix, 'sortthread', array('id' => $parentid, 'frameworkid' => $frameworkid)) . '.' . shezar_int2vancode(1);
            }
        }
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        $cancreateframeworks = has_capability('shezar/hierarchy:create' . $prefix . 'frameworks', $context, $user);
        $canupdateframeworks = has_capability('shezar/hierarchy:update' . $prefix . 'frameworks', $context, $user);
        $candeleteframeworks = has_capability('shezar/hierarchy:delete' . $prefix . 'frameworks', $context, $user);
        $canmanageframeworks = $cancreateframeworks || $canupdateframeworks || $candeleteframeworks;
        $canviewframeworks = has_capability('shezar/hierarchy:view' . $prefix . 'frameworks', $context, $user);

        $cancreateitems = has_capability('shezar/hierarchy:create' . $prefix, $context, $user);
        $canupdateitems = has_capability('shezar/hierarchy:update' . $prefix, $context, $user);
        $candeleteitems = has_capability('shezar/hierarchy:delete' . $prefix, $context, $user);
        $canmanageitems = $cancreateitems || $canupdateitems || $candeleteitems;
        $canviewitems = has_capability('shezar/hierarchy:view' . $prefix, $context, $user);

        if (file_exists($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/scale/lib.php')) {
            $cancreatescales = has_capability('shezar/hierarchy:create' . $prefix . 'scale', $context, $user);
            $canupdatescales = has_capability('shezar/hierarchy:update' . $prefix . 'scale', $context, $user);
            $candeletescales = has_capability('shezar/hierarchy:delete' . $prefix . 'scale', $context, $user);
            $canmanagescales = $cancreatescales || $canupdatescales || $candeletescales;
            $canviewscales = has_capability('shezar/hierarchy:view' . $prefix . 'scale', $context, $user);
    ...
    public static function construct_items_to_add($formdata, &$error) {
        ...
        foreach ($items as $item) {
            ...
            if (isset($levelparent[$level])) {
                ...
                $error = get_string('bulkaddparenterror', 'shezar_hierarchy', $name);
                return false;
            }
        ...
        if (count($items_to_add) == 0) {
            $error = get_string('bulkaddnoitems', 'shezar_hierarchy');
            return false;
        }
 ...
 */
function shezar_hierarchy_save_cohorts_for_type($shortprefix, $idfield, stdClass $typeobject) {
    global $DB, $USER;
