...
/**
* Standard HTML output renderer for shezar_hierarchy module
*/
class shezar_hierarchy_renderer extends plugin_renderer_base {
    ...
    */
    ...
    public function competency_view_evidence($item, $evidence=null, $can_edit=false) {
        ...
        if (!empty($CFG->competencyuseresourcelevelevidence)) {
            $table->head[] = get_string('type', 'shezar_hierarchy');
            $table->head[] = get_string('activity');
        }
        if ($can_edit) {
            $table->head[] = get_string('linktype', 'shezar_plan');
            $table->head[] = get_string('options', 'shezar_hierarchy');
            $js_params = array('prefix' => 'course');
            $PAGE->requires->js_call_amd('shezar_hierarchy/hierarchyitems', 'init', $js_params);
        }
        ...
        if ($evidence) {
            ...
            foreach ($evidence as $eitem) {
                ...
                if ($can_edit) {
                    ...
                    $content = html_writer::select(
                        array( //$options
                            PLAN_LINKTYPE_MANDATORY => get_string('mandatory','shezar_hierarchy'),
                            PLAN_LINKTYPE_OPTIONAL => get_string('optional','shezar_hierarchy'),
                        ),
                    ...
                    $link = $this->output->action_link(
                        new moodle_url('/shezar/hierarchy/prefix/competency/evidenceitem/remove.php', array('id' => $eitem->id)),
            ...
            // # cols varies
            $cols = $can_edit ? 4 : 3;
            $cell = new html_table_cell(html_writer::start_tag('i') . get_string('noevidenceitems', 'shezar_hierarchy') . html_writer::end_tag('i'));
            $cell->colspan = $cols;
        ...
        $can_edit = has_capability('shezar/hierarchy:updatecompetency', $context);
        // Display add evidence item button
        if ($can_edit) {
            $templatedata->canedit = true;

            $action = new moodle_url('/shezar/hierarchy/prefix/competency/evidenceitem/edit.php', array('id' => $item->id));
            $templatedata->formaction = $action->out();

            if (!empty($CFG->competencyuseresourcelevelevidence)) {
                $templatedata->evidencestring = get_string('assignnewevidenceitem', 'shezar_hierarchy');
            } else {
                $templatedata->evidencestring = get_string('assigncoursecompletions', 'shezar_hierarchy');
            }
        ...
        return $this->render_from_template('shezar_hierarchy/competency_view_evidence', $templatedata);
    }
    ...
    */
    public function competency_view_related($item, $can_edit=false, $related=null) {
        ...
        // Set up table header.
        $table->head = array(
            get_string('competencyframework', 'shezar_hierarchy'),
            get_string('name'),
        );

        if ($can_edit) {
            $table->head[] = get_string('options', 'shezar_plan');
        }

        // Now the rows if any.
        if ($related) {
            $sitecontext = context_system::instance();
            $can_manage_fw = has_capability('shezar/hierarchy:updatecompetencyframeworks', $sitecontext);

            $oddeven = 1;
            foreach ($related as $ritem) {
                $cells = array();
                $framework_text = ($can_manage_fw) ?
                    $this->output->action_link(new moodle_url('/shezar/hierarchy/index.php', array('prefix' => 'competency', 'frameworkid' => $ritem->fid)),
                    format_string($ritem->framework)) : format_string($ritem->framework);
                $oddeven = ++$oddeven % 2;

                $cells[] = new html_table_cell($framework_text);
                $link = html_writer::link(new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'competency', 'id' => $ritem->id)), format_string($ritem->fullname));
                $cells[] = new html_table_cell($link);

                if ($can_edit) {
                    $str_remove = get_string('remove');
                    $content = $this->output->action_link(
                        new moodle_url("/shezar/hierarchy/prefix/competency/related/remove.php", array('id' => $item->id, 'related' => $ritem->id)),
            ...
            // # cols varies
            $cols = $can_edit ? 4 : 3;
            $cell = new html_table_cell(html_writer::start_tag('i') . get_string('norelatedcompetencies', 'shezar_hierarchy') . html_writer::end_tag('i'));
            $cell->colspan = $cols;
            $row = new html_table_row(array($cell));
        ...
        if ($can_edit) {
            $action = new moodle_url('/shezar/hierarchy/prefix/competency/related/find.php', array('id' => $item->id, 'frameworkid' => $item->frameworkid));
        ...
        return $this->render_from_template('shezar_hierarchy/competency_view_related', $templatedata);
    }
    ...
    public function goal_view_assignments($item, $can_edit = false, $assignments = null, $dialog_box = false) {
        ...
        $table = new html_table();
        $table->head = array(
            get_string('goaltable:name', 'shezar_hierarchy'),
            get_string('goaltable:type', 'shezar_hierarchy'),
            get_string('goaltable:numusers', 'shezar_hierarchy')
        );
        ...
        $andchildstr = get_string('andchildren', 'shezar_hierarchy');
        ...
        return $this->render_from_template('shezar_hierarchy/goal_view_assignments', $templatedata);
    }
    ...
    */
    function print_hierarchy_items($framework, $prefix, $shortprefix, $displaytitle, $addurl, $itemid, $items, $can_edit=false){
        ...
        if ($displaytitle == 'assignedcompetencies') {
            ...
            $headers = array(
            get_string('type', 'shezar_hierarchy'),
            get_string('name', 'shezar_hierarchy')
            );
        ...
        if ($can_edit) {
            ...
            $str_remove = get_string('remove');
            $columns[] = 'linktype';
            $headers[] = get_string('linktype', 'shezar_plan');
            $columns[] = 'options';
            $headers[] = get_string('options', 'shezar_hierarchy');
            $js_args = array('prefix' => $shortprefix);
            $PAGE->requires->js_call_amd('shezar_hierarchy/hierarchyitems', 'init', $js_args);
        }
        $out = '';
        if (is_array($items) && count($items)) {
            ...
            $table->define_baseurl("{$CFG->wwwroot}/shezar/hierarchy/item/view.php?prefix={$displayprefix}&id={$itemid}");
            ...
            foreach ($items as $ritem) {
                $content = array();
                $content[] = empty($ritem->type) ? get_string('unclassified', 'shezar_hierarchy') : $ritem->type;

                if ($displaytitle == 'assignedcompetencies') {
                    $content[] = $this->output->action_link(new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => $displayprefix, 'id' => $ritem->id)), format_string($ritem->fullname));
                } elseif ($displaytitle == 'assignedcompetencytemplates') {
                    $content[] = $this->output->action_link(new moodle_url('/shezar/hierarchy/prefix/competency/template/view.php', array('id' => $ritem->id)), format_string($ritem->fullname));
                }
                ...
                if ($can_edit) {
                    ...
                    $content[] = $this->output->action_icon(
                        new moodle_url('/shezar/hierarchy/prefix/' . $prefix . '/assigncompetency/remove.php',
            ...
            $out .= get_string('no'.$displaytitle, 'shezar_hierarchy');
        ...
        if ($can_edit) {
            ...
            $add_button_text = get_string('add'.$displayprefix, 'shezar_hierarchy');
    ...
    public function assigned_goals($prefix, $shortprefix, $addgoalurl, $itemid) {
        ...
        $remove = get_string('remove');
        $level_only = get_string('goalassignthislevelonly', 'shezar_hierarchy');
        $level_below = get_string('goalassignthislevelbelow', 'shezar_hierarchy');
        $can_edit = has_capability('shezar/hierarchy:managegoalassignments', context_system::instance());
        ...
        if (!empty($assigned_goals)) {
            ...
            $table->head = array(
                get_string('goaltable:name', 'shezar_hierarchy'),
                get_string('goaltable:assignmentlevel', 'shezar_hierarchy')
            );
            ...
            foreach ($assigned_goals as $goal) {
                ...
                $nameurl = new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'goal', 'id' => $goal->goalid));
                ...
                if ($goal->includechildren) {
                    if ($goal->$field == $itemid) {
                        ...
                        $parent_url = new moodle_url('/shezar/hierarchy/item/view.php', $parent_params);
                        $parent_link = html_writer::link($parent_url, format_string($goal->parentname));
                        $level = get_string('goalassignlevelparent', 'shezar_hierarchy', $parent_link);
                    }
                } else {
                    $level = $level_only;
                }
        ...
        return $this->render_from_template('shezar_hierarchy/assigned_goals', $templatedata);
    }
    ...
    public function mygoals_company_table($userid, $can_edit, $display = false) {
        ...
        $company_table->head = array(
            get_string('goaltable:name', 'shezar_hierarchy'),
            get_string('goaltable:status', 'shezar_hierarchy'),
            get_string('goaltable:assigned', 'shezar_hierarchy')
        );
        ...
        foreach ($assignments as $goalid => $assignment) {
            ...
            if ($can_edit) {
                ...
                $PAGE->requires->js_call_amd('shezar_hierarchy/mygoals', 'init_company', $jsparams);
                ...
                $scaleurl = new moodle_url('/shezar/hierarchy/prefix/goal/update-scalevalue.php', array('nojs' => true));
        ...
        return $this->render_from_template('shezar_hierarchy/mygoals_company_table', $templatedata);
    }
    ...
    public function mygoals_personal_table($userid, $can_edit, $display = false) {
        ...
        $personal_table->head = array(
            get_string('goaltable:name', 'shezar_hierarchy'),
            get_string('goaltable:due', 'shezar_hierarchy'),
            get_string('goaltable:status', 'shezar_hierarchy'),
            get_string('goaltable:assigned', 'shezar_hierarchy'),
            get_string('edit')
        );
        ...
        foreach ($assignments as $goalid => $assignment) {
            ...
            if ($can_edit[$assignment->assigntype]) {
                ...
                $PAGE->requires->js_call_amd('shezar_hierarchy/mygoals', 'init_personal', $jsparams);
                // Set up the edit and delete icons.
                $edit_url = new moodle_url('/shezar/hierarchy/prefix/goal/item/edit_personal.php', array('id' => $goalid));
                $edit_str = get_string('edit');
                $edit_button = $this->output->action_icon($edit_url, new pix_icon('t/edit', $edit_str));
                $delete_url = new moodle_url('/shezar/hierarchy/prefix/goal/item/delete.php', array('goalpersonalid' => $goalid));
                ...
                $edit_button = $this->output->pix_icon('t/edit_gray',
                        get_string('error:editgoals', 'shezar_hierarchy'), 'moodle', array('class' => 'iconsmall action-icon'));
                $delete_button = $this->output->pix_icon('t/delete_gray',
                        get_string('error:deletegoalassignment', 'shezar_hierarchy'), 'moodle', array('class' => 'iconsmall action-icon'));
            }

            $duedate = '';
            if (!empty($assignment->targetdate)) {
                $duedate = userdate($assignment->targetdate, get_string('datepickerlongyearphpuserdate', 'shezar_core'),
                    $CFG->timezone, false);
            }

            $assign = goal::get_assignment_string(goal::SCOPE_PERSONAL, $assignment);
            $nameurl = new moodle_url('/shezar/hierarchy/prefix/goal/item/view.php', array('id' => $goalid));
        ...
        return $this->render_from_template('shezar_hierarchy/mygoals_personal_table', $templatedata);
    }
}
