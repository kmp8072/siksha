...
if ($canmanage) {
    ...
    $PAGE->set_url(new moodle_url('/shezar/hierarchy/framework/index', $url_params));
...
if ($canupdateframeworks) {
    ...
    if ($hide or $show or $moveup or $movedown) {
        require_capability('shezar/hierarchy:update'.$prefix.'frameworks', $sitecontext);
        // Hide an item.
        if ($hide) {
...
if ($frameworks) {
    ...
    $headers = array();
    $headers[] = get_string('name', 'shezar_hierarchy');
    if (!empty($CFG->showhierarchyshortnames)) {
        $headers[] = get_string('shortnameframework', 'shezar_hierarchy');
    }
    $headers[] = get_string($prefix.'plural', 'shezar_hierarchy');
    ...
    foreach ($frameworks as $framework) {
        ...
        $link_url = new moodle_url('/shezar/hierarchy/index.php', $link_params);
...
$templatedata->heading = get_string($prefix.'frameworks', 'shezar_hierarchy') . ' '
    . $OUTPUT->help_icon($prefix.'frameworks', 'shezar_hierarchy', false);
...
if ($cancreateframeworks) {
    ...
    $templatedata->createframeworkbutton = $OUTPUT->single_button(new moodle_url('edit.php', array('prefix' => $prefix)),
                get_string($prefix.'addnewframework', 'shezar_hierarchy'), 'get');
}
...
if ($canviewframeworks) {
    if ($frameworks) {
        ...
        $templatedata->noframeworkmessage = get_string($prefix.'noframeworks', 'shezar_hierarchy');
    }
}
echo $OUTPUT->render_from_template('shezar_hierarchy/admin_frameworks', $templatedata);

// Display scales.
if ($hierarchyhasscales) {
    include($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/scale/lib.php');
    $scales = $hierarchy->get_scales();
    call_user_func("{$prefix}_scale_display_table", $scales);
}

\shezar_hierarchy\event\framework_viewed::create_from_prefix($prefix)->trigger();
echo $OUTPUT->footer();
