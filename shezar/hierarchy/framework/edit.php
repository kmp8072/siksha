...
if (file_exists($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/framework/edit.php')) {
    require_once($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/framework/edit_form.php');
    require_once($CFG->dirroot.'/shezar/hierarchy/prefix/'.$prefix.'/framework/edit.php');
    die;
} else {
    require_once($CFG->dirroot.'/shezar/hierarchy/framework/edit_form.php');
}

// Make this page appear under the manage 'hierarchy' admin menu.
admin_externalpage_setup($prefix.'manage', '', array('prefix' => $prefix, 'id' => $id), $CFG->wwwroot.'/shezar/hierarchy/framework/edit.php');
...
if ($frameworkform->is_cancelled()) {
    ...
    if ($frameworknew->id == 0) {
        ...
        $frameworknew = file_postupdate_standard_editor($frameworknew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_hierarchy', $shortprefix.'_framework', $frameworknew->id);
    ...
    shezar_set_notification(get_string($prefix.$notification->text, 'shezar_hierarchy', format_string($frameworknew->fullname)), "$CFG->wwwroot/shezar/hierarchy/framework/index.php?prefix=$prefix", array('class' => 'notifysuccess'));
}
...
if ($framework->id == 0) {
    ...
    $PAGE->navbar->add(get_string('editgeneric', 'shezar_hierarchy', format_string($framework->fullname)));
}
