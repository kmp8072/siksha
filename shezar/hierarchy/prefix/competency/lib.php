...
class competency extends hierarchy {
        ...
        switch ($page) {
            case 'item/view':
                ...
                $PAGE->requires->strings_for_js(array('assignrelatedcompetencies',
                        'assignnewevidenceitem','assigncoursecompletions'), 'shezar_hierarchy');
                $PAGE->requires->js_call_amd('shezar_hierarchy/competency_item', 'item', $args);
            ...
            case 'template/view':
                ...
                $PAGE->requires->string_for_js('assignnewcompetency', 'shezar_competency');
                $PAGE->requires->js_call_amd('shezar_hierarchy/competency_item', 'template', $args);

                break;
            case 'item/add':
                $selected_position = json_encode(dialog_display_currently_selected(get_string('selected', 'shezar_hierarchy'), 'position'));
                $selected_organisation = json_encode(dialog_display_currently_selected(get_string("currentlyselected", "shezar_hierarchy"), "organisation"));
                $args = array('args'=>'{"userid":'.$USER->id.','.
                              '"can_edit": true,'.
                              '"dialog_display_position":'.$selected_position.','.
                              '"dialog_display_organisation":'.$selected_organisation.'}');
                $PAGE->requires->strings_for_js(array('chooseposition', 'choosemanager','chooseorganisation'), 'shezar_hierarchy');
                $jsmodule = array(
                        'name' => 'shezar_competencyaddevidence',
                        'fullpath' => '/shezar/plan/components/competency/competency.add_evidence.js',
                        'requires' => array('json'));
                $PAGE->requires->js_init_call('M.shezar_competencyaddevidence.init', $args, false, $jsmodule);
                break;
        }
    }
    ...
    function display_extra_view_info($item, $section='') {
        ...
        $can_edit = has_capability('shezar/hierarchy:updatecompetency', $sitecontext);
        if ($can_edit) {
    ...
    function get_item_data($item, $cols = NULL) {
        ...
        $prefix = get_string($this->prefix, 'shezar_hierarchy');
        // Add aggregation method
        $data[] = array(
            'title' => get_string('aggregationmethodview', 'shezar_hierarchy', $prefix),
            'value' => get_string('aggregationmethod'.$item->aggregationmethod, 'shezar_hierarchy')
        );
    ...
    function print_linked_evidence_list($courseid) {
        ...
        $can_edit = has_capability('shezar/hierarchy:updatecompetency', $system_context);
        $can_manage_fw = has_capability('shezar/hierarchy:updatecompetencyframeworks', $system_context);
        ...
        $heading0->text = get_string('competencyframework', 'shezar_hierarchy');
        ...
        if (!empty($CFG->competencyuseresourcelevelevidence)) {
            ...
            $heading2->text = get_string('evidence', 'shezar_hierarchy');
        ...
        if ($can_edit) {
            ...
            $heading3->text = get_string('linktype', 'shezar_plan');
            $heading3->header = true;
            $head[] = $heading3;

            $heading4 = new html_table_cell();
            $heading4->text = get_string('options', 'shezar_hierarchy');
        ...
        if ($competencies) {
            ...
            foreach ($competencies as $competency) {
                $framework_text = ($can_manage_fw) ?
                     $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/index.php', array('prefix' => 'competency', 'frameworkid' => $competency->fid)), format_string($competency->framework))
                ...
                $cell = new html_table_cell($OUTPUT->action_link(new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'competency', 'id' => $competency->id)), format_string($competency->fullname)));
                $row->cells[] = $cell;
                ...
                if ($can_edit) {
                    ...
                    $cell = new html_table_cell();
                    $cell->text = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/competency/evidenceitem/remove.php',
            ...
            $cell = new html_table_cell();
            $cell->colspan = 5;
            $cell->text = html_writer::tag('i', get_string('nocoursecompetencies', 'shezar_hierarchy'));
            $row->cell[0] = $cell;
    ...
    function add_additional_item_form_fields(&$mform) {
        ...
        foreach ($COMP_AGGREGATION as $title => $key) {
            $aggregations[$key] = get_string('aggregationmethod'.$key, 'shezar_hierarchy');
        }
        ...
        $mform->addElement('select', 'aggregationmethod', get_string('aggregationmethod', 'shezar_hierarchy'), $aggregations);
        $mform->addHelpButton('aggregationmethod', 'competencyaggregationmethod', 'shezar_hierarchy');
        $mform->addRule('aggregationmethod', get_string('aggregationmethod', 'shezar_hierarchy'), 'required', null);

        $mform->addElement('static', 'scalename', get_string('scale'), ($scaledesc) ? format_string($scaledesc) : get_string('none'));
        $mform->addHelpButton('scalename', 'competencyscale', 'shezar_hierarchy');
    ...
    public function output_delete_message($stats) {
        ...
        if ($stats['user_achievement'] > 0) {
            $message .= get_string('deleteincludexuserstatusrecords', 'shezar_hierarchy', $stats['user_achievement']) . html_writer::empty_tag('br');
        }

        if ($stats['evidence'] > 0) {
            $message .= get_string('deleteincludexevidence', 'shezar_hierarchy', $stats['evidence']) . html_writer::empty_tag('br');
        }

        if ($stats['related'] > 0) {
            $message .= get_string('deleteincludexrelatedcompetencies', 'shezar_hierarchy', $stats['related']). html_writer::empty_tag('br');
        }
    ...
    public static function check_feature_enabled() {
        if (shezar_feature_disabled('competencies')) {
            print_error('competenciesdisabled', 'shezar_hierarchy');
        }
    }
