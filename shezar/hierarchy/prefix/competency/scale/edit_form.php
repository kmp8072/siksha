...
class edit_scale_form extends moodleform {
    function definition() {
        ...
        $mform->addHelpButton('name', 'competencyscalescalename', 'shezar_hierarchy');
        $mform->addRule('name', get_string('required'), 'required', null, 'client');
        $mform->setType('name', PARAM_TEXT);

        // If it's a new scale, get them to define scale values.
        if ($this->_customdata['scaleid'] == 0) {
            $mform->addElement('textarea', 'scalevalues', get_string('scalevalues', 'shezar_hierarchy') .
                html_writer::empty_tag('br') . html_writer::start_tag('i') . '(' . get_string('notescalevalueentry', 'shezar_hierarchy') . ')' . html_writer::end_tag('i'), 'rows="5" cols="30"');
            $mform->addHelpButton('scalevalues', 'competencyscalescalevalues', 'shezar_hierarchy');
            ...
            $link = html_writer::link($linkurl, get_string('linktoscalevalues', 'shezar_hierarchy'));
