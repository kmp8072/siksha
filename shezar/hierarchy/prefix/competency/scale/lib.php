...
function competency_scale_display_table($scales) {
    ...
    // Cache permissions
    $can_edit = has_capability('shezar/hierarchy:updatecompetencyscale', $sitecontext);
    $can_delete = has_capability('shezar/hierarchy:deletecompetencyscale', $sitecontext);
    $can_add = has_capability('shezar/hierarchy:createcompetencyscale', $sitecontext);
    $can_view = has_capability('shezar/hierarchy:viewcompetencyscale', $sitecontext);
    ...
    $stroptions = get_string('options', 'shezar_core');
    ...
    if ($scales) {
        ...
        foreach ($scales as $scale) {
            ...
            $line = array();
            $line[] = $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/prefix/competency/scale/view.php', array('id' => $scale->id, 'prefix' => 'competency')), format_string($scale->name));
            if ($scale_used) {
                $line[] = get_string('yes');
            } else if ($scale_assigned) {
                $line[] = get_string('assignedonly', 'shezar_hierarchy');
            } else {
                $line[] = get_string('no');
            }
            ...
            if ($can_edit || $can_delete) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/competency/scale/edit.php', array('id' => $scale->id, 'prefix' => 'competency')),
                        new pix_icon('t/edit', $stredit), null, array('title' => $stredit));
                }

                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscaleinuse', 'shezar_hierarchy'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletecompetencyscaleinuse', 'shezar_hierarchy')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletecompetencyscaleassigned', 'shezar_hierarchy'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletecompetencyscaleassigned', 'shezar_hierarchy')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/competency/scale/delete.php', array('id' => $scale->id, 'prefix' => 'competency')),
    ...
    $templatedata->heading = get_string('competencyscales', 'shezar_hierarchy');
    ...
    if ($can_add) {
        $templatedata->addbuttons = $OUTPUT->single_button(new moodle_url('/shezar/hierarchy/prefix/competency/scale/edit.php',
            array('prefix' => 'competency')), get_string('scalescompcustomcreate', 'shezar_hierarchy'), 'get');
        $templatedata->addbuttons .= $OUTPUT->help_icon('competencyscalesgeneral', 'shezar_hierarchy');
    }

    echo $OUTPUT->render_from_template('shezar_hierarchy/admin_scales', $templatedata);
}
