...
// Permissions.
require_capability('shezar/hierarchy:deletecompetencyscale', $sitecontext);
...
$returnurl = "{$CFG->wwwroot}/shezar/hierarchy/framework/index.php?prefix=competency";
$deleteurl = "{$CFG->wwwroot}/shezar/hierarchy/prefix/competency/scale/delete.php?id={$scale->id}&amp;delete=".md5($scale->timemodified)."&amp;sesskey={$USER->sesskey}&amp;prefix=competency";

// Can't delete if the scale is in use or assigned
if (competency_scale_is_used($id)) {
    print_error('error:nodeletecompetencyscaleinuse', 'shezar_hierarchy', $returnurl);
}
if (competency_scale_is_assigned($id)) {
    print_error('error:nodeletecompetencyscaleassigned', 'shezar_hierarchy', $returnurl);
}

if (!$delete) {
    echo $OUTPUT->header();
    $strdelete = get_string('deletecheckscale', 'shezar_hierarchy');
...
// redirect
shezar_set_notification(get_string('deletedcompetencyscale', 'shezar_hierarchy', format_string($scale->name)), $CFG->wwwroot . '/shezar/hierarchy/framework/index.php?prefix=competency', array('class' => 'notifysuccess'));
