...
require_once($CFG->dirroot . '/shezar/hierarchy/prefix/competency/scale/lib.php');
...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'shezar_hierarchy', 'comp_scale_values', $value->id);
...
if ($valueform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/shezar/hierarchy/prefix/competency/scale/view.php?id={$value->scaleid}&amp;prefix=competency");
...
// Update data
    ...
    if ($valuenew->id == 0) {
        ...
        $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_hierarchy', 'comp_scale_values', $valuenew->id);
        ...
        $notification->url = "$CFG->wwwroot/shezar/hierarchy/prefix/competency/scale/view.php?id={$valuenew->scaleid}&amp;prefix=competency";
