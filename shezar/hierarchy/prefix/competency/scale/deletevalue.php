...
$returnurl = new moodle_url('/shezar/hierarchy/prefix/competency/scale/view.php', $returnparams);
$deleteparams = array('id' => $value->id, 'delete' => md5($value->timemodified), 'sesskey' => $USER->sesskey, 'prefix' => 'competency');
$deleteurl = new moodle_url('/shezar/hierarchy/prefix/competency/scale/deletevalue.php', $deleteparams);
...
if ($value->id == $scale->defaultid) {
    shezar_set_notification(get_string('error:nodeletecompetencyscalevaluedefault', 'shezar_hierarchy'), $returnurl);
}
...
shezar_set_notification(get_string('deletedcompetencyscalevalue', 'shezar_hierarchy', format_string($value->name)), $returnurl, array('class' => 'notifysuccess'));
