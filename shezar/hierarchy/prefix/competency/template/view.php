...
$can_edit   = has_capability('shezar/hierarchy:update'.$hierarchy->prefix.'template', $sitecontext);
...
$sitecontext = context_system::instance();
require_capability('shezar/hierarchy:view'.$hierarchy->prefix, $sitecontext);
...
if ($competencies) {
    ...
    echo html_writer::tag('p', get_string('noassignedcompetenciestotemplate', 'shezar_hierarchy'));
}
...
if ($can_edit) {
    ...
    $action = new moodle_url('/shezar/hierarchy/prefix/' . $hierarchy->prefix . '/template/find_competency.php', array('templateid' => $item->id));
    $out .= html_writer::start_tag('form', array('action' => $action->out(), 'method' => 'get'));
    $out .= html_writer::start_tag('div');
    $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => "show-assignment-dialog", 'value' => get_string('assignnewcompetency', 'shezar_hierarchy')));
