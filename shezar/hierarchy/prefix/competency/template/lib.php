...
function competency_template_display_table($templates, $frameworkid) {
    ...
    $can_add = has_capability('shezar/hierarchy:createcompetencytemplate', $sitecontext);
    $can_edit = has_capability('shezar/hierarchy:updatecompetencytemplate', $sitecontext);
    $can_delete = has_capability('shezar/hierarchy:deletecompetencytemplate', $sitecontext);
    ...
    if ($templates) {
        ...
        $table->head = array();
        $table->align = array();
        $table->head[] = get_string('template', 'shezar_hierarchy');
        $table->align[] = 'left';
        $table->head[] = get_string('competencies', 'shezar_hierarchy');
        $table->align[] = 'center';
        $table->head[] = get_string('createdon', 'shezar_hierarchy');
        $table->align[] = 'left';
    ...
    echo $OUTPUT->heading(get_string('competencytemplates', 'shezar_hierarchy'));

    if ($templates) {
        echo html_writer::table($table);
    } else {
        echo html_writer::tag('p', get_string('notemplateinframework', 'shezar_hierarchy'));
    }
    ...
    if ($can_add) {
        ...
        $OUTPUT->single_button(new moodle_url('prefix/competency/template/edit.php', $data), get_string('addnewtemplate', 'shezar_hierarchy'), 'get'),
        array('class' => 'buttons'));
    }
}
