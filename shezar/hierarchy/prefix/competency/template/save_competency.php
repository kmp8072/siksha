...
// Setup page
admin_externalpage_setup('competencyframework', '', array(), '/shezar/hierarchy/prefix/competency/template/update_assignments.php');

// Check permissions
$sitecontext = context_system::instance();
require_capability('shezar/hierarchy:updatecompetencytemplate', $sitecontext);
...
if (!$template = $hierarchy->get_template($id)) {
    print_error('incorrecttemplateid', 'shezar_hierarchy');
}
...
if ($deleteexisting) {
    ...
    foreach ($removeditems as $ritem) {
        ...
        echo get_string('reloadpage', 'shezar_hierarchy');  // Indicate that a page reload is required
    }
}
...
foreach ($assignments as $assignment) {
    ...
    if ($nojs) {
        ...
        if ($editingon) {
            ...
            echo $OUTPUT->action_link(new moodle_url("/shezar/hierarchy/prefix/{$hierarchy->prefix}/template/remove_assignment.php", array('templateid' => $template->id, 'assignment' => $competency->id)),
