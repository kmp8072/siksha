...
if ($nojs) {
    ...
    echo $OUTPUT->header();
    $out = html_writer::tag('h2', get_string('assignnewevidenceitem', 'shezar_hierarchy'));
    ...
    if ($category != 0) {
        if ($courses = $DB->get_records('course', array('category' => $category), 'sortorder')) {
            ...
            print html_writer::tag('p', get_string('nocoursesincat','shezar_hierarchy'));
        }
    }
    echo $OUTPUT->footer();
    ...
    $dialog = new shezar_dialog_content_courses($parentid, false);
