...
function comp_evitem_print_course_evitems($course, $competency_id, $addurl ) {
    ...
    $alreadystr = get_string('alreadyselected', 'shezar_core');
        ...
        if ($evidence) {
            ...
            foreach ($evidence as $activity) {
                ...
                if (array_key_exists("activitycompletion-{$activity->id}", $existingevidencelookup)) {
                    echo html_writer::tag('span', get_string('activitycompletion', 'shezar_hierarchy') . ' - ' . format_string($activity->name) . ' ' . $alreadystr, array('class' => "unclickable"));
                } else {
                    echo html_writer::start_tag('span', array('type' => 'activitycompletion', 'id' => $activity->id));
                    echo $OUTPUT->action_link(new moodle_url('#'), get_string('activitycompletion', 'shezar_hierarchy') . ' - ' . format_string($activity->name));
    ...
    if ($completion_info->is_enabled() &&
        ...
        if (array_key_exists("coursecompletion-{$course->id}", $existingevidencelookup)) {
            echo html_writer::tag('span', get_string('coursecompletion', 'shezar_core') . ' ' . $alreadystr, array('class' => "unclickable"));
        } else {
    ...
    if (!$available) {
        echo html_writer::tag('em', get_string('noevidencetypesavailable', 'shezar_hierarchy'));
    }
}
