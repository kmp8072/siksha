 ...
 */
require_once($CFG->dirroot.'/shezar/hierarchy/prefix/competency/evidence/evidence.php');
...
function hierarchy_can_add_competency_evidence($plan, $component, $userid, $competencyid) {
    ...
    if (!has_capability('shezar/plan:accessanyplan', $systemcontext) && ($plan->get_setting('view') < DP_PERMISSION_ALLOW)) {
        return array('error:nopermissions', 'shezar_plan');
    }

    if ($component->get_setting('setproficiency') != DP_PERMISSION_ALLOW) {
        return array('error:competencystatuspermission', 'shezar_plan');
    }

    // Validate whether the plan belongs to the specified user
    if (!$plan->userid == $userid) {
        return array('error:usernotfound','shezar_plan');
    }
...
function hierarchy_add_competency_evidence($competencyid, $userid, $prof, $component, $details, $reaggregate = true, $notify = true) {
    ...
    $count = $DB->count_records('block_shezar_stats', array('userid' => $currentuser, 'eventtype' => $event, 'data2' => $data2));
    ...
    if ($isproficient && $count == 0) {
        shezar_stats_add_event($time, $currentuser, $event, '', $data2);
        if ($notify && $component instanceof dp_competency_component) {
            ...
            $alert_detail->text = get_string('competencycompleted', 'shezar_plan');
    ...
    // check record exists for removal and is set to "not proficient"
        ...
        shezar_stats_remove_event($currentuser, $event, $data2);
    }
