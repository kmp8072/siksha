...
require_once($CFG->dirroot.'/shezar/hierarchy/prefix/competency/related/lib.php');
...
if ($showhidden && !has_capability('shezar/hierarchy:updatecompetencyframeworks', $context)) {
    print_error('nopermviewhiddenframeworks', 'shezar_hierarchy');
}
...
// Setup page
admin_externalpage_setup('competencymanage', '', array(), '/shezar/hierarchy/prefix/competency/related/add.php');
...
if (!$nojs) {
    ...
    $dialog = new shezar_dialog_content_hierarchy_multi('competency', $frameworkid, $showhidden);
    ...
    echo $OUTPUT->header();
    $out = html_writer::tag('h2', get_string('assignrelatedcompetencies', 'shezar_hierarchy'));
    ...
    if (empty($frameworkid) || $frameworkid == 0) {
        ...
        $out .= build_nojs_frameworkpicker(
            $hierarchy,
            '/shezar/hierarchy/prefix/competency/related/find.php',
            array(
                'returnurl' => $returnurl,
                's' => $s,
                'nojs' => 1,
        ...
        $out .=build_nojs_treeview(
            ...
            '/shezar/hierarchy/prefix/competency/related/save.php',
            array(
                's' => $s,
                'returnurl' => $returnurl,
                'nojs' => 1,
