...
if (!$item) {
    print_error('competencyrelationshipnotfound', 'shezar_hierarchy');
}
...
// Cancel/return url.
$return = "{$CFG->wwwroot}/shezar/hierarchy/item/view.php?prefix={$hierarchy->prefix}&id={$id}";


if (!$delete) {
    $message = get_string('relateditemremovecheck', 'shezar_hierarchy'). html_writer::empty_tag('br') . html_writer::empty_tag('br');
    $message .= format_string($rcompetency->fullname);

    $action = "{$CFG->wwwroot}/shezar/hierarchy/prefix/competency/related/remove.php?id={$id}&amp;related={$related}&amp;delete=".md5($rcompetency->timemodified)."&amp;sesskey={$USER->sesskey}";
...
$message = get_string('removed'.$hierarchy->prefix.'relateditem', 'shezar_hierarchy', format_string($rcompetency->fullname));
