...
// Setup page
admin_externalpage_setup('competencymanage', '', array(), '/shezar/hierarchy/prefix/competency/related/save.php');
...
if (!$competency = $hierarchy->get_item($compid)) {
    print_error('competencynotfound', 'shezar_hierarchy');
}
...
foreach ($relidlist as $relid) {
    // Check id
    if (!is_numeric($relid)) {
        print_error('baddatanonnumeric', 'shezar_hierarchy', null, $relid);
    }
