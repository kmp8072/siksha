...
if ($id == 0) {
    ...
    if (!competency_scales_available()) {
        ...
        /// Display page header
        echo $OUTPUT->header();
        notice(get_string('nocompetencyscales','shezar_hierarchy'), "{$CFG->wwwroot}/shezar/hierarchy/framework/index.php?prefix=competency" );
        echo $OUTPUT->footer();
        die();
    }
...
// Update data
    ...
    if (isset($frameworknew->scale)) {
        ...
        foreach ($scales_new as $key) {
            ...
            if (!$DB->insert_record($shortprefix.'_scale_assignments', $assignment)) {
                print_error('addscaleassignment', 'shezar_hierarchy');
            }
        }
    ...
    if ($new) {
        ...
        \hierarchy_competency\event\framework_updated::create_from_instance($frameworknew)->trigger();
    }
