...
if (!$assignment = $DB->get_record('pos_competencies', array('id' => $id))) {
    print_error('posassignmentnotexist', 'shezar_hierarchy');
}
...
$returnurl = new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'position', 'id' => $position, 'framework' => $frameworkid));
$deleteurl = new moodle_url('/shezar/hierarchy/prefix/position/assigncompetency/remove.php',
...
if ($delete) {
    ...
    shezar_set_notification(get_string('positiondeletedassignedcompetency', 'shezar_hierarchy'), $returnurl, array('class' => 'notifysuccess'));
} else {
    /// Display page
    echo $OUTPUT->header();
