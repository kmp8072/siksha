...
class goal extends hierarchy {
    ...
    public function hierarchy_page_setup($page = '', $item = null) {
        ...
        $PAGE->requires->strings_for_js(array('assigngroup'), 'shezar_hierarchy');

        // Selector setup.
        $jsmodule = array(
            'name' => 'shezar_assigngroups',
            'fullpath' => '/shezar/hierarchy/prefix/goal/assign/assigngroup_dialog.js',
            'requires' => array('json'));
        $args = array('args'=>'{"module":"goal","sesskey":"'.sesskey().'"}');
        $PAGE->requires->js_init_call('M.shezar_assigngroupdialog.init', $args, false, $jsmodule);
    }
    ...
    public function display_extra_view_info($item) {
        ...
        require_once($CFG->dirroot . '/shezar/hierarchy/prefix/goal/assign/lib.php');

        // Set up some variables.
        $renderer = $PAGE->get_renderer('shezar_hierarchy');
        $can_assign = has_capability('shezar/hierarchy:managegoalassignments', context_system::instance());
        $assignclass = new shezar_assign_goal('goal', $item);
        $options = array(
            "" => get_string('assigngroup', 'shezar_hierarchy'),
            "org" => get_string('addorganisations', 'shezar_hierarchy'),
            'pos' => get_string('addpositions', 'shezar_hierarchy'),
            'cohort' => get_string('addcohorts', 'shezar_hierarchy'),
        );
        ...
        if ($can_assign) {
            echo html_writer::label(get_string('assigngroup', 'shezar_hierarchy'), 'menugroupselector', true, array('class' => 'sr-only'));
    ...
    public function get_item_data($item, $cols = null) {
        ...
        if (!empty($item->targetdate)) {
            $format_date = userdate($item->targetdate, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
            $data[] = array(
        ...
        if (!empty($scaleid)) {
            $data[] = array(
                'title' => get_string('goalscale', 'shezar_hierarchy'),
    ...
    public function add_additional_item_form_fields(&$mform) {
        ...
        $mform->addHelpButton('scalename', 'goalscale', 'shezar_hierarchy');

        $mform->addElement('date_selector', 'targetdate', get_string('goaltargetdate', 'shezar_hierarchy'), array('optional' => true));
        $mform->addHelpButton('targetdate', 'goaltargetdate', 'shezar_hierarchy');
    ...
    public function display_additional_item_form_fields($item, $cssclass) {
        ...
        if (!empty($item->targetdate)) {
            $targetdate = userdate($item->targetdate, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
            $out .= html_writer::tag('div', html_writer::tag('strong', get_string('goaltargetdate', 'shezar_hierarchy') . ': ') . $targetdate,
    ...
    public function output_delete_message($stats) {
        ...
        if ($stats['user_assignments'] > 0) {
            $message .= get_string('deleteincludexuserassignments', 'shezar_hierarchy',
    ...
    public static function delete_group_assignment($assigntype, $assignmentid, $type, $delete_params) {
        ...
        if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
            ...
            print_error('error:deletingindiviualassignments', 'shezar_hierarchy');
        }
    ...
    public function create_user_assignments($type, $assignment, $includechildren = null) {
        ...
        if (!empty($includechildren)) {
            if ($type == GOAL_ASSIGNMENT_AUDIENCE) {
                ...
                print_error('error:includechildrencohort', 'shezar_hierarchy');
            }
    ...
    public static function get_user_assignments($userid, $canedit = null, $display = false) {
        ...
        foreach ($assignments as $assignment) {
            ...
            if ($assignment->assigntype == GOAL_ASSIGNMENT_INDIVIDUAL && $canedit) {
                ...
                $del_url = new moodle_url('/shezar/hierarchy/prefix/goal/assign/remove.php', $del_params);
            ...
            if (!empty($assignment_info[$goalid])) {
                ...
                if (has_capability('shezar/hierarchy:viewgoal', context_system::instance())) {
                    // Put a link in so they can actually see what the goal is.
                    $goal_params = array('id' => $goalid, 'prefix' => 'goal');
                    $goal_url = new moodle_url('/shezar/hierarchy/item/view.php', $goal_params);
        ...
        if ($scope == self::SCOPE_PERSONAL) {
            ...
            switch ($assignment->assigntype) {
                case GOAL_ASSIGNMENT_SELF:
                    // Self: unassign?
                    $assignment_string = get_string('goalassignmentself', 'shezar_hierarchy');
                    break;
                case GOAL_ASSIGNMENT_MANAGER:
                    // Mananger: {$managername}.
                    $manager = fullname($DB->get_record('user', array('id' => $assignment->usercreated)));
                    $assignment_string = get_string('goalassignmentmanager', 'shezar_hierarchy', $manager);
                    break;
                case GOAL_ASSIGNMENT_ADMIN:
                    // Admin: {$adminname}.
                    $admin = fullname($DB->get_record('user', array('id' => $assignment->usercreated)));
                    $assignment_string = get_string('goalassignmentadmin', 'shezar_hierarchy', $admin);
                    break;
                default:
                    // Unrecognised assignment type.
                    print_error('invalidassigntype', 'shezar_hierarchy', 'personal');
            }
            ...
            switch ($assignment->assigntype) {
                case GOAL_ASSIGNMENT_INDIVIDUAL:
                    ...
                    if (!empty($assignment->extrainfo) && ($extrainfo = str_replace('OLD:', '', $assignment->extrainfo))) {
                        ...
                        $replace->type = get_string('assign' . $type->fullname, 'shezar_hierarchy');
                        $replace->name = format_string($type->modname);
                        $assignment_string = get_string('oldgoalassignment', 'shezar_hierarchy', $replace);
                    } else {
                        // This is your average individual assignment.
                        $user = $DB->get_record('user', array('id' => $assignment->usermodified));
                        $name = fullname($user);
                        $assignment_string = get_string('goalassignmentindividual', 'shezar_hierarchy', $name);
                    }
                ...
                case GOAL_ASSIGNMENT_AUDIENCE:
                    ...
                    $assignment_string = get_string('goalassignmentaudience', 'shezar_hierarchy',
                ...
                case GOAL_ASSIGNMENT_POSITION:
                    ...
                    $assignment_string = get_string('goalassignmentposition', 'shezar_hierarchy',
                ...
                case GOAL_ASSIGNMENT_ORGANISATION:
                    ...
                    $assignment_string = get_string('goalassignmentorganisation', 'shezar_hierarchy',
                            format_string($org->fullname));
                    break;
                default:
                    // Unrecognised assignment type.
                    print_error('invalidassigntype', 'shezar_hierarchy', 'company');
            }
        } else {
            // Unrecognised goal type.
            print_error('invalidgoaltype', 'shezar_hierarchy');
        }
     ...
     *
     * @param string $grp_type  The type as handed back by shezar_assign_goal class
    ...
    public static function can_view_goals($userid = null) {
        ...
        $view_personal = has_capability('shezar/hierarchy:viewownpersonalgoal', $usercontext);
        if ($view_personal) {
            $has_personal = $DB->record_exists('goal_personal', array('userid' => $userid, 'deleted' => 0));
            if ($has_personal) {
                return true;
            }
            $add_personal = has_capability('shezar/hierarchy:manageownpersonalgoal', $usercontext);
            if ($add_personal) {
                return true;
            }
        }
        ...
        $view_company = has_capability('shezar/hierarchy:viewowncompanygoal', $usercontext);
        if ($view_company) {
            $has_company = $DB->record_exists('goal_user_assignment', array('userid' => $userid));
            if ($has_company) {
                return true;
            }
            $add_company = has_capability('shezar/hierarchy:manageowncompanygoal', $usercontext);
            if ($add_company) {
                return true;
            }
        }
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        if (!empty($user)) {
            ...
            $jobassignments = \shezar_job\job_assignment::get_all($userid, false);
            $teamleaders = array();
            $appraisers = array();
            ...
            if (has_capability('shezar/hierarchy:managegoalassignments', $syscontext)) {
                ...
                $permissions['can_view_personal'] = has_capability('shezar/hierarchy:viewstaffpersonalgoal', $context);
                $permissions['can_edit_personal'] = has_capability('shezar/hierarchy:managestaffpersonalgoal', $context);
                $permissions['can_view_company'] = has_capability('shezar/hierarchy:viewstaffcompanygoal', $context);
                $permissions['can_edit_company'] = has_capability('shezar/hierarchy:managestaffcompanygoal', $context);
                $permissions['can_edit'] = array(
                ...
                // User permissions.
                $permissions['can_view_personal'] = has_capability('shezar/hierarchy:viewownpersonalgoal', $context);
                $permissions['can_edit_personal'] = has_capability('shezar/hierarchy:manageownpersonalgoal', $context);
                $permissions['can_view_company'] = has_capability('shezar/hierarchy:viewowncompanygoal', $context);
                $permissions['can_edit_company'] = has_capability('shezar/hierarchy:manageowncompanygoal', $context);
                $permissions['can_edit'] = array(
                    GOAL_ASSIGNMENT_INDIVIDUAL => $permissions['can_edit_company'],
                    GOAL_ASSIGNMENT_SELF => $permissions['can_edit_personal'],
    ...
    public static function check_feature_enabled() {
        if (shezar_feature_disabled('goals')) {
            print_error('goalsdisabled', 'shezar_hierarchy');
        }
    }
    ...
    public function display_hierarchy_item($record, $include_custom_fields=false, $indicate_depth=true, $cfields=array(), $types=array()) {
        ...
        if (isset($record->name)) {
            $out .= $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/prefix/goal/item/view.php',
                array('id' => $record->id)), $record->name, null, array('class' => $cssclass));
        } else {
            $out .= $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/item/view.php',
        ...
        if ($include_custom_fields) {
            ...
            if ($record->description) {
                $record->description = file_rewrite_pluginfile_urls($record->description, 'pluginfile.php',
                    context_system::instance()->id, 'shezar_hierarchy', $this->shortprefix, $record->id);
            ...
            if ($record->typeid !=0 && is_array($types) && array_key_exists($record->typeid, $types)) {
                $out .= html_writer::tag('div', html_writer::tag('strong', get_string('type', 'shezar_hierarchy') . ': ') .
                    format_string($types[$record->typeid]->fullname), array('class' => 'itemtype' . $cssclass));
                if (!empty($CFG->showhierarchyshortnames)) {
                    $out .= html_writer::tag('div', html_writer::tag('strong', get_string('shortnametype', 'shezar_hierarchy') . ': ') .
    ...
    public function display_add_type_button($page=0, $class='') {
        ...
        echo $OUTPUT->single_button($url, get_string('add' . $type . 'type', 'shezar_hierarchy'), 'get');
    }
