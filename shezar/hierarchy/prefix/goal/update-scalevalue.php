...
if (!$permissions = $goal->get_permissions(null, $userid)) {
    echo get_string('error:viewusergoals', 'shezar_hierarchy');
    exit;
}
...
if ($scope == goal::SCOPE_PERSONAL && !$can_edit[$goalitem->assigntype]) {
    echo get_string('error:updatescalevalue', 'shezar_hierarchy');
    exit;
}
...
$return = new moodle_url('/shezar/hierarchy/prefix/goal/mygoals.php', array('userid' => $userid));

if ($result) {
    if ($nojs) {
        $message = get_string('updatescalevaluesuccess', 'shezar_hierarchy');
        shezar_set_notification($message, $return, array('class' => 'notifysuccess'));
    }
    echo "OK";
} else {
    if ($nojs) {
        $message = get_string('updatescalevaluefailure', 'shezar_hierarchy');
        shezar_set_notification($message, $return, array('class' => 'notifyproblem'));
    }
    echo get_string('error:updatingscalevalue', 'shezar_hierarchy');
}
