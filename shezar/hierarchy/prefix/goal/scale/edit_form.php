...
class edit_scale_form extends moodleform {
    public function definition() {
        ...
        $mform->addHelpButton('name', 'goalscalescalename', 'shezar_hierarchy');
        $mform->addRule('name', get_string('required'), 'required', null, 'client');
        $mform->setType('name', PARAM_TEXT);

        // If it's a new scale, get them to define scale values.
        if ($this->_customdata['scaleid'] == 0) {
            $mform->addElement('textarea', 'scalevalues', get_string('scalevalues', 'shezar_hierarchy'), 'rows="5" cols="30"');
            $mform->addHelpButton('scalevalues', 'goalscalescalevalues', 'shezar_hierarchy');
            ...
            $link = html_writer::link($linkurl, get_string('linktoscalevaluesgoal', 'shezar_hierarchy'));
