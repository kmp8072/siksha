...
$goalscalestr = get_string('goalscale', 'shezar_hierarchy');
$pageurl = new moodle_url('/shezar/hierarchy/prefix/goal/scale/view.php', array('id' => $id, 'prefix' => $prefix));
...
if ($canupdatescales || $candeletescales) {
    ...
    if ((!empty($moveup) or !empty($movedown))) {
        ...
        if (goal_scale_is_used($scale->id)) {
            $returnurl = new moodle_url('/shezar/hierarchy/prefix/goal/scale/view.php',
    ...
    if ($default) {
        ...
        if (!$DB->update_record('goal_scale', $s)) {
            print_error('updategoalscale', 'shezar_hierarchy');
        } else {
            // Fetch the update scale record so it'll show up to the user.
            $scale = $DB->get_record('goal_scale', array('id' => $id));
            shezar_set_notification(get_string('scaledefaultupdated', 'shezar_hierarchy'), null,
...
$PAGE->navbar->add(get_string("goalframeworks", 'shezar_hierarchy'),
                    new moodle_url('/shezar/hierarchy/framework/index.php', array('prefix' => 'goal')));
$PAGE->navbar->add(format_string($scale->name));

echo $OUTPUT->header();

echo $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/framework/index.php', array('prefix' => 'goal')),
    '&laquo; ' . get_string('allgoalscales', 'shezar_hierarchy'));

// Display info about scale.
echo $OUTPUT->heading(get_string('scalex', 'shezar_hierarchy', format_string($scale->name)));
$scale->description = file_rewrite_pluginfile_urls($scale->description, 'pluginfile.php', $sitecontext->id,
    'shezar_hierarchy', 'goal_scale', $scale->id);
...
if ($values) {
    if ($canupdatescales) {
        echo html_writer::start_tag('form', array('id' => 'compscaledefaultprofform',
            'action' => new moodle_url('/shezar/hierarchy/prefix/goal/scale/view.php'), 'method' => 'POST'));
    ...
    $table->head[] = get_string('goalscaledefault', 'shezar_hierarchy').' '.
        $OUTPUT->help_icon('goalscaledefault', 'shezar_hierarchy');
    $table->align[] = 'center';

    $table->head[] = get_string('goalscaleproficient', 'shezar_hierarchy').' '.
        $OUTPUT->help_icon('goalscaleproficient', 'shezar_hierarchy');
    $table->align[] = 'center';
    ...
    foreach ($values as $value) {
        ...
        if ($canupdatescales || $candeletescales) {
            ...
            $label = html_writer::tag('label', get_string('goalscalesetdefault', 'shezar_hierarchy', $value->name), $labelattrs);
            ...
            $buttons[] = $OUTPUT->action_icon(
                new moodle_url('/shezar/hierarchy/prefix/goal/scale/editvalue.php', array('id' => $value->id, 'prefix' => 'goal')),
            ...
            if (!$scale_used && $candeletescales) {
                if ($value->id == $scale->defaultid) {
                    ...
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                            get_string('error:nodeletegoalscalevaluedefault', 'shezar_hierarchy'), 'shezar_core',
                            array('class' => 'iconsmall action-icon'));
                } else if ($value->id == $onlyprof) {
                    // Prevent deleting last proficient value.
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                            get_string('error:nodeletegoalscalevalueonlyprof', 'shezar_hierarchy'), 'shezar_core',
                            array('class' => 'iconsmall action-icon'));
                } else {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/goal/scale/deletevalue.php',
            ...
            if ($count > 1 && !$scale_used && $canupdatescales) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/goal/scale/view.php',
...
$button_editscale = '';
if ($canupdatescales && !$scale_used) {
    $button_editscale = $OUTPUT->single_button(new moodle_url('/shezar/hierarchy/prefix/goal/scale/editvalue.php',
        array('scaleid' => $scale->id, 'prefix' => 'goal')), get_string('addnewscalevalue', 'shezar_hierarchy'), 'get');
}
