...
$returnurl = "{$CFG->wwwroot}/shezar/hierarchy/framework/index.php?prefix=goal";
$deleteurl = "{$CFG->wwwroot}/shezar/hierarchy/prefix/goal/scale/delete.php?"
...
if (goal_scale_is_used($id)) {
    print_error('error:nodeletegoalscaleinuse', 'shezar_hierarchy', $returnurl);
}
if (goal_scale_is_assigned($id)) {
    print_error('error:nodeletegoalscaleassigned', 'shezar_hierarchy', $returnurl);
}
...
// Redirect.
shezar_set_notification(get_string('deletedgoalscale', 'shezar_hierarchy', format_string($scale->name)),
    $CFG->wwwroot . '/shezar/hierarchy/framework/index.php?prefix=goal', array('class' => 'notifysuccess'));
