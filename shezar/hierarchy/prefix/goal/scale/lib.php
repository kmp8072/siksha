...
function goal_scale_display_table($scales) {
    ...
    // Cache permissions.
    $can_edit = has_capability('shezar/hierarchy:updategoalscale', $sitecontext);
    $can_delete = has_capability('shezar/hierarchy:deletegoalscale', $sitecontext);
    $can_add = has_capability('shezar/hierarchy:creategoalscale', $sitecontext);
    $can_view = has_capability('shezar/hierarchy:viewgoalscale', $sitecontext);
    ...
    if ($scales) {
        ...
        foreach ($scales as $scale) {
            ...
            $line = array();
            $line[] = $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/prefix/goal/scale/view.php',
            ...
            if ($can_edit || $can_delete) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/goal/scale/edit.php',
                ...
                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                                get_string('error:nodeletegoalscaleinuse', 'shezar_hierarchy'), 'shezar_core',
                                array('class' => 'iconsmall disabled',
                                      'title' => get_string('error:nodeletegoalscaleinuse', 'shezar_hierarchy')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey',
                                get_string('error:nodeletegoalscaleassigned', 'shezar_hierarchy'), 'shezar_core',
                                array('class' => 'iconsmall disabled',
                                      'title' => get_string('error:nodeletegoalscaleassigned', 'shezar_hierarchy')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/goal/scale/delete.php',
    ...
    $templatedata->heading = get_string('goalscales', 'shezar_hierarchy');
    ...
    if ($can_add) {
        $buttonurl = new moodle_url('/shezar/hierarchy/prefix/goal/scale/edit.php', array('prefix' => 'goal'));

        $templatedata->addbuttons = $OUTPUT->single_button($buttonurl, get_string('scalesgoalcustomcreate', 'shezar_hierarchy'), 'get');
        $templatedata->addbuttons .= $OUTPUT->help_icon('goalscalesgeneral', 'shezar_hierarchy');
    }
