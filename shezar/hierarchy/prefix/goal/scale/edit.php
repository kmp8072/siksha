...
if ($mform->is_cancelled()) {
    // If cancelled.
    redirect("$CFG->wwwroot/shezar/hierarchy/framework/index.php?prefix=goal");
    ...
    if (empty($scalenew->id)) {
        ...
        $scalenew = file_postupdate_standard_editor($scalenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
            'shezar_hierarchy', 'goal_scale', $scalenew->id);
        ...
        $notification->url = "$CFG->wwwroot/shezar/hierarchy/prefix/goal/scale/view.php?id={$scalenew->id}&amp;prefix=goal";
