...
require_once($CFG->dirroot . '/shezar/hierarchy/prefix/goal/item/edit_form.php');
...
$pageurl = new moodle_url('/shezar/hierarchy/prefix/goal/item/edit_personal.php', array('userid' => $userid));
...
if (!empty($id)) {
    ...
    if (!$can_edit[$goalpersonal->assigntype]) {
        print_error('error:editgoals', 'shezar_hierarchy');
    }
} else {
    $goalpersonal->userid = $userid;
    $goalname = get_string('addgoalpersonal', 'shezar_hierarchy');
    ...
    if (!$can_edit[GOAL_ASSIGNMENT_SELF] && !$can_edit[GOAL_ASSIGNMENT_MANAGER] && !$can_edit[GOAL_ASSIGNMENT_ADMIN]) {
        print_error('error:createpersonalgoal', 'shezar_hierarchy');
    }
}
...
$item->descriptionformat = FORMAT_HTML;
$item = file_prepare_standard_editor($item, 'description', $options, $options['context'], 'shezar_hierarchy','goal', $item->id);
...
if ($mform->is_cancelled()) {
    // Cancelled.
    redirect("{$CFG->wwwroot}/shezar/hierarchy/prefix/goal/mygoals.php?userid={$userid}");
    ...
    if (isset($existingrecord)) {
        ...
        $fromform = file_postupdate_standard_editor($fromform, 'description', $TEXTAREA_OPTIONS, $context,
            'shezar_hierarchy', 'goal', $fromform->id);
        ...
        } else if (\shezar_job\job_assignment::is_managing($USER->id, $todb->userid) && $can_edit[GOAL_ASSIGNMENT_MANAGER]) {
    ...
    redirect("{$CFG->wwwroot}/shezar/hierarchy/prefix/goal/mygoals.php?userid={$todb->userid}");
}
...
if ($id) {
    echo $OUTPUT->heading(get_string("editpersonalgoal", 'shezar_hierarchy') . $item->name);
} else {
    echo $OUTPUT->heading(get_string("newpersonalgoal", 'shezar_hierarchy'));
}
