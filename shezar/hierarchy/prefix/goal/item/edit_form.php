...
class goal_edit_personal_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addRule('name', get_string('goalmissingname', 'shezar_hierarchy'), 'required', null);
        $mform->setType('name', PARAM_MULTILANG);

        // Description.
        $mform->addElement('editor', 'description_editor', get_string('description', 'shezar_hierarchy'),
                null, $TEXTAREA_OPTIONS);
        $mform->addHelpButton('description_editor', 'goaldescription', 'shezar_hierarchy');
        ...
        if ($item->id) {
            ...
            $mform->addElement('static', 'type', get_string('type', 'shezar_hierarchy'));
            $mform->setDefault('type', $typename);
            $mform->addHelpButton('type', $prefix.'type', 'shezar_hierarchy');
            ...
            if ($types) {
                ...
                $select = array('0' => get_string('unclassified', 'shezar_hierarchy'));
                ...
                $mform->addElement('select', 'typeid', get_string('type', 'shezar_hierarchy'),
                    $select, shezar_select_width_limiter());
        ...
        $mform->addHelpButton('scaleid', 'goalscale', 'shezar_hierarchy');
    ...
    public function validation($fromform, $files) {
        ...
        if (!$DB->record_exists('user', array('id' => $fromform->userid))) {
            $errors['user'] = get_string('userdoesnotexist', "shezar_core");
        }

        // Check scale exists.
        if (!empty($fromform->scaleid) && !$DB->record_exists('goal_scale', array('id' => $fromform->scaleid))) {
            $errors['scale'] = get_string('invalidgoalscale', "shezar_hierarchy");
        }

        // Check target date is in the future.
        if (!empty($fromform->targetdate) && $fromform->targetdate < time()) {
            $errors['targetdate'] = get_string('error:invaliddatepast', 'shezar_hierarchy');
        }
