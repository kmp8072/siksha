...
$straddgoals = get_string('addgoals', 'shezar_hierarchy');
...
if (!($admin || $manager || $self)) {
    print_error('error:addgoals', 'shezar_hierarchy');
}
...
if (empty($returnurl)) {
    switch ($assigntype) {
        case GOAL_ASSIGNMENT_INDIVIDUAL:
            $returnurl = "/shezar/hierarchy/prefix/goal/mygoals.php?userid={$assignto}";
            break;
        ...
        case GOAL_ASSIGNMENT_ORGANISATION:
            $returnurl = "/shezar/hierarchy/item/view.php?prefix={$type->fullname}&id={$assignto}";
            break;
    }
}
...
// Set up the page.
$PAGE->set_url(new moodle_url('/shezar/hierarchy/prefix/goal/assign/assign.php'), $urlparams);
...
if ($add) {
    ...
    foreach ($items as $item) {
        ...
        if (goal::currently_assigned($assigntype, $assignto, $item)) {
            print_error('error:alreadyassigned', 'shezar_hierarchy');
        }
    ...
    if ($nojs) {
        ...
        switch ($assigntype) {
            ...
            case GOAL_ASSIGNMENT_ORGANISATION:
                $renderer = $PAGE->get_renderer('shezar_hierarchy');
                $addgoalurl = new moodle_url('/shezar/hierarchy/prefix/goal/assign/find.php',
