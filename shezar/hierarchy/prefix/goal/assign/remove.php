...
// Set up the page.
$PAGE->set_url(new moodle_url('/shezar/hierarchy/prefix/goal/assign/remove.php'), $urlparams);
...
$admin = has_capability('shezar/hierarchy:managegoalassignments', $sitecontext);
$manager = false;
$self = false;

if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
        $user_context = context_user::instance($modid);
        $manager = \shezar_job\job_assignment::is_managing($USER->id, $modid) && has_capability('shezar/hierarchy:managestaffcompanygoal', $user_context);
        $self = ($USER->id == $modid) && has_capability('shezar/hierarchy:manageowncompanygoal', $user_context);
}

if (!($admin || $manager || $self)) {
    print_error('error:deletegoalassignment', 'shezar_hierarchy');
}

if ($view_type) {
    // If the flag is set, return to the goal item page.
    $returnurl = new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'goal', 'id' => $goalid));
} else if ($assigntype == GOAL_ASSIGNMENT_POSITION || $assigntype == GOAL_ASSIGNMENT_ORGANISATION) {
    // Return to viewing the hierarchy item.
    $returnurl = new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => $type->fullname, 'id' => $modid));
} else if ($assigntype == GOAL_ASSIGNMENT_AUDIENCE) {
    // Return to the audiences goal tab.
    $returnurl = new moodle_url('/shezar/cohort/goals.php', array('id' => $modid));
} else {
    // Return to the users my goals page.
    $returnurl = new moodle_url('/shezar/hierarchy/prefix/goal/mygoals.php', array('userid' => $modid));
}

$deleteurl = new moodle_url('/shezar/hierarchy/prefix/goal/assign/remove.php',
...
if ($delete) {
    ...
    if ($delete != md5($type->timecreated)) {
        print_error('error:checkvariable', 'shezar_hierarchy');
    }
    ...
    shezar_set_notification(get_string('goaldeletedassignment', 'shezar_hierarchy'), $returnurl,
            array('class' => 'notifysuccess'));
} else {
    // Display page.
    echo $OUTPUT->header();
    $strdelete = get_string('goalassigndeletecheck', 'shezar_hierarchy');
