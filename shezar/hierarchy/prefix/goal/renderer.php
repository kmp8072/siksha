...
class hierarchy_goal_renderer extends plugin_renderer_base {
    ...
    public function report_frameworks($goalframeworks = array()) {
        if (empty($goalframeworks)) {
            return get_string('goalnoframeworks', 'shezar_hierarchy');
        }

        $tableheader = array(get_string('goalframework', 'shezar_hierarchy'),
                             get_string('goalcount', 'shezar_hierarchy'),
                             get_string('goalreports', 'shezar_hierarchy'));
        ...
        foreach ($goalframeworks as $goalframework) {
            ...
            $frameworkurl = new moodle_url('/shezar/hierarchy/index.php',
                    array('prefix' => 'goal', 'frameworkid' => $goalframework->id));
            $summaryreporturl = new moodle_url('/shezar/hierarchy/prefix/goal/summaryreport.php',
            ...
            $row[] = html_writer::link($summaryreporturl, get_string('goalsummaryreport', 'shezar_hierarchy'));
        ...
        $frameworksurl = new moodle_url('/shezar/hierarchy/framework/index.php', array('prefix' => 'goal'));
        $statusreporturl = new moodle_url('/shezar/hierarchy/prefix/goal/statusreport.php', array('clearfilters' => 1));

        $row[] = html_writer::link($frameworksurl, get_string('goalallframeworks', 'shezar_hierarchy'));

        $row[] = $totalgoals;

        $row[] = html_writer::link($statusreporturl, get_string('goalstatusreport', 'shezar_hierarchy'));
    ...
    public function summary_report_table($summaryreportid, $frameworks = array()) {
        ...
        foreach ($frameworks as $framework) {
            ...
            $summaryreporturl = new moodle_url('/shezar/reportbuilder/report.php',
