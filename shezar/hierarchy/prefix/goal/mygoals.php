...
// Set up the page.
$PAGE->set_url(new moodle_url('/shezar/hierarchy/prefix/goal/mygoals.php'));
...
$showhide_url = new moodle_url('/shezar/hierarchy/prefix/goal/mygoals.php', $showhide_params);
$showhide_text = $display ? get_string('hidedetails', 'shezar_hierarchy') : get_string('showdetails', 'shezar_hierarchy');
$personalshowhide_text = $personaldisplay ? get_string('hidedetails', 'shezar_hierarchy') : get_string('showdetails', 'shezar_hierarchy');
...
$PAGE->requires->strings_for_js(array('addgoal', 'assigngoals'), 'shezar_hierarchy');
$PAGE->requires->strings_for_js(array('continue', 'cancel'), 'moodle');
$jsmodule = array(
    'name' => 'shezar_assignindividual',
    'fullpath' => '/shezar/hierarchy/prefix/goal/assign/individual_dialog.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.shezar_assignindividual.init', $args, false, $jsmodule);
...
if ($can_view_company) {
    ...
    if ($can_edit_company) {
        ...
        $addgoalurl = new moodle_url('/shezar/hierarchy/prefix/goal/assign/find.php', $add_params);
        $add_button_text = get_string('addgoalcompany', 'shezar_hierarchy');
    ...
    if ($can_edit_company) {
        ...
        $detailsurl = new moodle_url('/shezar/hierarchy/framework/index.php', array('prefix' => 'goal', 'readonly' => true));
        $company_edit .= html_writer::link($detailsurl, get_string('companygoaldetails', 'shezar_hierarchy'));
    }
    ...
    $company .= $OUTPUT->heading(get_string('companygoals', 'shezar_hierarchy'), 3) . $company_edit;
...
if ($can_view_personal) {
    ...
    if ($can_edit_personal) {
        ...
        $personal_edit_url = new moodle_url('/shezar/hierarchy/prefix/goal/item/edit_personal.php', array('userid' => $userid));
        $personal_edit = $OUTPUT->single_button($personal_edit_url, get_string('addgoalpersonal', 'shezar_hierarchy'), 'get');
    ...
    $personal .= $OUTPUT->heading(get_string('personalgoals', 'shezar_hierarchy'), 3);
