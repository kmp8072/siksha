...
foreach ($add as $addition) {
    ...
    if (!is_numeric($addition)) {
        print_error('baddatanonnumeric', 'shezar_hierarchy', 'id');
    }
    ...
    if ($nojs) {
        ...
        $row->cells[] = new html_table_cell($OUTPUT->action_link(new moodle_url('/shezar/hierarchy/prefix/competency/template/view.php', array('id' => $related->id)), $related->fullname));

        $row->cells[] = new html_table_cell($OUTPUT->action_icon(new moodle_url('/shezar/hierarchy/prefix/organisation/assigncompetency/remove.php', array('id' => $relationship->id, 'organisation' => $organisation->id)),
