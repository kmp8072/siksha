...
if (!$assignment = $DB->get_record('org_competencies', array('id' => $id))) {
    print_error('assignmentnotexist', 'shezar_hierarchy');
}
...
$returnurl = $CFG->wwwroot.'/shezar/hierarchy/item/view.php?prefix=organisation&amp;id='.$organisation.'&amp;framework='.$frameworkid;
$deleteurl = $CFG->wwwroot.'/shezar/hierarchy/prefix/organisation/assigncompetency/remove.php?id='.$id.'&amp;organisation='.$organisation.'&amp;framework='.$frameworkid.'&amp;delete='.md5($assignment->timecreated).'&amp;sesskey='.$USER->sesskey;
...
if ($delete) {
    ...
    shezar_set_notification(get_string('organisationdeletedassignedcompetency', 'shezar_hierarchy'), $returnurl, array('class' => 'notifysuccess'));
} else {
    /// Display page
    echo $OUTPUT->header();

    if (!$delete) {
        $strdelete = get_string('competencyassigndeletecheck', 'shezar_hierarchy');
