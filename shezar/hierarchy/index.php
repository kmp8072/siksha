...
$canview       = has_capability('shezar/hierarchy:view' . $prefix . 'frameworks', $sitecontext);
$canmanagetype = (count($types) > 0) && has_capability('shezar/hierarchy:update' . $prefix . 'type', $sitecontext);
...
if ($format!='') {
    \shezar_hierarchy\event\framework_exported::create_from_instance($prefix, $framework)->trigger();
...
if ($canmanage) {
    ...
    $detailsstr = get_string($prefix . 'details', 'shezar_hierarchy');
    $url_params = array('prefix' => $prefix, 'frameworkid' => $frameworkid);
    $PAGE->set_url(new moodle_url('/shezar/hierarchy/index', $url_params));
...
$table = new shezar_table($prefix.'-framework-index-'.$frameworkid);
$table->define_baseurl(new moodle_url('/shezar/hierarchy/index.php', array('prefix' => $prefix, 'frameworkid' => $frameworkid)));
...
if ($extrafields = $hierarchy->get_extrafields()) {
    foreach ($extrafields as $extrafield) {
        ...
        $row->value = new stdClass();
        $row->value->fullname = get_string($prefix . $extrafield, 'shezar_hierarchy');
        $headerdata[] = $row;
    }
}
...
$baseurl = new moodle_url('/shezar/hierarchy/index.php', $urlparams);
...
if ($searchactive || !$displaymode) {
    ...
    if ($searchactive) {
        ...
        $keywords = shezar_search_parse_keywords($search);
        ...
        list($searchsql, $searchparams) =
            shezar_search_get_keyword_where_clause($keywords, $searchfields, SQL_PARAMS_NAMED, 'search');
...
echo $OUTPUT->container($OUTPUT->action_link(
    new moodle_url('/shezar/hierarchy/framework/index.php', array('prefix' => $prefix)), '&laquo; ' .
            get_string($prefix.'backtoallframeworks', 'shezar_hierarchy')), 'back-link'
);
...
$framework->description = file_rewrite_pluginfile_urls($framework->description, 'pluginfile.php', $sitecontext->id,
        'shezar_hierarchy', $shortprefix.'_framework', $frameworkid);
...
$table->set_no_records_message(get_string('no'.$prefix, 'shezar_hierarchy'));
...
if ($searchactive) {
    if ($filteredcount > 0) {
        ...
        echo html_writer::tag('strong', get_string('showingxofyforsearchz', 'shezar_hierarchy', $a));
    } else {
        echo html_writer::start_tag('p');
        echo html_writer::tag('strong', get_string('noresultsforsearchx', 'shezar_hierarchy', $search));
    }
    echo $OUTPUT->action_link(new moodle_url('/shezar/hierarchy/index.php',
            array('prefix' => $prefix, 'frameworkid' => $frameworkid)), get_string('clearsearch', 'shezar_hierarchy'));
