 ...
 */
M.shezar_certificationconfirm = M.shezar_certificationconfirm  || {
    ...
    init: function(Y, args){
        ...
        if (!this.config.id) {
            throw new Error('M.shezar_certificationconfirm init()-> Required config \'id\' not available.');
        }

        // check jQuery dependency and continue with setup
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_certificationconfirm.init()-> jQuery dependency required for this module to function.');
        }


        // The save changes confirmation dialog
        this.shezarDialog_savechanges = function() {
            ...
            var buttonsObj = {};
            buttonsObj[M.util.get_string('saveallchanges', 'shezar_certification')] = function() { self.save() };
            buttonsObj[M.util.get_string('editcertif', 'shezar_certification')] = function() { handler._cancel() };
            ...
            {
                buttons: buttonsObj,
                title: '<h2>'+ M.util.get_string('confirmchanges', 'shezar_certification') +'</h2>'
            },
        ...
        window.onbeforeunload = function(e) {
            ...
            var str = M.util.get_string('youhaveunsavedchanges', 'shezar_certification');
        ...
        shezarDialogs['savechanges'] = new this.shezarDialog_savechanges();
    ...
    handleSaveChanges: function(){
        ...
        dialog.open(M.util.get_string('tosaveall', 'shezar_certification'));
