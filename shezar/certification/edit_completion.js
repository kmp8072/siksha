 ...
 */
M.shezar_editcertcompletion = M.shezar_editcertcompletion || {
    ...
    init: function(Y, config){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_editcertcompletion.init()-> jQuery dependency required for this module to function.');
        }
        ...
        if ($('input[name="showinitialstateinvalid"]').val() == "1") {
            ...
            stateelement.after("&nbsp;" + M.util.get_string('bestguess', 'shezar_program', bestguessstate));
        }

        this.updateFormState();

        $('#id_state').on('change', function(e) {
            M.shezar_editcertcompletion.updateFormState();
        });
        ...
        $('.fitem_fdate_time_selector select').on('change', function(e) {
            M.shezar_editcertcompletion.updateApparentPeriods();
        });

        $('.deletecompletionhistorybutton').on('click', function(e) {
            if (!confirm(M.util.get_string('confirmdeletecompletion', 'shezar_program'))) {
                e.preventDefault();
            }
        });
        ...
        $('#id_savechanges').on('click', function(e) {
            ...
            switch(state) {
                case 2: // CERTIFCOMPLETIONSTATE_CERTIFIED.
                    M.shezar_editcertcompletion.copyDate('timeexpires', 'timedue');
                    M.shezar_editcertcompletion.copyDate('timecompleted', 'progtimecompleted');
                    break;
    ...
    updateApparentPeriods: function() {
        ...
        switch(state) {
            ...
            case 3: // CERTIFCOMPLETIONSTATE_WINDOWOPEN.
                ...
                apparentwindowperiod = M.shezar_editcertcompletion.getPeriod(
                    windowday, windowmonth, windowyear, expiryday, expirymonth, expiryyear);

                apparentactiveperiod = M.shezar_editcertcompletion.getPeriod(
                    completionday, completionmonth, completionyear, expiryday, expirymonth, expiryyear);
                break;
            case 1: // CERTIFCOMPLETIONSTATE_ASSIGNED.
            case 4: // CERTIFCOMPLETIONSTATE_EXPIRED.
                apparentactiveperiod = M.util.get_string('notapplicable', 'shezar_certification');
                apparentwindowperiod = M.util.get_string('notapplicable', 'shezar_certification');
                break;
        }
    ...
    getPeriod: function(fromDay, fromMonth, fromYear, toDay, toMonth, toYear) {
        if (toDay == fromDay && toMonth == fromMonth) {
            ...
            return M.util.get_string('periodyears', 'shezar_certification', toYear - fromYear);
            ...
            return M.util.get_string('periodmonths', 'shezar_certification', months);
            ...
            if (fromDate.getDay() == toDate.getDay()) {
                ...
                return M.util.get_string('periodweeks', 'shezar_certification', days / 7);

            } else {
                // Otherwise we just have to say how many days it it.
                return M.util.get_string('perioddays', 'shezar_certification', days);
            }
        }
    }
}
