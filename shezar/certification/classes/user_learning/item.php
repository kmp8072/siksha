...
class item extends item_base implements item_has_progress, item_has_dueinfo {
    ...
    public function ensure_completion_loaded() {
        ...
        if ($this->progress_canbecompleted === null) {
            ...
            if ($certificationprogress > 0) {
                $this->progress_summary = get_string('xpercentcomplete', 'shezar_core', $certificationprogress);
            } else {
    ...
    public function export_dueinfo_for_template() {
        ...
        if ($now > $this->duedate) {
            // Overdue.
            $dueinfo->duetext = get_string('userlearningoverduesincex', 'shezar_core', $duedateformatted); // Add this format to language pack?
            ...
            $dueinfo->tooltip = get_string('certificationduex', 'shezar_certification', $duedateformattedtooltip);
        }
