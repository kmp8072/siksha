...
if ($delete) {
    if (!$scale = $DB->get_record('dp_priority_scale', array('id' => $delete))) {
        print_error('error:invalidpriorityscaleid', 'shezar_plan');
    }
    if (dp_priority_scale_is_used($delete)) {
        print_error('error:nodeletepriorityscaleinuse', 'shezar_plan');
    }
    if (dp_priority_scale_is_assigned($delete)) {
        print_error('error:nodeletepriorityscaleassigned', 'shezar_plan');
    }
    ...
    if ($confirm) {
        ...
        \shezar_plan\event\priority_scale_deleted::create_from_scale($scale)->trigger();

        shezar_set_notification(get_string('deletedpriorityscale', 'shezar_plan', format_string($scale->name)), $CFG->wwwroot.'/shezar/plan/priorityscales/index.php', array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/shezar/plan/priorityscales/index.php');
        $deleteurl = new moodle_url('/shezar/plan/priorityscales/index.php', array('delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckpriority', 'shezar_plan');
