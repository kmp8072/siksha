...
function dp_priority_scale_is_used($scaleid) {
    ...
    foreach ($DP_AVAILABLE_COMPONENTS as $component) {
        ...
        $component_class_file = $CFG->dirroot . "/shezar/plan/components/{$component}/{$component}.class.php";
...
function dp_priority_display_table($priorities, $editingon=0) {
    ...
    // Cache permissions
    $can_edit = has_capability('shezar/plan:managepriorityscales', $sitecontext);
    $can_delete = has_capability('shezar/plan:managepriorityscales', $sitecontext);
    ...
    if ($priorities) {
        ...
        foreach ($priorities as $priority) {
            ...
            $title = html_writer::link(new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id)), format_string($priority->name));
            if ($count == 1) {
            ...
            if ($scale_used) {
                ...
                $line[] = get_string('assignedonly', 'shezar_plan');
            } else {
                $line[] = get_string('no');
            }
            ...
            if ($editingon) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/edit.php', array('id' => $priority->id)), new pix_icon('t/edit', $stredit));
                }

                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletepriorityscaleinuse', 'shezar_plan'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletepriorityscaleinuse', 'shezar_plan')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletepriorityscaleassigned', 'shezar_plan'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletepriorityscaleassigned', 'shezar_plan')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/index.php', array('delete' => $priority->id)), new pix_icon('t/delete', $strdelete));
                    }
                }
                // If value can be moved up
                if ($can_edit && $count > 1) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/index.php', array('moveup' => $priority->id)), new pix_icon('t/up', $str_moveup));
                } else {
                    $buttons[] = $spacer;
                }
                ...
                if ($can_edit && $count < $numvalues) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/index.php', array('movedown' => $priority->id)), new pix_icon('t/down', $str_movedown));
                } else {
                    $buttons[] = $spacer;
                }
    ...
    echo $OUTPUT->heading(get_string('priorityscales', 'shezar_plan'));

    if ($priorities) {
        echo html_writer::table($table);
    } else {
        echo html_writer::tag('p', get_string('noprioritiesdefined', 'shezar_plan'));
    }

    $button = $OUTPUT->single_button(new moodle_url("edit.php"), get_string('priorityscalecreate', 'shezar_plan'), 'get');
