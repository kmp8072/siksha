...
if ($delete) {
    if (!$value = $DB->get_record('dp_priority_scale_value', array('id' => $delete, 'priorityscaleid' => $priority->id))) {
        print_error('error:invalidpriorityscalevalueid', 'shezar_plan');
    }
    if ($scale_used) {
        print_error('error:nodeletepriorityscalevalueinuse', 'shezar_plan');
    }

    if ($value->id == $priority->defaultid) {
        print_error('error:nodeletepriorityscalevaluedefault', 'shezar_plan');
    }
    ...
    if ($confirm) {
        ...
        \shezar_plan\event\priority_scale_updated::create_from_scale($priority)->trigger();
        shezar_set_notification(get_string('deletedpriorityscalevalue', 'shezar_plan', format_string($value->name)), $CFG->wwwroot.'/shezar/plan/priorityscales/view.php?id='.$priority->id, array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id));
        $deleteurl = new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id, 'delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckpriorityvalue', 'shezar_plan');
...
if ((!empty($moveup) or !empty($movedown))) {
    ...
    if  ($scale_used) {
        $returnurl = new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id));
        print_error('error:noreorderpriorityinuse', 'shezar_plan', $returnurl);
    }
...
if ($default) {
    ...
    shezar_set_notification(get_string('priorityscaledefaultupdated', 'shezar_plan'), null, array('class' => 'notifysuccess'));
...
echo $OUTPUT->single_button(new moodle_url('/shezar/plan/priorityscales/index.php'), get_string('allpriorityscales', 'shezar_plan'), 'get');

// Display info about scale
echo $OUTPUT->heading(get_string('priorityscalex', 'shezar_plan', format_string($priority->name)));
$priority->description = file_rewrite_pluginfile_urls($priority->description, 'pluginfile.php', $context->id, 'shezar_plan', 'dp_priority_scale', $priority->id);
echo html_writer::tag('p', format_text($priority->description, FORMAT_HTML));

// Display warning if scale is in use
if ($scale_used) {
    echo $OUTPUT->container(get_string('priorityscaleinuse', 'shezar_plan'), 'notifymessage');
}

// Display priority scale values
if ($values) {
    echo html_writer::start_tag('form', array('id' => "dppriorityscaledefaultform", 'action' => new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $id)), 'method' => "POST"));
    ...
    $table->head[] = get_string('defaultvalue', 'shezar_plan').' '.
        $OUTPUT->help_icon('priorityscaledefault', 'shezar_plan', false);
    $table->align[] = 'center';
    ...
    foreach ($values as $value) {
        ...
        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/editvalue.php', array('id' => $value->id)), new pix_icon('t/edit', $str_edit));

        if (!$scale_used) {
            if ($value->id == $priority->defaultid) {
                $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletepriorityscalevaluedefault', 'shezar_plan'), 'shezar_core',
                    array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeletepriorityscalevaluedefault', 'shezar_plan')));
            } else {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id, 'delete' => $value->id)), new pix_icon('t/delete', $str_delete));
            }
        }

        // If value can be moved up
        if ($count > 1 && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id, 'moveup' => $value->id)), new pix_icon('t/up', $str_moveup));
        } else {
            $buttons[] = $spacer;
        }
        ...
        if ($count < $numvalues && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/priorityscales/view.php', array('id' => $priority->id, 'movedown' => $value->id)), new pix_icon('t/down', $str_movedown));
        } else {
            $buttons[] = $spacer;
        }
    ...
    echo html_writer::empty_tag('br') . $OUTPUT->container(get_string('nopriorityscalevalues', 'shezar_plan')) . html_writer::empty_tag('br');
}
...
if (!$scale_used) {
    ...
    $button = $OUTPUT->single_button(new moodle_url('/shezar/plan/priorityscales/editvalue.php', $options), get_string('addnewpriorityvalue', 'shezar_plan'), 'get');
}
