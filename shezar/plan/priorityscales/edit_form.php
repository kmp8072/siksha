...
class edit_priority_form extends moodleform {
    function definition() {
        ...
        $mform =& $this->_form;
        // visible elements
        $mform->addElement('header', 'general', get_string('priority', 'shezar_plan'));

        $mform->addElement('text', 'name', get_string('name'), 'size="40" maxlength="230"');
        $mform->addHelpButton('name', 'priorityscalename', 'shezar_plan');
        $mform->addRule('name', get_string('required'), 'required', null, 'client');
        $mform->setType('name', PARAM_TEXT);

        // If it's a new priority, give them the option to define priority values.
        if ($this->_customdata['priorityid'] == 0) {
            $mform->addElement('static', 'priorityvaluesexplain', '', get_string('explainpriorityscalevals', 'shezar_plan'));
            $mform->addElement('textarea', 'priorityvalues', get_string('priorityvalues', 'shezar_plan'), 'rows="5" cols="30"');
            $mform->addRule('priorityvalues', get_string('required'), 'required', null, 'server');
            $mform->addHelpButton('priorityvalues', 'priorityscalevalues', 'shezar_plan');
            ...
            $link = html_writer::link($linkurl, get_string('linktopriorityvalues', 'shezar_plan'));
