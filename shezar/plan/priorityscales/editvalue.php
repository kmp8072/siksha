...
$value = file_prepare_standard_editor($value, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                     'shezar_plan', 'dp_priority_scale_value', $value->id);
...
if ($valueform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/shezar/plan/priorityscales/view.php?id={$value->priorityscaleid}");
...
// Update data
    ...
    if ($valuenew->id == 0) {
        ...
        $valuenew = file_postupdate_standard_editor($valuenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_plan', 'dp_priority_scale_value', $valuenew->id);
        $DB->update_record('dp_priority_scale_value', $valuenew);

        $notification = get_string('priorityscalevalueadded', 'shezar_plan', format_string($valuenew->name));
        ...
        $notification = get_string('priorityscalevalueupdated', 'shezar_plan', format_string($valuenew->name));
    }

    \shezar_plan\event\priority_scale_updated::create_from_scale($scale)->trigger();

    shezar_set_notification($notification,
                            "$CFG->wwwroot/shezar/plan/priorityscales/view.php?id={$valuenew->priorityscaleid}",
...
if ($id == 0) {
    echo $OUTPUT->heading(get_string('addnewpriorityvalue', 'shezar_plan'));
} else {
    echo $OUTPUT->heading(get_string('editpriorityvalue', 'shezar_plan'));
}
