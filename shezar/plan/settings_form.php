...
function development_plan_build_settings_form(&$mform, $customdata) {
    ...
    //Settings
    $mform->addElement('header', 'plansettings', get_string('plansettings', 'shezar_plan'));
    $mform->addHelpButton('plansettings', 'advancedsettingsplansettings', 'shezar_plan', '', true);
    ...
    $plancompletiongroup[] =& $mform->createElement('advcheckbox', 'manualcomplete', null, get_string('manualcomplete', 'shezar_plan'));
    $plancompletiongroup[] =& $mform->createElement('advcheckbox', 'autobyitems', null, get_string('autobyitems', 'shezar_plan'));
    $plancompletiongroup[] =& $mform->createElement('advcheckbox', 'autobyplandate', null, get_string('autobyplandate', 'shezar_plan'));

    $mform->addGroup($plancompletiongroup, 'plancomplete', get_string('planmarkedcomplete', 'shezar_plan'), array(html_writer::empty_tag('br')), false);
    ...
    //Permissions
    $mform->addElement('header', 'planpermissions', get_string('planpermissions', 'shezar_plan'));
    $mform->addHelpButton('planpermissions', 'advancedsettingsplanpermissions', 'shezar_plan', '', true);
...
function development_plan_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/shezar/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'plan'));
        ...
        if ($currentworkflow != 'custom') {
            ...
            if ($template = $DB->get_record('dp_template', array('id' => $id))) {
                \shezar_plan\event\template_updated::create_from_template($template, 'plan')->trigger();
            }
        }
        $todb = new stdClass();
        $todb->templateid = $id;
    ...
    shezar_set_notification(get_string('update_plan_settings', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
}
