...
if ($delete) {
    if (!$value = $DB->get_record('dp_objective_scale_value', array('id' => $delete, 'objscaleid' => $objective->id))) {
        print_error('error:invalidobjectivescalevalueid', 'shezar_plan');
    }
    if ($scale_used) {
        print_error('error:nodeleteobjectivescalevalueinuse', 'shezar_plan');
    }

    if ($value->id == $objective->defaultid) {
        print_error('error:nodeleteobjectivescalevaluedefault', 'shezar_plan');
    }
    ...
    if ($confirm) {
        ...
        \shezar_plan\event\objective_scale_updated::create_from_scale($objective)->trigger();
        shezar_set_notification(get_string('deletedobjectivescalevalue', 'shezar_plan', format_string($objective->name)), $CFG->wwwroot.'/shezar/plan/objectivescales/view.php?id='.$objective->id, array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $objective->id));
        $deleteurl = new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $objective->id, 'delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckobjectivevalue', 'shezar_plan');
...
$str_changeto = get_string('changeto', 'shezar_plan');
$str_set = get_string('set', 'shezar_plan');
...
if ((!empty($moveup) or !empty($movedown))) {
    ...
    if ($scale_used) {
        $returnurl = "{$CFG->wwwroot}/shezar/plan/objectivescales/view.php?id={$objective->id}";
        print_error('error:noreorderobjectiveinuse', 'shezar_plan', $returnurl);
    }
...
if ($default) {
    ...
    shezar_set_notification(get_string('objectivescaledefaultupdated', 'shezar_plan'), null, array('class' => 'notifysuccess'));
...
echo $OUTPUT->single_button('/shezar/plan/objectivescales/index.php', get_string('allobjectivescales', 'shezar_plan'), 'get');

// Display info about scale
echo $OUTPUT->heading(get_string('objectivescalex', 'shezar_plan', format_string($objective->name)));
$objective->description = file_rewrite_pluginfile_urls($objective->description, 'pluginfile.php', $context->id, 'shezar_plan', 'dp_objective_scale', $objective->id);
...
if (isset($max_achieved) && isset($min_notachieved) && $max_achieved > $min_notachieved) {
    echo $OUTPUT->container(get_string('nonsensicalachievedvalues', 'shezar_plan'), 'notifyproblem');
}

// Display objective scale values
if ($values) {
    echo html_writer::start_tag('form', array('id' => "objscaleupdateform", 'action' => new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $id)), 'method' => "POST"));
    ...
    $table->head[] = get_string('defaultvalue', 'shezar_plan').' '.
        $OUTPUT->help_icon('objectivescaledefault', 'shezar_plan', false);
    $table->align[] = 'center';

    $table->head[] = get_string('achieved', 'shezar_plan').' '.
        $OUTPUT->help_icon('objectivescalevalueachieved', 'shezar_plan', false);
    $table->align[] = 'center';
    ...
    foreach ($values as $value) {
        ...
        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/editvalue.php', array('id' => $value->id)), new pix_icon('t/edit', $str_edit));

        if (!$scale_used) {
            if ($value->id == $objective->defaultid) {
                $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescalevaluedefault', 'shezar_plan'), 'shezar_core',
                    array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescalevaluedefault', 'shezar_plan')));
            } else {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $objective->id, 'delete' => $value->id)), new pix_icon('t/delete', $str_delete));
            }
        } else {
            $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescalevalueinuse', 'shezar_plan'), 'shezar_core',
                array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescalevalueinuse', 'shezar_plan')));
        }

        // If value can be moved up
        if ($count > 1 && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $objective->id, 'moveup' => $value->id)), new pix_icon('t/up', $str_moveup));
        } else {
            $buttons[] = $spacer;
        }
        ...
        if ($count < $numvalues && !$scale_used) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/view.php', array('id' => $objective->id, 'movedown' => $value->id)), new pix_icon('t/down', $str_movedown));
        } else {
            $buttons[] = $spacer;
        }
    ...
    echo html_writer::empty_tag('br') . $OUTPUT->container(get_string('noobjectivescalevalues', 'shezar_plan')) . html_writer::empty_tag('br');
...
if (!$scale_used) {
    ...
    $button = $OUTPUT->single_button(new moodle_url('/shezar/plan/objectivescales/editvalue.php', $options), get_string('addnewobjectivevalue', 'shezar_plan'), 'get');
}
