...
class edit_objective_form extends moodleform {
    function definition() {
        ...
        $mform =& $this->_form;
        // visible elements
        $mform->addElement('header', 'general', get_string('objective', 'shezar_plan'));

        $mform->addElement('text', 'name', get_string('name'), 'size="40" maxlength="230"');
        $mform->addHelpButton('name', 'objectivescalename', 'shezar_plan');
        $mform->addRule('name', get_string('required'), 'required', null, 'client');
        $mform->setType('name', PARAM_TEXT);

        // If it's a new objective, give them the option to define objective values.
        if ($this->_customdata['objectiveid'] == 0) {
            $mform->addElement('static', 'objectivevaluesexplain', '', get_string('explainobjscalevals', 'shezar_plan'));
            $mform->addElement('textarea', 'objectivevalues', get_string('objectivevalues', 'shezar_plan'), 'rows="5" cols="30"');
            $mform->addRule('objectivevalues', get_string('required'), 'required', null, 'server');
            $mform->addHelpButton('objectivevalues', 'objectivescalevalues', 'shezar_plan');
            ...
            $link = html_writer::link($linkurl, get_string('linktoobjectivevalues', 'shezar_plan'));
