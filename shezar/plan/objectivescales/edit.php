...
if ($id == 0) {
    ...
    if (!$objective = $DB->get_record('dp_objective_scale', array('id' => $id))) {
        print_error('error:objectivescaledidincorrect', 'shezar_plan');
    }
}
...
$objective = file_prepare_standard_editor($objective, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                         'shezar_plan', 'dp_objective_scale', $objective->id);
...
if ($mform->is_cancelled()) {
    ...
    redirect("$CFG->wwwroot/shezar/plan/objectivescales/index.php");
...
// Update data
    ...
    if (empty($objectivenew->id)) {
        ...
        $objectivenew = file_postupdate_standard_editor($objectivenew, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_plan', 'dp_objective_scale', $objectivenew->id);
        ...
        \shezar_plan\event\objective_scale_created::create_from_scale($objectivenew)->trigger();

        $notification = get_string('objectivescaleadded', 'shezar_plan', format_string($objectivenew->name));
        ...
        $objectivenew = $DB->get_record('dp_objective_scale', array('id' => $objectivenew->id));
        \shezar_plan\event\objective_scale_updated::create_from_scale($objectivenew)->trigger();

        $notification = get_string('objectivescaleupdated', 'shezar_plan', format_string($objectivenew->name));
    }

    shezar_set_notification($notification,
        "$CFG->wwwroot/shezar/plan/objectivescales/view.php?id={$objectivenew->id}",
        array('class' => 'notifysuccess'));
}

/// Print Page
$PAGE->navbar->add(get_string("objectivescales", 'shezar_plan'), new moodle_url('/shezar/plan/objectivescales/index.php'));
if ($id == 0) { // Add
    $PAGE->navbar->add(get_string('objectivesscalecreate', 'shezar_plan'));
    $heading = get_string('objectivesscalecreate', 'shezar_plan');
} else {    //Edit
    $PAGE->navbar->add(get_string('editobjective', 'shezar_plan', format_string($objective->name)));
    $heading = get_string('editobjective', 'shezar_plan', format_string($objective->name));
}
