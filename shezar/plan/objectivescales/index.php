...
if ($delete) {
    if (!$scale = $DB->get_record('dp_objective_scale', array('id' => $delete))) {
        print_error('error:invalidobjectivescaleid', 'shezar_plan');
    }
    if (dp_objective_scale_is_used($delete)) {
        print_error('error:nodeleteobjectivescaleinuse', 'shezar_plan');
    }
    if (dp_objective_scale_is_assigned($delete)) {
        print_error('error:nodeleteobjectivescaleassigned', 'shezar_plan');
    }
    ...
    if ($confirm) {
        ...
        \shezar_plan\event\objective_scale_deleted::create_from_scale($scale)->trigger();
        shezar_set_notification(get_string('deletedobjectivescale', 'shezar_plan', format_string($scale->name)), $CFG->wwwroot.'/shezar/plan/objectivescales/index.php', array('class' => 'notifysuccess'));

    } else {
        $returnurl = new moodle_url('/shezar/plan/objectivescales/index.php');
        $deleteurl = new moodle_url('/shezar/plan/objectivescales/index.php', array('delete' => $delete, 'confirm' => '1', 'sesskey' => sesskey()));

        echo $OUTPUT->header();
        $strdelete = get_string('deletecheckobjective', 'shezar_plan');
