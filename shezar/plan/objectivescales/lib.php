...
/**
 * shezar/plan/objectivescales/lib.php
 *
...
function dp_objective_display_table($objectives, $editingon=0) {
    ...
    // Cache permissions
    $can_edit = has_capability('shezar/plan:manageobjectivescales', $sitecontext);
    $can_delete = has_capability('shezar/plan:manageobjectivescales', $sitecontext);
    ...
    if ($objectives) {
        ...
        foreach ($objectives as $objective) {
            ...
            $title = $OUTPUT->action_link(new moodle_url("/shezar/plan/objectivescales/view.php", array('id' => $objective->id)), format_string($objective->name));
            if ($count == 1) {
            ...
            if ($editingon) {
                if ($can_edit) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/edit.php', array('id' => $objective->id)), new pix_icon('t/edit', $stredit));
                }

                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescaleinuse', 'shezar_plan'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescaleinuse', 'shezar_plan')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeleteobjectivescaleassigned', 'shezar_plan'), 'shezar_core',
                            array('class' => 'iconsmall action-icon', 'title' => get_string('error:nodeleteobjectivescaleassigned', 'shezar_plan')));
                    } else {
                        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/index.php', array('delete' => $objective->id)), new pix_icon('t/delete', $strdelete));
                    }
                }

                // If value can be moved up
                if ($can_edit && $count > 1) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/index.php', array('moveup' => $objective->id)), new pix_icon('t/up', $str_moveup));
                } else {
                    $buttons[] = $spacer;
                }
                ...
                if ($can_edit && $count < $numvalues) {
                    $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/objectivescales/index.php', array('movedown' => $objective->id)), new pix_icon('t/down', $str_movedown));
                } else {
                    $buttons[] = $spacer;
                }
    ...
    echo $OUTPUT->heading(get_string('objectivescales', 'shezar_plan'));

    if ($objectives) {
        echo html_writer::table($table);
    } else {
        echo html_writer::tag('p', get_string('noobjectivesdefined', 'shezar_plan'));
    }

    $button = $OUTPUT->single_button(new moodle_url("edit.php"), get_string('objectivesscalecreate', 'shezar_plan'), 'get');
