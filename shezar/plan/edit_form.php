...
class plan_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform =& $this->_form;
        $mform->addElement('header', 'details', get_string('plandetails', 'shezar_plan'));
        ...
        $canselectplan = has_capability('shezar/plan:canselectplantemplate', context_system::instance());
        ...
        if ($action == 'delete') {
            ...
            $mform->addElement('html', get_string('checkplandelete', 'shezar_plan', $plan->name));
            $buttonarray = array();
        ...
        if ($action == 'complete') {
            ...
            $mform->addElement('html', get_string('checkplancomplete11', 'shezar_plan', $plan->name));
            $buttonarray = array();
            $buttonarray[] = $mform->createElement('submit', 'completeyes', get_string('completeplan', 'shezar_plan'));
        ...
        if ($action == 'add') {
            if ($canselectplan) {
                $mform->addElement('select', 'templateid', get_string('plantemplate', 'shezar_plan'), $template_options);
        ...
        $mform->addElement('text', 'name', get_string('planname', 'shezar_plan'), array('size' => 50));
        ...
        if ($action == 'view') {
            $plan->description = file_rewrite_pluginfile_urls($plan->description, 'pluginfile.php', context_system::instance()->id, 'shezar_plan', 'dp_plan', $plan->id);
            $mform->addElement('static', 'description', get_string('plandescription', 'shezar_plan'), format_text($plan->description, FORMAT_HTML));
        } else {
            $mform->addElement('editor', 'description_editor', get_string('plandescription', 'shezar_plan'), null, $TEXTAREA_OPTIONS);
            $mform->setType('description_editor', PARAM_CLEANHTML);
        }

        $mform->addElement('date_selector', 'startdate', get_string('datestarted', 'shezar_plan'));
        $mform->addRule('startdate', get_string('err_required', 'form'), 'required', '', 'client', false, false);
        if ($action == 'add') {
            $mform->setDefault('startdate', 0);
        }

        $mform->addElement('date_selector', 'enddate', get_string('completiondate', 'shezar_plan'));
        ...
        if ($action == 'view') {
            ...
            if ($can_manage) {
                ...
                if ($plan->get_setting('update') == DP_PERMISSION_ALLOW && $plan->status != DP_PLAN_STATUS_COMPLETE) {
                    $buttonarray[] = $mform->createElement('submit', 'edit', get_string('editdetails', 'shezar_plan'));
                }
                if ($plan->get_setting('delete') == DP_PERMISSION_ALLOW) {
                    $buttonarray[] = $mform->createElement('submit', 'delete', get_string('deleteplan', 'shezar_plan'));
                }
                if ($plan->get_setting('completereactivate') >= DP_PERMISSION_ALLOW && $plan->status == DP_PLAN_STATUS_APPROVED) {
                    $buttonarray[] = $mform->createElement('submit', 'complete', get_string('plancomplete', 'shezar_plan'));
                }
            ...
            $this->add_action_buttons(true, get_string($actionstr, 'shezar_plan'));
        }
    }
    ...
    function validation($data, $files) {
        ...
        if (in_array($action, array('add', 'edit'))) {
            ...
            if ($startdate > $enddate) {
                ...
                $errstr = get_string('error:creationaftercompletion', 'shezar_plan');
