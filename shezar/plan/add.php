...
require_once($CFG->dirroot . '/shezar/plan/edit_form.php');
...
$PAGE->set_url('/shezar/plan/add.php', array('userid' => $userid));
...
$allplansurl = "{$CFG->wwwroot}/shezar/plan/index.php?userid={$userid}";
...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        ...
        $viewurl = "{$CFG->wwwroot}/shezar/plan/view.php?id={$newid}";
        \shezar_plan\event\plan_created::create_from_plan($plan)->trigger();

        // Free memory
        unset($plan);

        shezar_set_notification(get_string('plancreatesuccess', 'shezar_plan'), $viewurl, array('class' => 'notifysuccess'));
    }
}
...
/// Display
///
$heading = get_string('createnewlearningplan', 'shezar_plan');
$pagetitle = format_string(get_string('learningplan', 'shezar_plan').': '.$heading);
...
$PAGE->requires->js_call_amd('shezar_plan/templates', 'init', $args);
...
echo html_writer::tag('p', get_string('createplan_instructions', 'shezar_plan'));
