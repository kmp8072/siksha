...
/**
* Standard HTML output renderer for shezar_plan module
*/
class shezar_plan_renderer extends plugin_renderer_base {
     ...
     */
    public function shezar_print_approval_form($requested_items, $require_approval) {
        $output = '';
        ...
        foreach ($require_approval as $componentname => $component) {
            ...
            $output .= $this->output->heading(get_string($component->component.'plural', 'shezar_plan'));

            $output .= $component->display_approval_list($requested_items[$componentname]);
        }

        $output .= html_writer::empty_tag('br');
        $submit_attributes = array('type' => 'submit', 'name' => 'submitbutton', 'value' => get_string('updatesettings', 'shezar_plan'));
    ...
    */
    function print_add_plan_button($userid) {
        $action = new moodle_url('/shezar/plan/add.php', array('userid' => $userid));
        $title = get_string('createnewlearningplan', 'shezar_plan');
