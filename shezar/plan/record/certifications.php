...
if (!empty($certifid) && (!$certification = $DB->get_record('prog', array('certifid' => $certifid), 'fullname'))) {
    print_error(get_string('error:incorrectcertifid', 'shezar_certification', null, $certifid));
}
...
$PAGE->set_url(new moodle_url('/shezar/plan/record/certifications.php', $pageparams));
...
$strsubheading = get_string($rolstatus.'certificationssubhead', 'shezar_plan');
...
if ($history) {
    ...
    if (!empty($certifid)) {
        ...
        $strsubheading = get_string('certificationshistoryforsubhead', 'shezar_plan', $certification->fullname);
    } else {
        $strsubheading = get_string('certificationshistorysubhead', 'shezar_plan');
    }
}
// Set report.
