...
if (!empty($programid) && (!$program = $DB->get_record('prog', array('id' => $programid), 'fullname'))) {
    print_error(get_string('programnotfound', 'shezar_plan'));
}
...
$PAGE->set_url(new moodle_url('/shezar/plan/record/programs.php', $pageparams));
...
$strsubheading = get_string($rolstatus.'programssubhead', 'shezar_plan');
...
if ($history) {
    ...
    if (!empty($programid)) {
        ...
        $strsubheading = get_string('programscompletionhistoryforsubhead', 'shezar_plan', $program->fullname);
    } else {
        $strsubheading = get_string('programscompletionhistorysubhead', 'shezar_plan');
    }
}
// Set report.
