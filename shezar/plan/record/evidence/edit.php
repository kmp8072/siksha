...
$PAGE->set_url('/shezar/plan/record/evidence/edit.php',
...
$canaccess = has_capability('shezar/plan:accessanyplan', $systemcontext);
$canedit = has_capability('shezar/plan:editsiteevidence', $systemcontext);
...
if (!empty($evidenceid)) {
    ...
    if (!$item = $DB->get_record('dp_plan_evidence', array('id' => $evidenceid))) {
        ...
        if ($item->readonly && !($canaccess || $canedit)) {
            print_error('evidence_readonly', 'shezar_plan');
        }
...
if ($USER->id != $userid && !(\shezar_job\job_assignment::is_managing($USER->id, $userid)) && !($canaccess || $canedit)) {
...
if (!empty($evidenceid) || $deleteflag) {
    ...
    $itemurl = new moodle_url('/shezar/plan/record/evidence/view.php', array('id' => $evidenceid));
...
if ($deleteflag && $deleteconfirmed) {
    ...
    */
    ...
    shezar_set_notification(get_string('evidencedeleted', 'shezar_plan'),
...
if ($data = $mform->get_data()) {
    ...
    if (empty($data->id)) {
        ...
        \shezar_plan\event\evidence_created::create_from_instance($item)->trigger();

        shezar_set_notification(get_string('evidenceadded', 'shezar_plan'), $itemurl, array('class' => 'notifysuccess'));
        ...
        $item = $DB->get_record('dp_plan_evidence', array('id' => $data->id), '*', MUST_EXIST);
        \shezar_plan\event\evidence_updated::create_from_instance($item)->trigger();

        shezar_set_notification(get_string('evidenceupdated', 'shezar_plan'), $itemurl, array('class' => 'notifysuccess'));
    }
...
$PAGE->navbar->add(get_string($action . 'evidence', 'shezar_plan'));
...
switch($action){
    case 'add':
    case 'edit':
        echo $OUTPUT->heading(get_string($action . 'evidence', 'shezar_plan'));
        $mform->display();
        break;
    ...
    case 'delete':
        ...
        $deleteurl = new moodle_url('/shezar/plan/record/evidence/edit.php', $params);
        echo list_evidence_in_use($item->id);
        echo $OUTPUT->confirm(get_string('deleteevidenceareyousure', 'shezar_plan'), $deleteurl, $indexurl);
        break;
}
