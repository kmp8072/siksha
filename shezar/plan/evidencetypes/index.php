...
if (!empty($deleteid)) {
    $indexurl = new moodle_url('/shezar/plan/evidencetypes/index.php');

    if (!$item = $DB->get_record('dp_evidence_type', array('id' => $deleteid))) {
        print_error('error:invalidevidencetypeid', 'shezar_plan');
    }
    ...
    if ($confirm) {
        ...
        \shezar_plan\event\evidence_type_deleted::create_from_type($item)->trigger();

        shezar_set_notification(get_string('deletedevidencetype', 'shezar_plan',
                format_string($item->name)),
                $indexurl,
                array('class' => 'notifysuccess'));
    } else {
        $deletemsg = get_string('deletecheckevidencetype', 'shezar_plan');
        if (dp_evidence_type_is_used($deleteid)) {
            $deletemsg .= '<br/><br/>' . get_string('deletecheckevidencetypeinuse', 'shezar_plan');
        }
        $deleteurl = new moodle_url('/shezar/plan/evidencetypes/index.php',
            ...
            if ($swapsortorder = $DB->get_field_sql($sqlminmax, array($thissortorder))) {
                ...
                if (!empty($swapid)) {
                    ...
                    if (!($DB->set_field('dp_evidence_type', 'sortorder', $thissortorder, array('id' => $swapid))
                        ...
                        print_error('error:updateevidencetypeordering', 'shezar_plan');
                    }
                    $transaction->allow_commit();
                }
            }
    ...
    $table->head  = array(
        get_string('evidencetypename', 'shezar_plan'),
        get_string('used'),
        get_string('options', 'shezar_core'));
    ...
    foreach ($items as $item) {
        ...
        $line[] = $OUTPUT->action_link(
                new moodle_url("/shezar/plan/evidencetypes/view.php", array('id' => $item->id)),
                format_string($item->name));
        ...
        $buttons[] = $OUTPUT->action_icon(
                new moodle_url("/shezar/plan/evidencetypes/edit.php", array('id' => $item->id)),
                new pix_icon('t/edit', get_string('edit')));

        // Delete
        $buttons[] = $OUTPUT->action_icon(
                new moodle_url("/shezar/plan/evidencetypes/index.php", array('delete' => $item->id)),
                new pix_icon('t/delete', get_string('delete')));

        // If value can be moved up
        if ($count > 1) {
            $buttons[] = $OUTPUT->action_icon(
                    new moodle_url("/shezar/plan/evidencetypes/index.php", array('moveup' => $item->id)),
        ...
        if ($count < $numvalues) {
            $buttons[] = $OUTPUT->action_icon(
                    new moodle_url("/shezar/plan/evidencetypes/index.php", array('movedown' => $item->id)),
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('evidencetypes', 'shezar_plan'));

    if ($items) {
        echo html_writer::table($table);
    } else {
        echo html_writer::tag('p', get_string('noevidencetypesdefined', 'shezar_plan'));
    }

    // Add type
    echo $OUTPUT->single_button(
            new moodle_url("/shezar/plan/evidencetypes/edit.php"),
            get_string('evidencetypecreate', 'shezar_plan'), 'get');
