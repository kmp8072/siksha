...
abstract class dp_base_workflow {
    ...
    function __construct() {
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property)) {
                ...
                throw new Exception(get_string('error:propertymustbeset', 'shezar_plan', $msg));
            }
        }
        // reserve the name 'custom' for use by the system
        if ($this->classname == 'custom') {
            throw new Exception(get_string('error:cantcreatecustomworkflow', 'shezar_plan'));
        }

        // get name and description lang string based on name
        $this->name = get_string($this->classname . 'workflowname', 'shezar_plan');
        $this->description = get_string($this->classname . 'workflowdesc', 'shezar_plan');
    }
    ...
    function copy_to_db($templateid) {
        ...
        $returnurl = $CFG->wwwroot . '/shezar/plan/template/workflow?id=' . $templateid;

        if (!$templateid) {
            print_error('error:templateid', 'shezar_plan');
        }
