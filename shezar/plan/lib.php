...
require_once($CFG->dirroot . '/shezar/plan/role.class.php');
require_once($CFG->dirroot . '/shezar/plan/component.class.php');
require_once($CFG->dirroot . '/shezar/plan/workflow.class.php');
require_once($CFG->dirroot . '/shezar/program/lib.php'); // needed to display required learning in plans menu
...
// shezar/plan/roles/[ROLE]/[ROLE].class.php
...
*/
function shezar_plan_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    global $DB, $USER;
    ...
    if ($USER->id != $evidence->userid && !(\shezar_job\job_assignment::is_managing($USER->id, $userid)) && !has_capability('shezar/plan:accessanyplan', $context)) {
    ...
    $fullpath = "/{$context->id}/shezar_plan/$filearea/$evidenceid/$relativepath";
...
function dp_can_view_users_plans($ownerid) {
    ...
    if (has_capability('shezar/plan:accessanyplan', $systemcontext) ||
        has_capability('shezar/plan:manageanyplan', $systemcontext)) {
    ...
    if (\shezar_job\job_assignment::is_managing($USER->id, $ownerid)) {
...
function dp_can_manage_users_plans($ownerid) {
    ...
    if (has_capability('shezar/plan:manageanyplan', $systemcontext)) {
        // The current user has manager's capability (whether or not they are the actual manager, includes site admin).
        return true;
    }
...
function dp_get_alert_receivers($contextuser, $type) {
    ...
    $users = get_users_by_capability($contextuser, "shezar/plan:receive{$type}alerts");
...
function dp_add_permissions_select(&$form, $name, $requestable) {
    ...
    $select_options[DP_PERMISSION_ALLOW] = get_string('allow', 'shezar_plan');
    $select_options[DP_PERMISSION_DENY] = get_string('deny', 'shezar_plan');

    if ($requestable) {
        $select_options[DP_PERMISSION_REQUEST] = get_string('request', 'shezar_plan');
        $select_options[DP_PERMISSION_APPROVE] = get_string('approve', 'shezar_plan');
    }
...
function dp_add_permissions_table_headings(&$form) {
    ...
    $out .= html_writer::start_tag('tr') . html_writer::tag('th', get_string('action', 'shezar_plan'));

    foreach ($DP_AVAILABLE_ROLES as $role) {
        $out .= html_writer::tag('th', get_string($role, 'shezar_plan'));
    }
...
function dp_get_rol_tabs_visible($userid) {
    ...
    if (shezar_feature_visible('competencies') && ($assigned_comps > 0 || $show_competency_tab)) {
    ...
    if (($programscount > 0 || $show_program_tab) && shezar_feature_visible('programs')) {
        $visible[] = 'programs';
    }
    ...
    if (($certification_progs > 0 || $unassignedcertifications) && shezar_feature_visible('certifications')) {
    ...
    if ($visible = dp_get_rol_tabs_visible($userid)) {
        foreach ($visible as $element) {
            if ($element !== 'evidence') {
                $row[] = new tabobject(
                        $element,
                        new moodle_url("/shezar/plan/record/{$element}.php", $params),
                        get_string($rolstatus . $element, 'shezar_plan')
                );
            } else {
                $row[] = new tabobject(
                        'evidence',
                        new moodle_url('/shezar/plan/record/evidence/index.php', $params),
                        get_string($rolstatus . 'evidence', 'shezar_plan')
                );
            }
        }
    }
...
function dp_print_workflow_diff($diff_array) {
    global $OUTPUT;
    $columns[] = 'component';
    $headers[] = get_string('component', 'shezar_plan');
    $columns[] = 'setting';
    $headers[] = get_string('setting', 'shezar_plan');
    $columns[] = 'role';
    $headers[] = get_string('role', 'shezar_plan');
    $columns[] = 'before';
    $headers[] = get_string('before', 'shezar_plan');
    $columns[] = 'after';
    $headers[] = get_string('after', 'shezar_plan');

    $baseurl = new moodle_url( '/shezar/plan/template/workflow.php');
    $table = new flexible_table('Templates');
    $table->define_columns($columns);
    $table->define_headers($headers);
    $table->define_baseurl($baseurl);
    $return = html_writer::start_tag('p');
    $return .= $OUTPUT->heading(get_string('changes', 'shezar_plan'), 3);

    $table->setup();

    $permission_options = array(DP_PERMISSION_ALLOW => get_string('allow', 'shezar_plan'),
        DP_PERMISSION_DENY => get_string('deny', 'shezar_plan'),
        DP_PERMISSION_REQUEST => get_string('request', 'shezar_plan'),
        DP_PERMISSION_APPROVE => get_string('approve', 'shezar_plan')
    );

    $duedate_options = array(DP_DUEDATES_NONE => get_string('none'),
        DP_DUEDATES_OPTIONAL => get_string('optional', 'shezar_plan'),
        DP_DUEDATES_REQUIRED => get_string('required', 'shezar_plan')
    );

    $priority_options = array(DP_PRIORITY_NONE => get_string('none'),
        DP_PRIORITY_OPTIONAL => get_string('optional', 'shezar_plan'),
        DP_PRIORITY_REQUIRED => get_string('required', 'shezar_plan')
    );
    ...
    foreach ($diff_array as $item => $values) {
        ...
        if ($parts[0] == 'perm') {
            if ($parts[1] != 'plan') {
                ...
                $compname = $configsetting ? $configsetting : get_string($parts[1], 'shezar_plan');
                $tablerow[] = $compname;
            } else {
                $tablerow[] = get_string($parts[1], 'shezar_plan');
            }
            $tablerow[] = get_string($parts[2], 'shezar_plan');
            $tablerow[] = get_string($parts[3], 'shezar_plan');
            ...
            $tablerow[] = get_string($parts[2], 'shezar_plan');
            $tablerow[] = get_string('na', 'shezar_plan');
            switch($parts[2]) {
                case 'duedatemode':
...
function dp_display_plans($userid, $statuses=array(DP_PLAN_STATUSAPPROVED), $cols=array('enddate', 'status', 'completed'), $firstcolheader='') {
    ...
    if (empty($firstcolheader)) {
        $tableheaders[] = get_string('plan', 'shezar_plan');
    } else {
        $tableheaders[] = $firstcolheader;
    }

    if (in_array('enddate', $cols)) {
        $tableheaders[] = get_string('duedate', 'shezar_plan');
        $tablecols[] = 'enddate_'.$statuses_undrsc;
    }
    if (in_array('status', $cols)) {
        $tableheaders[] = get_string('status', 'shezar_plan');
        $tablecols[] = 'status_'.$statuses_undrsc;
    }
    if (in_array('completed', $cols)) {
        $tableheaders[] = get_string('completed', 'shezar_plan');
        $tablecols[] = 'timemodified_'.$statuses_undrsc;
    }

    // Actions
    if ($can_manage && $can_update) {
        $tableheaders[] = get_string('actions', 'shezar_plan');
        $tablecols[] = 'actioncontrols';
    }

    $baseurl = $CFG->wwwroot . '/shezar/plan/index.php';
    if ($userid != $USER->id) {
...
function dp_display_plans_menu($userid, $selectedid=0, $role='learner', $rolpage='courses', $rolstatus='none', $showrol=true, $selectedprogid=0, $showrequired=true) {
    ...
    $list = array();
    $attr = array();
    $enableplans = shezar_feature_visible('learningplans');
    ...
    if ($role == 'manager') {
        if (shezar_feature_visible('myteam')) {
            ...
            $out .= $OUTPUT->heading(get_string('teammembers', 'shezar_plan'), 3, 'main');
            $out .= html_writer::alist(array($OUTPUT->action_link(new moodle_url('/my/teammembers.php'), get_string('allteammembers', 'shezar_plan'))), array('class' => $class));
        }
        if ($userid) {
            if ($enableplans && !shezar_feature_disabled('learningplans')) {
                ...
                $out .= $OUTPUT->heading(get_string('learningplans', 'shezar_plan'), 4, 'dp-plans-menu-sub-header');
                $out .= html_writer::alist(array($OUTPUT->action_link(new moodle_url('/shezar/plan/index.php', array('userid' => $userid)), get_string('manageplans', 'shezar_plan'))), array('class' => $class));
            }
        }
    } else {
        if ($enableplans && !shezar_feature_disabled('learningplans')) {
            $out .= $OUTPUT->heading(get_string('learningplans', 'shezar_plan'), 3, 'main');
            $out .= html_writer::alist(array($OUTPUT->action_link(new moodle_url('/shezar/plan/index.php'), get_string('manageplans', 'shezar_plan'))), array('class' => $class));
        }
    }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, array(DP_PLAN_STATUS_APPROVED))) {
        if ($role == 'manager') {
            ...
            $out .= $OUTPUT->heading(get_string('activeplans', 'shezar_plan'), 5);
        } else {
            $out .= $OUTPUT->heading(get_string('activeplans', 'shezar_plan'), 4, 'dp-plans-menu-sub-header');
        }

        $list = array();
        foreach ($plans as $p) {
            $attr['class'] = $p->id == $selectedid ? 'dp-menu-selected' : '';
            $list[] = $OUTPUT->action_link(new moodle_url('/shezar/plan/view.php', array('id' => $p->id)), $p->name);
        }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, array(DP_PLAN_STATUS_UNAPPROVED, DP_PLAN_STATUS_PENDING))) {
        if ($role == 'manager') {
            $out .= $OUTPUT->container_start(null, 'dp-plans-menu-section');
            $out .= $OUTPUT->heading(get_string('unapprovedplans', 'shezar_plan'), 5);
        } else {
            $out .= $OUTPUT->heading(get_string('unapprovedplans', 'shezar_plan'), 4, 'dp-plans-menu-sub-header');
        }
    ...
    if ($enableplans && $plans = dp_get_plans($userid, DP_PLAN_STATUS_COMPLETE)) {
        if ($role == 'manager') {
            $out .= $OUTPUT->container_start(null, 'dp-plans-menu-section');
            $out .= $OUTPUT->heading(get_string('completedplans', 'shezar_plan'), 5);
        } else {
            $out .= $OUTPUT->heading(get_string('completedplans', 'shezar_plan'), 4, 'dp-plans-menu-sub-header');
        }
    ...
    if ($showrequired) {
        ...
        if ($programs || $certifications) {
            $canviewprograms = shezar_feature_visible('programs');
            $canviewcertifications = shezar_feature_visible('certifications');
            $extraparams = array();
            $headingclass = 'main';
            if ($role == 'manager') {
                $extraparams['userid'] = $userid;
                $out .= $OUTPUT->container_start(null, 'dp-plans-menu-section');
                $out .= $OUTPUT->heading(get_string('requiredlearning', 'shezar_program'), 4, 'dp-plans-menu-sub-header');
            } else {
                $out .= $OUTPUT->heading(get_string('requiredlearning', 'shezar_program'), 3, 'main');
            }

            if ($programs && $canviewprograms) {
                $list = dp_display_plans_menu_required($programs, $extraparams);
                if ($role == 'manager') {
                    $out .= $OUTPUT->heading(get_string('programs', 'shezar_program'), 5);
                } else {
                    $out .= $OUTPUT->heading(get_string('programs', 'shezar_program'), 4, 'dp-plans-menu-sub-header');
                }
                $out .= html_writer::alist($list, $attr);
            }
            if ($certifications && $canviewcertifications) {
                ...
                if ($role == 'manager') {
                    $out .= $OUTPUT->heading(get_string('certifications', 'shezar_program'), 5);
                } else {
                    $out .= $OUTPUT->heading(get_string('certifications', 'shezar_program'), 4, 'dp-plans-menu-sub-header');
                }
                $out .= html_writer::alist($list, $attr);
            }
            if ($role == 'manager') {
    ...
    $fake_block->title = get_string('learningplans', 'shezar_plan');;
...
function dp_display_plans_menu_required($programs, $extraparams, $progcount=0) {
    ...
    foreach ($programs as $p) {
        ...
        if (count($list) + $progcount >= DP_MAX_PROGS_TO_DISPLAY) {
            $list[] = $OUTPUT->action_link(new moodle_url('/shezar/program/required.php', $urlparams), get_string('viewallrequiredlearning', 'shezar_program'));
            break;
        }
        ...
        $urlparams['id'] = $p->id;
        $list[] = $OUTPUT->action_link(new moodle_url('/shezar/program/required.php', $urlparams), $p->fullname);
    }
    return($list);
}
...
function dp_display_user_message_box($planuser) {
    ...
    return html_writer::tag('div', $OUTPUT->user_picture($user) . get_string('youareviewingxsplans', 'shezar_plan', $a), array('class' => "plan_box notifymessage alert alert-info"));
}
...
function dp_record_status_picker($pagename, $status, $userid=null) {
    ...
    foreach (array('all','active','completed') as $s) {
        ...
        $options[$s] = get_string($s . 'learning', 'shezar_plan');
    }

    $label = html_writer::tag('strong', get_string('filterbystatus', 'shezar_plan')) . '&nbsp;';

    // display status pulldown
    $form = $OUTPUT->single_select(
        new moodle_url("/shezar/plan/record/{$pagename}.php", array('userid' => $userid, 'status' => '')),
        'status',
        $options,
        $selected
    );
...
function dp_record_status_menu($pagename, $status, $userid=null, $role) {
    ...
    if ($role == 'manager') {
        $out = $OUTPUT->heading(get_string('recordoflearning', 'shezar_core'), 4, 'dp-plans-menu-sub-header');
    } else {
        $out = $OUTPUT->heading(get_string('recordoflearning', 'shezar_core'), 3, 'main');
    }
    ...
    $items = array();
    foreach (array('all','active','completed') as $s) {
        $filter[$s] = get_string($s . 'learning', 'shezar_plan');
        $class = $status == $s ? "dp-menu-selected" : '';
        $items[] = $OUTPUT->action_link(new moodle_url("/shezar/plan/record/{$pagename}.php", array('userid' => $userid, 'status' => $s)), $filter[$s], null, array('class' => $class));
    }
...
function dp_get_plan_base_navlinks($userid) {
    ...
    if ($userid == $USER->id) {
        $PAGE->navbar->add(get_string('learningplans', 'shezar_plan'), new moodle_url('/shezar/plan/index.php'));
        return true;
    }
    ...
    if ($user) {
        ...
        $PAGE->navbar->add(get_string('xslearningplans', 'shezar_plan', fullname($user)), new moodle_url('/shezar/plan/index.php', array('userid' => $userid)));
    } else {
        $PAGE->navbar->add(get_string('unknownuserslearningplans', 'shezar_plan'), new moodle_url('/shezar/plan/index.php', array('userid' => $userid)));
    }
}
...
function dp_get_approval_status_from_code($code) {
    switch ($code) {
        case DP_APPROVAL_DECLINED:
            $status = get_string('declined', 'shezar_plan');
            break;
        case DP_APPROVAL_UNAPPROVED:
            $status = get_string('unapproved', 'shezar_plan');
            break;
        case DP_APPROVAL_REQUESTED:
            $status = get_string('pendingapproval', 'shezar_plan');
            break;
        case DP_APPROVAL_APPROVED:
            $status = get_string('approved', 'shezar_plan');
            break;
        default:
            $status = get_string('unknown', 'shezar_plan');
            break;
    }
...
function dp_create_template($templatename, $enddate, &$error) {
    ...
    foreach ($DP_AVAILABLE_COMPONENTS as $component) {
        $classfile = $CFG->dirroot .
            "/shezar/plan/components/{$component}/{$component}.class.php";
        if (!is_readable($classfile)) {
            $string_properties = new stdClass();
            $string_properties->classfile = $classfile;
            $string_properties->component = $component;
            $error = get_string('noclassfileforcomponent', 'shezar_plan', $string_properties);
            return false;
        }
        ...
        if (!class_exists($class)) {
            ...
            $string_properties->component = $component;
            $error = get_string('noclassforcomponent', 'shezar_plan', $string_properties);
            return false;
        }
        $cn = new stdClass();
    ...
    $classfile = $CFG->dirroot . "/shezar/plan/workflows/{$workflow}/{$workflow}.class.php";
    if (!is_readable($classfile)) {
        $string_properties = new stdClass();
        $string_properties->classfile = $classfile;
        $string_properties->workflow = $workflow;
        $error = get_string('noclassfileforworkflow', 'shezar_plan', $string_properties);
        return false;
    }
    include_once($classfile);
    // check class exists
    ...
    if (!class_exists($class)) {
        ...
        $string_properties->workflow = $workflow;
        $error = get_string('noclassforworkflow', 'shezar_plan', $string_properties);
        return false;
    }
    ...
    if (!$workflow_class->copy_to_db($newtemplateid)) {
        $error = get_string('error:newdptemplate', 'shezar_plan');
        return false;
    }
...
function dp_plan_item_updated($userid, $component, $componentid) {
    ...
    $component_include = $CFG->dirroot . '/shezar/plan/components/' . $component . '/' . $component . '.class.php';
...
function shezar_plan_comment_permissions($details) {
    global $DB;
    ...
    $plan = new development_plan($planid);
    if (!has_capability('shezar/plan:accessanyplan', $details->context) && ($plan->get_setting('view') < DP_PERMISSION_ALLOW)) {
...
function shezar_plan_comment_template() {
    global $OUTPUT, $PAGE;

    // Use the shezar default comment template
 ...
 * @return boolean
 */
function shezar_plan_comment_validate($comment_param) {
    global $DB;
    // comment itemid and comment area already validated as part of permission check (shezar_plan_comment_permissions)
...
function shezar_plan_comment_add($comment) {
    global $CFG, $DB, $USER;
    ...
    switch ($comment->commentarea) {
        case 'plan_overview':
            ...
            $contexturl = new moodle_url('/shezar/plan/view.php', array('id' => $plan->id.'#comments'));
        ...
        case 'plan_course_item':
            ...
            if (!$record = $DB->get_record_sql($sql, $params)) {
                print_error('commenterror:itemnotfound', 'shezar_plan');
            }
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('course', 'shezar_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/shezar/plan/components/course/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
        ...
        case 'plan_competency_item':
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('competency', 'shezar_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/shezar/plan/components/competency/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
        ...
        case 'plan_objective_item':
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('objective', 'shezar_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);
            $contexturl = new moodle_url('/shezar/plan/components/objective/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
        ...
        case 'plan_program_item':
            ...
            $params = array($comment->itemid);
            if (!$record = $DB->get_record_sql($sql, $params)) {
                print_error('comment_error:itemnotfound', 'shezar_plan');
            }
            $plan = new development_plan($record->planid);
            ...
            $msgobj->plan = $plan->name;
            $msgobj->planowner = fullname($DB->get_record('user', array('id' => $plan->userid)));
            $msgobj->component = get_string('program', 'shezar_plan');
            $msgobj->componentname = $record->fullname;
            $msgobj->comment = format_text($comment->content);
            $msgobj->commentby = fullname($commentuser);
            $msgobj->commentdate = userdate($comment->timecreated);

            $contexturl = new moodle_url('/shezar/plan/components/program/view.php', array('id' => $plan->id, 'itemid' => $comment->itemid.'#comments'));
            $contexturlname = $record->fullname;
            $icon = 'program-newcomment';

            break;
        default:
            print_error('commenterror:unsupportedcomment', 'shezar_plan');
            break;
    }
    ...
    /// Send message
    require_once($CFG->dirroot . '/shezar/message/eventdata.class.php');
    ...
    foreach ($subscribers as $sid) {
        ...
        if ($comment->commentarea == 'plan_overview') {
            $subject = $stringmanager->get_string('commentmsg:planoverview', 'shezar_plan', $msgobj, $userto->lang);
            $fullmsg = $stringmanager->get_string('commentmsg:planoverviewdetail', 'shezar_plan', $msgobj, $userto->lang);
        } else {
            $subject = $stringmanager->get_string('commentmsg:componentitem', 'shezar_plan', $msgobj, $userto->lang);
            $fullmsg = $stringmanager->get_string('commentmsg:componentitemdetail', 'shezar_plan', $msgobj, $userto->lang);
        }
...
function plan_mark_competency_default($competencyid, $userid, $component) {
    ...
    $details->assessmenttype = get_string('automateddefault', 'shezar_plan');
...
function check_learningplan_enabled() {
    if (shezar_feature_disabled('learningplans')) {
        print_error('learningplansdisabled', 'shezar_plan');
    }
}

/**
 * Display shezar My Learning links in the user profile.
 *
 ...
 */
function shezar_plan_myprofile_navigation(\core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    global $USER;
    ...
    $category = new core_user\output\myprofile\category('mylearning', get_string($currentuser ? 'mylearning' : 'learning' , 'shezar_core'), 'contact');
    $tree->add_category($category);

    // Record of learning.
    if (shezar_feature_visible('recordoflearning')) {
        if ($currentuser || \shezar_job\job_assignment::is_managing($USER->id, $user->id) || has_capability('shezar/core:viewrecordoflearning', $usercontext)) {
            $title = get_string('recordoflearning', 'shezar_core');
            $url = new moodle_url('/shezar/plan/record/index.php', array('userid' => $user->id));
    ...
    // Learning plans.
    if (shezar_feature_visible('learningplans') && dp_can_view_users_plans($user->id)) {
        $title = get_string('learningplans', 'shezar_plan');
        $url = new moodle_url('/shezar/plan/index.php', array('userid' => $user->id));
        $content =  html_writer::link($url, $title);
