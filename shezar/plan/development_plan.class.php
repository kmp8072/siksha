...
class development_plan {
    ...
    function __construct($id, $viewas=null) {
        ...
        if (!$plan) {
            throw new PlanException(get_string('planidnotfound', 'shezar_plan', $id));
        }
    ...
    function load_roles() {
        ...
        foreach ($DP_AVAILABLE_ROLES as $role) {
            ...
            if (!is_readable($classfile)) {
                ...
                throw new PlanException(get_string('noclassfileforrole', 'shezar_plan', $string_params));
            }
            ...
            if (!class_exists($class)) {
                ...
                $string_params->role = $role;
                throw new PlanException(get_string('noclassforrole', 'shezar_plan', $string_params));
            }
    ...
    function load_components() {
        ...
        foreach ($DP_AVAILABLE_COMPONENTS as $component) {
            ...
            if (!is_readable($classfile)) {
                ...
                throw new PlanException(get_string('noclassfileforcomponent', 'shezar_plan', $string_params));
            }
            include_once($classfile);
            ...
            if (!class_exists($class)) {
                ...
                $string_params->class = $class;
                $string_params->component = $component;
                throw new PlanException(get_string('noclassforcomponent', 'shezar_plan', $string_params));
            }
    ...
    public function get_components() {
        ...
        if (shezar_feature_disabled('programs')) {
            $active_components = shezar_search_for_value($active_components, 'component', shezar_SEARCH_OP_NOT_EQUAL, 'program');
        }
        if (shezar_feature_disabled('competencies')) {
            $active_components = shezar_search_for_value($active_components, 'component', shezar_SEARCH_OP_NOT_EQUAL, 'competency');
        }
    ...
    function get_component_setting($component, $action) {
        ...
        print_error('error:settingdoesnotexist', 'shezar_plan', '', (object)array('component' => $component, 'action' => $action));
    }
    ...
    function initialize_settings() {
        ...
        $issuperuser = has_capability('shezar/plan:manageanyplan', context_system::instance());
        ...
        foreach ($components as $component) {
            ...
            $this->settings[$component->component.'_name'] = $name ? $name :
                get_string($component->component, 'shezar_plan');
        }
    ...
    function display_summary_widget() {
        ...
        $link = html_writer::link(new moodle_url('/shezar/plan/view.php', array('id' => $this->id)), $this->name);
        ...
        if (!shezar_feature_visible('programs')) {
            $components = shezar_search_for_value($components, 'component', shezar_SEARCH_OP_NOT_EQUAL, 'program');
        }

        if (!shezar_feature_visible('competencies')) {
            $components = shezar_search_for_value($components, 'component', shezar_SEARCH_OP_NOT_EQUAL, 'competency');
        }

        foreach ($components as $c) {
            $component = $this->get_component($c->component);
            $compname = get_string($component->component.'plural', 'shezar_plan');
        ...
        if ($pendingitems && $can_manage && $can_approve) {
            ...
            $link = new moodle_url('/shezar/plan/approve.php', array('id' => $this->id));
            $a->link = $link->out();
            $content .= html_writer::tag('span', get_string('pendingitemsx', 'shezar_plan', $a),
                    array('class' => 'dp-summary-widget-pendingitems-text'));
        }
        $out .= $OUTPUT->container($content, 'dp-summary-widget-components');
        $description = file_rewrite_pluginfile_urls($this->description, 'pluginfile.php', context_system::instance()->id, 'shezar_plan', 'dp_plan', $this->id);
    ...
    function display_add_plan_icon() {
        ...
        return $OUTPUT->action_icon(new moodle_url('/shezar/plan/edit.php'), new pix_icon('t/add', get_string('addplan', 'shezar_plan')), null, array('title' => get_string('addplan', 'shezar_plan')));
    }
    ...
    function display_enddate_highlight_info($enddate) {
        ...
        if (isset($enddate)) {
            if (($enddate < $now) && ($now - $enddate < 60*60*24)) {
                ...
                $out .= html_writer::tag('span', get_string('duetoday', 'shezar_plan'), array('class' => 'plan_highlight'));
            } else if ($enddate < $now) {
                $out .= html_writer::empty_tag('br');
                $out .= html_writer::tag('span', get_string('overdue', 'shezar_plan'), array('class' => 'plan_highlight'));
            } else if ($enddate - $now < 60*60*24*7) {
                $days = ceil(($enddate - $now)/(60*60*24));
                $out .= html_writer::empty_tag('br');
                $out .= html_writer::tag('span', get_string('dueinxdays', 'shezar_plan', $days), array('class' => 'plan_highlight'));
            }
        }
        return $out;
    }
    ...
    public function display_progress() {
        ...
        if ($this->status == DP_PLAN_STATUS_UNAPPROVED || $this->status == DP_PLAN_STATUS_PENDING) {
            $out = get_string('planstatusunapproved', 'shezar_plan');
            if ($this->status == DP_PLAN_STATUS_UNAPPROVED) {
                // Approval request
                if ($this->get_setting('approve') == DP_PERMISSION_REQUEST) {
                    $out .= html_writer::empty_tag('br');
                    $url = new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'approvalrequest' => 1, 'sesskey' => sesskey()));
                    $out .= $OUTPUT->action_link($url, get_string('requestapproval', 'shezar_plan'));
                }
            } else {
                $out .= html_writer::empty_tag('br');
                $out .= get_string('approvalrequested', 'shezar_plan');
            }
        ...
        // Get shezar core renderer
        $shezar_renderer = $PAGE->get_renderer('shezar_core');

        // Get relevant progress bar and return for display
        return $shezar_renderer->progressbar($overall_progress, 'medium', false, $tooltipstr);
    }
    ...
    function display_completeddate() {
        ...
        if ($this->status != DP_PLAN_STATUS_COMPLETE) {
            return get_string('notcompleted', 'shezar_plan');
        }
        ...
        return ($latestmodification->status != DP_PLAN_STATUS_COMPLETE) ? get_string('notcompleted', 'shezar_plan') : userdate($latestmodification->timemodified, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
    }
    ...
    function display_actions() {
        ...
        if ($this->status == DP_PLAN_STATUS_UNAPPROVED || $this->status == DP_PLAN_STATUS_PENDING) {
            ...
            if (in_array($this->get_setting('approve'), array(DP_PERMISSION_ALLOW, DP_PERMISSION_APPROVE))) {
                echo $OUTPUT->action_icon(new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'approve' => 1, 'sesskey' => sesskey())), new pix_icon('/t/go', get_string('approve', 'shezar_plan')));
                echo $OUTPUT->action_icon(new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'decline' => 1, 'sesskey' => sesskey())), new pix_icon('/t/stop', get_string('decline', 'shezar_plan')));
            }
        }
        ...
        if ($this->status == DP_PLAN_STATUS_APPROVED && $this->get_setting('completereactivate') >= DP_PERMISSION_ALLOW  && $this->get_setting('manualcomplete')) {
            echo $OUTPUT->action_icon(new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'complete' => 1, 'sesskey' => sesskey())), new pix_icon('/i/star', get_string('plancomplete', 'shezar_plan'), 'shezar_plan'));
        }

        // Reactivate
        if ($this->status == DP_PLAN_STATUS_COMPLETE && $this->get_setting('completereactivate') >= DP_PERMISSION_ALLOW) {
            echo $OUTPUT->action_icon(new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'reactivate' => 1, 'sesskey' => sesskey())), new pix_icon('/i/star_grey', get_string('planreactivate', 'shezar_plan'), 'shezar_plan'));
        }

        // Delete
        if ($this->get_setting('delete') == DP_PERMISSION_ALLOW) {
            echo $OUTPUT->action_icon(new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'delete' => 1, 'sesskey' => sesskey())), new pix_icon('/i/invalid', get_string('delete')));
        }
...
//                DP_APPROVAL_APPROVED,
    ...
    public function display_completed_plan_message($can_manage = false) {
        ...
        case DP_PLAN_REASON_MANUAL_COMPLETE:
            $message = get_string('plancompleted', 'shezar_plan');
            break;

        case DP_PLAN_REASON_AUTO_COMPLETE_ITEMS:
            $message = get_string('planautocompleteditems', 'shezar_plan');
            break;

        case DP_PLAN_REASON_AUTO_COMPLETE_DATE:
            $message = get_string('planautocompleteddate', 'shezar_plan');
            break;
        }
        ...
        if ($this->get_setting('completereactivate') == DP_PERMISSION_ALLOW && $can_manage) {
            $url = new moodle_url('/shezar/plan/action.php', array('id' => $this->id, 'reactivate' => 1, 'sesskey' => sesskey()));
            $extramessage = html_writer::tag('p', get_string('reactivateplantext', 'shezar_plan', $url->out()));
        }
    ...
    public function display_unapproved_plan_message($can_manage = false) {
        ...
        $out = '';
        $out .= html_writer::start_tag('form', array('action' => new moodle_url('/shezar/plan/action.php'), 'method' => 'post', 'class' => 'approvalform'));
        ...
        $out .= get_string('plannotapproved', 'shezar_plan');

        if ($can_manage) {
            if ($canapproveplan) {
                $out .= html_writer::start_div();
                $out .= html_writer::tag('label', get_string('reasonfordecision', 'shezar_message'), array('for' => 'id_reasonfordecision'));
    ...
    public function display_inactive_plan_message($can_manage = false) {
        $out = '';
        if ($can_manage) {
            $out .= html_writer::start_tag('form', array('action' => new moodle_url('/shezar/plan/action.php'),
                                            'method' => 'post', 'class' => 'approvalform'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'id', 'value' => $this->id));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
            $out .= get_string('plannotactivated', 'shezar_plan');
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'activate',
                                            'value' => get_string('activateplan', 'shezar_plan')));
            $out .= html_writer::end_tag('form');
        } else {
            $out .= get_string('plannotactivated', 'shezar_plan');
        }
    ...
    public function display_pending_items($pendinglist = null, $can_manage = false) {
        ...
        foreach ($DP_AVAILABLE_COMPONENTS as $componentname) {
            ...
            if ($enabled && !empty($pendinglist[$component->component])) {
                ...
                $a->name = (get_string($langkey, 'shezar_plan') ? get_string($langkey, 'shezar_plan') : $name);
                $a->link = $component->get_url()->out();
                $list[] = get_string('xitemspending', 'shezar_plan', $a);
            }
        }
        ...
        if (count($list)) {
            ...
            $description .= html_writer::tag('p', get_string($descriptor, 'shezar_plan'));
            $description .= html_writer::alist($list);
            $description .= $OUTPUT->container_end();

            $url = new moodle_url('/shezar/plan/approve.php', array('id' => $this->id));
            $actionbutton = $canapprove ? $OUTPUT->single_button($url, get_string('review', 'shezar_plan'), 'get') : '';
    ...
    public function display_unapproved_items($unapproved, $can_manage = false) {
        ...
        foreach ($unapproved as $component => $items) {
            ...
            $a->name = get_string($langkey, 'shezar_plan');
            $list[] = get_string('xitemsunapproved', 'shezar_plan', $a);
        }

        // put the heading on now we know how many
        $out = $OUTPUT->container(html_writer::tag('p', get_string(($totalitems > 1 ? 'planhasunapproveditems' : 'planhasunapproveditem'), 'shezar_plan')) . html_writer::alist($list), 'plan_box_wrap');

        // Show request button if plan is active
        if ($this->status == DP_PLAN_STATUS_APPROVED && $can_manage) {
            $out .= html_writer::start_tag('form', array('action' => new moodle_url('/shezar/plan/action.php'), 'method' => 'post'));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'id', 'value' => $this->id));
            $out .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
            $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => 'sendapprovalrequest',
                                        'name' => 'approvalrequest', 'value' => get_string('sendapprovalrequest', 'shezar_plan')));
            $out .= html_writer::end_tag('form');
        }
    ...
    static public function display_viewing_users_plan($userid) {
        ...
        return $OUTPUT->user_picture($user) . get_string('youareviewingxsplan', 'shezar_plan', $a);
    }
    ...
    function delete() {
        ...
        \shezar_plan\event\plan_deleted::create_from_plan($this)->trigger();
    ...
    function get_manager() {
        ...
        $job_assignment = \shezar_job\job_assignment::get_first($this->userid);
    ...
    public function get_all_managers() {
        ...
        if (empty($this->managers)) {
            ...
            $mgrids = \shezar_job\job_assignment::get_all_manager_userids($this->userid);
            if (empty($mgrids)) {
                $this->managers = array();
            } else {
    ...
    function send_manager_plan_approval_request() {
        ...
        if (!empty($managers) && $learner) {
            ...
            foreach($managers as $manager) {
                ...
                $event->subject = $stringmanager->get_string('plan-request-manager-short', 'shezar_plan', $a, $managerlang);
                $event->fullmessage = $stringmanager->get_string('plan-request-manager-long', 'shezar_plan', $a, $managerlang);
                $event->acceptbutton = $stringmanager->get_string('approve', 'shezar_plan', null, $managerlang) . ' ' . $stringmanager->get_string('plan', 'shezar_plan', null, $managerlang);
                $event->accepttext = $stringmanager->get_string('approveplantext', 'shezar_plan', null, $managerlang);
                $event->rejectbutton = $stringmanager->get_string('decline', 'shezar_plan', null, $managerlang) . ' ' . $stringmanager->get_string('plan', 'shezar_plan', null, $managerlang);
                $event->rejecttext = $stringmanager->get_string('declineplantext', 'shezar_plan', null, $managerlang);
                $event->infobutton = $stringmanager->get_string('review', 'shezar_plan', null, $managerlang) . ' ' . $stringmanager->get_string('plan', 'shezar_plan', null, $managerlang);
                $event->infotext = $stringmanager->get_string('reviewplantext', 'shezar_plan', null, $managerlang);
                $event->data = $data;

                tm_workflow_send($event);
            }
            $this->set_status(DP_PLAN_STATUS_PENDING, DP_PLAN_REASON_APPROVAL_REQUESTED);
            \shezar_plan\event\approval_requested::create_from_plan($this)->trigger();
    ...
    public function send_manager_item_approval_request($unapproved) {
        ...
        if (empty($managers) || !$learner) {
            print_error('error:couldnotloadusers', 'shezar_plan');
            die();
        }
        ...
        foreach($managers as $manager) {
            ...
            foreach ($unapproved as $component => $items) {
                ...
                if ($items) {
                    ...
                    $message_data[] = count($items) . ' ' . $stringmanager->get_string($comp->component, 'shezar_plan', null, $manager->lang);
                }
            }
            ...
            //ensure the message is actually coming from $learner, default to support
            $event->userfrom = ($USER->id == $learner->id) ? $learner : core_user::get_support_user();
            $event->contexturl = "{$CFG->wwwroot}/shezar/plan/approve.php?id={$this->id}";
            $event->contexturlname = $this->name;
            $event->icon = 'learningplan-request';
            ...
            $event->subject = $stringmanager->get_string('item-request-manager-short', 'shezar_plan', $a, $manager->lang);
            $event->fullmessage = $stringmanager->get_string('item-request-manager-long', 'shezar_plan', $a, $manager->lang);
            unset($event->acceptbutton);
            unset($event->onaccept);
            unset($event->rejectbutton);
            unset($event->onreject);
            $event->infobutton = $stringmanager->get_string('review', 'shezar_plan', null, $manager->lang) . ' ' . $stringmanager->get_string('items', 'shezar_plan', null, $manager->lang);
            $event->infotext = $stringmanager->get_string('reviewitemstext', 'shezar_plan', null, $manager->lang);
            $event->data = $data;
     ...
     * @param string $icon filename of icon (in theme/shezar/pix/msgicons/)
     * @param string $subjectstring lang string in shezar_plan
     * @param string $fullmessagestring lang string in shezar_plan
     * @return boolean
     */
    public function send_alert($tolearner, $icon, $subjectstring, $fullmessagestring) {
        ...
        if ($learner && $manager) {
            ...
            $a->learner = fullname($learner);
            $stringmanager = get_string_manager();
            $event->subject = $stringmanager->get_string($subjectstring,'shezar_plan',$a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string($fullmessagestring,'shezar_plan',$a, $userto->lang);
    ...
    public function send_alert_to_learner($fromuser, $icon, $subjectstring, $fullmessagestring, $a) {
        ...
        $stringmanager = get_string_manager();
        $event->subject = $stringmanager->get_string($subjectstring, 'shezar_plan', $a, $learner->lang);
        $event->fullmessage = $stringmanager->get_string($fullmessagestring, 'shezar_plan', $a, $learner->lang);
    ...
    public function send_alert_to_managers($fromuser, $icon, $subjectstring, $fullmessagestring, $a) {
        ...
        foreach($managers as $manager) {
            ...
            $stringmanager = get_string_manager();
            $event->subject = $stringmanager->get_string($subjectstring, 'shezar_plan', $a, $manager->lang);
            $event->fullmessage = $stringmanager->get_string($fullmessagestring, 'shezar_plan', $a, $manager->lang);
    ...
    function send_approved_alert($reasonfordecision) {
        ...
        $event->contexturl = $CFG->wwwroot.'/shezar/plan/view.php?id='.$this->id;
        $a = new stdClass();
        $a->name = $this->name;
        $a->user = fullname($USER); // User that approved the plan request.
        $event->subject = $stringmanager->get_string('planapprovedby', 'shezar_plan', $a, $userto->lang);
        $event->fullmessage = $stringmanager->get_string('approvedplanrequest', 'shezar_plan', $this->name, $userto->lang);

        if (!empty($reasonfordecision)) {
            $event->fullmessage .= html_writer::empty_tag('br') . html_writer::empty_tag('br');
            $event->fullmessage .= $stringmanager->get_string('reasonapprovedplanrequest', 'shezar_plan', $reasonfordecision, $userto->lang);
        }
    ...
    function send_declined_alert($reasonfordecision = '') {
        ...
        $event->subject = format_string($stringmanager->get_string('plandeclinedby', 'shezar_plan', $a, $userto->lang));
        $event->fullmessage = $event->subject;
        $event->fullmessage = $stringmanager->get_string('declinedplanrequest', 'shezar_plan', $this->name, $userto->lang);

        if (!empty($reasonfordecision)) {
            $event->fullmessage .= html_writer::empty_tag('br') . html_writer::empty_tag('br');
            $event->fullmessage .= $stringmanager->get_string('reasondeclinedplanrequest', 'shezar_plan', $reasonfordecision, $userto->lang);
        }
    ...
    function send_activated_alert() {
        ...
        $event->icon = 'learningplan-approve';
        $event->contexturl = $CFG->wwwroot.'/shezar/plan/view.php?id='.$this->id;
        $event->subject = format_string($stringmanager->get_string('planactivated', 'shezar_plan', $this->name, $userto->lang));
        $event->fullmessage = $event->subject;
    ...
    function send_completion_alert() {
        ...
        foreach($managers as $manager) {
            if ($manager->id != $USER->id) {
                ...
                //ensure the message is actually coming from $learner, default to support
                $event->userfrom = shezar_get_user_from($fromuser);
                $event->icon = 'learningplan-complete';
                $event->contexturl = $CFG->wwwroot . '/shezar/plan/view.php?id=' . $this->id;
                $a = new stdClass();
                $a->learner = fullname($learner);
                $a->plan = $this->name;
                $event->subject = $stringmanager->get_string('plan-complete-manager-short', 'shezar_plan', $a, $manager->lang);
                $event->fullmessage = $stringmanager->get_string('plan-complete-manager-long', 'shezar_plan', $a, $manager->lang);
                tm_alert_send($event);
            }
        }
        ...
        $event->icon = 'learningplan-complete';
        $event->contexturl = $CFG->wwwroot.'/shezar/plan/view.php?id='.$this->id;
        $msg = $stringmanager->get_string('plancompletesuccess', 'shezar_plan', $this->name, $learner->lang);
        $event->subject = $msg;
    ...
    public function get_display_url(){
        ...
        if ($DB->record_exists('dp_plan', array('id' => $this->id))) {
            return "{$CFG->wwwroot}/shezar/plan/view.php?id={$this->id}";
        } else {
            //If plan doesnt exist show plan index page for that user
            return "{$CFG->wwwroot}/shezar/plan/index.php?userid={$this->userid}";
        }
    }
    ...
    public function display_tabs($currenttab) {
        ...
        $row[] = new tabobject(
                'plan',
                "{$CFG->wwwroot}/shezar/plan/view.php?id={$this->id}",
                get_string('overview', 'shezar_plan')
        );
        ...
        if ($components) {
            foreach ($components as $component) {
                ...
                $row[] = new tabobject(
                    $component->component,
                    $component->get_url(),
                    $componentname = get_string("{$component->component}plural", 'shezar_plan')
                );
            }
        }
        ...
        if ($pitems = $this->num_pendingitems() && $can_manage && $can_approve) {
            $row[] = new tabobject(
                'pendingitems',
                "{$CFG->wwwroot}/shezar/plan/approve.php?id={$this->id}",
                get_string('pendingitems', 'shezar_plan').' ('.$pitems.')'
            );
        }
    ...
    public function print_header($currenttab, $navlinks = array(), $printinstructions=true) {
        global $CFG;
        require("{$CFG->dirroot}/shezar/plan/header.php");
    }
