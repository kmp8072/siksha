...
abstract class dp_base_component {
    ...
    public function __construct($plan) {
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property) && !property_exists(get_class($this), $property)) {
                ...
                throw new Exception(get_string('error:propertymustbeset', 'shezar_plan', $string_properties));
            }
        }
    }
    ...
    public function get_url() {
        ...
        return new moodle_url('/shezar/plan/component.php', array('id' => $this->plan->id, 'c' => $this->component));
    }
    ...
    public function display_list($restrict = null) {
        ...
        if (!$count = $this->count_assigned_items($restrict)) {
            $plural = mb_strtolower(get_string($this->component.'plural', 'shezar_plan'), "UTF-8");
            return html_writer::tag('span', get_string('nox', 'shezar_plan', $plural), array('class' => 'noitems-assign'.$this->component));
        }
    ...
    protected function get_list_headers() {
        ...
        $tableheaders = array(
            get_string($this->component.'name', 'shezar_plan'),
        ...
        if (($this->component == 'competency' || $this->component == 'objective')
            ...
            $tableheaders[] = get_string('numberoflinkedcourses', 'shezar_plan');
            $tablecolumns[] = 'linkedcourses';
            $tablehide[] = 'linkedcourses';
        }

        $tableheaders[] = get_string('numberoflinkedevidence', 'shezar_plan');
    ...
    public function display_approval_list($pendingitems) {
        ...
        foreach ($pendingitems as $item) {
            ...
            $controls .= get_string('reasonfordecision', 'shezar_message');
    ...
    public function update_assigned_items($items) {
        ...
        if ($items) {
            foreach ($items as $itemid) {
                ...
                if (!is_numeric($itemid)) {
                    throw new Exception(get_string('baddata', 'shezar_plan'));
                }
                // Check if not already assigned
                if (!isset($assigned_ids[$itemid])) {
                    $result = $this->assign_new_item($itemid);
                    if (!$result) {
                        print_error('error:couldnotassignnewitem', 'shezar_plan');
                    }
                    $updates .= get_string('addedx', 'shezar_plan', $result->fullname).html_writer::empty_tag('br');
                }
        ...
        foreach ($assigned as $item) {
            ...
            $updates .= get_string('removedx', 'shezar_plan', $assigned[$item->id]->fullname).html_writer::empty_tag('br');
        }
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $a->planhtml = $OUTPUT->action_link(new moodle_url('/shezar/plan/view.php', array('id' => $this->plan->id)),
                        $this->plan->name, null, array('title' => $this->plan->name));
                    $a->component = get_string($this->component.'plural', 'shezar_plan');
                    $a->updates = text_to_html($update_info, 75, false);
                    $a->updateshtml = $update_info;
                    $event->userto = $manager;
                    $a->user = fullname($USER);
                    $event->subject = $stringmanager->get_string('componentupdateshortmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('componentupdatelongmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('componentupdatelongmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('componentupdateshortlearner', 'shezar_plan', $a->component, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('componentupdatelonglearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('componentupdatelonglearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $event->userto = $manager;
                    $a->user = fullname($USER);
                    $event->subject = $stringmanager->get_string('component'.$type.'shortmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('component'.$type.'longmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('component'.$type.'longmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    if (!empty($reasonfordecision)) {
                        $breakline = html_writer::empty_tag('br') . html_writer::empty_tag('br');
                        $decision = $stringmanager->get_string('reasongivenfordecision', 'shezar_plan', $reasonfordecision, $manager->lang);
            ...
            $event->userto = $userto;
            $a->user = fullname($USER);
            $event->subject = $stringmanager->get_string('component'.$type.'shortlearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('component'.$type.'longlearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('component'.$type.'longlearnerhtml', 'shezar_plan', $a, $userto->lang);
            if (!empty($reasonfordecision)) {
                $breakline = html_writer::empty_tag('br') . html_writer::empty_tag('br');
                $decision = $stringmanager->get_string('reasongivenfordecision', 'shezar_plan', $reasonfordecision, $userto->lang);
                $event->fullmessage .= $breakline . $decision;
                $event->fullmessagehtml .= $breakline . $decision;
            }
            tm_alert_send($event);
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $event->userto = $manager;
                    $a->user = fullname($USER);
                    $event->subject = $stringmanager->get_string('componentcompleteshortmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('componentcompletelongmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('componentcompletelongmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('componentcompleteshortlearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('componentcompletelonglearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('componentcompletelonglearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    protected function display_list_item_comments($item) {
        ...
        if ($count = $comment->count()) {
            ...
            $tooltip = get_string('latestcommentby', 'shezar_plan').' '.$latestcomment->firstname.' '.get_string('on', 'shezar_plan').' '.userdate($latestcomment->ctimecreated).': '.format_string(substr($latestcomment->ccontent, 0, 50));
            $tooltip = format_string(strip_tags($tooltip));
            $commentclass = 'comments-icon-some';
        } else {
            $tooltip = get_string('nocomments', 'shezar_plan');
            $commentclass = 'comments-icon-none';
        }
        return $OUTPUT->action_link(new moodle_url("/shezar/plan/components/{$this->component}/view.php", array('id' => $this->plan->id, 'itemid' => $item->id), 'comments'),
    ...
    public function display_picker() {
        ...
        $add_button_text = get_string('add'.$this->component.'s', 'shezar_plan');
    ...
    function display_duedate_as_form($duedate, $name, $inputclass='', $itemid) {
        global $CFG;
        $duedatestr = !empty($duedate) ? userdate($duedate, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false) : '';
        return html_writer::empty_tag('input',
            array('id' => $name, 'type' => "text", 'name' => $name, 'placeholder' => get_string('datepickerlongyearplaceholder', 'shezar_core'),
    ...
    function display_priority_picker($name, $priorityid, $itemid, $priorityvalues, $prioritydefaultid, $priorityrequired=false) {
        ...
        $choose = ($priorityrequired) ? null : get_string('none', 'shezar_plan');
    ...
    function display_back_to_index_link() {
        ...
        debugging("display_back_to_index_link has been deprecated as back links are not standard navigation in shezar.");

        $url = new moodle_url('/shezar/plan/component.php', array('id' => $this->plan->id, 'c' => $this->component));
        $link = $OUTPUT->action_link($url,
            get_string('backtoallx', 'shezar_plan', get_string("{$this->component}plural", 'shezar_plan')));
    ...
    function display_approval($obj, $canapprove) {
        ...
        case DP_APPROVAL_DECLINED:
            $out .= html_writer::tag('span', get_string('declined', 'shezar_plan'), array('class' => 'plan_highlight'));
            break;
        case DP_APPROVAL_UNAPPROVED:
            $out .= $OUTPUT->pix_icon('/learning_plan_alert', get_string('unapproved', 'shezar_plan'), 'shezar_plan');
            $out .= get_string('unapproved', 'shezar_plan');
            if ($canapprove) {
                $out .= ' '.$this->display_approval_options($obj, $approvalstatus);
            }
            break;
        case DP_APPROVAL_REQUESTED:
            $out .= html_writer::tag('span', get_string('pendingapproval', 'shezar_plan'), array('class' => 'plan_highlight'));
        ...
        case DP_APPROVAL_APPROVED:
            $out .= get_string('approved', 'shezar_plan');
        }
    ...
    function display_approval_options($obj, $approvalstatus) {
        ...
        $options = array(
            DP_APPROVAL_APPROVED => get_string('approve', 'shezar_plan'),
            DP_APPROVAL_DECLINED => get_string('decline', 'shezar_plan'),
        );
