...
/**
 * Event triggered when approval for a plan, or a component in a plan, is declined
 *
 ...
 *
 */
class approval_declined extends \core\event\base {
    /**
    ...
    public static function get_name() {
        return get_string('eventapprovaldeclined', 'shezar_plan');
    }
    ...
    public function get_description() {
        // Is this approval of the entire plan, or a component within a plan?
        if ($this->other['component'] === 'plan') {
            $desc = "plan";
        } else {
            $desc = "{$this->other['component']} {$this->other['componentid']}:{$this->other['componentname']} in plan";
        }
        return "The user with id '{$this->userid}' declined approval of {$desc} {$this->objectid}:{$this->other['name']} for user {$this->relateduserid}";
    }
    ...
    protected function get_legacy_logdata() {
        ...
        return array(SITEID, 'plan', "declined {$component}", $logurl, $this->other['componentname']);
    }
    ...
    public function get_url() {
        ...
        switch ($component) {
            ...
            case 'objective':
            case 'program':
                $logurl = new \moodle_url('/shezar/plan/component.php', array('id' => $this->objectid, 'c' => $component));
                break;
            case 'evidence':
                $logurl = new \moodle_url('/shezar/plan/record/evidence/edit.php', array('id' => $this->other['componentid']));
                break;
            default:
