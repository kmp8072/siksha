...
class dp_template_general_settings_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'generalsettings', get_string('generalsettings', 'shezar_plan'));

        $mform->addElement('text', 'templatename', get_string('name', 'shezar_plan'), 'maxlength="255"');
        $mform->setType('templatename', PARAM_TEXT);
        $mform->setDefault('templatename', $templatename);
        $mform->addRule('templatename', null, 'required');

        $mform->addElement('date_selector', 'enddate', get_string('enddate', 'shezar_plan'));
        $mform->setDefault('enddate', $template->enddate);
        $mform->addRule('enddate', null, 'required');
        $mform->addHelpButton('enddate', 'templateenddate', 'shezar_plan', '', true);
...
class dp_template_new_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'newtemplate', get_string('newtemplate', 'shezar_plan'));
...
class dp_template_workflow_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'workflowsettings', get_string('workflowsettings', 'shezar_plan'));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'workflow', '', get_string('customworkflowname', 'shezar_plan') . html_writer::tag('p', get_string('customworkflowdesc', 'shezar_plan')), 'custom');
        ...
        $mform->addElement('submit', 'advancedsubmitbutton', get_string('advancedworkflow', 'shezar_plan'));
    ...
    function definition() {
        ...
        if ($component == 'plan') {
            ...
            if (!is_readable($formfile)) {
                ...
                throw new PlanException(get_string('noclassfileforcomponent', 'shezar_plan', $string_parameters));
            }
            ...
            if (!class_exists($class)) {
                ...
                $string_parameters->component = $component;
                throw new PlanException(get_string('noclassforcomponent', 'shezar_plan', $string_parameters));
            }
        }
