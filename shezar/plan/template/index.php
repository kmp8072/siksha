...
$returnurl = new moodle_url('/shezar/plan/template/index.php');
...
if ($hide) {
    if ($template = $DB->get_record('dp_template', array('id' => $hide))) {
        if ($template->isdefault == 1) {
            $message = get_string('cannothidedefault', 'shezar_plan');
            shezar_set_notification($message, new moodle_url('/shezar/plan/template/index.php'));
        }
        $visible = 0;
        $DB->set_field('dp_template', 'visible', $visible, array('id' => $template->id));
    }
...
if ($delete && $confirm) {
    if (confirm_sesskey()) {
        ...
        $transaction->allow_commit();
        shezar_set_notification(get_string('deletedp', 'shezar_plan'), new moodle_url('/shezar/plan/template/index.php'), array('class' => 'notifysuccess'));
    }
} else if ($delete) {
    $template = $DB->get_record('dp_template', array('id' => $delete));

    if ($DB->count_records('dp_plan', array('templateid' => $template->id)) > 0) {
        shezar_set_notification(get_string('cannotdelete_inuse', 'shezar_plan'), $CFG->wwwroot.'/shezar/plan/template/index.php');
    }

    if ($template->isdefault == 1) {
        shezar_set_notification(get_string('cannotdeletetemplate_default', 'shezar_plan'), new moodle_url('/shezar/plan/template/index.php'));
    }

    echo $OUTPUT->header();
    $deleteurl = new moodle_url('/shezar/plan/template/index.php', array('delete' => $delete, 'confirm' => 'true', 'sesskey' => sesskey()));
    $returnurl = new moodle_url('/shezar/plan/template/index.php');
    $strdelete = get_string('deletecheckdptemplate', 'shezar_plan');
...
if ($fromform = $mform->get_data()) {
    if (empty($fromform->submitbutton)) {
        ...
        if (!$DB->count_records('dp_priority_scale')) {
            print_error('error:notemplatewithoutpriorityscale', 'shezar_plan');
        }
        if (!$DB->count_records('dp_objective_scale')) {
            print_error('error:notemplatewithoutobjectivescale', 'shezar_plan');
        }
        ...
        if ($newtemplateid) {
            redirect(new moodle_url('/shezar/plan/template/general.php', array('id' => $newtemplateid)));
        } else {
            shezar_set_notification($error, $CFG->wwwroot . '/shezar/plan/template/index.php');
        }
    }
}
...
echo $OUTPUT->heading(get_string('managetemplates', 'shezar_plan'));
...
if ($templates) {
    ...
    $str_remove_default = get_string('deletedefault', 'shezar_plan');
    $str_moveup = get_string('moveup');
    $str_movedown = get_string('movedown');

    $columns[] = 'name';
    $headers[] = get_string('name', 'shezar_plan');
    $columns[] = 'default';
    $headers[] = get_string('default');
    $columns[] = 'instances';
    $headers[] = get_string('instances', 'shezar_plan');
    $columns[] = 'options';
    $headers[] = get_string('options', 'shezar_plan');
    $baseurl = $CFG->wwwroot . '/shezar/plan/template/index.php';

    $table = new flexible_table('Templates');
    echo html_writer::start_tag('form', array('id' => 'plantemplatedefaultform', 'action' => new moodle_url('/shezar/plan/template/index.php'), 'method' => 'POST'));
    ...
    foreach ($templates as $template) {
        ...
        $title = html_writer::link(new moodle_url('/shezar/plan/template/general.php', array('id' => $template->id)), format_string($template->fullname), array('class' => $cssclass));
        ...
        if ($instancecount) {
            $tablerow[] = html_writer::link(new moodle_url('/shezar/plan/template/templateinstances.php', array('id' => $template->id)), $instancecount);
        } else {
            $tablerow[] = $instancecount;
        }

        $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/general.php', array('id' => $template->id)), new pix_icon('t/edit', $str_edit));

        if ($template->isdefault == 1) {
            $buttons[] = $OUTPUT->pix_icon('t/delete_grey', $str_remove_default, 'shezar_core', array('class' => 'action-icon iconsmall'));
        } else {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/index.php', array('delete' => $template->id)), new pix_icon('t/delete', $str_remove));
        }
        ...
        if ($template->isdefault == 1) {
            ...
            if (!empty($template->visible)) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/index.php', array('hide' => $template->id)), new pix_icon('t/hide', $str_hide));
            } else {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/index.php', array('show' => $template->id)), new pix_icon('t/show', $str_show));
            }
        }

        if ($count > 1) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/index.php', array('moveup' => $template->id)), new pix_icon('t/up', $str_moveup));
        } else {
            $buttons[] = $spacer;
        }
        ...
        if ($count < $numvalues) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/index.php', array('movedown' => $template->id)), new pix_icon('t/down', $str_movedown));
        } else {
            $buttons[] = $spacer;
        }
    ...
    echo get_string('notemplates', 'shezar_plan');
}
