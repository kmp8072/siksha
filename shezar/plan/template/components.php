...
$returnurl = $CFG->wwwroot . '/shezar/plan/template/components.php?id=' . $id;

if ($save) {
    if (update_plan_component_name('componentname', $id)) {
        shezar_set_notification(get_string('update_components_settings', 'shezar_plan'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        shezar_set_notification(get_string('error:update_components_settings', 'shezar_plan'), $returnurl);
    }
}
...
echo $OUTPUT->heading_with_help(get_string('componentsettings', 'shezar_plan'), 'templatecomponentsettings', 'shezar_plan');
...
if ($components) {
    ...
    $table->define_baseurl('/shezar/plan/template/components.php?id=' . $id);
    ...
    foreach ($components as $component) {
        ...
        $compname = $configsetting ? $configsetting : get_string($component->component.'plural', 'shezar_plan');
        $tablerow[] = html_writer::tag('span', $compname, array('class' => $cssclass));

        if ($component->enabled) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/components.php', array('id' => $id, 'hide' => $component->id)), new pix_icon('t/hide', $str_disable));
        } else {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/components.php', array('id' => $id, 'show' => $component->id)), new pix_icon('t/show', $str_enable));
        }

        if ($count > 1) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/components.php', array('id' => $id, 'moveup' => $component->id)), new pix_icon('t/up', $str_moveup));
        } else {
            $buttons[] = $spacer;
        }
        ...
        if ($count < $numvalues) {
            $buttons[] = $OUTPUT->action_icon(new moodle_url('/shezar/plan/template/components.php', array('id' => $id, 'movedown' => $component->id)), new pix_icon('t/down', $str_movedown));
        } else {
            $buttons[] = $spacer;
        }
