...
from
  ...
  on p.id = pc.planid)",
    ...
    public function rb_display_proficiency_and_approval_menu($status, $row) {
        ...
        if (!$planid) {
            ...
            if ($competencycomponent->can_update_competency_evidence($row)) {
                ...
                $action = "var response; ".
                          "response = \$.get(".
                              "'{$CFG->wwwroot}/shezar/plan/components/competency/update-competency-setting.php".
                ...
                $content[] = html_writer::select($compscale,
                                              ...
                                              $compevscalevalueid,
                                              array(($compevscalevalueid ? '' : 0) => ($compevscalevalueid ? '' : get_string('notset', 'shezar_hierarchy'))),
                                              $attributes);
            } else if ($status) {
                $content[] = $status;
            }
    ...
    public function is_ignored() {
        return (!shezar_feature_visible('recordoflearning') or !shezar_feature_visible('competencies'));
    }
}
