...
class dp_evidence_relation {
    ...
    public function display_linked_evidence($currenturl, $canupdate, $plancompleted) {
        ...
        $evidencename = get_string('evidence', 'shezar_plan');
        $out = '';

        $out .= html_writer::empty_tag('br');
        $out .= $OUTPUT->heading(get_string('linkedx', 'shezar_plan', $evidencename), 3, null, 'dp-component-evidence-header');
        ...
        if ($this->linked_evidence_exists()) {
            ...
            $out .=  html_writer::tag('p', get_string('nolinkedx', 'shezar_plan', mb_strtolower($evidencename, "UTF-8")),
        ...
        if ($canupdate && !$plancompleted) {
            ...
            $out .=  html_writer::empty_tag('input', array(
                'type' => 'submit',
                'value' => get_string('removeselected', 'shezar_plan'),
                'class' => $class,
        ...
        if ($canupdate && !$plancompleted) {
            ...
            $out .= $OUTPUT->single_submit(get_string('addlinkedevidence', 'shezar_plan'),
    ...
    public function remove_linked_evidence($selectedids, $currenturl) {
        ...
        if (!empty($selectedids)) {
            ...
            if (!empty($deleteids)) {
                ...
                shezar_set_notification(get_string('selectedlinkedevidenceremovedfrom'.$this->componentname, 'shezar_plan'),
    ...
    public function list_linked_evidence($canremove) {
        ...
        $tableheaders = array(get_string('evidencename', 'shezar_plan'));

        $tablecolumns = array('fullname');

        if ($canremove) {
            $tableheaders[] = get_string('remove', 'shezar_plan', get_string('evidence', 'shezar_plan'));
            $tablecolumns[] = 'remove';
        }
        ...
        $url = new moodle_url('/shezar/plan/component.php', array('id' => $this->planid, 'c' => $this->componentname));
    ...
    public function display_delete_warning() {
        ...
        if ($items = $DB->get_fieldset_sql($sql, array($this->planid, $this->componentname, $this->itemid))) {
            ...
            $out .= $OUTPUT->heading(get_string('deletelinkedevidenceheader', 'shezar_plan'));
            $out .= html_writer::tag('p', get_string('deletelinkedevidencemessage', 'shezar_plan'));
    ...
    public function display_item_name($item) {
        ...
        $url = new moodle_url('/shezar/plan/components/evidence/view.php',
                array('id' => $item->id));
