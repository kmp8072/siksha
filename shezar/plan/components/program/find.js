 ...
 */
M.shezar_plan_program_find = M.shezar_plan_program_find || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_plan_program_find.init()-> jQuery dependency required for this module to function.');
        }

        var url = M.cfg.wwwroot + '/shezar/plan/components/program/';
        var saveurl = url + 'update.php?id='+this.config.plan_id+'&update=';

        require(['shezar_plan/component'], function (component) {
            component.init(M.shezar_plan_program_find.config);
            ...
            shezarDialogs['evidence'] = new shezarDialog(
                ...
                {
                    buttons: buttonsObj,
                    title: '<h2>' + M.util.get_string('addprograms', 'shezar_plan') + '</h2>'
                },
                url + 'find.php?id=' + M.shezar_plan_program_find.config.plan_id,
                handler
            );
        });
    }
