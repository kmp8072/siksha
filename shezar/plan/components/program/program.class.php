...
class dp_program_component extends dp_base_component {
    ...
    public function get_search_info(stdClass $search_info, array $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, array('p.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function process_settings_update($ajax = false) {
        ...
        if (!empty($duedates) && $cansetduedates) {
            $datepickerlongyearplaceholder = get_string('datepickerlongyearplaceholder', 'shezar_core');
        ...
        if (!empty($stored_records)) {
            ...
            if ($this->plan->reviewing_pending) {
                ...
                if (!$ajax) {
                    shezar_set_notification(get_string('programsupdated', 'shezar_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                }
            }
        }
    ...
    public function assign_new_item($itemid, $checkpermissions = true, $manual = true) {
        ...
        if ($checkpermissions) {
            if (!$permission = $this->can_update_items()) {
                print_error('error:cannotupdateprograms', 'shezar_plan');
            }
        } else {
        ...
        \shezar_plan\event\component_created::create_from_component($this->plan, 'program', $itemid, $item->fullname)->trigger();
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items()) {
            ...
            $PAGE->requires->string_for_js('addprograms', 'shezar_plan');

            $jsmodule = array(
                'name' => 'shezar_plan_program_find',
                'fullpath' => '/shezar/plan/components/program/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.shezar_plan_program_find.init', array('args' => '{"plan_id":'.$this->plan->id.', "page":"'.$paginated.'", "component_name":"'.$component_name.'"}'), false, $jsmodule);
        }
    }
    ...
    protected function display_list_item_progress($item) {
        return $this->is_item_approved($item->approved) ? prog_display_progress($item->programid, $this->plan->userid) : get_string('unapproved', 'shezar_plan');
    }
    ...
    public function display_item_name($item) {
        ...
        $img = html_writer::empty_tag('img', array('src' => shezar_get_icon($item->programid, shezar_ICON_TYPE_PROGRAM),
            'class' => 'course_icon', 'alt' => ''));
        if ($approved && $accessible) {
            $link = $OUTPUT->action_link(
                    new moodle_url('/shezar/plan/components/' . $this->component . '/view.php',array('id' => $this->plan->id, 'itemid' => $item->id, 'userid' => $extraparams)),
    ...
    function display_program_detail($progassid) {
        ...
        if (!$item) {
            return get_string('programnotfound', 'shezar_plan');
        }

        $out = '';
        $itemicon = (!empty($item->icon)) ? $item->icon : 'default';
        $icon = html_writer::empty_tag('img', array('src' => shezar_get_icon($item->id, shezar_ICON_TYPE_PROGRAM),
            'class' => 'course_icon', 'alt' => ''));
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($programs) . " " .
            get_string('programscomplete', 'shezar_program') . ", {$inprogresscount} " .
