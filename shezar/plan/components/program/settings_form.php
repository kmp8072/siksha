...
require_once($CFG->dirroot.'/shezar/plan/components/program/program.class.php');
...
function dp_program_component_build_settings_form(&$mform, $customdata) {
    ...
    $mform->addElement('header', 'programsettings', get_string('programsettings', 'shezar_plan'));
    $mform->addHelpButton('programsettings', 'advancedsettingsprogramsettings', 'shezar_plan', '', true);

    if ($templatesettings = $DB->get_record('dp_program_settings', array('templateid' => $customdata['id']))) {
    ...
    $mform->addElement('header', 'programpermissions', get_string('programpermissions', 'shezar_plan'));
    $mform->addHelpButton('programpermissions', 'advancedsettingsprogrampermissions', 'shezar_plan', '', true);
    ...
    foreach (dp_program_component::$permissions as $action => $requestable) {
        foreach ($DP_AVAILABLE_ROLES as $role) {
            ...
            $params = array($role, 'program', $action, $customdata['id']);
...
function dp_program_component_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/shezar/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'program'));
        ...
        if ($currentworkflow != 'custom') {
            ...
            if ($template = $DB->get_record('dp_template', array('id' => $id))) {
                \shezar_plan\event\template_updated::create_from_template($template, 'program')->trigger();
            }
        }
        $todb = new stdClass();
        $todb->templateid = $id;
        ...
        if ($programsettings = $DB->get_record('dp_program_settings', array('templateid' => $id))) {
            // update
            $todb->id = $programsettings->id;
            $DB->update_record('dp_program_settings', $todb);
        } else {
            // insert
            $DB->insert_record('dp_program_settings', $todb);
        }
        foreach (dp_program_component::$permissions as $action => $requestable) {
            foreach ($DP_AVAILABLE_ROLES as $role) {
                ...
                $params = array($id, 'program', $action, $role);
        ...
        shezar_set_notification(get_string('update_program_settings', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
}
