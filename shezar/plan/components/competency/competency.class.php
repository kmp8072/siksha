...
class dp_competency_component extends dp_base_component {
    ...
    public function __construct($plan) {
        ...
        $this->defaultname = get_string('competencies', 'shezar_plan');
    }
    ...
    public function get_assigned_items($approved = null, $orderby='', $limitfrom='', $limitnum='', $linkedcounts=false) {
        ...
        list($visibilitysql, $visibilityparams) = shezar_visibility_where($this->plan->userid, 'c2.id', 'c2.visible', 'c2.audiencevisible', 'c2', 'course');
    ...
    public function get_search_info(stdClass $search_info, $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, array('c.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function process_action_hook() {
        ...
        if ($delete && $confirm) {
            if (!confirm_sesskey()) {
                shezar_set_notification(get_string('confirmsesskeybad', 'error'), $currenturl);
            }
            ...
            if ($this->remove_competency_assignment($delete)) {
                ...
                shezar_set_notification(get_string('canremoveitem', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
            } else {
                shezar_set_notification(get_string('cannotremoveitem', 'shezar_plan'), $currenturl);
            }
        }
    }
    ...
    public function plan_create_hook() {
        if ($this->get_setting('autoassignorg')) {
            ...
            if (!$this->assign_from_org()) {
                print_error('unabletoassigncompsfromorg', 'shezar_plan');
            }
        }
        if ($this->get_setting('autoassignpos')) {
            // From position
            if (!$this->assign_from_pos()) {
                print_error('unabletoassigncompsfrompos', 'shezar_plan');
            }
        }
    }
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items() || hierarchy_can_add_competency_evidence($this->plan, $this, $this->plan->viewas, null)) {
            ...
            $PAGE->requires->string_for_js('addcompetencys', 'shezar_plan');
            $PAGE->requires->string_for_js('error:nocompetency', 'shezar_program');

            $jsmodule = array(
                'name' => 'shezar_plan_competency_find',
                'fullpath' => '/shezar/plan/components/competency/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.shezar_plan_competency_find.init', array('args' => '{"plan_id":'.$this->plan->id.', "page":"'.$paginated.'", "component_name":"'.$component_name.'"}'), false, $jsmodule);
        }
    }
    ...
    public function post_header_hook() {
        ...
        if ($delete) {
            ...
            if ($courses) {
                ...
                echo $OUTPUT->heading(get_string('deletelinkedcoursesheader', 'shezar_plan', s(format_string($compname))));

                if ($USER->id == $this->plan->userid) {
                    echo html_writer::tag('p', get_string('deletelinkedcoursesinstructionslearner', 'shezar_plan'));
                } else {
                    if ($planowner = $DB->get_record('user', array('id' => $this->plan->userid), 'firstname, lastname')) {
                        $planowner_name = fullname($planowner);
                    }

                    echo html_writer::tag('p', get_string('deletelinkedcoursesinstructionsmanager', 'shezar_plan', $planowner_name));
                }

                $form = html_writer::start_tag('form', array('method' => 'get', 'action' => "{$CFG->wwwroot}/shezar/plan/component.php"));
                ...
                $form .= $OUTPUT->container(html_writer::empty_tag("input", array('type' => "submit", 'value' => s(get_string('deletelinkedcoursessubmit', 'shezar_plan')))), 'buttons');
                ...
                echo $OUTPUT->confirm(get_string('confirmitemdelete', 'shezar_plan'), $continueurl, $currenturl);
                echo $OUTPUT->footer();
                die();
    ...
    function get_course_evidence_items($competencies) {
        ...
        list($visibilitysql, $visibilityparams) = shezar_visibility_where($this->plan->userid, 'c.id', 'c.visible', 'c.audiencevisible', 'c', 'course');
        ...
        $out = shezar_group_records($rs, 'competencyid');
        $rs->close();
    ...
    function display_approval_list($pendingitems) {
        ...
        foreach ($pendingitems as $item) {
            ...
            if (array_key_exists($item->competencyid, $evidence)) {
                ...
                $name .= html_writer::tag('strong', get_string('includerelatedcourses', 'shezar_plan'));
                foreach ($evidence[$item->competencyid] as $course) {
                    $message = '';
                    if ($this->plan->get_component('course')->is_item_assigned($course->courseid)) {
                        $message = ' (' . get_string('alreadyassignedtoplan', 'shezar_plan') . ')';
                    }
                    $name .= html_writer::empty_tag('br');
    ...
    function display_linked_competencies($list, $mandatory_list = null) {
        ...
        if (!$this->plan->is_complete() && $this->can_update_items()) {
            $tableheaders[] = get_string('remove', 'shezar_plan', get_string('competencies', 'shezar_plan'));
            $tablecolumns[] = 'remove';
        }
                ...
                if (!$this->plan->is_complete() && $this->can_update_items()) {
                    if (!empty($mandatory_list) && in_array($ca->id, $mandatory_list)) {
                        ...
                        $row[] = html_writer::checkbox('delete_linked_comp_assign['.$ca->id.']', '1', false,
                            get_string('mandatory', 'shezar_plan'), array('disabled' => 'true'));
                    } else {
                        $row[] = html_writer::checkbox('delete_linked_comp_assign['.$ca->id.']', '1', false);
                    }
                }
    ...
    public function display_item_name($item) {
        ...
        $img = $OUTPUT->pix_icon("/msgicons/" . $icon, '', 'shezar_core', array('class' => 'competency-state-icon'));
    ...
    function display_competency_detail($caid) {
        ...
        if (!$item) {
            return get_string('error:competencynotfound', 'shezar_plan');
        }
        ...
        $icon = $this->determine_item_icon($item);
        $outdata->icon = $OUTPUT->pix_icon("/msgicons/" . $icon, '', 'shezar_core', array('class' => "competency_state_icon"));
        ...
        if ($duedateenabled && !empty($item->duedate)) {
            $outdata->extras[] = get_string('duedate', 'shezar_plan') . ': ' . $this->display_duedate_as_text($item->duedate) . html_writer::empty_tag('br') . $this->display_duedate_highlight_info($item->duedate);
        }
        if ($status = $this->get_status($item->competencyid)) {
            $outdata->extras[] = get_string('status', 'shezar_plan'). ': ' . format_string($status);
        }
        ...
        $item->description = file_rewrite_pluginfile_urls($item->description, 'pluginfile.php',
            context_system::instance()->id, 'shezar_hierarchy', 'comp', $item->id);
        $outdata->description = format_text($item->description, FORMAT_HTML);

        return $OUTPUT->render_from_template('shezar_plan/view_plan_component', $outdata) . $out;
    }
        ...
        if (!empty($evidences)) {
            ...
            foreach ($evidences as $id => $evidence) {
                ...
                if (hierarchy_can_add_competency_evidence($this->plan, $this, $this->plan->userid, $competencyid)) {
                    ...
                    $jobassignment = \shezar_job\job_assignment::get_first($this->plan->userid, false);
                    if ($jobassignment) {
            ...
            if ($status) {
                ...
                foreach ($stored_records as $itemid => $record) {
                    ...
                    if (!empty($record->priority) && $oldrecords[$itemid]->priority != $record->priority) {
                        ...
                        $updates .= $compheader;
                        $compprinted = true;
                        $updates .= get_string('priority', 'shezar_plan').' - '.get_string('changedfromxtoy',
                            'shezar_plan', (object)array('before' => $oldpriority, 'after' => $newpriority)).html_writer::empty_tag('br');
                    }
                    if (!empty($record->duedate) && $oldrecords[$itemid]->duedate != $record->duedate) {
                        ...
                        $updates .= get_string('duedate', 'shezar_plan').' - '.
                            get_string('changedfromxtoy', 'shezar_plan', (object)array('before' => empty($oldrecords[$itemid]->duedate) ? '' :
                    ...
                    if (!empty($record->approved) && $oldrecords[$itemid]->approved != $record->approved) {
                        ...
                        $text = $compheader;
                        $text .= get_string('approval', 'shezar_plan').' - '.
                            get_string('changedfromxtoy', 'shezar_plan', (object)array('before' => dp_get_approval_status_from_code($oldrecords[$itemid]->approved),
                ...
                if ($status) {
                    ...
                    if (!empty($badduedates)) {
                        $issuesnotification .= $this->get_setting('duedatemode') == DP_DUEDATES_REQUIRED ?
                            html_writer::empty_tag('br').get_string('noteduedateswrongformatorrequired', 'shezar_plan') : html_writer::empty_tag('br').get_string('noteduedateswrongformat', 'shezar_plan');
                    }

                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        shezar_set_notification(get_string('competenciesupdated', 'shezar_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                    }
                } else {
                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        shezar_set_notification(get_string('error:competenciesupdated', 'shezar_plan'), $currenturl);
                    }
                }
            }
        }
    ...
    function remove_competency_assignment($caid) {
        ...
        if (!$this->can_delete_item($item)) {
            print_error('error:nopermissiondeletemandatorycomp', 'shezar_plan');
        }
    ...
    public function assign_new_item($itemid, $checkpermissions = true, $manual = true) {
        ...
        \shezar_plan\event\component_created::create_from_component($this->plan, 'competency', $itemid, $item->fullname)->trigger();
    ...
    function assign_from_pos() {
        ...
        $jobassignment = \shezar_job\job_assignment::get_first($this->plan->userid);
    ...
    public function get_competency_menu($item) {
        ...
        $output = html_writer::select($formatscale,
                                    ...
                                    array(($item->profscalevalueid ? '' : 0) => ($item->profscalevalueid ? '' : get_string('notset', 'shezar_hierarchy'))),
                                    $attributes);
    ...
    public function display_comp_add_evidence_icon($item, $returnurl=false) {
        ...
        if ($this->can_update_competency_evidence($item)) {
            $straddevidence = get_string('setstatusicon', 'shezar_plan');
            return $OUTPUT->action_icon(new moodle_url('/shezar/plan/components/competency/add_evidence.php',
    ...
    public function display_course_picker($competencyid) {
        ...
        $btntext_addfromplan = get_string('addlinkedcourses', 'shezar_plan');
        $btntext_addfromcomp = get_string('addlinkedcoursescompetency', 'shezar_plan');
    ...
    public function progress_stats() {
        ...
        $progress_str =  get_string('xofy', 'shezar_core', $a) . " " .
            get_string('competenciesachieved', 'shezar_plan') . "\n";
