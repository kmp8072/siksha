...
require_once($CFG->dirroot.'/shezar/plan/components/competency/competency.class.php');
...
function dp_competency_component_build_settings_form(&$mform, $customdata) {
    ...
    $mform->addElement('header', 'competencysettings', get_string('competencysettings', 'shezar_plan'));
    $mform->addHelpButton('competencysettings', 'advancedsettingscompetencysettings', 'shezar_plan', '', true);
    ...
    $radiogroup = array();
    $radiogroup[] =& $mform->createElement('radio', 'duedatemode', '', get_string('none', 'shezar_plan'), DP_DUEDATES_NONE);
    $radiogroup[] =& $mform->createElement('radio', 'duedatemode', '', get_string('optional', 'shezar_plan'), DP_DUEDATES_OPTIONAL);
    $radiogroup[] =& $mform->createElement('radio', 'duedatemode', '', get_string('required', 'shezar_plan'), DP_DUEDATES_REQUIRED);
    $mform->addGroup($radiogroup, 'duedategroup', get_string('duedates', 'shezar_plan'), html_writer::empty_tag('br'), false);
    $mform->setDefault('duedatemode', $defaultduedatesmode);

    // priorities mode options
    $radiogroup = array();
    $radiogroup[] =& $mform->createElement('radio', 'prioritymode', '', get_string('none', 'shezar_plan'), DP_PRIORITY_NONE);
    $radiogroup[] =& $mform->createElement('radio', 'prioritymode', '', get_string('optional', 'shezar_plan'), DP_PRIORITY_OPTIONAL);
    $radiogroup[] =& $mform->createElement('radio', 'prioritymode', '', get_string('required', 'shezar_plan'), DP_PRIORITY_REQUIRED);
    $mform->addGroup($radiogroup, 'prioritygroup', get_string('priorities', 'shezar_plan'), html_writer::empty_tag('br'), false);
    ...
    if ($priorities = dp_get_priorities()) {
        ...
        $mform->addElement('static', 'nopriorityscales', null, get_string('nopriorityscales', 'shezar_plan'));
    ...
    $mform->addElement('select', 'priorityscale', get_string('priorityscale', 'shezar_plan'), $prioritymenu);
    $mform->disabledIf('priorityscale', 'prioritymode', 'eq', DP_PRIORITY_NONE);
    if (!empty($customdata['templateinuse'])) {
        $mform->addElement('static', 'priorityscalesdisabledtemplateinuse', null, get_string('priorityscalesdisabledtemplateinuse', 'shezar_plan'));
    ...
    $autoassigngroup[] =& $mform->createElement('advcheckbox', 'autoassignpos', null, get_string('autoassignpos', 'shezar_plan'));
    $autoassigngroup[] =& $mform->createElement('advcheckbox', 'autoassignorg', null, get_string('autoassignorg', 'shezar_plan'));
    $autoassigngroup[] =& $mform->createElement('advcheckbox', 'includecompleted', null, get_string('includecompleted', 'shezar_plan'));
    $autoassigngroup[] =& $mform->createElement('advcheckbox', 'autoassigncourses', null, get_string('autoassigncourses', 'shezar_plan'));

    $mform->addGroup($autoassigngroup, 'autoassign', get_string('autoassign', 'shezar_plan'), array(html_writer::empty_tag('br')), false);
    ...
    $mform->addElement('advcheckbox', 'autoadddefaultevidence', get_string('defaultstatus', 'shezar_plan'), get_string('setdefaultstatus', 'shezar_plan'), null, array(0,1));
    $mform->setDefault('autoadddefaultevidence', $defaultautoadddefaultevidence);


    //Permissions
    $mform->addElement('header', 'competencypermissions', get_string('competencypermissions', 'shezar_plan'));
    $mform->addHelpButton('competencypermissions', 'advancedsettingscompetencypermissions', 'shezar_plan', '', true);

    dp_add_permissions_table_headings($mform);

    foreach (dp_competency_component::$permissions as $action => $requestable) {
        dp_add_permissions_table_row($mform, $action, get_string($action, 'shezar_plan'), $requestable);
    }
...
function dp_competency_component_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/shezar/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'competency'));
    ...
    if ($currentworkflow != 'custom') {
        ...
        if ($template = $DB->get_record('dp_template', array('id' => $id))) {
            \shezar_plan\event\template_updated::create_from_template($template, 'competency')->trigger();
        }
    }
    $todb = new stdClass();
    $todb->templateid = $id;
    ...
    shezar_set_notification(get_string('update_competency_settings', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
}
