...
$form = $OUTPUT->heading(get_string('confirmlinkedcourses', 'shezar_plan'));
$form .= html_writer::tag('p', get_string('confirmlinkedcoursesdesc', 'shezar_plan'));
...
foreach ($evidence as $compid => $linkedcourses) {
    $form .= get_string('competency', 'shezar_hierarchy') . ' '. format_string($compnames[$compid]) . ':' . html_writer::empty_tag('br');
