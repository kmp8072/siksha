...
class dp_course_component extends dp_base_component {
    ...
    public function get_assigned_items($approved = null, $orderby='', $limitfrom='', $limitnum='', $linkedcounts=false) {
        ...
        list($visibilitysql, $visibilityparams) = shezar_visibility_where($this->plan->userid,
                                                                          'c.id',
                                                                          'c.visible',
                                                                          'c.audiencevisible',
                                                                          'c',
    ...
    public function process_action_hook() {
        ...
        if ($delete && $confirm) {
            ...
            if (!$deleteitem = $this->get_assignment($delete)) {
                print_error('error:couldnotfindassigneditem', 'shezar_plan');
            }

            // Check mandatory permissions
            if (!$this->can_delete_item($deleteitem)) {
                print_error('error:nopermissiondeletemandatorycourse', 'shezar_plan');
            }
            ...
            if ($this->unassign_item($deleteitem)) {
                ...
                print_error('error:couldnotunassignitem', 'shezar_plan');
            }
        }
    }
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items() && dp_can_manage_users_plans($this->plan->userid)) {
            ...
            $PAGE->requires->string_for_js('addcourses', 'shezar_plan');

            $jsmodule = array(
                'name' => 'shezar_plan_course_find',
                'fullpath' => '/shezar/plan/components/course/find.js',
                'requires' => array('json'));
            $PAGE->requires->js_init_call('M.shezar_plan_course_find.init', array('args' => '{"plan_id":'.$this->plan->id.', "page":"'.$paginated.'", "component_name":"'.$component_name.'"}'), false, $jsmodule);
        }
    }
    ...
    public function assign_new_item($itemid, $checkpermissions = true, $manual = true) {
        ...
        \shezar_plan\event\component_created::create_from_component($this->plan, 'course', $itemid, $item->fullname)->trigger();
    ...
    function display_linked_courses($list, $mandatory_list = null) {
        ...
        $tableheaders = array(
            get_string('coursename', 'shezar_plan'),
        );
        $tablecolumns = array(
            'fullname',
        );
        ...
        $tableheaders[] = get_string('progress', 'shezar_plan');
        $tablecolumns[] = 'progress';

        if (!$this->plan->is_complete() && $this->can_update_items()) {
            $tableheaders[] = get_string('remove', 'shezar_plan', get_string('courses'));
            $tablecolumns[] = 'remove';
        }
    ...
    public function display_item_name($item) {
        ...
        if ($approved) {
            $class = '';
            $action_link = $OUTPUT->action_link(new moodle_url('/course/view.php', array('id' => $item->courseid)), get_string('launchcourse', 'shezar_plan'), null, array('class' => 'link-as-button'));
        ...
        $img = html_writer::empty_tag('img', array('src' => shezar_get_icon($item->courseid, shezar_ICON_TYPE_COURSE),
            'class' => 'course_icon', 'alt' => ''));
        $url = new moodle_url('/shezar/plan/components/' . $this->component . '/view.php', array('id' => $this->plan->id, 'itemid' => $item->id));
    ...
    function display_course_detail($caid) {
        ...
        if (!$item) {
            return get_string('coursenotfound', 'shezar_plan');
        }
        ...
        $outdata->icon = html_writer::empty_tag('img', array('src' => shezar_get_icon($item->courseid, shezar_ICON_TYPE_COURSE),
            'class' => 'course_icon', 'alt' => ''));
        ...
        if ($progressbar = $this->display_status_as_progress_bar($item)) {
            ...
            $outdata->extras[] = get_string('progress', 'shezar_plan');
    ...
    function display_status_as_progress_bar($item) {
        return shezar_display_course_progress_icon($this->plan->userid, $item->courseid, $item->coursecompletion);
    }
        ...
        if (!empty($stored_records)) {
                ...
                foreach ($stored_records as $itemid => $record) {
                    ...
                    if (!empty($record->priority) && $oldrecords[$itemid]->priority != $record->priority) {
                        ...
                        $updates .= get_string('priority', 'shezar_plan').' - '.
                            get_string('changedfromxtoy', 'shezar_plan', (object)array('before' => $oldpriority, 'after' => $newpriority)).html_writer::empty_tag('br');
                    }
            ...
            if ($this->plan->reviewing_pending) {
                ...
                if ($status) {
                    ...
                    if (!$ajax) {
                        shezar_set_notification(get_string('coursesupdated', 'shezar_plan').$issuesnotification, $currenturl, array('class' => 'notifysuccess'));
                    }
                } else {
                    // Do not create notification or redirect if ajax request
                    if (!$ajax) {
                        shezar_set_notification(get_string('coursesnotupdated', 'shezar_plan'), $currenturl);
                    }
                }
            }
    ...
    function get_list_headers() {
        ...
        foreach ($headers->headers as $i => $h) {
            if ($h == get_string('status', 'shezar_plan')) {
                // Replace 'Status' header with 'Progress'
                $headers->headers[$i] = get_string('progress', 'shezar_plan');
                break;
            }
        }
    ...
    protected function display_list_item_actions($item) {
        ...
        if (dp_can_manage_users_plans($this->plan->userid)) {
            ...
            if ($cansetcompletion && $approved && $CFG->enablecourserpl) {
                $strrpl = get_string('addrpl', 'shezar_plan');
                $proficient = $OUTPUT->action_icon(new moodle_url('/shezar/plan/components/course/rpl.php',
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($courses) . " " .
            get_string('coursescomplete', 'shezar_plan') . ", {$inprogresscount} " .
            get_string('inprogress', 'shezar_plan') . "\n";
    ...
    public function display_competency_picker($courseid) {
        ...
        $btntext = get_string('addlinkedcompetencies', 'shezar_plan');
