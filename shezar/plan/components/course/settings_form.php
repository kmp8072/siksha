...
require_once($CFG->dirroot.'/shezar/plan/components/course/course.class.php');
...
function dp_course_component_build_settings_form(&$mform, $customdata) {
    ...
    $mform->addElement('header', 'coursesettings', get_string('coursesettings', 'shezar_plan'));
    $mform->addHelpButton('coursesettings', 'advancedsettingscoursesettings', 'shezar_plan', '', true);
    ...
    $mform->addElement('header', 'coursepermissions', get_string('coursepermissions', 'shezar_plan'));
    $mform->addHelpButton('coursepermissions', 'advancedsettingscoursepermissions', 'shezar_plan', '', true);
...
function dp_course_component_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/shezar/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'course'));
        ...
        if ($currentworkflow != 'custom') {
            ...
            if ($template = $DB->get_record('dp_template', array('id' => $id))) {
                \shezar_plan\event\template_updated::create_from_template($template, 'course')->trigger();
            }
        }
        $todb = new stdClass();
        $todb->templateid = $id;
        ...
        shezar_set_notification(get_string('update_course_settings', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
}
