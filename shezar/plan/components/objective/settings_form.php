...
require_once($CFG->dirroot.'/shezar/plan/components/objective/objective.class.php');
...
function dp_objective_component_build_settings_form(&$mform, $customdata) {
    ...
    $mform->addElement('header', 'objectivesettings', get_string('objectivesettings', 'shezar_plan'));
    $mform->addHelpButton('objectivesettings', 'advancedsettingsobjectivesettings', 'shezar_plan', '', true);
    ...
    $mform->addElement('select', 'objectivescale', get_string('objectivescale', 'shezar_plan'), $objectivemenu);
    if (!empty($customdata['templateinuse'])) {
        ...
        $mform->addElement('static', 'objectivescalesdisabledtemplateinuse', null, get_string('objectivescalesdisabledtemplateinuse', 'shezar_plan'));
    }

    $mform->setDefault('objectivescale', $defaultobjectivescale);

    //Permissions
    $mform->addElement('header', 'objectivepermissions', get_string('objectivepermissions', 'shezar_plan'));
    $mform->addHelpButton('objectivepermissions', 'advancedsettingsobjectivepermissions', 'shezar_plan', '', true);
...
function dp_objective_component_process_settings_form($fromform, $id) {
    ...
    $currenturl = new moodle_url('/shezar/plan/template/advancedworkflow.php', array('id' => $id, 'component' => 'objective'));
    ...
    if ($currentworkflow != 'custom') {
        ...
        if ($template = $DB->get_record('dp_template', array('id' => $id))) {
            \shezar_plan\event\template_updated::create_from_template($template, 'objective')->trigger();
        }
    }
    $todb = new stdClass();
    $todb->templateid = $id;
    ...
    shezar_set_notification(get_string('update_objective_settings', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
}
