...
class plan_objective_edit_form extends moodleform {
    ...
    function definition() {
        ...
        if ($prioritymode > DP_PRIORITY_NONE) {
            ...
            if ($scaleid) {
                ...
                if ($prioritymode == DP_PRIORITY_OPTIONAL) {
                    $select[] = get_string('none', 'shezar_plan');
                }
                ...
                $prioritylist = array( get_string('none', 'shezar_plan') );
            }
        }
        ...
        $mform->addElement('text', 'fullname', get_string('objectivetitle', 'shezar_plan'));
        $mform->setType('fullname', PARAM_TEXT);
        $mform->addRule('fullname', get_string('err_required', 'form'), 'required', '', 'client', false, false);
        $mform->addElement('editor', 'description_editor', get_string('objectivedescription', 'shezar_plan'), null, $TEXTAREA_OPTIONS);
        $mform->setType('description_editor', PARAM_CLEANHTML);

        // Due dates
        if ($duedateallow && ($duedatemode == DP_DUEDATES_OPTIONAL || $duedatemode == DP_DUEDATES_REQUIRED)) {
            $mform->addElement('date_selector', 'duedate', get_string('duedate', 'shezar_plan'));
        ...
        if ($prioritymode == DP_PRIORITY_OPTIONAL || $prioritymode == DP_PRIORITY_REQUIRED) {
            $mform->addElement('select', 'priority', get_string('priority', 'shezar_plan'), $prioritylist);
        ...
        // Proficiency
        $mform->addElement('select', 'scalevalueid', get_string('status', 'shezar_plan'), $proflist);
        ...
        $this->add_action_buttons(true, empty($this->_customdata['objectiveid']) ?
            get_string('addobjective', 'shezar_plan') : get_string('updateobjective', 'shezar_plan'));
    }
}
