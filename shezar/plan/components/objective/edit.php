...
$PAGE->set_url(new moodle_url('/shezar/plan/components/objective/edit.php', array('id' => $planid)));
...
if ($objectiveid) {
    $objviewurl = "{$CFG->wwwroot}/shezar/plan/components/objective/view.php?id={$planid}&amp;itemid={$objectiveid}";
} else {
...
if (!$component->can_update_items()) {
    print_error('error:cannotupdateobjectives', 'shezar_plan');
}
if ($plancompleted) {
...
$objective = file_prepare_standard_editor($objective, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                          'shezar_plan', 'dp_plan_objective', $objective->itemid);
$mform = $component->objective_form($objectiveid);
$mform->set_data($objective);
if (isset($objective->duedate)) {
    $objective->duedate = userdate($objective->duedate, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
}

if ($deleteyes) {
    require_sesskey();
    if (!$component->delete_objective($objectiveid)) {
        print_error('error:objectivedeleted', 'shezar_plan');
    } else {
        shezar_set_notification(get_string('objectivedeleted', 'shezar_plan'), $objallurl, array('class' => 'notifysuccess'));
    }
} else if ($deleteno) {
    redirect($objallurl);
    ...
    if (empty($data->itemid)) {
        ...
        if (!$result) {
            print_error('error:objectiveupdated', 'shezar_plan');
        }
        $data->itemid = $result;
        $notification = get_string('objectivecreated', 'shezar_plan');

        $data = file_postupdate_standard_editor($data, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'shezar_plan', 'dp_plan_objective', $data->itemid);
        ...
        \shezar_plan\event\component_updated::create_from_component($plan, 'objective', $record->id, $record->fullname)->trigger();
        ...
        $notification = get_string('objectiveupdated', 'shezar_plan');
    }
    shezar_set_notification($notification, $objviewurl, array('class' => 'notifysuccess'));
}
...
$PAGE->navbar->add(get_string("{$component->component}plural", 'shezar_plan'), new moodle_url('/shezar/plan/component.php', array('id' => $planid, 'c' =>'objective')));

switch($action) {
    case 'add':
        $PAGE->navbar->add(get_string('addnewobjective', 'shezar_plan'));
        break;
    case 'delete':
        $PAGE->navbar->add(get_string('deleteobjective', 'shezar_plan', format_string($objective->fullname)));
        break;
    case 'edit':
        $PAGE->navbar->add(get_string('editobjective', 'shezar_plan', format_string($objective->fullname)));
        break;
}
...
switch($action) {
    case 'add':
        echo $OUTPUT->heading(get_string('addnewobjective', 'shezar_plan'));
        $mform->display();
        break;
    case 'delete':
        echo $OUTPUT->heading(get_string('deleteobjective', 'shezar_plan'));
        ...
        echo $OUTPUT->confirm(get_string('deleteobjectiveareyousure', 'shezar_plan'),
                new moodle_url('/shezar/plan/components/objective/edit.php',
                    array('id' => $planid, 'itemid' => $objectiveid, 'deleteyes' => 'Yes', 'sesskey' => sesskey())),
                new moodle_url('/shezar/plan/components/objective/edit.php',
                        array('id' => $planid, 'itemid' => $objectiveid, 'deleteno' => 'No')));
        break;
    case 'edit':
        echo $OUTPUT->heading(get_string('editobjective', 'shezar_plan', $objective->fullname));
        $mform->display();
        break;
}
