...
class dp_objective_component extends dp_base_component {
    ...
    public function __construct($plan) {
        ...
        $this->defaultname = get_string('objectives', 'shezar_plan');
    }
    ...
    public function get_search_info(stdClass $search_info, array $keywords, $parentid = 0, $approved = null) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, array('a.fullname'),
                SQL_PARAMS_NAMED);
    ...
    public function setup_picker() {
        ...
        if ($this->can_update_items()) {
            ...
            $PAGE->requires->js_call_amd('shezar_plan/component', 'init', array('args' => '{"plan_id":'.$this->plan->id.', "page":"'.$paginated.'", "component_name":"'.$component_name.'"}'));
    ...
    function display_linked_objectives($list) {
        ...
        $objectivename = get_string('objective', 'shezar_plan');
        ...
        if ($showpriorities) {
            $tableheaders[] = get_string('priority', 'shezar_plan');
    ...
    function display_item_name($item) {
        ...
        $img = $OUTPUT->pix_icon("/msgicons/" . $icon, format_string($item->fullname), 'shezar_core', array('class' => 'objective-state-icon'));
        $link = $OUTPUT->action_link(
                new moodle_url('/shezar/plan/components/' . $this->component . '/view.php', array('id' => $this->plan->id, 'itemid' => $item->id)),
    ...
    function objective_form($objectiveid=null) {
        global $CFG, $DB;
        require_once($CFG->dirroot.'/shezar/plan/components/objective/edit_form.php');
        $customdata = array(
            'plan' => $this->plan,
            'objective' => $this
        );
        if (empty($objectiveid)) {
            ...
            if (!$objective = $DB->get_record('dp_plan_objective', array('id' => $objectiveid))) {
                print_error('error:objectiveidincorrect', 'shezar_plan');
            }
    ...
    public function process_settings_update($ajax = false) {
        ...
        if (!empty($duedates) && $cansetduedates) {
            ...
            foreach ($duedates as $id => $duedate) {
                ...
                if ($duedate == '' || $duedate == get_string('datepickerlongyearplaceholder', 'shezar_core')) {
                    ...
                    $datepattern = get_string('datepickerlongyearregexphp', 'shezar_core');
                    if (preg_match($datepattern, $duedate, $matches) == 0) {
                        // skip badly formatted date strings
                        continue;
                    }
                    $duedateout = shezar_date_parse_from_format(get_string('datepickerlongyearparseformat', 'shezar_core'), $duedate);
                }
                $todb = new stdClass();
                $todb->id = $id;
        ...
        if (!empty($proficiencies) && $cansetprofs) {
            foreach ($proficiencies as $id => $proficiency) {
                ...
                $count = $DB->count_records('block_shezar_stats', array('userid' => $currentuser, 'eventtype' => STATS_EVENT_OBJ_ACHIEVED, 'data2' => $id));
                $scalevalue = $DB->get_record('dp_objective_scale_value', array('id' => $proficiency));
                if (empty($scalevalue)) {
                    print_error('error:priorityscalevalueidincorrect', 'shezar_plan');
                    // checks objective can only be achieved once.
                } else if ($scalevalue->achieved == 1 && $count < 1) {
                    shezar_stats_add_event(time(), $currentuser, STATS_EVENT_OBJ_ACHIEVED, '', $id);
                    // checks objective exists for removal
                } else if ($scalevalue->achieved == 0 && $count > 0) {
                    shezar_stats_remove_event($currentuser, STATS_EVENT_OBJ_ACHIEVED, $id);
                }
            }
        }
        ...
        if (!empty($stored_records)) {
            ...
            foreach ($stored_records as $itemid => $record) {
                ...
                if (!empty($record->priority) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $updates .= $objheader;
                    $objprinted = true;
                    $updates .= get_string('priority', 'shezar_plan').' - '.
                        get_string('changedfromxtoy', 'shezar_plan',
                ...
                if (!empty($record->duedate) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $objprinted = true;
                    $updates .= $stringmanager->get_string('duedate', 'shezar_plan', null, $currentuserobj->lang).' - '.
                            get_string('changedfromxtoy', 'shezar_plan',
                ...
                if (!empty($record->scalevalueid) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $updates .= $objprinted ? '' : $objheader;
                    $objprinted = true;
                    $updates .= $stringmanager->get_string('status', 'shezar_plan', null, $currentuserobj->lang).' - '.
                        get_string('changedfromxtoy', 'shezar_plan',
                ...
                if (!empty($record->approved) && array_key_exists($itemid, $orig_objectives) &&
                    ...
                    $text = $objheader;
                    $text .= $stringmanager->get_string('approval', 'shezar_plan', null, $currentuserobj->lang).' - '.
                            get_string('changedfromxtoy', 'shezar_plan',
            ...
            if ($this->plan->status != DP_PLAN_STATUS_UNAPPROVED && count($approvals)>0) {
                foreach ($approvals as $approval) {
                    ...
                    if ($approval->after == DP_APPROVAL_APPROVED) {
                        \shezar_plan\event\approval_approved::create_from_component(
                            $this->plan, 'objective', $approval->itemid, $approval->itemname)->trigger();
                    } else {
                        \shezar_plan\event\approval_declined::create_from_component(
                            $this->plan, 'objective', $approval->itemid, $approval->itemname)->trigger();
                    }
                }
            }
            ...
            if ($this->plan->reviewing_pending) {
                ...
                shezar_set_notification(get_string('objectivesupdated', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
            }
        }
    ...
    function delete_objective($caid) {
        ...
        \shezar_plan\event\component_deleted::create_from_component(
    ...
    public function create_objective($fullname, $description=null, $priority=null, $duedate=null, $scalevalueid=null) {
        ...
        \shezar_plan\event\component_created::create_from_component($this->plan, 'objective', $rec->id, $rec->fullname)->trigger();
    ...
    function send_deletion_alert($objective) {
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $event->contexturl = new moodle_url("/shezar/plan/view.php", array('id' => $this->plan->id));
                    ...
                    $event->subject = $stringmanager->get_string('objectivedeleteshortmanager', 'shezar_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectivedeletelongmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectivedeletelongmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        }
        // notify user that someone else did it
            ...
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectivedeleteshortlearner', 'shezar_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectivedeletelonglearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectivedeletelonglearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    function send_creation_alert($objid, $fullname) {
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $userfrom = $DB->get_record('user', array('id' => $USER->id));
                    $event->userfrom = $userfrom;
                    $event->contexturl = new moodle_url('/shezar/plan/components/objective/view.php', array('id' => $this->plan->id, 'itemid' => $objid));
                    $event->icon = 'objective-add';
                    $a = new stdClass();
                    $a->objective = $fullname;
                    $a->objectivehtml = $OUTPUT->action_link($event->contexturl, $fullname);
                    $url = new moodle_url('/shezar/plan/view.php', array('id' => $this->plan->id));
                    $a->plan = $this->plan->name;
                    $a->planhtml = $OUTPUT->action_link($url, $this->plan->name, null, array('title' => $this->plan->name));
                    $a->userfrom = fullname($USER);
                    $a->userfromhtml = $this->current_user_link();
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objectivenewshortmanager', 'shezar_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectivenewlongmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectivenewlongmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        }
        // notify user that someone else did it
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectivenewshortlearner', 'shezar_plan', $fullname, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectivenewlonglearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectivenewlonglearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $userfrom = $DB->get_record('user', array('id' => $USER->id));
                    $event->userfrom = $userfrom;
                    $event->contexturl = new moodle_url("/shezar/plan/components/objective/view.php", array('id' => $this->plan->id, 'itemid' => $objective->id));
                    ...
                    $a->userfrom = fullname($USER);
                    $a->userfromhtml = $this->current_user_link();
                    $a->field = $stringmanager->get_string('objective'.$field, 'shezar_plan', $manager->lang);
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objectiveeditshortmanager', 'shezar_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objectiveeditlongmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objectiveeditlongmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            // notify user that someone else did it
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $a->field = $stringmanager->get_string('objective'.$field, 'shezar_plan', null, $userto->lang);
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objectiveeditshortlearner', 'shezar_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objectiveeditlonglearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objectiveeditlonglearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
        ...
        if ($USER->id == $this->plan->userid) {
            ...
            if ($this->plan->is_active()) {
                ...
                foreach($managers as $manager) {
                    ...
                    $a->userfromhtml = $this->current_user_link();
                    $event->userto = $manager;
                    $event->subject = $stringmanager->get_string('objective'.$status.'shortmanager', 'shezar_plan', fullname($USER), $manager->lang);
                    $event->fullmessage = $stringmanager->get_string('objective'.$status.'longmanager', 'shezar_plan', $a, $manager->lang);
                    $event->fullmessagehtml = $stringmanager->get_string('objective'.$status.'longmanagerhtml', 'shezar_plan', $a, $manager->lang);
                    tm_alert_send($event);
                }
            }
        } else {
            ...
            $userto = $DB->get_record('user', array('id' => $this->plan->userid));
            $event->userto = $userto;
            $event->subject = $stringmanager->get_string('objective'.$status.'shortlearner', 'shezar_plan', $a->objective, $userto->lang);
            $event->fullmessage = $stringmanager->get_string('objective'.$status.'longlearner', 'shezar_plan', $a, $userto->lang);
            $event->fullmessagehtml = $stringmanager->get_string('objective'.$status.'longlearnerhtml', 'shezar_plan', $a, $userto->lang);
            tm_alert_send($event);
        }
    }
    ...
    protected function display_list_item_actions($item) {
        ...
        if ($this->can_delete_item($item) && dp_can_manage_users_plans($this->plan->userid)) {
            $deleteurl = new moodle_url('/shezar/plan/components/objective/edit.php',
                array('id' => $this->plan->id, 'itemid' => $item->id, 'd' => 1));
            $strdelete = get_string('delete', 'shezar_plan');
    ...
    public function display_picker() {
        ...
        $button = $OUTPUT->single_button(new moodle_url("/shezar/plan/components/objective/edit.php",
                                            array('id' => $this->plan->id)), get_string('addnewobjective', 'shezar_plan'), 'get');
    ...
    public function display_course_picker($objectiveid) {
        ...
        $btntext = get_string('addlinkedcourses', 'shezar_plan');
    ...
    public function display_objective_detail($objectiveid, $canupdate) {
        ...
        if (!$item) {
            return get_string('error:objectivenotfound', 'shezar_plan');
        }
        ...
        $icon = $this->determine_item_icon($item);
        $outdata->icon = $OUTPUT->pix_icon("/msgicons/" . $icon, '', 'shezar_core', array('class' => 'objective_state_icon'));
        ...
        if (!$plancompleted && $canupdate) {
            if ($this->will_an_update_revoke_approval( $objectiveid )) {
                $buttonlabel = get_string('editdetailswithapproval', 'shezar_plan');
            } else {
                $buttonlabel = get_string('editdetails', 'shezar_plan');
            }
            $single_button = new single_button(new moodle_url("/shezar/plan/components/objective/edit.php", array('id' => $this->plan->id, 'itemid' => $objectiveid)), $buttonlabel);
        ...
        if ($priorityenabled && !empty($item->priority)) {
            $outdata->extras[] = get_string('priority', 'shezar_plan') . ': ' . $this->display_priority_as_text($item->priority, $item->priorityname, $priorityvalues);
        }
        if ($duedateenabled && !empty($item->duedate)) {
            $extra = get_string('duedate', 'shezar_plan') . ': ' . $this->display_duedate_as_text($item->duedate);
            if (!$item->achieved) {
        ...
        if (!empty($item->profname)) {
            $outdata->extras[] = get_string('status', 'shezar_plan') .": \n" . "  {$item->profname}\n";
        }
        ...
        $item->description = file_rewrite_pluginfile_urls($item->description, 'pluginfile.php', context_system::instance()->id, 'shezar_plan', 'dp_plan_objective', $item->id);
        $outdata->description = format_text($item->description, FORMAT_HTML);

        print $OUTPUT->render_from_template('shezar_plan/view_plan_component', $outdata) . $out;
    }
    ...
    public function progress_stats() {
        ...
        $progress_str = "{$completedcount}/" . count($objectives) . " " .
            get_string('objectivesmet', 'shezar_plan') . "\n";
