...
$objectivename = get_string($componentname, 'shezar_plan');
$coursename = get_string('courseplural', 'shezar_plan');
$currenturl = new moodle_url('/shezar/plan/components/objective/view.php', array('id' => $id, 'itemid' => $caid));
...
if ($canupdate) {
    ...
    $jsmodule = array(
                                'name' => 'shezar_plan_objective_find_course',
                                'fullpath' => '/shezar/plan/components/objective/find-course.js',
                                'requires' => array('json'));
    $PAGE->requires->js_init_call('M.shezar_plan_objective_find_course.init', array('args' => '{"plan_id":'.$id.',"objective_id":'.$caid.'}'), false, $jsmodule);
...
if ($data = data_submitted() && $canupdate) {
    ...
    if ($action === 'removelinkedcourses' && !$plan->is_complete()) {
        ...
        if ($deletions) {
            shezar_set_notification(get_string('selectedlinkedcoursesremovedfromobjective', 'shezar_plan'), $currenturl, array('class' => 'notifysuccess'));
        } else {
            redirect($currenturl);
        }
        die();
...
if ($data = $mform->get_data()) {
    if (isset($data->edit)) {
        redirect("{$CFG->wwwroot}/shezar/plan/components/objective/edit.php?id={$id}&itemid={$caid}");
    } else if (isset($data->delete)) {
        redirect("{$CFG->wwwroot}/shezar/plan/components/objective/edit.php?id={$id}&itemid={$caid}&d=1");
    }
}
