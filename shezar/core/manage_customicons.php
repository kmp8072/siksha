...
require_once($CFG->dirroot . '/shezar/core/customicon_form.php');
...
$heading = get_string('customicons', 'shezar_core');
$url = new moodle_url('/shezar/core/manage_customicons.php');
...
if ($form->is_cancelled()) {
    ...
    shezar_resize_images_filearea($usercontext->id, 'user', 'draft', $data->course_filemanager, 35, 35, true);
    shezar_resize_images_filearea($usercontext->id, 'user', 'draft', $data->program_filemanager, 35, 35, true);
    foreach ($filetypes as $ft) {
        $formdata = file_postupdate_standard_filemanager($data, $ft, $options, $context, $component, $ft, 0);
    }
    shezar_set_notification(get_string('successuploadicon', 'shezar_core'), null, array('class' => 'notifysuccess'));
}

echo $OUTPUT->header();
echo $OUTPUT->heading($heading);
echo get_string('inforesizecustomicons', 'shezar_core');
