...
// Constants defined to be used in shezar_search_for_value function.
define('shezar_SEARCH_OP_EQUAL', 0);
define('shezar_SEARCH_OP_NOT_EQUAL', 1);
define('shezar_SEARCH_OP_GREATER_THAN', 2);
define('shezar_SEARCH_OP_GREATER_THAN_OR_EQUAL', 3);
define('shezar_SEARCH_OP_LESS_THAN', 4);
define('shezar_SEARCH_OP_LESS_THAN_OR_EQUAL', 5);

// Type of icon.
define ('shezar_ICON_TYPE_COURSE', 'course');
define ('shezar_ICON_TYPE_PROGRAM', 'program');
 ...
 */
function shezar_pop_n(&$items, $number) {
    if (count($items) == 0) {
...
function sql_sequence($field, $items, $type=SQL_PARAMS_QM, $negate = false) {
    ...
    $out = array();
    $outparams = array();
    while ($some_items = shezar_pop_n($items, 1000)) {
        list($sql, $params) = $DB->get_in_or_equal($items, $type, 'param', !$negate);
        $out[] =" $field " . $sql;
 ...
 *               form element
 */
function shezar_select_width_limiter() {
    return array(
        'class' => 'shezar-limited-width'
    );
}
 ...
 */
function shezar_group_records($rs, $field, $property = null) {
    if (!$rs) {
        return false;
    }
    $out = array();
 ...
 */
function shezar_int2vancode($int = 0) {
 ...
 *
 * See {@link shezar_int2vancode()} for details
 *
 ...
 */
function shezar_vancode2int($char = '00') {
 ...
 */
function shezar_increment_vancode($char, $inc = 1) {
    return shezar_int2vancode(shezar_vancode2int($char) + (int) $inc);
}
 ...
 */
function shezar_get_lineage($items, $itemid, $pathsofar = array()) {
    ...
    if (!isset($items[$itemid]) || empty($items[$itemid])) {
        ...
        // keep going
        return shezar_get_lineage($items, $items[$itemid], $pathsofar);
    }
}
/**
 ...
 */
function shezar_search_for_value($arraytosearch, $property, $operator, $searchvalue) {
    $objectsfound = array();
    switch ($operator) {
        case shezar_SEARCH_OP_EQUAL:
            ...
            break;
        case shezar_SEARCH_OP_NOT_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} != $searchvalue;
                }
            ...
            break;
        case shezar_SEARCH_OP_GREATER_THAN:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} > $searchvalue;
                }
            ...
            break;
        case shezar_SEARCH_OP_GREATER_THAN_OR_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} >= $searchvalue;
                }
            ...
            break;
        case shezar_SEARCH_OP_LESS_THAN:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} < $searchvalue;
                }
            ...
            break;
        case shezar_SEARCH_OP_LESS_THAN_OR_EQUAL:
            $objectsfound = array_filter($arraytosearch,
                function ($objtofind) use($searchvalue, $property) {
                    return $objtofind->{$property} <= $searchvalue;
                }
