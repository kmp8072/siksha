...
$error = get_string('error:menuitemcannotremove', 'shezar_core', $title);
$returnurl = new moodle_url('/shezar/core/menu/index.php');

if ($confirm) {
    require_sesskey();
    try {
        $item->delete();
        shezar_set_notification(get_string('menuitem:deletesuccess', 'shezar_core'), $returnurl, array('class' => 'notifysuccess'));
...
if ($children) {
    ...
    foreach ($children as $child) {
        ...
        if ($child->custom == menu::DB_ITEM) {
            ...
            $canremove = false;
            $childtoremove .= html_writer::tag('li', $childnode->get_title() . get_string('error:menuitemcannotremovechild', 'shezar_core'));
        }
    }
    $childtoremove .= html_writer::end_tag('ul');
}

$url = new moodle_url('/shezar/core/menu/delete.php', array('id' => $id));
$PAGE->set_url($url);
...
$url = new moodle_url('/shezar/core/menu/delete.php', array('id' => $id, 'confirm' => 'true'));
...
if ($canremove) {
    ...
    echo html_writer::tag('p', get_string('menuitem:delete', 'shezar_core', $title));
    if ($children) {
        echo html_writer::tag('p', get_string('menuitem:deletechildren', 'shezar_core', $title));
        echo $childtoremove;
    }
} else {
