...
class edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('select', 'parentid', get_string('menuitem:formitemparent', 'shezar_core'), $options);
        $mform->setType('parentid', PARAM_INT);

        $mform->addElement('text', 'title', get_string('menuitem:formitemtitle', 'shezar_core'), 'maxlength="1024"');
        $mform->addRule('title', get_string('required'), 'required', null);
        $mform->setType('title', PARAM_MULTILANG);
        $mform->addHelpButton('title', 'menuitem:formitemtitle', 'shezar_core');
        ...
        $mform->addElement('select', 'visibility', get_string('menuitem:formitemvisibility', 'shezar_core'), $options);
        $mform->setType('visibility', PARAM_INT);

        if ($item->custom == menu::DB_ITEM) {
            $mform->addElement('text', 'url', get_string('menuitem:formitemurl', 'shezar_core'), 'maxlength="255" size="100"');
            $mform->setType('url', PARAM_TEXT);
            $mform->addHelpButton('url', 'menuitem:formitemurl', 'shezar_core');
        }

        $options = array(menu::TARGET_ATTR_SELF, menu::TARGET_ATTR_BLANK);
        $mform->addElement('advcheckbox', 'targetattr', get_string('menuitem:formitemtargetattr', 'shezar_core'), '', null, $options);
        $mform->addHelpButton('targetattr', 'menuitem:formitemtargetattr', 'shezar_core');

        if ($item->id > 0) {
            $strsubmit = get_string('savechanges');
        } else {
            $strsubmit = get_string('menuitem:addnew', 'shezar_core');
        }
