 ...
 *
 * shezar navigation edit form page
 *
...
class rules_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform = & $this->_form;
        /* @var \shezar_core\shezar\menu\menu $item */
        ...
        $mform->addElement('header', "accesscontrols", get_string('menuitem:accesscontrols', 'shezar_core'));

        // The menu for the visibility between rulesets.
        $radiogroup = array();
        $radiogroup[] =& $mform->createElement('radio', 'item_visibility', '',
            get_string('menuitem:withrestrictionany', 'shezar_core'), menu::AGGREGATION_ANY);
        $radiogroup[] =& $mform->createElement('radio', 'item_visibility', '',
            get_string('menuitem:withrestrictionall', 'shezar_core'), menu::AGGREGATION_ALL);
        $mform->addGroup($radiogroup, 'item_visibility',
            get_string('menuitem:restrictaccess', 'shezar_core'), html_writer::empty_tag('br'), false);
        $mform->setDefault('item_visibility', $item->get_setting('visibility_restriction', 'item_visibility'));
        $mform->addHelpButton('item_visibility', 'menuitem:accessmode', 'shezar_core');
        ...
        $mform->addElement('header', 'accessbyroles', get_string('menuitem:accessbyrole', 'shezar_core'));

        if ($enable) {
            $mform->setExpanded('accessbyroles');
        }

        $mform->addElement('checkbox', 'role_enable', '', get_string('menuitem:accessbyrole', 'shezar_core'));
        ...
        // Role aggregation.
        $mform->addElement('select', 'role_aggregation', get_string('menuitem:roleaggregation', 'shezar_core'), $aggregationoptions);
        ...
        $mform->addHelpButton('role_aggregation', 'menuitem:roleaggregation', 'shezar_core');

        $systemcontext = context_system::instance();
        $roles = role_get_names($systemcontext);
        if (!empty($roles)) {
            $contextoptions = array(
                'site' => get_string('menuitem:systemcontext', 'shezar_core'),
                'any' => get_string('menuitem:anycontext', 'shezar_core')
            );

            // Set context for role-based access.
            $mform->addElement('select', 'role_context', get_string('menuitem:context', 'shezar_core'), $contextoptions);
            ...
            $mform->addHelpButton('role_context', 'menuitem:context', 'shezar_core');
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('menuitem:roleswithaccess', 'shezar_core'),
                html_writer::empty_tag('br'), false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            $mform->disabledIf('roles', 'role_enable', 'notchecked');
            $mform->addHelpButton('roles', 'menuitem:roleswithaccess', 'shezar_core');
        } else {
            $mform->addElement('html', html_writer::tag('p', get_string('menuitem:norolesfound', 'shezar_core')));
        }
        // End role selector.
        ...
        $mform->addElement('header', "accessbyaudiences", get_string('menuitem:accessbyaudience', 'shezar_core'));
        if ($enable) {
            $mform->setExpanded('accessbyaudiences');
        }

        $mform->addElement('checkbox', 'audience_enable', '', get_string('menuitem:restrictaccessbyaudience', 'shezar_core'));
        $mform->setType('audience_enable', PARAM_INT);
        $mform->setDefault('audience_enable', $enable);

        // Audience aggregation.
        $mform->addElement('select', 'audience_aggregation', get_string('menuitem:audienceaggregation', 'shezar_core'), $aggregationoptions);
        ...
        $mform->addHelpButton('audience_aggregation', 'menuitem:audienceaggregation', 'shezar_core');
        ...
        $audienceclass = new shezar_cohort_visible_learning_cohorts();
        ...
        $mform->addElement('button', 'cohortsaddvisible', get_string('menuitem:addcohorts', 'shezar_core'));
        ...
        $mform->addElement('header', "accessbypreset", get_string('menuitem:accessbypreset', 'shezar_core'));
        ...
        $mform->addElement('checkbox', 'preset_enable', '', get_string('menuitem:accessbypreset', 'shezar_core'));
        $mform->setType('preset_enable', PARAM_INT);
        $mform->setDefault('preset_enable', $enable);

        // Preset rule aggregation.
        $mform->addElement('select', 'preset_aggregation', get_string('menuitem:presetaggregation', 'shezar_core'), $aggregationoptions);
        ...
        $mform->addHelpButton('preset_aggregation', 'menuitem:presetaggregation', 'shezar_core');
        ...
        $mform->addGroup($presetgroup, 'preset', get_string('menuitem:presetwithaccess', 'shezar_core'),
            html_writer::empty_tag('br'), false);
        $mform->disabledIf('preset', 'accessenabled', 'eq', 0);
        $mform->disabledIf('preset', 'preset_enable', 'notchecked');
        $mform->addHelpButton('preset', 'menuitem:presetwithaccess', 'shezar_core');
    ...
    public function validation($data, $files) {
        ...
        if (!isset($data['role_enable']) && !isset($data['audience_enable']) && !isset($data['preset_enable'])) {
            $errors[] = shezar_set_notification(get_string('error:menuitemrulerequired', 'shezar_core'));
        }

        // Check if a role is selected.
        if (isset($data['role_enable']) && $data['role_enable'] === 1 && !in_array('1', $data['role_activeroles'], TRUE)) {
            $errors['role_enable'] = get_string('error:menuitemrulerolerequired', 'shezar_core');
        }

        // Check if an audience is selected.
        if (isset($data['audience_enable']) && $data['audience_enable'] === 1 && empty($data['cohortsvisible'])) {
            $errors['audience_enable'] = get_string('error:menuitemruleaudiencerequired', 'shezar_core');
        }

        // Check if a preset is selected.
        if (isset($data['preset_enable']) && $data['preset_enable'] === 1 && !in_array('1', $data['preset_active_presets'], TRUE)) {
            $errors['preset_enable'] = get_string('error:menuitemrulepresetrequired', 'shezar_core');
        }
