...
class shezarlib_test extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $teamleaderja = \shezar_job\job_assignment::create_default($this->teamleader->id);
        $managerja = \shezar_job\job_assignment::create_default($this->manager->id,
            array('managerjaid' => $teamleaderja->id));
        \shezar_job\job_assignment::create_default($this->user->id,
            array('managerjaid' => $managerja->id, 'appraiserid' => $this->appraiser->id));
    }

    public function test_shezar_is_manager() {
        $this->resetAfterTest();

        // shezar_is_manager should return true when there is a role assignment for managerid at the user context for userid.
        $this->assertTrue(shezar_is_manager($this->user->id, $this->manager->id));
        $this->assertDebuggingCalled('The function shezar_is_manager has been deprecated since 9.0. Please use \shezar_job\job_assignment::is_managing instead.');

        // shezar_is_manager should return false when there is not role assignment record for managerid on userid's user context.
        $this->assertFalse(shezar_is_manager($this->user->id, $this->invaliduserid));
        $this->assertDebuggingCalled('The function shezar_is_manager has been deprecated since 9.0. Please use \shezar_job\job_assignment::is_managing instead.');
        $this->assertFalse(shezar_is_manager($this->user->id, $this->appraiser->id));
        $this->assertDebuggingCalled('The function shezar_is_manager has been deprecated since 9.0. Please use \shezar_job\job_assignment::is_managing instead.');
        $this->assertFalse(shezar_is_manager($this->user->id, $this->teamleader->id));
        $this->assertDebuggingCalled('The function shezar_is_manager has been deprecated since 9.0. Please use \shezar_job\job_assignment::is_managing instead.');
    }

    public function test_shezar_get_manager() {
        $this->resetAfterTest();

        // Return value should be user object.
        $this->assertEquals(shezar_get_manager($this->user->id)->id, $this->manager->id);
        $this->assertDebuggingCalled('shezar_get_manager has been deprecated since 9.0. You will need to use methods from \shezar_job\job_assignment instead.');

        // shezar_get_manager returns get_record_sql. expecting false here.
        $this->assertFalse(shezar_get_manager($this->teamleader->id));
        $this->assertDebuggingCalled('shezar_get_manager has been deprecated since 9.0. You will need to use methods from \shezar_job\job_assignment instead.');
    }

    public function test_shezar_get_teamleader() {
        $this->resetAfterTest();

        // Return value should be user object.
        $this->assertEquals($this->teamleader->id, shezar_get_teamleader($this->user->id)->id);
        ...
        // shezar_get_manager returns get_record_sql. expecting false here.
        $this->assertFalse(shezar_get_teamleader($this->manager->id));
        $debugging = $this->getDebuggingMessages();
        $this->assertCount(3, $debugging);
        $this->resetDebugging();
    }

    public function test_shezar_get_appraiser() {
        $this->resetAfterTest();

        // Return value should be user object.
        $this->assertEquals($this->appraiser->id, shezar_get_appraiser($this->user->id)->id);
        $this->assertDebuggingCalled('shezar_get_appraiser is deprecated. Use \shezar_job\job_assignment methods instead.');

        // shezar_get_manager returns get_record_sql. expecting false here.
        $this->assertFalse(shezar_get_appraiser($this->manager->id));
        $this->assertDebuggingCalled('shezar_get_appraiser is deprecated. Use \shezar_job\job_assignment methods instead.');
    }

    public function test_shezar_get_staff() {
        $this->resetAfterTest();

        // Expect array of id numbers.
        $this->assertEquals(array($this->user->id), shezar_get_staff($this->manager->id));
        $this->assertDebuggingCalled('shezar_get_staff has been deprecated since 9.0. Use \shezar_job\job_assignment::get_staff_userids instead.');

        // Expect false when the 'managerid' being inspected has no staff.
        $this->assertFalse(shezar_get_staff($this->user->id));
        $this->assertDebuggingCalled('shezar_get_staff has been deprecated since 9.0. Use \shezar_job\job_assignment::get_staff_userids instead.');
    }

    public function test_shezar_create_icon_picker() {
        $this->resetAfterTest();

        // Test with js.
        $mform = new MoodleQuickForm('mform', 'post', '');
        $picker = shezar_create_icon_picker($mform, 'edit', 'course', 'default', 0, '_tst');
        ...
        $mform = new MoodleQuickForm('mform', 'post', '');
        $picker = shezar_create_icon_picker($mform, '', 'course', '', 0, '_tst');
        $this->assertArrayHasKey('currenticon_tst', $picker);
        ...
        // Test with nojs.
        $mform = new MoodleQuickForm('mform', 'post', '');
        $picker = shezar_create_icon_picker($mform, 'edit', 'course', '', 1, '_tst');
        $this->assertArrayHasKey('icon_tst', $picker);
        $this->assertArrayHasKey('currenticon_tst', $picker);
        $this->assertCount(2, $picker);
        ...
        $mform = new MoodleQuickForm('mform', 'post', '');
        $picker = shezar_create_icon_picker($mform, '', 'course', '', 1, '_tst');
        $this->assertArrayHasKey('currenticon_tst', $picker);
        $this->assertCount(1, $picker);
        $this->assertInstanceOf('MoodleQuickForm_static', $picker['currenticon_tst']);
    }

    public function test_shezar_get_sender_from_user_by_id(){
        $this->resetAfterTest();

        $this->assertEquals('admin', shezar_get_sender_from_user_by_id(core_user::SUPPORT_USER)->username);
        $this->assertEquals('admin', shezar_get_sender_from_user_by_id(0)->username);

        // Below return value not expected. Assertion added for awareness of potential issue.
        $this->assertEquals('admin', shezar_get_sender_from_user_by_id('')->username);

        $this->assertEquals('noreply', shezar_get_sender_from_user_by_id(core_user::NOREPLY_USER)->username);
        $this->assertEquals('facetoface', shezar_get_sender_from_user_by_id(\mod_facetoface\facetoface_user::FACETOFACE_USER)->username);
        ...
        $this->assertEquals('testuser1', shezar_get_sender_from_user_by_id($user1->id)->username);
        $this->assertEquals('testuser2@test.com', shezar_get_sender_from_user_by_id($user2->id)->email);

        // user id -46 can't exist
        $this->assertNotInstanceOf('stdClass', shezar_get_sender_from_user_by_id(-46));
        $this->assertFalse(shezar_get_sender_from_user_by_id(-46));
    }

    /**
     * Run shezar_core_update_module_completion_data() when activity uses manual completion.
     *
     ...
     */
    public function test_shezar_core_update_module_completion_data_manual_completion() {
        ...
        shezar_core_update_module_completion_data($cminfo, $moduleinfo, $course, $completion);
        ...
        sleep(1);
        shezar_core_reaggregate_course_modules_completion();
    ...
    /**
     * Run shezar_core_update_module_completion_data() when activity uses completion when viewed.
     *
     * Reaggregation of activity completion is done via shezar_core_reaggregate_course_modules_completion().
     */
    public function test_shezar_core_update_module_completion_data_viewed_completion() {
        $this->resetAfterTest(true);
        global $DB;
    ...
    /**
     * Run shezar_core_update_module_completion_data() when there are values other than those for unlock and delete.
     *
     ...
     */
    public function test_shezar_core_update_module_completion_data_unlock_delete_only() {
        $this->resetAfterTest(true);
        global $DB;
    ...
    /**
     * Similar to test_shezar_core_update_module_completion_data_viewed_completion but we're
     * not using shezar_core_update_module_completion_data so that we can test shezar_reaggregate_course_modules_completion
     * more specifically.
     */
    public function test_shezar_reaggregate_course_modules_completion() {
        $this->resetAfterTest(true);
        global $DB;
        ...
        //  shezar_reaggregate_course_modules_completion() in between.

        $modulecompletion = $DB->get_record('course_modules_completion', array('coursemoduleid' => $cminfo->id, 'userid' => $learner->id));

        // Set to incomplete and add reaggregate flag. This is a typical scenario after using shezar_uncomplete_course_modules_completion.
        ...
        // The results should be the same as calling shezar_reaggregate_course_modules_completion() directly.
        $this->assertEquals(true, $DB->record_exists('course_modules_completion',
            array('coursemoduleid' => $cminfo->id, 'userid' => $learner->id, 'completionstate' => COMPLETION_COMPLETE)));
        $modulecompletion = $DB->get_record('course_modules_completion', array('coursemoduleid' => $cminfo->id, 'userid' => $learner->id));
        // Let's make sure the reaggregate flag is now 0.
    ...
    public function test_shezar_uncomplete_course_modules_completion() {
        $this->resetAfterTest(true);
        global $DB;
        ...
        $now = time();
        shezar_core_uncomplete_course_modules_completion($cm, $completion, $now);
