...
class shezar_core_event_reminder_deleted_testcase extends advanced_testcase {
    public function test_event() {
        ...
        $event = \shezar_core\event\reminder_deleted::create_from_reminder($reminder);
        $event->trigger();
        ...
        $this->assertSame(array('type' => $reminder->type, 'title' => $reminder->title), $event->other);
        $this->assertEquals(new moodle_url('/course/reminders.php', array('courseid' => $course->id)), $event->get_url());

        $this->assertEventContextNotUsed($event);
        $this->assertEventLegacyLogData(array($reminder->courseid, 'course', 'reminder deleted',
            'reminders.php?courseid='.$reminder->courseid, $reminder->title), $event);
    }
}
