 ...
 * Tests search stuff.
 */
class shezar_core_searchlib_testcase extends advanced_testcase {
    public function test_shezar_search_parse_keywords() {
        global $CFG;
        require_once("$CFG->dirroot/shezar/core/searchlib.php");

        $this->assertSame(array('xx'), shezar_search_parse_keywords('xx'));
        $this->assertSame(array('xx '), shezar_search_parse_keywords("'xx '"));
        $this->assertSame(array('xx', 'yy'), shezar_search_parse_keywords('xx yy'));
        $this->assertSame(array('xx', 'yy'), shezar_search_parse_keywords('xx yy '));
        $this->assertSame(array('xx', 'yy'), shezar_search_parse_keywords('   xx  yy '));
        $this->assertSame(array('xx', 'yy zz'), shezar_search_parse_keywords('   xx  "yy zz"'));
        $this->assertSame(array('xx', 'yy zz '), shezar_search_parse_keywords('   xx  "yy zz "'));
    }

    public function test_shezar_search_get_keyword_where_clause() {
        global $CFG, $DB;
        require_once("$CFG->dirroot/shezar/core/searchlib.php");
        ...
        $result = shezar_search_get_keyword_where_clause(array('druh'), array('username'));
        ...
        $result = shezar_search_get_keyword_where_clause(array('prvni'), array('username'));
        $this->assertCount(2, $result);
        $this->assertStringStartsWith('(', $result[0]);
        $this->assertStringEndsWith(')', $result[0]);
        $this->assertInternalType('array', $result[1]);
        ...
        $result = shezar_search_get_keyword_where_clause(array('prv', 'vnii'), array('username'));
        $this->assertCount(2, $result);
        $this->assertStringStartsWith('(', $result[0]);
        $this->assertStringEndsWith(')', $result[0]);
        $this->assertInternalType('array', $result[1]);
        ...
        $result = shezar_search_get_keyword_where_clause(array('rvn'), array('username', 'firstname'));
        $this->assertCount(2, $result);
        $this->assertStringStartsWith('(', $result[0]);
        $this->assertStringEndsWith(')', $result[0]);
        $this->assertInternalType('array', $result[1]);
        ...
        $result = shezar_search_get_keyword_where_clause(array(), array('username'));
        $this->assertCount(2, $result);
