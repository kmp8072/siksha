...
class event_user_position_test extends advanced_testcase {
    ...
    public function test_job_assignment_updated_event() {
        ...
        if ($eventdata1['eventname'] == '\shezar_job\event\job_assignment_updated') {
        ...
        $this->assertEquals('shezar_job', $eventdata1['component']);
        $this->assertEquals('\shezar_job\event\job_assignment_updated', $eventdata1['eventname']);
    ...
    public function test_job_assignment_viewed_event() {
        ...
        $event = \shezar_job\event\job_assignment_viewed::create_from_instance($jobassignment, $coursecontext);
        $event->trigger();
        $data = $event->get_data();
