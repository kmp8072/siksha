...
class shezar_core_date_legacy_testcase extends advanced_testcase {
    public function test_shezar_date_parse_from_format() {
        global $USER;
        ...
        foreach ($timezones as $tz) {
            ...
            foreach ($years as $year) {
                foreach ($dates as $date) {
                    ...
                    $result = shezar_date_parse_from_format($format, $string);
        ...
        foreach ($timezones as $tz) {
            ...
            foreach ($years as $year) {
                foreach ($dates as $date) {
                    ...
                    $expected->setTime($date[2], $date[3], $date[4]);
                    $string = $expected->format($format);
                    $result = shezar_date_parse_from_format($format, $string, true);
                    $this->assertSame($expected->getTimestamp(), $result, "$string in $tz is expected to have different timestamp");
                }
            }
        }
        ...
        $result = shezar_date_parse_from_format('j. n. Y', '158803200', true);
        $this->assertEmpty(0, $result);

        // Test timezone forcing.
        $result = shezar_date_parse_from_format('j. n. Y', '13. 1. 1975', true, 'UTC');
        $this->assertSame(158803200, $result);
    }

    public function test_shezar_get_clean_timezone_list() {
        $phpzones = DateTimeZone::listIdentifiers();

        $zones = shezar_get_clean_timezone_list();
        foreach ($zones as $k => $v) {
            $this->assertInternalType('integer', $k);
            $this->assertContains($v , $phpzones);
        }

        $zones = shezar_get_clean_timezone_list(true);
        foreach ($zones as $k => $v) {
            $this->assertSame($v, $k);
            $this->assertContains($v , $phpzones);
        }
