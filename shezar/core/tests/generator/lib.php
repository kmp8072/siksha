 ...
 * @category test
 */
class shezar_core_generator extends component_generator_base {
    ...
    /** @var number of created shezar field instances */
    protected $shezarfieldcount = 0;
    ...
    /**
     * Create general custom shezar field.
     *
    ...
    public function create_custom_field($prefix, $record) {
        ...
        $record = (object)(array)$record;
        $this->shezarfieldcount++;
        $data = new stdClass();
        $data->id = 0;

        if (empty($prefix)) {
            throw new coding_exception('Must specify custom shezar field type in $type');
        }
        ...
        if (empty($record->datatype)) {
            throw new coding_exception('Must specify custom shezar field data type in $record->datatype');
        }
        $data->datatype = $record->datatype;
        $data->shortname = isset($record->shortname) ? $record->shortname : 'field' . $this->shezarfieldcount;
        if (clean_param($data->shortname, PARAM_ALPHANUM) !== $data->shortname) {
            throw new coding_exception('Invalid custom shezar field shortname in $record->shortname: ' . $data->shortname);
        }
        if (empty($data->typeid)) {
            if ($DB->record_exists($prefix . '_info_field', array('shortname' => $data->shortname))) {
                throw new coding_exception('Duplicate custom shezar field shortname in $record->shortname: ' . $data->shortname);
            }
        } else {
        ...
        $data->fullname = !empty($record->fullname) ? $record->fullname : 'Custom field ' . $this->shezarfieldcount;
        $data->description_editor = array();
        $data->description_editor['text'] = !empty($record->description) ? $record->description : 'Some description ' . $this->shezarfieldcount;
        ...
        if ($record->datatype === 'checkbox') {
            ...
            throw new coding_exception('Invalid custom shezar field type in $record->datatype: ' . $record->datatype);
        }

        require_once($CFG->dirroot.'/shezar/customfield/field/' . $record->datatype. '/define.class.php');
