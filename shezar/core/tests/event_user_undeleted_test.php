...
class shezar_core_event_user_undeleted_testcase extends advanced_testcase {
    public function test_event() {
        $this->resetAfterTest();

        $user = $this->getDataGenerator()->create_user();

        $event = \shezar_core\event\user_undeleted::create_from_user($user);
        $event->trigger();
        ...
        try {
            ...
            $event = \shezar_core\event\user_undeleted::create($data);
            $event->trigger();
    ...
    public function test_undelete_user() {
        ...
        set_config('authdeleteusers', 'partial'); // The old shezar delete that can be partially reverted.
        delete_user($user);
        ...
        $event = $events[0];
        $this->assertInstanceOf('\shezar_core\event\user_undeleted', $event);
        $this->assertSame($user->id, $event->objectid);
