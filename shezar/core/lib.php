...
require_once($CFG->dirroot . '/shezar/core/deprecatedlib.php');
 ...
 */
function shezar_site_version_tracking() {
    global $CFG, $PAGE, $shezar;

    //Params for JS
    $shezar_version = $shezar->version;
    $major_version = substr($shezar->version, 0, 3);
    ...
    $PAGE->requires->strings_for_js(array('unsupported_branch_text', 'supported_branch_text', 'supported_branch_old_release_text'), 'shezar_core', $major_version);
    $PAGE->requires->strings_for_js(array('old_release_text_singular', 'old_release_text_plural', 'old_release_security_text_singular', 'old_release_security_text_plural', 'shezarreleaselink'), 'shezar_core');

    $args = array('args' => '{"shezar_version":"'.$shezar_version.'", "major_version":"'.$major_version.'", "protocol":"'.$protocol.'"}');

    $jsmodule = array(
        'name' => 'shezar_version_tracking',
        'fullpath' => '/shezar/core/js/version_tracking.js',
        'requires' => array('json'));
    $PAGE->requires->js_init_call('M.shezar_version_tracking.init', $args, false, $jsmodule);

}

/**
 * To download the file we upload in shezar_core filearea
 *
 * @param $course
 * @param $cm
 * @param $context
 ...
 */
function shezar_core_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
 ...
 */
function shezar_resize_images_filearea($contextid, $component, $filearea, $itemid, $width, $height, $replace=false) {
    global $CFG, $USER;
...
/**
 * Create recursively shezar menu table
 *
 * @param html_table $table to add data to.
 * @param \shezar_core\shezar\menu\menu $item to render
 ...
 */
function shezar_menu_table_load(html_table &$table, \shezar_core\shezar\menu\menu $item, $depth = 0, $up = false, $down = false) {
    global $OUTPUT;
    ...
    if ($item->id) {
        $node = \shezar_core\shezar\menu\menu::node_instance($item->get_property());
        if ($node === false) {
        ...
        $url = '/shezar/core/menu/index.php';
        ...
        $attributes['class'] = 'shezar_item_depth'.$depth.$dimmed;
        $itemtitle = html_writer::link(new moodle_url('/shezar/core/menu/edit.php',
                array('id' => $item->id)), $itemtitle, $attributes);

        $icons = array();
        // Edit category.
        $icons[] = $OUTPUT->action_icon(
                        new moodle_url('/shezar/core/menu/edit.php', array('id' => $item->id)),
                        new pix_icon('t/edit', $str->edit, 'moodle', array('class' => 'iconsmall')),
                        null, array('title' => $str->edit)
        );
        // Change visibility.
        if ($item->visibility != \shezar_core\shezar\menu\menu::HIDE_ALWAYS) {
            $icons[] = $OUTPUT->action_icon(
        ...
        // Delete item.
        if ($item->custom == \shezar_core\shezar\menu\menu::DB_ITEM) {
            $icons[] = $OUTPUT->action_icon(
                            new moodle_url('/shezar/core/menu/delete.php', array('id' => $item->id)),
        ...
        $table->data[] = new html_table_row(array(
                         ...
                         new html_table_cell(\shezar_core\shezar\menu\menu::get_visibility($item->visibility)),
    ...
    if ($items = $item->get_children()) {
        ...
        foreach ($items as $node) {
            ...
            shezar_menu_table_load($table, $node, $depth+1, $up, $down);
        }
    }
}
 ...
 */
function shezar_get_user_from($fromuser = null) {
    global $CFG, $USER;
 ...
 */
function shezar_get_sender_from_user_by_id($useridfrom) {
    global $DB;
...
function shezar_get_categoryid_with_capability($capability) {
    global $DB;
 ...
 */
function shezar_core_update_module_completion_data($cm, $moduleinfo, $course, $completion) {
    global $DB;
    ...
    if ($completion->is_enabled()) {
        if (!empty($moduleinfo->completionunlocked) && empty($moduleinfo->completionunlockednoreset)) {
            ...
            shezar_core_uncomplete_course_modules_completion($cm, $completion);
            ...
            \shezar_core\event\module_completion_reset::create_from_module($moduleinfo)->trigger();
        }
        ...
        \shezar_core\event\module_completion_criteria_updated::create_from_module($moduleinfo)->trigger();
    }
}
 ...
 * avoid potential Moodle merge conflicts.
 */
function shezar_core_reaggregate_course_modules_completion() {
    global $DB;
    ...
    // cases in shezar and Moodle code.
    $resetsql = '
 ...
 * and flags the records for reaggregation when the shezar_core_reaggregate_course_modules_completion function is next run in cron.
 *
 ...
 */
function shezar_core_uncomplete_course_modules_completion($cm, $completion, $now = null) {
    global $DB, $SESSION;
 ...
 */
function shezar_update_temporary_managers() {
    global $CFG, $DB;

    debugging('shezar_update_temporary_managers has been deprecated since 9.0. Use \shezar_job\job_assignment::update_temporary_managers instead.', DEBUG_DEVELOPER);
