...
/**
 * base assignment classes shezar_assign_core and shezar_assign_core_groups
...
class shezar_assign_core {
     ...
     * Prepare instance
     *
     * @param string $module shezar component name (subfolder in /shezar)
    ...
    public function __construct($module, $moduleinstance, $suffix = '') {
        ...
        $this->basepath = $CFG->dirroot . "/shezar/{$module}/lib/assign{$suffixstr}/";
    }
    ...
    public function load_grouptype($grouptype) {
        ...
        $classname = "shezar_assign_{$module}_grouptype_{$grouptype}{$suffixstr}";
        $classfile = $this->basepath . "groups/{$grouptype}.class.php";

        // Check class file exists.
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'shezar_core', '', $grouptype);
        }
    ...
    public static function get_assignable_grouptypes() {
        ...
        $basepath = $CFG->dirroot . "/shezar/{$module}/lib/assign{$suffixstr}/";
    ...
    public function get_group_instance_name($grouptype, $groupid) {
        ...
        if (!in_array($grouptype, $groups)) {
            print_error('error:invalidgrouptype', 'shezar_core');
        }
    ...
    public function delete_assigned_group($grouptype, $deleteid) {
        if (!in_array($grouptype, $this->get_assignable_grouptypes())) {
            print_error('error:assigncannotdeletegrouptypex', 'shezar_core', $grouptype);
        }
        if ($this->is_locked()) {
            print_error('error:assignmentmoduleinstancelocked', 'shezar_core');
        }
        $grouptypeobj = $this->load_grouptype($grouptype);
    ...
    public function get_group_assignedvia_details($userids) {
        ...
        foreach ($assignedgroups as $assignedgroup) {
            ...
            if (!empty($assignedgroup->includechildren)) {
                $string .= get_string('assignincludechildren', 'shezar_core');
            }
            $grouptypeobj = $this->load_grouptype($assignedgroup->grouptype);
...
abstract class shezar_assign_core_grouptype {
    ...
    public function __construct($assignobject) {
        // Store the whole assignment object from shezar_assign or child class of shezar_assign.
    ...
    public static function load_grouptype($assignobject) {
        ...
        $classname = "shezar_assign_{$assignobject->module}_group_{$assignobject->grouptype}{$suffixstr}";
        // Check group class file exists.
        $classfile = $assignobject->basepath . "groups/{$assignobject->grouptype}.class.php";
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'shezar_core', '', $assignobject->grouptype);
        }
        // Load class file.
        require_once($classfile);
        // Check class exists.
...
class shezar_assign_ui_picker_cohort extends shezar_dialog_content {
    ...
    public function set_parameters($newparams = array()) {
        if (!is_array($newparams)) {
            print_error('error:assignmentbadparameters', 'shezar_core', null, null);
            die();
        }
 ...
 */
class shezar_assign_ui_picker_hierarchy extends shezar_dialog_content_hierarchy_multi {
    public $params = array(
        'equal' => 0,
        'includechildren' => 0,
        'listofvalues' => 1,
...
class shezar_assign_ui_picker_user extends shezar_dialog_content {
    public $handlertype = 'treeview';
    public $params = array(
        'equal' => 1,
        'listofvalues' => 1,
    ...
    public function generate_item_selector($hidden = array(), $selectedids = array()) {
        global $DB;
        $fullname = $DB->sql_concat_join("' '", shezar_get_all_user_name_fields_join('u'));
        // Get users.
 ...
 * @param string $module The shezar module
 ...
 * @return void
 */
function shezar_setup_assigndialogs($module, $itemid, $datatable = false, $notice = "", $suffix = '') {
    global $CFG, $PAGE;
    ...
    $PAGE->requires->strings_for_js(array('assigngroup'), 'shezar_' . $module);
    $jsmodule = array(
        'name' => 'shezar_assigngroups',
        'fullpath' => '/shezar/core/lib/assign/assigngroup_dialog.js',
        'requires' => array('json'));
    ...
    if ($datatable) {
        ...
        $PAGE->requires->js_call_amd('shezar_core/datatable', 'init', $args);
    }
}
