...
// shezar plugins need to implement this capability otherwise they have to do heir own assign stuff, see goals for example.
require_capability("shezar/{$module}:assign{$module}togroup", $sitecontext);

$features = shezar_advanced_features_list();
if (in_array($module, $features)) {
    if (shezar_feature_disabled($module)) {
        die;
    }
}

// Try to find the module class library.
$assignclassname = "shezar_assign_{$module}";
require_once($CFG->dirroot."/shezar/{$module}/lib.php");
...
if (!in_array($grouptype, $grouptypes)) {
    ...
    $a->module = $module;
    print_error('error:assignmentgroupnotallowed', 'shezar_core', null, $a);
}
...
if ($add) {
    ...
    if (!empty($listofvalues) && $grouptypeobj->validate_item_selector($listofvalues)) {
        ...
        $output = $PAGE->get_renderer("shezar_{$module}");
