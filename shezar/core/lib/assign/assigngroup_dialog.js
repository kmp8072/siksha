...
/**
 * This file defines the shezar dialog for adding groups of users
 */

M.shezar_assigngroupdialog = M.shezar_assigngroupdialog || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ == 'undefined') {
            throw new Error('M.shezar_assigngroupdialog.init()-> jQuery dependency required for this module.');
        }

        this.shezar_assigngroupdialog_init_dialogs();
    },

    shezar_assigngroupdialog_init_dialogs: function() {
        var url = M.cfg.wwwroot + '/shezar/' + M.shezar_assigngroupdialog.config.module + '/lib/assign/assigngroup.php';
        var saveurl = url;
        // Dialog & handler for hierarchy picker

        var thandler = new shezarDialog_handler_assigngrouptreeview();
        var tbuttons = {};
        ...
        var tdialog = new shezarDialog(
            ...
            {
                buttons: tbuttons,
                title: '<h2>' + M.util.get_string('assigngroup', 'shezar_' + M.shezar_assigngroupdialog.config.module) + '</h2>'
            },
            url,
            thandler
        );
        tdialog.assigngroup_base_url = url;
        shezarDialogs['assigngrouptreeview'] = tdialog;
        ...
        $(document).on('change', 'select.group_selector', function(event) {
            ...
            var module = M.shezar_assigngroupdialog.config.module;
            var suffix = M.shezar_assigngroupdialog.config.suffix;
            var sesskey = M.shezar_assigngroupdialog.config.sesskey;

            var dialog = shezarDialogs['assigngrouptreeview'];
...
var assigngroup_handler_responsefunc = function(response) {
    ...
    if (response.substr(0,4) == 'DONE') {
        ...
        if (this.responsetype == 'newgroup') {
            ...
            require(['jquery', 'shezar_core/jquery.dataTables-lazy'], function($) {
        ...
        if (notice !== null ) {
            notice.innerHTML = M.shezar_assigngroupdialog.config.notice;
        }
...
shezarDialog_handler_assigngrouptreeview = function() {};
shezarDialog_handler_assigngrouptreeview.prototype = new shezarDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
shezarDialog_handler_assigngrouptreeview.prototype._save = function() {
    // Serialize data
...
shezarDialog_handler_assigngrouptreeview.prototype._update = assigngroup_handler_responsefunc;
