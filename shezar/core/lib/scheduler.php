...
class scheduler {
    ...
    public function get_formatted($user = null) {
        ...
        switch($this->subject->{$this->map['frequency']}) {
            case self::MINUTELY:
                $out .= new lang_string('scheduledminutely', 'shezar_core', $schedule, $lang);
                break;
            case self::HOURLY:
                $out .= new lang_string('scheduledhourly', 'shezar_core', $schedule, $lang);
                break;
            case self::DAILY:
                $date->setTime($schedule, 0, 0);
                date_default_timezone_set('UTC');
                $out .= new lang_string('scheduleddaily', 'shezar_core',
                    strftime(get_string('strftimetime', 'langconfig'), $date->getTimestamp()), $lang);
                core_date::set_default_server_timezone();
                break;
            case self::WEEKLY:
                $out .= new lang_string('scheduledweekly', 'shezar_core',  $calendardays[$schedule]['fullname'], $lang);
                break;
            case self::MONTHLY:
                $dateformat = ($lang === 'en') ? 'jS' : 'j';
                $date->setTime(0, 0, 0);
                $date->setDate(2000, 1, $schedule);
                $out .= new lang_string('scheduledmonthly', 'shezar_core', $date->format($dateformat), $lang);
                break;
        }
