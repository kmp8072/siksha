...
defined('shezar_DIALOG_SEARCH') || die();

require_once("{$CFG->dirroot}/shezar/core/dialogs/search_form.php");
 ...
 *     shezar_dialog_get_search_clause()
 */
switch ($searchtype) {
    /**
     * User search
    ...
    case 'user':
        ...
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $canviewemail ? array_merge ($fields, array('email' => 'email')) : $fields);
    ...
    case 'hierarchy':
        ...
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $fields);
    ...
    case 'coursecompletion':
        ...
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $fields, SQL_PARAMS_NAMED);
    ...
    case 'certification':
        ...
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $fields, SQL_PARAMS_NAMED);
        list($visibilitysql, $visibilityparams) = shezar_visibility_where(null,
                                                                          'p.id',
                                                                          'p.visible',
                                                                          'p.audiencevisible',
                                                                          'p',
    ...
    case 'manager':
        ...
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $allfields);
    ...
    case 'assign_manager':
        ...
        $keywords = shezar_search_parse_keywords($query);
        list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, $allfields, SQL_PARAMS_NAMED, 'u');
    ...
    case 'temporary_manager':
        ...
        $keywords = shezar_search_parse_keywords($query);
        $fields = get_all_user_name_fields(false, '','u.');
        list($searchsql, $params) = shezar_search_get_keyword_where_clause($keywords, $canviewemail ? array_merge ($fields, array('email' => 'u.email')) : $fields);
    ...
    default:
        print_error('invalidsearchtable', 'shezar_core');
}
...
if (strlen($query)) {
    ...
    $strqueryerror = get_string('queryerror', 'shezar_core');
    ...
    if ($total) {
            ...
            if (method_exists($this, 'get_search_items_array')) {
                ...
                foreach ($results as $result) {
                    ...
                    if (isset($result->email)) {
                        ...
                        $item->fullname = get_string('assignindividual', 'shezar_program', $username);
                    } else {
        ...
        $message = get_string('noresultsfor', 'shezar_core', $params);

        if (!empty($frameworkid)) {
            $params->framework = $DB->get_field($shortprefix.'_framework', 'fullname', array('id' => $frameworkid));
            $message = get_string('noresultsforinframework', 'shezar_hierarchy', $params);
        }
