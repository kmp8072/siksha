...
class shezar_dialog_content_plan_evidence extends shezar_dialog_content_plan {
    ...
    public function __construct($component, $planid = 0, $showhidden = false, $userid = 0) {
        ...
        if (empty($this->plan)) {
            echo '<p>' . get_string('noplansavailable', 'shezar_plan') . '</p>';
            die();
        }
    ...
    public function put_search_info(stdClass $search_info, array &$formdata, array $keywords, $parentid = 0, $approved = null) {
        ...
        if (is_array($plans) && count($plans) > 1) {
            $formdata['othertree'] = array_merge(array('0' => get_string('alllearning', 'shezar_plan')), $plans);
        }
    ...
    public function get_search_info(stdClass $search_info, array $keywords) {
        ...
        if ($keywords) {
            list($searchsql, $searchparams) = shezar_search_get_keyword_where_clause($keywords, array('pe.name'),
                SQL_PARAMS_NAMED);
