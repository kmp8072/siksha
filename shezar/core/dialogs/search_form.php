...
class dialog_search_form extends moodleform {
    ...
    function definition() {
        ...
        // Query box
        $query = $this->_customdata['query'];
        $searcharray[] =& $mform->addElement('text', 'query', get_string('search', 'shezar_core'), 'maxlength="254"');
        ...
        if (($searchtype == 'hierarchy' && $showpicker) || !empty($othertree)) {
            if (empty($othertree)) {
                ...
                $options = array(0 => get_string('allframeworks', 'shezar_hierarchy'));
                if ($frameworks) {
            ...
            $attr = array(
                'class' => 'shezar-limited-width-150'
            );

            $item =& $mform->addElement('select', 'frameworkid', '', $options, $attr);
            $item->setHiddenLabel(get_string('framework', 'shezar_core'));
            $mform->setDefault('frameworkid', $frameworkid);
        }
