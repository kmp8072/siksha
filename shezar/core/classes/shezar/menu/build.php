...
class build {
    ...
    // @var object shezar_core_menu object for requested menu item.
    private $item = null;

     /**
     * Adds a item to shezar menu tree
     *
    ...
    /**
     * Create/upgrade/update the shezar main menu navigation
     */
    ...
    protected function children_sync() {
        foreach ($this->children as $classname => $node) {
            if (isset($this->parent[$node->get_parent()])) {
                ...
                $node->set_parentid(0);
                shezar_set_notification(get_string('error:parentnotexists', 'shezar_core', $node->get_parent()));
            }
            try {
                $data = $this->get_data($node);
            } catch (\Exception $e) {
