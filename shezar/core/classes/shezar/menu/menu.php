...
/**
 * Class to store, render and manage shezar navigation category
 *
...
class menu implements \renderable, \IteratorAggregate {
    ...
    // Custom field values.
    // shezar menu default item - delete is forbidden
    const DEFAULT_ITEM = 0;
    // shezarmenu default classname - add sting to database
    const DEFAULT_CLASSNAME = '\shezar_core\shezar\menu\item';
    ...
    /**
    * @var \shezar_core\shezar\menu\menu stores pseudo category with id=0.
    * Use shezar_core_menu::get(0) to retrieve.
    */
    ...
    */
    ...
    public function __get($name) {
        ...
        if (array_key_exists($name, self::$menufields)) {
            if ($this->$name === false) {
                ...
                $rs = $DB->get_record('shezar_navigation', array('id' => $this->id), join(',', array_keys($notretrievedfields)), MUST_EXIST);
        ...
        debugging('Invalid shezar_core_menu property accessed! ' . $name, DEBUG_DEVELOPER);
        return null;
    }
    ...
    /**
     * Constructor.
     * Constructor is protected, use shezar_core_menu::get($id) to retrieve category.
     *
    ...
    /**
     * Returns shezar_core_menu object for requested category.
     ...
     * $data->url = '/shezar/core/tm_sync4.php';
     * $data->parentid = 0|19;
    ...
    public static function sync($data) {
        ...
        $data->id = $DB->insert_record('shezar_navigation', $data);

        // Update path (only possible after we know the category id.
        $data->path = $parent->path . '/' . $data->id;
        $DB->set_field('shezar_navigation', 'path', $data->path, array('id' => $data->id));
        // Add to log.
        \shezar_core\event\menuitem_sync::create_from_item($data->id)->trigger();
    ...
    public function create($data) {
        ...
        \shezar_core\event\menuitem_created::create_from_item($data->id)->trigger();
    ...
    public function update($data) {
        ...
        if (!$this->id) {
            ...
            throw new \moodle_exception('error:findingmenuitem', 'shezar_core');
        }

        if ((int)$data->parentid > 0) {
            // Check if the children exists.
            if ($this->get_children()) {
                throw new \moodle_exception('error:itemhaschildren', 'shezar_core');
            }
        }
        ...
        if ($this->custom == self::DEFAULT_ITEM) {
            // If shezar default menu title changed.
        ...
        $DB->update_record('shezar_navigation', $data);
    ...
    public static function validation($data) {
        ...
        if (isset($data->title) && empty($data->title)) {
            $errors['title'] = get_string('error:menuitemtitlerequired', 'shezar_core');
        }
        if (\core_text::strlen($data->title) > 1024) {
            $errors['title'] = get_string('error:menuitemtitletoolong', 'shezar_core');
        }

        if ($data->custom == self::DB_ITEM) {
            if (isset($data->url) && empty($data->url)) {
                $errors['url'] = get_string('error:menuitemurlrequired', 'shezar_core');
            }
            if (\core_text::strlen($data->url) > 255) {
                $errors['url'] = get_string('error:menuitemurltoolong', 'shezar_core');
            }
            if (!empty($data->url)) {
                $url = $data->url;
                if ($url[0] === '/') {
                ...
                if (!filter_var($url, FILTER_VALIDATE_URL)) {
                    $errors['url'] = get_string('error:menuitemurlinvalid', 'shezar_core');
                }
            }
        }
        ...
        if (isset($data->classname)) {
            if (\core_text::strlen($data->classname) > 255) {
                $errors['classname'] = get_string('error:menuitemclassnametoolong', 'shezar_core');
            }
        }

        if (isset($data->targetattr)) {
            if (\core_text::strlen($data->targetattr) > 100) {
                $errors['targetattr'] = get_string('error:menuitemtargetattrtoolong', 'shezar_core');
            }
        }
    ...
    /**
     * Retrieves number of records from shezar_navigation table
     *
     * @global \moodle_database $DB
    ...
    public static function get_records($whereclause, $params = array()) {
        ...
        $sql = "SELECT tn.". join(',tn.', $fields). " FROM {shezar_navigation} tn WHERE ". $whereclause." ORDER BY tn.sortorder";
        return $DB->get_records_sql($sql, $params);
    }

    /**
     * Retrieves number of records from shezar_navigation table where visibility true
     *
     * @global \moodle_database $DB
     * @return array of stdClass objects
     */
    public static function get_nodes() {
        ...
        $sql = "SELECT
                    tn.". join(',tn.', $fields). ",
                    (SELECT classname FROM {shezar_navigation} WHERE tn.parentid = id) AS parent,
                    (SELECT visibility FROM {shezar_navigation} WHERE tn.parentid = id) AS parentvisibility
                FROM
                    {shezar_navigation} tn
                WHERE
     ...
     *
     * @global \moodle_database $DB
     * @return \shezar_core\shezar\menu\menu[] Array of shezar_core_menu objects indexed by category id
     */
    ...
    public function delete() {
        ...
        // If category is from shezarMenu throw exception, can't delete it.
        if (!$this->custom) {
            throw new \moodle_exception('error:menuitemcannotberemoved', 'shezar_core', null, $this->title);
        }
        ...
        $DB->delete_records('shezar_navigation_settings', array('itemid' => $this->id));
        // Finally delete the category and it's context.
        $DB->delete_records('shezar_navigation', array('id' => $this->id));
        $transaction->allow_commit();

        // Add to log.
        \shezar_core\event\menuitem_deleted::create_from_item($this->id)->trigger();
    ...
    public static function get_visibility_list() {
        return array(
            self::HIDE_ALWAYS => get_string('menuitem:hide', 'shezar_core'),
            self::SHOW_ALWAYS => get_string('menuitem:show', 'shezar_core'),
            self::SHOW_WHEN_REQUIRED => get_string('menuitem:showwhenrequired', 'shezar_core'),
            self::SHOW_CUSTOM => get_string('menuitem:showcustom', 'shezar_core'),
        );
    }
    ...
    /**
     * Set child node as a parent during shezar_upgrade_menu().
     *
     * @global \moodle_database $DB
     */
    public function set_parent() {
        ...
        $DB->set_field('shezar_navigation', 'parentid', '0', array('id' => (int)$this->id));
        // Add to log.
        \shezar_core\event\menuitem_setparent::create_from_item($this->id)->trigger();
    }

    /*
     * Change $custom property to delete a shezar menu item during shezar_upgrade_menu().
     */
    ...
    public static function make_menu_list($excludeid = 0) {
        ...
        $sql = 'SELECT tn.id, tn.title FROM {shezar_navigation} tn WHERE tn.id <> ? AND tn.parentid = 0 ORDER BY tn.sortorder';
    ...
    public static function change_sortorder($id = 0, $up = false) {
        ...
        $swapcategory = $DB->get_records_select('shezar_navigation', $select, $params, $sort, '*', 0, 1);
        $swapcategory = reset($swapcategory);
        if (!$swapcategory) {
            return false;
        }

        $DB->set_field('shezar_navigation', 'sortorder', $swapcategory->sortorder, array('id' => $data->id));
        $DB->set_field('shezar_navigation', 'timemodified', time(), array('id' => $data->id));
        $DB->set_field('shezar_navigation', 'sortorder', $data->sortorder, array('id' => $swapcategory->id));
        $DB->set_field('shezar_navigation', 'timemodified', time(), array('id' => $swapcategory->id));

        \shezar_core\event\menuitem_sortorder::create_from_item($data->id, $action)->trigger();
        return true;
    }
    ...
    public static function change_visibility($id = 0, $hide = false) {
        ...
        if ($hide) {
            $visibility = $DB->get_field('shezar_navigation', 'visibility', array('id' => $itemid), MUST_EXIST);
            $update->visibility = self::HIDE_ALWAYS;
            $update->visibilityold = $visibility;
            $DB->update_record('shezar_navigation', $update);
        } else {
        ...
        \shezar_core\event\menuitem_visibility::create_from_item($itemid, $hide)->trigger();
    ...
    public static function reset_menu() {
        ...
        $DB->delete_records('shezar_navigation');
        // And the menu settings.
        $DB->delete_records('shezar_navigation_settings');
     ...
     * the class \shezar_core\shezar\menu\item, for other items it is
     * the item classname.
     *
     * Returns false if the classname provided is not found.
     *
     * @param object item
     * @return \shezar_core\shezar\menu\item|bool - new instance or false if not found.
     */
    public static function node_instance($item) {
        ...
        if ($item->custom) {
            return new \shezar_core\shezar\menu\item($item);
        }
    ...
    private static function calculate_item_sortorder($defaultsort, $parentid = 0) {
        global $DB;
        $existingsortorders = $DB->get_records_menu('shezar_navigation', null, 'sortorder DESC', 'id,sortorder');
        ...
        if ($parentid > 0) {
            ...
            $existingchildren = $DB->get_records_menu('shezar_navigation', array('parentid' => $parentid), 'sortorder DESC', 'id,sortorder');
            ...
            $toplevelitems = $DB->get_records_menu('shezar_navigation', array('parentid' => 0), 'sortorder DESC', 'id,sortorder');
            $maxvalue = false;
    ...
    public function get_setting($type, $name) {
        global $DB;
        return $DB->get_field('shezar_navigation_settings', 'value',
    ...
    public function get_settings() {
        global $DB;
        $settings = array();
        foreach ($DB->get_records('shezar_navigation_settings', array('itemid' => $this->id)) as $setting) {
    ...
    public function update_setting($type, $name, $value) {
        ...
        $existing = $DB->get_record('shezar_navigation_settings', array('itemid' => $this->id, 'type' => $type, 'name' => $name));
        ...
        if ($existing && $existing->value !== $value) {
            ...
            $record->value = $value;
            $DB->update_record('shezar_navigation_settings', $record);
        } else if (!$existing) {
            ...
            $record->name = $name;
            $record->value = $value;
            $DB->insert_record('shezar_navigation_settings', $record);
        }
    }
    ...
    public static function get_preset_rule_choices() {
        $choices = array(
            'is_logged_in'              => get_string('menuitem:rulepreset_is_logged_in', 'shezar_core'),
            'is_not_logged_in'          => get_string('menuitem:rulepreset_is_not_logged_in', 'shezar_core'),
            'is_guest'                  => get_string('menuitem:rulepreset_is_guest', 'shezar_core'),
            'is_not_guest'              => get_string('menuitem:rulepreset_is_not_guest', 'shezar_core'),
            'is_site_admin'             => get_string('menuitem:rulepreset_is_site_admin', 'shezar_core'),
            'can_view_required_learning'=> get_string('menuitem:rulepreset_can_view_required_learning', 'shezar_core'),
            'can_view_my_team'          => get_string('menuitem:rulepreset_can_view_my_team', 'shezar_core'),
            'can_view_my_reports'       => get_string('menuitem:rulepreset_can_view_my_reports', 'shezar_core'),
            'can_view_certifications'   => get_string('menuitem:rulepreset_can_view_certifications', 'shezar_core'),
            'can_view_programs'         => get_string('menuitem:rulepreset_can_view_programs', 'shezar_core'),
            'can_view_allappraisals'    => get_string('menuitem:rulepreset_can_view_allappraisals', 'shezar_core'),
            'can_view_latestappraisal'  => get_string('menuitem:rulepreset_can_view_latest_appraisal', 'shezar_core'),
            'can_view_appraisal'        => get_string('menuitem:rulepreset_can_view_appraisal', 'shezar_core'),
            'can_view_feedback_360s'    => get_string('menuitem:rulepreset_can_view_feedback_360s', 'shezar_core'),
            'can_view_my_goals'         => get_string('menuitem:rulepreset_can_view_my_goals', 'shezar_core'),
            'can_view_learning_plans'   => get_string('menuitem:rulepreset_can_view_learning_plans', 'shezar_core'),
        );
