 ...
 *
 * @package shezar_core\watcher
 */
class core_course_edit_form {

    /**
     * Hook watcher that extends the course edit form with shezar specific elements.
     *
    ...
    public static function extend_form(edit_form_definition_complete $hook) {
        ...
        // Add the shezar course type before startdate.
    ...
    public static function save_form(edit_form_save_changes $hook) {
        ...
        if (!empty($CFG->audiencevisibility) && has_capability('shezar/coursecatalog:manageaudiencevisibility', $hook->context)) {
    ...
    protected static function add_course_type_controls_to_form(edit_form_definition_complete $hook) {
        ...
        $mform->insertElementBefore(
            $mform->createElement('select', 'coursetype', get_string('coursetype', 'shezar_core'), $coursetypeoptions),
            'startdate'
        );
    }
    ...
    protected static function add_course_icons_controls_to_form(edit_form_definition_complete $hook) {
        ...
        $iconhtml = shezar_icon_picker_preview('course', $courseicon);

        $mform->insertElementBefore(
            $mform->createElement('header', 'iconheader', get_string('courseicon', 'shezar_core')),
            $beforename
        );
        if ($nojs == 1) {
            $mform->insertElementBefore(
                $mform->createElement('static', 'currenticon', get_string('currenticon', 'shezar_core'), $iconhtml),
                $beforename
            );
            $path = $CFG->dirroot . '/shezar/core/pix/courseicons';
            $replace = array(
                '.png' => '',
                '_' => ' ',
                '-' => ' '
            ...
            $mform->insertElementBefore(
                $mform->createElement('select', 'icon', get_string('icon', 'shezar_core'), $icons),
                $beforename
            );
            ...
            $buttonhtml = \html_writer::empty_tag('input', array(
                'type' => 'button',
                'value' => get_string('chooseicon', 'shezar_program'),
                'id' => 'show-icon-dialog'
            ));
            ...
            $mform->setType('icon', PARAM_TEXT);
            $mform->insertElementBefore(
                $mform->createElement('static', 'currenticon', get_string('currenticon', 'shezar_core'), $iconhtml . $buttonhtml),
                $beforename
            );
        }
    ...
    protected static function add_enrolled_learning_controls_to_form(edit_form_definition_complete $hook) {
        ...
        $beforename = 'groups';
        $mform->insertElementBefore(
            $mform->createElement('header','enrolledcohortshdr', get_string('enrolledcohorts', 'shezar_cohort')),
            $beforename
        );
        ...
        if (empty($course->id)) {
            ...
            $cohorts = shezar_cohort_get_course_cohorts($course->id, null, 'c.id');
        ...
        $cohortsclass = new \shezar_cohort_course_cohorts(COHORT_ASSN_VALUE_ENROLLED);
        ...
        $mform->insertElementBefore(
            $mform->createElement('button', 'cohortsaddenrolled', get_string('cohortsaddenrolled', 'shezar_cohort')),
            $beforename
        );
    ...
    protected static function add_audience_visibility_controls_to_form(edit_form_definition_complete $hook) {
        ...
        $mform->insertElementBefore(
            $mform->createElement('header', 'visiblecohortshdr', get_string('audiencevisibility', 'shezar_cohort')),
            $beforename
        );
        $mform->insertElementBefore(
            $mform->createElement('select', 'audiencevisible', get_string('visibility', 'shezar_cohort'), $COHORT_VISIBILITY),
            $beforename
        );
        ...
        if (empty($course->id)) {
            ...
            $cohorts = shezar_cohort_get_visible_learning($course->id);
            $cohorts = !empty($cohorts) ? implode(',', array_keys($cohorts)) : '';
        }
        ...
        $mform->insertElementBefore(
            $mform->createElement('button', 'cohortsaddvisible', get_string('cohortsaddvisible', 'shezar_cohort')),
            $beforename
        );
    ...
    protected static function initialise_enrolled_learning_js(edit_form_display $hook) {
        ...
        if (empty($course->id)) {
            ...
            $enrolledselected = shezar_cohort_get_course_cohorts($course->id, null, 'c.id');
            $enrolledselected = !empty($enrolledselected) ? implode(',', array_keys($enrolledselected)) : '';
        }

        $PAGE->requires->strings_for_js(array('coursecohortsenrolled'), 'shezar_cohort');
        $jsmodule = array(
            'name' => 'shezar_cohortdialog',
            'fullpath' => '/shezar/cohort/dialog/coursecohort.js',
            'requires' => array('json'));
        $args = array(
        ...
        $PAGE->requires->js_init_call('M.shezar_coursecohort.init', $args, true, $jsmodule);
    }
    ...
    protected static function initialise_audience_visibility_js(edit_form_display $hook) {
        ...
        if (empty($course->id)) {
            ...
            $instancetype = COHORT_ASSN_ITEMTYPE_COURSE;
            $instanceid = $course->id;
            $visibleselected = shezar_cohort_get_visible_learning($course->id);
            $visibleselected = !empty($visibleselected) ? implode(',', array_keys($visibleselected)) : '';
        }

        $PAGE->requires->strings_for_js(array('coursecohortsvisible'), 'shezar_cohort');
        $jsmodule = array(
    ...
    protected static function save_enrolled_learning_changes(edit_form_save_changes $hook) {
        ...
        $currentcohorts = shezar_cohort_get_course_cohorts($courseid, null, 'c.id, e.id AS associd');
        ...
        if ($todelete = array_diff(array_keys($currentcohorts), $newcohorts)) {
            ...
            foreach ($todelete as $cohortid) {
                shezar_cohort_delete_association($cohortid, $currentcohorts[$cohortid]->associd, COHORT_ASSN_ITEMTYPE_COURSE);
            }
            $changesmade = true;
        }
        ...
        if ($newcohorts = array_diff($newcohorts, array_keys($currentcohorts))) {
            ...
            foreach ($newcohorts as $cohortid) {
                ...
                shezar_cohort_add_association($cohortid, $courseid, COHORT_ASSN_ITEMTYPE_COURSE);
            }
            $changesmade = true;
        }
    ...
    protected static function save_audience_visibility_changes(edit_form_save_changes $hook) {
        ...
        if (!has_capability('shezar/coursecatalog:manageaudiencevisibility', $hook->context)) {
        ...
        $visiblecohorts = shezar_cohort_get_visible_learning($courseid);
        ...
        if ($newvisible = array_diff($newvisible, array_keys($visiblecohorts))) {
            ...
            foreach ($newvisible as $cohortid) {
                ...
                shezar_cohort_add_association($cohortid, $courseid, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
            }
            $changesmade = true;
        }
        \cache_helper::purge_by_event('changesincourse');
