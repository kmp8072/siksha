...
/**
 * Setup error/exception handlers for shezar
 *
 * @access  public
 * @return  void
 */
function shezar_setup_error_handlers() {
    set_error_handler('shezar_error_handler');
    set_exception_handler('shezar_exception_handler');
}


/**
 * shezar error handler
 *
 * @access  public
 ...
 * @return  bool
 */
function shezar_error_handler($errno, $errstr, $errfile = '', $errline = 0, $errcontext = array()) {
    global $CFG, $DB, $shezar;
    ...
    // Only log error in database if shezar is installed and it would recorded at "DEVELOPER" level
    ...
    if (!empty($CFG->local_postinst_hasrun) && ($errno & DEBUG_DEVELOPER) && $insertcount < 100) {
        ...
        if (!in_array($hash, $previous_errors)) {
            ...
            $error->version = $shezar->version;
            $error->build = $shezar->build;
...
/**
 * shezar exception handler
 *
 * @access  public
 * @param   $exception  Exception
 * @return  bool
 */
function shezar_exception_handler($exception) {
    ...
    $result = shezar_error_handler($errno, $errstr, $errfile, $errline);
 ...
 */
function shezar_crop_error_log() {
    global $DB;
