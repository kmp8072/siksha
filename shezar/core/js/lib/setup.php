 ...
 */
define('shezar_JS_DIALOG',         1);
define('shezar_JS_TREEVIEW',       2);
define('shezar_JS_DATEPICKER',     3);
define('shezar_JS_PLACEHOLDER',    4);
define('shezar_JS_ICON_PREVIEW',   5);
define('shezar_JS_UI',             6);
/**
 * @deprecated deprecated since 9.0
 */
define('shezar_JS_DATATABLES',     7);
/**
...
function local_js($options = array()) {
    ...
    if (count($options) === 0) {
        debugging('shezar loads jQuery on every page since 2.9.0', DEBUG_DEVELOPER);
        return;
    }
    ...
    // If UI
    if (in_array(shezar_JS_UI, $options)) {

        $PAGE->requires->js('/shezar/core/js/lib/jquery-ui-1.10.4.custom.min.js');

    }

    // If dialog
    if (in_array(shezar_JS_DIALOG, $options)) {

        $PAGE->requires->js('/shezar/core/js/lib/jquery-ui-1.10.4.custom.min.js');

        // Load required strings into the JS global namespace in the form
        // M.str.COMPONENT.IDENTIFIER, eg; M.str.shezar_core['save']. Can also
        ...
        $PAGE->requires->strings_for_js(array('save', 'delete'), 'shezar_core');
        $PAGE->requires->strings_for_js(array('ok', 'cancel'), 'moodle');

        // Include the shezar_dialog JS module. Args supplied to the module's
        ...
        // which is then available via M.shezar_dialog.config.id once the module
        ...
        $jsmodule = array(
                'name' => 'shezar_dialog',
                'fullpath' => '/shezar/core/js/lib/' . $directory . '/shezar_dialog' . $min . '.js',
                'requires' => array('json'));
        $PAGE->requires->js_init_call('M.shezar_dialog.init', null, false, $jsmodule);
    }

    // If treeview enabled
    if (in_array(shezar_JS_TREEVIEW, $options)) {

        $PAGE->requires->js('/shezar/core/js/lib/' . $directory . '/jquery.treeview' . $min . '.js');

    }

    // If datepicker enabled
    if (in_array(shezar_JS_DATEPICKER, $options)) {
        ...
        $file = "/shezar/core/js/lib/i18n/jquery.ui.datepicker-{$lang}.js";
    ...
    if (in_array(shezar_JS_PLACEHOLDER, $options)) {
        $PAGE->requires->js('/shezar/core/js/lib/' . $directory . '/jquery.placeholder' . $min . '.js');
        $PAGE->requires->js('/shezar/core/js/lib/' . $directory . '/load.placeholder' . $min . '.js');

    }

    // If Icon preview is enabled
    if (in_array(shezar_JS_ICON_PREVIEW, $options)) {

        $PAGE->requires->js('/shezar/core/js/icon.preview.js');

    }

    if (in_array(shezar_JS_DATATABLES, $options)) {
        debugging('shezar_JS_DATATABLES has been deprecated and may cause unexpected results - please convert your JS to an AMD version', DEBUG_DEVELOPER);
        $PAGE->requires->js('/shezar/core/js/lib/jquery.dataTables' . $min . '.js');
    }
}
...
function build_datepicker_js($selector, $dateformat=null) {
    ...
    if (empty($dateformat)) {
        $dateformat = get_string('datepickerlongyeardisplayformat', 'shezar_core');
    }
    $button_img = array('t/calendar', 'shezar_core');
    $args = array($selector, $dateformat, $button_img);
    $PAGE->requires->js_init_call('M.shezar_core.build_datepicker', $args);
}
...
function build_treeview($elements, $error_string, $hierarchy = null, $disabledlist = array()) {
    ...
    $maxitems = shezar_DIALOG_MAXITEMS;
    ...
    if (is_array($elements) && !empty($elements)) {
        ...
        if(count($elements) > $maxitems) {
            ...
            $html .= get_string('error:morethanxitemsatthislevel', 'shezar_core', $maxitems);
            $html .= ' <a href="#search-tab" onclick="$(\'#tabs\').tabs(\'select\', 1);return false;">';
            $html .= get_string('trysearchinginstead', 'shezar_core');
            $html .= '</a>';
...
function build_nojs_treeview($elements, $error_string, $actionurl, $actionparams, $expandurl, $expandparams, $parents = array(), $disabledlist = array()) {
    ...
    if (is_array($elements) && !empty($elements)) {
        ...
        foreach ($elements as $element) {
            ...
            $cells = array();
            $cells[] = new html_table_cell($OUTPUT->single_button(new moodle_url($actionurl, $params), get_string('assign','shezar_hierarchy'), 'get', array('disabled' => array_key_exists($element->id, $disabledlist))));
...
function build_nojs_breadcrumbs($hierarchy, $parentid, $url, $urlparams, $allfws=true) {
    ...
    if ($allfws) {
        $first = false;
        $html .= html_writer::tag('li', html_writer::link($nofwurl, get_string('allframeworks','shezar_hierarchy')), array('class' => 'first'));
    }
    if ($parentid) {
...
function build_nojs_frameworkpicker($hierarchy, $url, $urlparams) {
    ...
    if ($fws = $DB->get_records($hierarchy->shortprefix.'_framework', null, 'sortorder')) {
        ...
        $out .= html_writer::tag('p', get_string('pickaframework','shezar_hierarchy'));
        ...
        print_error($hierarchy->prefix . 'noframeworks', 'shezar_hierarchy');
    }
}
...
function build_nojs_jobassignmentpicker($url, $urlparams) {
    ...
    if ($positions) {
        ...
        $html .= get_string('chooseposition','shezar_hierarchy');
        ...
        print_error('nopositionsassigned', 'shezar_hierarchy');
    }
    return $html;
}
