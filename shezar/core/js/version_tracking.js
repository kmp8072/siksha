...
M.shezar_version_tracking = M.shezar_version_tracking || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_version_tracking.init()-> jQuery dependency required for this module to function.');
        }

        $.ajax({
            url: this.config.protocol + '://community.totaralms.com/admin/current_release.php',
            dataType: 'jsonp',
            data: {'version': this.config.shezar_version},
            ...
            success: function(data) {
                ...
                if (currentbranch == BRANCH_UNSUPPORTED) {
                    var message = M.util.get_string('unsupported_branch_text', 'shezar_core', module.config.major_version);
                ...
                if (currentbranch == BRANCH_SUPPORTED && currentrelease != RELEASE_NOT_CURRENT) {
                    var message = M.util.get_string('supported_branch_text', 'shezar_core', module.config.major_version);
                    message = message.replace('[[CURRENT_MAJOR_VERSION]]', current_major_version);
                    module.display_message(message);
                    return true;
                }
                ...
                var message = (alltypes_count == 1) ?
                    M.util.get_string('old_release_text_singular', 'shezar_core') :
                    M.util.get_string('old_release_text_plural', 'shezar_core');
                message = message.replace('[[ALLTYPES_COUNT]]', alltypes_count);
                if (security_count > 0) {
                    message += (security_count == 1) ?
                        M.util.get_string('old_release_security_text_singular', 'shezar_core') :
                        M.util.get_string('old_release_security_text_plural', 'shezar_core');
                    message = message.replace('[[SECURITY_COUNT]]', security_count);
                }

                if (currentbranch == BRANCH_SUPPORTED) {
                    message += M.util.get_string('supported_branch_old_release_text', 'shezar_core', module.config.major_version);
                    message = message.replace('[[CURRENT_MAJOR_VERSION]]', current_major_version);
                }
                module.display_message(message);
                return true;
    ...
    display_message: function(text) {
        text += M.util.get_string('shezarreleaselink', 'shezar_core');
        $('div.shezar-copyright p:last').append('<p>'+text+'</p>');
        return true;
    }
};
