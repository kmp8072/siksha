 ...
 */
M.shezar_iconpicker = M.shezar_iconpicker || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_iconpicker.init()-> jQuery dependency required for this module to function.');
        }
        ...
        $(btnids).each(function(idx, btnid) {
            var handler = new shezarDialog_handler_selectable(M.shezar_iconpicker.config.selected_icon);
            ...
            var dialog = new shezarDialog(
                    ...
                    {
                        buttons: buttonsObj,
                        title: '<h2>' + M.util.get_string('chooseicon', 'shezar_program') + '</h2>'
                    },
                    M.cfg.wwwroot + '/shezar/core/icons.php?type=' + M.shezar_iconpicker.config.type,
                    handler
                );
            ...
            shezarDialogs['icon-dialog' + suffix] = dialog;
