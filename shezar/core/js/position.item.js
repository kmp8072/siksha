 ...
 */
M.shezar_positionitem = M.shezar_positionitem || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_positionitem.init()-> jQuery dependency required for this module to function.');
        }
        ...
        //
        (function() {
            var url = M.cfg.wwwroot+ '/shezar/hierarchy/prefix/goal/assign/';
            var saveurl = url + 'assign.php';
            var findurl = url + 'find.php';

            // Dialog & handler for hierarchy picker.
            var thandler = new shezarDialog_handler_assigngoaltreeview();
            var tbuttons = {};
            ...
            var tdialog = new shezarDialog(
                ...
                {
                    buttons: tbuttons,
                    title: '<h2>' + M.util.get_string('assigngoals', 'shezar_hierarchy') + '</h2>'
                },
                findurl,
                thandler
            );
            tdialog.assigngoal_base_url = url;
            shezarDialogs['assigngoaltreeview'] = tdialog;
            ...
            $(document).on('click', '#show-assignedgoals-dialog', function(event) {
                ...
                // Open default url
                var select = $(this);
                var id = M.shezar_positionitem.config.id;
                var sesskey = M.shezar_positionitem.config.sesskey;

                var dialog = shezarDialogs['assigngoaltreeview'];
                ...
                dialog.default_url = findurl + '?assigntype=3&assignto=' + id;
                dialog.saveurl = saveurl + '?assigntype=3&assignto=' + id + '&sesskey=' + sesskey + '&add=1';
                dialog.open();
        ...
        ///
        (function() {
            var url = M.cfg.wwwroot+ '/shezar/hierarchy/prefix/position/assigncompetency/';
            var id = M.shezar_positionitem.config.id;
            var fid = M.shezar_positionitem.config.frameworkid;
            shezarMultiSelectDialog(
                'assignedcompetencies',
                M.util.get_string('assigncompetencies', 'shezar_hierarchy'),
        ...
        (function() {
            var url = M.cfg.wwwroot+ '/shezar/hierarchy/prefix/position/assigncompetencytemplate/';
            var id = M.shezar_positionitem.config.id;
            var fid = M.shezar_positionitem.config.frameworkid;
            shezarMultiSelectDialog(
                'assignedcompetencytemplates',
                M.util.get_string('assigncompetencytemplate', 'shezar_hierarchy'),
                url+'find.php?assignto='+id+'&frameworkid='+fid+'&add=',
                url+'assign.php?assignto='+id+'&frameworkid='+fid+'&deleteexisting=1&add='
            );
        })();
        ...
        $(shezarDialogs.assignedcompetencies.dialog).bind('ajaxComplete', function(event) {
...
shezarDialog_handler_assigngoaltreeview = function() {};
shezarDialog_handler_assigngoaltreeview.prototype = new shezarDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
shezarDialog_handler_assigngoaltreeview.prototype._save = function() {
    // Serialize data
...
shezarDialog_handler_assigngoaltreeview.prototype._update = assigngoal_handler_responsefunc;
