...
class shezar_generator_site_backend extends tool_generator_site_backend {
    ...
    */
    ...
    /**
     * @var shezar_cohort_generator Data generator for hierarchy
     */
    protected $cohort_generator;

    /**
     * @var shezar_hierarchy_generator Data generator for hierarchy
     */
    protected $hierarchy_generator;

    /**
     * @var shezar_program_generator Data generator for program
     */
    ...
    public function make() {
        ...
        //Set up basic shezar users, positions, hierarchies etc.
        $this->create_shezar_objects();
        ...
        while ($coursesenrolled < $numcourses) {
            $courseid = shezar_generator_util::get_random_record_id('course');
            if ($courseid == $site->id) {
        ...
        $plan_generator = new shezar_generator_learning_plan_backend($this->size);
    ...
    */
    protected function set_custom_generators() {
        $this->hierarchy_generator = $this->generator->get_plugin_generator('shezar_hierarchy');
        $this->cohort_generator = $this->generator->get_plugin_generator('shezar_cohort');
    ...
    /**
     * Create shezar objects, such as: audiences, positions, organisations, managers,
     * assign primary position to students, create programs, etc.
     *
     */
    protected function create_shezar_objects() {
        // Set custom data generators.
        ...
        // First competency.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_FRAMEWORK_COMPETENCY;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        $name .= ' ' . shezar_generator_util::get_next_record_number('comp_framework', 'fullname', $name);
        ...
        $name = shezar_hierarchy_generator::DEFAULT_NAME_HIERARCHY_COMPETENCY;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        ...
        // Then organisation.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_FRAMEWORK_ORGANISATION;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        $name .= ' ' . shezar_generator_util::get_next_record_number('org_framework', 'fullname', $name);
        $data = array ('fullname' => $name);
        ...
        $name = shezar_hierarchy_generator::DEFAULT_NAME_HIERARCHY_ORGANISATION;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        ...
        // Create a base organisation hierarchy name.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_HIERARCHY_POSITION;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        ...
        // Then goals.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_FRAMEWORK_GOAL;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
        $name .= ' ' . shezar_generator_util::get_next_record_number('goal_framework', 'fullname', $name);
        $data = array ('fullname' => $name);
        // Create goal framework.
        $goalframework = $this->hierarchy_generator->create_framework('goal', $data);
        // Create a base goal hierarchy name.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_HIERARCHY_GOAL;
        $name = trim($name) . ' ' . shezar_generator_util::get_size_name($this->size);
    ...
    protected function run_create_command($component, $size) {
        ...
        $command = "php shezar/generator/cli/maketest{$component}.php";
    ...
    private function create_users($usertype, $count) {
        ...
        $nextnumber = shezar_generator_util::get_next_record_number('user', 'username', $username);
    ...
    private function create_user_accounts($first, $last, $usertype) {
        ...
        for ($number = $first; $number <= $last; $number++, $done++) {
            ...
            // Create user account.
            $record = array('firstname' => get_string('firstname', 'shezar_generator', $usertype),
                    'lastname' => get_string('lastname', 'shezar_generator', $number),
                    'username' => $username);
    ...
    private function create_job_assignments($users, $managerids, $positionids, $organisationids) {
        ...
        foreach ($users as $user) {
            ...
            \shezar_job\job_assignment::create_default($user, $data);
            $this->dot($done, $count);
        }
        $this->end_log();
    }
