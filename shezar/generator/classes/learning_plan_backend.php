...
class shezar_generator_learning_plan_backend extends tool_generator_backend {
    ...
    /**
     * @var shezar_plan_generator Learning plan data generator.
     */
    ...
    public function make() {
        ...
        $this->create_shezar_objects($learning_plan);
    ...
    protected function set_custom_generators() {
        $this->learning_plan_generator = $this->generator->get_plugin_generator('shezar_plan');
    }
    ...
    protected function create_learning_plan() {
        ...
        if ($this->name) {
            ...
            $default_name = shezar_plan_generator::DEFAULT_NAME;
        }

        // Create the name we want to use.
        $default_name = trim($default_name) . ' ' . shezar_generator_util::get_size_name($this->size);
        $default_name = $default_name . ' ' . shezar_generator_util::get_next_record_number('dp_plan', 'name', $default_name);
        ...
        $record['userid'] = shezar_generator_util::get_random_record_id('user', false, array(1, 2));
    ...
    /**
     * Create shezar objects, assign courses, competencies, programs and create objectives.
     *
     * @todo Competencies need to be assigned need completing.
     */
    protected function create_shezar_objects($learning_plan) {
        global $DB;
        ...
        for ($i = 0; $i < $numitems; $i++) {
            ...
            $course_id = shezar_generator_util::get_random_record_id('course',false,array(1));
            if ($course_id) {
                if (shezar_generator_util::get_random_act($this->component_chance_percentage['course'])) {
        ...
        for ($i = 0; $i < $numitems; $i++) {
            ...
            $competency_id = shezar_generator_util::get_random_record_id('comp');
            if ($competency_id) {
                if (shezar_generator_util::get_random_act($this->component_chance_percentage['competency'])) {
        ...
        $default_name = shezar_plan_generator::DEFAULT_NAME_OBJECTIVE;
        $default_name = trim($default_name) . ' ' . shezar_generator_util::get_size_name($this->size);
        $name_number = shezar_generator_util::get_next_record_number('dp_plan_objective', 'fullname', $default_name);

        $objective_data = array ();
        // Create some objectives for the learning plan.
        for ($i = 0; $i < $numitems; $i++) {
            // Randomly add an objective to the learning plan.
            if (shezar_generator_util::get_random_act($this->component_chance_percentage['objective'])) {
        ...
        for ($i = 0; $i < $numitems; $i++) {
            ...
            $program_id = shezar_generator_util::get_random_record_id('prog');
            if ($program_id) {
                if (shezar_generator_util::get_random_act($this->component_chance_percentage['program'])) {
