...
class shezar_generator_hierarchy_backend extends tool_generator_backend {
    ...
    /**
     * @var shezar_hierarchy_generator Hierarchy data generator.
     */
    protected $shezar_hierarchy_generator;
    ...
    public function assign_hierarchies() {
        ...
        foreach ($this->hierarchy_type_prefix as $type => $prefix) {
            ...
            $this->shezar_hierarchy_generator->$function($size);
        }
    }
    ...
    protected function set_custom_generators() {
        $this->shezar_hierarchy_generator = $this->generator->get_plugin_generator('shezar_hierarchy');
    }


    /**
     * Create shezar objects,
     */
    ...
    protected function create_framework() {
        ...
        if ($this->name) {
            ...
            $default_name = constant('shezar_hierarchy_generator::DEFAULT_NAME_FRAMEWORK_' . strtoupper($this->type));
        }
        ...
        $default_name = $default_name . ' ' . shezar_generator_util::get_next_record_number($shortprefix . '_framework', 'fullname', $default_name);
        ...
        $result = $this->shezar_hierarchy_generator->create_framework($this->type, $record);
    ...
    protected function create_hierarchies($framework_id) {
        ...
        if ($this->name) {
            ...
            $default_name = constant('shezar_hierarchy_generator::DEFAULT_NAME_HIERARCHY_' . strtoupper($this->type));
        }
        ...
        $hierarchy_ids = $this->shezar_hierarchy_generator->create_hierarchies($framework_id,
                                                                                $this->type,
        ...
        foreach ($hierarchy_ids as $hierarchy_id) {
            $hierarchy_children_ids = $this->shezar_hierarchy_generator->create_hierarchies($framework_id,
                                                                                        $this->type,
                                                                                        $this->hierarchy_size_quantities[$this->size],
                                                                                        $default_name,
                                                                                        $this->hierarchy_chance_percentage[$this->type],
