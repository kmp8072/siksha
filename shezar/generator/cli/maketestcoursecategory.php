...
if (!empty($options['help']) || empty($options['size'])) {
    echo "
Utility to create standard test course category.

Not for use on live sites! Only works if debugging is set to DEVELOPER level.

Options:
--size           Size of course category to create XS, S, M, L, XL, or XXL (required)
--name           Name of course category to create
...
\$ sudo -u www-data php shezar/generator/cli/maketestcoursecategory.php --name=SIZE_S --size=S
";
...
// Check size.
$size = shezar_generator_course_backend::size_for_name($sizename);

// Do backend code to generate course category.
$backend = new shezar_generator_course_category_backend($size, $name, $fixeddataset, $filesizelimit, empty($options['quiet']));
$id = $backend->make();
