...
$can_edit = has_capability('shezar/hierarchy:managegoalassignments', $context)
...
if ($context->contextlevel == CONTEXT_SYSTEM) {
    ...
    $PAGE->set_url('/shezar/cohort/goals.php', array('id' => $id));
    $PAGE->set_heading($COURSE->fullname);
    $PAGE->set_title($cohort->name . ' : ' . get_string('goals', 'shezar_hierarchy'));
...
$jsmodule = array(
        'name' => 'shezar_cohort',
        'fullpath' => '/shezar/core/js/cohort.js',
        'requires' => array('json', 'shezar_core'));
$PAGE->requires->js_init_call('M.shezar_cohort.init',
    $args, false, $jsmodule);
...
// Goals header.
echo $OUTPUT->heading(get_string('goalsassigned', 'shezar_hierarchy'), 3);

// Add goal(s) button.
$button_form = '';
if ($can_edit) {
    // Needs to be done manually (not with single_button) to get correct ID on input button element.
    $add_button_text = get_string('addgoal', 'shezar_hierarchy');
    $add_goal_url = new moodle_url('/shezar/hierarchy/prefix/goal/assign/find.php', array('assignto' => $cohort->id));
