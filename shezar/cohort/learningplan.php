...
require_capability('shezar/plan:cancreateplancohort', $context);
...
$url = new moodle_url('/shezar/cohort/learningplan.php', array('id' => $id));
admin_externalpage_setup('cohorts', '', null, $url, array('pagelayout'=>'report'));

$cohort = $DB->get_record('cohort', array('id' => $id), '*', MUST_EXIST);

$PAGE->set_context($context);
$PAGE->set_url('/shezar/cohort/learningplan.php', array('id' => $id));
...
$PAGE->requires->strings_for_js(array('confirmcreateplans'), 'shezar_plan');
$PAGE->requires->strings_for_js(array('continue', 'cancel'), 'moodle');
$args = array('args' => '{"id":"'.$cohort->id.'"}');
$jsmodule = array(
        'name' => 'shezar_cohortplans',
        'fullpath' => '/shezar/cohort/dialog/learningplan.js',
        'requires' => array('json', 'shezar_core'));
$PAGE->requires->js_init_call('M.shezar_cohortplans.init', $args, false, $jsmodule);

$form = new cohort_learning_plan_settings_form($currenturl, array('data' => $cohort));

// Get a learning plan config object.
$planconfig = \shezar_cohort\learning_plan_config::get_config($id);
...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        ...
        $createdplancount = \shezar_cohort\learning_plan_helper::create_plans($planconfig);

        shezar_set_notification(get_string('saved', 'shezar_cohort'), null, array('class' => 'notifysuccess'));
        if ($createdplancount) {
            shezar_set_notification(get_string('successfullycreatedplans', 'shezar_cohort', $createdplancount), null, array('class' => 'notifysuccess'));
        }
    }
}
...
echo $OUTPUT->heading(get_string('createlpforaudience', 'shezar_cohort'));

echo get_string('createlpforaudienceblurb', 'shezar_cohort');

echo $form->display();

$tableheaders = array(
    get_string('template', 'shezar_core'),
    get_string('user'),
    get_string('date'),
    get_string('planstatus', 'shezar_plan'),
    get_string('numaffectedusers', 'shezar_plan'),
    get_string('manuallycreated', 'shezar_plan'),
    get_string('autocreated', 'shezar_plan'),
    get_string('complete')
);
$tablecolumns = array(
    'template',
...
echo $OUTPUT->heading(get_string('history', 'shezar_cohort'));
