...
require_once($CFG->dirroot.'/shezar/cohort/rules/settings.php');
...
$COHORT_RULES_OP_IN_LIST = array(
    COHORT_RULES_OP_IN_CONTAINS   => get_string('contains', 'shezar_cohort'),
    COHORT_RULES_OP_IN_NOTCONTAIN => get_string('doesnotcontain', 'shezar_cohort'),
    COHORT_RULES_OP_IN_ISEQUALTO  => get_string('isequalto', 'shezar_cohort'),
    COHORT_RULES_OP_IN_STARTSWITH => get_string('startswith', 'shezar_cohort'),
    COHORT_RULES_OP_IN_ENDSWITH   => get_string('endswith', 'shezar_cohort'),
    COHORT_RULES_OP_IN_ISEMPTY    => get_string('isempty', 'shezar_cohort'),
    COHORT_RULES_OP_IN_NOTEQUALTO => get_string('isnotequalto', 'shezar_cohort'),
);
...
function cohort_rules_get_menu_options() {
    ...
    if (!$rulesmenu) {
        ...
        foreach ($rules as $groupname => $group) {
            ...
            if ($groupname === 'pos' && shezar_feature_disabled('positions')) {
                continue;
            }

            $curlabel = get_string("rulegroup-{$groupname}", 'shezar_cohort');
        ...
        $rulesmenu = array_merge(
            array(
                get_string('new') => array(
                    '' => get_string('addrule', 'shezar_cohort')
                )
            ),
            $rulesmenu
        );
...
function cohort_rule_create_ruleset($collectionid) {
    ...
    $todb->name = get_string('rulesetname', 'shezar_cohort', $todb->sortorder);
    ...
    $log = array(SITEID, 'cohort', 'create ruleset', 'shezar/cohort/rules.php?id='.$cohort->id, "rulesetid={$todb->id}");
    $event = \shezar_cohort\event\ruleset_created::create_from_instance($todb, $cohort);
...
function cohort_rule_create_rule($rulesetid, $type, $name) {
    ...
    $log = array(SITEID, 'cohort', 'create rule', 'shezar/cohort/rules.php?id='.$cohort->id, $loginfo);
    $event = \shezar_cohort\event\rule_created::create_from_instance($todb, $cohort);
    $event->set_legacy_logdata($log);
    $event->trigger();
...
function cohort_rule_form_html($ruleid, $description, $rulegroup = '', $rulename = '', $first = false, $operator = '',
    ...
    // As part of shezar 9.0 we rearranged how this was working cleaning up the output to use a list and cleaned up the JS handling
    // of rule changes.
...
function cohort_ruleset_form_template_object($ruleset) {
    ...
    switch ($ruleset->operator) {
        case COHORT_RULES_OP_OR:
            $rulesetdata->operator = get_string('or', 'shezar_cohort');
            break;
        case COHORT_RULES_OP_AND:
            $rulesetdata->operator = get_string('and', 'shezar_cohort');
            break;
        default:
    ...
    foreach ($ruleset->rules as $rulerec) {
        ...
        $data->id = $rulerec->id;
        $data->first = false;
        $data->editurl = new moodle_url('/shezar/cohort/rules/ruledetail.php', array('type' => 'rule', 'id' => $rulerec->id));
...
function cohort_rules_approve_changes($cohort, $syncmembers = true) {
    ...
    \shezar_cohort\event\draftcollection_saved::create_from_instance($cohort)->trigger();

    if ($syncmembers) {
        shezar_cohort_update_dynamic_cohort_members($cohort->id, 0, true);
    }
...
function cohort_rules_cancel_changes($cohort) {
    ...
    $event = \shezar_cohort\event\draftcollection_discarded::create_from_instance($cohort);
...
function cohort_delete_param($ruleparam) {
    ...
    $log = array(SITEID, 'cohort', 'delete rule param ' . $ruleparam->id, 'shezar/cohort/rules.php');
    $event = \shezar_cohort\event\rule_param_deleted::create_from_instance($ruleparam, $cohort);
    $event->set_legacy_logdata($log);
