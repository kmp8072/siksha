...
class cohort_rule_option {
     ...
     * set to get_string("rulename-{$this->group}-{$this->label}", 'shezar_cohort').
     ...
     * you should put them in shezar_cohort.
     * @var string
     */
    public $label;
    ...
    public function __construct($group, $name, $ui, $sqlhandler, $label=false, $hiddenfrommenu=false) {
        ...
        // If this is false we'll want to get it from shezar_cohort based on the
     ...
     * shezar_cohort if it's not already populated.
     */
    public function getLabel(){
        if (!$this->label){
            $this->label = get_string("rulename-{$this->group}-{$this->name}", 'shezar_cohort');
        }
        return $this->label;
    }
}