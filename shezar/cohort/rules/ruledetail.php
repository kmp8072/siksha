...
if (!preg_match('/^([a-z]+)-([a-z0-9_]+)$/', $rulegroupname, $matches)) {
    print_error('error:rulemissing', 'shezar_cohort');
} else {
    $rulegroup = $matches[1];
    $rulename = $matches[2];
    unset($matches);
...
if (!$rule) {
    print_error('error:badrule', 'shezar_cohort', '', "{$rulegroup}-{$rulename}");
}
    ...
    if ($ui->validateResponse()) {
        ...
        switch($type) {
            case 'cohort':
                ...
                if (!$cohort = $DB->get_record('cohort', array('id' => $id))) {
                    print_error(get_string('error:badcohortid', 'shezar_cohort'));
                }
                if (!$cohort->cohorttype == cohort::TYPE_DYNAMIC) {
                    print_error(get_string('error:notdynamiccohort', 'shezar_cohort'));
                }
            ...
            case 'rule':
                ...
                $log = array(SITEID, 'cohort', 'edit rule', 'shezar/cohort/rules.php?id='. $cohort->id, "ruleid={$id}");
                $event = \shezar_cohort\event\rule_updated::create_from_instance($ruleobj, $cohort);
        ...
        switch($type) {
            ...
            case 'rule':
                ...
                $ruledata = cohort_ruleset_form_template_object($ruleset);
                echo $OUTPUT->render_from_template('shezar_cohort/editing_ruleset', $ruledata);
                break;
        }
        exit();
