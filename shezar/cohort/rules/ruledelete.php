...
$event = \shezar_cohort\event\rule_deleted::create_from_instance($rule, $cohort);
$event->set_legacy_logdata(array(SITEID, 'cohort', 'delete rule', 'shezar/cohort/rules.php?id='.$cohort->id, $info));
...
if ($DB->record_exists('cohort_rules', array('rulesetid' => $rule->rulesetid))) {
    ...
    $event = \shezar_cohort\event\ruleset_deleted::create_from_instance($ruleset, $cohort);
