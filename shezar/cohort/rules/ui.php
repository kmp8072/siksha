...
abstract class cohort_rule_ui {
    ...
    public function param_delete_action_icon($paramid) {
        ...
        $icon = new \core\output\flex_icon('delete', array(
            'alt' => get_string('deleteruleparam', 'shezar_cohort'),
...
abstract class cohort_rule_ui_form extends cohort_rule_ui {
    ...
    public function constructForm(){
        global $CFG;
        if ($this->form == null) {
            $this->form = new $this->formclass($CFG->wwwroot.'/shezar/cohort/rules/ruledetail.php');
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        echo $OUTPUT->heading(get_string('ruledialogdesc', 'shezar_cohort', $this->description), '2', 'cohort-rule-dialog-heading');
    ...
    protected function addFormFields(&$mform) {
        $mform->addElement('static', 'noconfig', '', get_string('ruleneedsnoconfiguration', 'shezar_cohort'));
    }
}
    ...
    protected function addFormFields(&$mform) {
        ...
        $mform->addGroupRule(
            'row1',
                array(
                    1 => array(
                        array(0 => get_string('error:mustpickonevalue', 'shezar_cohort'), 1 => 'callback', 2 => 'validate_emptyruleuiform', 3 => 'client')
                    )
                )
        );

        $error = get_string('error:mustpickonevalue', 'shezar_cohort');
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if (!isset($this->equal) || !isset($this->listofvalues)) {
            return get_string('error:rulemissingparams', 'shezar_cohort');
        }
        ...
        return get_string('ruleformat-descjoinvars', 'shezar_cohort', $strvar);
    }
    ...
    protected function addFormFields(&$mform) {
        ...
        $row[0] = $mform->createElement(
            ...
            array(
                ...
                COHORT_RULES_OP_IN_EQUAL    => get_string('equalto','shezar_cohort'),
                COHORT_RULES_OP_IN_NOTEQUAL => get_string('notequalto', 'shezar_cohort')
            )
        );
        ...
        $mform->addGroupRule(
            'row1',
                array(
                    0=>array(
                        array(0=>get_string('error:mustpickonevalue', 'shezar_cohort'), 1=>'callback',2=>'validate_menu', 3=>'client')
                    )
                )
        );
        $js = <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = $this->description;
        $strvar->join = get_string("is{$COHORT_RULES_OP_IN[$this->equal]}to", 'shezar_cohort');
...
class ruleuidateform extends emptyruleuiform {
    public function validation($data, $files){
        ...
        if (empty($data['fixedordynamic']) || !in_array($data['fixedordynamic'], array(1,2))) {
            $errors['beforeafterrow'] = get_string('error:baddateoption', 'shezar_cohort');
            $errors['durationrow'] = ' ';
        }
        ...
        if (
            ...
            $errors['beforeafterrow'] = get_string('error:baddate', 'shezar_cohort');
        }
        ...
        if (
            ...
            $errors['durationrow'] = get_string('error:badduration', 'shezar_cohort');
        }
...
class cohort_rule_ui_date extends cohort_rule_ui_form {
    ...
    protected function addFormData() {
        ...
        $formdata['beforeafterdate'] = get_string('datepickerlongyearplaceholder', 'shezar_core');
        if (isset($this->operator)) {
            if ($this->operator == COHORT_RULE_DATE_OP_AFTER_FIXED_DATE || $this->operator == COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE) {
                ...
                if (!empty($this->date)) {
                    // todo: make this configurable!
                    $formdata['beforeafterdate'] = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
                }
            } else if (
                    in_array(
                        $this->operator,
    ...
    public function addFormFields(&$mform) {
        ...
        $row[1] = $mform->createElement(
            ...
            '',
            array(
                COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE=>get_string('datemenufixeddatebefore', 'shezar_cohort'),
                COHORT_RULE_DATE_OP_AFTER_FIXED_DATE=>get_string('datemenufixeddateafter', 'shezar_cohort')
            )
        );
        ...
        $('#id_beforeafterdate').datepicker(
            {
                dateFormat: '
JS;
        $datepickerjs .= get_string('datepickerlongyeardisplayformat', 'shezar_core');
        $datepickerjs .= <<<JS
',
                showOn: 'both',
...
JS;
        ...
        $durationmenu = array(
            COHORT_RULE_DATE_OP_BEFORE_PAST_DURATION =>   get_string('datemenudurationbeforepast', 'shezar_cohort'),
            COHORT_RULE_DATE_OP_WITHIN_PAST_DURATION =>   get_string('datemenudurationwithinpast', 'shezar_cohort'),
            COHORT_RULE_DATE_OP_WITHIN_FUTURE_DURATION => get_string('datemenudurationwithinfuture', 'shezar_cohort'),
            COHORT_RULE_DATE_OP_AFTER_FUTURE_DURATION =>  get_string('datemenudurationafterfuture', 'shezar_cohort'),
        );
        ...
        $row[3] = $mform->createElement('static', '', '', get_string('durationdays', 'shezar_cohort'));
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        switch ($this->operator) {
            ...
            case COHORT_RULE_DATE_OP_AFTER_FIXED_DATE:
                $a = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
                break;
        ...
        $strvar->vars = get_string("dateis{$COHORT_RULE_DATE_OP[$this->operator]}", 'shezar_cohort', $a);

        return get_string('ruleformat-descvars', 'shezar_cohort', $strvar);
    }
    ...
    public function handleDialogUpdate($sqlhandler){
        ...
        switch($fixedordynamic) {
            case 1:
                ...
                $date = shezar_date_parse_from_format(get_string('datepickerlongyearparseformat', 'shezar_core'), required_param('beforeafterdate', PARAM_TEXT));
                break;
            case 2:
        ...
        if (isset($this->operator)) {
            if ($this->operator == COHORT_RULE_DATE_OP_AFTER_FIXED_DATE || $this->operator == COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE) {
                ...
                if (!empty($this->date)) {
                    ...
                    $formdata['beforeafterdate'] = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 'UTC', false);
                }
            } else if (
            in_array(
                $this->operator,
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        switch ($this->operator) {
            case COHORT_RULE_DATE_OP_BEFORE_FIXED_DATE:
            case COHORT_RULE_DATE_OP_AFTER_FIXED_DATE:
                $a = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 'UTC', false);
                break;
            case COHORT_RULE_DATE_OP_BEFORE_PAST_DURATION:
            case COHORT_RULE_DATE_OP_WITHIN_PAST_DURATION:
            case COHORT_RULE_DATE_OP_WITHIN_FUTURE_DURATION:
    ...
    public function handleDialogUpdate($sqlhandler) {
        ...
        switch($fixedordynamic) {
            case 1:
                ...
                $dateformat = get_string('datepickerlongyearparseformat', 'shezar_core');
                // We save the date as a timestamp with time of midday UTC.
                $dateparam .= " 12:00:00";
                $dateformat .= " H:i:s";
                $date = shezar_date_parse_from_format($dateformat, $dateparam, false, 'UTC');
                break;
            case 2:
                $operator = required_param('durationmenu', PARAM_INT);
                $date = required_param('durationdate', PARAM_INT);
...
class shezar_dialog_content_hierarchy_multi_cohortrule extends shezar_dialog_content_hierarchy_multi {
    ...
    */
    public function populate_selected_items_pane($elements) {
        ...
        $operatormenu = array();
        $operatormenu[1] = get_string('equalto', 'shezar_cohort');
        $operatormenu[0] = get_string('notequalto', 'shezar_cohort');
        ...
        $childmenu = array();
        $childmenu[0] = get_string('includechildrenno', 'shezar_cohort');
        $childmenu[1] = get_string('includechildrenyes', 'shezar_cohort');
...
//        $compid = required_param('id', PARAM_INT);
        ...
        if ($showhidden && !has_capability('shezar/hierarchy:updatecompetencyframeworks', $context)) {
        ...
        $dialog = new shezar_dialog_content_hierarchy_multi_cohortrule($this->prefix, $frameworkid, $showhidden);
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if ($this->includechildren) {
            $strvar->ext = get_string('orachildof', 'shezar_cohort');
        }
        ...
        if (!empty($strvar->ext)) {
            return get_string('ruleformat-descjoinextvars', 'shezar_cohort', $strvar);
        } else {
            return get_string('ruleformat-descjoinvars', 'shezar_cohort', $strvar);
        }
    }
...
class shezar_dialog_content_cohort_rules_courses extends shezar_dialog_content_courses {
...
abstract class cohort_rule_ui_picker_course_program extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        if ($this->pickertype == COHORT_PICKER_COURSE_COMPLETION) {
            ...
            echo get_string('error:typecompletion', 'shezar_cohort');
            return;
        }
        ...
        // Load dialog content generator.
        if ($this->pickertype == COHORT_PICKER_COURSE_COMPLETION) {
            $dialog = new shezar_dialog_content_cohort_rules_courses($categoryid);
        } else {
            $dialog = new shezar_dialog_content_cohort_rules_programs($categoryid);
        }
...
class cohort_rule_ui_picker_course_allanynotallnone extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        $operatormenu = array();
        $operatormenu[COHORT_RULE_COMPLETION_OP_ALL] = get_string('completionmenuall', 'shezar_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_ANY] = get_string('completionmenuany', 'shezar_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_NOTALL] = get_string('completionmenunotall', 'shezar_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_NONE] = get_string('completionmenunotany', 'shezar_cohort');
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        switch ($this->operator) {
            case COHORT_RULE_COMPLETION_OP_ALL:
                $strvar->desc = get_string('ccdescall', 'shezar_cohort');
                break;
            case COHORT_RULE_COMPLETION_OP_ANY:
                $strvar->desc = get_string('ccdescany', 'shezar_cohort');
                break;
            case COHORT_RULE_COMPLETION_OP_NOTALL:
                $strvar->desc = get_string('ccdescnotall', 'shezar_cohort');
                break;
            default:
                $strvar->desc = get_string('ccdescnotany', 'shezar_cohort');
        }
...
class cohort_rule_ui_picker_course_duration extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $operatormenu = array();
        $operatormenu[COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN] = get_string('completiondurationmenulessthan', 'shezar_cohort');
        $operatormenu[COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN] = get_string('completiondurationmenumorethan', 'shezar_cohort');
        $selected = isset($this->operator) ? $this->operator : '';
        ...
        $html .= '" /> ' . get_string('completiondurationdays', 'shezar_cohort');
        $html .= '</fieldset>';
        $html .= '</div>';
        $validnumberstr = get_string('error:badduration', 'shezar_cohort');
        $html .= <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($descstr, 'shezar_cohort', $this->date);
...
class cohort_rule_ui_picker_course_program_date extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $opmenufix[COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN] = get_string('datemenufixeddatebefore', 'shezar_cohort');
        $opmenufix[COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN] = get_string('datemenufixeddateafter', 'shezar_cohort');

        $opmenurel[COHORT_RULE_COMPLETION_OP_BEFORE_PAST_DURATION] = get_string('datemenudurationbeforepast', 'shezar_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_WITHIN_PAST_DURATION] = get_string('datemenudurationwithinpast', 'shezar_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_WITHIN_FUTURE_DURATION] = get_string('datemenudurationwithinfuture', 'shezar_cohort');
        $opmenurel[COHORT_RULE_COMPLETION_OP_AFTER_FUTURE_DURATION] = get_string('datemenudurationafterfuture', 'shezar_cohort');

        // Set default values.
        $selected = isset($this->operator) ? $this->operator : '';
        $htmldate = get_string('datepickerlongyearplaceholder', 'shezar_core');
        ...
        if (isset($this->operator) && array_key_exists($this->operator, $opmenufix)) {
            ...
            $htmldate = userdate($this->date, get_string('datepickerlongyearphpuserdate', 'shezar_core'), 99, false);
        ...
        // Fixed date.
        $html .= get_string('completionusercompletedbeforeafter', 'shezar_cohort');
        ...
        // Relative date.
        $html .= get_string('or', 'shezar_cohort');
        $html .= html_writer::start_tag('fieldset');
    ...
    public function handleDialogUpdate($sqlhandler){
        ...
        switch($fixedordynamic) {
            case 1:
                $operator = required_param('beforeaftermenu', PARAM_INT);
                $date = shezar_date_parse_from_format(get_string('datepickerlongyearparseformat', 'shezar_core'),
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar->join = get_string("dateis{$COHORT_RULE_COMPLETION_OP[$this->operator]}", 'shezar_cohort', $a);
...
require_once($CFG->dirroot.'/shezar/core/dialogs/dialog_content_programs.class.php');
class shezar_dialog_content_cohort_rules_programs extends shezar_dialog_content_programs {
...
class cohort_rule_ui_picker_program_allanynotallnone extends cohort_rule_ui_picker_course_program {
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($getstr, 'shezar_cohort');
...
class cohort_rule_ui_picker_program_duration extends cohort_rule_ui_picker_course_program {
    ...
    public function getExtraSelectedItemsPaneWidgets(){
        ...
        $html .= '</fieldset>';
        $html .= '</div>';
        $badduration = get_string('error:badduration', 'shezar_cohort');
        $html .= <<<JS
...
JS;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        $strvar->desc = get_string($getstr, 'shezar_cohort', $this->date);
...
class shezar_dialog_content_manager_cohort extends shezar_dialog_content_users {
    /**
    * Returns markup to be used in the selected pane of a multi-select dialog
    *
    * @param   $elements    array elements to be created in the pane
    ...
    */
    public function populate_selected_items_pane($elements) {
        $operatormenu = array();
        $operatormenu[0] = get_string('reportsto', 'shezar_cohort');
        $operatormenu[1] = get_string('reportsdirectlyto', 'shezar_cohort');
...
class cohort_rule_ui_reportsto extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        $dialog = new shezar_dialog_content_manager_cohort();
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        if ($this->isdirectreport) {
            $strvar->desc = get_string('userreportsdirectlyto', 'shezar_cohort');
        } else {
            $strvar->desc = get_string('userreportsto', 'shezar_cohort');
        }
...
class cohort_rule_ui_managersja extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        $dialog = new shezar_dialog_content_manager_cohortreportsto();
...
require_once($CFG->dirroot . '/shezar/core/dialogs/dialog_content_manager.class.php');
class shezar_dialog_content_manager_cohortmember extends shezar_dialog_content_manager {
    /**
    * Returns markup to be used in the selected pane of a multi-select dialog
    *
    * @param   $elements    array elements to be created in the pane
    ...
    */
    public function populate_selected_items_pane($elements) {
        ...
        $operatormenu = array();
        $operatormenu[1] = get_string('incohort', 'shezar_cohort');
        $operatormenu[0] = get_string('notincohort', 'shezar_cohort');
...
class cohort_rule_ui_cohortmember extends cohort_rule_ui {
    ...
    public function printDialogContent($hidden=array(), $ruleinstanceid=false) {
        ...
        // Set up dialog
        $dialog = new shezar_dialog_content_manager_cohortmember();
        $dialog->type = shezar_dialog_content::TYPE_CHOICE_MULTI;
        $dialog->items = $items;
    ...
    public function getRuleDescription($ruleid, $static=true) {
        ...
        $strvar = new stdClass();
        if ($this->incohort) {
            $strvar->desc = get_string('useriscohortmember', 'shezar_cohort');
        } else {
            $strvar->desc = get_string('userisnotcohortmember', 'shezar_cohort');
        }
