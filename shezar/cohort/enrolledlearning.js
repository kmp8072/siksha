 ...
 */
M.shezar_cohortenrolledlearning = M.shezar_cohortenrolledlearning || {
    ...
    init: function(Y) {
        ...
        $('.assignment-duedates').on('click', function(){
            $.get($(this).attr('href'), function(result){
                M.shezar_cohortenrolledlearning.datesDialogue = new M.core.dialogue({
                    headerContent: null,
                    bodyContent  : result,
                    width        : 500,
                    centered     : true,
                ...
                M.shezar_cohortenrolledlearning.datesDialogue.show();
            });
            return false;
        });

        // On click events for column sorting and paging inside "View dates" popup.
        $('#page-admin-shezar-cohort-enrolledlearning').on('click',
            ...
            if (!event.target.closest('td') || !$(event.target.closest('td')).hasClass('cell')) {
                $.get($(event.target).attr('href'), function(result){
                    M.shezar_cohortenrolledlearning.datesDialogue.bodyNode.setHTML(result);
                });
            } else {
