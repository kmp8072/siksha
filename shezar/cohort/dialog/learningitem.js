...
M.shezar_cohortlearning = M.shezar_cohortlearning || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_cohortlearning.init()-> jQuery dependency required for this module.');
        }

        var url = M.cfg.wwwroot + '/shezar/cohort/dialog/';
        ...
        var chandler = new shezarDialog_handler_cohortlearning();
        chandler.baseurl = url;
        var cbuttons = {};
        cbuttons[M.util.get_string('save','shezar_core')] = function() { chandler._save(csaveurl) }
        cbuttons[M.util.get_string('cancel','moodle')] = function() { chandler._cancel() }

        shezarDialogs['learningitemcourses'] = new shezarDialog(
            'learningitemcourses',
            'add-course-learningitem-dialog',
            {
                buttons: cbuttons,
                title: '<h2>' + M.util.get_string('assign'+assgnstring+'learningcourse', 'shezar_cohort') + '</h2>'
            },
        ...
        var phandler = new shezarDialog_handler_cohortlearning();
        phandler.baseurl = url;
        var pbuttons = {};
        pbuttons[M.util.get_string('save','shezar_core')] = function() { phandler._save(psaveurl) }
        pbuttons[M.util.get_string('cancel','moodle')] = function() { phandler._cancel() }

        shezarDialogs['learningitemprograms'] = new shezarDialog(
            'learningitemprograms',
            'add-program-learningitem-dialog',
            {
                buttons: pbuttons,
                title: '<h2>' + M.util.get_string('assign'+assgnstring+'learningprogram', 'shezar_cohort') + '</h2>'
            },
        ...
        var pcsaveurl = url + 'updatelearning.php?type=' + this.config.COHORT_ASSN_ITEMTYPE_CERTIF + '&cohortid=' + this.config.cohortid + '&sesskey=' +
            M.cfg.sesskey + '&v=' + assgnval + '&u=';
        var pchandler = new shezarDialog_handler_cohortlearning();
        pchandler.baseurl = url;
        var pcbuttons = {};
        pcbuttons[M.util.get_string('save','shezar_core')] = function() { pchandler._save(pcsaveurl) }
        pcbuttons[M.util.get_string('cancel','moodle')] = function() { pchandler._cancel() }

        shezarDialogs['learningitemcertifications'] = new shezarDialog(
            'learningitemcertifications',
            'add-certification-learningitem-dialog',
            {
                buttons: pcbuttons,
                title: '<h2>' + M.util.get_string('assign'+assgnstring+'learningcertification', 'shezar_cohort') + '</h2>'
            },
    ...
    init_deletelisteners: function() {
        ...
        $('a.learning-delete').bind('click', function(e, postdeletecallback) {
            ...
            var confirmed = confirm(M.util.get_string('deletelearningconfirm', 'shezar_cohort'));
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    require(['core/templates'], function (templates) {
                        templates.renderIcon('loading', M.util.get_string('savingrule', 'shezar_cohort')).done(function (html) {
...
shezarDialog_handler_cohortlearning = function() {
    // Base url
    var baseurl = '';
}

shezarDialog_handler_cohortlearning.prototype = new shezarDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
shezarDialog_handler_cohortlearning.prototype._update = function(response) {

    // Hide dialog
    this._dialog.hide();

    //TODO: the stuff to add table rows :-P
    if (M.shezar_cohortlearning.config.saveurl) {
        location.replace(M.cfg.wwwroot + M.shezar_cohortlearning.config.saveurl + '?id=' + M.shezar_cohortlearning.config.cohortid);
    } else {
        location.replace(M.cfg.wwwroot + '/shezar/cohort/enrolledlearning.php?id=' + M.shezar_cohortlearning.config.cohortid);
    }
}
