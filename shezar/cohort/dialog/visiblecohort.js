...
M.shezar_visiblecohort = M.shezar_visiblecohort || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_visiblecohort.init()-> jQuery dependency required for this module.');
        }
        ...
        $('select[name="audiencevisible"]').change(M.shezar_visiblecohort.checkcohortaddvisibility);
    ...
    init_dialogs: function() {
        ...
        var ehandler = new shezarDialog_handler_visiblecohorts();
        ...
        shezarDialogs['id_cohortsaddvisible'] = new shezarDialog(
            'course-cohorts-visible-dialog',
            'id_cohortsaddvisible',
            {
                buttons: dbuttons,
                title: '<h2>' + M.util.get_string(this.config.type + 'cohortsvisible', 'shezar_cohort') + '</h2>'
            },
    ...
    checkcohortaddvisibility: function() {
        ...
        if (option.val() != M.shezar_visiblecohort.config.cohortvisibleaudience) {
...
shezarDialog_handler_visiblecohorts = function() {
    this.baseurl = '';
...
shezarDialog_handler_visiblecohorts.prototype = new shezarDialog_handler_treeview_multiselect();
 ...
 * @return void
 */
shezarDialog_handler_visiblecohorts.prototype._update = function(response) {
    ...
    if (newids.length > 0) {
        ...
        var ajax_url = M.cfg.wwwroot + '/shezar/cohort/dialog/cohort_item.php?module=course&itemid=' + newids.join(',') + params;
 ...
 **/
shezarDialog_handler_visiblecohorts.prototype.cohort_item_exists = function(itemid) {
...
shezarDialog_handler_visiblecohorts.prototype.check_table_hidden_status = function() {
...
shezarDialog_handler_visiblecohorts.prototype.add_cohort_delete_event_handlers = function() {
 ...
 ** Adds an item
 **/
shezarDialog_handler_visiblecohorts.prototype.add_cohort_item = function(itemid) {
 ...
 **/
shezarDialog_handler_visiblecohorts.prototype.create_item = function(html) {
    var element = $(html);
...
shezarDialog_handler_visiblecohorts.prototype.remove_cohort_item = function(item) {
