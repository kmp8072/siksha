...
if (($data = data_submitted()) && confirm_sesskey()) {
    ...
    $success = shezar_cohort_process_assigned_roles($cohort->id, $roles);
}

// Get list of all roles assigned to this cohort.
$rolesassigned = array_keys(shezar_get_cohort_roles($cohort->id));

$PAGE->set_context($contextsystem);
$PAGE->set_url('/shezar/cohort/assignroles.php', array('id' => $id));
$PAGE->set_title(format_string($cohort->name));
$PAGE->set_heading(format_string($cohort->name));

echo $OUTPUT->header();

$PAGE->requires->js_call_amd('shezar_cohort/assignroles', 'init');

echo cohort_print_tabs('roles', $cohort->id, $cohort->cohorttype, $cohort);

// Assign roles header.
echo $OUTPUT->heading(get_string('assignroles', 'shezar_cohort'), 3);

// Notify result operation.
if ($success) {
    echo $OUTPUT->notification(get_string('updatedrolesuccessful','shezar_cohort'), 'notifysuccess');
} else if ($success === false) {
    echo $OUTPUT->notification(get_string('updatedroleunsuccessful','shezar_cohort'));
}

echo html_writer::tag('p', get_string('instructions:assignmentroles', 'shezar_cohort', $contextcohort->get_context_name()));
...
$table = new shezar_table('cohort-assignroles');
$table->define_baseurl(new moodle_url('/shezar/cohort/assignroles.php', array('id' => $id)));
...
$attributes = array('id' => 'updateroles', 'type' => 'submit', 'value' => get_string('updateroles', 'shezar_cohort'));
...
$table->get_no_records_message(get_string('norolestoassign', 'shezar_cohort'));
$table->finish_html();
