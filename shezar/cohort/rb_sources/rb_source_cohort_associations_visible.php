...
class rb_source_cohort_associations_visible extends rb_base_source {
    ...
    protected function define_columnoptions() {
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'name',
            get_string('associationname', 'shezar_cohort'),
            'associations.name',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'status',
            get_string('visibility', 'shezar_cohort'),
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'type',
            get_string('associationtype', 'shezar_cohort'),
            'base.instancetype',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'nameiconlink',
            get_string('associationnameiconlink', 'shezar_cohort'),
            'associations.name',
            array(
                'joins' => 'associations',
        ...
        $columnoptions[] = new rb_column_option(
            'associations',
            'actionsvisible',
            get_string('associationactionsvisible', 'shezar_cohort'),
            'base.id',
            array(
        ...
        $columnoptions[] = new rb_column_option(
            ...
            get_string('associationprogramcompletionlink', 'shezar_cohort'),
            'base.instanceid',
            array(
    ...
    protected function define_filteroptions() {
        ...
        $filteroptions[] = new rb_filter_option(
            ...
            array(
                'selectchoices' => array(
                    COHORT_ASSN_ITEMTYPE_COURSE => get_string('associationcoursesonly', 'shezar_cohort'),
                    COHORT_ASSN_ITEMTYPE_PROGRAM  => get_string('associationprogramsonly', 'shezar_cohort'),
                    COHORT_ASSN_ITEMTYPE_CERTIF  => get_string('associationcertificationsonly', 'shezar_cohort'),
                ),
                'simplemode' => true,
            )
        );
    ...
    public function rb_display_associationtype($instancetype, $row) {
        switch ($instancetype) {
            ...
            case COHORT_ASSN_ITEMTYPE_PROGRAM:
                $ret = get_string('program', 'shezar_program');
                break;
            case COHORT_ASSN_ITEMTYPE_CERTIF:
                $ret = get_string('certification', 'shezar_certification');
                break;
            default:
                $ret = '';
        }
    ...
    public function rb_display_visibility_status($status, $row) {
        ...
        if ($type == COHORT_ASSN_ITEMTYPE_COURSE) {
            ...
            $hascapability = has_capability('shezar/program:configuredetails', $programcontext);
        }
    ...
    public function rb_display_associationnameiconlink($instancename, $row) {
        ...
        if ($row->type == COHORT_ASSN_ITEMTYPE_COURSE) {
            ...
            $url = new moodle_url('/shezar/program/view.php', array('id' => $row->insid));
        }
    ...
    private function cohort_association_delete_link($associationid, $row) {
        ...
        if ($strdelete === false) {
            $strdelete = get_string('deletelearningitem', 'shezar_cohort');
        }
        $delurl = new moodle_url('/shezar/cohort/dialog/updatelearning.php',
    ...
    public function rb_display_programcompletionlink($instanceid, $row) {
        ...
        if ($canedit && $row->type == COHORT_ASSN_ITEMTYPE_PROGRAM) {
            return shezar_cohort_program_completion_link($row->cohortid, $instanceid);
        }
        return get_string('na', 'shezar_cohort');
    }
    ...
    public function get_required_jss() {
        ...
        $jsdetails->initcall = 'M.shezar_cohortvisiblelearning.init';
        $jsdetails->jsmodule = array(
            'name' => 'shezar_cohortvisiblelearning',
            'fullpath' => '/shezar/cohort/visiblelearning.js',
            'requires' => array('json'));
