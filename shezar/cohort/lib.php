...
global $COHORT_ALERT;
$COHORT_ALERT = array(
    COHORT_ALERT_NONE => get_string('alertmembersnone', 'shezar_cohort'),
    COHORT_ALERT_AFFECTED => get_string('alertmembersaffected', 'shezar_cohort'),
    COHORT_ALERT_ALL => get_string('alertmembersall', 'shezar_cohort'),
);
...
/**
 * This function is called automatically when the shezar cohort module is installed.
 *
 * @return bool Success
 */
function shezar_cohort_install() {
    global $CFG, $DB;
    return true;
}
 ...
 */
function shezar_cohort_update_audience_visibility($type, $instanceid, $visiblevalue) {
    global $DB, $CFG, $COHORT_VISIBILITY;

    $result = false;

    // Capabilities to update programs or certifications.
    $progcap = 'shezar/program:configuredetails';
    $certcap = 'shezar/certification:configuredetails';
 ...
 * @return bool
 */
function shezar_cohort_add_association($cohortid, $instanceid, $instancetype, $value=COHORT_ASSN_VALUE_ENROLLED) {
    ...
    if ($value == COHORT_ASSN_VALUE_ENROLLED) {
        switch ($instancetype) {
            case COHORT_ASSN_ITEMTYPE_COURSE:
                $courseassociations = array_map(
                    ...
                    shezar_cohort_get_associations($cohortid, COHORT_ASSN_ITEMTYPE_COURSE)
                );
                ...
                $event = \shezar_cohort\event\enrolled_course_item_added::create_from_data($assid, $cohort);
            ...
            case COHORT_ASSN_ITEMTYPE_PROGRAM:
                ...
                $progassociations = array_map(
                    ...
                    shezar_cohort_get_associations($cohortid, $itemtype)
                );
                ...
                // Trigger event.
                $log[] = "itemid={$instanceid};associationid={$assid}";
                $event = \shezar_cohort\event\enrolled_program_item_added::create_from_data($assid, $cohort);
                $event->set_legacy_logdata($log);
                $event->trigger();
        ...
        if (!$DB->record_exists_sql('SELECT 1 FROM {cohort_visibility} WHERE cohortid = ? AND instanceid = ? AND '
            ...
            // Trigger event.
            $log[] = "itemid={$instanceid};associationid={$assid}";
            $event = \shezar_cohort\event\visible_learning_item_added::create_from_data($assid, $cohort);
            $event->set_legacy_logdata($log);
            $event->trigger();
            return $assid;
        }
 ...
 * @return bool
 */
function shezar_cohort_delete_association($cohortid, $assid, $instancetype, $value=COHORT_ASSN_VALUE_ENROLLED) {
    ...
    if ($value == COHORT_ASSN_VALUE_VISIBLE) {
        ...
        $event = \shezar_cohort\event\visible_learning_item_deleted::create_from_data($assid, $cohort);
        $event->set_legacy_logdata($log);
    ...
    switch ($instancetype) {
        case COHORT_ASSN_ITEMTYPE_COURSE:
            ...
            if (!empty($enrolinstance)) {
                ...
                // Trigger event.
                $log[] = "associationid={$assid}";
                $event = \shezar_cohort\event\enrolled_course_item_deleted::create_from_data($assid, $cohort);
                $event->set_legacy_logdata($log);
        ...
        case COHORT_ASSN_ITEMTYPE_CERTIF:
            ...
            if (!empty($record)) {
                ...
                // Trigger event.
                $log[] = "associationid={$assid}";
                $event = \shezar_cohort\event\enrolled_program_item_deleted::create_from_data($assid, $cohort);
                $event->set_legacy_logdata($log);
 ...
 */
function shezar_cohort_get_associations($cohortid, $instancetype=null, $value=null) {
    global $DB;
...
class cohort {
    ...
    public static function getCohortTypes() {
        return array(
            self::TYPE_STATIC => get_string('set', 'shezar_cohort'),
            self::TYPE_DYNAMIC => get_string('dynamic', 'shezar_cohort')
        );
    }
    ...
    public static function display_goal_table($cohort, $can_edit) {
        ...
        foreach ($assignments as $assignment) {
            $nameurl = new moodle_url('/shezar/hierarchy/item/view.php', array('prefix' => 'goal', 'id' => $assignment->goalid));
            ...
            if ($can_edit) {
                ...
                $url = new moodle_url('/shezar/hierarchy/prefix/goal/assign/remove.php', $params);
...
/******************************************************************************
 * Cohort event handlers, called from /shezar/cohort/db/events.php
 ******************************************************************************/
class shezarcohort_event_handler {
    /**
     * Event handler for when a user custom profiler field is deleted.
     *
     * @param \shezar_customfield\event\profilefield_deleted $event
     * @return boolean
     */
    public static function profilefield_deleted(\shezar_customfield\event\profilefield_deleted $event) {
     ...
     *
     * @param \shezar_cohort\event\members_updated $event
     * @return bool
     */
    public static function members_updated(\shezar_cohort\event\members_updated $event) {

        // Get the plan config for the audience, the audience id is the event objectid.
        $config = \shezar_cohort\learning_plan_config::get_config($event->objectid);
        if ($config->auto_create_new()) {
            // Create the plans.
            \shezar_cohort\learning_plan_helper::create_plans($config);
        }
 ...
 */
function shezar_cohort_update_operator($cohortid, $id, $type, $operatorvalue) {
    global $DB, $USER;
    ...
    if ($type === COHORT_OPERATOR_TYPE_COHORT) {
        ...
        if ($operatorvalue != $cohort->rulesetoperator) {
            ...
            $event = \shezar_cohort\event\operator_updated::create_from_instance($rulecollection, $cohort);
            $event->set_legacy_logdata($log);
            $event->trigger();
        }
        ...
        if ($operator != $operatorvalue) {
            ...
            $event = \shezar_cohort\event\ruleset_operator_updated::create_from_instance($ruleset, $cohort);
            $event->set_legacy_logdata($log);
            $event->trigger();
        }
    }
 ...
 */
function shezar_cohort_update_membership_options($id, $addnewmembers, $removeoldmembers) {
    global $DB, $USER;
    ...
    $event = \shezar_cohort\event\option_updated::create_from_instance($rulecollection, $cohort);
    $event->trigger();
 ...
 * @return string
 */
function shezar_cohort_get_dynamic_cohort_whereclause($cohortid) {
 ...
 */
function shezar_cohort_update_dynamic_cohort_members($cohortid, $userid=0, $delaymessages=false, $updatenested=true) {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/shezar/cohort/rules/lib.php');

    /// update necessary nested cohorts first (if any)
    if ($updatenested) {
        $nestedcohorts = shezar_cohort_get_nested_dynamic_cohorts($cohortid, array(), true);
        foreach ($nestedcohorts as $ncohortid) {
            shezar_cohort_update_dynamic_cohort_members($ncohortid, $userid, $delaymessages, false);
        }
    }
    ...
    $whereclause = shezar_cohort_get_dynamic_cohort_whereclause($cohortid);
    ...
    $numadd = 0;
    $numdel = 0;
    $currentcohortroles = shezar_get_cohort_roles($cohortid); // Current roles assigned to this cohort.
    foreach ($changedmembers as $mem) {
        ...
        if (!empty($newmembers)) {
            ...
            shezar_set_role_assignments_cohort($currentcohortroles, $cohortid, array_keys($newmembers));
            shezar_cohort_notify_add_users($cohortid, array_keys($newmembers), $delaymessages);
        ...
        if (!empty($delmembers)) {
            ...
            shezar_unset_role_assignments_cohort($currentcohortroles, $cohortid, $delids);
            shezar_cohort_notify_del_users($cohortid, $delids, $delaymessages);
            $numdel += count($delids);
            unset($delids);
        }
    ...
    if ($numadd > 0 || $numdel > 0) {
        $event = \shezar_cohort\event\members_updated::create(
            array(
                'objectid' => $cohortid,
            )
        );
 ...
 */
function shezar_cohort_get_nested_dynamic_cohorts($cohortid, $current = array(), $activeonly = false) {
    global $DB;
    ...
    foreach ($cohorts as $ncohortid) {
        $current = array_merge($current, shezar_cohort_get_nested_dynamic_cohorts($ncohortid, $current, $activeonly));
    }
 ...
 */
function shezar_cohort_send_queued_notifications(){
    global $CFG, $DB;
    ...
    foreach ($batchlist as $batch) {
        ...
        shezar_cohort_notify_users($batch->cohortid, array_keys($msglist), $batch->action);
 ...
 * @return void
 */
function shezar_cohort_check_and_update_dynamic_cohort_members($courseid, progress_trace $trace, $cohortid = NULL) {
    global $DB;

    $trace->output('removing user memberships of deleted users...');
    shezar_cohort_clean_deleted_users();
    ...
    if (empty($courseid)) {
        ...
        $dcohorts = shezar_cohort_get_course_cohorts($courseid, cohort::TYPE_DYNAMIC);
    }

    // Looking for cohorts with broken rules.
    $cohortwithbrokenrules = shezar_cohort_broken_rules($courseid, null, $trace);
    ...
    if (!empty($cohortwithbrokenrules)) {
        ...
        foreach ($cohortwithbrokenrules as $brokencohort) {
            if ($brokencohort->broken == COHORT_BROKEN_RULE_NONE) { // Cohort has not been marked as broken.
                ...
                if (shezar_send_notification_broken_rule($siteadmins, $brokencohort)) {
        ...
        shezar_update_broken_field(array_keys($notifiedcohorts), COHORT_BROKEN_RULE_NOTIFIED);
        // Update broken field for cohorts that could have not been notified.
        $notnotified = shezar_search_for_value($cohortwithbrokenrules, 'broken', shezar_SEARCH_OP_NOT_EQUAL,
                        COHORT_BROKEN_RULE_NOTIFIED);
        $notnotified = array_diff(array_keys($notnotified), array_keys($notifiedcohorts));
        shezar_update_broken_field($notnotified, COHORT_BROKEN_RULE_NOT_NOTIFIED);
    }

    // Update broken field for active cohorts.
    $brokencohorts = shezar_search_for_value($dcohorts, 'broken', shezar_SEARCH_OP_NOT_EQUAL, COHORT_BROKEN_RULE_NONE);
    shezar_update_broken_field(array_keys($brokencohorts), COHORT_BROKEN_RULE_NONE);

    foreach ($dcohorts as $cohort) {
        $active = shezar_cohort_is_active($cohort);
        if (!$active) {
        ...
        try {
            ...
            $result = shezar_cohort_update_dynamic_cohort_members($cohort->id);
    ...
    // Set lastrun time to be checked by shezar_cohort cron - if $courseid and $cohortid is null we have arrived here from cron.
    if (empty($courseid) and empty($cohortid)) {
        $timenow = time();
        if (!set_config('lasthourlycron', $timenow, 'shezar_cohort')) {
            $trace->output("Error: could not update lasthourlycron timestamp for shezar_cohort plugin.");
        }
    }
}


function shezar_cohort_clean_deleted_users() {
    global $DB;
 ...
 * @return string
 */
function shezar_cohort_date_where_clause($cohorttablealias = null, $now = null) {
    if ($now === null) {
        $now = time();
    }
 ...
 */
function shezar_cohort_delete_stale_memberships() {
    global $DB;
 ...
 * @param $cohortid
 */
function shezar_cohort_clone_cohort($oldcohortid) {
    global $CFG, $DB, $USER;
    ...
    $newcohort->name =              get_string('clonename', 'shezar_cohort', $oldcohort->name);
    $newcohort->idnumber =          shezar_cohort_next_automatic_id();
    ...
    $oldlearningitems = shezar_cohort_get_associations($oldcohortid);
    foreach ($oldlearningitems as $olditem) {
        shezar_cohort_add_association($newcohort->id, $olditem->instanceid, $olditem->type);
    }
...
function shezar_cohort_notify_add_users($cohortid, $adduserids, $delaymessages=false) {
    return shezar_cohort_notify_users($cohortid, $adduserids, 'membersadded', $delaymessages);
}

function shezar_cohort_notify_del_users($cohortid, $deluserids, $delaymessages=false) {
    return shezar_cohort_notify_users($cohortid, $deluserids, 'membersremoved', $delaymessages);
}

/**
 * Processor function to be passed in to {@link insert_records_via_batch()}. Used by
 * {@link shezar_cohort_notify_users()}.
 *
 ...
 */
function shezar_process_user_notifications($userid, $templateobject) {
 ...
 */
function shezar_cohort_notify_users($cohortid, $userids, $action, $delaymessages=false) {
    global $CFG, $DB, $USER;
    ...
    if ($delaymessages) {
        ...
        return $DB->insert_records_via_batch('cohort_msg_queue', $userids, 'shezar_process_user_notifications', array($msg));
    }
    ...
    foreach ($tousers as $touser) {
        ...
        $emailsubject = $strmgr->get_string("msg:{$action}_{$towho}_emailsubject", 'shezar_cohort', $a, $touser->lang);
        $notice = $strmgr->get_string("msg:{$action}_{$towho}_notice", 'shezar_cohort', $a, $touser->lang);
 ...
 * @return bool
 */
function shezar_cohort_is_active($cohort, $now = null){
    if ($now === null) {
        $now = time();
    }
 ...
 * NOTE: After using this function, make sure to call shezar_cohort_increment_automatic_id
 * to "mark off" the id number you used.
 * @return string
 */
function shezar_cohort_next_automatic_id() {
    global $CFG, $DB;
...
/**
 * Increment the automatic id number counter from shezar_cohort_next_automatic_id()
 * @param $idnumberused
 */
function shezar_cohort_increment_automatic_id($idnumberused) {
    global $CFG;
 ...
 */
function shezar_cohort_navlinks($cohortid=false, $cohortname=false, $subpagetitle=false) {
    global $CFG, $PAGE;
 ...
 * @param $programid
 */
function shezar_cohort_program_completion_link($cohortid, $programid){
    global $DB;
 ...
 */
function shezar_cohort_get_course_cohorts($courseid, $type=null, $fields='c.*') {
    global $DB;
 ...
 * @return object cohort database table records
 */
function shezar_cohort_get_goal_type_cohorts($goaltypeid, $fields = 'c.*') {
    global $DB;
 ...
 */
function shezar_cohort_get_visible_learning($instanceid, $instancetype = COHORT_ASSN_ITEMTYPE_COURSE) {
    global $DB;
 ...
 */
function shezar_cohort_broken_rules($courseid, $cohortid, progress_trace $trace) {
    global $DB, $CFG;
    ...
    if (empty($courseid)) {
        ...
        $dynamiccohorts = shezar_cohort_get_course_cohorts($courseid, cohort::TYPE_DYNAMIC);
        if (!empty($cohortid)) {
            // Get cohort.
            $dynamiccohorts = shezar_search_for_value($dynamiccohorts, 'id', shezar_SEARCH_OP_EQUAL, $cohortid);
        }
    }
 ...
 * @return void
 */
function shezar_display_broken_rules_box($class = 'notifyproblem clearfix', $style='display:block') {
    $contents =  html_writer::tag('span', get_string('cohortbrokenrulesnotice', 'shezar_cohort'));
 ...
 * @return boolean $sent
 */
function shezar_send_notification_broken_rule($users, $cohort) {
    $sent = false;

    $subject = get_string('cohortbrokenrulesubject', 'shezar_cohort');
    $fullmessage = get_string('cohortbrokenrulesmessage', 'shezar_cohort');
    $fullmessage .= html_writer::start_tag('ul');
    $url = html_writer::link(new moodle_url('/shezar/cohort/rules.php', array('id' => $cohort->id)), $cohort->name);
    ...
    foreach ($users as $user) {
        ...
        $newevent->urgency     = shezar_MSG_URGENCY_URGENT;
 ...
 * @return boolean.
 */
function shezar_update_broken_field($cohortids, $status) {
    global $DB;
 ...
 */
function shezar_get_members_cohort($cohortid) {
    global $DB;
 ...
 */
function shezar_cohort_get_user_cohorts($userid, $activeonly = true) {
    global $DB;
    $sql = 'SELECT c.id FROM {cohort} c
            INNER JOIN {cohort_members} cm ON (c.id = cm.cohortid)
            WHERE cm.userid = ?';
    if ($activeonly) {
        $sqltime = shezar_cohort_date_where_clause('c');
        $sql .= ' AND ' . $sqltime;
    }
 ...
 */
function shezar_get_cohort_roles($cohortid) {
    global $DB;
 ...
 */
function shezar_delete_roles_cohort($roleids, $cohortid) {
    global $DB;
    $success = true;
...
/**
 * Processor function to be passed in to {@link insert_records_via_batch()}. Used by
 * {@link shezar_create_roles_cohort()}.
 *
 ...
 */
function shezar_process_role_cohort($role, $templateobject) {
 ...
 * @return bool True success, false otherwise
 */
function shezar_create_roles_cohort($roleids, $cohortid) {
    global $DB, $USER;
    ...
    return $DB->insert_records_via_batch('cohort_role', $roleids, 'shezar_process_role_cohort', array($cohortrole));
}
 ...
 * @return void
 */
function shezar_unset_role_assignments_cohort($roles, $cohortid, $members) {
    foreach ($roles as $role) {
        $params = array(
            'roleid' => $role->roleid,
            'userids' => $members,
            'contextid' => $role->contextid,
            'component' => 'shezar_cohort',
            'itemid' => $cohortid
        );
 ...
 * @return void
 */
function shezar_set_role_assignments_cohort($roles, $cohortid, $members) {
    foreach ($roles as $role) {
        role_assign_bulk($role->roleid, $members, $role->contextid, 'shezar_cohort', $cohortid);
    }
}
 ...
 * @return bool
 */
function shezar_assign_roles_cohort($roles, $cohortid, $members) {
    global $DB;
    ...
    try {
        ...
        if (!empty($members)) {
            shezar_set_role_assignments_cohort($roles, $cohortid, $members);
        }
        shezar_create_roles_cohort($roles, $cohortid);
 ...
 * @return bool
 */
function shezar_unassign_roles_cohort($roles, $cohortid, $members) {
    global $DB;
    ...
    try {
        ...
        if (!empty($members)) {
            shezar_unset_role_assignments_cohort($roles, $cohortid, $members);
        }
        shezar_delete_roles_cohort($roles, $cohortid);
 ...
 */
function shezar_cohort_process_assigned_roles($cohortid, $roles) {
    $success = true;
    ...
    $memberids = array();
    if ($members = shezar_get_members_cohort($cohortid)) {
        $memberids = array_keys($members);
    }

    // Current roles assigned to this cohort.
    $currentroles = shezar_get_cohort_roles($cohortid);

    // Unassign roles.
    if (!empty($currentroles)) {
        $rolestounassign = array_diff_key($currentroles, $selectedroles);
        $success = shezar_unassign_roles_cohort($rolestounassign, $cohortid, $memberids);
    }

    // Assign roles.
    $rolestoassign = array_diff_key($selectedroles, $currentroles);
    if (!empty($rolestoassign)) {
        $success = $success && shezar_assign_roles_cohort($rolestoassign, $cohortid, $memberids);
    }
 ...
 *
 */
function shezar_cohort_process_assig_roles() {
    global $DB;
    ...
    $sql = "SELECT cm.userid, cr.roleid, cr.cohortid, cr.contextid
             ...
             LEFT JOIN {role_assignments} ra ON (ra.roleid = cr.roleid AND ra.contextid = cr.contextid AND ra.userid = cm.userid
                                                 AND ra.component = 'shezar_cohort' AND ra.itemid = cr.cohortid )
                 WHERE ra.id IS NULL";
    $rs = $DB->get_recordset_sql($sql);
    foreach ($rs as $ra) {
        role_assign($ra->roleid, $ra->userid, $ra->contextid, 'shezar_cohort', $ra->cohortid);
    }
    $rs->close();
    ...
    $sql = "SELECT ra.roleid, ra.userid, ra.contextid, ra.itemid
         ...
         LEFT JOIN {cohort_members} cm ON (cm.cohortid = c.id AND cm.userid = ra.userid)
             WHERE ra.component = 'shezar_cohort' AND cm.id IS NULL";
    $rs = $DB->get_recordset_sql($sql);
    foreach ($rs as $ra) {
        role_unassign($ra->roleid, $ra->userid, $ra->contextid, 'shezar_cohort', $ra->itemid);
    }
    $rs->close();
}
...
function check_access_audience_visibility($type, $instance, $userid = null) {
    ...
    list($visibilityjoinsql, $visibilityjoinparams) = shezar_visibility_join($userid, $type, $alias);
    ...
    if ($itemcontext == CONTEXT_COURSE) {
        ...
        if (empty($object->certifid) && has_capability('shezar/program:viewhiddenprograms', $context, $userid) ||
            !empty($object->certifid) && has_capability('shezar/certification:viewhiddencertifications', $context, $userid)) {
            return true;
        }
    }
...
class shezar_cohort_visible_learning_cohorts extends shezar_cohort_course_cohorts {
    function build_visible_learning_table($instanceid, $instancetype, $readonly = false) {
        $this->headers = array(
            get_string('cohortname', 'shezar_cohort'),
            get_string('type', 'shezar_cohort'),
            get_string('numlearners', 'shezar_cohort')
        );

        $items = shezar_cohort_get_visible_learning($instanceid, $instancetype);
...
class shezar_cohort_course_cohorts
{
    function build_table($courseid) {
        ...
        $items = shezar_cohort_get_course_cohorts($courseid, null,
...
/**
 * Class shezar_cohort_goal_cohorts
 */
class shezar_cohort_goal_cohorts {
    ...
    public function build_table($goaltypeid) {
        ...
        $items = shezar_cohort_get_goal_type_cohorts($goaltypeid, 'c.id, c.name AS fullname, c.cohorttype');
