 ...
 * vendor/bin/phpunit shezar_cohort_organisation_rules_testcase
 *
 */
class shezar_cohort_organisation_rules_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        for ($i = 1; $i <= self::TEST_ORGANISATION_COUNT_MEMBERS; $i++) {
            ...
            \shezar_job\job_assignment::create_default($this->{'user'.$i}->id, array('organisationid' => $orgid));
    ...
    */
    public function test_organisation_rule($params, $organisations, $usercount, $membersmatched) {
        ...
        \shezar_job\job_assignment::create_default($newuser1->id, array('organisationid' => $this->org4->id));
        \shezar_job\job_assignment::create_default($newuser2->id, array('organisationid' => $this->org5->id));
        \shezar_job\job_assignment::create_default($newuser3->id, array('organisationid' => $this->org4->id));
        \shezar_job\job_assignment::create_default($newuser4->id, array('organisationid' => $this->org5->id));
        \shezar_job\job_assignment::create_default($newuser5->id, array('organisationid' => $this->org4->id));
