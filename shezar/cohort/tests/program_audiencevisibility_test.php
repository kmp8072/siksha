 ...
 * vendor/bin/phpunit shezar_cohort_program_audiencevisibility_testcase
 *
 */
class shezar_cohort_program_audiencevisibility_testcase extends reportcache_advanced_testcase {
    ...
    public function setUp() {
        ...
        unassign_capability('shezar/program:viewhiddenprograms', $rolestaffmanager->id, $syscontext->id);

        $roletrainer = $DB->get_record('role', array('shortname'=>'teacher'));
        role_assign($roletrainer->id, $this->user9->id, $syscontext->id);
        assign_capability('shezar/program:viewhiddenprograms', CAP_ALLOW, $roletrainer->id, $syscontext);
        ...
        shezar_cohort_add_association($this->audience1->id, $this->program2->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience2->id, $this->program2->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);

        // Assign audience2 to program3 and program4.
        shezar_cohort_add_association($this->audience2->id, $this->program3->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience2->id, $this->program4->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
    ...
    public function test_audiencevisibility($user, $progsvisible, $progsnotvisible,
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            $this->assertEquals($countprogramsvisbile, shezar_get_category_item_count($this->category->id, 'program'));
    ...
    protected function get_visible_info($user, $content, $program) {
        ...
        if ($CFG->enhancedcatalog) { // New catalog.
            $search = array();
            if (is_array($content)) {
                $search = shezar_search_for_value($content, 'prog_progexpandlink', shezar_SEARCH_OP_EQUAL, $program->fullname);
    ...
    protected function create_programs_old_visibility() {
        ...
        shezar_cohort_add_association($this->audience2->id, $this->program6->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience1->id, $this->program5->id,
                                        COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
    }
    ...
    protected function create_programs_with_availability() {
        ...
        shezar_cohort_add_association($this->audience1->id, $this->program7->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience1->id, $this->program8->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience1->id, $this->program9->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);
    }
