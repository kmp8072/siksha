 ...
 * vendor/bin/phpunit shezar_cohort_events_testcase
 *
 */
class shezar_cohort_events_testcase extends advanced_testcase {
    ...
    public function test_draft_events() {
        ...
        $event = $events[0];
        $this->assertInstanceOf('shezar_cohort\event\draftcollection_saved', $event);
        ...
        $event = $events[1];
        $this->assertInstanceOf('shezar_cohort\event\members_updated', $event);
        ...
        $event = $events[0];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\draftcollection_discarded', $event);
        $this->assertEventContextNotUsed($event);
        $this->assertEquals('cohort', $eventdata['objecttable']);
    ...
    public function test_enrolled_course_item_events() {
        ...
        // Enrol course item.
        $assncourseid = shezar_cohort_add_association($cohortid, $course->id, COHORT_ASSN_ITEMTYPE_COURSE, $value);
        ...
        $event = $events[1];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\enrolled_course_item_added', $event);
        $this->assertEventContextNotUsed($event);
        ...
        // Delete course item.
        $sink->clear();
        shezar_cohort_delete_association($cohortid, $assncourseid, COHORT_ASSN_ITEMTYPE_COURSE, $value);
        $events = $sink->get_events();
        $this->assertCount(2, $events);
        $this->assertInstanceOf('core\event\enrol_instance_deleted', $events[0]);
        $event = $events[1];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\enrolled_course_item_deleted', $event);
        $this->assertEventContextNotUsed($event);
        $this->assertEquals('enrol', $eventdata['objecttable']);
        $this->assertEquals($assncourseid, $eventdata['objectid']);
    ...
    public function test_enrolled_program_item_events() {
        ...
        // Enrol program item.
        $assnprogid = shezar_cohort_add_association($cohortid, $program->id, COHORT_ASSN_ITEMTYPE_PROGRAM, $value);
        ...
        $event = $events[0];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\enrolled_program_item_added', $event);
        $this->assertEventContextNotUsed($event);
        ...
        // Delete program item.
        $sink->clear();
        shezar_cohort_delete_association($cohortid, $assnprogid, COHORT_ASSN_ITEMTYPE_PROGRAM, $value);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = $events[0];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\enrolled_program_item_deleted', $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_enrolled_certification_item_events() {
        ...
        $assncertifid = shezar_cohort_add_association($cohortid, $certification->id, COHORT_ASSN_ITEMTYPE_CERTIF, $value);
        ...
        $sink->clear();
        shezar_cohort_delete_association($cohortid, $assncertifid, COHORT_ASSN_ITEMTYPE_CERTIF, $value);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = $events[0];
        $eventdata = $event->get_data();
    ...
    public function test_cohort_update_operator_event() {
        ...
        $cohortid = $this->cohort->id;
        $sink->clear();
        shezar_cohort_update_operator($cohortid, $cohortid, COHORT_OPERATOR_TYPE_COHORT, COHORT_RULES_OP_OR);
    ...
    public function test_rule_events() {
        ...
        $event = \shezar_cohort\event\rule_updated::create_from_instance($ruleobj, $this->cohort);
        $event->trigger();
        ...
        $event = \shezar_cohort\event\rule_deleted::create_from_instance($ruleobj, $this->cohort);
        $event->trigger();
    ...
    public function test_ruleset_events() {
        ...
        $sink->clear();
        shezar_cohort_update_operator($this->cohort->id, $rulesetid, COHORT_OPERATOR_TYPE_RULESET, COHORT_RULES_OP_OR);
        ...
        $event = \shezar_cohort\event\ruleset_deleted::create_from_instance($ruleset, $this->cohort);
    ...
    public function test_visible_learning_events() {
        ...
        // Create association.
        $assid = shezar_cohort_add_association($cohortid, $program->id, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = $events[0];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\visible_learning_item_added', $event);
        $this->assertEventContextNotUsed($event);
        ...
        // Delete association.
        shezar_cohort_delete_association($cohortid, $assid, COHORT_ASSN_ITEMTYPE_PROGRAM, COHORT_ASSN_VALUE_VISIBLE);

        $events = $sink->get_events();
        $event = $events[0];
        $eventdata = $event->get_data();
        $this->assertInstanceOf('shezar_cohort\event\visible_learning_item_deleted', $event);
        $this->assertEventContextNotUsed($event);
        $this->assertEquals('cohort_visibility', $eventdata['objecttable']);
        $this->assertEquals($assid, $eventdata['objectid']);
        $this->assertEquals('d', $eventdata['crud']);
