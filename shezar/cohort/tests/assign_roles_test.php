 ...
 * vendor/bin/phpunit shezar_cohort_assign_roles_testcase
 *
 */
class shezar_cohort_assign_roles_testcase extends advanced_testcase {
    ...
    public function test_assign_roles_to_cohort() {
        ...
        $this->assertTrue(shezar_cohort_process_assigned_roles($this->cohort->id, $roles));

        // Validate the roles were added correctly to all members in the cohort.
        $roleids = array_keys($this->assignableroles);
        $rolesincohort = shezar_get_cohort_roles($this->cohort->id);
        ...
        $countmembers = count(shezar_get_members_cohort($this->cohort->id));
        foreach ($this->assignableroles as $key => $value) {
            $params = array(
        ...
        $this->assertTrue(shezar_cohort_process_assigned_roles($this->cohort->id, array()));

        // First validation: Roles were unassigned in the cohort_role table.
        $this->assertEmpty(shezar_get_cohort_roles($this->cohort->id));
        ...
        $members = shezar_get_members_cohort($this->cohort->id);
    ...
    public function test_unassign_roles_to_cohort() {
        ...
        $this->assertTrue(shezar_cohort_process_assigned_roles($cohort1->id, $roles));
        $this->assertTrue(shezar_cohort_process_assigned_roles($cohort2->id, $roles));
        $this->assertTrue(shezar_cohort_process_assigned_roles($cohort3->id, $roles));
        ...
        shezar_cohort_process_assig_roles();
