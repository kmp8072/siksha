 ...
 */
class shezar_cohort_course_audiencevisibility_testcase extends reportcache_advanced_testcase {
    /** @var stdClass $user1 */
    private $user1 = null;
    /** @var stdClass $user2 */
    private $user2 = null;
    ...
    protected function setUp() {
        ...
        shezar_cohort_add_association($this->audience1->id, $this->course2->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience2->id, $this->course2->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);

        // Assign audience2 to course3 and course4.
        shezar_cohort_add_association($this->audience2->id, $this->course3->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience2->id, $this->course4->id,
                                        COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
    ...
    protected function get_visible_info($audiencevisibility, $content, $course, $userid = null) {
        ...
        if ($CFG->enhancedcatalog) { // New catalog.
            $search = array();
            if (is_array($content)) {
                $search = shezar_search_for_value($content, 'course_courseexpandlink', shezar_SEARCH_OP_EQUAL, $course->fullname);
    ...
    protected function create_courses_old_visibility() {
        ...
        shezar_cohort_add_association($this->audience2->id, $this->course6->id, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
        shezar_cohort_add_association($this->audience1->id, $this->course5->id, COHORT_ASSN_ITEMTYPE_COURSE, COHORT_ASSN_VALUE_VISIBLE);
    }
}
