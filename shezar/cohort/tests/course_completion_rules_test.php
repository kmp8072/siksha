 ...
 * vendor/bin/phpunit shezar_cohort_course_completion_rules_testcase
 *
 */
class shezar_cohort_course_completion_rules_testcase extends reportcache_advanced_testcase {
    ...
    */
    ...
    public function test_coursecompletion_date_rule($params, $courses, $usercount) {
        ...
        // Use shezar_date_parse_from_format to transform a date to int as we do when creating this type of rules.
        if ($params['operator'] === COHORT_RULE_COMPLETION_OP_DATE_LESSTHAN ||
            $params['operator'] === COHORT_RULE_COMPLETION_OP_DATE_GREATERTHAN ) {
            $format = get_string('datepickerlongyearparseformat', 'shezar_core');
            $time = time() + ($params['date'] * DAYSECS);
            $params['date'] = shezar_date_parse_from_format($format, date($format, $time));
        }
