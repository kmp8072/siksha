 ...
 * vendor/bin/phpunit shezar_cohort_position_rules_testcase
 *
 */
class shezar_cohort_position_rules_testcase extends advanced_testcase {
    ...
    /** @var shezar_cohort_generator $cohort_generator */
    ...
    public function setUp() {
        ...
        $this->users = array();
        $this->dateformat = get_string('datepickerlongyearparseformat', 'shezar_core');

        // Set shezar_cohort generator.
        $this->cohort_generator = $this->getDataGenerator()->get_plugin_generator('shezar_cohort');

        // Set shezar_hierarchy generator.
        $this->hierarchy_generator = $this->getDataGenerator()->get_plugin_generator('shezar_hierarchy');

        // Create positions and position fw.
        $name = shezar_hierarchy_generator::DEFAULT_NAME_FRAMEWORK_POSITION;
        $name .= ' ' . shezar_generator_util::get_next_record_number('pos_framework', 'fullname', $name);
        ...
        for ($i = 1; $i <= self::TEST_POSITION_COUNT_MEMBERS; $i++) {
            ...
            $jobassignstartdate = shezar_date_parse_from_format($this->dateformat, $jobassignstartdate);
            $jobassignenddate = shezar_date_parse_from_format($this->dateformat, $jobassignenddate);
            $data = array('positionid' => $posid, 'startdate' => $jobassignstartdate, 'enddate' => $jobassignenddate);
            \shezar_job\job_assignment::create_default($this->{'user'.$i}->id, $data);
    ...
    public function test_position_date_rules($rulename, $params, $usercount, $sourcemembersmatched) {
        ...
        if ($params['operator'] === COHORT_RULE_DATE_OP_AFTER_FIXED_DATE ||
            ...
            $params['date'] = shezar_date_parse_from_format($this->dateformat, date($this->dateformat, $time));
        }
    ...
    public function test_jobassign_fullname_rule() {
        ...
        \shezar_job\job_assignment::create_default($my_users[0]->id, array('fullname' => 'Campaign Manager Online Marketing'));
        \shezar_job\job_assignment::create_default($my_users[1]->id, array('fullname' => 'Account Manager'));
        \shezar_job\job_assignment::create_default($my_users[2]->id, array('fullname' => 'Inside Sales Representative'));
        \shezar_job\job_assignment::create_default($my_users[3]->id, array('fullname' => 'Inside Sales Representative'));
    ...
    public function test_direct_reports_rule($params, $listofvalues, $usercount) {
        ...
        \shezar_job\job_assignment::get_first($this->user1->id)->update(array('managerjaid' => $manager1ja->id));
        \shezar_job\job_assignment::get_first($this->user2->id)->update(array('managerjaid' => $manager2ja->id));
        \shezar_job\job_assignment::get_first($this->user3->id)->update(array('managerjaid' => $manager1ja->id));
        \shezar_job\job_assignment::get_first($this->user4->id)->update(array('managerjaid' => $manager2ja->id));
        \shezar_job\job_assignment::get_first($this->user5->id)->update(array('managerjaid' => $manager1ja->id));
    ...
    public function test_manager_rule($params, $managerids, $usercount) {
        ...
        // Hierarchy of managers.
        $manager1ja = \shezar_job\job_assignment::create_default($manager1->id);
        $manager2ja = \shezar_job\job_assignment::create_default($manager2->id, array('managerjaid' => $manager1ja->id));
        $manager3ja = \shezar_job\job_assignment::create_default($manager3->id, array('managerjaid' => $manager2ja->id));
        ...
        \shezar_job\job_assignment::get_first($this->user3->id)->update(array('managerjaid' => $manager1ja->id));
        \shezar_job\job_assignment::get_first($this->user4->id)->update(array('managerjaid' => $manager2ja->id));
        \shezar_job\job_assignment::get_first($this->user5->id)->update(array('managerjaid' => $manager3ja->id));
    ...
    public function test_position_rule($params, $positions, $usercount, $sourcemembersmatched) {
        ...
        // Assign positions.
        \shezar_job\job_assignment::create_default($newuser1->id, array('positionid' => $this->pos4->id));
        \shezar_job\job_assignment::create_default($newuser2->id, array('positionid' => $this->pos5->id));
        \shezar_job\job_assignment::create_default($newuser3->id, array('positionid' => $this->pos5->id));
        \shezar_job\job_assignment::create_default($newuser4->id, array('positionid' => $this->pos4->id));
        \shezar_job\job_assignment::create_default($newuser5->id, array('positionid' => $this->pos4->id));
