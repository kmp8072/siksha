...
require_once($CFG->dirroot . '/shezar/cohort/db/upgradelib.php');

/**
 * DB upgrades for shezar dynamic cohorts
 */

function xmldb_shezar_cohort_upgrade($oldversion) {
    ...
    if ($oldversion < 2012060500) {
                    ...
                    if (!empty($criterion->profilefield) && !empty($criterion->profilefieldvalues)) {
                        if (substr($criterion->profilefield, 0, 11) == 'customfield') {
                            ...
                            if ($fieldtype == 'textarea') {
                                $warning = get_string('error:badruleonupgrade', 'shezar_cohort', $cohort->name);
        ...
        upgrade_plugin_savepoint(true, 2012060500, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2012061800) {
        ...
        upgrade_plugin_savepoint(true, 2012061800, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2012072600) {
        ...
        upgrade_plugin_savepoint(true, 2012072600, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2012080202) {
        ...
        upgrade_plugin_savepoint(true, 2012080202, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2012082100) {
        ...
        upgrade_plugin_savepoint(true, 2012082100, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2012102300) {
        ...
        upgrade_plugin_savepoint(true, 2012102300, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2013100100) {
        ...
        upgrade_plugin_savepoint(true, 2013100100, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2013100300) {
        ...
        upgrade_plugin_savepoint(true, 2013100300, 'shezar', 'cohort');
    }

    if ($oldversion < 2013103000) {
        // Don't run again if this upgrade has occurred before.
        $hasrun = get_config('shezar_cohort', 'cohort_rule_fix_has_run');
        if (empty($hasrun)) {
            ...
            if (!empty($brokenrules)) {
                echo $OUTPUT->notification(
                    get_string('cohortbugheading', 'shezar_cohort'),
                    'notifysuccess');
                ...
                foreach ($brokenrules as $rule) {
                    if (shezar_cohort_is_rule_fixable($rule)) {
                        $rulestofix[] = $rule;
                    } else {
                        $unfixablerules[] = $rule;
                    }
                ...
                if (!empty($rulestofix)) {
                    echo $OUTPUT->notification(
                        get_string('cohortbugfixingxrules', 'shezar_cohort', count($rulestofix)),
                        'notifysuccess');
                    shezar_cohort_remap_rules($rulestofix);
                }

                // Record that we've run this upgrade now.
                set_config('cohort_rule_fix_has_run', 1, 'shezar_cohort');

                // Advise users about remaining bad rules.
                if (!empty($unfixablerules)) {
                    $output = $PAGE->get_renderer('shezar_core', null);
                    echo $output->show_text_broken_rules($unfixablerules);
                } else {
                    echo $OUTPUT->notification(
                        get_string('cohortbugnounfixable', 'shezar_cohort'),
                        'notifysuccess');
                }
        ...
        // Cohort savepoint reached.
        upgrade_plugin_savepoint(true, 2013103000, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2014031900) {
        ...
        upgrade_plugin_savepoint(true, 2014031900, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2014040700) {
        ...
        // Cohort savepoint reached.
        upgrade_plugin_savepoint(true, 2014040700, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2014090300) {
        ...
        upgrade_plugin_savepoint(true, 2014090300, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2014120400) {
        ...
        upgrade_plugin_savepoint(true, 2014120400, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2015020900) {
        ...
        // Main savepoint reached.
        upgrade_plugin_savepoint(true, 2015020900, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2015042200) {
        ...
        // Main savepoint reached.
        upgrade_plugin_savepoint(true, 2015042200, 'shezar', 'cohort');
    }

    if ($oldversion < 2016061000) {
        // This flag can be safely removed now because the plugin version itself does the same job.
        get_config('shezar_cohort', 'cohort_rule_fix_has_run');
        // Main savepoint reached.
        upgrade_plugin_savepoint(true, 2016061000, 'shezar', 'cohort');
    }
    ...
    if ($oldversion < 2016071800) {
        ...
        upgrade_plugin_savepoint(true, 2016071800, 'shezar', 'cohort');
    }

    if ($oldversion < 2016072800) {

        // Move old pos_assignment rules to job_assignment rules.
        shezar_cohort_migrate_position_rules();

        // Main savepoint reached.
        upgrade_plugin_savepoint(true, 2016072800, 'shezar', 'cohort');
    }
