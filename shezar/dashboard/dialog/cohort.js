...
M.shezar_dashboardcohort = M.shezar_dashboardcohort || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_dashboardcohort.init()-> jQuery dependency required for this module.');
        }
    ...
    init_dialogs: function() {
        var url = M.cfg.wwwroot + '/shezar/dashboard/dialog/';

        var ehandler = new shezarDialog_handler_dashboardcohorts();
        ...
        shezarDialogs['id_cohortsbtn'] = new shezarDialog(
            'dashboard-cohorts-assigned-dialog',
            'id_cohortsbtn',
            {
                buttons: dbuttons,
                title: '<h2>' + M.util.get_string('assignedcohorts', 'shezar_dashboard') + '</h2>'
            },
...
shezarDialog_handler_dashboardcohorts = function() {
    this.baseurl = '';
...
shezarDialog_handler_dashboardcohorts.prototype = new shezarDialog_handler_treeview_multiselect();
 ...
 */
shezarDialog_handler_dashboardcohorts.prototype._update = function(response) {
    ...
    if (newids.length > 0) {
        ...
        var ajax_url = M.cfg.wwwroot + '/shezar/dashboard/dialog/cohort_item.php?itemid=' + newids.join(',') + params;
 ...
 */
shezarDialog_handler_dashboardcohorts.prototype.cohort_item_exists = function(itemid) {
...
shezarDialog_handler_dashboardcohorts.prototype.check_table_hidden_status = function() {
...
shezarDialog_handler_dashboardcohorts.prototype.add_cohort_delete_event_handlers = function() {
 ...
 */
shezarDialog_handler_dashboardcohorts.prototype.add_published_status_change_handlers = function() {
 ...
 * @param int itemid item id
 */
shezarDialog_handler_dashboardcohorts.prototype.add_cohort_item = function(itemid) {
 ...
 * @param string html
 */
shezarDialog_handler_dashboardcohorts.prototype.create_item = function(html) {
    var element = $(html);
    this.cohort_table.append(element);
};

shezarDialog_handler_dashboardcohorts.prototype.remove_cohort_item = function(item) {
