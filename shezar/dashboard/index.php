...
$availabledash = array_keys(shezar_dashboard::get_user_dashboards($userid));

// Validate dashboard id.
if ($id) {
    if (!in_array($id, $availabledash)) {
        // If not available, redirect to default dashboard.
        redirect(new moodle_url('/shezar/dashboard/index.php'));
    }
} else if (get_user_preferences('user_home_page_preference') == HOMEPAGE_shezar_DASHBOARD &&
        in_array(get_user_preferences('user_home_shezar_dashboard_id'), $availabledash)) {
    // Set home page dashboard id.
    $id = get_user_preferences('user_home_shezar_dashboard_id');
...
$params = array('id' => $id);
$PAGE->set_url('/shezar/dashboard/index.php', $params);
$PAGE->set_shezar_menu_selected('dashboard');

if (!$id) {
    $header = $SITE->shortname. ': ' . get_string('dashboard', 'shezar_dashboard');
    $PAGE->set_context(context_user::instance($USER->id));
    shezar_set_notification(get_string('noavailabledashboards', 'shezar_dashboard'));
} else {
    ...
    $PAGE->set_blocks_editing_capability('shezar/dashboard:manageblocks');
    ...
    if (!empty($CFG->allowdefaultpageselection) and (get_home_page() == HOMEPAGE_SITE)) {
        if (optional_param('setdefaulthome', 0, PARAM_BOOL)) {
            require_sesskey();
            set_user_preference('user_home_page_preference', HOMEPAGE_shezar_DASHBOARD);
            set_user_preference('user_home_shezar_dashboard_id', $id);
            $url = new moodle_url('/shezar/dashboard/index.php', array('id' => $id));
            shezar_set_notification(get_string('userhomepagechanged', 'shezar_dashboard'), $url, array('class' => 'notifysuccess'));
        }
        $newhomeurl = new moodle_url('/shezar/dashboard/index.php', array('setdefaulthome' => 1, 'id' => $id, 'sesskey' => sesskey()));
        $PAGE->settingsnav->add(get_string('makedashboardmyhomepage', 'shezar_dashboard'), $newhomeurl, navigation_node::TYPE_SETTING);
    }
    ...
    if ($PAGE->user_allowed_editing() && !$dashboard->is_locked()) {
        if ($reset !== null) {
            if (!is_null($userid)) {
                ...
                redirect(new moodle_url('/shezar/dashboard/index.php', array('id' => $dashboard->get_id())));
            }
        } else if ($edit !== null) {
            $USER->editing = $edit;
        ...
        $resetstring = get_string('resetdashboard', 'shezar_dashboard');
        $reseturl = new moodle_url("/shezar/dashboard/index.php", array('id' => $id, 'edit' => 1, 'reset' => 1));

        if ($userpageid == 'default') {
            // Viewing a system page - let the user customise it.
            $editstring = get_string('customiseon', 'shezar_dashboard');
            $params['edit'] = 1;
        } else if (empty($edit)) {
            $editstring = get_string('customiseon', 'shezar_dashboard');
        } else {
            $editstring = get_string('customiseoff', 'shezar_dashboard');
            $resetbutton = $OUTPUT->single_button($reseturl, $resetstring);
        }

        $editurl = new moodle_url("/shezar/dashboard/index.php", $params);
    ...
    // Set page buttons.
    $managebutton = '';
    if (has_capability('shezar/dashboard:manage', context_system::instance())) {
        $managestring = get_string('managedashboards', 'shezar_dashboard');
        $managebutton = $OUTPUT->single_button(new moodle_url("/shezar/dashboard/manage.php"), $managestring);
    }
