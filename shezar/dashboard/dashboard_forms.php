 ...
 */
class shezar_dashboard_edit_form extends moodleform {
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('name', 'shezar_dashboard'), 'maxlength="255" size="50"');
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required');

        $mform->addElement('checkbox', 'locked', get_string('locked', 'shezar_dashboard'));
        $mform->addHelpButton('locked', 'locked', 'shezar_dashboard');

        // Cohorts.
        $mform->addElement('header', 'assignedcohortshdr', get_string('assignedcohorts', 'shezar_dashboard'));
        ...
        // Availability.
        $availopts = array();
        $availopts[] = $mform->createElement('radio', 'published', '', get_string('availablenone', 'shezar_dashboard'), shezar_dashboard::NONE);
        $availopts[] = $mform->createElement('radio', 'published', '', get_string('availableall', 'shezar_dashboard'), shezar_dashboard::ALL);
        $availopts[] = $mform->createElement('radio', 'published', '', get_string('availableaudience', 'shezar_dashboard'), shezar_dashboard::AUDIENCE);

        $mform->addGroup($availopts, 'published', get_string('availability', 'shezar_dashboard'), html_writer::empty_tag('br'));

        // Set a default.
        $defaultpublished = $this->_customdata['dashboard']->published;
        if (empty($this->_customdata['dashboard']->id)) {
            $defaultpublished = shezar_dashboard::ALL;
        }
        ...
        $cohortsclass = new shezar_cohort_dashboard_cohorts();
        $cohortsclass->build_table($dashboard->id);
        $mform->addElement('html', $cohortsclass->display(true, 'assigned'));

        $mform->addElement('button', 'cohortsbtn', get_string('assigncohorts', 'shezar_dashboard'));
        $mform->disabledIf('cohortsbtn', 'published[published]', 'neq', 1);
        $mform->setExpanded('assignedcohortshdr');

        $submittitle = get_string('createdashboard', 'shezar_dashboard');
        if ($dashboard->id > 0) {
            $submittitle = get_string('savechanges', 'shezar_dashboard');
        }
 ...
 */
class shezar_cohort_dashboard_cohorts extends shezar_cohort_course_cohorts {
    /**
    ...
    public function build_table($dashboardid) {
        ...
        $sql = "SELECT c.id, c.name AS fullname, c.cohorttype
                FROM {shezar_dashboard_cohort} tdc
