...
/**
 * Test shezar dashboard
 *
 * To test, run this from the command line from the $CFG->dirroot
 * vendor/bin/phpunit shezar_dashboard_testcase
 *
 */
class shezar_dashboard_testcase extends advanced_testcase {
    /**
    ...
    public function test_dashboard_create() {
        ...
        $cohorts_gen = $this->getDataGenerator()->get_plugin_generator('shezar_cohort');
        ...
        $count = $DB->count_records('block_instances',
                array('pagetypepattern' => 'my-shezar-dashboard-' . $id, 'blockname' => 'shezar_dashboard'));
    ...
    public function test_dashboard_edit() {
        ...
        $dashboard = $this->getDataGenerator()->get_plugin_generator('shezar_dashboard')->create_dashboard($data);
        ...
        $this->assertEquals(shezar_dashboard::NONE, $check->get_published());
        foreach ($cohorts as $cohort) {
            $this->assertContains($cohort, $check->get_cohorts());
        }
    }
    ...
    public function test_dashboard_clone() {
        ...
        /* @var shezar_dashboard $dashboard */
        $dashboard = $this->getDataGenerator()->get_plugin_generator('shezar_dashboard')->create_dashboard($data);
        $newdata = new stdClass();
        ...
        $page->set_pagetype('my-shezar-dashboard-' . $dashboard->get_id());
        ...
        $clonedata->published = shezar_dashboard::ALL;
        $clonedata->locked = 1;
        ...
        unset($dashboard);
        unset($clone);
        $original = new shezar_dashboard($originalid);
        $clone = new shezar_dashboard($cloneid);
        ...
        $this->assertEquals(shezar_dashboard::NONE, $original->get_published());
        foreach ($cohorts as $cohort) {
        ...
        $this->assertEquals(shezar_dashboard::ALL, $clone->get_published());
        foreach ($cohorts as $cohort) {
            $this->assertContains($cohort, $clone->get_cohorts());
        }
        ...
        $expected = array('block_shezar_dashboard', 'block_html');
        $actual = array();
    ...
    public function test_manage_list() {
        ...
        $listbefore = shezar_dashboard::get_manage_list();
        $cohorts_gen = $this->getDataGenerator()->get_plugin_generator('shezar_cohort');
        $cohorts = array($cohorts_gen->create_cohort()->id, $cohorts_gen->create_cohort()->id, $cohorts_gen->create_cohort()->id);
        $dashboard_gen = $this->getDataGenerator()->get_plugin_generator('shezar_dashboard');
        ...
        $listafter = shezar_dashboard::get_manage_list();
    ...
    public function test_user_copy() {
        ...
        // Check that instance of shezar_dashboard block is created for user.
        $count = $DB->count_records('block_instances',
                array('pagetypepattern' => 'my-shezar-dashboard-' . $dashboard->get_id(),
                      'blockname' => 'shezar_dashboard',
    ...
    public function test_get_user_dashboards() {
        ...
        $user1dashes = shezar_dashboard::get_user_dashboards($user1->id);
        ...
        $user2dashes = shezar_dashboard::get_user_dashboards($user2->id);
        ...
        $user3dashes = shezar_dashboard::get_user_dashboards($user3->id);
    ...
    public function test_user_reset() {
        ...
        // Check that instance of shezar_dashboard block is deleted for user.
        $count = $DB->count_records('block_instances',
                array('pagetypepattern' => 'my-shezar-dashboard-' . $dashboard->get_id(),
                      'blockname' => 'shezar_dashboard',
                      'subpagepattern' => $pageid));
    ...
    public function test_sorting() {
        global $DB;
        $this->resetAfterTest(true);
        $DB->delete_records('shezar_dashboard');
        ...
        $order1 = $DB->get_records('shezar_dashboard', array(), 'sortorder');
        ...
        $order2 = $DB->get_records('shezar_dashboard', array(), 'sortorder');
        ...
        // Move up 3 => 231.
        $dashboard3 = new shezar_dashboard($dashboard3->get_id());
        $dashboard3->move_up();
        $order3 = $DB->get_records('shezar_dashboard', array(), 'sortorder');
        ...
        $dashboard3 = new shezar_dashboard($dashboard3->get_id());
        $dashboard3->move_up();
        $order4 = $DB->get_records('shezar_dashboard', array(), 'sortorder');
        ...
        // Move down 2 => 312.
        $dashboard2 = new shezar_dashboard($dashboard2->get_id());
        $dashboard2->move_down();
        $order5 = $DB->get_records('shezar_dashboard', array(), 'sortorder');
    ...
    public function test_delete() {
        ...
        // Check order.
        $order = $DB->get_records('shezar_dashboard', array(), 'sortorder');
        $i = 1;
        ...
        $cohorts = $DB->get_records_sql("SELECT * FROM {shezar_dashboard_cohort} WHERE dashboardid IN (?, ?)",
                array($dashboard1id, $dashboard3id));
        $this->assertCount(2, $cohorts);

        // Check that assignment of dashbord2 gone.
        $cohorts2 = $DB->get_records('shezar_dashboard_cohort', array('dashboardid' => $dashboard2id));
        $this->assertEmpty($cohorts2);

        // Check that user copy of dashboard2 removed.
        $userpages2 = $DB->get_records('shezar_dashboard_user', array('dashboardid' => $dashboard2id));
        $this->assertEmpty($userpages2);

        $count2 = $DB->count_records('block_instances', array('pagetypepattern' => 'my-shezar-dashboard-' . $dashboard2id));
        $this->assertEquals(0, $count2);

        // Check that user copy of dashboard3 left.
        $userpages3 = $DB->get_records('shezar_dashboard_user', array('dashboardid' => $dashboard3id));
        $this->assertCount(1, $userpages3);

        $count3 = $DB->count_records('block_instances', array('pagetypepattern' => 'my-shezar-dashboard-' . $dashboard3id));
    ...
    public function test_allowed_all() {
        ...
        $data = array(
            'name' => 'Test',
            'locked' => 1,
            'published' => shezar_dashboard::ALL,
            'cohorts' => array()
        );
        ...
        $user1 = $this->getDataGenerator()->create_user();
        $user1dash = shezar_dashboard::get_user_dashboards($user1->id);
