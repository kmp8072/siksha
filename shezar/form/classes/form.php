...
abstract class form implements \templatable {
    ...
    public function get_action_url() {
        ...
        if (AJAX_SCRIPT) {
            ...
            if ($action === false) {
                ...
                return new \moodle_url(('/shezar/form/ajax.php'));
            }
            ...
            if (strpos($stripped_action, $stripped_wwwroot) !== 0) {
                throw new \coding_exception('shezar form ajax script returns an original action that was outside wwwroot');
            }
    ...
    final protected function is_valid_submission() {
        ...
        foreach ($errors as $elname => $error) {
            ...
            $item = $this->model->find($elname, 'get_name', 'shezar_form\item');
            if (!$item) {
    ...
    final public function update_file_area($elname, \context $context = null, $itemid = null) {
        $element = $this->model->find($elname, 'get_name', 'shezar_form\item');
        if (!$element) {
    ...
    public function __call($name , array $arguments) {
        ...
        if ($name === 'display') {
            debugging('form::display() is deprecated in shezar forms, use "echo form::render()" instead', DEBUG_DEVELOPER);
            echo $this->render();
            return null;
        }
        if ($name === 'add_action_buttons') {
            debugging('form::add_action_buttons() is deprecated in shezar forms, use "$this->model->add_action_buttons()" instead', DEBUG_DEVELOPER);
            return call_user_func_array(array($this->model, 'add_action_buttons'), $arguments);
        }
        if ($name === 'focus') {
            debugging('form::focus() is not available any more in shezar forms', DEBUG_DEVELOPER);
            return null;
        }
