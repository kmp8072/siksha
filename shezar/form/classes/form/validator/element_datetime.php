...
use shezar_form\item,
    ...
    shezar_form\form\element\datetime;

/**
 * shezar form validator for datetime element.
 *
 * @package   shezar_form
...
class element_datetime extends element_validator {
     ...
     *
     * @throws \coding_exception If the item is not an instance of \shezar_form\form\element\datetime
     * @param item $item
     */
    ...
    public function validate() {
        ...
        if (isset($data[$name])) {
            $value = $data[$name];
            if (!is_number($value)) {
                $this->element->add_error(get_string('datetimevalidationerror', 'shezar_form'));
                return;
            }
        }
