...
use shezar_form\element,
    ...
    shezar_form\form\validator\element_datetime;
...
class datetime extends element {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $result = array(
            'form_item_template' => 'shezar_form/element_datetime',
            'name__' . $this->get_name() => true,
            'name' => $this->get_name(),
            'id' => $this->get_id(),
            'label' => (string)$this->label,
            'frozen' => $this->is_frozen(),
            'amdmodule' => 'shezar_form/form_element_datetime',
        );
        ...
        foreach (\core_date::get_list_of_timezones($attributes['tz'], true) as $value => $text) {
            if ($value == 99) {
                ...
                $text = get_string('mytimezone', 'shezar_form', $usertz);
            }
