...
use shezar_form\element,
    shezar_form\file_area,
    shezar_form\form\validator\attribute_required,
    shezar_form\form\validator\element_editor,
    shezar_form\item;
...
class editor extends element {
    ...
    public function set_parent(item $parent = null) {
        ...
        if ($parent) {
            ...
            if ($parent->get_model()->find(true, 'is_name_used', 'shezar_form\item', true, array($name . 'format'), false)) {
                throw new \coding_exception('Duplicate name "' . $name . 'format" detected!');
            }
            if ($parent->get_model()->find(true, 'is_name_used', 'shezar_form\item', true, array($name . 'filearea'), false)) {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $result = array(
            'form_item_template' => 'shezar_form/element_editor',
            'name__' . $this->get_name() => true,
            'name' => $this->get_name(),
            'id' => $this->get_id(),
            'label' => (string)$this->label,
            'frozen' => $this->is_frozen(),
            'amdmodule' => 'shezar_form/form_element_editor',
        );
        ...
        $editor->shezar_form_use_editor($result, $editoroptions, $fpoptions, $fptemplates);
    ...
    protected function get_current_file_area() {
        ...
        if ($current) {
            ...
            debugging('Current filearead must be instance of \shezar_form\file_area', DEBUG_DEVELOPER);
        }
        return null;
    }
