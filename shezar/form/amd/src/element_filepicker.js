...
define(['core/yui'], function(Y) {
    ...
    M.shezar_form.element_filepicker = M.shezar_form.element_filepicker || {};
    M.shezar_form.element_filepicker.instances = M.shezar_form.element_filepicker.instances || [];

    M.shezar_form.element_filepicker.callback = function (params) {
        var html = '<div class="filepicker-container"><a href="' + params.url + '">' + params.file + '</a></div>';
        html += '<div class="dndupload-progressbars"></div>';
        M.shezar_form.element_filepicker.Y.one('#file_info_' + params.client_id + ' .filepicker-filename').setContent(html);
        ...
        M.shezar_form.element_filepicker.instances[elementname].fileadded = true;
    };

    /**
     * This function is called for each file picker on page.
     */
    M.shezar_form.element_filepicker.init = function (Y, options) {
        M.shezar_form.element_filepicker.Y = Y;
        // For client side validation, initialize file status for this filepicker.
        M.shezar_form.element_filepicker.instances[options.elementname] = {};
        M.shezar_form.element_filepicker.instances[options.elementname].fileadded = false;

        // Set filepicker callback.
        options.formcallback = M.shezar_form.element_filepicker.callback;
    ...
    return {
        init_filepicker : function(options) {
            Y.use(['core_filepicker', 'node', 'node-event-simulate', 'core_dndupload'], function (Y) {
                ...
                M.shezar_form.element_filepicker.init(Y, options);
            });
        }
    };
});
