 ...
 *
 * NOTE: this file is based on lib/form/filemanager.js code
 *
 * @module     shezar_form/element_filepicker
...
define(['core/yui', 'core/templates', 'core/str'], function(Y, templates, core_strings) {
    ...
    M.shezar_form = M.shezar_form || {};
    M.shezar_form.element_filemanager = M.shezar_form.element_filemanager || {};
    M.shezar_form.element_filemanager.templates = M.shezar_form.element_filemanager.templates || {};

    /**
     * This function is called for each file picker on page.
     */
    M.shezar_form.element_filemanager.init = function(Y, options) {
        ...
        Y.extend(FileManagerHelper, Y.Base, {
            ...
            initializer: function(options) {
                ...
                this.selectnode = Y.Node.createWithFilesSkin(M.shezar_form.element_filemanager.templates.fileselectlayout);
            ...
            print_msg: function(msg, type) {
                ...
                if (!this.msg_dlg) {
                    this.msg_dlg_node = Y.Node.createWithFilesSkin(M.shezar_form.element_filemanager.templates.message);
                ...
                if (this.options.subdirs && !this.is_frozen()) {
                    button_create.on('click',function(e) {
                        ...
                        if (!this.mkdir_dialog) {
                            var node = Y.Node.createWithFilesSkin(M.shezar_form.element_filemanager.templates.mkdir);
            ...
            view_files: function(appendfiles) {
                ...
                if (this.viewmode == 2 || this.viewmode == 3) {
                    element_template = Y.Node.create(M.shezar_form.element_filemanager.templates.listfilename);
                } else {
                    this.viewmode = 1;
                    element_template = Y.Node.create(M.shezar_form.element_filemanager.templates.iconfilename);
                }
                var options = {
            ...
            set_current_tree: function(tree) {
                ...
                var node = selectnode.one('.fp-path select');
                node.setContent('');
                for (var i in list) {
            ...
            show_confirm_dialog: function(dialog_options) {
                ...
                if (!this.confirm_dlg) {
                    this.confirm_dlg_node = Y.Node.createWithFilesSkin(M.shezar_form.element_filemanager.templates.confirmdialog);
            ...
            select_file: function(node) {
                        ...
                        callback: function(id, obj, args) {
                            ...
                            if (scope.selectui.fileinfo && node &&
                                ...
                                if (obj.original) {
                                    ...
                                    selectnode.one('.fp-original .fp-value').setContent(
                                        M.util.get_string('unknownsource', 'repository'));
                                }
                            }
                        }
                    }, false);
                ...
                selectnode.one('.fp-refcount').setContent(node.refcount ? M.util.get_string('referencesexist', 'repository',
                    node.refcount) : '');
                ...
                var editstr = M.util.get_string('edit', 'moodle') + ' ' + nodename;
                Y.one('#fm-dialog-label_' + selectnode.get('id')).setContent(Y.Escape.html(editstr));
                // Show panel.
                this.selectui.show();
    ...
    return {
        init_filemanager : function(options) {
            ...
            requiredstrings.push({key: 'error', component: 'moodle'});
            requiredstrings.push({key: 'info', component: 'moodle'});
            requiredstrings.push({key: 'newfolder', component: 'repository'});
            requiredstrings.push({key: 'invalidjson', component: 'repository'});
            requiredstrings.push({key: 'draftareanofiles', component: 'repository'});
            requiredstrings.push({key: 'entername', component: 'repository'});
            requiredstrings.push({key: 'confirmrenamefolder', component: 'repository'});
            requiredstrings.push({key: 'enternewname', component: 'repository'});
            requiredstrings.push({key: 'confirmrenamefile', component: 'repository'});
            requiredstrings.push({key: 'confirmdeletefolder', component: 'repository'});
            requiredstrings.push({key: 'confirmdeletefilewithhref', component: 'repository'});
            requiredstrings.push({key: 'confirmdeletefile', component: 'repository'});
            requiredstrings.push({key: 'unknownsource', component: 'repository'});
            requiredstrings.push({key: 'referencesexist', component: 'repository'});
            requiredstrings.push({key: 'unknownsource', component: 'repository'});
            requiredstrings.push({key: 'unknownsource', component: 'repository'});
            ...
            core_strings.get_strings(requiredstrings).done(function () {
                ...
                Y.use(reqmodules, function (Y) {
                    ...
                    M.shezar_form.element_filemanager.templates = options.fmtemplates;
                    options.fmtemplates = null;
                    M.shezar_form.element_filemanager.init(Y, options);
                });
            });
        }
    };
