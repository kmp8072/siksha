 ...
 */
class shezar_flavour_overview_setting_testcase extends advanced_testcase {
    ...
    public function test_current_flavour() {
        ...
        $this->assertInstanceOf('shezar_flavour\\overview_setting', $setting);
        $this->assertSame('enablegoals', $setting->name);
        $this->assertSame('moodle', $setting->component);
        $this->assertEquals(shezar_DISABLEFEATURE, $setting->currentvalue);
        ...
        set_config('enablegoals', shezar_SHOWFEATURE);
        $overview = new overview();
        $setting = $overview->settings['moodle|enablegoals'];
        $this->assertInstanceOf('shezar_flavour\\overview_setting', $setting);
        $this->assertSame('enablegoals', $setting->name);
        $this->assertSame('moodle', $setting->component);
        $this->assertEquals(shezar_SHOWFEATURE, $setting->currentvalue);
        $this->assertTrue($setting->is_prohibited('flavour_test'));
        $this->assertFalse($setting->is_prohibited('flavour_enterprise'));
        $this->assertTrue($setting->is_on());
        $this->assertFalse($setting->is_set_in_configphp());

        set_config('enablegoals', shezar_HIDEFEATURE);
        $overview = new overview();
        $setting = $overview->settings['moodle|enablegoals'];
        $this->assertInstanceOf('shezar_flavour\\overview_setting', $setting);
        $this->assertSame('enablegoals', $setting->name);
        $this->assertSame('moodle', $setting->component);
        $this->assertEquals(shezar_HIDEFEATURE, $setting->currentvalue);
        $this->assertTrue($setting->is_prohibited('flavour_test'));
        $this->assertFalse($setting->is_prohibited('flavour_enterprise'));
        $this->assertTrue($setting->is_on());
        $this->assertFalse($setting->is_set_in_configphp());

        set_config('enablegoals', shezar_DISABLEFEATURE);
        $CFG->enablegoals = (string)shezar_SHOWFEATURE;
