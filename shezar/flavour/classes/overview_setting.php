...
namespace shezar_flavour;

defined('MOODLE_INTERNAL') || die();

/**
 * shezar feature overview setting renderable
 *
 * @author Sam Hemelryk <sam.hemelryk@shezarlms.com>
 * @package shezar_flavour
 */
class overview_setting implements \renderable {
    ...
    public function get_name() {
        if (get_string_manager()->string_exists('setting_' . $this->component . '_' . $this->name, 'shezar_flavour')) {
            // If there is a string defined in the flavour pack use that.
            return get_string('setting_' . $this->component . '_' . $this->name, 'shezar_flavour');
        } else if ($this->component === 'moodle' and get_string_manager()->string_exists('setting_core_' . $this->name, 'shezar_flavour')) {
                // If there is a string defined in the flavour pack use that.
                return get_string('setting_core_' . $this->name, 'shezar_flavour');
        } else {
    ...
    public function get_description() {
        ...
        if (get_string_manager()->string_exists('setting_' . $this->component . '_' . $this->name . '_desc', 'shezar_flavour')) {
            // If there is a string defined in the flavour pack use that.
            return get_string('setting_' . $this->component . '_' . $this->name . '_desc', 'shezar_flavour');
        } else if ($this->component === 'moodle' and  get_string_manager()->string_exists('setting_core_' . $this->name.'_desc', 'shezar_flavour')) {
                // If there is a string defined in the flavour pack use that.
                return get_string('setting_core_' . $this->name . '_desc', 'shezar_flavour');
        } else {
            // Finally just use the name of setting as is.
            return '';
        }
    ...
    public function is_on() {
        $shezarfeatures = shezar_advanced_features_list();
        foreach ($shezarfeatures as $name) {
            if ($this->name === 'enable' . $name) {
                return ((int)$this->currentvalue !== shezar_DISABLEFEATURE);
            }
        }
