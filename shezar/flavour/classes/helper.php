...
class helper {
    ...
    protected static function enforce_flavour(definition $flavour, $forceactivation) {
        ...
        $previous = get_config('shezar_flavour', 'currentflavour');

        if ($previous !== $component) {
            set_config('currentflavour', $component, 'shezar_flavour');
    ...
    public static function get_available_flavour_definitions() {
        ...
        foreach (\core_component::get_plugin_list('flavour') as $fname => $unused) {
            ...
            if (!$definition) {
                debugging("Invalid shezar flavour '$component' detected, verify the definition class exists." , DEBUG_DEVELOPER);
                continue;
            }
