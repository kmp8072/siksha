...
class feedback360 {
    ...
    public function save() {
        ...
        if ($this->id > 0) {
            ...
            \shezar_feedback360\event\feedback360_updated::create_from_instance($this)->trigger();
        } else {
            $todb->userid = $USER->id;
            $this->id = $DB->insert_record('feedback360', $todb);

            \shezar_feedback360\event\feedback360_created::create_from_instance($this)->trigger();
        }
        // Refresh data.
        $this->load($this->id);
        return $this;
    ...
    public function delete() {
        ...
        $assign = new shezar_assign_feedback360('feedback360', $this);
        $assign->delete();

        // Delete files.
        $fs = get_file_storage();
        $fs->delete_area_files($TEXTAREA_OPTIONS['context']->id, 'shezar_feedback360', 'feedback360', $this->id);
        // Delete the feedback360.
        $DB->delete_records('feedback360', array('id' => $this->id));

        \shezar_feedback360\event\feedback360_deleted::create_from_instance($this)->trigger();
    }
    ...
    public function load() {
        ...
        if (!$feedback360) {
            throw new feedback360_exception(get_string('loadfeedback360failure', 'shezar_feedback360'), 1);
        }
    ...
    public static function get_manage_list($userid = 0) {
        ...
        require_capability('shezar/feedback360:managefeedback360', $context);
    ...
    public static function duplicate($feedback360id) {
        ...
        $context = context_system::instance();
        require_capability('shezar/feedback360:clonefeedback360', $context);
        ...
        $assign = new shezar_assign_feedback360('feedback360', new feedback360($feedback360id));
    ...
    public function set_status($newstatus) {
        ...
        if (!in_array($newstatus, $allowedstatus[$this->status])) {
            ...
            throw new feedback360_exception(get_string('error:cannotchangestatus', 'shezar_feedback360', $a));
        } else {
    ...
    public static function cancel_resp_assignment($resp_assignment, $asmanager = false) {
        ...
        if (!is_object($resp_assignment)) {
            print_error('error:unexpectedtype', 'shezar_feedback360');
        }
        ...
        if (!empty($resp_assignment->feedback360emailassignmentid)) {
            ...
            if ($asmanager) {
                $subject = get_string('managercancellationsubject', 'shezar_feedback360', $stringvars);
                $message = get_string('managercancellationemail', 'shezar_feedback360', $stringvars);
            } else {
                $subject = get_string('cancellationsubject', 'shezar_feedback360', $stringvars);
                $message = get_string('cancellationemail', 'shezar_feedback360', $stringvars);
            }
            ...
            $event->userto = $userto;
            if ($asmanager) {
                $event->subject = $stringmanager->get_string('managercancellationsubject', 'shezar_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessage = $stringmanager->get_string('managercancellationalert', 'shezar_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessagehtml = $stringmanager->get_string('managercancellationalert', 'shezar_feedback360',
                        $stringvars, $userto->lang);
            } else {
                $event->subject = $stringmanager->get_string('cancellationsubject', 'shezar_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessage = $stringmanager->get_string('cancellationalert', 'shezar_feedback360',
                        $stringvars, $userto->lang);
                $event->fullmessagehtml = $stringmanager->get_string('cancellationalert', 'shezar_feedback360',
                        $stringvars, $userto->lang);
            }
    ...
    public static function display_status($status) {
        switch ($status) {
            case self::STATUS_DRAFT:
                $result = get_string('draft', 'shezar_feedback360');
                break;
            case self::STATUS_ACTIVE:
                $result = get_string('active', 'shezar_feedback360');
                break;
            case self::STATUS_CLOSED:
                $result = get_string('closed', 'shezar_feedback360');
                break;
            case self::STATUS_COMPLETED:
                $result = get_string('completed', 'shezar_feedback360');
                break;
        }
    ...
    public static function can_view_feedback360s($userid = null) {
        ...
        if (has_capability('shezar/feedback360:viewownreceivedfeedback360', $systemcontext, $userid)) {
        ...
        if (has_capability('shezar/feedback360:viewownrequestedfeedback360', $systemcontext, $userid)) {
        ...
        if (has_capability('shezar/feedback360:viewstaffreceivedfeedback360', $usercontext, $userid)) {
    ...
    private function create_answers_table() {
        ...
        if ($this->id < 1) {
            throw new feedback360_exception(get_string('error:feedbacktablecreation', 'shezar_feedback360'), 4);
        }
    ...
    public function activate() {
        if (!in_array($this->status, array(self::STATUS_DRAFT, self::STATUS_CLOSED))) {
            throw new feedback360_exception(get_string('error:activationstatus', 'shezar_feedback360'), 5);
        }
    ...
    public function validate() {
        ...
        if (!$is_question) {
            $errors['questions'] = get_string('error:questionsrequired', 'shezar_feedback360');
        }
        ...
        if ($learners == 0) {
            $errors['learners'] = get_string('error:learnersrequired', 'shezar_feedback360');
        }
        // Check recipients.
        if ($this->recipients < 1) {
            $errors['recipients'] = get_string('error:recipientsrequired', 'shezar_feedback360');
        }
    ...
    public static function check_managing_assigned($feedback360id, $userid) {
        ...
        $staff = \shezar_job\job_assignment::get_staff_userids($userid);
    ...
    public static function check_feature_enabled() {
        if (shezar_feature_disabled('feedback360')) {
            print_error('feedback360disabled', 'shezar_feedback360');
        }
    }
    ...
    public static function can_view_other_feedback360s($userid) {
        ...
        if ($USER->id == $userid) {
            ...
            return has_capability('shezar/feedback360:viewstaffreceivedfeedback360', $usercontext) ||
                   has_capability('shezar/feedback360:viewstaffrequestedfeedback360', $usercontext);
...
class feedback360_responder {
    ...
    public static function by_user($userid, $feedback360id, $subjectid) {
        ...
        if (!$data = $DB->get_record_sql($sql, array($userid, $feedback360id, $subjectid))) {
            print_error('error:respassignmentaccess', 'shezar_feedback360');
        }
    ...
    public function save() {
        ...
        if ($this->id > 0) {
            ...
            \shezar_feedback360\event\request_created::create_from_instance($data, $userassign, $email)->trigger();
        }
    }
    ...
    public static function update_and_notify_system($data, $asmanager, $userfrom, $strvars) {
        ...
        if ($data->duenotifications && !empty($systemkeep)) {
            ...
            // Prepare strings.
            $strmanagerupdatesubject = new lang_string('managerupdatesubject', 'shezar_feedback360', $strvars);
            $strmanagerupdatealert = new lang_string('managerupdatealert', 'shezar_feedback360', $strvars);
            $strupdatesubject = new lang_string('updatesubject', 'shezar_feedback360', $strvars);
            $strupdatealert = new lang_string('updatealert', 'shezar_feedback360', $strvars);
    ...
    public static function update_system_assignments($new, $cancel, $userformid, $duedate, $asmanager = false) {
        ...
        foreach ($new as $userid) {
            ...
            if ($asmanager) {
                $eventdata->subject = $stringmanager->get_string('manageremailrequestsubject', 'shezar_feedback360',
                        $taskvars, $userto->lang);
                $eventdata->fullmessage = $stringmanager->get_string('manageremailrequeststr', 'shezar_feedback360',
                        $taskvars, $userto->lang);
            } else {
                $eventdata->subject = $stringmanager->get_string('emailrequestsubject', 'shezar_feedback360',
                        $taskvars, $userto->lang);
                $eventdata->fullmessage = $stringmanager->get_string('emailrequeststr', 'shezar_feedback360',
                        $taskvars, $userto->lang);
            }
            tm_task_send($eventdata);

            \shezar_feedback360\event\request_created::create_from_instance($resp_assignment, $user_assignment->userid)->trigger();
        }
    ...
    public static function update_and_notify_email($data, $asmanager, $userfrom, $strvars) {
        ...
        if ($data->duenotifications && !empty($emailkeep)) {
            ...
            if ($asmanager) {
                $emailsubject = get_string('managerupdatesubject', 'shezar_feedback360', $strvars);
                $email_str = get_string('managerupdateemail', 'shezar_feedback360', $strvars);
                $sendas = $USER;
            } else {
                $emailsubject = get_string('updatesubject', 'shezar_feedback360', $strvars);
                $email_str = get_string('updateemail', 'shezar_feedback360', $strvars);
                $sendas = $userfrom;
            }
    ...
    public static function update_external_assignments($newassignments, $cancellations, $userformid, $duedate, $asmanager = false) {
        ...
        foreach ($newassignments as $email) {
            ...
            if ($asmanager) {
                $emailplain = get_string('manageremailrequeststr', 'shezar_feedback360', $emailvars);
                $emailhtml = get_string('manageremailrequesthtml', 'shezar_feedback360', $emailvars);
                $emailsubject = get_string('manageremailrequestsubject', 'shezar_feedback360', $emailvars);
            } else {
                $emailplain = get_string('emailrequeststr', 'shezar_feedback360', $emailvars);
                $emailhtml = get_string('emailrequesthtml', 'shezar_feedback360', $emailvars);
                $emailsubject = get_string('emailrequestsubject', 'shezar_feedback360', $emailvars);
            }
            ...
            \shezar_feedback360\event\request_created::create_from_instance($resp_assignment, $user_assignment->userid, $email)->trigger();
        }
...
class feedback360_event_handler {
     ...
     *
     * @param \shezar_appraisal\event\appraisal_user_deleted $event
     */
 ...
 */
function shezar_feedback360_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $USER;
    ...
    $fs = get_file_storage();
    if (!$file = $fs->get_file($context->id, 'shezar_feedback360', $filearea, $assignmentid, '/', $filename)) {
        send_file_not_found();
    }
...
class request_feedback_potential_user_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        if ($search) {
            $group = get_string('potentialusersmatching', 'shezar_feedback360', $search);
        } else {
            $group = get_string('potentialusers', 'shezar_feedback360');
        }
...
class request_feedback_current_user_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        if ($search) {
            $group = get_string('currentusersmatching', 'shezar_feedback360', $search);
        } else {
            $group = get_string('currentusers', 'shezar_feedback360');
        }
