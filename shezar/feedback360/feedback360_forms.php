...
require_once($CFG->dirroot . '/shezar/question/libforms.php');
...
class feedback360_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($readonly) {
            $mform->addElement('static', 'name', get_string('name', 'shezar_feedback360'));
            $mform->addElement('static', 'description_ro', get_string('description'), $feedback360->description_editor['text']);

            $mform->addElement('static', 'anonymous_ro', get_string('anonymousfeedback', 'shezar_feedback360'),
                $feedback360->anonymous ? get_string('yes') : get_string('no'));
        } else {
            $mform->addElement('text', 'name', get_string('name', 'shezar_feedback360'), 'maxlength="255" size="50"');
            $mform->addRule('name', null, 'required');
            $mform->addHelpButton('name', 'name', 'shezar_feedback360');

            $mform->addElement('editor', 'description_editor', get_string('description'), null, $TEXTAREA_OPTIONS);
            $mform->addHelpButton('description_editor', 'description', 'shezar_feedback360');
            $mform->setType('description_editor', PARAM_CLEANHTML);

            $mform->addElement('advcheckbox', 'anonymous', get_string('anonymousfeedback', 'shezar_feedback360'));
            $mform->addHelpButton('anonymous', 'anonymousfeedback', 'shezar_feedback360');

            $submittitle = get_string('createfeedback360', 'shezar_feedback360');
            if ($feedback360->id > 0) {
...
class feedback360_recipients_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('advcheckbox', 'email', '', get_string('recipient:email', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'anyuser', '', get_string('recipient:anyuser', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'linemanager', '', get_string('recipient:linemanager', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'directreport', '', get_string('recipient:directreports', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'audiences', '', get_string('recipient:audiencies', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'samepos', '', get_string('recipient:samepos', 'shezar_feedback360'));
        $mform->addElement('advcheckbox', 'sameorg', '', get_string('recipient:sameorg', 'shezar_feedback360'));
...
class feedback360_answer_form extends moodleform {
    ...
    public function definition_after_data() {
        ...
        if (!$readonly && !$preview) {
            ...
            $this->add_action_buttons(false, get_string('submitfeedback', 'shezar_feedback360'));
    ...
    public function validation($data, $files) {
        ...
        if ($preview || $readonly) {
            return array('submitbutton' => get_string('error:readonly', 'shezar_feedback360'));
        }
...
class feedback360_quest_edit_form extends question_base_form {
    public function definition() {
        ...
        if ($question->get_element()->is_answerable()) {
            $mform->addElement('advcheckbox', 'required', '', get_string('required', 'shezar_question'));
        }

        if ($readonly) {
            $mform->freeze();
            $mform->addElement('static', 'backlink', '', html_writer::link(new moodle_url('/shezar/feedback360/content.php',
...
class request_select_form extends moodleform {
    ...
    public function definition() {
        ...
        // Header - select form.
        $mform->addElement('header', 'newrequestform', get_string('newrequest', 'shezar_feedback360'));
    ...
    public function set_data($data) {
        ...
        $popupurl = $CFG->wwwroot . "/shezar/feedback360/feedback.php?userid={$data['userid']}&preview=1&feedback360id=";
    ...
    public function definition_after_data() {
        ...
        foreach ($available_forms as $form) {
            ...
            $preview_link = html_writer::link($preview_url, get_string('previewencased', 'shezar_feedback360'),
        ...
        if (!empty($requestforms)) {
            $mform->addGroup($requestforms, 'formselector',
                    get_string('feedback360selectform', 'shezar_feedback360'), array(' '), false);
            $mform->addHelpButton('formselector', 'feedback360selectform', 'shezar_feedback360');

            $this->add_action_buttons(true, get_string('next', 'shezar_feedback360'));
        } else {
            $mform->addElement('static', 'noavailableforms', null, get_string('noavailableforms', 'shezar_feedback360'));
    ...
    public function validation($data, $files) {
        ...
        if (empty($data['formid'])) {
            $errors['formid'] = get_string('error:noformselected', 'shezar_feedback360');
        }
...
class request_select_users extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'manageuserrequests', get_string('manageuserrequests', 'shezar_feedback360'));
        ...
        $popupurl = $CFG->wwwroot . "/shezar/feedback360/feedback.php?userid={$USER->id}&preview=1&feedback360id=";
        ...
        $systemuserstr = html_writer::tag('strong', get_string('requestuserssystem', 'shezar_feedback360'));
        ...
        $mform->addElement('submit', 'addsystemusers', get_string('addsystemusers', 'shezar_feedback360'),
                array('id' => 'show-systemrequest-dialog'));

        // Text area to add new emails.
        $mform->addElement('textarea', 'emailnew', get_string('emailrequestsnew', 'shezar_feedback360'));
        $mform->addHelpButton('emailnew', 'emailrequestsnew', 'shezar_feedback360');

        // Create a place to show existing external users.
        $mform->addElement('static', 'existing_external', '', '');

        // Target date.
        $mform->addElement('date_selector', 'duedate', get_string('duedate', 'shezar_feedback360'), array('optional' => true));
        $mform->addHelpButton('duedate', 'duedate', 'shezar_feedback360');
    ...
    public function set_data($data) {
        ...
        if (!empty($data['formid'])) {
            ...
            print_error('error:noformselected', 'shezar_feedback360');
        }

        if ($data['anonymous']) {
            $notice = html_writer::start_tag('div', array('class' => 'notifynotice'));
            $notice .= get_string('anonrequestform', 'shezar_feedback360');
        ...
        if (!empty($data['feedbackid']) && !empty($data['feedbackname'])) {
            $title = get_string('manageuserrequests', 'shezar_feedback360');
        ...
        if (!empty($data['update'])) {
            $submitstr = get_string('update', 'shezar_feedback360');
        } else {
            $submitstr = get_string('request', 'shezar_feedback360');
        }
    ...
    public function validation($data, $files) {
        ...
        if ($count_emails + $count_system < 1) {
            $errors['systemnew'] = get_string('error:emptyuserrequests', 'shezar_feedback360');
        }
        ...
        if (!empty($emails)) {
            ...
            if (!empty($formaterror)) {
                $errors['emailnew'] = get_string('error:emailformat', 'shezar_feedback360') . implode($formaterror, "- ");
            }

            if (!empty($duplicateerror)) {
                $errors['emailnew'] = get_string('error:emailduplicate', 'shezar_feedback360') . implode($duplicateerror, "- ");
            }
        }
        ...
        if (!empty($data['duedate'])) {
            ...
            if (empty($data['duedate'])) {
                ...
                if ($data['duedate'] < time()) {
                    $errors['duedate'] = get_string('error:duedatepast', 'shezar_feedback360');
                }
                ...
                if (!empty($data['oldduedate'])) {
                    ...
                    if ($olddue > $data['duedate']) {
                        $errors['duedate'] = get_string('error:newduedatebeforeold', 'shezar_feedback360');
                    }
                }
            }
        }
...
class request_confirmation extends moodleform {
    public function definition() {
        ...
        $mform->addElement('header', 'requestfeedback360', get_string('requestfeedback360', 'shezar_feedback360'));

        // Set up the confirmation string.
        $strconfirm = get_string('requestfeedback360confirm', 'shezar_feedback360');
    ...
    public function set_data($data) {
        ...
        if (!empty($newsystem) || !empty($newexternal)) {
            ...
            $newrequests .= get_string('requestfeedback360create', 'shezar_feedback360');
        ...
        if (!empty($cancelsystem) || !empty($cancelexternal)) {
            ...
            $cancelledrequests .= get_string('requestfeedback360delete', 'shezar_feedback360');
        ...
        if ($duenotifications) {
            if (!empty($keepsystem) || !empty($keepexternal)) {
                ...
                $keeprequests .= get_string('requestfeedback360keep', 'shezar_feedback360');
