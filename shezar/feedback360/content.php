...
require_capability('shezar/feedback360:managepageelements', $systemcontext);
...
$returnurl = new moodle_url('/shezar/feedback360/content.php', array('feedback360id' => $feedback360->id));
...
switch ($action) {
    ...
    case 'edit':
        if ($mnewform->is_submitted()) {
            ...
            if (!$newelement) {
                shezar_set_notification(get_string('error:choosedatatype', 'shezar_question'), $returnurl,
        ...
        if ($isdraft && $fromform = $meditform->get_data()) {
            ...
            if (is_ajax_request($_SERVER)) {
                ...
                shezar_set_notification(get_string('contentupdated', 'shezar_feedback360'), $returnurl,
                        array('class' => 'notifysuccess'));
            }
        } else {
    ...
    default:
        ...
        if ($isdraft) {
            $jsmodule = array(
                'name' => 'shezar_feedback360_content',
                'fullpath' => '/shezar/feedback360/js/content.js',
                'requires' => array('json'));

            $PAGE->requires->js_init_call('M.shezar_feedback360_content.init', array(), false, $jsmodule);
        }
}
