...
/**
 * Output renderer for shezar_feedback360s module
 */
class shezar_feedback360_renderer extends plugin_renderer_base {
    ...
    public function create_feedback360_button() {
        return $this->output->single_button(new moodle_url('/shezar/feedback360/general.php'),
            get_string('createfeedback360', 'shezar_feedback360'), 'get');
    }

    public function preview_feedback360_button($feedback360id) {
        $preview_params = array('feedback360id' => $feedback360id, 'preview' => 1);
        $preview_url = new moodle_url('/shezar/feedback360/feedback.php', $preview_params);
        $preview_str = get_string('preview', 'shezar_feedback360');
    ...
    public function request_feedback360_button($userid) {
        ...
        $request_url = new moodle_url('/shezar/feedback360/request.php', $request_params);
        $request_str = get_string('requestfeedback360', 'shezar_feedback360');
    ...
    public function feedback360_manage_table($feedback360s = array(), $userid = null) {
        ...
        if (empty($feedback360s)) {
            return get_string('nofeedback360s', 'shezar_feedback360');
        }

        $tableheader = array(get_string('name', 'shezar_feedback360'),
                             get_string('assignments', 'shezar_feedback360'),
                             get_string('anonymous', 'shezar_feedback360'),
                             get_string('status', 'shezar_feedback360'),
                             get_string('options', 'shezar_feedback360'));
        ...
        $stractivate = get_string('activate', 'shezar_feedback360');
        $strclose = get_string('close', 'shezar_feedback360');
        $strsettings = get_string('settings', 'shezar_feedback360');
        $strdelete = get_string('delete', 'shezar_feedback360');
        ...
        foreach ($feedback360s as $feedback360) {
            ...
            $activateurl = new moodle_url('/shezar/feedback360/activation.php',
                    array('id' => $feedback360->id, 'action' => 'activate'));
            $closeurl = new moodle_url('/shezar/feedback360/activation.php',
                    array('id' => $feedback360->id, 'action' => 'close'));
            $editurl = new moodle_url('/shezar/feedback360/general.php',
                    array('id' => $feedback360->id));
            $deleteurl = new moodle_url('/shezar/feedback360/manage.php',
                    array('id' => $feedback360->id, 'action' => 'delete'));
            $cloneurl = new moodle_url('/shezar/feedback360/manage.php',
                    array('id' => $feedback360->id, 'action' => 'copy'));

            $row = array();
            if (has_capability('shezar/feedback360:managefeedback360', $systemcontext, $userid)) {
                $row[] = html_writer::link($editurl, $name);
            } else {
                $row[] = $name;
            }

            $assign = new shezar_assign_feedback360('feedback360', new feedback360($feedback360->id));
            $countassignments = $assign->get_current_users_count();
            if ($feedback360->status == feedback360::STATUS_DRAFT) {
                $row[] = get_string('assignedtoxdraftusers', 'shezar_feedback360', $countassignments);
            } else {
                $row[] = get_string('assignedtoxusers', 'shezar_feedback360', $countassignments);
            }
            ...
            if (has_capability('shezar/feedback360:managefeedback360', $systemcontext, $userid)) {
                ...
                if ($feedback360->status == feedback360::STATUS_ACTIVE) {
                    $edit_error = get_string('error:feedback360noteditable', 'shezar_feedback360');
                    $edit = $this->output->pix_icon('/t/edit_gray', $edit_error, 'moodle', array('class' => 'disabled iconsmall'));

                    $delete_error = get_string('error:feedback360isactive', 'shezar_feedback360');
            ...
            $activate = '';
            if (has_capability('shezar/feedback360:manageactivation', $systemcontext, $userid)) {
                if ($feedback360->status == feedback360::STATUS_ACTIVE) {
    ...
    public function feedback360_management_tabs($feedback360id, $currenttab = 'general') {
        ...
        $systemcontext = context_system::instance();
        if (has_capability('shezar/feedback360:managefeedback360', $systemcontext)) {
            $row[] = new tabobject('general', $CFG->wwwroot . '/shezar/feedback360/general.php?id='
                    . $feedback360id, get_string('general'));
        }
        if (has_capability('shezar/feedback360:managepageelements', $systemcontext)) {
            $row[] = new tabobject('content', $CFG->wwwroot . '/shezar/feedback360/content.php?feedback360id='
                    . $feedback360id, get_string('content', 'shezar_feedback360'));
        }
        $capabilities = array('shezar/feedback360:viewassignedusers', 'shezar/feedback360:assignfeedback360togroup');
        if (has_any_capability($capabilities, $systemcontext)) {
            $row[] = new tabobject('assignments', $CFG->wwwroot . '/shezar/feedback360/assignments.php?id='
                    . $feedback360id, get_string('assignments', 'shezar_feedback360'));
        }
    ...
    public function display_assigned_groups($assignments, $itemid) {
        $tableheader = array(get_string('assigngrouptype', 'shezar_feedback360'),
                             get_string('assignsourcename', 'shezar_feedback360'),
                             get_string('assignincludechildren', 'shezar_feedback360'),
                             get_string('assignnumusers', 'shezar_feedback360'),
                             get_string('actions'));
        ...
        if (empty($assignments)) {
            $table->data[] = array(get_string('nogroupassignments', 'shezar_feedback360'));
        } else {
            foreach ($assignments as $assign) {
                ...
                if ($feedback360->status == feedback360::STATUS_DRAFT) {
                    $delete = $this->output->action_icon(
                            new moodle_url('/shezar/feedback360/assignments.php',
    ...
    public function display_feedback_header(feedback360_responder $resp, $subjectuser, $anonymous, $numresponders) {
        ...
        $message = html_writer::tag('p', get_string($titlestr, 'shezar_feedback360', $a));
        if ($anonmessage) {
            if ($anonymous) {
                $anonmessage = get_string('feedbackanonymous', 'shezar_feedback360', $numresponders);
            } else {
                $anonmessage = get_string('feedbacknotanonymous', 'shezar_feedback360');
            }
        ...
        if (!$resp->is_completed() && !$resp->is_fake()) {
            $savebutton = new single_button(new moodle_url('#'), get_string('saveprogress', 'shezar_feedback360'));
    ...
    public function display_preview_feedback_header(feedback360_responder $resp, $feedbackname) {
        $headerstr = get_string('previewheader', 'shezar_feedback360', $feedbackname);
        $subheader = get_string('previewsubheader', 'shezar_feedback360');
    ...
    public function display_userview_header($user) {
        ...
        if ($USER->id != $user->id) {
            ...
            $viewstr = html_writer::tag('strong', get_string('viewinguserxfeedback360', 'shezar_feedback360', $link));
    ...
    public function display_user_datatable() {
        ...
        $table->head = array(get_string('learner'), get_string('assignedvia', 'shezar_core'));
    ...
    public function feedback360_additional_actions($status, $id) {
        $activateurl = new moodle_url('/shezar/feedback360/activation.php', array('id' => $id, 'action' => 'activate'));
        $closeurl = new moodle_url('/shezar/feedback360/activation.php', array('id' => $id, 'action' => 'close'));

        $strstatusnow = feedback360::display_status($status);
        $strstatusat = get_string('statusat', 'shezar_feedback360');
        $feedback360 = new feedback360($id);

        $preview = $this->preview_feedback360_button($id);
        $activate = '';
        if ($feedback360->status == feedback360::STATUS_ACTIVE) {
            $activate = $this->output->action_link($closeurl, get_string('closenow', 'shezar_feedback360'));
        } else if ($feedback360->status == feedback360::STATUS_DRAFT || $feedback360->status == feedback360::STATUS_CLOSED) {
            $activate = $this->output->action_link($activateurl,  get_string('activatenow', 'shezar_feedback360'));
        }
    ...
    public function confirm_delete_feedback360 (feedback360 $feedback360) {
        $msg = get_string('deletefeedback360s', 'shezar_feedback360', $feedback360->name);

        if ($feedback360->status != feedback360::STATUS_DRAFT) {
            $questions = $feedback360->fetch_questions();
            if (count($questions) > 0) {
                $msg .= html_writer::empty_tag('br');
                $msg .= get_string('deletefeedback360questions', 'shezar_feedback360');
            }
            $assign = new shezar_assign_feedback360('feedback360', $feedback360);
            $assignments = $assign->get_current_users_count();
            if ($assignments > 0) {
                $msg .= html_writer::empty_tag('br');
                $msg .= get_string('deletefeedback360assignments', 'shezar_feedback360');
            }
        }
        ...
        $continue = new moodle_url('/shezar/feedback360/manage.php', $params);
        $cancel = new moodle_url('/shezar/feedback360/manage.php');
    ...
    public function confirm_question_delete(feedback360_question $question) {
        $msg = get_string('confirmdeletequestion', 'shezar_feedback360', $question->name);
        ...
        $continue = new moodle_url('/shezar/feedback360/content.php', $params);
        $cancel = new moodle_url('/shezar/feedback360/content.php', array('feedback360id' => $question->feedback360id));
    ...
    public function confirm_activation_feedback360 ($feedback360, $errors) {
        ...
        if (!empty($errors)) {
            $out = $this->heading(get_string('error:activationconfirmation', 'shezar_feedback360'));
            $out .= html_writer::tag('p', get_string('feedback360fixerrors', 'shezar_feedback360'));
            $errordesc = array();
            ...
            $buttons = array();
            $buttons[] = $this->output->single_button(new moodle_url('/shezar/feedback360/content.php',
                    array('feedback360id' => $feedback360->id)), get_string('backtofeedback360', 'shezar_feedback360',
                            $feedback360->name), 'get');
            $out .= html_writer::tag('div', implode(' ', $buttons), array('class' => 'buttons'));
            return $out;
        } else {
            $msg = get_string('confirmactivatefeedback360', 'shezar_feedback360', $feedback360->name);
            ...
            $continueurl = new moodle_url('/shezar/feedback360/activation.php', $params);
            $cancelurl = new moodle_url('/shezar/feedback360/manage.php');
    ...
    public function confirm_close_feedback360 ($feedback360) {
        $msg = get_string('confirmclosefeedback360', 'shezar_feedback360', $feedback360->name);
        $params = array('id' => $feedback360->id,
                            'action' => 'close',
                            'confirm' => 1,
                            'sesskey' => sesskey()
    ...
    public function list_questions($quests) {
        ...
        if ($quests) {
            ...
            foreach ($quests as $quest) {
                ...
                if (!$first) {
                    $posupurl = new moodle_url('/shezar/feedback360/content.php', array('action' => 'posup',
                ...
                if ($quest->id != $last->id) {
                    $posdownurl = new moodle_url('/shezar/feedback360/content.php', array('action' => 'posdown',
                            'id' => $quest->id, 'feedback360id' => $feedback360->id, 'sesskey' => sesskey()));
                ...
                $editurl = new moodle_url('/shezar/feedback360/content.php', array('action' => 'edit',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));
                $cloneurl = new moodle_url('/shezar/feedback360/content.php', array('action' => 'clone',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));
                $deleteurl = new moodle_url('/shezar/feedback360/content.php', array('action' => 'delete',
                    'id' => $quest->id, 'feedback360id' => $feedback360->id));;
    ...
    public function myfeedback_user_table($userid, $user_assignments, $canmanage) {
        ...
        $user_table->head = array(
            get_string('name', 'shezar_feedback360'),
            get_string('responses', 'shezar_feedback360'),
            get_string('duedate', 'shezar_feedback360'),
            get_string('anonymous', 'shezar_feedback360')
        );
        if ($canmanage) {
            $user_table->head[] = get_string('options', 'shezar_feedback360');
        }

        $stryes = new lang_string('yes');
        $strno = new lang_string('no');
        $stranoncancel = new lang_string('anoncancel', 'shezar_feedback360');
        $strstop = new lang_string('stop', 'shezar_feedback360');
        ...
        foreach ($user_assignments as $user_assignment) {
            ...
            if (!empty($requests)) {
                ...
                $nameurl = new moodle_url('/shezar/feedback360/request/view.php',
                ...
                if ($canmanage) {
                    ...
                    $editurl = new moodle_url('/shezar/feedback360/request.php', $editparams);
                    ...
                    $remindurl = new moodle_url('/shezar/feedback360/request/remind.php', $remindparams);
                    $remindstr = get_string('remind', 'shezar_feedback360');
                    ...
                    if (!empty($user_assignment->anonymous)) {
                        ...
                        $cancelurl = new moodle_url('/shezar/feedback360/request/stop.php', $cancelparams);
                    ...
                    if ($user_assignment->status == feedback360::STATUS_ACTIVE) {
                        ...
                        $options = get_string('closed', 'shezar_feedback360');
                    }
                }

                $respond = get_string('responsecount', 'shezar_feedback360', $res);
                if (!empty($newresponses)) {
                    $respond .= html_writer::tag('strong', get_string('responsecountnew', 'shezar_feedback360', $res));
                }
        ...
        if ($nodata) {
            $cell = new html_table_cell(get_string('nofeedback360requested', 'shezar_feedback360'));
    ...
    public function myfeedback_colleagues_table($userid, $resp_assignments) {
        ...
        if (empty($resp_assignments)) {
            $cell = new html_table_cell(get_string('nofeedback360togive', 'shezar_feedback360'));
            ...
            foreach ($resp_assignments as $resp_assignment) {
                ...
                $answerurl = new moodle_url('/shezar/feedback360/feedback.php', $answerparams);
                if (!empty($completed)) {
                    // Complete.
                    $status = get_string('completed', 'shezar_feedback360');

                    $revstr = ($USER->id == $userid) ? 'reviewnow' : 'reviewnowmanager';
                    $options = $this->output->action_link($answerurl, get_string($revstr, 'shezar_feedback360'));
                } else {
                    if (!$hidebutton) {
                        $options = $this->output->single_button($answerurl, get_string('answernow', 'shezar_feedback360'), 'get');
                    } else {
                        $options = '';
                    }
                    if (empty($resp_assignment->timedue)) {
                        ...
                        // Overdue.
                        $status = get_string('overdue', 'shezar_feedback360');
                    } else {
                        // Pending.
                        $status = get_string('pending', 'shezar_feedback360');
                    }
                }
    ...
    public function view_request_infotable($user_assignment, $resp_assignments, $anonymous) {
        ...
        $request_infotable->head = array(
            get_string('nameemail', 'shezar_feedback360'),
            get_string('completed', 'shezar_feedback360'),
            get_string('response', 'shezar_feedback360'),
        );
        ...
        foreach ($resp_assignments as $resp_assignment) {
            if (!empty($resp_assignment->timecompleted)) {
                ...
                if (empty($resp_assignment->viewed)) {
                    ...
                    $view_str = html_writer::tag('strong', get_string('viewresponse', 'shezar_feedback360'));
                } else {
                    $view_str = get_string('viewresponse', 'shezar_feedback360');
                }

                $responseparam = array('myfeedback' => 1, 'responseid' => $resp_assignment->id);
                $responseurl = new moodle_url('/shezar/feedback360/feedback.php', $responseparam);
                $responselink = html_writer::link($responseurl, $view_str);
            } else {
                $comp_str = get_string('notcompleted', 'shezar_feedback360');
                $responselink = '';
            }

            if ($anonymous) {
                $name_str = html_writer::tag('em', get_string('anonymoususer', 'shezar_feedback360'));
                ...
                if (empty($resp_assignment->email)) {
                    $name_str = get_string('emailmissing', 'shezar_feedback360');
                } else {
    ...
    public function system_user_record($user, $userform, $resp, $anonymous) {
        ...
        $completestr = get_string('alreadyreplied', 'shezar_feedback360');
        ...
        if (!$anonymous || empty($resp)) {
            if (!empty($resp->timecompleted)) {
                ...
                $removestr = get_string('removeuserfromrequest', 'shezar_feedback360', $username);
    ...
    public function external_user_record($email, $userform, $resp, $anonymous) {
        ...
        $deleteurl = new moodle_url('/shezar/feedback360/request/delete.php', $deleteparams);
        ...
        if (!$anonymous) {
            if (!empty($resp->timecompleted)) {
                ...
                $removestr = get_string('removeuserfromrequest', 'shezar_feedback360', $email);
    ...
    public function nojs_feedback_request_users($selected, $returnurl, $add_user_selector, $remove_user_selector, $anonymous = false) {
        ...
        $out .= html_writer::tag('label',
                get_string('currentrequestees', 'shezar_feedback360'),
        ...
        $out .= html_writer::start_tag('div', array('class' => 'span5'));
        $out .= html_writer::tag('label',
                get_string('potentialrequestees', 'shezar_feedback360'),
                array('for' => 'addselect'));
        ...
        $out .= html_writer::empty_tag('input',
                array('type' => 'submit', 'name' => 'cancel',
                    'value' => get_string('backtofeedbackrequest', 'shezar_feedback360')));
