...
if (shezar_feature_disabled('reportgraphs')) {
    print_error('error:reportgraphsdisabled', 'shezar_reportbuilder');
}

// jQuery is loaded on each page since 8.0.
$PAGE->requires->yui_module('moodle-shezar_reportbuilder-graphicalreporting', 'M.reportbuilder.graphicalreport.init');
...
$returnurl = new moodle_url('/shezar/reportbuilder/graph.php', array('reportid' => $id));
...
if ($fromform = $mform->get_data()) {
    ...
    shezar_set_notification(get_string('graph_updated', 'shezar_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
}
