...
$returnurl = $CFG->wwwroot . '/shezar/reportbuilder/groupsettings.php';
...
echo $output->single_button($CFG->wwwroot . '/shezar/reportbuilder/groups.php', get_string('backtoallgroups', 'shezar_reportbuilder'), 'get');

echo $output->heading(get_string('activitygroupingx', 'shezar_reportbuilder', $group->name));

echo $output->heading(get_string('assignedactivities', 'shezar_reportbuilder'), 3);

$info = new stdClass();
$info->count = count($activities);
$info->tag = $tag->name;
echo html_writer::tag('p', get_string('groupcontents', 'shezar_reportbuilder', $info));

if (count($activities)) {
    echo $output->activity_group_activities_table($activities);
}

echo $output->heading(get_string('baseactivity', 'shezar_reportbuilder'), 3);

$info = new stdClass();
$info->url = $CFG->wwwroot . '/mod/feedback/view.php?id=' . $baseitem->cmid;
$info->activity = $baseitem->feedback;
echo html_writer::tag('p', get_string('baseitemdesc', 'shezar_reportbuilder', $info));

echo $output->heading(get_string('reports', 'shezar_reportbuilder'), 3);
