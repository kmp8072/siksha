...
require_once($CFG->dirroot.'/shezar/reportbuilder/dialogs/dialog_content_cachenow.class.php');
...
function forkreportcache($reportid) {
    ...
    $message = get_string('cachegenstarted', 'shezar_reportbuilder', userdate(time()));
    ob_start();
    switch($child) {
        case -1:
...
function cachenow_showresult($status, $message) {
    ...
    if (!$display && $message != '') {
        $display = true;
        $dialog = new shezar_dialog_content_cachenow();
...
if ($report->cache) {
    ...
    $message = get_string('cachegenstarted', 'shezar_reportbuilder', userdate($start));
}
