...
$returnurl = new moodle_url('/shezar/reportbuilder/filters.php', array('id' => $id));
...
foreach ($report->src->filteroptions as $option) {
    ...
    if (isset($option->filteroptions['addtypetoheading']) && $option->filteroptions['addtypetoheading']) {
        ...
        $heading = get_string ('headingformat', 'shezar_reportbuilder', $text);
    } else {
...
$PAGE->requires->strings_for_js(array('saving', 'confirmfilterdelete', 'confirmsearchcolumndelete', 'delete', 'moveup',
    'movedown', 'add', 'initialdisplay_error'), 'shezar_reportbuilder');
...
$jsmodule = array(
    'name' => 'shezar_reportbuilderfilters',
    'fullpath' => '/shezar/reportbuilder/filters.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.shezar_reportbuilderfilters.init', $args, false, $jsmodule);


// Delete fields or columns.
if ($d and $confirm) {
    if (!confirm_sesskey()) {
        shezar_set_notification(get_string('error:bad_sesskey', 'shezar_reportbuilder'), $returnurl);
    }
    if (isset($fid)) {
        if ($report->initialdisplay && count($report->filters) <= 1) {
                shezar_set_notification(get_string('initialdisplay_error', 'shezar_reportbuilder'), $returnurl);
        } else {
            if ($report->delete_filter($fid)) {
                \shezar_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
                shezar_set_notification(get_string('filterdeleted', 'shezar_reportbuilder'), $returnurl,
                    array('class' => 'notifysuccess'));
            } else {
                shezar_set_notification(get_string('error:filter_not_deleted', 'shezar_reportbuilder'), $returnurl);
            }
        }
        ...
        if ($report->initialdisplay && count($report->filters) <= 1) {
            ...
            if ($report->delete_search_column($searchcolumnid)) {
                \shezar_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
                shezar_set_notification(get_string('searchcolumndeleted', 'shezar_reportbuilder'), $returnurl,
                    array('class' => 'notifysuccess'));
            } else {
                shezar_set_notification(get_string('error:search_column_not_deleted', 'shezar_reportbuilder'), $returnurl);
            }
        }
    }
}
...
if ($d) {
    ...
    if (isset($fid)) {
        $confirmurl = new moodle_url('/shezar/reportbuilder/filters.php',
            array('d' => '1', 'id' => $id, 'fid' => $fid, 'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('confirmfilterdelete', 'shezar_reportbuilder'), $confirmurl, $returnurl);
    } else if (isset($searchcolumnid)) {
        $confirmurl = new moodle_url('/shezar/reportbuilder/filters.php',
            array('d' => '1', 'id' => $id, 'searchcolumnid' => $searchcolumnid, 'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('confirmsearchcolumndelete', 'shezar_reportbuilder'), $confirmurl, $returnurl);
    }
...
if ($m && isset($fid)) {
    if ($report->move_filter($fid, $m)) {
        \shezar_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        shezar_set_notification(get_string('filtermoved', 'shezar_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        shezar_set_notification(get_string('error:filter_not_moved', 'shezar_reportbuilder'), $returnurl);
    }
}
...
if ($mform->is_cancelled()) {
    redirect(new moodle_url('/shezar/reportbuilder/index.php'));
}
if ($fromform = $mform->get_data()) {
    ...
    if (build_filters($id, $fromform)) {
        ...
        \shezar_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        shezar_set_notification(get_string('filters_updated', 'shezar_reportbuilder'), $returnurl,
            array('class' => 'notifysuccess'));
    } else {
        shezar_set_notification(get_string('error:filters_not_updated', 'shezar_reportbuilder'), $returnurl);
    }
