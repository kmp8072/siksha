...
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_base_source.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_base_content.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_base_preproc.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_base_embedded.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_join.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_column.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_column_option.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_filter_option.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_param.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_param_option.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_content_option.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_global_restriction.php');
require_once($CFG->dirroot . '/shezar/reportbuilder/classes/rb_global_restriction_set.php');
...
class reportbuilder {
    ...
    public function __construct($id=null, $shortname=null, $embed_deprecated=false, $sid=null, $reportfor=null,
        ...
        if ($id != null) {
            ...
            print_error('noshortnameorid', 'shezar_reportbuilder');
        }
        ...
        if (!$report) {
            ...
            if ($embed) {
                ...
                if (! $id = reportbuilder_create_embedded_record($shortname, $embed, $error)) {
                    print_error('error:creatingembeddedrecord', 'shezar_reportbuilder', '', $error);
                }
        ...
        if (!$report) {
            print_error('reportwithidnotfound', 'shezar_reportbuilder', '', $id);
        }
    ...
    function include_js() {
        ...
        if (!$graph) {
            ...
            $jsdetails->initcall = 'M.shezar_reportbuilder_showhide.init';
            $jsdetails->jsmodule = array('name' => 'shezar_reportbuilder_showhide',
                'fullpath' => '/shezar/reportbuilder/showhide.js');
            $jsdetails->args = array('hiddencols' => $this->js_get_hidden_columns());
            $jsdetails->strings = array(
                'shezar_reportbuilder' => array('showhidecolumns'),
                'moodle' => array('ok')
            );
            $js[] = $jsdetails;
            ...
            // Add saved search.js.
            $jsdetails = new stdClass();
            $jsdetails->initcall = 'M.shezar_reportbuilder_savedsearches.init';
            $jsdetails->jsmodule = array('name' => 'shezar_reportbuilder_savedsearches',
                'fullpath' => '/shezar/reportbuilder/saved_searches.js');
            $jsdetails->strings = array(
                'shezar_reportbuilder' => array('managesavedsearches'),
                'form' => array('close')
            );
            $js[] = $jsdetails;
        }
        ...
        foreach ($this->filters as $filter) {
            ...
            $filterpath = $CFG->dirroot.'/shezar/reportbuilder/filters/'.$filtertype.'.php';
    ...
    public static function find_source_dirs() {
        ...
        $cache = cache::make_from_params(cache_store::MODE_APPLICATION, 'shezar_reportbuilder', 'rb_sources');
        ...
        if ($saved = $DB->get_record('report_builder_saved', array('id' => $this->_sid))) {
            ...
            if ($saved->ispublic != 0 || $saved->userid == $this->reportfor) {
                ...
                if (defined('FULLME') and FULLME === 'cron') {
                    ...
                    print_error('savedsearchnotfoundornotpublic', 'shezar_reportbuilder');
                }
                return false;
            }
        } else {
    ...
    function fetch_sql_filters($extrasql='', $extraparams=array()) {
        ...
        if ($extrasql != '') {
            if (strpos($extrasql, '?')) {
                print_error('extrasqlshouldusenamedparams', 'shezar_reportbuilder');
            }
            $where_sqls[] = $extrasql;
        }
    ...
    private function get_toolbar_sql_filter($toolbarsearchtext) {
        ...
        $keywords = shezar_search_parse_keywords($toolbarsearchtext);
        $columns = $this->get_search_columns();
        ...
        return shezar_search_get_keyword_where_clause($keywords, $dbfields, SQL_PARAMS_NAMED);
    }
    ...
    private function get_toolbar_text_filter($toolbarsearchtext) {
        ...
        if ($numberoffields == 0) {
            ...
            return get_string('toolbarsearchtextiscontainedinsingle', 'shezar_reportbuilder', $a);

        } else {
            $result = get_string('toolbarsearchtextiscontainedinmultiple', 'shezar_reportbuilder', $toolbarsearchtext);
            $columnnames = array();
            foreach ($columns as $column) {
                $columnobject = self::get_single_item($this->columnoptions, $column->type, $column->value);
    ...
    public function format_column_heading(rb_column $column, $plaintext) {
        if ($column->customheading) {
            ...
            if ($column->grouping === 'none') {
                if ($column->transform) {
                    $heading = get_string("transformtype{$column->transform}_heading", 'shezar_reportbuilder', $heading);
                } else if ($column->aggregate) {
                    $heading = get_string("aggregatetype{$column->aggregate}_heading", 'shezar_reportbuilder', $heading);
                }
            }
        }
    ...
    function get_default_headings_array() {
        ...
        foreach ($this->columnoptions as $option) {
            ...
            if (isset($option->addtypetoheading) && $option->addtypetoheading) {
                $type = get_string ('type_' . $option->type, 'shezar_reportbuilder');
                $text = (object) array ('column' => $defaultheading, 'type' => $type);
                $defaultheading = get_string ('headingformat', 'shezar_reportbuilder', $text);
            }
    ...
    function report_url($params = false) {
        ...
        if ($this->embeddedurl === null) {
            $url = new moodle_url($CFG->wwwroot . '/shezar/reportbuilder/report.php', array('id' => $this->_id));
        } else {
    ...
    public function display_restrictions() {
        ...
        $jsmodule = array('name' => 'shezar_email_scheduled_report',
            'fullpath' => '/shezar/reportbuilder/js/chooserestriction.js'
        );

        $PAGE->requires->strings_for_js(array('chooserestrictiontitle'), 'shezar_reportbuilder');
        $PAGE->requires->js_init_call('M.shezar_reportbuilder_chooserestriction.init', $args, false, $jsmodule);

        $chooselink = html_writer::link('#', get_string('changeglobalrestriction', 'shezar_reportbuilder'),
        ...
        if (!$appliedids) {
            ...
            $messagehtml = html_writer::div(get_string('selectedglobalrestrictionsselect', 'shezar_reportbuilder', $chooselink),
        ...
        $chooselink = html_writer::link('#', get_string('changeglobalrestriction', 'shezar_reportbuilder'), array(
        ...
        $messagehtml = html_writer::div(get_string('selectedglobalrestrictionsmany', 'shezar_reportbuilder', $a),
            'notifynotice globalrestrictionsnotice');
        echo html_writer::div($messagehtml, 'globalrestrictionscontainer');
    }
    ...
    public function display_sidebar_search() {
        ...
        $PAGE->requires->js_call_amd('shezar_reportbuilder/instantfilter', 'init', array('id' => $this->_id));
    }
    ...
    *
    * @return \shezar_reportbuilder\rb\access\base[] Array of access plugin classes indexed by type name
    */
    public static function get_all_access_plugins() {
        $plugins = array();
        $classes = core_component::get_namespace_classes('rb\access', 'shezar_reportbuilder\rb\access\base');
        foreach ($classes as $class) {
            /** @var shezar_reportbuilder\rb\access\base $obj */
            $obj = new $class();
            $type = $obj->get_type();
    ...
    */
    public static function get_reports_plugins_access($userid) {
        ...
        /** @var \shezar_reportbuilder\rb\access\base[] $active_plugins */
        ...
        if (isset($this->contentoptions) && is_array($this->contentoptions)) {
            foreach ($this->contentoptions as $option) {
                ...
                if (class_exists($classname)) {
                    ...
                    print_error('contentclassnotexist', 'shezar_reportbuilder', '', $classname);
                }
            }
        }
    ...
    function get_restriction_descriptions($which='all') {
        ...
        if ($this->contentmode != REPORT_BUILDER_CONTENT_MODE_NONE) {
            ...
            if ($this->contentmode == REPORT_BUILDER_CONTENT_MODE_ALL) {
                ...
                $content_restrictions[] = implode(get_string('or', 'shezar_reportbuilder'), $res);
            }
        }
    ...
    function get_single_join($join, $usage) {
        ...
        if ($match = $this->get_joinlist_item($join)) {
            ...
            print_error('joinnotinjoinlist', 'shezar_reportbuilder', '', (object)array('join' => $join, 'usage' => $usage));
            return false;
        }
    }
    ...
    function sort_joins($unsortedjoins) {
        ...
        if (count($items) != 0) {
            print_error('couldnotsortjoinlist', 'shezar_reportbuilder');
        }
    ...
    function get_full_count() {
        ...
        if (empty($this->_fullcount)) {
            ...
            try {
                ...
                print_error('error:problemobtainingreportdata', 'shezar_reportbuilder', '', $debuginfo);
            }
        }
    ...
    public function get_filtered_count($nocache = false) {
        ...
        if (empty($this->_filteredcount) || $nocache) {
            ...
            try {
                ...
                $debuginfo = $CFG->debugdeveloper ? $e->debuginfo : '';
                print_error('error:problemobtainingcachedreportdata', 'shezar_reportbuilder', '', $debuginfo);
            }
        }
    ...
    function export_data($format) {
        ...
        $format = \shezar_core\tabexport_writer::normalise_format($format);
        if ($format === 'fusion') {
            $this->download_fusion(); // Redirect.
            die;
        }

        $formats = \shezar_core\tabexport_writer::get_export_classes();
        ...
        $source = new \shezar_reportbuilder\tabexport_source($this);

        /** @var \shezar_core\tabexport_writer $writer */
        $writer = new $writerclass($source);

        // Log export event.
        \shezar_reportbuilder\event\report_exported::create_from_report($this, $format)->trigger();
    ...
    function display_table() {
        ...
        if (count($columns) == 0) {
            echo html_writer::tag('p', get_string('error:nocolumnsdefined', 'shezar_reportbuilder'));
            return;
        }

        $graphrecord = $DB->get_record('report_builder_graph', array('reportid' => $this->_id));
        if (!empty($graphrecord->type) && !shezar_feature_disabled('reportgraphs')) {
            $graph = new \shezar_reportbuilder\local\graph($graphrecord, $this, false);
        } else {
            $graph = null;
        }
        ...
        if ($cache) {
            ...
            $html .= get_string('report:cachelast', 'shezar_reportbuilder', $lastreport);
            $html .= html_writer::empty_tag('br');
            $html .= get_string('report:cachenext', 'shezar_reportbuilder', $nextreport);
            $html .= html_writer::end_tag('div');
            echo $html;
        }

        // Start the table.
        $table = new shezar_table($this->get_uniqueid('rb'));
        ...
        if ($initiallyhidden) {
            $table->set_no_records_message(get_string('initialdisplay_pending', 'shezar_reportbuilder'));
        } else {
            if ($this->is_report_filtered()) {
                $table->set_no_records_message(get_string('norecordswithfilter', 'shezar_reportbuilder'));
            } else {
                $table->set_no_records_message(get_string('norecordsinreport', 'shezar_reportbuilder'));
            }
        ...
        if ($graph and $graphdata = $graph->fetch_svg()) {
            if (core_useragent::check_browser_version('MSIE', '6.0') and !core_useragent::check_browser_version('MSIE', '9.0')) {
                ...
                $svgurl = new moodle_url('/shezar/reportbuilder/ajax/graph.php', array('id' => $this->_id, 'sid' => $this->_sid));
                if ($this->globalrestrictionset) {
                ...
                $nopdf = get_string('error:nopdf', 'shezar_reportbuilder');
        ...
        $jsmodule = array(
            'name' => 'shezar_reportbuilder_expand',
            'fullpath' => '/shezar/reportbuilder/js/expand.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.shezar_reportbuilder_expand.init', array(), true, $jsmodule);
    ...
    public function display_redirect_link() {
        if (isset($this->src->redirecturl)) {
            if (isset($this->src->redirectmessage)) {
                ...
                $message = '&laquo; ' . get_string('selectitem', 'shezar_reportbuilder');
            }
    ...
    public function is_initially_hidden() {
        ...
        // shezar hack to overcome limitations of the pagination library:
    ...
    function view_saved_menu() {
        ...
        if (count($savedoptions) > 0) {
            ...
            $select->label = get_string('viewsavedsearch', 'shezar_reportbuilder');
    ...
    function edit_button() {
        ...
        if (has_capability('shezar/reportbuilder:managereports', $context)) {
            return $OUTPUT->single_button(new moodle_url('/shezar/reportbuilder/general.php', array('id' => $this->_id)), get_string('editthisreport', 'shezar_reportbuilder'), 'get');
        } else {
            return '';
        }
    }
    ...
    function download_fusion() {
        $jump = new moodle_url('/shezar/reportbuilder/fusionexporter.php', array('id' => $this->_id, 'sid' => $this->_sid));
        redirect($jump->out());
        die;
    }
    ...
    public function get_filters_select($onlyinstant = false) {
        ...
        foreach ($filters as $filter) {
            if (!$onlyinstant || in_array($filter->filtertype, array('date', 'select', 'menuofchoices', 'multicheck'))) {
                ...
                } else if (get_string_manager()->string_exists($langstr, 'shezar_reportbuilder')) {
                    // How about in report builder?
                    $section = get_string($langstr, 'shezar_reportbuilder');
                } else {
    ...
    public function get_all_filters_select() {
        ...
        $allstandardfilters = array_merge(
                array(get_string('new') => array(0 => get_string('addanotherfilter', 'shezar_reportbuilder'))),
        ...
        $allsearchcolumns = array_merge(
            array(get_string('new') => array(0 => get_string('addanothersearchcolumn', 'shezar_reportbuilder'))),
    ...
    function print_feedback_results() {
        ...
        if ($this->is_initially_hidden()) {
            return get_string('initialdisplay_pending', 'shezar_reportbuilder');
        }
        ...
        if (!$groupid) {
            $out .= get_string('activitygroupnotfound', 'shezar_reportbuilder');
        }
        ...
        foreach ($data as $item) {
            ...
            $out .= html_writer::tag('p', get_string('resultsfromfeedback', 'shezar_reportbuilder', $item->responses_number));
    ...
    function get_feedback_standard_answer($qid, $item) {
        ...
        $answer = 'q' . $qid . '_list';
        if (isset($item->$count)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'shezar_reportbuilder', $item->$count));
        }
    ...
    function get_feedback_option_answer($qid, $options, $item) {
        ...
        if (isset($item->$numresp)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'shezar_reportbuilder', $item->$numresp));
        }
 ...
 *
 * @deprecated since shezar 2.9 - use reportbuilde::get_user_permitted_reports() instead
 *
 * @return array Array of report records
 */
...
function reportbuilder_send_scheduled_report($sched) {
    ...
    $subject = format_string($reportrecord->fullname) . ' ' . get_string('report', 'shezar_reportbuilder');

    if ($sched->savedsearchid != 0) {
        $messagedetails->savedtext = get_string('savedsearchmessage', 'shezar_reportbuilder', $saved->name);
    } else {
        $messagedetails->savedtext = '';
    }

    $message = get_string('scheduledreportmessage', 'shezar_reportbuilder', $messagedetails);
    ...
    if ($sched->exporttofilesystem == REPORT_BUILDER_EXPORT_EMAIL
        ...
        foreach ($externalusers as $email) {
            $userto = \shezar_core\shezar_user::get_external_user($email);
            $result = email_to_user($userto, $fromaddress, $subject, $messageplain, $messagehtml, $tempfile, $attachmentfilename);
            if ($result) {
                $emailedcount++;
            } else {
 ...
 * @param string $writerclass class extending \shezar_core\tabexport_writer
 *
...
function reportbuilder_export_schduled_report(stdClass $sched, reportbuilder $report, $writerclass) {
    ...
    $source = new \shezar_reportbuilder\tabexport_source($report);
...
function reportbuilder_get_export_filename($report, $userid, $scheduleid) {
    ...
    $reportfilename = format_string($report->fullname) . '_' .
            userdate(time(), get_string('datepickerlongyearphpuserdate', 'shezar_core')) . '_' . $scheduleid;
    ...
    if (!empty($path)) {
        ...
        if (preg_match($pattern, $path)) {
            mtrace(get_string('error:notapathexportfilesystempath', 'shezar_reportbuilder'));
        } else if (!is_dir($path)) {
            mtrace(get_string('error:notdirexportfilesystempath', 'shezar_reportbuilder'));
        } else if (!is_writable($path)) {
            mtrace(get_string('error:notwriteableexportfilesystempath', 'shezar_reportbuilder'));
        }
    }
...
function reportbuilder_get_report_url($report) {
    ...
    if ($report->embedded == 0) {
        return $CFG->wwwroot . '/shezar/reportbuilder/report.php?id=' . $report->id;
    } else {
...
function reportbuilder_get_embedded_report_object($embedname, $data=array()) {
    ...
    $sourcepaths[] = $CFG->dirroot . '/shezar/reportbuilder/embedded/';
 ...
 *
 * Looks in the shezar/reportbuilder/embedded/ directory and creates a new
...
function reportbuilder_create_embedded_record($shortname, $embed, &$error) {
    ...
    try {
        ...
        //\shezar_reportbuilder\event\report_created::create_from_report($report, true)->trigger();
...
function reportbuilder_get_export_options($currentoption = null, $includefusion = false) {
    ...
    foreach ($exportoptions as $option) {
        $option = \shezar_core\tabexport_writer::normalise_format($option);
        if ($option) {
            $enabled[$option] = true;
        }
    }
    ...
    $select = array();
    $alloptions = \shezar_core\tabexport_writer::get_export_options();
    ...
    if ($includefusion and isset($enabled['fusion'])) {
        $select['fusion'] = get_string('exportfusion', 'shezar_reportbuilder');
    }
...
*/
function shezar_reportbuilder_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/shezar_reportbuilder/$filearea/$args[0]/$args[1]";
...
function reportbuilder_set_default_access($reportid) {
    ...
    $acess = new \shezar_reportbuilder\rb\access\role();
