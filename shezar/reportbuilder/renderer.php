...
/**
* Output renderer for shezar_reportbuilder module
*/
class shezar_reportbuilder_renderer extends plugin_renderer_base {
    ...
    public function user_generated_reports_table($reports=array()) {
        ...
        if (empty($reports)) {
            return get_string('noreports', 'shezar_reportbuilder');
        }

        $tableheader = array(get_string('name', 'shezar_reportbuilder'),
                             get_string('source', 'shezar_reportbuilder'));
        if (!empty($CFG->enableglobalrestrictions)) {
            $tableheader[] = get_string('globalrestriction', 'shezar_reportbuilder');
        }
        $tableheader[] = get_string('options', 'shezar_reportbuilder');

        $data = array();

        $strsettings = get_string('settings', 'shezar_reportbuilder');
        $strclone = get_string('clonereport', 'shezar_reportbuilder');
        ...
        foreach ($reports as $report) {
            try {
                ...
                $editurl = new moodle_url('/shezar/reportbuilder/general.php', array('id' => $report->id));
                $deleteurl = new moodle_url('/shezar/reportbuilder/index.php', array('id' => $report->id, 'd' => 1));
                $cloneurl = new moodle_url('/shezar/reportbuilder/clone.php', array('id' => $report->id));
    ...
    public function embedded_reports_table($reports=array()) {
        ...
        if (empty($reports)) {
            return get_string('noembeddedreports', 'shezar_reportbuilder');
        }
        ...
        $strsettings = get_string('settings', 'shezar_reportbuilder');
        $strreload = get_string('restoredefaults', 'shezar_reportbuilder');
        $strclone = get_string('clonereport', 'shezar_reportbuilder');
        ...
        foreach ($reports as $report) {
            ...
            $editurl = new moodle_url('/shezar/reportbuilder/general.php', array('id' => $report->id));
            $reloadurl = new moodle_url('/shezar/reportbuilder/index.php', array('id' => $report->id, 'em' => 1, 'd' => 1));
            $cloneurl = new moodle_url('/shezar/reportbuilder/clone.php', array('id' => $report->id));
    ...
    public function format_records_to_view($records) {
        ...
        foreach ($records as $group => $entries) {
            ...
            $output .= get_string('groupassignlist', 'shezar_reportbuilder', $str);
    ...
    public function global_restrictions_table($globalrestrictions = array()) {
        ...
        if (empty($globalrestrictions)) {
            return get_string('noglobalrestrictionsfound', 'shezar_reportbuilder');
        }

        $tableheader = array(get_string('name', 'shezar_reportbuilder'),
            get_string('recordstoview', 'shezar_reportbuilder'),
            get_string('restrictedusers', 'shezar_reportbuilder'),
            get_string('options', 'shezar_reportbuilder'));
        ...
        foreach ($globalrestrictions as $restriction) {
            ...
            $baseurl = '/shezar/reportbuilder/restrictions/index.php';
            $viewurl = new moodle_url($baseurl, array('id' => $restriction->id, 'action' => 'view', 'sesskey' => sesskey()));
            $editurl = new moodle_url('/shezar/reportbuilder/restrictions/edit_general.php',
            ...
            if ($restriction->allrecords) {
                $row[] = get_string('restrictionallrecords', 'shezar_reportbuilder');
            } else {
                $row[] = $this->format_records_to_view($restriction->recordstoview);
            }
            if ($restriction->allusers) {
                $row[] = get_string('restrictionallusers', 'shezar_reportbuilder');
            } else {
            ...
            if ($restriction->active) {
                $tooltip = get_string('deactivateglobalrestriction', 'shezar_reportbuilder');
                $icon = 't/hide';
                $params = array('id' => $restriction->id, 'action' => 'deactivate', 'sesskey' => sesskey());
                $rowclass = '';
            } else {
                $tooltip = get_string('activateglobalrestriction', 'shezar_reportbuilder');
                $icon = 't/show';
    ...
    public function confirm_clone(reportbuilder $report) {
        ...
        foreach ($supportedproperties as $propertyname) {
            $strproperties[] = get_string($propertyname, 'shezar_reportbuilder');
        }
        $strpropertylist = html_writer::alist($strproperties);

        $out = '';
        if ($report->embedded){
            $out .= $this->output->notification(get_string('clonereportaccesswarning', 'shezar_reportbuilder'), 'notifynotice');
        }

        $info = new stdClass();
        $info->origname = $report->fullname;
        $info->clonename = get_string('clonenamepattern', 'shezar_reportbuilder', $report->fullname);
        $info->properties = $strpropertylist;

        $out .= html_writer::tag('p', get_string('clonedescrhtml', 'shezar_reportbuilder', $info));
        $buttons = array();
        $buttons[] = $this->output->single_button(new moodle_url('/shezar/reportbuilder/clone.php',
                array('confirm' => 1, 'id' => $report->_id)),
                get_string('clone', 'shezar_reportbuilder'), 'post');
        $buttons[] = $this->output->single_button(new moodle_url('/shezar/reportbuilder/index.php'),
    ...
    public function activity_groups_table($groups) {
        ...
        if (empty($groups)) {
            return html_writer::tag('p', get_string('nogroups', 'shezar_reportbuilder'));
        }

        $tableheader = array(get_string('name', 'shezar_reportbuilder'),
                             get_string('tag'),
                             get_string('baseitem', 'shezar_reportbuilder'),
                             get_string('activities', 'shezar_reportbuilder'),
                             get_string('reports', 'shezar_reportbuilder'),
                             get_string('options', 'shezar_reportbuilder'));
        ...
        foreach ($groups as $group) {
            ...
            $strsettings = get_string('settings', 'shezar_reportbuilder');
            $strdelete = get_string('delete', 'shezar_reportbuilder');
            $strcron = get_string('refreshdataforthisgroup', 'shezar_reportbuilder');

            $settingsurl = new moodle_url('/shezar/reportbuilder/groupsettings.php', array('id' => $group->id));
            ...
            $deleteurl = new moodle_url('/shezar/reportbuilder/groups.php', array('id' => $group->id, 'd' => 1));
            $deleteattr = array('title' => $strdelete);
            $delete = $this->output->action_icon($deleteurl, new pix_icon('t/delete', $strdelete), null, $deleteattr);

            $url = new moodle_url('/shezar/reportbuilder/groupsettings.php', array('id' => $group->id));
    ...
    public function activity_group_activities_table($activities) {
        ...
        $tableheader = array(get_string('course'),
                         get_string('feedback'),
                         get_string('lastchecked', 'shezar_reportbuilder'),
                         get_string('disabled', 'shezar_reportbuilder'));
        ...
        foreach ($activities as $activity) {
            ...
            if ($activity->course !== null) {
                ...
                $row[] = get_string('coursenotset', 'shezar_reportbuilder');
            }
            ...
            if ($activity->lastchecked !== null) {
                ...
                $row[] = get_string('notyetchecked', 'shezar_reportbuilder');
            }
    ...
    public function activity_group_reports_table($reports) {
        if (empty($reports)) {
            return html_writer::tag('p', get_string('noreportscount', 'shezar_reportbuilder'));
        }

        echo html_writer::tag('p', get_string('reportcount', 'shezar_reportbuilder', count($reports)));
    ...
    public function export_select($report, $sid = 0) {
        ...
        require_once($CFG->dirroot . '/shezar/reportbuilder/export_form.php');
    ...
    public function view_report_link($reporturl) {
        ...
        return html_writer::link($url, get_string('viewreport', 'shezar_reportbuilder'));
    }
    ...
    public function cache_pending_notification($report = 0) {
        ...
        if ($report instanceof reportbuilder) {
            ...
            if ($status == RB_CACHE_FLAG_FAIL) {
                $notice = $this->container(get_string('cachegenfail','shezar_reportbuilder'), 'notifyproblem clearfix');
            } else if ($status == RB_CACHE_FLAG_GEN) {
                $time = userdate($report->cacheschedule->genstart);
                $notice = $this->container(get_string('cachegenstarted','shezar_reportbuilder', $time), 'notifynotice clearfix');
            } else if ($status == RB_CACHE_FLAG_CHANGED) {
                $context = context_system::instance();
                if ($report->_id > 0 && has_capability('shezar/reportbuilder:managereports', $context)) {
                ...
                $notice = $this->container(get_string('cachepending','shezar_reportbuilder', $button),
    ...
    public function cachenow_button($reportid, $icon = false) {
        ...
        if (!$cachenowinit) {
            ...
            $PAGE->requires->strings_for_js(array('cachenow_title'), 'shezar_reportbuilder');
            $PAGE->requires->string_for_js('ok', 'moodle');
            $strcache = get_string('cachenow', 'shezar_reportbuilder');
            local_js(array(shezar_JS_DIALOG));
            $PAGE->requires->js_call_amd('shezar_reportbuilder/cachenow-lazy', 'init', array());
        }
    ...
    public function view_all_reports_link() {
        $url = new moodle_url('/shezar/reportbuilder/');
        return '&laquo; ' . html_writer::link($url, get_string('allreports', 'shezar_reportbuilder'));
    }
    ...
    public function save_button($report) {
        ...
        if ($search || $hasrequiredurlparams) {
            ...
            return $this->output->single_button(new moodle_url('/shezar/reportbuilder/save.php', $params),
                    get_string('savesearch', 'shezar_reportbuilder'), 'get');
        } else {
            return '';
        }
    }
    ...
    public function showhide_button($reportid, $reportshortname) {
        ...
        $html .= html_writer::empty_tag('input', array('type' => 'button',
            ...
            'value' => get_string('showhidecolumns', 'shezar_reportbuilder')
        ));
    ...
    public function manage_search_button($report) {
        ...
        $html .= html_writer::empty_tag('input', array('type' => 'button',
            ...
            'value' => get_string('managesavedsearches', 'shezar_reportbuilder')
        ));
        $html .= html_writer::end_tag('form');
        $html .= html_writer::end_tag('div');
    ...
    public function print_description($description, $reportid) {
        ...
        $description = file_rewrite_pluginfile_urls($description, 'pluginfile.php', $sitecontext->id, 'shezar_reportbuilder', 'report_builder', $reportid);
    ...
    public function print_result_count_string($countfiltered, $countall) {
        ...
        if ($countfiltered == $countall) {
            $heading = get_string('x' . $resultstr, 'shezar_reportbuilder', $countall);
        } else {
            $a = new stdClass();
            $a->filtered = $countfiltered;
            $a->unfiltered = $countall;
            $heading = get_string('xofy' . $resultstr, 'shezar_reportbuilder', $a);
        }
    ...
    public function saved_searches_table($searches, $report) {
        $tableheader = array(get_string('name', 'shezar_reportbuilder'),
                             get_string('publicsearch', 'shezar_reportbuilder'),
                             get_string('options', 'shezar_reportbuilder'));
        $data = array();
        $stredit = get_string('edit');
        $strdelete = get_string('delete', 'shezar_reportbuilder');
        ...
        foreach ($searches as $search) {
            $editurl = new moodle_url('/shezar/reportbuilder/savedsearches.php',
                array('id' => $search->reportid, 'action' => 'edit', 'sid' => $search->id));
            $deleteurl = new moodle_url('/shezar/reportbuilder/savedsearches.php',
    ...
    public function display_assigned_groups($assignments, $itemid, $suffix) {
        $tableheader = array(get_string('assigngrouptype', 'shezar_core'),
                             get_string('assignsourcename', 'shezar_core'),
                             get_string('assignincludechildrengroups', 'shezar_core'),
                             get_string('assignnumusers', 'shezar_core'),
                             get_string('actions'));
        if ($suffix === 'record') {
            $deleteurl = new moodle_url('/shezar/reportbuilder/restrictions/edit_recordstoview.php',
                    array('id' => $itemid, 'sesskey' => sesskey()));
        } else if ($suffix === 'user') {
            $deleteurl = new moodle_url('/shezar/reportbuilder/restrictions/edit_restrictedusers.php',
                    array('id' => $itemid, 'sesskey' => sesskey()));
        } else {
            $deleteurl = null;
        }
        ...
        if (empty($assignments)) {
            $table->data[] = array(get_string('nogroupassignments', 'shezar_core'));
        } else {
    ...
    public function edit_restriction_header(rb_global_restriction $restriction, $currenttab) {
        ...
        $url = new moodle_url('/shezar/reportbuilder/restrictions/index.php');
        $html .= $this->output->container_start('reportbuilder-navlinks');
        $html .= html_writer::link($url, get_string('allrestrictions', 'shezar_reportbuilder'));
        $html .= $this->output->container_end();

        if ($restriction->id) {
            $html .= $this->output->heading(get_string('editrestriction', 'shezar_reportbuilder', $restriction->name));
        } else {
            $html .= $this->output->heading(get_string('newrestriction', 'shezar_reportbuilder'));
        }
    ...
    public function edit_restriction_tabs(rb_global_restriction $restriction, $currenttab) {
        ...
        $tabgeneral = new tabobject(
            'general',
            new moodle_url('/shezar/reportbuilder/restrictions/edit_general.php', array('id' => $restriction->id)),
            get_string('general')
        );
        $tabrecordstoview = new tabobject(
            'recordstoview',
            new moodle_url('/shezar/reportbuilder/restrictions/edit_recordstoview.php', array('id' => $restriction->id)),
            get_string('recordstoview', 'shezar_reportbuilder')
        );
        $tabrestrictedusers = new tabobject(
            'restrictedusers',
            new moodle_url('/shezar/reportbuilder/restrictions/edit_restrictedusers.php', array('id' => $restriction->id)),
            get_string('restrictedusers', 'shezar_reportbuilder')
        );
