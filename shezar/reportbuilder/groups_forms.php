    ...
    function definition() {
        ...
        foreach ($activities as $activity) {
            $course = ($activity->fullname === null) ?
                get_string('coursenotset', 'shezar_reportbuilder') : $activity->fullname;
            $grouped_activities[$course][$activity->id] = $activity->name;
        }

        $mform->addElement('header', 'general', get_string('newgroup', 'shezar_reportbuilder'));

        // get all official tags
        $tags = $DB->get_records_menu('tag', array('tagtype' => 'official'), 'id', 'id, name');
        if (!$tags) {
            $mform->addElement('html', html_writer::tag('p', get_string('notags', 'shezar_reportbuilder')));
            return;
        }

        $mform->addElement('text', 'name', get_string('groupname', 'shezar_reportbuilder'),
            'maxlength="255"');
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required');
        $mform->addHelpButton('name', 'reportbuildergroupname', 'shezar_reportbuilder');

        $mform->addElement('select', 'assignvalue', get_string('grouptag',
            'shezar_reportbuilder'), $tags);
        // invalid if not set
        $mform->addHelpButton('assignvalue', 'reportbuildergrouptag', 'shezar_reportbuilder');
        ...
        $baseselect =& $mform->addElement('selectgroups', 'baseitem',
            get_string('basedon', 'shezar_reportbuilder'), $grouped_activities, $attributes);
        $mform->addHelpButton('baseitem', 'reportbuilderbaseitem', 'shezar_reportbuilder');
