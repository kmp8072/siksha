...
M.shezar_email_scheduled_report = M.shezar_email_scheduled_report || {
    ...
    init: function (Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_reportbuilder_filterdialogs.init()-> jQuery dependency required for this module to function.');
        }
    ...
    rb_init_load_dialogs: function () {
        ...
        $('#addexternalemail').click(function (event) {
            ...
            if ($.trim(emailvalue).length == 0 || !validEmail(emailvalue)) {
                ...
                alert(M.util.get_string('emailexternaluserisonthelist', 'shezar_reportbuilder'));
            } else{
    ...
    rb_add_external_email: function(email) {
        var url = M.cfg.wwwroot + '/shezar/reportbuilder/schedule_display_items_email.php';
...
rbShowUserDialog = function() {
    var selected = M.shezar_email_scheduled_report.config.existingsyusers;
    var url = M.cfg.wwwroot + '/shezar/reportbuilder/';

    rbDialog(
        'systemusers',
        M.util.get_string('addsystemusers', 'shezar_reportbuilder'),
...
rbShowAudienceDialog = function() {
    var selected = M.shezar_email_scheduled_report.config.existingaud;
    var url = M.cfg.wwwroot + '/shezar/reportbuilder/';

    rbDialog(
        'audiences',
        M.util.get_string('addcohorts', 'shezar_reportbuilder'),
