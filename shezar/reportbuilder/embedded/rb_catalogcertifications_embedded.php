 ...
 */
require_once($CFG->dirroot.'/shezar/reportbuilder/embedded/rb_findprograms_embedded.php');

class rb_catalogcertifications_embedded extends rb_base_embedded {
    public function __construct($data) {
        $this->url = '/shezar/coursecatalog/certifications.php';
        $this->source = 'certification';
        $this->shortname = 'catalogcertifications';
        $this->fullname = get_string('catalogcertifications', 'shezar_coursecatalog');

        $this->columns = array(
            array(
                'type' => 'prog',
                'value' => 'progexpandlink',
                'heading' => get_string('certifname', 'shezar_certification')
            ),
            array(
                'type' => 'prog',
                'value' => 'summary',
                'heading' => get_string('programsummary', 'shezar_certification')
            )
        );
    ...
    public function get_extrabuttons() {
        ...
        $categoryid = shezar_get_categoryid_with_capability('shezar/certification:createcertification');

        if ($categoryid !== false) {
            $createurl = new moodle_url("/shezar/program/add.php", array('category' => $categoryid, 'iscertif' => 1));
            $createbutton = new single_button($createurl, get_string('addcertification', 'shezar_coursecatalog'), 'get');
