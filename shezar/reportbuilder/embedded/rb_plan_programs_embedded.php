...
class rb_plan_programs_embedded extends rb_base_embedded {
    ...
    public function __construct($data) {
        ...
        $this->url = '/shezar/plan/record/programs.php';
        $this->source = 'dp_program';
        $this->shortname = 'plan_programs';
        $this->fullname = get_string('recordoflearningprograms', 'shezar_plan');
        $this->columns = array(
            array(
                'type' => 'program',
                'value' => 'proglinkicon',
                'heading' => get_string('programname', 'shezar_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'mandatory',
                'heading' => get_string('mandatory', 'shezar_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'recurring',
                'heading' => get_string('recurring', 'shezar_program'),
            ),
            array(
                'type' => 'program',
                'value' => 'timedue',
                'heading' => get_string('duestatus', 'shezar_program'),
            ),
            array(
            ...
            array(
                ...
                'value' => 'status',
                'heading' => get_string('progress', 'shezar_program'),
            ),
            array(
                'type' => 'program_completion',
                'value' => 'starteddate',
    ...
    public function is_capable($reportfor, $report) {
        ...
        return ($reportfor == $subjectid ||
            \shezar_job\job_assignment::is_managing($reportfor, $subjectid) ||
                has_capability('shezar/plan:accessanyplan', context_system::instance(), $reportfor) ||
                has_capability('shezar/core:viewrecordoflearning', context_user::instance($subjectid), $reportfor));
    }
