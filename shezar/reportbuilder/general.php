...
$returnurl = $CFG->wwwroot . "/shezar/reportbuilder/general.php?id=$id";
...
$record = file_prepare_standard_editor($record, 'description', $TEXTAREA_OPTIONS, context_system::instance(),
    'shezar_reportbuilder', 'report_builder', $record->id);

$mform = new report_builder_edit_form(null, array('report' => $report, 'record' => $record));

// form results check
if ($mform->is_cancelled()) {
    redirect($CFG->wwwroot . '/shezar/reportbuilder/index.php');
}
if ($fromform = $mform->get_data()) {
    ...
    $todb = file_postupdate_standard_editor($todb, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
        'shezar_reportbuilder', 'report_builder', $todb->id);

    $DB->update_record('report_builder', $todb);

    $report = new reportbuilder($id);
    \shezar_reportbuilder\event\report_updated::create_from_report($report, 'general')->trigger();
    shezar_set_notification(get_string('reportupdated', 'shezar_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
}
