...
class rb_filter_type {
    ...
    protected function get_filteroption($type, $value) {
        ...
        if (!$filteroption) {
            ...
            $a->value = $value;
            $a->source = $sourcename;
            debugging(get_string('error:filteroptiontypexandvalueynotfoundinz', 'shezar_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    protected function get_columnoption($type, $value) {
        ...
        if (!$columnoption) {
            ...
            $a->value = $value;
            $a->source = $sourcename;
            debugging(get_string('error:columnoptiontypexandvalueynotfoundinz', 'shezar_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    protected function get_joins($filteroption, $columnoption) {
        ...
        if (!reportbuilder::check_joins($this->report->src->joinlist, $joins)) {
            ...
            debugging(get_string('error:joinsforfiltertypexandvalueynotfoundinz', 'shezar_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    public static function get_filter($type, $value, $advanced, $region, $report) {
        ...
        if (!class_exists($classname)) {
            $filename = "{$CFG->dirroot}/shezar/reportbuilder/filters/{$filtertype}.php";
    ...
    function get_sql_filter($data) {
        print_error('abstractmethodcalled', 'shezar_reportbuilder', '', 'get_sql_filter()');
    }
    ...
    function check_data($formdata) {
        print_error('abstractmethodcalled', 'shezar_reportbuilder', '', 'check_data()');
    }
    ...
    function setupForm(&$mform) {
        print_error('abstractmethodcalled', 'shezar_reportbuilder', '', 'setupForm()');
    }
    ...
    function get_label($data) {
        print_error('abstractmethodcalled', 'shezar_reportbuilder', '', 'get_label()');
    }
