...
class rb_filter_grpconcat_jobassignment extends rb_filter_hierarchy_multi {
    ...
    function get_operators() {
        return array(
            ...
            self::JOB_OPERATOR_CONTAINS => get_string('filtercontains', 'shezar_reportbuilder'),
            self::JOB_OPERATOR_NOTCONTAINS => get_string('filtercontainsnot', 'shezar_reportbuilder'),
            self::JOB_OPERATOR_EQUALS => get_string('filterequals', 'shezar_reportbuilder'),
            self::JOB_OPERATOR_NOTEQUALS => get_string('filterequalsnot', 'shezar_reportbuilder')
        );
    }
    ...
    function setupForm(&$mform) {
        ...
        if ($this->showchildren) {
            $objs['child'] =& $mform->createElement('checkbox', $this->name.'_child', null, get_string('jobassign_children', 'shezar_reportbuilder'));
        }

        $grp =& $mform->addElement('group', $this->name . '_grp', $label, $objs, '', false);
        $mform->addHelpButton($grp->_name, 'reportbuilderjobassignmentfilter', 'shezar_reportbuilder');
    ...
    public function include_js() {
        ...
        $code[] = shezar_JS_TREEVIEW;
        local_js($code);

        $jsdetails = new stdClass();
        $jsdetails->strings = array(
            'shezar_job' => array('choosemanager'),
            'shezar_hierarchy' => array('chooseposition', 'selected', 'chooseorganisation', 'currentlyselected', 'selectcompetency'),
            'shezar_reportbuilder' => array('chooseorgplural', 'chooseposplural', 'choosecompplural')
        );
        ...
        $PAGE->requires->js_call_amd('shezar_reportbuilder/filter_dialogs', 'init', $jsdetails->args);
    }
}
...
function display_choose_user_items_link($filtername, $type) {
    return html_writer::tag('div', html_writer::link('#', get_string("choose{$type}plural", 'shezar_reportbuilder'),
