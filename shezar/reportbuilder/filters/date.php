...
class rb_filter_date extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        if ($includebetween) {
            ...
            $objs['pre'] =& $mform->createElement('checkbox', $this->name.'daysbeforechkbox', null,
                    get_string('dateisbetween', 'shezar_reportbuilder'));
            $accesslabel = html_writer::tag(
                'label',
                get_string('isbeforetoday', 'shezar_reportbuilder'),
                array('class' => 'accesshide', 'for' => 'id_' . $objs['pre']->getAttribute('name'))
            );
            $objs[] =& $mform->createElement('static', null, null, $accesslabel);
            $objs[] =& $mform->createElement('text', $this->name.'daysbefore', get_string('isbeforetoday', 'shezar_reportbuilder'), 'size="2"');
            $mform->setType($this->name.'daysbefore', PARAM_INT);
            $objs[] =& $mform->createElement('static', null, null, get_string('isbeforetoday', 'shezar_reportbuilder'));
            $objs[] =& $mform->createElement('static', null, null,
                    html_writer::span(get_string('isrelativetotoday', 'shezar_reportbuilder')));
            $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
            $objs['post'] =& $mform->createElement('checkbox', $this->name.'daysafterchkbox', null,
                    get_string('dateisbetween', 'shezar_reportbuilder'));
            $accesslabel = html_writer::tag(
                'label',
                get_string('isaftertoday', 'shezar_reportbuilder'),
                array('class' => 'accesshide', 'for' => 'id_' . $objs['post']->getAttribute('name'))
            );
            $objs[] =& $mform->createElement('static', null, null, $accesslabel);
            $objs[] =& $mform->createElement('text', $this->name.'daysafter', get_string('isaftertoday', 'shezar_reportbuilder'), 'size="2"');
            $mform->setType($this->name.'daysafter', PARAM_INT);
            $objs[] =& $mform->createElement('static', null, null, get_string('isaftertoday', 'shezar_reportbuilder'));
            $objs[] =& $mform->createElement('static', null, null,
                    html_writer::span(get_string('isrelativetotoday', 'shezar_reportbuilder')));
        }
    ...
    function get_label($data) {
        ...
        if ($daysafter and $daysbefore) {
            return get_string('datelabelisdaysbetween', 'shezar_reportbuilder', $a);

        } else if ($daysafter) {
            return get_string('datelabelisdaysafter', 'shezar_reportbuilder', $a);

        } else if ($daysbefore) {
            return get_string('datelabelisdaysbefore', 'shezar_reportbuilder', $a);
        }
        return '';
    }
}
