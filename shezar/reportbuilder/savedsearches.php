...
// This is the custom half ajax shezar page, we MUST send some headers here at least...
...
$returnurl = new moodle_url('/shezar/reportbuilder/savedsearches.php', array('id' => $id));;

$PAGE->set_context(context_system::instance());
$PAGE->set_url('/shezar/reportbuilder/savedsearches.php', array('id' => $id, 'sid' => $sid));
...
if ($sid) {
    ...
    if (!$search) {
        print_error('error:invalidsavedsearchid', 'shezar_reportbuilder');
    }
}
...
$pagetitle = format_string(get_string('savesearch', 'shezar_reportbuilder') . ': ' . $report->fullname);
...
if ($action === 'delete') {
    ...
    if ($scheduled = $DB->get_records('report_builder_schedule', array('savedsearchid' => $sid))) {
        ...
        ob_start();
        shezar_print_scheduled_reports(false, false, array("rbs.savedsearchid = ?", array($sid)));
        $out = ob_get_contents();
        ob_end_clean();

        $messageend = get_string('savedsearchinscheduleddelete', 'shezar_reportbuilder', $out) . str_repeat(html_writer::empty_tag('br'), 2);
    }

    echo $output->heading(get_string('savedsearches', 'shezar_reportbuilder'), 1);

    $messageend .= get_string('savedsearchconfirmdelete', 'shezar_reportbuilder', format_string($search->name));

    // Prompt to delete.
    $params = array('id' => $id, 'sid' => $sid, 'action' => 'delete', 'confirm' => 'true', 'sesskey' => $USER->sesskey);
    $confirmurl = new moodle_url('/shezar/reportbuilder/savedsearches.php', $params);
...
if (!empty($searches)) {
    ...
    echo html_writer::tag('p', get_string('error:nosavedsearches', 'shezar_reportbuilder'));
}
