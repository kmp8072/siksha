...
M.shezar_reportbuilder_savedsearches = M.shezar_reportbuilder_savedsearches || {
    ...
    init: function(Y, args) {
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_reportbuilder_savevedsearch.init()-> jQuery dependency required for this module to function.');
        }

        var shezarDialog_saved_search_handler = function() {};

        shezarDialog_saved_search_handler.prototype = new shezarDialog_handler();

        shezarDialog_saved_search_handler.prototype.every_load = function() {
        ...
        if (typeof managebutton.attr('id') != 'undefined') {
            var path = M.cfg.wwwroot + '/shezar/reportbuilder/';
            var handler = new shezarDialog_saved_search_handler();
            var name = 'searchlist';
            ...
            shezarDialogs[name] = new shezarDialog(
                ...
                {
                    buttons: buttons,
                    title: '<h2>' + M.util.get_string('managesavedsearches', 'shezar_reportbuilder') + '</h2>'
                },
