 ...
 */
    ...
    require_once($CFG->dirroot . '/shezar/reportbuilder/groups_forms.php');
    ...
    $returnurl = $CFG->wwwroot . '/shezar/reportbuilder/groups.php';
    ...
    if ($d && $confirm) {
        ...
        if (delete_group($id)) {
            shezar_set_notification(get_string('groupdeleted', 'shezar_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            shezar_set_notification(get_string('error:groupnotdeleted', 'shezar_reportbuilder'), $returnurl);
        }
    } else if ($d) {
        ...
        if ($reports) {
            ...
            shezar_set_notification(get_string('error:grouphasreports', 'shezar_reportbuilder'), $returnurl);
            die;
        } else {
            // prompt to delete
            echo $output->header();
            echo $output->heading(get_string('activitygroups', 'shezar_reportbuilder'));
            echo $output->confirm(get_string('groupconfirmdelete', 'shezar_reportbuilder'),
                new moodle_url('/shezar/reportbuilder/groups.php',
    ...
    if ($fromform = $mform->get_data()) {
        ...
        if ($newid = create_group($fromform, $errorcode)) {
            redirect($CFG->wwwroot . '/shezar/reportbuilder/groupsettings.php?id=' .
                $newid);
            die;
        } else {
            shezar_set_notification(get_string($errorcode, 'shezar_reportbuilder'), $returnurl);
            die;
        }
    ...
    echo html_writer::tag('p', get_string('activitygroupdesc', 'shezar_reportbuilder'));
