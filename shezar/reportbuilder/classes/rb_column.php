...
class rb_column {
     ...
     * or even:
     *
     * <code>$DB->sql_concat_join("' '", shezar_get_all_user_name_fields_join())</code>
     *
     * @access public
     * @var string
     */
    ...
    public function get_fields($src = null, $aliasmode = self::REGULAR, $returnextrafields=true) {
        ...
        if ($field === null) {
            ...
            $classname = "\\shezar_reportbuilder\\rb\\transform\\$this->transform";
            if (class_exists($classname)) {
                $fields[] = $classname::get_field($this, $src, $aliasmode, $returnextrafields);
            }

        } else if ($this->aggregate) {
            $classname = "\\shezar_reportbuilder\\rb\\aggregate\\$this->aggregate";
            if (class_exists($classname)) {
                $fields[] = $classname::get_field($this, $src, $aliasmode, $returnextrafields);
            }
    ...
    /**
     * Returns class name of shezar_reportbuilder\rb\display\ class used for
    ...
    public function get_display_class(reportbuilder $report) {
        ...
        if (!$displayfunc) {
            return '\shezar_reportbuilder\rb\display\legacy';
        }
