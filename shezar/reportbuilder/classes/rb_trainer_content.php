...
class rb_trainer_content extends rb_base_content {
    ...
    function text_restriction($title, $reportid) {
        ...
        case 'own':
            return $title . ' ' . get_string('is', 'shezar_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'reports':
            return $title . ' ' . get_string('reportsto', 'shezar_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'ownandreports':
            return $title . ' ' . get_string('is', 'shezar_reportbuilder') . ' "' .
                fullname($user) . '"' . get_string('or', 'shezar_reportbuilder') .
                get_string('reportsto', 'shezar_reportbuilder') . ' "' . fullname($user) . '"';
        default:
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerownrecords', 'shezar_reportbuilder'), 'own');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerstaffrecords', 'shezar_reportbuilder'), 'reports');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('both', 'shezar_reportbuilder'), 'ownandreports');
        $mform->addGroup($radiogroup, 'trainer_who_group',
            get_string('includetrainerrecords', 'shezar_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->addHelpButton('trainer_header', 'reportbuildertrainer', 'shezar_reportbuilder');
    }
