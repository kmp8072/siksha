...
abstract class base {
    ...
    public static function get_types() {
        ...
        foreach (scandir(__DIR__) as $filename) {
            ...
            $classname = '\shezar_reportbuilder\rb\transform\\' . $name;
    ...
    public static function get_options() {
        ...
        foreach (self::get_types() as $name => $classname) {
            $options[$classname::get_typename()][$name] = get_string("transformtype{$name}_name", 'shezar_reportbuilder');
        }
    ...
    public static function require_column_heading_strings() {
        ...
        foreach (self::get_types() as $name => $classname) {
            $PAGE->requires->string_for_js("transformtype{$name}_heading", 'shezar_reportbuilder');
        }
    }
    ...
    public static function get_typename() {
        return get_string('advancedgrouptimedate', 'shezar_reportbuilder');
    }
    ...
    /**
     * Is the result of this transformation usable for graph series?
     * @param \rb_column $column
     * @param \rb_column_option $option
