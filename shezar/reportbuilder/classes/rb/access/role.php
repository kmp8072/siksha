...
namespace shezar_reportbuilder\rb\access;
...
class role extends base {
    ...
    public function form_template($mform, $reportid) {
        ...
        $mform->addElement('header', 'accessbyroles', get_string('accessbyrole', 'shezar_reportbuilder'));
        ...
        if (!empty($roles)) {
            $contextoptions = array('site' => get_string('systemcontext', 'shezar_reportbuilder'), 'any' => get_string('anycontext', 'shezar_reportbuilder'));

            // set context for role-based access
            $mform->addElement('select', 'role_context', get_string('context', 'shezar_reportbuilder'), $contextoptions);
            $mform->setDefault('role_context', $context);
            $mform->disabledIf('role_context', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('role_context', 'reportbuildercontext', 'shezar_reportbuilder');
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('roleswithaccess', 'shezar_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('roles', 'reportbuilderrolesaccess', 'shezar_reportbuilder');
        } else {
            $mform->addElement('html', html_writer::tag('p', get_string('error:norolesfound', 'shezar_reportbuilder')));
        }
    }
