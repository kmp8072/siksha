...
class shezar_reportbuilder_scheduled_export_testcase extends advanced_testcase {
    ...
    public function test_process_scheduled_task() {
        ...
        $task = new \shezar_reportbuilder\task\process_scheduled_task();
        ob_start(); // Verify diagnostic output.
        $task->execute();
        $info = ob_get_contents();
        ob_end_clean();
    ...
    public function test_export_formats() {
        ...
        $schedules = array();
        $plugins = \shezar_core\tabexport_writer::get_export_classes();
