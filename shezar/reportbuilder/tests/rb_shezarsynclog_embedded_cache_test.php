...
class shezar_reportbuilder_rb_shezarsynclog_embedded_cache_testcase extends reportcache_advanced_testcase {
    // testcase data
    protected $report_builder_data = array('id' => 15, 'fullname' => 'Sync log', 'shortname' => 'shezarsynclog',
                                           'source' => 'shezar_sync_log', 'hidden' => 1, 'embedded' => 1);

    protected $report_builder_columns_data = array(
                        array('id' => 69, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'id',
                              'heading' => 'A', 'sortorder' => 1),
                        array('id' => 70, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'time',
                              'heading' => 'B', 'sortorder' => 2),
                        array('id' => 71, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'element',
                              'heading' => 'C', 'sortorder' => 3),
                        array('id' => 72, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'logtype',
                              'heading' => 'D', 'sortorder' => 4),
                        array('id' => 73, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'action',
                              'heading' => 'E', 'sortorder' => 5),
                        array('id' => 74, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'info',
                              'heading' => 'F', 'sortorder' => 6));

    protected $report_builder_filters_data = array(
                        array('id' => 31, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'time',
                              'sortorder' => 1, 'advanced' => 0),
                        array('id' => 32, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'element',
                              'sortorder' => 2, 'advanced' => 0),
                        array('id' => 33, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'logtype',
                              'sortorder' => 3, 'advanced' => 0),
                        array('id' => 34, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'action',
                              'sortorder' => 4, 'advanced' => 0),
                        array('id' => 35, 'reportid' => 15, 'type' => 'shezar_sync_log', 'value' => 'info',
    ...
    public function test_synclog($usecache) {
        ...
        $form = array('shezar_sync_log-info' => array('operator' => 0, 'value' => 'critical'));
        $result = $this->get_report_result($this->report_builder_data['shortname'], array(), $usecache, $form);
        $this->assertCount(2, $result);
        $was = array();
        foreach ($result as $r) {
             $this->assertStringMatchesFormat('Record with critical word %d', $r->shezar_sync_log_info);
    ...
    public function test_is_capable() {
        ...
        assign_capability('tool/shezar_sync:manage', CAP_ALLOW, $roleuser->id, $syscontext);
        $syscontext->mark_dirty();
        $this->assertTrue($embeddedobject->is_capable($userid, $report),
                'user with capability tool/shezar_sync:manage cannot access report');
        assign_capability('tool/shezar_sync:manage', CAP_INHERIT, $roleuser->id, $syscontext);
    }
    ...
    protected function create_synclog($data = array()) {
        ...
        $lid = shezar_sync_log($element, $info, $type, $action);
        $record = $DB->get_record('shezar_sync_log', array('id' => $lid));
        return $record;
    }
}
