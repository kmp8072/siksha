 ...
 *
 * @package shezar
 * @subpackage shezar_coursecatalog
 */
 ...
 */
function shezar_get_category_item_count($categoryids, $viewtype = 'course') {
    global $CFG, $USER, $DB;
    ...
    // Add visibility.
    list($visibilityjoinsql, $visibilityjoinparams) = shezar_visibility_join($USER->id, $viewtype, $itemalias);
    ...
    foreach ($items as $id => $item) {
        if ($item->isvisibletouser) {
            ...
            if ($viewtype == 'course') {
                ...
                if (($viewtype == 'program') && has_capability('shezar/program:viewhiddenprograms', $context) ||
                    ($viewtype == 'certification') && has_capability('shezar/certification:viewhiddencertifications', $context) ||
                    !empty($CFG->audiencevisibility) && has_capability('shezar/coursecatalog:manageaudiencevisibility', $context)) {
                    unset($item->isvisibletouser); // Visible.
                } else {
                    unset($items[$id]); // Not visible.
 ...
 */
function shezar_course_cmp_by_count($a, $b) {
 ...
 */
function shezar_get_style_visibility($component, $oldvisfield = 'visible', $audvisfield = 'audiencevisible') {
    global $CFG;
    $dimmed = '';
 ...
 */
function shezar_visibility_where($userid = null, $fieldbaseid = 'course.id', $fieldvisible = 'course.visible',
    ...
    switch($type) {
        case 'course':
            ...
            break;
        case 'program':
            $capability = 'shezar/program:viewhiddenprograms';
            ...
            break;
        case 'certification':
            $capability = 'shezar/certification:viewhiddencertifications';
    ...
    if (is_siteadmin($userid)) {
        ...
        $canmanagevisibility = has_capability('shezar/coursecatalog:manageaudiencevisibility', $systemcontext, $userid);
 ...
 *
 * list($visibilityjoinsql, $visibilityjoinparams) = shezar_visibility_join($userid, 'program', 'p');
 *
 ...
 *                         'JOIN' will remove rows from you main table (like using shezar_visibility_where).
 * @return array list(string $joinsql, array $joinparams)
 */
function shezar_visibility_join($userid = null, $type = 'course', $mainalias = 'course', $joinalias = 'visibilityjoin',
                                $jointype = 'LEFT JOIN') {
    global $USER, $DB;
    ...
    // Get the shezar_visibility_where sql, which the join will be based on.
    list($visibilitysql, $visibilityparams) = shezar_visibility_where($userid, 'tvjoinsub.id', 'tvjoinsub.visible',
