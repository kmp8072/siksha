...
class shezar_connect_courses {
    ...
    public function init_page_js() {
        ...
        $PAGE->requires->strings_for_js(array('courses'), 'shezar_connect');
        $jsmodule = array(
            'name' => 'shezar_connect_course',
            'fullpath' => '/shezar/connect/dialog/course.js');
        $instanceid = empty($this->client->id) ? -1 : $this->client->id;
        $args = array('selected' => $selected, 'instanceid' => $instanceid);
        $PAGE->requires->js_init_call('M.shezar_connect_course.init', $args, true, $jsmodule);
