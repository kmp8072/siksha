...
class shezar_connect_cohorts {
    ...
    public function init_page_js() {
        ...
        $PAGE->requires->strings_for_js(array('cohorts'), 'shezar_connect');
        $jsmodule = array(
            'name' => 'shezar_connect_cohort',
            'fullpath' => '/shezar/connect/dialog/cohort.js');
        $instanceid = empty($this->client->id) ? -1 : $this->client->id;
        $args = array('selected' => $selected, 'instanceid' => $instanceid);
        $PAGE->requires->js_init_call('M.shezar_connect_cohort.init', $args, true, $jsmodule);
