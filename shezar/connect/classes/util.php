...
class util {
     ...
     * Register new client.
     *
     * @param \stdClass $data from shezar_connect_form_client_add
    ...
    public static function add_client($data) {
        ...
        $client->clientidnumber = self::create_unique_hash('shezar_connect_clients', 'clientidnumber');
        $client->clientsecret   = self::create_unique_hash('shezar_connect_clients', 'clientsecret');
        ...
        $client->serversecret   = self::create_unique_hash('shezar_connect_clients', 'serversecret');
        ...
        $client->id = $DB->insert_record('shezar_connect_clients', $client);
        ...
        $data = array(
            ...
            'serveridnumber' => get_config('shezar_connect', 'serveridnumber'),
        ...
        $DB->set_field('shezar_connect_clients', 'status', self::CLIENT_STATUS_DELETED, array('id' => $client->id));
        $client->status = self::CLIENT_STATUS_DELETED;
        self::purge_deleted_client($client);
        $DB->delete_records('shezar_connect_clients', array('id' => $client->id));
     ...
     * Edit client.
     *
     * @param \stdClass $data from shezar_connect_form_client_edit
     */
    public static function edit_client($data) {
        ...
        $DB->update_record('shezar_connect_clients', $client);
        ...
        $current = $DB->get_records('shezar_connect_client_cohorts', array('clientid' => $client->id));
        ...
        $current = $DB->get_records('shezar_connect_client_courses', array('clientid' => $client->id));
        foreach ($current as $a) {
    ...
    public static function purge_deleted_client($client) {
        ...
        $DB->delete_records('shezar_connect_sso_sessions', array('clientid' => $client->id));
        $DB->delete_records('shezar_connect_client_cohorts', array('clientid' => $client->id));
        $DB->delete_records('shezar_connect_client_courses', array('clientid' => $client->id));
    }
    ...
    public static function add_client_cohort($client, $cohortid) {
        ...
        $DB->insert_record('shezar_connect_client_cohorts', $rec);
    }
    ...
    public static function remove_client_cohort($client, $cohortid) {
        global $DB;
        $DB->delete_records('shezar_connect_client_cohorts', array('clientid' => $client->id, 'cohortid' => $cohortid));
    }
    ...
    public static function add_client_course($client, $courseid) {
        ...
        $rec->timecreated = time();
        $DB->insert_record('shezar_connect_client_courses', $rec);
    }
    ...
    public static function remove_client_course($client, $courseid) {
        global $DB;
        $DB->delete_records('shezar_connect_client_courses', array('clientid' => $client->id, 'courseid' => $courseid));
    }
    ...
    public static function create_sso_session(\stdClass $client) {
        ...
        $session->ssotoken    = self::create_unique_hash('shezar_connect_sso_sessions', 'ssotoken');
        $session->timecreated = time();

        $session->id = $DB->insert_record('shezar_connect_sso_sessions', $session);

        return $DB->get_record('shezar_connect_sso_sessions', array('id' => $session->id));
    }
    ...
    public static function ignore_timeout_hook($user, $sid, $timecreated, $timemodified) {
        ...
        $ssosessions = $DB->get_records('shezar_connect_sso_sessions', array('sid' => $sid, 'userid' => $user->id));
        foreach ($ssosessions as $ssosession) {
            $client = $DB->get_record('shezar_connect_clients', array('id' => $ssosession->clientid));
    ...
    public static function terminate_sso_session(\stdClass $client, \stdClass $ssosession) {
        ...
        $DB->delete_records('shezar_connect_sso_sessions', array('id' => $ssosession->id));
    ...
    public static function login_page_info() {
        ...
        if (empty($CFG->enableconnectserver) or empty($SESSION->shezarconnectssostarted)) {
            return null;
        }

        $clientidnumber = $SESSION->shezarconnectssostarted['clientidnumber'];
        $requesttoken = $SESSION->shezarconnectssostarted['requesttoken'];

        $client = $DB->get_record('shezar_connect_clients', array('clientidnumber' => $clientidnumber));
        if (!$client) {
            return null;
        }

        $url = new \moodle_url('/shezar/connect/sso_request.php', array(
        ...
        return '<div class="subcontent">' . $OUTPUT->single_button($url, get_string('cancelsso', 'shezar_connect', format_string($client->clientname))) . '</div>';
    }
    ...
    public static function warn_if_not_https() {
        ...
        if (strpos($CFG->wwwroot, 'https://') !== 0) {
            return $OUTPUT->notification(get_string('errorhttpserver', 'shezar_connect'), 'notifyproblem');
        }
        return '';
    }
    ...
    public static function prepare_user_for_client($user, $sso = false) {
        ...
        if (!get_config('shezar_connect', 'syncpasswords')) {
            $user->password = null;
        }
