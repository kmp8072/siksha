...
class core_user_editadvanced_form {
    ...
    /**
     * Hook watcher that extends the user edit form with shezar specific elements.
     *
    ...
    public static function extend_form(editadvanced_form_definition_complete $hook) {
        ...
        if (shezar_gap_can_edit_aspirational_position($userid)) {
    ...
    public static function save_form(editadvanced_form_save_changes $hook) {
        ...
        if (shezar_gap_can_edit_aspirational_position($hook->userid)) {
    ...
    protected static function add_aspirational_position_controls_to_form(editadvanced_form_definition_complete $hook) {
        ...
        if ($submittedaspirationalpositionid) {
            ...
            $position = shezar_gap_get_aspirational_position($user->id);
            if (!empty($position)) {
        ...
        $buttonhtml = \html_writer::empty_tag('input', array(
            'type' => 'button',
            'value' => get_string('chooseposition', 'shezar_hierarchy'),
        ...
        $poselem = $mform->createElement(
            ...
            get_string('useraspirationalposition', 'shezar_hierarchy'),
            $poshtml . $buttonhtml
        );
        ...
        $mform->addHelpButton('aspirationalposition', 'useraspirationalposition', 'shezar_hierarchy');
    ...
    protected static function initialise_position_dialog_js(editadvanced_form_display $hook) {
        ...
        $PAGE->requires->strings_for_js(array('chooseposition'), 'shezar_hierarchy');

        $jsmodule = array(
            'name' => 'shezar_positionuser',
            'fullpath' => '/shezar/gap/js/aspirational_position.js',
            'requires' => array('json')
        );

        $selected_position = json_encode(dialog_display_currently_selected(get_string('selected', 'shezar_hierarchy'), 'aspirationalposition'));
    ...
    protected static function save_aspirational_position_changes(editadvanced_form_save_changes $hook) {
        ...
        shezar_gap_assign_aspirational_position($userid, $positionid);
    }
}