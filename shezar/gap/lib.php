 ...
 */
function shezar_gap_can_edit_aspirational_position($userid) {
    global $USER;

    // can assign any user's position
    if (has_capability('shezar/gap:assignaspirationalposition', context_system::instance())) {
        return true;
    }
    ...
    if (!empty($userid)) {
        ...
        if (has_capability('shezar/gap:assignaspirationalposition', $personalcontext)) {
            return true;
        }

        // editing own position and have capability to assign own position
        if ($USER->id == $userid && has_capability('shezar/gap:assignselfaspirationalposition', context_system::instance())) {
            return true;
        }
    }
    return false;
 ...
 */
function shezar_gap_get_aspirational_position($userid) {
    global $DB;
    if ($userid < 1) {
        return false;
    }
 ...
 */
function shezar_gap_assign_aspirational_position($userid, $positionid) {
    global $USER, $DB;
    if ($userid < 1) {
