 ...
 */
function xmldb_shezar_mssql_install() {
    ...
    if ($DB->get_dbfamily() == 'mssql') {
        ...
        if (!$DB->get_record_sql('SELECT * FROM sys.assemblies WHERE name = ?',
            ...
            $sqllist = require($CFG->dirroot . '/shezar/mssql/db/mssqlgroupconcat.php');
            try {
                ...
                echo $OUTPUT->notification(get_string('mssqlgroupconcatfail', 'shezar_mssql',
                        $message));
 ...
 */
function xmldb_shezar_mssql_install_recovery() {
    xmldb_shezar_mssql_install();
}
 ...
 */
function shezar_mssql_environment_check(environment_results $result) {
    global $DB;
    $status = true;
