...
$PAGE->requires->strings_for_js(array('chooseposition', 'choosemanager', 'chooseorganisation'), 'shezar_job');
$PAGE->requires->strings_for_js(array('error:positionnotselected', 'error:organisationnotselected', 'error:managernotselected'), 'shezar_job');
$jsmodule = array(
        'name' => 'shezar_jobassignment',
        'fullpath' => '/shezar/job/js/jobassignment.js',
        'requires' => array('json'));
$selected_position = json_encode( dialog_display_currently_selected(get_string('selected', 'shezar_job'), 'position') );
$selected_organisation = json_encode( dialog_display_currently_selected(get_string("selected", "shezar_job"), "organisation") );
$selected_manager = json_encode( dialog_display_currently_selected(get_string("selected", "shezar_job"), "manager") );
...
$PAGE->requires->js_init_call('M.shezar_positionuser.init', $args, false, $jsmodule);
...
echo html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseposition', 'shezar_job'), 'id' => 'show-position-dialog'));
echo $OUTPUT->container_end();

echo $OUTPUT->container_start();
echo html_writer::tag('span', '', array('class' => '', 'id' => 'organisationtitle'));
echo html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseorganisation', 'shezar_job'), 'id' => 'show-organisation-dialog'));
echo $OUTPUT->container_end();

echo $OUTPUT->container_start();
echo html_writer::tag('span', '', array('class' => '', 'id' => 'managertitle'));
echo html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('choosemanager', 'shezar_job'), 'id' => 'show-manager-dialog'));
echo $OUTPUT->container_end();
...
if (!$item) {
    ...
    $out .= html_writer::empty_tag('input', array('type' => 'submit', 'id' => "show-related-dialog", 'value' => get_string('assignrelatedcompetencies', 'shezar_hierarchy')));
