...
$strheading = 'Element Library: shezar Toolbar';
...
echo html_writer::tag('p', 'The shezar Toolbar is currently only available on tables that use flexible_tables(), we need to decide if we should keep that requirement, and update other tables to use flexible_tables, or if we should allow toolbars to be generated independently for use in manually generated tables or those built using the html_table class.');

$table = new shezar_table('uniqueid');
...
echo $OUTPUT->container('Unlike the flexible table class, shezar tables are still shown if there are no results, so you can access the toolbar options and for consistency. You can customise the "Nothing to display" message via the method set_no_records_message().');

$table = new shezar_table('uniqueid2');
$table->attributes = array('class' => 'generaltable fullwidth');
