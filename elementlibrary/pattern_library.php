...
$extendsbootstrap = '<span class="label label-info">Extends Bootstrap ' . $OUTPUT->flex_icon('shezar') . '</span>';
...
echo '<p>shezar provides some additional variants on top of Bootstrap defaults.</p>';
...
echo '<p>Notifications use alert markup. Note that the info type doesn\'t yet have a shezar notification equivalent.</p>';
...
echo '<p>Note shezar badges extend the Bootstrap 3 API by providing brand and state-based variants illustrated below.</p>';
echo '<p><a href="http://getbootstrap.com/components/#badges">' . $bootstrapdocstext . '</a></p>';
...
echo <<<EOF
<p>Buttons in shezar are based on the Bootstrap defaults with some additional types.</p>
EOF;
...
echo <<<EOF
<p>Tabs in shezar are based on the Bootstrap Nav defaults with some additional types.</p>
EOF;
