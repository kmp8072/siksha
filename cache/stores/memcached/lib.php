...
class cachestore_memcached extends cache_store implements cache_is_configurable {
    ...
    public function __construct($name, array $configuration = array()) {
        ...
        if (!empty($configuration['prefix'])) {
            // shezar: Hopefully this is clusters sharing caches only.
            $prefix = (string)$configuration['prefix'];
        } else {
            // shezar: Make sure prefix is different if the way of storing data in memcached server changes.
        ...
        if ($this->clustered) {
            ...
            foreach ($configuration['setservers'] as $server) {
                ...
                // shezar: one of the set servers should always match the read server,
            ...
            // shezar: buffering mey get everything out of sync really quick.
            $bufferwrites = false;
        }
        ...
        // shezar: make sure if any settings change the connection is not reused.
        ...
        if ($this->clustered) {
            foreach ($this->setservers as $setserver) {
                // shezar: make sure the persistent connection is shared with the reading server.
    ...
    public function delete_many(array $keys) {
        if ($this->clustered) {
            ...
            // shezar: count($keys) is expensive in PHP!
            $count = null;
            foreach ($this->setconnections as $connection) {
                if (isset($count)) {
    ...
    public function instance_deleted() {
        if ($this->connection) {
            ...
            // shezar: first make sure servers are actually configured.
            if (!$this->servers or !$this->options) {
                return;
            }
            // shezar: use consistent hash.
            $persistenthash = sha1(serialize($this->options) . serialize($this->servers) . $this->name);
            $connection = new Memcached($persistenthash);
            $servers = $connection->getServerList();
            if (empty($servers)) {
        ...
        @$connection->flush();
        unset($connection);
        // shezar: never unset standard object properties, use NULL instead.
