...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
        if (($iscurrentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('moodle/user:update',
            ...
            if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                ...
                if (empty($url)) {
                    // shezar: 'id' is the name of parameter, Moodle messed it up during rewrite.
                    if (empty($course)) {
    ...
    // shezar: Preference page. Show it to the current user, administrators and people with the right capabilities.
