...
/* NOTE: following servers are not fully tested! */
        ...
        global $CFG;
        // shezar: built-in PHP Development Server is not intended for production use!
...
function redirect_if_major_upgrade_required() {
    global $CFG;
    $lastmajordbchanges = 2015111606.00; // Anything before shezar 9.0rc1
    if (empty($CFG->version) or (float)$CFG->version < $lastmajordbchanges or
            during_initial_install() or !empty($CFG->adminsetuppending) or !isset($CFG->shezar_version)) {
        try {
 ...
 *
 * shezar: we have moved it here so that bootstrap render may extend this!
 *
...
class bootstrap_renderer extends renderer_base {
     ...
     */
    public function __construct() {
        // shezar: do not call parent, we need the interface only.
    }

    /**
     * shezar hack: make this compatible with renderer_base interface.
     *
     * @param $templatename
     * @param $context
     * @return bool|string
