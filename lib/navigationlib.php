...
class global_navigation extends navigation_node {
    ...
    public function __construct(moodle_page $page) {
        ...
        if (shezar_feature_visible('shezardashboard') && get_home_page() == HOMEPAGE_shezar_DASHBOARD) {
            // We are using shezar dashboard for the root element.
            $properties = array(
                'key' => 'mydashboard',
                'type' => navigation_node::TYPE_SYSTEM,
                'text' => get_string('dashboard'),
                'action' => new moodle_url('/shezar/dashboard/index.php')
            );
        } else {
    ...
    public function initialise() {
        ...
        if (!isloggedin() or isguestuser()) {
            // shezar: One home entry is enough, do not link Dashboard.
            ...
            if (shezar_feature_visible('shezardashboard') && (count(shezar_dashboard::get_user_dashboards($USER->id)))) {
                $this->rootnodes['home'] = $this->add(get_string('dashboard'), new moodle_url('/shezar/dashboard/index.php'));
            }
        } else {
            // The home element should be the site because the root node is shezar dashboard
        ...
        if (!empty($CFG->navexpandmycourses) && $this->page->pagelayout === 'dashboard') { // shezar has separate dashboard layout.
    ...
    protected function load_all_courses($categoryids = null) {
        ...
        list($visibilitysql, $visibilityparams) = shezar_visibility_where(null, 'c.id', 'c.visible', 'c.audiencevisible');
    ...
    public function add_course(stdClass $course, $forcegeneric = false, $coursetype = self::COURSE_OTHER) {
        ...
        $dimmed = ($issite) ? '' : shezar_get_style_visibility($course);
...
class settings_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        switch ($context->contextlevel) {
            ...
            case CONTEXT_PROGRAM:
                shezar_load_program_settings($this, $this->page->context, true);
                break;
            case CONTEXT_COURSE:
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            ...
            if ($CFG->enablecompletion && $course->enablecompletion) {
                ...
                // shezar CHANGE - manual archive course completions link.
            ...
            if (shezar_feature_visible('competencies')) {
                ...
                $coursenode->add(get_string('competencies', 'shezar_hierarchy'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/settings', ''));
            }
            ...
            if (has_capability('moodle/course:managereminders', $coursecontext)) {
                ...
                $coursenode->add(get_string('remindersmenuitem', 'shezar_coursecatalog'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/email', ''));
            }
    ...
    public function can_view_user_preferences($userid) {
        // shezar: TL-6675 admin and other ppl with edit cap may see the profile of other users too.
...
class navigation_json {
    ...
    protected function convert_child($child, $depth=1) {
        ...
        if ($child->icon instanceof pix_icon) {
            ...
            // shezar: convert the icon to a flex icon, if we can.
