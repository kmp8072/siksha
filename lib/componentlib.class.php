...
define('DOWNLOAD_BASE', 'https://download.shezarlms.com/lang');
/**
...
class lang_installer {
    ...
    public function __construct($langcode = '') {
        ...
        $shezarversion = shezar_major_version(); //may return false if something is badly wrong
        if ($shezarversion) {
            $this->version = 'T' . $shezarversion;
        } else {
            debugging('shezar version number could not be determined from version.php!');
        }
    ...
    protected function install_language_pack($langcode) {
        ...
        if ($status == COMPONENT_ERROR) {
            if ($OUTPUT) {
                // shezar: $OUTPUT is not available in the installer.
            ...
            echo $OUTPUT->notification(get_string('unexpected_installer_result', 'shezar_core', $status), 'notifyproblem');
            return self::RESULT_DOWNLOADERROR;
        }
    }
}
