...
class core_component {
    ...
    protected static $supportsubplugins = array('mod', 'editor', 'shezar', 'tool', 'local');
    ...
    /** @var string shezar build */
    protected static $shezarbuild = null;
    ...
    protected static function init() {
        ...
        if (!CACHE_DISABLE_ALL and !self::is_developer()) {
            ...
            if (is_readable($cachefile)) {
                ...
                } else if (!isset($cache['shezarbuild']) or $cache['shezarbuild'] !== self::fetch_shezarbuild()) {
                    error_log('Resetting core_component cache after shezar upgrade to build ' . self::fetch_shezarbuild());
...
';
    ...
    /**
     * Get the shezar build.
     *
     * In order for this to work properly, opcache should be reset beforehand.
     *
     * @return string build number
     */
    protected static function fetch_shezarbuild() {
        global $CFG;
        if (self::$shezarbuild === null) {

            $shezar = new stdClass(); // Prevent IDE complaints.
            require($CFG->dirroot . '/version.php');
            self::$shezarbuild = $shezar->build;
        }
        return self::$shezarbuild;
    }
    ...
    protected static function fetch_subsystems() {
        ...
        $info = array(
            ...
            // shezar: extra subsystems - do not add any new!
    ...
    protected static function fetch_plugintypes() {
        ...
        $types = array(
            ...
            'plagiarism'    => $CFG->dirroot.'/plagiarism',
            'shezar'        => $CFG->dirroot.'/shezar', // must be before admin tools so shezar_sync can add to shezar tables
    ...
    /**
     * Returns classes from given namespace across all shezar plugins and core.
     *
     ...
     * @param string $component restrict results to one plugin only, for example 'shezar_reportbuilder'
    ...
    public static function get_namespace_classes($namespace, $instanceof = null, $component = null, $excludeabstract = true) {
        ...
        foreach (self::$classmap as $class => $unused) {
            if (strpos($class, $quickmatch) === false) {
                // There are very many classes in shezar, this should be faster than regrex
    ...
    public static function get_all_versions_hash() {
        ...
        $versions['shezarbuild'] = self::fetch_shezarbuild();
