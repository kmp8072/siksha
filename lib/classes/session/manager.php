...
class manager {
    ...
    protected static function prepare_cookies() {
        ...
        $sessionname = 'shezarSession'.$CFG->sessioncookie;
        ...
        if (isset($_SESSION['USER']->id)) {
            ...
            if (isguestuser($userid) or empty($userid)) {
                ...
                foreach ($authsequence as $authname) {
                    ...
                    // shezar Connect hack - client SSO sessions extend master session.
                    if (\shezar_connect\util::ignore_timeout_hook($_SESSION['USER'], $record->sid, $record->timecreated, $record->timemodified)) {
                        $timeout = false;
                        break;
                    }
                }
        ...
        try {
            ...
            foreach ($rs as $user) {
                foreach ($authplugins as $authplugin) {
                    ...
                    // shezar Connect hack - client SSO sessions extend master session.
                    if (\shezar_connect\util::ignore_timeout_hook($user, $user->sid, $user->s_timecreated, $user->s_timemodified)) {
                        continue;
                    }
                }
