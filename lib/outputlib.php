...
class theme_config {
    ...
    private function __construct($config) {
        ...
        // shezar: all themes must extend the bare bones basic theme.
        if ($this->name !== 'base' and !in_array('base', $this->parents)) {
            error_log("All themes in shezar must extend the 'base' theme, it needs to be added as the last parent theme in {$config->dir}/config.php");
            $this->parents[] = 'base';
        }
    ...
    protected function get_css_files($themedesigner, $rtl = false) {
        ...
        if (is_array($this->sheets)) {
            foreach ($this->sheets as $sheet) {
                ...
                // shezar: Direction-specific stylesheets are detected automatically.
                ...
                // shezar: Check for a RTL sheet first so we can fall back to
    ...
    public function pix_url($imagename, $component) {
        ...
        $params['image'] = ltrim($imagename, '/'); // shezar: always remove leading slashes.
