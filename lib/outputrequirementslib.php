...
class page_requirements_manager {
    ...
    public function __construct() {
        ...
        $this->js_module($this->find_module('core_dndupload')); // shezar: necessary for new forms via ajax

        // shezar needs jQuery - load it on each page.
        $this->jquery();
        ...
        if (defined('BEHAT_SITE_RUNNING') && BEHAT_SITE_RUNNING) {
            if (!is_ajax_request($_SERVER)) {
                // shezar: sometimes shezar loads forms via ajax, we do not need second init there when the html is injected into dialog.
    ...
    /**
     * shezar hack, return the raw AMD JS code for the page.
     *
     ...
     * This only happens within shezar Dialogs presently.
     *
     * DO NOT USE THIS ANYWHERE OTHER THAN THE PAGE RESPONDING FOR A shezar DIALOG.
     *
     * @return string[]
     */
     ...
     *
     * @param bool $initialiseamd initialise AMD. This is probably useful only in shezar forms ajax where we don't want it.
    ...
    public function get_end_code($initialiseamd = true) {
        ...
        if ($initialiseamd) {
            // shezar: ajaxy form fetching does not like AMD.
        ...
        // shezar: kill the jQuery datepicker that only obscures other elements during testing!
        if (defined('BEHAT_SITE_RUNNING')) {
