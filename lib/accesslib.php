...
function role_assign_bulk($roleid, $userids, $contextid, $component = '', $itemid = 0, $timemodified = '') {
    ...
    \shezar_core\event\bulk_role_assignments_started::create_from_context($context)->trigger();
    \shezar_core\event\bulk_role_assignments_ended::create_from_context($context)->trigger();
...
function update_capabilities($component = 'moodle') {
    ...
    foreach ($newcaps as $capname => $capdef) {
        ...
        // shezar: make sure there are no preexisting caps with the same name.
...
function get_component_string($component, $contextlevel) {
    ...
    if ($component === 'moodle' or $component === 'core') {
        switch ($contextlevel) {
            ...
            case CONTEXT_PROGRAM:   return get_string('program', 'shezar_program');
...
function get_role_users($roleid, context $context, $parent = false, $fields = '',
    ...
    // shezar: Moodle had some weird code dealing with multiple roles introduced by MDL-22309 - let's use recordsets instead!
...
function role_get_name(stdClass $role, $context = null, $rolenamedisplay = ROLENAME_ALIAS) {
    ...
    if (trim($role->name) !== '') {
        ...
        switch ($role->shortname) {
            case 'manager':         $original = get_string('sitemanager', 'shezar_core'); break;
...
class context_program extends context {
     ...
     */
    public static function get_level_name() {
        return get_string('program', 'shezar_program');
    }
    ...
    public function get_context_name($withprefix = true, $short = false) {
        ...
        if ($program = $DB->get_record('prog', array('id' => $this->_instanceid))) {
            if ($withprefix){
                $name = get_string('program', 'shezar_program').': ';
            }
            if ($short){
     ...
     */
    public function get_url() {
        return new moodle_url('/shezar/program/view.php', array('id'=>$this->_instanceid));
    }
    ...
    protected static function create_level_instances() {
        ...
        // to shezar and the table hasn't yet been created
