...
if (CLI_SCRIPT) {
    ...
    if (version_compare(phpversion(), '5.5.9') < 0) {
        ...
        echo "shezar 9.0 or later requires at least PHP 5.5.9 (currently using version $phpversion).\n";
...
// shezar: force-disable some settings that are not functional in shezar.
// Always force autoupdates off in shezar.
...
/* Requires for shezar */
require_once($CFG->dirroot .'/shezar/core/shezar.php');// Standard functions used by shezar
shezar_setup();
...
if (!empty($CFG->version) and $CFG->version < 2011120507) {
    if (empty($CFG->shezar_release)) {
        ...
        print_error('error:cannotupgradefrommoodle', 'shezar_core');
    } else {
        // We cannot upgrade from shezar older than v2.2.13.
        print_error('error:cannotupgradefromshezar', 'shezar_core');
    }
    die;
}
...
// shezar: force https-only-access if requested, note you cannot easily disable this setting later!
if (!empty($CFG->stricttransportsecurity)) {
    if (strpos($CFG->wwwroot, 'https:') === 0) {
        header('Strict-Transport-Security: max-age=16070400'); // To be remembered for 186 days.
    }
}
// shezar: prevent embedding of server files in external PDF/Flash.
...
// shezar: hack settings a bit so that we do not have to look for disabled subsystems everywhere.
