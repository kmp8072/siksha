...
function upgrade_stale_php_files_present() {
    ...
    $someexamplesofremovedfiles = array(
        ...
        // Removed in shezar 9.0.
        '/blocks/facetoface/lib.php',
        '/shezar/core/db/pre_any_upgrade.php',
        // Removed in shezar 2.7.
...
/// special cases
    ...
    if ($type === 'shezarcoreonly') {
        $type = 'shezar';
        $plugs = array('core' => realpath("$CFG->dirroot/shezar/core"));
        // Note we can safely run the shezar_core upgrade again later.
    } else {
    ...
    foreach ($plugs as $plug=>$fullplug) {
        ...
        /// Run the upgrade function for the plugin.
            ...
            if (is_readable($fullplug.'/db/shezar_postupgrade.php')) {
                require_once($fullplug.'/db/shezar_postupgrade.php');
                $upgradefunction = 'xmldb_'.$plugin->fullname.'_shezar_postupgrade';
...
function upgrade_plugins_modules($startcallback, $endcallback, $verbose) {
    ...
    foreach ($mods as $mod=>$fullmod) {
        ...
        /// If versions say that we need to upgrade but no upgrade files are available, notify and continue
            ...
            if (is_readable($fullmod.'/db/shezar_postupgrade.php')) {
                require_once($fullmod.'/db/shezar_postupgrade.php');
                $upgradefunction = 'xmldb_'.$module->name.'_shezar_postupgrade';
                $upgradefunction($plugin->version);
            }
...
function upgrade_plugins_blocks($startcallback, $endcallback, $verbose) {
    ...
    foreach ($blocks as $blockname=>$fullblock) {
        ...
        if (empty($installedversion)) { // block not installed yet, so install it
            ...
            if (is_readable($fullblock.'/db/shezar_postupgrade.php')) {
                require_once($fullblock.'/db/shezar_postupgrade.php');
                $upgradefunction = 'xmldb_block_'.$blockname.'_shezar_postupgrade';
                $upgradefunction($plugin->version);
            }
...
function upgrade_started($preinstall=false) {
    global $CFG, $DB, $PAGE, $OUTPUT, $shezar;
    ...
    if ($preinstall) {
        ...
        if (!CLI_SCRIPT and !$PAGE->headerprinted) {
            ...
            $PAGE->set_title($strupgrade.' - shezar '.$shezar->release);
...
function install_core($version, $verbose) {
    ...
    // shezar: Make sure we have the correct versions, ignore the sloppy param.
    $version = null;
    $release = null;
    $branch = null;
    $shezar = new stdClass();
    ...
    try {
        ...
        // shezar: update all version info consistently.
        set_config('release', $release);
        set_config('branch', $branch);
        set_config('shezar_release', $shezar->release);
        set_config('shezar_build', $shezar->build);
        set_config('shezar_version', $shezar->version);
        ...
        \shezar_flavour\helper::execute_post_install_steps();
...
function upgrade_core($version, $verbose) {
    global $CFG, $SITE, $DB, $COURSE, $shezar;
    ...
    require_once($CFG->dirroot.'/shezar/core/db/utils.php'); // Include shezar upgrade stuff.
    ...
    try {
        ...
        // shezar; Run any pre-upgrade special fixes that may be required and update all languages.
        shezar_preupgrade();
...
function upgrade_noncore($verbose) {
    ...
    try {
        ...
        // shezar; Run any pre-upgrade special fixes that may be required.
        shezar_preupgrade();

        $plugintypes = core_component::get_plugin_types();

        // shezar: we give shezar_core preferential treatment, it always gets executed first.
        $plugintypes = array_merge(array('shezarcoreonly' => null), $plugintypes);
        ...
        // Upgrade shezar navigation menu.
        shezar_upgrade_menu();
        // Allow the shezar Flavour to act upon the upgrade if need be.
        \shezar_flavour\helper::execute_post_upgrade_steps();
