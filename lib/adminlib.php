...
/** @var array List of errors */
    ...
    public function __construct($fulltree) {
        ...
        // shezar: Default settings may be overridden from the config.php OR admin settings UI.
        $this->custom_defaults = array();

        // shezar: Load flavour defaults as the base.
        $flavourdefaults = \shezar_flavour\helper::get_defaults_setting();
        ...
        // shezar: Local server defaults may override any flavour recommended defaults.
        ...
        // shezar: Finally override with the enforced flavour setting values,
        // enforced settings cannot be changed via admin UI.
        $enforedsettings = \shezar_flavour\helper::get_enforced_settings();
...
abstract class admin_setting {
    ...
    public function post_write_settings($original) {
        ...
        // shezar: support general callbacks, not just functions.
...
class admin_setting_courselist_frontpage extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        for ($i = 0; $i < count($this->choices) - 1; $i++) {
            ...
            // TL-7745: added label
            $return .= html_writer::tag('label', get_string('positiona', 'shezar_core', $i+1), array('for' => $this->get_id().$i, 'class' => 'accesshide'));
...
function admin_write_settings($formdata) {
    ...
    // shezar: Enforce any of these values, no manual admin changes are allowed.
    $enforcedsettings = \shezar_flavour\helper::get_enforced_settings();
...
function format_admin_setting($setting, $title='', $form='', $description='', $label=true, $warning='', $defaultinfo=NULL, $query='') {
    ...
    $override = '';
    $errorstr = (empty($CFG->cloudconfigoverride)) ? get_string('configoverride', 'admin') : get_string('cloudconfigoverride', 'shezar_core');
    ...
    // shezar: check if the setting is enforced via flavour and prevent any changes if yes.
    $enforcedsetting = '';
    if (empty($override)) {
        $enforcedsettings = \shezar_flavour\helper::get_enforced_settings();
        $plugin = is_null($setting->plugin) ? 'moodle' : $setting->plugin;
        if (isset($enforcedsettings[$plugin][$setting->name])) {
            ...
            $enforcedsetting = '<div class="form-overridden flavourlock">'.get_string('settinglocked', \shezar_flavour\helper::get_active_flavour_component()).'</div>';
            // Add a notice to the title so that it can't be missed.
            $override .= '<div class="form-overridden flavourlock">'.get_string('settinglocked', 'shezar_flavour').'</div>';
