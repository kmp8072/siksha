...
class completion_info {
    ...
    public function update_state($cm, $possibleresult = COMPLETION_UNKNOWN, $userid = 0, $current = null) {
        ...
        if ($newstate != $current->completionstate) {
            ...
            // shezar - Whether or not this was 0 before, it should be 0 now as no reaggregation is necessary after this.
        ...
        if (in_array($newstate, array(COMPLETION_COMPLETE, COMPLETION_COMPLETE_PASS, COMPLETION_COMPLETE_FAIL))) {
            ...
            $event = \shezar_core\event\module_completion::create(
                array(
                    'other' => array(
    ...
    public function delete_course_completion_data_including_rpl() {
        ...
        $DB->delete_records('block_shezar_stats', array('eventtype' => STATS_EVENT_COURSE_STARTED, 'data2' => $this->course_id));
        $DB->delete_records('block_shezar_stats', array('eventtype' => STATS_EVENT_COURSE_COMPLETE, 'data2' => $this->course_id));
    }
    ...
    public function delete_course_completion_data($userid = null) {
        ...
        if (empty($userid)) {
            ...
            $statssql .=
                " AND NOT EXISTS (SELECT 1
                                   FROM {course_completions} cc
                                  WHERE cc.userid = {block_shezar_stats}.userid
                                    AND cc.course = {block_shezar_stats}.data2
                                    AND cc.status = :status)";
            ...
            $statssql .= " AND {block_shezar_stats}.userid = :userid";
        ...
        cm.course=? AND cmc.userid=?", array($this->course->id, $userid));
            ...
            foreach ($modinfo->cms as $othercm) {
                if (array_key_exists($othercm->id, $alldata)) {
                    ...
                    // shezar - Check the criteria in case it should be complete.
    ...
    public static function get_all_courses($userid, $limit=0) {
        ...
        list($visibilitysql, $visibilityparams) = shezar_visibility_where($userid, 'c.id', 'c.visible', 'c.audiencevisible');
    ...
    /**
     * shezar - Checks if a user has some form of completion status in a course.
     *
