...
class coursecat implements renderable, cacheable_object, IteratorAggregate {
        ...
        if ($programs = $DB->get_records('prog', array('category' => $this->id), 'sortorder ASC', 'id, shortname')) {
            ...
            foreach ($programs as $program) {
                ...
                if (!empty($prog->certifid)) {
                    if (!$prog->delete()) {
                        throw new moodle_exception('certifdeletefail', 'shezar_certification', '', $program->shortname);
                    }
                    $deletedcertifs[] = $program;
                } else {
                    if (!$prog->delete()) {
                        throw new moodle_exception('programdeletefail', 'shezar_program', '', $program->shortname);
                    }
    ...
    public function move_content_targets_list() {
        ...
        if (prog_has_programs($this)) {
            $testcaps[] = 'shezar/program:createprogram';
        }
        ...
        if (certif_has_certifications($this)) {
            $testcaps[] = 'shezar/certification:createcertification';
        }
    ...
    public function delete_move($newparentid, $showfeedback = false) {
        ...
        if ($programs = $DB->get_records_select('prog', 'category = ? AND certifid IS NULL', array($this->id), 'sortorder ASC', 'id')) {
            if (!prog_move_programs(array_keys($programs), $newparentid)) {
                if ($showfeedback) {
                    echo $OUTPUT->notification(get_string('error:progsnotmoved', 'shezar_program', $catname));
                }
                return false;
            }
            if ($showfeedback) {
                echo $OUTPUT->notification(get_string('programsmovedout', 'shezar_program', $catname), 'notifysuccess');
            }
        }
        ...
        if ($certifs = $DB->get_records_select('prog', 'category = ? AND certifid IS NOT NULL', array($this->id), 'sortorder ASC', 'id')) {
            if (!prog_move_programs(array_keys($certifs), $newparentid)) {
                if ($showfeedback) {
                    echo $OUTPUT->notification(get_string('error:certifsnotmoved', 'shezar_certification', $catname));
                }
                return false;
            }
            if ($showfeedback) {
                echo $OUTPUT->notification(get_string('certifsmovedout', 'shezar_certification', $catname), 'notifysuccess');
            }
        }
