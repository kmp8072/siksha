...
class behat_base extends Behat\MinkExtension\Context\RawMinkContext {
    ...
    protected function spin($lambda, $args = false, $timeout = false, $exception = false, $microsleep = false) {
        ...
        // shezar: this is the start for timeout calculation.
        $start = time();
        ...
        for ($i = 0; $i < $loops; $i++) {
            // shezar: run at least once, but then use the total time spent here for timeout calculation.
            if ($i != 0) {
    ...
    protected function resize_window($windowsize, $viewport = false) {
        ...
        // shezar: Firefox is notoriously bad with scrolling these days, use a taller standard screen.
        //         This resolves problems with clicking of things that are close to the bottom end of page,
        //         also it somehow prevents shezar menu from popping up randomly.
    ...
    public function wait_for_pending_js() {
        ...
        // shezar: this is the start for timeout calculations.
        $start = time();
        ...
        for ($i = 0; $i < self::EXTENDED_TIMEOUT * 10; $i++) {
            ...
            try {
                ...
                // shezar: this is a common problem in Chrome, the JS just stops executing with long timeouts.
                $pending = 'timeout';
    ...
    protected function execute($contextapi, $params = array()) {
        ...
        // shezar: make sure the thing actually exists!
        ...
        // shezar: we want to know if there are some steps that were not properly converted!
        if (is_array($result)) {
