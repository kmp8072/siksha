...
function file_save_draft_area_files($draftitemid, $contextid, $component, $filearea, $itemid, array $options=null, $text=null, $forcehttps=false) {
    ...
    // shezar:  allow array of files instead of draftitemid
 ...
 */
function shezar_tweak_file_sending($mimetype, $forcedownload) {
    if (!$forcedownload) {
        return $mimetype;
    }
...
function send_file($path, $filename, $lifetime = null , $filter=0, $pathisstring=false, $forcedownload=false, $mimetype='', $dontdie=false) {
    ...
    // shezar: improve file serving security if possible.
    $mimetype = shezar_tweak_file_sending($mimetype, $forcedownload);
...
class curl {
    ...
    public function resetopt() {
        $this->options = array();
        $this->options['CURLOPT_USERAGENT']         = 'shezarBot/1.0';
