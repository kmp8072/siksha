...
class behat_general extends behat_base {
    ...
    public function switch_to_window($windowname) {
        ...
        // shezar: do not rely on tags, force browser restart here!
        behat_hooks::$forcerestart = true;

        // shezar: this sleep is mega super important, if we do not do it Chrome may get stuck randomly!
        sleep(1);
    ...
    /**
     * shezar: Work around missing support for alert confirmation in some Selenium browsers.
     *
    ...
    protected function download_file_from_link($link) {
        ...
        $session = $this->getSession()->getCookie('shezarSession');
        return download_file_content($url, array('Cookie' => 'shezarSession=' . $session));
    }
