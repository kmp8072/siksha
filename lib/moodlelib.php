...
/**
 * HOMEPAGE_MY was removed in shezar 9, it is
 * replaced with a default shezar dashboard.
 * @deprecated
 */
define('HOMEPAGE_MY', 1);
/**
 * HOMEPAGE_USER was removed in shezar 9, it is
...
/**
 * The home page shold be shezar dashboard
 */
define('HOMEPAGE_shezar_DASHBOARD', 105);
 ...
 *
 * @param int $date the timestamp - since Moodle 2.9 and shezar 2.7 this is a real UTC timestamp
...
function date_format_string($date, $format, $tz = 99) {
    ...
    // shezar: fix all broken % format strings first, we need to work with ascii chars only here.
    ...
    if (stristr(PHP_OS, 'darwin')) {
        ...
        // shezar: calendars have absolutely nothing to do with current Windows encoding!!!
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    // shezar: This script cannot be returned to, no matter when require_login is called we do not ever want to return to the
    // current script.
    ...
    if ($course->id == SITEID) {
        ...
        if (is_role_switched($course->id)) {
            ...
            if (!shezar_course_is_viewable($course->id)) {
    ...
    // shezar: If completion is enabled and mark as started on first view is on
...
function delete_user(stdClass $user) {
    ...
    // Legacy shezar user deleting - it is now recommended to suspend users instead.
 ...
 *
 * NOTE: this is a shezar feature, it does not work with accounts deleted from Moodle,
...
function undelete_user($user) {
    ...
    \shezar_core\event\user_undeleted::create_from_user($newuser)->trigger();
...
function remove_course_contents($courseid, $showfeedback = true, array $options = null) {
    ...
    $cc->clear_criteria();
    // shezar: disable completion in activities, we are going to drop data later anyway, this prevents bogus completion updates during deleting.
    ...
    // shezar - The current completion records have been cleared, but we need to clean up historical completions as well.
    ...
    $cleanuplugintypes = array('report', 'coursereport', 'format', 'shezar');
    ...
    // shezar - Delete course reminders.
    if (delete_reminders($course->id) === true && $showfeedback) {
        echo $OUTPUT->notification($strdeleted.get_string('remindersmenuitem', 'shezar_coursecatalog'), 'notifysuccess');
    }
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    // shezar: RTL hack that is not in upstream...
    ...
    // shezar: make sure developers included all necessary fields in $user object.
    if ($user->id > 0) {
    ...
    if ((isset($user->auth) && $user->auth=='nologin') or (isset($user->suspended) && $user->suspended)) {
        // shezar: this does not work properly without the hack above because devs keep forgetting to add these fields.
        return true;
    }
    ...
    if ($attachment && $attachname) {
        if (preg_match( "~\\.\\.~" , $attachment )) {
            ...
            // shezar: make sure the file exists and add ical data if ics file specified as attachment.
...
function setnew_password_and_mail($user, $fasthash = false) {
    ...
    $a = new stdClass();
    // shezar: Keep the firstname because it is used in language packs.
    $a->firstname   = fullname($user, true);
    $a->fullname    = $a->firstname;
    // shezar: New first_name is intended for language pack customisations.
...
function remove_dir($dir, $contentonly=false) {
    ...
    // shezar: This is our version of remove_dir, its much safer than Moodle's version.
...
function setup_lang_from_browser() {
    ...
    // shezar: Safari sends only one full lang name - add the basic form without dash.
    if (count($langs) === 1) {
        $lang = reset($langs);
        $upos = strpos($lang, '_');
        if ($upos !== false) {
 ...
 *
 * @return int HOMEPAGE_shezar_DASHBOARD or HOMEPAGE_SITE
 */
function get_home_page() {
    ...
    if (empty($CFG->allowdefaultpageselection)) {
        ...
        return HOMEPAGE_shezar_DASHBOARD;
    }
    ...
    if (count(shezar_dashboard::get_user_dashboards($USER->id))) {
        return HOMEPAGE_shezar_DASHBOARD;
    }
