...
$device_token = optional_param('device_token', '', PARAM_RAW);


 $DB->execute("UPDATE {user} SET device_token='$device_token' WHERE username='$username'");
...
if (!empty($user)) {
    ...
    if (count($tokens) > 0) {
        ...
        if ( ($serviceshortname == MOODLE_OFFICIAL_MOBILE_SERVICE and has_capability('moodle/webservice:createmobiletoken', context_system::instance()))
            ...
            // Create a new token.
            $token = new stdClass;
            $token->token = shezar_core_generate_unique_db_value('external_tokens', 'token');
    ...
    $usertoken->token = $token->token;
     ...
     if(!empty($usertoken)){
    if (!$DB->record_exists('download_app_user',array('userid'=>$user->id))) {
              $record = new object();
           $record->userid =$user->id;
         //  $currenttimestamp=now();
          $record->created_date=time();
          $DB->insert_record('download_app_user', $record);
         }
    }
