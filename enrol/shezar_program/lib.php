...
class enrol_shezar_program_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if ($DB->record_exists('enrol', array('courseid' => $courseid, 'enrol' => 'shezar_program'))) {
            return NULL;
        }

        return new moodle_url('/enrol/shezar_program/addinstance.php', array('sesskey' => sesskey(), 'id' => $courseid));
    }
    ...
    public function can_delete_instance($instance) {
        if (!shezar_feature_visible('programs')) {
            ...
            return has_capability('enrol/shezar_program:unenrol', $context);
        }
    ...
    /**
     * Add new instance of enrol_shezar_program plugin.
     * @param object $course
     ...
     *
     * @param stdClass instance of the enrol_shezar_program class for a particular course
            ...
            case ENROL_EXT_REMOVED_SUSPENDNOROLES:
                foreach ($active_enrolments as $userid) {
                    ...
                    if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                        role_unassign_all(array('userid' => $userid, 'contextid' => $context->id, 'component' => 'enrol_shezar_program', 'itemid' => $instance->id));
                    }
                }
                break;
            default:
    ...
    public function get_instance_for_course($courseid) {
        global $DB;
        return $DB->get_record('enrol', array('enrol' => 'shezar_program', 'courseid' => $courseid));
    }
        ...
        if ($course = $DB->get_record('course', array('id' => $instance->courseid))) {
            ...
            if ($result->enroled) {
                ...
                if ($result->notify) {
                    ...
                    shezar_set_notification(get_string('nowenrolled', 'enrol_shezar_program', $a), $viewurl->out(), array('class' => 'notifysuccess'));
                }
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/shezar_program:unenrol", $context)) {
    ...
    public function restore_instance(restore_enrolments_structure_step $step, stdClass $data, $course, $oldid) {
        global $DB;
        // There is only one shezar_program enrol instance allowed per course.
 ...
 */
function enrol_shezar_program_supports($feature) {
    switch($feature) {
