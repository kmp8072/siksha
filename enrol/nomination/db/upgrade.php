...
function xmldb_enrol_nomination_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2016012801) {
        ...
        $table = new xmldb_table('enrol_nomination_applicationinfo');
        ...
        upgrade_plugin_savepoint(true, 2016012801, 'enrol', 'nomination');
    }

    if ($oldversion < 2016042202) {
        // Invert settings for showing standard and extra user profile fields.
        $enrolnomination = enrol_get_plugin('nomination');
        $showstandarduserprofile = $enrolnomination->get_config('show_standard_user_profile') == 0 ? true : false;
        $enrolnomination->set_config('show_standard_user_profile', $showstandarduserprofile);
        $showextrauserprofile = $enrolnomination->get_config('show_extra_user_profile') == 0 ? true : false;
        $enrolnomination->set_config('show_extra_user_profile', $showextrauserprofile);

        $instances = $DB->get_records('enrol', array('enrol' => 'nomination'));
    ...
    if ($oldversion < 2016060803) {
        ...
        $sendmailtoteacher = $enrolnomination->get_config('sendmailtoteacher');
        $notifycoursebased = $sendmailtoteacher;
        $enrolnomination->set_config('notifycoursebased', $notifycoursebased);
        $enrolnomination->set_config('sendmailtoteacher', null);

        $sendmailtomanager = $enrolnomination->get_config('sendmailtomanager');
        $notifyglobal = $sendmailtomanager ? '$@ALL@$' : '';
        $enrolnomination->set_config('notifyglobal', $notifyglobal);
        $enrolnomination->set_config('sendmailtomanager', null);
