...
require_once($CFG->dirroot . '/enrol/shezar_facetoface/externallib.php');

class enrol_shezar_facetoface_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_instance_info() {
        ...
        // Check if shezar_facetoface enrolment plugin is enabled.
        ...
        $instanceinfo1 = enrol_shezar_facetoface_external::get_instance_info($instanceid1);

        $this->assertEquals($instanceid1, $instanceinfo1['id']);
        $this->assertEquals($course->id, $instanceinfo1['courseid']);
        $this->assertEquals('shezar_facetoface', $instanceinfo1['type']);
        $this->assertEquals('Test instance 1', $instanceinfo1['name']);
        $this->assertTrue($instanceinfo1['status']);

        $instanceinfo2 = enrol_shezar_facetoface_external::get_instance_info($instanceid2);
        $this->assertEquals($instanceid2, $instanceinfo2['id']);
        $this->assertEquals($course->id, $instanceinfo2['courseid']);
        $this->assertEquals('shezar_facetoface', $instanceinfo2['type']);
        $this->assertEquals('Test instance 2', $instanceinfo2['name']);
        $this->assertEquals(get_string('cannotenrol', 'enrol_shezar_facetoface'), $instanceinfo2['status']);

        $instanceinfo3 = enrol_shezar_facetoface_external::get_instance_info($instanceid3);
        $this->assertEquals($instanceid3, $instanceinfo3['id']);
        $this->assertEquals($course->id, $instanceinfo3['courseid']);
        $this->assertEquals('shezar_facetoface', $instanceinfo3['type']);
