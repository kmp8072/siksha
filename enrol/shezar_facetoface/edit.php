...
require_once($CFG->dirroot . '/enrol/shezar_facetoface/edit_form.php');
...
require_capability('enrol/shezar_facetoface:config', $context);

$PAGE->set_url('/enrol/shezar_facetoface/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'shezar_facetoface', 'id'=>$instanceid), '*', MUST_EXIST);
} else {
...
$mform = new enrol_shezar_facetoface_edit_form(null, array($instance, $plugin, $context));
...
$PAGE->set_title(get_string('pluginname', 'enrol_shezar_facetoface'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_shezar_facetoface'));
$mform->display();
echo $OUTPUT->footer();
