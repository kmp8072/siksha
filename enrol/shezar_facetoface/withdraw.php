...
$enrol = $DB->get_record('enrol', array('id' => $eid, 'enrol' => 'shezar_facetoface'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id' => $enrol->courseid), '*', MUST_EXIST);
$shezar_facetoface = new enrol_shezar_facetoface_plugin();
$baseurl = new moodle_url('/enrol/shezar_facetoface/withdraw.php', array('eid' => $eid));
...
if ($confirm) {
    ...
    $DB->delete_records('enrol_shezar_f2f_pending', array('enrolid' => $eid, 'userid' => $USER->id));
...
echo $OUTPUT->heading(get_string('withdrawconfifm', 'enrol_shezar_facetoface'), 3);
