...
class enrol_shezar_facetoface_signup_form extends moodleform {
    protected $instance;
    protected $toomany = false;

    /**
     * Overriding this function to get unique form id for multiple shezar_facetoface enrolments.
     *
     ...
     */
    private function add_signup_elements($mform, $instance, $shezar_facetoface) {
        global $DB, $OUTPUT, $USER;

        $courseid = $instance->courseid;

        $settingautosignup = enrol_shezar_facetoface_plugin::SETTING_AUTOSIGNUP;

        $sessions = $shezar_facetoface->get_enrolable_sessions($courseid);

        if (empty($sessions)) {
            $mform->addElement('static', 'managermissing', get_string('managermissingallsessions', 'enrol_shezar_facetoface'));
        ...
        if (!empty($instance->$settingautosignup)) {
            ...
            if ($shezar_facetoface->sessions_require_manager()) {
                $mform->addElement('static', 'managermissing', get_string('managermissingsomesessions', 'enrol_shezar_facetoface'));
            }

            $mform->addElement('static', 'signuptoenrol', get_string('signuptoenrol', 'enrol_shezar_facetoface'));
            ...
            foreach ($sessrows as $facetofaceid => $sessions) {
                ...
                $activejobassigns = \shezar_job\job_assignment::get_all($USER->id, $managerreqd);
                if ($facetoface->forceselectjobassignment && empty($activejobassigns)) {
                    ...
                    $this->enrol_shezar_facetoface_addsessrows($mform, $sessions, $facetoface, $force);
                    $sessionsavailable = true;
                }
     ...
     */
    private function enrol_shezar_facetoface_addsessrows($mform, $sessions, $facetoface, $force = false) {
        global $DB, $PAGE;
        ...
        $mform->addElement('html', html_writer::tag('th', get_string('selectsession', 'enrol_shezar_facetoface'))); // No title for radio button col.
        ...
        $mform->addElement('html', html_writer::tag('th', get_string('additionalinformation', 'enrol_shezar_facetoface')));
        ...
        if (!$force) {
            ...
            $mform->addElement('html', get_string('donotsignup', 'enrol_shezar_facetoface'));
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['sid'])) {
            ...
            if ($nosid) {
                $errors['signuperrorcontainer'] = get_string('error:choosession', 'enrol_shezar_facetoface');
            }
        }
        return $errors;
    }
