 ...
 * @package shezar
 * @subpackage enrol_shezar_learningplan
 */

defined('MOODLE_INTERNAL') || die();

class enrol_shezar_learningplan_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if ($DB->record_exists('enrol', array('courseid' => $courseid, 'enrol' => 'shezar_learningplan'))) {
            return NULL;
        }

        return new moodle_url('/enrol/shezar_learningplan/addinstance.php', array('sesskey' => sesskey(), 'id' => $courseid));
    }
    ...
    public function can_delete_instance($instance) {
        if (!shezar_feature_visible('learningplans')) {
            ...
            return has_capability('enrol/shezar_learningplan:unenrol', $context);
        }
    ...
    /**
     * Add new instance of enrol_shezar_learningplan plugin.
     * @param object $course
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if ($this->is_user_approved($instance->courseid)) {
            ...
            shezar_set_notification($OUTPUT->container(get_string('nowenrolled', 'enrol_shezar_learningplan', $course->fullname), 'plan_box'), null, array('class' => 'notifysuccess'));
        } else {
            // this isn't an approved course in their learning plan or learning plan isn't approved
            $link = $OUTPUT->action_link(new moodle_url('/shezar/plan/index.php', array('userid' => $USER->id)), get_string('learningplan', 'enrol_shezar_learningplan'));
            $form = get_string('notpermitted', 'enrol_shezar_learningplan', $link);

            if (!empty($course->guest)) {
                $destination = new moodle_url('/course/view.php', array('id' => $course->id));
                $form .= get_string('guestaccess', 'enrol_shezar_learningplan', $destination);
            } else {
    ...
    public function try_autoenrol(stdClass $instance) {
        ...
        if ($this->is_user_approved($instance->courseid)) {
            ...
            shezar_set_notification(get_string('nowenrolled', 'enrol_shezar_learningplan', $course->fullname), $viewurl->out(), array('class' => 'notifysuccess'));
        }
        return false;
    }
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/shezar_learningplan:unenrol", $context)) {
