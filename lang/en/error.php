...
$string['backupcontainexternal'] = 'This backup file contains external shezar Network Hosts that are not configured locally';
...
//$string['cannotdownloadlanguageupdatelist'] = 'Cannot download list of language updates from download.shezarlms.com'; // Not used in shezar.
...
$string['pluginrequirementsnotmet'] = 'Plugin "{$a->pluginname}" ({$a->pluginversion}) could not be installed.  It requires a newer version of shezar (currently you are using {$a->currentmoodle}, you need {$a->requiremoodle}).';
...
$string['unicodeupgradeerror'] = 'Sorry, but your database is not already in Unicode, and this version of shezar is not able to migrate your database to Unicode.  Please upgrade to shezar 1.7.x first and perform the Unicode migration from the Admin page.  After that is done you should be able to migrate to shezar {$a}';
...
$string['upgraderequires19'] = 'Error: New shezar version was installed on server, unfortunately upgrade from the previous version is not supported.<br />Please upgrade first to latest 1.9.x release. You can also return to previous version by reinstalling original files.';
