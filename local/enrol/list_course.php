...
?>
<body>
	<h2>List Of Enrolment Request By <?php echo $manager_name->firstname." ".$manager_name->lastname; ?> </h2>
	<?php
		$select = $DB->get_records_sql("SELECT * FROM {request_to_enrol} WHERE status = '1' AND manager_id = ".$mid." group by course_id");
		foreach ($select as $key => $value) {
			$mid = $value->manager_id;
			$course_id = $value->course_id;
			
			$course_name = $DB->get_record_sql("SELECT fullname FROM {course} WHERE id = ".$course_id."");
			
			echo "<div style = 'border: 1px solid;padding: 10px'>";
   			echo "Course Name : ".$course_name->fullname."<br>";
    		echo "<a href='detail.php?cid=".$course_id."&mid=".$mid."'>Details</a></div><br>";
