...
class login_signup_form extends moodleform {
    function definition() {
        ...
        if ($requirenojs) {
            if (!$nojs) {
                $mform->addElement('html', html_writer::tag('noscript', html_writer::tag('p', get_string('formrequiresjs', 'shezar_hierarchy') .
                    html_writer::link(new moodle_url(qualified_me(), array('nojs' => '1')), get_string('clickfornonjsform', 'shezar_hierarchy')))));
            }
        }
    ...
    function validation($data, $files) {
        ...
        // shezar: We need to validate that managerid is correct for the managerjaid specified.
        if (get_config('shezar_job', 'allowsignupmanager')) {
            if (!empty($data['managerjaid'])) {
                if (empty($data['managerid'])) {
                    // Something's wrong. Advise the user to try again or select a manager later.
                    $errors['managerselector'] = get_string('managernomatchja', 'shezar_job');
                } else {
                    $managerja = \shezar_job\job_assignment::get_with_id($data['managerjaid']);
