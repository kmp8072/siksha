...
class profile_field_base {
    ...
    /**
     * shezar-specific function.
     * Does some extra pre-processing for shezar sync uploads.
     ...
     */
    public function shezar_sync_data_preprocess($itemnew) {
        return $itemnew;
    }
...
/**
 * shezar: Saves job assignment data specified from a self-registration.
 *
...
function position_save_data($userprofile) {
    ...
    $allowsignupposition = get_config('shezar_job', 'allowsignupposition');
    $allowsignuporganisation = get_config('shezar_job', 'allowsignuporganisation');
    $allowsignupmanager = get_config('shezar_job', 'allowsignupmanager');
    ...
    $jobassignment = \shezar_job\job_assignment::get_first($userprofile->id, false);
    if (empty($jobassignment)) {
        $data->userid = $userprofile->id;
        \shezar_job\job_assignment::create_default($data->userid, $data);
    } else {
...
function profile_display_hierarchy_fields($userid) {
    ...
    if (isset($record->pos)) {
        echo html_writer::tag('dt', get_string('position', 'shezar_job'));
        echo html_writer::tag('dd', $record->pos);
    }

    if (isset($record->org)) {
        echo html_writer::tag('dt', get_string('organisation', 'shezar_job'));
    ...
    if (isset($record->manid)) {
        ...
        echo html_writer::tag('dt', get_string('manager', 'shezar_job'));
 ...
 * @param stdClass $usernew
 * @param boolean $sync     shezar - Whether this is being called from sync and needs pre-preprocessing.
 */
function profile_save_data($usernew, $sync = false) {
    ...
    if ($fields = $DB->get_records('user_info_field')) {
        foreach ($fields as $field) {
            ...
            $formfield = new $newfield($field->id, $usernew->id);
            if ($sync) {
                $usernew = $formfield->shezar_sync_data_preprocess($usernew);
            }
...
function profile_signup_position($mform, $nojs, $positionid = 0) {
    ...
    if ($nojs) {
        ...
        if (is_array($allpositions) && !empty($allpositions)) {
            $mform->addElement('select', 'positionid', get_string('chooseposition', 'shezar_job'),
                array(0 => get_string('chooseposition', 'shezar_job')) + $allpositions);
        } else {
            $mform->addElement('static', 'positionid', get_string('chooseposition', 'shezar_job'), get_string('noposition', 'shezar_job') );
        }
        $mform->addHelpButton('positionid', 'chooseposition', 'shezar_job');
    } else {
        ...
        $mform->addElement('static', 'positionselector', get_string('position', 'shezar_job'),
            ...
            html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseposition', 'shezar_job'), 'id' => 'show-position-dialog'))
        );
...
function profile_signup_organisation($mform, $nojs, $organisationid = 0) {
    ...
    if ($nojs) {
        ...
        if (is_array($allorgs) && !empty($allorgs)) {
            $mform->addElement('select', 'organisationid', get_string('chooseorganisation', 'shezar_job'),
                array(0 => get_string('chooseorganisation', 'shezar_job')) + $allorgs);
        } else {
            $mform->addElement('static', 'organisationid', get_string('chooseorganisation', 'shezar_job'), get_string('noorganisation', 'shezar_job') );
        }
        $mform->addHelpButton('organisationid', 'chooseorganisation', 'shezar_job');
    } else {
        ...
        $mform->addElement('static', 'organisationselector', get_string('organisation', 'shezar_job'),
            ...
            html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('chooseorganisation', 'shezar_job'), 'id' => 'show-organisation-dialog'))
        );
...
function profile_signup_manager($mform, $nojs, $managerjaid = 0) {
    ...
    if ($managerjaid) {
        ...
        $managername = shezar_job_display_user_job($manager, $managerja, $canviewemail);
    }
    ...
    if ($nojs) {
        ...
        if (is_array($allmanagers) && !empty($allmanagers)) {
            // Manager.
            $mform->addElement('select', 'managerid', get_string('choosemanager', 'shezar_job'),
                array(0 => get_string('choosemanager', 'shezar_job')) + $allmanagers);
            $mform->setType('managerid', PARAM_INT);
        } else {
            $mform->addElement('static', 'managerid', get_string('choosemanager', 'shezar_job'),
                get_string('error:dialognotreeitems', 'shezar_core'));

        }
        $mform->addHelpButton('managerid', 'choosemanager', 'shezar_job');
    } else {
        ...
        $mform->addElement(
            ...
            html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('choosemanager', 'shezar_job'), 'id' => 'show-manager-dialog'))
        );
