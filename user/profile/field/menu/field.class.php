...
class profile_field_menu extends profile_field_base {
    ...
    public function __construct($fieldid = 0, $userid = 0) {
        ...
        // shezar - changed to always use choosedots as it never makes sense to blindly save the first value.
        $this->options[''] = get_string('choosedots');
        foreach ($options as $key => $option) {
            $this->options[$key] = format_string($option); // Multilang formatting.
        }

        // TODO: TL-7042 decide if we want MDL-43205 in shezar 2.9.0
    ...
    public function shezar_sync_data_preprocess($itemnew) {
        ...
        foreach ($this->options as $key => $option) {
            // shezar Sync matches with case insensitivity, do the same in sync preprocess for consistency.
