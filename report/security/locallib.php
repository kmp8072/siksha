...
function report_security_check_cookiesecure($detailed=false) {
    ...
    // shezar: show this always, not just on HTTPS sites.
...
function report_security_check_riskxss($detailed=false) {
    ...
    if ($count === 0) {
        // shezar: no users means no warning, this is good for new installs.
        $result->status = REPORT_SECURITY_OK;
    }
...
/**
 * Verifies if https used in shezar.
 *
 * @param bool $detailed
...
/**
 * Verifies if URL downloader repository enabled in shezar.
 *
 * @param bool $detailed
 * @return stdClass result
 */
