...
Some tool providers support reporting grades back to shezar based on actions taken within the tool, creating a more integrated
experience.';
...
Some tool providers support reporting grades back to shezar based on actions taken within the tool, creating a more integrated experience.
...
$string['allowsetting'] = 'Allow tool to store 8K of settings in shezar';
...
real estate to the tool, and others provide a more integrated feel with the shezar environment.

* **Default** - Use the launch container specified by the tool configuration.
* **Embed** - The tool is displayed within the existing shezar window, in a manner similar to most other Activity types.
* **Embed, without blocks** - The tool is displayed within the existing shezar window, with just the navigation controls
...
$string['errormisconfig'] = 'Misconfigured tool. Please ask your shezar administrator to fix the configuration of the tool.';
...
$string['external_tool_type_help'] = 'The main purpose of a tool configuration is to set up a secure communication channel between shezar and the tool provider.
It also provides an opportunity for configuration defaults and setting up additional services provided by the tool.

* **Automatic, based on Launch URL** - This setting should be used in almost all cases. shezar will select the most appropriate tool configuration
       based on the Launch URL. Tools configured by both an administrator or within this course will be used.
       When the Launch URL is specified, shezar will provide feedback on whether it recognizes it or not. If shezar does not recognize the Launch URL,
       you may need to enter the tool configuration details manually.
* **A specific tool type** - By selecting a specific tool type, you can force shezar to use that tool configuration when communicating with the
...
$string['failedtoconnect'] = 'shezar was unable to communicate with the "{$a}" system';
...
If using this option, confirm that this shezar site and the tool provider support SSL.';
...
$string['launch_in_moodle'] = 'Launch tool in shezar';
...
$string['main_admin_help'] = 'External tools allow shezar users to seamlessly interact with learning resources hosted remotely. Through a special
...
* **Active** - These tool providers have been approved and configured by an administrator. They can be used from within any
        course on this shezar instance. If a consumer key and shared secret are entered, a trust relationship is established
        between this shezar instance and the remote tool, providing a secure communication channel.
...
* **Rejected** - These tools providers are flagged as ones which an administrator has no intention of making available to the entire
        shezar instance. Teachers may still use tools from these providers if they have a consumer key and shared secret, or if none is required.';
...
$string['module_class_type'] = 'shezar module type';
...
$string['notypes'] = 'There are currently no LTI tools set up in shezar. Click the Install link above to add some.';
...
$string['organizationid_help'] = 'A unique identifier for this shezar instance. Typically, the DNS name of the organization is used.

If this field is left blank, the host name of this shezar site will be used as the default value.';
$string['organizationurl'] = 'Organization URL';
$string['organizationurl_help'] = 'The base URL of this shezar instance.
...
Tools which do not require secure communication from shezar and do not provide additional services (such as grade reporting)
...
It can be used by the tool provider to uniquely identify the shezar site from which users launch into the tool.
...
$string['secure_icon_url_help'] = 'Similar to the icon URL, but used if the user accessing shezar securely through SSL. The main purpose for this field is to prevent
...
$string['secure_launch_url_help'] = 'Similar to Launch URL, but used instead of the launch url if high security is required. shezar will use the
secure launch URL instead of the launch URL if the shezar site is accessed through SSL, or if the tool configuration
...
$string['typename_help'] = 'The tool name is used to identify the tool provider within shezar. The name entered will be visible
