...
if ($facetoface->approvaltype == APPROVAL_MANAGER || $facetoface->approvaltype == APPROVAL_ADMIN) {
    ...
    $staffids = \shezar_job\job_assignment::get_staff_userids($USER->id);
    if (!empty($staffids)) {
...
if ($action == 'cancellations') {
    ...
    // Get list of actions
    $exports = array(
        'exportxls'     => get_string('exportxls', 'shezar_reportbuilder'),
        'exportods'     => get_string('exportods', 'shezar_reportbuilder'),
        'exportcsv'     => get_string('exportcsv', 'shezar_reportbuilder')
    );
...
if ($form = data_submitted()) {
    ...
    if ($action == 'approvalrequired' && !empty($form->requests)) {
        ...
        if (($result = facetoface_approve_requests($form)) === true) {
            ...
            shezar_set_notification($output);
        }
        redirect($return);
        die();
    }
    ...
    if ($action == 'takeattendance' && $takeattendance) {
        if (facetoface_take_attendance($form)) {
            ...
            shezar_set_notification(get_string('updateattendeessuccessful', 'facetoface'), $return,
                    array('class' => 'notifysuccess'));
        }
        shezar_set_notification(get_string('error:takeattendance', 'facetoface'), $return, array('class' => 'notifyproblem'));
    }
            ...
            foreach ($recipients as $recipient) {
                ...
                if (email_to_user($recipient, $facetofaceuser, $data->subject, $bodyplain, $body) === true) {
                    ...
                    if (!empty($recipient->jobassignmentid)) {
                        $ja = \shezar_job\job_assignment::get_with_id($recipient->jobassignmentid);
...
if (!$onlycontent) {
    ...
    if ($action == 'messageusers') {
        ...
        $jsmodule = array(
            'name' => 'shezar_f2f_attendees_message',
            'fullpath' => '/mod/facetoface/attendees_messaging.js',
            'requires' => array('json', 'shezar_core'));

        $PAGE->requires->js_init_call('M.shezar_f2f_attendees_messaging.init', $args, false, $jsmodule);
    } else {
        $jsmodule = array(
            'name' => 'shezar_f2f_attendees',
            'fullpath' => '/mod/facetoface/attendees.js',
        ...
        $PAGE->requires->js_init_call('M.shezar_f2f_attendees.init', $args, false, $jsmodule);
    }
...
if ($show_table) {
    ...
    if ($action == 'attendees') {
        ...
        $table = new shezar_table('facetoface-attendees');
    ...
    if (has_any_capability(array('mod/facetoface:addattendees', 'mod/facetoface:removeattendees'), $context)) {
        ...
        if ($exports) {
            ...
            // Action selector.
            echo html_writer::label(get_string('attendeeactions', 'mod_facetoface'), 'f2f-actions', true, array('class' => 'sr-only'));
            echo html_writer::select($exports, 'f2f-actions', '', array('' => get_string('export', 'shezar_reportbuilder')));
            echo $OUTPUT->container_end();
        }
    }
}
