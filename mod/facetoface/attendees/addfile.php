...
if ($formdata = $mform->get_data()) {
        ...
        while ($signup = $cir->next()) {
            ...
            if ($facetoface->selectjobassignmentonsignup) {
                if (!empty($data['jobassignmentidnumber'])) {
                    try {
                        $jobassignment = \shezar_job\job_assignment::get_with_idnumber($user->id, $data['jobassignmentidnumber'], true);
    ...
    if (!empty($errors)) {
        ...
        foreach ($errors as $error) {
            shezar_set_notification($error, null, array('class' => 'notifyproblem'));
        }
    } else {
