...
M.shezar_f2f_attendees = M.shezar_f2f_attendees || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_f2f_attendees.init()-> jQuery dependency required for this module to function.');
        }
        ...
        var notsetoption = M.shezar_f2f_attendees.config.notsetop.toString();
        ...
        shezarDialog_handler_form.prototype._updatePage = function(response) {
            ...
            if (M.shezar_f2f_attendees.config.approvalreqd == "1") {
                if ($('<div></div>').append(newtable).find('div.addedattendees').length > 0) {
                    ...
                    if (tab.length > 0) {
                        ...
                        tab.parent('a').attr("href", M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' + M.shezar_f2f_attendees.config.sessionid + '&action=approvalrequired');
            ...
            M.shezar_f2f_attendees.attachCustomClickEvents();
        ...
        (function() {
            ...
            shezarDialogs[name] = new shezarDialog(
                ...
                M.cfg.wwwroot + '/mod/facetoface/attendees/bulkadd_results.php?s=' + M.shezar_f2f_attendees.config.sessionid,
                handler
                );
        })();
        ...
        $('select#menubulk_select').change(function() {
            ...
            switch(selected) {
                case M.shezar_f2f_attendees.config.selectall.toString():
                    $('.selectedcheckboxes').prop('checked', true);
                    break;
                case M.shezar_f2f_attendees.config.selectnone.toString():
                    $('.selectedcheckboxes').prop('checked', false);
                    break;
                case M.shezar_f2f_attendees.config.selectset.toString():
                    mark_set_unset(notsetoption, 'NE');
                    break;
                case M.shezar_f2f_attendees.config.selectnotset.toString():
        ...
        */
        ...
        $(document).on('change', 'select#menuf2f-actions', function() {
            ...
            var data = {
                submitbutton: "1",
                ajax: "1",
                sesskey: M.shezar_f2f_attendees.config.sesskey
            };
            ...
            switch (current) {
                case "add":
                    window.location.href = M.cfg.wwwroot + '/mod/facetoface/attendees/add.php?s=' + M.shezar_f2f_attendees.config.sessionid;
                    break;
                case "bulkaddfile":
                    window.location.href = M.cfg.wwwroot + '/mod/facetoface/attendees/addfile.php?s=' + M.shezar_f2f_attendees.config.sessionid;
                    break;
                case "bulkaddinput":
                    window.location.href = M.cfg.wwwroot + '/mod/facetoface/attendees/addlist.php?s=' + M.shezar_f2f_attendees.config.sessionid;
                    break;
                case "remove":
                    window.location.href = M.cfg.wwwroot + '/mod/facetoface/attendees/remove.php?s=' + M.shezar_f2f_attendees.config.sessionid;
                    break;
            }
            ...
            if (current.substr(0, 6) == "export") {
                var url = M.cfg.wwwroot + '/mod/facetoface/attendees.php?s=' + M.shezar_f2f_attendees.config.sessionid + '&action=' + M.shezar_f2f_attendees.config.action + '&download=';
        ...
        function update_waitlist(action, updateusers, checked) {
            ...
            function do_post() {
                Y.io(M.cfg.wwwroot + '/mod/facetoface/updatewaitlist.php', {
                    data: {
                        courseid: M.shezar_f2f_attendees.config.courseid,
                        sessionid: M.shezar_f2f_attendees.config.sessionid,
                        context: this,
                        action: action,
