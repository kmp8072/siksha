...
function facetoface_add_instance($facetoface) {
    ...
    if (!empty($missingtemplates)) {
        ...
        shezar_set_notification($message);
    }
...
function facetoface_notify_under_capacity() {
    ...
    foreach ($tocheck as $session) {
        ...
        $eventdata = (object)array(
            ...
            'msgtype' => shezar_MSG_TYPE_FACE2FACE,
            'msgstatus' => shezar_MSG_STATUS_NOTOK,
            'urgency' => shezar_MSG_URGENCY_NORMAL,
            'sendmail' => shezar_MSG_EMAIL_YES,
        );
...
function facetoface_is_session_in_progress($session, $timenow) {
    ...
    $startedsessions = shezar_search_for_value($session->sessiondates, 'timestart', shezar_SEARCH_OP_LESS_THAN, $timenow);
    $unfinishedsessions = shezar_search_for_value($session->sessiondates, 'timefinish', shezar_SEARCH_OP_GREATER_THAN, $timenow);
    ...
    foreach ($sessions as $session) {
        ...
        if (!empty($sessionsignups[$session->id])) {
            foreach ($sessionsignups[$session->id] as $attendee) {
                ...
                $selectjobassignmentonsignupglobal = get_config(null, 'facetoface_selectjobassignmentonsignupglobal');
                if (!empty($selectjobassignmentonsignupglobal)) {
                    $jobassignment = \shezar_job\job_assignment::get_with_id($attendee->jobassignmentid);
 ...
 * @param \shezar_job\job_assignment $jobassignment object containing the selected job assignment
                ...
                if ($facetoface->approvaltype == APPROVAL_ADMIN && $currentstatus == MDL_F2F_STATUS_REQUESTED) {
                    ...
                    if (facetoface_session_has_capacity($session, $context)) {
                        ...
                        if ($session->allowoverbook) {
                            ...
                            shezar_set_notification(get_string('error:cannotapprovefull', 'facetoface'), $url);
                        }
                    }
...
function facetoface_print_session($session, $showcapacity, $calendaroutput=false, $return=true, $hidesignup=false, $class='f2f') {
    ...
    if (get_config(null, 'facetoface_selectjobassignmentonsignupglobal') &&
        ...
        if (isset($session->bookedsession->jobassignmentid) && $session->bookedsession->jobassignmentid) {
            $jobassignment = \shezar_job\job_assignment::get_with_id($session->bookedsession->jobassignmentid);
            $output .= html_writer::empty_tag('br');
    ...
    if ($facetoface->approvaltype != APPROVAL_NONE && $facetoface->approvaltype != APPROVAL_SELF) {
        ...
        if (isset($session->bookedsession->managerid) && $session->bookedsession->managerid) {
            ...
            if (!isset($session->managerids)) {
                $session->managerids   = \shezar_job\job_assignment::get_all_manager_userids($USER->id);
            }
...
function facetoface_get_customfielddata($sessionid) {
    ...
    foreach($types as $type => $def) {
        ...
        foreach ($fields as $field) {
            require_once($CFG->dirroot.'/shezar/customfield/field/'.$field->datatype.'/field.class.php');
...
class facetoface_event_handler {
     ...
     * when there is limited capacity.
     *
     * @param \shezar_core\event\user_suspended $event
     */
    public static function user_suspended(\shezar_core\event\user_suspended $event) {
        global $DB;
...
function facetoface_user_import($course, $facetoface, $session, $userid, $params = array()) {
    ...
    if (!empty($selectjobassignmentonsignupglobal) && !empty($facetoface->selectjobassignmentonsignup)) {
        ...
        if (!empty($params['jobassignmentid'])) {
            $jobassignment = \shezar_job\job_assignment::get_with_id($params['jobassignmentid']);
...
/**
 * Sets shezar_set_notification message describing bulk import results
 * @param array $results
 * @param string $type
 */
function facetoface_set_bulk_result_notification($results, $type = 'bulkadd') {
    ...
    if ($result_message != '') {
        shezar_set_notification($result_message, null, array('class' => $noticeclass));
    }
}
/**
...
function facetoface_can_reserve_or_allocate($facetoface, $sessions, $context, $managerid = null) {
    ...
    if (!\shezar_job\job_assignment::has_staff($managerid)) {
...
function facetoface_get_staff_to_allocate($facetoface, $session, $managerid = null) {
    ...
    $staff = \shezar_job\job_assignment::get_staff_userids($managerid);
    if (empty($staff)) {
        return $ret;
    }
    ...
    foreach ($staff as $member) {
        ...
        $usersignups = shezar_search_for_value($users, 'id', shezar_SEARCH_OP_EQUAL, $member);

        // Get signup for this user in this session (if exists).
        $usersignupsession = shezar_search_for_value($usersignups, 'sessionid', shezar_SEARCH_OP_EQUAL, $session->id);
...
function facetoface_get_session_managers($userid, $sessionid, $jobassignmentid = null) {
    ...
    if ($managerselect && !empty($signup->managerid)) {
        ...
        $ja = \shezar_job\job_assignment::get_with_id($jobassignmentid);
        if ($ja->managerid) {
            $managers[] = $DB->get_record('user', array('id' => $ja->managerid));
        }
    } else {
        $managerids = \shezar_job\job_assignment::get_all_manager_userids($userid);
        if (!empty($managerids)) {
