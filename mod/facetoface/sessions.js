...
M.shezar_f2f_room = M.shezar_f2f_room || {
    ...
    init: function(Y, config){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.shezar_f2f_room.init()-> jQuery dependency required for this module to function.');
        }
    ...
    init_dates: function() {
        ...
        $('.show-selectdate-dialog').each(function() {
            ...
            // Date dialog & handler.
            var handler = new shezarDialog_handler_form();
            ...
            shezarDialogs['selectdate'+offset] = new shezarDialog(
                ...
                function() {
                    return url + 'room/ajax/sessiondates.php?sessiondateid=' + $('input[name="sessiondateid[' + offset + ']"]').val() +
                        '&facetofaceid=' + M.shezar_f2f_room.config.facetofaceid +
    ...
    init_rooms: function() {
        ...
        $('.show-selectroom-dialog').each(function() {
            ...
            var handler = new shezarDialog_handler_treeview_singleselect('roomid[' + offset + ']', 'roomname' + offset);
                        ...
                        function(elem) {
                            ...
                            if (Number(elem.custom) > 0) {
                                ...
                                $editbutton.click(function(e) {
                                    e.preventDefault();
                                    M.shezar_f2f_room.config.editroom = elem.id;
                                    shezarDialogs['editcustomroom' + offset].config.title = '<h2>' + M.util.get_string('editroom', 'facetoface') + '</h2>';
                                    shezarDialogs['editcustomroom' + offset].open();
                                });
            ...
            var editcustomroomhandler = new shezarDialog_handler_form();
            editcustomroomhandler.every_load = function() {
                shezarDialog_handler_form.prototype.every_load.call(this);
                handler._dialog.hide();
            };
            // Change behaviour of update function.
            ...
            shezarDialogs['editcustomroom' + offset] = new shezarDialog(
                ...
                function() {
                    var id = 0;
                    // Store id in M.shezar_f2f_room.config for now to allow edit custom rooms.
                    if (typeof M.shezar_f2f_room.config.editroom !== "undefined") {
                        id = Number(M.shezar_f2f_room.config.editroom);
                        M.shezar_f2f_room.config.editroom = 0;
                    }
                    return url + 'room/ajax/room_edit.php?id=' + id + '&f=' + M.shezar_f2f_room.config.facetofaceid +
                        '&s=' + M.shezar_f2f_room.config.sessionid + '&sesskey=' + M.cfg.sesskey;
                },
    ...
    init_assets: function() {
        ...
        var render_asset_item = function(data, $input, offset) {
            ...
            require(['core/templates'], function (templates) {
                if (Number(data.custom) > 0) {
                    ...
                    $editbutton.click(function(e) {
                        e.preventDefault();
                        M.shezar_f2f_room.config.editasset = data.id;
                        shezarDialogs['editcustomasset' + offset].config.title = '<h2>' + M.util.get_string('editasset', 'facetoface') + '</h2>';
                        shezarDialogs['editcustomasset' + offset].open();
                    });
        ...
        $('.show-selectassets-dialog').each(function() {
            ...
            // Create new room dialog handler.
            var editcustomassethandler = new shezarDialog_handler_form();
            ...
            shezarDialogs['editcustomasset' + offset] = new shezarDialog(
                ...
                function() {
                    var id = 0;
                    // Store id in M.shezar_f2f_room.config for now to allow edit custom assets.
                    if (typeof M.shezar_f2f_room.config.editasset !== "undefined") {
                        id = Number(M.shezar_f2f_room.config.editasset);
                        M.shezar_f2f_room.config.editasset = 0;
                    }
                    return url + 'asset/ajax/asset_edit.php?id=' + id + '&f=' + M.shezar_f2f_room.config.facetofaceid +
                        '&s=' + M.shezar_f2f_room.config.sessionid + '&sesskey=' + M.cfg.sesskey;
                },
            ...
            shezarDialogs['selectassets'+offset] = new shezarDialog(
                ...
                function() {
                    return url + 'asset/ajax/sessionassets.php?sessionid=' + M.shezar_f2f_room.config.sessionid +
                        '&facetofaceid=' + M.shezar_f2f_room.config.facetofaceid +
                        '&timestart=' + $('input[name="timestart[' + offset + ']"]').val() +
                        '&timefinish=' + $('input[name="timefinish[' + offset + ']"]').val() +
