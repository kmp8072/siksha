...
if ($do_show == 'showoneentry') {
    ...
    if (is_array($feedbackitems)) {
        ...
        $completed=$feedbackcompleted->id;

        $query="select tagid from {tag_instance} where itemtype='feedback' and itemid=$instanceid and contextid=$contextid";
                $result=$DB->get_record_sql($query);
                $tagid=$result->tagid;

                if ($tagid==1) {

                 $query1="select ispresent from {feedback_tracking} where feedback=$instanceid and userid=$userid and completed=$completed";
			        $result1=$DB->get_record_sql($query1);
			        $ispresent=$result1->ispresent;
			        ...
			        <div class="container">
                       <div class="row">
                       	...
                       	<label class="control-label" for="attendance">Attendance : <?php echo $attendance_status;?></label>
...
<style>
.path-mod-feedback .boxalign_left{display:flex;}
/*.box, .generalbox, .boxalign_left {display: flex;}*/
/*.feedback_item_number_left {width: 100%;}
.feedback_item_label_left {width: 55%;}
.generalbox, .boxalignleft {width: 100%;}*/
</style>
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$(".feedback_item_label_left").each(function() {
